...
const data=await res.json();if(!data||typeof data!=="object")throw new Error("Invalid server response");if(!res.ok){return Object.assign({},data,{ok:false,status:Number(res.status||0),backendUnavailable:res.status===429||res.status>=500,error:data.error||`HTTP ${res.status || "error"}`});}
return data;}catch(err){if(timer)window.clearTimeout(timer);if(i<tries-1){await new Promise((resolve)=>window.setTimeout(resolve,280+i*320));continue;}
...
function fetchCommentsForPath(path,opts){const key=`${path}|${getVisitorId()}`;const now=Date.now();if(commentsMemo.key===key){if(commentsMemo.pending)return commentsMemo.pending;if(!(opts&&opts.fresh)&&commentsMemo.data&&now-commentsMemo.at<COMMENTS_MEMO_MS){return Promise.resolve(commentsMemo.data);}}
const pending=apiGet(`/comments?path=${encodeURIComponent(path)}&visitorId=${encodeURIComponent(getVisitorId())}`).then((data)=>{if(commentsMemo.key===key&&commentsMemo.pending===pending){commentsMemo.pending=null;if(data&&Array.isArray(data.comments)){commentsMemo.at=Date.now();commentsMemo.data=data;}}
...
const commentListData=new WeakMap();async function loadCommentsInto(list,opts){const options=opts&&typeof opts==="object"?opts:{};const path=currentPath();if(!options.silent&&!list.children.length){list.appendChild(el("div","mk-comments-loading","Loading comments..."));}
const key=`${path}:${getVisitorId()}`;let cached=commentListData.get(list);if(!cached||cached.key!==key||cached.epoch!==commentsMemoEpoch){cached={key,epoch:commentsMemoEpoch,data:null,pending:null};commentListData.set(list,cached);}
let data=options.reuseData?cached.data:null;if(!data){if(!cached.pending)cached.pending=fetchCommentsForPath(path,{fresh:true}).finally(()=>{cached.pending=null;});data=await cached.pending;if(data&&Array.isArray(data.comments))cached.data=data;}
if(path!==currentPath()||commentListData.get(list)!==cached||cached.epoch!==commentsMemoEpoch)return;if(!data||!Array.isArray(data.comments)){if(!options.silent&&!options.keepExistingOnError&&!list.querySelector(".mk-comment-card")){list.replaceChildren(el("div","mk-comments-empty","Could not load comments."));}
return;}
