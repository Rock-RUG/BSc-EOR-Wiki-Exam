...
modal.tabIndex=-1;const isTop=()=>{const dialogs=Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]'));return dialogs[dialogs.length-1]===modal;};const focusable=()=>Array.from(modal.querySelectorAll('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter((node)=>node.getClientRects().length&&!node.closest('[hidden]'));const onKey=(event)=>{if(event.isComposing||event.keyCode===229)return;if(!isTop())return;if(event.key==="Escape"){event.preventDefault();event.stopPropagation();close();return;}
if(event.key!=="Tab")return;const items=focusable();const first=items[0]||modal,last=items[items.length-1]||modal;if(!modal.contains(document.activeElement)||(event.shiftKey?document.activeElement===first:document.activeElement===last)||!items.length){event.preventDefault();(event.shiftKey?last:first).focus();}};const onFocus=(event)=>{if(isTop()&&!modal.contains(event.target))(focusable()[0]||modal).focus();};document.addEventListener("keydown",onKey,true);document.addEventListener("focusin",onFocus,true);window.requestAnimationFrame(()=>{if(modal.isConnected&&isTop()&&!modal.contains(document.activeElement))(focusable()[0]||modal).focus();});return()=>{document.removeEventListener("keydown",onKey,true);document.removeEventListener("focusin",onFocus,true);const canRestore=(node)=>node&&node.isConnected&&!node.disabled&&typeof node.focus==="function"&&node.getClientRects().length&&!node.closest('[hidden],[inert]')&&(typeof window.getComputedStyle!=="function"||window.getComputedStyle(node).visibility==="visible");const target=canRestore(previous)?previous:(previousId?document.getElementById(previousId):null);if(canRestore(target))target.focus();};}
...
function renderPrivacySettings(host){if(!host)return;host.innerHTML="";const profile=readLocalProfile();const accountContext=captureSocialAccountContext();const wrap=el("div","mk-privacy-card");let draftRevision=0;let saving=false;const formIsMounted=()=>wrap.isConnected&&wrap.parentNode===host;const markEdited=()=>{draftRevision+=1;};wrap.appendChild(el("div","mk-local-activity-meta",profile.accountKey?"Saved to your cloud profile. Connections only means accepted study connections can see it.":"Save or connect a username first. Until then, activity stays private and local."));const p=normalisePrivacy(profile.privacy,profile.isPublic);const list=el("div","mk-privacy-list");list.appendChild(privacyToggle("Profile name & avatar, including intro","Who can open your profile card. Your avatar and intro appear on public rankings only when this is Public.","profilePublic",p));list.appendChild(privacyToggle("User rankings","Public lists your username, XP, quiz results, lifetime shop spending and mastered-concept count. Avatar and intro also require the Profile setting to be Public.","rankingPublic",p));list.appendChild(privacyToggle("Visit history","Who can see recently visited concepts.","visitsPublic",p));list.appendChild(privacyToggle("Saved pages","Who can see Favourites, Study later, and Review later.","actionsPublic",p));list.appendChild(privacyToggle("Comments","Who can see recent comments on your profile.","commentsPublic",p));list.appendChild(privacyToggle("Readiness","Whether readiness contributes to public averages and your profile.","readinessPublic",p));list.querySelectorAll("select[data-privacy-key]").forEach(input=>{input.addEventListener("input",markEdited);input.addEventListener("change",markEdited);});wrap.appendChild(list);const quick=el("div","mk-local-profile-actions mk-privacy-actions");const allPub=el("button","mk-privacy-action-btn","All public");const allConn=el("button","mk-privacy-action-btn","Connections only");const allPriv=el("button","mk-privacy-action-btn","All private");const save=el("button","mk-privacy-action-btn","Save settings");allPub.type=allConn.type=allPriv.type=save.type="button";const applyPreset=value=>{markEdited();applyPrivacyToForm(wrap,defaultPrivacy(value));};allPub.addEventListener("click",()=>applyPreset(true));allConn.addEventListener("click",()=>applyPreset("connections"));allPriv.addEventListener("click",()=>applyPreset(false));save.addEventListener("click",async()=>{if(!formIsMounted()||saving)return;if(!socialAccountContextIsCurrent(accountContext)){renderPrivacySettings(host);return;}
const next=readPrivacyForm(wrap);const submittedRevision=++draftRevision;saving=true;save.disabled=true;let res;try{res=await savePrivacySettings(next);}
catch(_){res={ok:false,error:"Could not save privacy settings. Please try again."};}
finally{saving=false;save.disabled=false;}
if(!formIsMounted())return;if(!socialAccountContextIsCurrent(accountContext)){renderPrivacySettings(host);return;}
if(!res||!res.ok){window.alert((res&&res.error)||"Could not save privacy settings. Save or connect a username first.");return;}
const hasNewerDraft=draftRevision!==submittedRevision;if(!hasNewerDraft)applyPrivacyToForm(wrap,res.privacy||next);window.alert(hasNewerDraft?"Privacy settings saved. Your newer changes are not saved yet.":"Privacy settings saved.");try{refreshReadinessCardSoon(100);}catch(_){}});quick.appendChild(allPub);quick.appendChild(allConn);quick.appendChild(allPriv);quick.appendChild(save);wrap.appendChild(quick);host.appendChild(wrap);const initialRevision=draftRevision;fetchPrivacySettings().then((res)=>{if(!formIsMounted()||draftRevision!==initialRevision||!socialAccountContextIsCurrent(accountContext))return;if(res&&res.ok&&res.privacy)applyPrivacyToForm(wrap,res.privacy);}).catch(()=>{});}
...
const modal=document.createElement("div");modal.className="mk-local-activity-modal mk-local-activity-modal--account";modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.innerHTML=`
