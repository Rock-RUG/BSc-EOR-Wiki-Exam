...
window.alert("The account has been deleted. Existing comments now show as Deleted account.");try{window.location.reload();}catch(_){refreshInfo();}}finally{deleteBtn.disabled=!readLocalProfile().accountKey;}});host.appendChild(wrap);}
...
cloudSyncRenderTimer=window.setTimeout(()=>{cloudSyncRenderTimer=0;const active=(tabs.find((b)=>b.classList.contains("is-active"))||{}).dataset;const t=active&&active.type||body.dataset.type||initial;if(t!=="info"&&t!=="privacy"&&t!=="connections"&&t!=="notifications")renderLocalActivityList(body,t);},350);};window.addEventListener("mk-local-activity-change",refreshOnCloudSync);let releaseFocus=()=>{};let didClose=false;const close=()=>{if(didClose)return;didClose=true;try{if(window.__mkAccountSyncCountdownTimer)window.clearInterval(window.__mkAccountSyncCountdownTimer);window.__mkAccountSyncCountdownTimer=0;}catch(_){}
