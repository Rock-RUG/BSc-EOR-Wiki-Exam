...
async function loadOnlineProfile(){if(identityProfileUpdateBusy)return readLocalProfile();const cached=readLocalProfile();const profileBefore=JSON.stringify(cached);const accountContext=captureSocialAccountContext();if(isMobilePowerSensitiveViewport()&&cached&&cached.accountKey){const last=readLastOnlineProfileAt();if(last&&Date.now()-last<5*60*1000&&!identityIntentMatchesContext(readIdentityRekeyIntent(),accountContext))return cached;}
const data=await apiGet(`/identity?visitorId=${encodeURIComponent(getVisitorId())}`);if(!socialAccountContextIsCurrent(accountContext))return readLocalProfile();if(JSON.stringify(readLocalProfile())!==profileBefore)return readLocalProfile();if(data&&data.ok&&data.profile){const applied=await applyOnlineIdentityProfile(data,accountContext);if(applied.ok){touchOnlineProfileRefresh();return applied.profile;}
return readLocalProfile();}
if(data&&data.ok&&!data.profile){clearIdentityRekeyIntentForContext(accountContext);if(!accountContext.accountKey)setIdentityRecoveryStatus("",accountContext);else identityProfileFailure("This browser's cloud identity needs reconnecting. Your local progress is preserved; connect the intended account in Account.",accountContext,false);}
...
const IDENTITY_REKEY_INTENT_KEY="mk_account_identity_rekey_pending_v1";let identityProfileUpdateBusy=false;function readIdentityRekeyIntent(){try{const value=JSON.parse(localStorage.getItem(IDENTITY_REKEY_INTENT_KEY)||"null");return value&&typeof value==="object"&&value.oldOwner&&value.targetOwner&&value.visitorId?value:null;}catch(_){return null;}}
...
async function applyOnlineIdentityProfile(response,context){if(identityProfileUpdateBusy)return{ok:false,busy:true,error:"An account update is already in progress."};identityProfileUpdateBusy=true;try{return await persistOnlineIdentityProfile(response,context);}
finally{identityProfileUpdateBusy=false;}}
async function saveOnlineProfile(name,avatar,options){if(identityProfileUpdateBusy)return{ok:false,busy:true,error:"An account update is already in progress."};identityProfileUpdateBusy=true;try{return await saveOnlineProfileNow(name,avatar,options);}
finally{identityProfileUpdateBusy=false;}}
...
if(res&&res.ok===false&&!res.backendUnavailable&&Number(res.status||0)>=400&&Number(res.status||0)<500&&Number(res.status)!==429)clearIdentityRekeyIntentForContext(accountContext);return Object.assign({ok:false,error:(res&&res.error)||"Could not save this username."},res||{});}
...
const mini=openLocalMiniModal("Change username");const input=document.createElement("input");input.type="text";input.maxLength=40;input.autocomplete="nickname";input.value=curr.name||"";input.placeholder="Username";const actions=el("div","mk-local-mini-actions");const cancel=iconButton("mk-comment-small-btn","x","Cancel");const save=iconButton("mk-comment-primary-btn","check","Save username");cancel.type=save.type="button";actions.appendChild(cancel);actions.appendChild(save);mini.body.appendChild(input);mini.body.appendChild(actions);cancel.addEventListener("click",mini.close);save.addEventListener("click",async()=>{if(!socialAccountContextIsCurrent(accountContext)){mini.close();refreshInfo();return;}
