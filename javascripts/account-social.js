...
async function persistOnlineIdentityProfile(response,context){if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();const profile=responseProfileWithPrivacy(response);const key=String(profile&&profile.accountKey||"").trim().toLowerCase();if(!key)return{ok:false,error:"The server did not return an account identity."};const workspaces=window.MkAccountWorkspaces;const targetOwner=workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(key):"";const ownerChanged=targetOwner?targetOwner!==context.owner:key!==context.accountKey;const pending=readIdentityRekeyIntent();if(!ownerChanged&&identityIntentMatchesContext(pending,context))return pendingIdentityRecovery(pending,context);let saved;if(ownerChanged){const intent=readIdentityRekeyIntent();if(!identityIntentMatchesContext(intent,context)||intent.targetOwner!==targetOwner||!workspaces||typeof workspaces.rekeyActiveWorkspace!=="function"){return identityProfileFailure("Account identity needs recovery. Your local progress is preserved; connect the intended account in Account before syncing.",context,false);}
...
async function refreshLocalProfileFromServerNow(){const accountContext=captureSocialAccountContext();const profileBefore=JSON.stringify(readLocalProfile());const data=await apiGet(`/identity?visitorId=${encodeURIComponent(getVisitorId())}&t=${Date.now()}`);if(!socialAccountContextIsCurrent(accountContext))return readLocalProfile();if(JSON.stringify(readLocalProfile())!==profileBefore)return readLocalProfile();if(data&&data.ok&&data.profile){const applied=await persistOnlineIdentityProfile(data,accountContext);if(applied.ok){touchOnlineProfileRefresh();return applied.profile;}
return readLocalProfile();}
if(data&&data.ok&&!data.profile){const pending=readIdentityRekeyIntent();if(identityIntentMatchesContext(pending,accountContext))pendingIdentityRecovery(pending,accountContext);else if(!accountContext.accountKey)setIdentityRecoveryStatus("",accountContext);else identityProfileFailure("This browser's cloud identity needs reconnecting. Your local progress is preserved; connect the intended account in Account.",accountContext,false);}
if(data&&data.revoked){const cleared=await clearLocalAccountState({clearActivity:false});return cleared&&cleared.ok?cleared.profile:readLocalProfile();}
return readLocalProfile();}
...
let accountDialogSequence=0;function bindAccountDialogFocus(modal,close,titleSelector){const previous=document.activeElement;const title=modal.querySelector(titleSelector);const id=`mk-account-dialog-${++accountDialogSequence}`;if(title){title.id=`${id}-title`;modal.setAttribute("aria-labelledby",title.id);}
modal.tabIndex=-1;const isTop=()=>{const dialogs=Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]'));return dialogs[dialogs.length-1]===modal;};const focusable=()=>Array.from(modal.querySelectorAll('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter((node)=>node.getClientRects().length&&!node.closest('[hidden]'));const onKey=(event)=>{if(!isTop())return;if(event.key==="Escape"){event.preventDefault();event.stopPropagation();close();return;}
if(event.key!=="Tab")return;const items=focusable();const first=items[0]||modal,last=items[items.length-1]||modal;if(!modal.contains(document.activeElement)||(event.shiftKey?document.activeElement===first:document.activeElement===last)||!items.length){event.preventDefault();(event.shiftKey?last:first).focus();}};const onFocus=(event)=>{if(isTop()&&!modal.contains(event.target))(focusable()[0]||modal).focus();};document.addEventListener("keydown",onKey,true);document.addEventListener("focusin",onFocus,true);window.requestAnimationFrame(()=>{if(modal.isConnected&&isTop()&&!modal.contains(document.activeElement))(focusable()[0]||modal).focus();});return()=>{document.removeEventListener("keydown",onKey,true);document.removeEventListener("focusin",onFocus,true);if(previous&&previous.isConnected&&typeof previous.focus==="function")previous.focus();};}
