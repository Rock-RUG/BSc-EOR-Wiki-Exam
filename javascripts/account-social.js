...
let __localSyncQueueFlushTimer=0;let __localSyncQueueFlushAt=0;let __localSyncQueueFlushPromise=null;let __localSyncQueueFlushContext=null;function pageActionResponseIsValid(data){return!!data&&data.ok===true&&!!data.actions&&typeof data.actions==="object"&&!Array.isArray(data.actions)&&savedPageActions().every((action)=>!Object.prototype.hasOwnProperty.call(data.actions,action)||typeof data.actions[action]==="boolean");}
async function flushPageActionSetTask(item){const context=captureSocialAccountContext();if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();const body=item&&item.body&&typeof item.body==="object"?item.body:{};const path=String(body.path||"");const action=String(body.action||"");const desired=!!body.active;if(!path||!action)return{ok:true,skipped:true};const state=await apiGet(`/page-state?path=${encodeURIComponent(path)}&visitorId=${encodeURIComponent(body.visitorId || getVisitorId())}`);if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(!pageActionResponseIsValid(state))return{ok:false,error:state&&state.error||"page_action_state_unavailable"};const serverActions=state.actions;if(!!serverActions[action]===desired)return{ok:true,already:true};const res=await apiPost("/page-action/toggle",{path,title:body.title||path,visitorId:body.visitorId||getVisitorId(),action});if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(!res||res.ok===false)return res||{ok:false,error:"page_action_sync_failed"};if(!pageActionResponseIsValid(res))return{ok:false,error:"page_action_response_invalid"};const nextActions=res.actions;if(!!nextActions[action]!==desired)return{ok:false,error:"page_action_state_mismatch"};return res;}
...
async function flushLocalSyncQueue(opts){const options=opts&&typeof opts==="object"?opts:{};const force=!!options.force;const context=captureSocialAccountContext();if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(!force&&shouldSkipMobileBackgroundWork(options.reason||"local-sync-queue"))return{ok:true,skipped:true,lowHeat:true};if(pageIsHiddenForAccountSync()&&!force)return null;if(__localSyncQueueFlushPromise&&socialAccountContextIsCurrent(__localSyncQueueFlushContext))return __localSyncQueueFlushPromise;const run=(async()=>{const now=Date.now();const lock=readJsonLocal(LOCAL_SYNC_QUEUE_LOCK_KEY,null);if(!force&&lock&&Number(lock.until||0)>now)return{ok:true,locked:true};writeJsonLocal(LOCAL_SYNC_QUEUE_LOCK_KEY,{until:now+25000,startedAt:now});const q=compactLocalSyncQueue(localSyncQueueRead());if(!q.length){writeNumberLocal(LOCAL_SYNC_QUEUE_LAST_KEY,now);try{localStorage.removeItem(LOCAL_SYNC_QUEUE_LOCK_KEY);}catch(_){}return{ok:true,empty:true};}
const completedVersions=new Set();const failedVersions=new Map();let sent=0;const maxPerFlush=force?80:28;for(let i=0;i<q.length;i+=1){const item=q[i];if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(sent>=maxPerFlush)continue;try{const res=await flushOneLocalSyncTask(item);if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(res&&res.ok!==false){sent+=1;completedVersions.add(JSON.stringify(item));}
else failedVersions.set(JSON.stringify(item),Object.assign({},item,{attempts:Math.max(0,Number(item.attempts||0))+1,lastError:String(res&&res.error||"sync_failed").slice(0,160),updatedAt:Date.now()}));}catch(err){if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();failedVersions.set(JSON.stringify(item),Object.assign({},item,{attempts:Math.max(0,Number(item.attempts||0))+1,lastError:String(err&&err.message||err||"sync_failed").slice(0,160),updatedAt:Date.now()}));}}
if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();const remaining=compactLocalSyncQueue(localSyncQueueRead()).filter((item)=>!completedVersions.has(JSON.stringify(item))).map((item)=>failedVersions.get(JSON.stringify(item))||item);if(!localSyncQueueWrite(remaining)){try{localStorage.removeItem(LOCAL_SYNC_QUEUE_LOCK_KEY);}catch(_){}
scheduleLocalSyncQueueFlush(LOCAL_SYNC_QUEUE_INTERVAL_MS);return{ok:false,sent,remaining:localSyncQueueSize(),error:"local_sync_queue_ack_failed"};}
writeNumberLocal(LOCAL_SYNC_QUEUE_LAST_KEY,Date.now());try{localStorage.removeItem(LOCAL_SYNC_QUEUE_LOCK_KEY);}catch(_){}
...
if(localSyncQueueSize()>0)scheduleLocalSyncQueueFlush(LOCAL_SYNC_QUEUE_INTERVAL_MS);return{ok:true,sent,remaining:localSyncQueueSize()};})();__localSyncQueueFlushPromise=run;__localSyncQueueFlushContext=context;try{return await run;}finally{if(__localSyncQueueFlushPromise===run){__localSyncQueueFlushPromise=null;__localSyncQueueFlushContext=null;}}}
...
async function clearLocalAccountState(options){const opts=options&&typeof options==="object"?options:{};const accountContext=captureSocialAccountContext();const workspaces=window.MkAccountWorkspaces||null;if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!workspaces||typeof workspaces.activateGuest!=="function")return identityProfileFailure("Account storage is not ready. Reload before removing this account from the browser.",accountContext,false);let result;window.__mkAccountWorkspaceSwitching=true;try{result=await workspaces.activateGuest({expectedOwner:accountContext.owner,discardCurrent:!!opts.clearActivity,reason:opts.clearActivity?"account-local-delete":"account-logout",commit:()=>{for(const key of[LOCAL_PROFILE_KEY,"mk_comment_name_v1","mk_hot_visitor_id_v1"]){localStorage.removeItem(key);if(localStorage.getItem(key)!==null)throw new Error("Could not remove the local account identity.");}
for(const key of[CLOUD_SYNC_LAST_KEY,ACCOUNT_JSON_SYNC_LAST_SUMMARY_KEY,ACCOUNT_SYNC_LAST_RESULT_KEY,ACCOUNT_DATA_SYNC_SUMMARY_KEY]){try{localStorage.removeItem(key);}catch(_){}}
return readLocalProfile();}});}catch(error){result={ok:false,error:String(error&&error.message||error)};}finally{window.__mkAccountWorkspaceSwitching=false;}
if(!result||!result.ok){return identityProfileFailure("This browser could not finish removing the account. Keep its local data and retry. "+String(result&&result.error||""),accountContext,false);}
...
return{ok:true,profile:result.commitResult||readLocalProfile()};}
function publicAvatarLocalUrl(avatar){const raw=String(avatar||"").trim();const avatarPathUrl=(key,base)=>{try{const safe=String(key||"").split("/").map((part)=>encodeURIComponent(part)).join("/");return new URL(`/avatar/${safe}`,base||API_BASE).toString();}
...
function emojiFamilyOwns(entMap,codepoint){if(!entMap)return false;const sharedOwns=window.MkEC&&window.MkEC.emojiEntitlementFamilyOwns;if(typeof sharedOwns==="function")return sharedOwns(entMap,codepoint);const cp=String(codepoint||"").toLowerCase();if(cp&&entMap[cp])return true;return emojiFamilyCodepoints(cp).some((c)=>!!entMap[c]);}
...
function initialAvatarStyleInventory(){try{const api=window.MkAccountData||null;return api&&typeof api.getShopInventory==="function"?api.getShopInventory():{owned:{}};}catch(_){return{owned:{}};}}
function initialAvatarStyleOwned(type,id,inventory){const def=type==="font"?initialAvatarFontDef(id):(type==="text"?initialAvatarTextColorDef(id):(type==="scale"?initialAvatarScaleDef(id):initialAvatarColorDef(id)));if(!def.price)return true;const inv=inventory||initialAvatarStyleInventory();return!!(inv&&inv.owned&&inv.owned[initialAvatarStyleItemId(type,def.id)]);}
...
async function loadOnlineProfile(){const result=await withIdentityProfileUpdate(loadOnlineProfileNow);return result&&result.ok===false?readLocalProfile():result;}
async function loadOnlineProfileNow(){const cached=readLocalProfile();const profileBefore=JSON.stringify(cached);const accountContext=captureSocialAccountContext();if(isMobilePowerSensitiveViewport()&&cached&&cached.accountKey){const last=readLastOnlineProfileAt();if(last&&Date.now()-last<5*60*1000&&!identityIntentMatchesContext(readIdentityRekeyIntent(),accountContext))return cached;}
const data=await fetchFreshIdentityProfile(accountContext);if(!socialAccountContextIsCurrent(accountContext))return readLocalProfile();if(JSON.stringify(readLocalProfile())!==profileBefore)return readLocalProfile();if(data&&data.ok&&data.profile){const applied=await persistOnlineIdentityProfile(data,accountContext);if(applied.ok){touchOnlineProfileRefresh();return applied.profile;}
return readLocalProfile();}
if(data&&data.ok&&!data.profile){const pending=readIdentityRekeyIntent();if(identityIntentMatchesContext(pending,accountContext))pendingIdentityRecovery(pending,accountContext);else if(!accountContext.accountKey)setIdentityRecoveryStatus("",accountContext);else identityProfileFailure("This browser's cloud identity needs reconnecting. Your local progress is preserved; connect the intended account in Account.",accountContext,false);}
if(data&&data.revoked){const cleared=await clearLocalAccountState({clearActivity:false});return cleared&&cleared.ok?cleared.profile:readLocalProfile();}
return readLocalProfile();}
...
const IDENTITY_REKEY_INTENT_KEY="mk_account_identity_rekey_pending_v1";let identityProfileUpdateBusy=false;const freshIdentityProfiles=new WeakSet();async function fetchFreshIdentityProfile(context){if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();const workspaces=window.MkAccountWorkspaces;if(workspaces&&typeof workspaces.ownerForAccount==="function"&&context.owner!==workspaces.ownerForAccount(context.accountKey))return socialAccountChangedResult();const path=`/identity?visitorId=${encodeURIComponent(context.visitorId)}&previousAccountKey=${encodeURIComponent(context.accountKey)}&t=${Date.now()}`;const data=await fetchJsonWithRetry(API_BASE+path,{cache:"no-store",timeoutMs:ACCOUNT_SYNC_FETCH_TIMEOUT_MS,accountContext:context},2);if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(data&&data.ok&&data.profile)freshIdentityProfiles.add(data);return data;}
async function refreshAccountIdentityForSync(){return withIdentityProfileUpdate(async()=>{const context=captureSocialAccountContext();if(!context.accountKey)return{ok:false,error:"Connect or save a username first."};const before=JSON.stringify(readLocalProfile());const data=await fetchFreshIdentityProfile(context);if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();if(JSON.stringify(readLocalProfile())!==before)return{ok:false,skipped:true,error:"profile_changed"};if(!data||!data.ok||!data.profile){return identityProfileFailure(data&&data.error||"This browser's cloud identity could not be confirmed. Your local progress is preserved; retry syncing when the account is available.",context,false);}
const applied=await persistOnlineIdentityProfile(data,context);if(!applied.ok)return applied;touchOnlineProfileRefresh();return Object.assign({},applied,{accountContext:captureSocialAccountContext()});});}
window.MkAccountIdentity={refreshForSync:refreshAccountIdentityForSync};async function withIdentityProfileUpdate(action){const busy={ok:false,busy:true,error:"An account update is already in progress. Please try again shortly."};if(identityProfileUpdateBusy)return busy;const locks=window.navigator&&window.navigator.locks;if(!locks||typeof locks.request!=='function')return{ok:false,error:"This browser cannot safely coordinate account updates. Use a browser with Web Locks support; your local progress is preserved."};const context=captureSocialAccountContext();return locks.request('mk-account-identity-profile-v1',{ifAvailable:true},async lock=>{if(!lock)return busy;if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();identityProfileUpdateBusy=true;try{return await action();}
finally{identityProfileUpdateBusy=false;}});}
function readIdentityRekeyIntent(){try{const value=JSON.parse(localStorage.getItem(IDENTITY_REKEY_INTENT_KEY)||"null");return value&&typeof value==="object"&&value.oldOwner&&value.targetOwner&&value.visitorId?value:null;}catch(_){return null;}}
function identityIntentMatchesContext(intent,context){return!!(intent&&context&&intent.oldOwner===context.owner&&intent.visitorId===context.visitorId);}
function clearIdentityRekeyIntentForContext(context){const intent=readIdentityRekeyIntent();if(identityIntentMatchesContext(intent,context)){try{localStorage.removeItem(IDENTITY_REKEY_INTENT_KEY);}catch(_){}}}
function setIdentityRecoveryStatus(error,context){window.__mkAccountIdentityRecoveryState=error?{error:String(error),context}:null;try{window.dispatchEvent(new CustomEvent("mk-account-identity-recovery"));}catch(_){}}
function identityProfileFailure(error,context,cloudSaved){const result={ok:false,localFailure:true,cloudSaved:!!cloudSaved,recoveryRequired:true,error:String(error)};setIdentityRecoveryStatus(result.error,context);return result;}
function pendingIdentityRecovery(intent,context){const name=String(intent.targetName||intent.targetOwner.replace(/^account:/,""));return identityProfileFailure('The previous account update is not confirmed yet. Keep the username "'+name+'" and select Save to retry it. Your local progress is preserved.',context,false);}
async function persistOnlineIdentityProfile(response,context){if(!socialAccountContextIsCurrent(context))return socialAccountChangedResult();const freshRead=!!(response&&freshIdentityProfiles.has(response));if(freshRead)freshIdentityProfiles.delete(response);const profile=responseProfileWithPrivacy(response);const key=String(profile&&profile.accountKey||"").trim().toLowerCase();if(!key)return{ok:false,error:"The server did not return an account identity."};const workspaces=window.MkAccountWorkspaces;const targetOwner=workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(key):"";const ownerChanged=targetOwner?targetOwner!==context.owner:key!==context.accountKey;const pending=readIdentityRekeyIntent();if(!ownerChanged&&identityIntentMatchesContext(pending,context))return pendingIdentityRecovery(pending,context);let saved;if(ownerChanged){let intent=readIdentityRekeyIntent();const proof=response&&response.identityRename;if(!intent&&context.accountKey&&freshRead&&proof&&proof.from===context.accountKey&&proof.to===key&&targetOwner&&workspaces&&typeof workspaces.rekeyActiveWorkspace==="function"){intent={oldOwner:context.owner,targetOwner,targetName:String(profile.name||key),visitorId:context.visitorId};try{const serialized=JSON.stringify(intent);localStorage.setItem(IDENTITY_REKEY_INTENT_KEY,serialized);if(localStorage.getItem(IDENTITY_REKEY_INTENT_KEY)!==serialized)throw new Error("Intent was not saved.");}catch(_){return identityProfileFailure("The cloud username changed, but this browser could not save its recovery record. Free browser storage and retry syncing; your local progress is preserved.",context,true);}}
if(!identityIntentMatchesContext(intent,context)||intent.targetOwner!==targetOwner||!workspaces||typeof workspaces.rekeyActiveWorkspace!=="function"){return identityProfileFailure("Account identity needs recovery. Your local progress is preserved; connect the intended account in Account before syncing.",context,false);}
const previousSwitching=window.__mkAccountWorkspaceSwitching;window.__mkAccountWorkspaceSwitching=true;let migration;try{migration=await workspaces.rekeyActiveWorkspace(targetOwner,{expectedOwner:context.owner,reason:"profile-identity-save",commit:()=>writeLocalProfile(profile,{requirePersistence:true})});}finally{window.__mkAccountWorkspaceSwitching=previousSwitching;}
if(!migration||!migration.ok){const message=migration&&migration.rollbackOk===false?"The cloud username was saved, but the local update and recovery failed. Keep this browser's data and use Account recovery before syncing. ":"The cloud username was saved, but this browser could not finish updating its local account. Local progress is preserved. Free browser storage if needed, then reload to retry recovery. ";return identityProfileFailure(message+String(migration&&migration.error||""),context,true);}
saved=migration.commitResult;}else{try{saved=writeLocalProfile(profile,{requirePersistence:true});}
catch(_){return identityProfileFailure("The cloud profile is available, but it could not be saved on this browser. Free browser storage and reload to retry.",context,true);}}
clearIdentityRekeyIntentForContext(context);setIdentityRecoveryStatus("",context);return{ok:true,profile:saved};}
async function applyOnlineIdentityProfile(response,context){return withIdentityProfileUpdate(()=>persistOnlineIdentityProfile(response,context));}
async function saveOnlineProfile(name,avatar,options){return withIdentityProfileUpdate(()=>saveOnlineProfileNow(name,avatar,options));}
async function saveOnlineProfileNow(name,avatar,options){const nm=cleanProfileNameLocal(name);const av=cleanAvatarLocal(avatar);const opts=options&&typeof options==="object"?options:{};if(!nm)return{ok:false,error:"Please choose a username first."};const currentProfile=readLocalProfile();const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const isNewAccount=!currentProfile.accountKey;if(isNewAccount){const consented=await requestAccountConsent();if(!consented)return{ok:false,cancelled:true,error:"Account creation was cancelled."};if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();}
...
if(opts.isPublic!=null)body.isPublic=opts.isPublic;if(opts.privacy)body.privacy=opts.privacy;const workspaces=window.MkAccountWorkspaces;const targetOwner=workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(nm):"";const pending=readIdentityRekeyIntent();const retryingPending=identityIntentMatchesContext(pending,accountContext);if(retryingPending){const verified=await apiGet(`/identity?visitorId=${encodeURIComponent(accountContext.visitorId)}&t=${Date.now()}`);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!verified||!verified.ok){return identityProfileFailure("The previous username change could not be verified. Your local progress is preserved. Check your connection and try Save again.",accountContext,false);}
const verifiedKey=String(verified.profile&&verified.profile.accountKey||"").trim().toLowerCase();if(verifiedKey&&verifiedKey!==accountContext.accountKey){const recovered=await persistOnlineIdentityProfile(verified,accountContext);if(!recovered.ok)return recovered;return{ok:false,retryRequired:true,error:"Your account identity has been recovered. Reopen the profile editor, review your details, then save your change again."};}
if(targetOwner!==pending.targetOwner||(!verifiedKey&&accountContext.accountKey))return pendingIdentityRecovery(pending,accountContext);}
if(targetOwner?targetOwner!==accountContext.owner:nm.toLowerCase()!==accountContext.accountKey){if(!workspaces||typeof workspaces.rekeyActiveWorkspace!=="function")return{ok:false,error:"Account storage is not ready. Reload before changing the username."};const preflight=await workspaces.rekeyActiveWorkspace(targetOwner,{expectedOwner:accountContext.owner,dryRun:true});if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!preflight||!preflight.ok)return{ok:false,error:String(preflight&&preflight.error||"The local account could not be prepared for this username.")};try{const serialized=retryingPending?JSON.stringify(pending):JSON.stringify({oldOwner:accountContext.owner,targetOwner,targetName:nm,visitorId:accountContext.visitorId});localStorage.setItem(IDENTITY_REKEY_INTENT_KEY,serialized);if(localStorage.getItem(IDENTITY_REKEY_INTENT_KEY)!==serialized)throw new Error("Intent was not saved.");}catch(_){return{ok:false,error:"The local recovery record could not be saved. Free browser storage before changing the username."};}}
const res=await apiPost("/identity",body);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(res&&res.ok&&res.profile){const out=await persistOnlineIdentityProfile(res,accountContext);if(!out.ok)return out;invalidateAccountXpCache();try{const oldBio=String(currentProfile.bio||"");const newBio=String(body.bio||"");if(hasBioOpt&&oldBio!==newBio)trackActivity("intro_update",{source:"profile-save",hadIntroBefore:!!oldBio,introLength:newBio.length},{scope:"intro_update",throttleMs:0});if(av||opts.keepAvatar){const oldAvatar=String(currentProfile.avatar||"");const newAvatar=String(res.profile&&res.profile.avatar||av||"");if(oldAvatar!==newAvatar)trackActivity("avatar_upload",{source:"profile-save",avatarMode:av?"emoji":"upload"},{scope:"avatar_upload",throttleMs:0});}
...
if(!retryingPending&&res&&res.ok===false&&res.renameNotApplied===true){clearIdentityRekeyIntentForContext(accountContext);setIdentityRecoveryStatus("",accountContext);}
return Object.assign({ok:false,error:(res&&res.error)||"Could not save this username."},res||{});}
async function saveAvatarFrameOnline(frame){const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const requested=cleanAvatarFrameLocal(frame||"level-1");const curr=readLocalProfile();if(!curr.name)return{ok:false,error:"Please choose a username before changing the avatar frame."};let next;try{next=writeLocalProfile(Object.assign({},curr,{avatarFrame:requested,selectedAvatarFrame:requested,highestLevelSeen:Math.max(Number(curr.highestLevelSeen||curr.level||1),avatarFrameLevelLocal(requested)),unlockedAvatarFrames:avatarFramesForLevelLocal(Math.max(Number(curr.highestLevelSeen||curr.level||1),avatarFrameLevelLocal(requested)))}),{requirePersistence:true});}
catch(_){return identityProfileFailure("This browser could not save the avatar frame. Free browser storage and try again.",accountContext,false);}
try{window.setTimeout(()=>{if(!socialAccountContextIsCurrent(accountContext))return;apiPost("/identity/avatar-frame",{visitorId:accountContext.visitorId,frame:requested,avatarFrame:requested}).catch(()=>null);},0);}catch(_){}
return{ok:true,localOnly:true,profile:next,avatarFrame:requested,unlockedAvatarFrames:next.unlockedAvatarFrames};}
async function refreshLocalProfileFromServer(){const result=await withIdentityProfileUpdate(refreshLocalProfileFromServerNow);return result&&result.ok===false?readLocalProfile():result;}
async function refreshLocalProfileFromServerNow(){const accountContext=captureSocialAccountContext();const profileBefore=JSON.stringify(readLocalProfile());const data=await fetchFreshIdentityProfile(accountContext);if(!socialAccountContextIsCurrent(accountContext))return readLocalProfile();if(JSON.stringify(readLocalProfile())!==profileBefore)return readLocalProfile();if(data&&data.ok&&data.profile){const applied=await persistOnlineIdentityProfile(data,accountContext);if(applied.ok){touchOnlineProfileRefresh();return applied.profile;}
return readLocalProfile();}
if(data&&data.ok&&!data.profile){const pending=readIdentityRekeyIntent();if(identityIntentMatchesContext(pending,accountContext))pendingIdentityRecovery(pending,accountContext);else if(!accountContext.accountKey)setIdentityRecoveryStatus("",accountContext);else identityProfileFailure("This browser's cloud identity needs reconnecting. Your local progress is preserved; connect the intended account in Account.",accountContext,false);}
if(data&&data.revoked){const cleared=await clearLocalAccountState({clearActivity:false});return cleared&&cleared.ok?cleared.profile:readLocalProfile();}
return readLocalProfile();}
async function applyLevelUpRewardsOnline(level){const accountContext=captureSocialAccountContext();const lvl=Math.max(1,Math.min(10,Math.floor(Number(level||1))));const res=await apiPost("/identity/level-up-rewards",{visitorId:getVisitorId(),level:lvl});if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(res&&res.ok&&res.profile)writeLocalProfile(res.profile);else if(res&&res.ok)await refreshLocalProfileFromServer().catch(()=>readLocalProfile());return res||{ok:false};}
...
async function fetchPrivacySettings(){const accountContext=captureSocialAccountContext();const profileBefore=JSON.stringify(readLocalProfile());const data=await apiGet(`/identity/privacy?visitorId=${encodeURIComponent(getVisitorId())}`);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(JSON.stringify(readLocalProfile())!==profileBefore)return{ok:false,skipped:true,error:"profile_changed"};if(data&&data.ok){const merged=mergeProfilePrivacy(data.profile||readLocalProfile(),data.privacy||(data.profile&&data.profile.privacy));try{writeLocalProfile(merged,{requirePersistence:true});}
catch(_){return identityProfileFailure("The cloud privacy settings could not be saved on this browser. Free browser storage and reload.",accountContext,true);}
setIdentityRecoveryStatus("",accountContext);}
return data;}
async function savePrivacySettings(privacy){const accountContext=captureSocialAccountContext();const nextPrivacy=normalisePrivacy(privacy,false);const data=await apiPost("/identity/privacy",{visitorId:getVisitorId(),privacy:nextPrivacy});if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(data&&data.ok){const merged=mergeProfilePrivacy(data.profile||readLocalProfile(),data.privacy||nextPrivacy);try{writeLocalProfile(merged,{requirePersistence:true});}
catch(_){return identityProfileFailure("Privacy settings were saved in the cloud, but this browser could not save them locally. Free browser storage and reload.",accountContext,true);}
setIdentityRecoveryStatus("",accountContext);try{trackActivity("privacy_update",{source:"privacy-settings"},{scope:"privacy_update",throttleMs:0});}catch(_){}
...
function captureSocialAccountContext(allowWorkspaceSwitch){const profile=readLocalProfile();const workspaces=window.MkAccountWorkspaces;return{accountKey:String(profile&&profile.accountKey||"").trim().toLowerCase(),visitorId:String(getVisitorId()||""),owner:workspaces&&typeof workspaces.activeOwner==="function"?workspaces.activeOwner():"",allowWorkspaceSwitch:!!allowWorkspaceSwitch};}
function socialAccountContextIsCurrent(context){if(!context||(!context.allowWorkspaceSwitch&&window.__mkAccountWorkspaceSwitching))return false;const current=captureSocialAccountContext(context.allowWorkspaceSwitch);return current.accountKey===context.accountKey&&current.visitorId===context.visitorId&&current.owner===context.owner;}
function socialAccountChangedResult(){return{ok:false,skipped:true,error:"account_workspace_changed"};}
async function fetchJsonWithRetry(url,init,attempts){const tries=Math.max(1,Number(attempts||1));const input=init&&typeof init==="object"?Object.assign({},init):{};const timeoutMs=Math.max(4000,Number(input.timeoutMs||input.__timeoutMs||ACCOUNT_SYNC_FETCH_TIMEOUT_MS)||ACCOUNT_SYNC_FETCH_TIMEOUT_MS);const accountContext=input.accountContext;delete input.accountContext;delete input.timeoutMs;delete input.__timeoutMs;for(let i=0;i<tries;i++){if(accountContext&&!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();let controller=null;let timer=null;try{if(typeof AbortController!=="undefined"){controller=new AbortController();input.signal=controller.signal;timer=window.setTimeout(()=>{try{controller.abort();}catch(_){}},timeoutMs+i*2500);}
const res=await fetch(url,Object.assign({cache:"no-store"},input));if(timer)window.clearTimeout(timer);if(accountContext&&!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!res||!res.ok){if(i<tries-1){await new Promise((resolve)=>window.setTimeout(resolve,280+i*320));continue;}}
const data=await res.json();if(accountContext&&!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!data||typeof data!=="object")throw new Error("Invalid server response");if(!res.ok){return Object.assign({},data,{ok:false,status:Number(res.status||0),backendUnavailable:res.status===429||res.status>=500,error:data.error||`HTTP ${res.status || "error"}`});}
return data;}catch(err){if(timer)window.clearTimeout(timer);if(accountContext&&!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(i<tries-1){await new Promise((resolve)=>window.setTimeout(resolve,280+i*320));continue;}
return{ok:false,backendUnavailable:true,error:err&&err.name==="AbortError"?"Request timed out. Please try again.":(err&&err.message||"Network request failed")};}}
return{ok:false,error:"Network request failed"};}
async function apiGet(path){const accountContext=captureSocialAccountContext();const timeoutMs=String(path||"").includes("/identity/json-sync")?180000:ACCOUNT_SYNC_FETCH_TIMEOUT_MS;const attempts=String(path||"").includes("/identity/json-sync")?4:2;const data=await fetchJsonWithRetry(API_BASE+path,{cache:"no-store",timeoutMs,accountContext},attempts);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(data){if(data.ok!==false)writeCachedApiGet(path,data);return data;}
return readCachedApiGet(path,shouldCacheApiGet(path)?API_GET_CACHE_MAX_AGE_MS:0);}
const COMMENTS_MEMO_MS=2500;let commentsMemo={key:"",at:0,data:null,pending:null};let commentsMemoEpoch=0;function invalidateCommentsMemo(){commentsMemoEpoch+=1;commentsMemo={key:"",at:0,data:null,pending:null};}
function fetchCommentsForPath(path,opts){const context=captureSocialAccountContext();const key=`${path}|${context.visitorId}|${context.accountKey}|${context.owner}`;const now=Date.now();if(commentsMemo.key===key){if(commentsMemo.pending)return commentsMemo.pending;if(!(opts&&opts.fresh)&&commentsMemo.data&&now-commentsMemo.at<COMMENTS_MEMO_MS){return Promise.resolve(commentsMemo.data);}}
const pending=apiGet(`/comments?path=${encodeURIComponent(path)}&visitorId=${encodeURIComponent(getVisitorId())}`).then((data)=>{if(commentsMemo.key===key&&commentsMemo.pending===pending){commentsMemo.pending=null;if(commentsResponseIsValid(data)){commentsMemo.at=Date.now();commentsMemo.data=data;}}
return data;}).catch((err)=>{if(commentsMemo.key===key&&commentsMemo.pending===pending)commentsMemo.pending=null;throw err;});commentsMemo={key,at:commentsMemo.key===key?commentsMemo.at:0,data:commentsMemo.key===key?commentsMemo.data:null,pending};return pending;}
function reportAccountWriteOutcome(ok,path,backendUnavailable){try{window.dispatchEvent(new CustomEvent("mk-account-write-outcome",{detail:{ok:!!ok,path:String(path||""),backendUnavailable:!!backendUnavailable}}));}catch(_){}}
function apiPost(path,body){const allowWorkspaceSwitch=["/identity/sync-claim","/identity/recovery-claim","/identity/unlink"].includes(path);const accountContext=captureSocialAccountContext(allowWorkspaceSwitch);if(String(path||"").startsWith("/comments"))invalidateCommentsMemo();const timeoutMs=String(path||"").includes("/identity/json-sync")?180000:ACCOUNT_SYNC_FETCH_TIMEOUT_MS;const attempts=path==="/identity"?1:String(path||"").includes("/identity/json-sync")?4:2;const simplePost=path==="/identity"||path==="/comments"||String(path||"").startsWith("/connections/")||path==="/notifications/seen";return fetchJsonWithRetry(API_BASE+path,{method:"POST",headers:{"Content-Type":simplePost?"text/plain;charset=UTF-8":"application/json"},body:JSON.stringify(body),timeoutMs,accountContext,},attempts).then((res)=>{if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(String(path||"").startsWith("/comments"))invalidateCommentsMemo();reportAccountWriteOutcome(!!res&&res.ok!==false,path,!!(res&&res.backendUnavailable));return res;},(err)=>{if(String(path||"").startsWith("/comments"))invalidateCommentsMemo();reportAccountWriteOutcome(false,path,true);throw err;});}
...
function queuePendingXpActivity(metric,details,opts){const m=String(metric||"").trim();if(!m)return{ok:false,queued:false,error:"empty metric"};const nowTs=Date.now();const item={id:`pending_${m}_${nowTs}_${Math.random().toString(36).slice(2, 8)}`,metric:m,details:details&&typeof details==="object"?details:{},opts:opts&&typeof opts==="object"?opts:{},queuedAt:nowTs,source:"engagement-comments-pending"};const q=readPendingXpActivityQueue();q.push(item);if(!writePendingXpActivityQueue(q))return{ok:false,queued:false,error:"pending activity could not be saved"};try{markCloudSyncDirty("pending-xp-activity");}catch(_){}
...
let pendingXpActivityFlushPromise=null;function flushPendingXpActivityQueue(reason){if(pendingXpActivityFlushPromise)return pendingXpActivityFlushPromise;const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext))return Promise.resolve(socialAccountChangedResult());const work=(async()=>{try{if(!window.MkAccountData||typeof window.MkAccountData.recordActivity!=="function")return{ok:false,deferred:true};const q=readPendingXpActivityQueue();if(!q.length)return{ok:true,empty:true};let imported=0;const acknowledged=new Set();for(const item of q.slice(-300)){if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();try{const opts=Object.assign({},item.opts||{},{id:item.id||(item.opts&&item.opts.id),ts:item.queuedAt||Date.now(),throttleMs:0,scope:(item.opts&&item.opts.scope)||`pending:${item.metric}:${item.queuedAt || Date.now()}`});const result=await window.MkAccountData.recordActivity(item.metric,Object.assign({source:reason||"pending-xp-flush"},item.details||{}),opts);if(result&&result.ok===true){acknowledged.add(JSON.stringify(item));imported+=1;}}catch(_){}}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const remaining=readPendingXpActivityQueue().filter((item)=>!acknowledged.has(JSON.stringify(item)));if(!writePendingXpActivityQueue(remaining))return{ok:false,imported,error:"pending activity acknowledgement could not be saved"};if(imported)refreshAccountXpSoon("pending-xp-flush",120);return{ok:true,imported,remaining:remaining.length};}catch(_){return{ok:false,error:"pending flush failed"};}})();pendingXpActivityFlushPromise=work;work.finally(()=>{if(pendingXpActivityFlushPromise===work)pendingXpActivityFlushPromise=null;});return work;}
...
async function apiUploadAvatar(file,name){const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const fd=new FormData();fd.append("visitorId",accountContext.visitorId);fd.append("name",cleanProfileNameLocal(name||""));fd.append("avatar",file);const controller=typeof AbortController!=="undefined"?new AbortController():null;let timer=0;try{if(controller)timer=window.setTimeout(()=>controller.abort(),ACCOUNT_SYNC_FETCH_TIMEOUT_MS);const res=await fetch(API_BASE+"/identity/avatar",{method:"POST",body:fd,cache:"no-store",signal:controller?controller.signal:undefined,});let data=null;try{data=await res.json();}catch(_){data=null;}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!data||typeof data!=="object"||Array.isArray(data)){return{ok:false,status:res.status,error:"Avatar upload returned an invalid response."};}
if(!res.ok){return Object.assign({},data,{ok:false,status:res.status,error:data.error||`Avatar upload failed with HTTP ${res.status}.`});}
return data;}catch(err){const timedOut=err&&err.name==="AbortError";return{ok:false,error:timedOut?"Avatar upload timed out. Please try again.":"Avatar upload could not reach the server. Please hard refresh and try again.",detail:err&&err.message?String(err.message):"",};}finally{if(timer)window.clearTimeout(timer);}}
...
async function claimAccountAccessCode(name,code,reason){if(identityProfileUpdateBusy||window.__mkAccountWorkspaceSwitching)return{ok:false,busy:true,error:"An account update is already in progress."};const nm=cleanProfileNameLocal(name||"");const cd=String(code||"").trim();if(!nm||!cd)return{ok:false,error:"Missing account name or access code."};const accountContext=captureSocialAccountContext(true);const workspaces=window.MkAccountWorkspaces||null;const currentOwner=workspaces&&typeof workspaces.activeOwner==="function"?workspaces.activeOwner():"";const requestedOwner=workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(nm):"";const targetAlreadyLocal=!!(workspaces&&requestedOwner&&typeof workspaces.hasWorkspace==="function"&&workspaces.hasWorkspace(requestedOwner));const guestHasData=!!(workspaces&&currentOwner===workspaces.guestOwner&&typeof workspaces.hasMeaningfulActiveData==="function"&&workspaces.hasMeaningfulActiveData());let importGuest=false;if(guestHasData&&!targetAlreadyLocal){importGuest=window.confirm("This browser has guest learning data.\n\n"+"Press OK to import that guest data into this account.\n"+"Press Cancel to keep the guest data separate and load only this account.");}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();identityProfileUpdateBusy=true;window.__mkAccountWorkspaceSwitching=true;let res=null;let method="sync-claim";let keepWorkspaceSwitchLock=false;try{res=await claimSyncCode(nm,cd);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!res||!res.ok){const fallback=await claimRecoveryCode(nm,cd);if(fallback&&fallback.ok){res=fallback;method="recovery-claim";}}
if(!res||!res.ok||!res.profile)return res||{ok:false,error:"Could not connect to this account."};if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const targetKey=String(res.profile.accountKey||res.profile.account_key||"").trim();const targetOwner=workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(targetKey):"";let workspaceResult={ok:false,error:"Account storage is not ready. Reload before connecting this account."};if(workspaces&&targetOwner){const options={expectedOwner:currentOwner,reason:reason||method,commit:()=>writeLocalProfile(res.profile,{requirePersistence:true})};try{if(importGuest&&currentOwner===workspaces.guestOwner&&!workspaces.hasWorkspace(targetOwner)){workspaceResult=await workspaces.adoptActiveWorkspace(targetOwner,options);}else{workspaceResult=await workspaces.activate(targetOwner,options);}}catch(error){workspaceResult={ok:false,error:String(error&&error.message||error)};}}
if(!workspaceResult||workspaceResult.ok===false){if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();let disconnected=false;try{const unlinked=await apiPost("/identity/unlink",{visitorId:accountContext.visitorId});disconnected=!!(unlinked&&unlinked.ok);}catch(_){}
const failure={ok:false,localFailure:true,error:`The account code was valid, but this browser could not finish connecting its local workspace. ${disconnected ? "The new cloud connection was removed." : "The cloud connection could not be verified; reconnect before syncing."} ${workspaceResult && workspaceResult.error ? workspaceResult.error : ""}`.trim()};if(socialAccountContextIsCurrent(accountContext))setIdentityRecoveryStatus(failure.error,accountContext);return failure;}
const workspaceChanged=!workspaceResult.unchanged;if(workspaceChanged){keepWorkspaceSwitchLock=true;try{sessionStorage.setItem(ACCOUNT_WORKSPACE_RESUME_SYNC_KEY,JSON.stringify({accountKey:targetKey,reason:reason||method,createdAt:Date.now()}));}catch(_){}
...
return res;}finally{identityProfileUpdateBusy=false;if(!keepWorkspaceSwitchLock)window.__mkAccountWorkspaceSwitching=false;}}
...
if(!token)return;const pending=window.__mkAccountLoginTokensInFlightV1||(window.__mkAccountLoginTokensInFlightV1=new Set());if(pending.has(token))return;pending.add(token);const context=captureSocialAccountContext();const tokenIsCurrent=()=>{try{return new URL(window.location.href).searchParams.get("mk_account_login")===token;}catch(_){return false;}};window.setTimeout(async()=>{try{if(!tokenIsCurrent()||!socialAccountContextIsCurrent(context))return;const res=await claimAccountLoginToken(token,{reason:"qr-url-login"}).catch((err)=>({ok:false,error:String(err&&err.message||err)}));if(!tokenIsCurrent())return;if(res&&res.ok){const targetKey=String(res.profile&&(res.profile.accountKey||res.profile.account_key)||"").trim().toLowerCase();const workspaces=window.MkAccountWorkspaces;const targetContext=Object.assign({},context,{accountKey:targetKey,owner:workspaces&&typeof workspaces.ownerForAccount==="function"?workspaces.ownerForAccount(targetKey):"",allowWorkspaceSwitch:!!res.workspaceChanged});if(!targetKey||!socialAccountContextIsCurrent(targetContext))return;removeAccountLoginParamFromUrl(token);try{window.alert(`This device is now logged in as ${res.profile && res.profile.name ? res.profile.name : "the scanned account"}.`);}catch(_){}
try{mountComments();}catch(_){}
return;}
try{window.alert((res&&res.error)||"Could not use this QR login link.");}catch(_){}}finally{pending.delete(token);}},250);}
async function fetchAccountDevices(options){const opts=options||{};const context=opts.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!opts.isCurrent||opts.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiGet(`/identity/devices?visitorId=${encodeURIComponent(context.visitorId)}&deviceName=${encodeURIComponent(getDeviceName())}`);return current()?res:socialAccountChangedResult();}
async function saveCurrentDeviceName(name,options){const opts=options||{};const context=opts.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context);if(!current()||(opts.isCurrent&&!opts.isCurrent()))return socialAccountChangedResult();const nm=setDeviceNameLocal(name);if(!current())return socialAccountChangedResult();const res=await apiPost("/identity/device-name",{visitorId:context.visitorId,deviceName:nm});if(!current())return socialAccountChangedResult();if(res&&res.ok)refreshAccountXpSoon("device-name",120);return res;}
async function disconnectAccountDevice(visitorHashValue,options){const opts=options||{};const context=opts.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!opts.isCurrent||opts.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiPost("/identity/device-disconnect",{visitorId:context.visitorId,targetVisitorHash:visitorHashValue});return current()?res:socialAccountChangedResult();}
...
async function requestConnection(targetName,options){const context=captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!options||!options.isCurrent||options.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiPost("/connections/request",{visitorId:context.visitorId,targetName});if(!current())return socialAccountChangedResult();if(res&&res.ok){try{trackActivity("connection_request",{targetName,source:"connection-request"},{scope:`connection_request:${targetName}`,throttleMs:0});}catch(_){}
refreshAccountXpSoon("connection-request",120);}
return res;}
async function respondConnection(requesterKey,action,options){const context=captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!options||!options.isCurrent||options.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiPost("/connections/respond",{visitorId:context.visitorId,requesterKey,action});if(!current())return socialAccountChangedResult();if(res&&res.ok){try{if(String(action||"")==="accept")trackActivity("connection_added",{requesterKey,source:"connection-accept"},{scope:`connection_added:${requesterKey}`,throttleMs:0});}catch(_){}
refreshAccountXpSoon("connection-response",120);}
return res;}
async function removeConnection(accountKey,options){const context=captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!options||!options.isCurrent||options.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiPost("/connections/remove",{visitorId:context.visitorId,accountKey});if(!current())return socialAccountChangedResult();if(res&&res.ok){refreshAccountXpSoon("connection-remove",120);updateNotificationBadgesSoon(120,{force:true,reason:"connection-remove"});}
return res;}
...
const pageSize=isMobilePowerSensitiveViewport()?120:240;let uploaded=0;for(let i=0;i<events.length;i+=pageSize){if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const batch=events.slice(i,i+pageSize).map(compactActivityEventForStorage);if(onProgress)onProgress({phase:"upload",label:`Uploading learning history ${i + 1}-${Math.min(i + batch.length, events.length)}/${events.length}`,chunkIndex:Math.floor(i/pageSize)+1,chunkCount:Math.ceil(events.length/pageSize)});const res=await apiPost("/identity/import-local",{visitorId:getVisitorId(),deviceName:getDeviceName(),eventsOnly:true,events:batch});if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!res||res.ok===false)return Object.assign({ok:false,uploaded,error:res&&res.error||"learning_history_upload_failed"},res||{});uploaded+=batch.length;await new Promise((resolve)=>window.setTimeout(resolve,isMobilePowerSensitiveViewport()?30:5));}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();try{safeSetLocalStorageItem(LOCAL_ACTIVITY_EVENTS_UPLOAD_FINGERPRINT_KEY,fingerprint,"activity-events-upload-fingerprint");}catch(_){}
...
function activityEventsMemoryCache(){try{const context=window[LOCAL_ACTIVITY_EVENTS_MEMORY_KEY+"_context"];if(!socialAccountContextIsCurrent(context))return null;const arr=window[LOCAL_ACTIVITY_EVENTS_MEMORY_KEY];return Array.isArray(arr)?arr.filter((x)=>x&&typeof x==="object").slice(0,ACCOUNT_SYNC_ACTIVITY_EVENT_MAX):null;}catch(_){return null;}}
function setActivityEventsMemoryCache(arr){try{window[LOCAL_ACTIVITY_EVENTS_MEMORY_KEY]=(Array.isArray(arr)?arr:[]).filter((x)=>x&&typeof x==="object").slice(0,ACCOUNT_SYNC_ACTIVITY_EVENT_MAX);window[LOCAL_ACTIVITY_EVENTS_MEMORY_KEY+"_context"]=captureSocialAccountContext();}catch(_){}}
...
async function writeActivityEventsToIdbUnlocked(arr,accountContext){if(!socialAccountContextIsCurrent(accountContext))throw new Error("account_workspace_changed");const clean=(Array.isArray(arr)?arr:[]).filter((x)=>x&&typeof x==="object").slice(0,ACCOUNT_SYNC_ACTIVITY_EVENT_MAX);const row={key:LOCAL_ACTIVITY_EVENTS_IDB_KEY,data:JSON.stringify(clean),total:clean.length,cap:ACCOUNT_SYNC_ACTIVITY_EVENT_CAP_VERSION,updatedAt:Date.now()};const db=await openActivityEventsIdb();if(!socialAccountContextIsCurrent(accountContext)){try{db.close();}catch(_){}throw new Error("account_workspace_changed");}
return await new Promise((resolve,reject)=>{let tx=null;try{tx=db.transaction(LOCAL_ACTIVITY_EVENTS_IDB_STORE,"readwrite");}
catch(err){try{db.close();}catch(_){}reject(err);return;}
tx.oncomplete=()=>{try{db.close();}catch(_){}resolve(true);};tx.onerror=()=>{try{db.close();}catch(_){}reject(tx.error||new Error("IndexedDB write failed"));};tx.onabort=tx.onerror;try{tx.objectStore(LOCAL_ACTIVITY_EVENTS_IDB_STORE).put(row);}
catch(err){try{db.close();}catch(_){}reject(err);}});}
async function writeActivityEventsToIdb(arr){const accountContext=captureSocialAccountContext();const workspaces=window.MkAccountWorkspaces;if(!workspaces||typeof workspaces.withActiveWorkspace!=="function")throw new Error("Account storage is not ready.");const result=await workspaces.withActiveWorkspace(()=>writeActivityEventsToIdbUnlocked(arr,accountContext),{expectedOwner:accountContext.owner});if(!result||!result.ok)throw new Error(result&&result.error||"Activity history was not saved. Retry when the account is ready.");return result.value;}
async function ensureLocalActivityEventsHydrated(){const cached=activityEventsMemoryCache();if(cached&&cached.length)return cached;const accountContext=captureSocialAccountContext();const workspaces=window.MkAccountWorkspaces;if(!workspaces||typeof workspaces.withActiveWorkspace!=="function")return[];const result=await workspaces.withActiveWorkspace(async()=>{if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const local=readLocalActivityEvents({ignoreMemory:true});let fromIdb;try{fromIdb=await readActivityEventsFromIdb();}
catch(_){if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();setActivityEventsMemoryCache(local||[]);return local||[];}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const merged=mergeActivityEvents(fromIdb||[],local||[]);setActivityEventsMemoryCache(merged);if(JSON.stringify(merged)!==JSON.stringify(fromIdb||[])){try{await writeActivityEventsToIdbUnlocked(merged,accountContext);}catch(_){}}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();try{writeLocalActivityEvents(merged,{skipIdbMirror:true});}catch(_){}
return merged;},{expectedOwner:accountContext.owner});return result&&result.ok&&Array.isArray(result.value)?result.value:[];}
...
function writeLocalActivityEvents(arr,options){const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext))return false;const workspaces=window.MkAccountWorkspaces;if(workspaces&&typeof workspaces.ownerForAccount==="function"&&workspaces.ownerForAccount(accountContext.accountKey)!==accountContext.owner)return false;const clean=(Array.isArray(arr)?arr:[]).slice(0,ACCOUNT_SYNC_ACTIVITY_EVENT_MAX).map(compactActivityEventForStorage);setActivityEventsMemoryCache(clean);if(!(options&&options.skipIdbMirror))writeActivityEventsToIdb(clean).catch(()=>{});const text=JSON.stringify(clean);const chunks=[];for(let i=0;i<text.length;i+=LOCAL_ACTIVITY_EVENTS_CHUNK_SIZE)chunks.push(text.slice(i,i+LOCAL_ACTIVITY_EVENTS_CHUNK_SIZE));const writeChunksAtomically=()=>{const prefix=`${LOCAL_ACTIVITY_EVENTS_CHUNK_PREFIX}${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}:`;try{for(let i=0;i<chunks.length;i+=1){if(!safeSetLocalStorageItem(`${prefix}${i}`,chunks[i],"activity-events-chunk"))throw new Error("activity chunk write failed");}
...
function applyCloudActivity(data,options){if(!data||!data.ok)return false;try{const visits=mergeByPathTime(getLocalVisitsForSync(),data.visits||[],{key:"path",timeKeys:["ts","lastVisited","last_visited"],max:1000});writeLocalArray(LOCAL_VISITS_KEY,visits,1000);}catch(_){}
...
try{const events=mergeActivityEvents(readLocalActivityEvents(),data.events||[]);writeLocalActivityEvents(events,options);}catch(_){}
...
async function fetchAndApplyCloudActivityPaged(opts){const accountContext=captureSocialAccountContext();const workspaces=window.MkAccountWorkspaces;if(!workspaces||typeof workspaces.withActiveWorkspace!=="function")return{ok:false,error:"Account storage is not ready."};const commitLocal=(action)=>workspaces.withActiveWorkspace(async()=>{if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();return action();},{expectedOwner:accountContext.owner});const options=opts&&typeof opts==="object"?opts:{};const onProgress=typeof options.onProgress==="function"?options.onProgress:null;await ensureLocalActivityEventsHydrated();if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();const visitorId=encodeURIComponent(getVisitorId());const startedStats=accountJsonSyncStats(localAccountJsonSnapshotPayload());let cloudStats=null;const light=await apiGet(`/identity/activity?visitorId=${visitorId}&light=1&t=${Date.now()}`);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!light||light.ok===false)return Object.assign({ok:false,error:light&&light.error||"Could not load cloud activity."},light||{});cloudStats=light.stats||null;const totalEvents=Math.max(0,Number(options.expectedTotal||light.eventTotal||light.eventsTotal||light.totalEvents||(light.stats&&light.stats.stores&&light.stats.stores.activityEvents)||0)||0);const targetEvents=Math.min(ACCOUNT_SYNC_ACTIVITY_EVENT_MAX,totalEvents);const appliedLight=await commitLocal(()=>{applyCloudActivity(Object.assign({},light,{events:[]}),{skipIdbMirror:true});if(!targetEvents)resetActivityDownloadCursor();return true;});if(!appliedLight||!appliedLight.ok)return appliedLight||{ok:false,error:"Activity history could not be applied."};if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!targetEvents){return{ok:true,pagedActivity:true,eventTotal:totalEvents,eventReceived:0,stats:cloudStats||{total:0,stores:{activityEvents:0}},applied:{changed:false,before:startedStats,after:accountJsonSyncStats(localAccountJsonSnapshotPayload()),downloaded:{total:0,stores:{}}},localApplyMissing:{total:0,stores:{}},partialLocalApply:false};}
const stored=readActivityDownloadCursor(targetEvents);let cursor=stored&&stored.cursor?stored.cursor:null;let scanned=stored?Math.max(0,Number(stored.scanned||0)||0):0;let mergedEvents=readLocalActivityEvents();let received=0;let newUnique=0;let guard=0;let pageFailure=null;let persisted=false;const pageLimit=isMobilePowerSensitiveViewport()?100:220;const estimatedRemaining=Math.max(1,targetEvents-scanned);const maxPages=Math.max(80,Math.ceil(estimatedRemaining/pageLimit)+20);while(guard<maxPages){if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();guard+=1;const from=Math.min(scanned+1,targetEvents);const to=Math.min(scanned+pageLimit,targetEvents);if(onProgress)onProgress({phase:"download",label:`Downloading learning history ${from}-${to}/${targetEvents}`,chunkIndex:guard,chunkCount:Math.max(1,Math.ceil(targetEvents/pageLimit))});let part=null;try{part=await apiGet(`/identity/activity?visitorId=${visitorId}&eventsOnly=1&compact=1&cursor=1${activityCursorQuery(cursor)}&limit=${pageLimit}&t=${Date.now()}`);}
catch(err){pageFailure=err&&err.message||"learning_history_page_fetch_failed";break;}
if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();if(!part||part.ok===false){pageFailure=part&&part.error||"Could not load learning history page.";break;}
const events=Array.isArray(part.events)?part.events:[];if(!events.length){const cleared=await commitLocal(()=>{clearActivityDownloadCursor();return true;});if(!cleared||!cleared.ok)pageFailure=cleared&&cleared.error||"History checkpoint could not be cleared.";break;}
const beforeLen=mergedEvents.length;mergedEvents=mergeActivityEvents(mergedEvents,events);const afterLen=mergedEvents.length;if(afterLen>beforeLen)newUnique+=afterLen-beforeLen;received+=events.length;scanned+=events.length;const nextCursor=part.nextCursor&&typeof part.nextCursor==="object"?part.nextCursor:null;if(nextCursor&&Number(nextCursor.ts))cursor={ts:Number(nextCursor.ts||0),id:String(nextCursor.id||"")};else{const last=events[events.length-1]||null;cursor=last?{ts:Number(last.ts||last.createdAt||0),id:String(last.id||"")}:cursor;}
const done=part.done||scanned>=targetEvents||events.length<pageLimit;const pageSaved=await commitLocal(async()=>{await writeActivityEventsToIdbUnlocked(mergedEvents,accountContext);if(!socialAccountContextIsCurrent(accountContext))return socialAccountChangedResult();writeLocalActivityEvents(mergedEvents,{skipIdbMirror:true});if(done)clearActivityDownloadCursor();else if(!writeActivityDownloadCursor({cursor,scanned,totalEvents:targetEvents}))return{ok:false,error:"History checkpoint could not be saved."};return true;});if(!pageSaved||!pageSaved.ok){pageFailure=pageSaved&&pageSaved.error||"Activity history could not be saved.";break;}
persisted=true;if(done)break;await new Promise((resolve)=>window.setTimeout(resolve,isMobilePowerSensitiveViewport()?45:8));}
...
async function deleteComment(commentId,opts){const options=opts&&typeof opts==="object"?opts:{};const context=options.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context);const visible=()=>current()&&(!options.isCurrent||options.isCurrent());if(!visible())return false;const isOwn=!!options.own;let token=readAdminToken();if(!isOwn&&!token)token=promptForAdminToken();if(!isOwn&&!token)return false;const ok=window.confirm(isOwn?"Delete your comment from this page? If it has replies, they will be hidden too.":"Delete this comment? If it has replies, they will be hidden too.");if(!ok||!visible())return false;if(typeof options.onConfirmed==="function"){try{options.onConfirmed();}catch(_){}}
if(!current())return false;markCommentUiBusyNoPreload(6000);const body={path:options.path||currentPath(),commentId,visitorId:context.visitorId};if(!isOwn&&token)body.adminKey=token;if(isOwn&&token)body.adminKey=token;const res=await apiPost("/comments/delete",body);if(!current())return false;if(res&&res.ok){markLocalCommentDeleted(commentId);return true;}
if(!visible())return false;if(res&&/unauthorized/i.test(String(res.error||""))){if(!isOwn){writeAdminToken("");window.alert("Admin key was rejected. Please enter it again.");}else{window.alert("Only comments linked to your current account or browser can be deleted here.");}}else{window.alert((res&&res.error)||"Could not delete this comment.");}
return false;}
...
async function editComment(commentId,text,opts){const options=opts&&typeof opts==="object"?opts:{};const context=options.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context);const visible=()=>current()&&(!options.isCurrent||options.isCurrent());if(!visible())return false;const isOwn=!!options.own;let token=readAdminToken();if(!isOwn&&!token)token=promptForAdminToken();if(!isOwn&&!token)return false;const tx=String(text||"").trim();if(!tx||!visible())return false;const body={path:options.path||currentPath(),commentId,visitorId:context.visitorId,text:tx};if(!isOwn&&token)body.adminKey=token;if(isOwn&&token)body.adminKey=token;const res=await apiPost("/comments/edit",body);if(!current())return false;if(res&&res.ok){const editedAt=Number(res.comment&&res.comment.editedAt||Date.now());updateLocalComment(commentId,{text:tx,editedAt,deleted:false});return true;}
if(!visible())return false;if(res&&/unauthorized/i.test(String(res.error||""))){if(!isOwn){writeAdminToken("");window.alert("Admin key was rejected. Please enter it again.");}else{window.alert("Only comments linked to your current account or browser can be edited here.");}}else{window.alert((res&&res.error)||"Could not edit this comment.");}
return false;}
async function editCommentByAdmin(commentId,text,path){return editComment(commentId,text,{own:false,path:path||currentPath()});}
async function editOwnComment(commentId,path,text,options){return editComment(commentId,text,Object.assign({own:true,path},options||{}));}
...
.mk-comments-sort{ --mk-comments-sort-radius:.72rem; --mk-comments-sort-surface:#fff; --mk-comments-sort-hover:#fff; position:relative; display:inline-block; box-sizing:border-box; width:7.75rem; color:var(--md-default-fg-color); font-family:inherit; font-size:.64rem; font-weight:750; line-height:1.15; isolation:isolate; }
html[data-md-color-scheme="slate"] .mk-comments-sort,body[data-md-color-scheme="slate"] .mk-comments-sort{ --mk-comments-sort-surface:#1f242c; --mk-comments-sort-hover:#1f242c; }
@supports(color:color-mix(in srgb,#fff,#000)){
  .mk-comments-sort{ --mk-comments-sort-hover:color-mix(in srgb,#fff 93%,var(--md-accent-fg-color) 7%); }
  html[data-md-color-scheme="slate"] .mk-comments-sort,body[data-md-color-scheme="slate"] .mk-comments-sort{ --mk-comments-sort-hover:color-mix(in srgb,#1f242c 91%,var(--md-accent-fg-color) 9%); }
}
...
.mk-page-edit-editor-line{ border:1px solid color-mix(in srgb,#10b981 52%,var(--md-default-fg-color) 14%); border-radius:12px; padding:7px 10px; background:color-mix(in srgb,#10b981 10%,var(--md-default-bg-color) 90%); color:var(--md-default-fg-color); font-size:.66rem; line-height:1.4; overflow-wrap:anywhere; }
.mk-page-edit-editor-line:empty{ display:none; }
...
async function markNotificationsSeen(options){const opts=options||{};const context=opts.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!opts.isCurrent||opts.isCurrent());if(!current())return socialAccountChangedResult();const ts=Date.now();const latest=Number(opts.latestNotificationAt);let seenAt=context.accountKey?ts:Math.max(getNotificationSeenAt(),Number.isFinite(latest)?latest:0);let awarded=false;if(context.accountKey){let res;try{res=await apiPost("/notifications/seen",{visitorId:context.visitorId,seenAt:ts});}
catch(_){res={ok:false,error:"Could not mark notifications as read."};}
if(!current())return socialAccountChangedResult();if(!res||!res.ok||!Number.isFinite(Number(res.seenAt))||Number(res.seenAt)<=0){return{ok:false,error:"Could not mark notifications as read."};}
seenAt=Number(res.seenAt);awarded=!!res.xpAwarded;}
if(!current())return socialAccountChangedResult();setNotificationSeenAt(seenAt);if(getNotificationSeenAt()!==seenAt)return{ok:false,error:"Could not save read status on this browser."};if(typeof invalidateNotificationBadgeRefresh==="function")invalidateNotificationBadgeRefresh();setNotificationBadgeCount(Math.max(0,Number(opts.remainingUnread)||0));if(!context.accountKey&&opts.hadUnread){try{trackActivity("notification_read",{source:"notifications-tab",seenAt},{scope:`notification_read:${new Date(ts).toISOString().slice(0,10)}`,throttleMs:0});}catch(_){}}
if(awarded)refreshAccountXpSoon("notifications-seen",120);return{ok:true,seenAt,xpAwarded:awarded};}
...
function invalidateNotificationBadgeRefresh(){__mkNotificationBadgeEpoch+=1;__mkNotificationBadgePromise=null;__mkNotificationBadgeContext=null;}
async function refreshNotificationBadgeCount(opts){const options=opts&&typeof opts==="object"?opts:{};const force=!!options.force;if(pageIsHiddenForAccountSync()&&!force)return;const context=captureSocialAccountContext();if(!socialAccountContextIsCurrent(context)){invalidateNotificationBadgeRefresh();return;}
if(!context.accountKey){invalidateNotificationBadgeRefresh();setNotificationBadgeCount(0);setConnectionBadgeCount(0);return;}
const mobilePowerMode=isMobilePowerSensitiveViewport();const last=readLastNotificationBadgeAt();if(!force&&mobilePowerMode&&last&&Date.now()-last<MOBILE_NOTIFICATION_MIN_MS)return;const seenAt=getNotificationSeenAt();const connectionSeenAt=getConnectionNotificationSeenAt();const pending=__mkNotificationBadgeContext;if(__mkNotificationBadgePromise&&!force&&pending&&socialAccountContextIsCurrent(pending.account)&&pending.seenAt===seenAt&&pending.connectionSeenAt===connectionSeenAt){return __mkNotificationBadgePromise;}
const epoch=++__mkNotificationBadgeEpoch;__mkNotificationBadgeContext={account:context,seenAt,connectionSeenAt};const load=(async()=>{const qs=new URLSearchParams();qs.set("visitorId",context.visitorId);qs.set("limit","80");const data=await apiGet(`/notifications?${qs.toString()}`);if(epoch!==__mkNotificationBadgeEpoch||!socialAccountContextIsCurrent(context)||getNotificationSeenAt()!==seenAt||getConnectionNotificationSeenAt()!==connectionSeenAt)return;if(!data||!data.ok||!Array.isArray(data.notifications))return;touchNotificationBadgeRefresh();const serverSeen=Number(data.seenAt||0)||0;if(serverSeen>getNotificationSeenAt())setNotificationSeenAt(serverSeen);const seen=Math.max(getNotificationSeenAt(),serverSeen);const count=data.notifications.filter((x)=>!isConnectionNotificationType(x&&x.type)&&(!!(x&&x.forceUnread)||Number(x&&x.createdAt||0)>seen)).length;const connectionSeen=getConnectionNotificationSeenAt();const connectionCount=data.notifications.filter((x)=>isConnectionNotificationType(x&&x.type)&&Number(x&&x.createdAt||0)>connectionSeen).length;setNotificationBadgeCount(count);setConnectionBadgeCount(connectionCount);})();__mkNotificationBadgePromise=load;try{return await load;}finally{if(__mkNotificationBadgePromise===load){__mkNotificationBadgePromise=null;__mkNotificationBadgeContext=null;}}}
...
const PAGE_EDIT_EDITOR_TOKEN_KEY="mk_page_edit_editor_token_v1";function readPageEditEditorToken(){try{return String(localStorage.getItem(PAGE_EDIT_EDITOR_TOKEN_KEY)||"").trim();}catch(_){return"";}}
function writePageEditEditorToken(token){const t=String(token||"").trim();if(!t)return;try{localStorage.setItem(PAGE_EDIT_EDITOR_TOKEN_KEY,t);}catch(_){}}
function clearPageEditEditorToken(){try{localStorage.removeItem(PAGE_EDIT_EDITOR_TOKEN_KEY);}catch(_){}}
async function fetchEditorIdentity(token){const t=String(token||"").trim();if(!t)return null;try{const resp=await fetch(`${API_BASE}/page-edit/whoami`,{method:"POST",cache:"no-store",headers:{"Content-Type":"application/json"},body:JSON.stringify({editorToken:t})});const data=resp?await resp.json().catch(()=>null):null;if(!data)return{canPublish:false,error:`Editor check failed${resp ? `with HTTP ${resp.status}` : ""}.`};if(data.ok===false)return{canPublish:false,error:String(data.error||"That token was not accepted.")};return data;}catch(err){return{canPublish:false,error:String(err&&err.message||err||"Network error while checking the token.")};}}
async function publishPageEdit(payload){try{const resp=await fetch(`${API_BASE}/page-edit/publish`,{method:"POST",cache:"no-store",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload||{})});const data=resp?await resp.json().catch(()=>null):null;if(!data)return{ok:false,error:`Publish request failed${resp ? `with HTTP ${resp.status}` : ""}.`};return data;}catch(err){return{ok:false,error:String(err&&err.message||err||"Network error while publishing.")};}}
function renderActionMenu(menu,btn,actions,onToggle,extras){const labels=pageActionLabels();const extra=extras&&typeof extras==="object"?extras:{};menu.innerHTML="";Object.keys(labels).forEach((action)=>{const row=document.createElement("button");row.type="button";row.className="mk-page-action-item"+(actions&&actions[action]?" is-active":"");row.dataset.action=action;row.innerHTML=`${commentSvgIcon(actionIconName(action), 16)}<span>${escapeHtml(labels[action])}</span>${actions && actions[action] ? commentSvgIcon("check", 15) : ""}`;row.addEventListener("click",async(ev)=>{ev.preventDefault();ev.stopPropagation();if(!row.isConnected||!menu.contains(row)||row.disabled)return;row.disabled=true;try{await onToggle(action);}finally{row.disabled=false;}});menu.appendChild(row);});const sep=document.createElement("div");sep.className="mk-page-action-separator";menu.appendChild(sep);if(typeof extra.onEditPage==="function")menu.appendChild(pageEditRow("fileEdit","Edit page",extra.onEditPage));if(typeof extra.onAdminEdits==="function")menu.appendChild(pageEditRow("admin","Review edits",extra.onAdminEdits));}
...
return`# ${heading}\n\n<!--\n${lines.join("\n")}\n-->\n\n${text}\n`;}
async function loadCurrentPageMarkdown(path){const mapped=await fetchMarkdownThroughWorker("",path);if(mapped&&mapped.ok!==false){return{ok:true,text:mapped.text,sourceUrl:mapped.sourceUrl||"",sourcePath:mapped.sourcePath||sourceMdPathFromConceptPath(path),sha:String(mapped.sha||""),method:mapped.method||"worker-source-map"};}
const mdPath=sourceMdPathFromConceptPath(path);return{ok:false,text:articleFallbackMarkdown(path,mapped),sourceUrl:"",sourcePath:mdPath||path,sha:"",method:"worker-source-failed",sourceError:mapped||null,};}
...
const body=panel.body;body.innerHTML=`<div class="mk-local-activity-meta">Loading the Markdown source for this page…</div>`;loadCurrentPageMarkdown(p).then((loaded)=>{const original=String(loaded&&loaded.text||"");body.innerHTML="";const wrap=el("div","mk-page-edit-body");const meta=el("div","mk-page-edit-meta");const sourceNote=loaded&&loaded.ok?(/^private-source/.test(String(loaded.method||""))?" · loaded through protected source proxy":""):" · exact source not available, using a generated draft";meta.innerHTML=`<div><strong>${escapeHtml(title() || p)}</strong></div><div>Source: <code>${escapeHtml((loaded && loaded.sourcePath) || sourceMdPathFromConceptPath(p) || p)}</code>${sourceNote}</div><div>Edit the Markdown below, then submit it as a suggested typo / bug / improvement fix.</div>`;const sourceError=(!loaded||!loaded.ok)?el("div",""):null;if(sourceError)sourceError.innerHTML=pageEditSourceErrorHtml(loaded&&loaded.sourceError);const ta=document.createElement("textarea");ta.className="mk-page-edit-textarea";ta.spellcheck=false;ta.value=original;const note=document.createElement("textarea");note.className="mk-page-edit-note";note.placeholder="Optional note for the admin, for example: typo fix, bug in explanation, improved wording…";const actions=el("div","mk-page-edit-actions");const status=el("span","mk-page-edit-status","");const cancel=iconButton("mk-comment-small-btn","x","Cancel");const submit=iconButton("mk-comment-small-btn","send","Submit suggestion");const editorBtn=iconButton("mk-comment-small-btn","key","Course editor sign-in");const editorLine=el("div","mk-page-edit-editor-line","");meta.appendChild(editorLine);let editor=null;const baseSha=String((loaded&&loaded.sha)||"");const canPublish=()=>!!(editor&&editor.canPublish&&baseSha);const setButtonLabel=(btn,icon,label)=>{btn.innerHTML=`${commentSvgIcon(icon, 16)}${iconLabel(label)}`;btn.setAttribute("title",label);btn.setAttribute("aria-label",label);};const renderEditorLine=()=>{if(!editor||!editor.canPublish){editorLine.textContent="";setButtonLabel(editorBtn,"key","Course editor sign-in");setButtonLabel(submit,"send","Submit suggestion");return;}
setButtonLabel(editorBtn,"logout","Sign out of editor access");const who=escapeHtml(editor.displayName||"Course editor");const where=escapeHtml(editor.scope?`docs/${editor.scope}`:"the whole wiki");if(!baseSha){editorLine.innerHTML=`<strong>Signed in as ${who}</strong> · publishing is off for this page because its exact Markdown source could not be loaded. An edit here still goes to the review queue.`;setButtonLabel(submit,"send","Submit suggestion");return;}
editorLine.innerHTML=editor.publishMode==="commit"?`<strong>Signed in as ${who}</strong> · edits within ${where} are committed straight to the source repository and go live without review.`:`<strong>Signed in as ${who}</strong> · edits within ${where} open a pull request for the wiki owner to merge.`;setButtonLabel(submit,"send",editor.publishMode==="commit"?"Publish to the site":"Publish as pull request");};const updateSubmit=()=>{submit.disabled=String(ta.value||"")===original||!String(ta.value||"").trim();};updateSubmit();renderEditorLine();ta.addEventListener("input",updateSubmit);cancel.addEventListener("click",()=>{try{panel.close();}catch(_){}});editorBtn.addEventListener("click",async()=>{if(editor&&editor.canPublish){clearPageEditEditorToken();editor=null;renderEditorLine();status.textContent="Signed out of course editor access.";return;}
const entered=window.prompt("Course editor token (issued by the wiki owner):","");if(entered===null)return;const token=String(entered||"").trim();if(!token)return;status.textContent="Checking token…";const who=await fetchEditorIdentity(token);if(!who||!who.canPublish){status.textContent=String((who&&who.error)||"That token was not recognised.");return;}
writePageEditEditorToken(token);editor=who;status.textContent="";renderEditorLine();});submit.addEventListener("click",async()=>{const proposed=String(ta.value||"");if(!proposed.trim()||proposed===original)return;submit.disabled=true;if(canPublish()){status.textContent=editor.publishMode==="commit"?"Publishing…":"Opening pull request…";const res=await publishPageEdit({editorToken:readPageEditEditorToken(),path:p,title:title()||p,baseSha,proposedMd:proposed,note:note.value||""});if(res&&res.ok){status.textContent=String(res.message||"Published.");window.setTimeout(()=>{try{panel.close();}catch(_){}},2400);return;}
if(res&&res.conflict){status.textContent=String(res.error||"This page changed in the repository.");const reload=window.confirm(`${res.error || "This page changed in the repository."}\n\nReload the editor with the current repository text? Your unsaved change will be lost.`);if(reload){try{panel.close();}catch(_){}openPageEditModal(p);return;}
submit.disabled=false;return;}
status.textContent=String((res&&res.error)||"Publishing failed. Nothing was changed.");submit.disabled=false;return;}
status.textContent="Submitting…";try{const prof=readLocalProfile();const res=await apiPost("/page-edit/submit",{path:p,title:title()||p,visitorId:getVisitorId(),accountKey:prof.accountKey||"",name:prof.name||getStoredName(),sourcePath:(loaded&&loaded.sourcePath)||sourceMdPathFromConceptPath(p)||p,sourceUrl:(loaded&&loaded.sourceUrl)||"",originalMd:original,proposedMd:proposed,note:note.value||""});if(!res||res.ok===false)throw new Error(res&&res.error||"Submit failed");status.textContent="Submitted. Thank you.";window.setTimeout(()=>{try{panel.close();}catch(_){}},650);}catch(err){status.textContent=String(err&&err.message||err||"Submit failed");submit.disabled=false;}});const savedEditorToken=readPageEditEditorToken();if(savedEditorToken){fetchEditorIdentity(savedEditorToken).then((who)=>{if(who&&who.canPublish){editor=who;renderEditorLine();return;}
if(who&&who.error)clearPageEditEditorToken();}).catch(()=>{});}
actions.appendChild(status);actions.appendChild(editorBtn);actions.appendChild(cancel);actions.appendChild(submit);wrap.appendChild(meta);if(sourceError&&String(sourceError.innerHTML||"").trim())wrap.appendChild(sourceError);wrap.appendChild(ta);wrap.appendChild(note);wrap.appendChild(actions);body.appendChild(wrap);try{ta.scrollTop=0;ta.selectionStart=0;ta.selectionEnd=0;}catch(_){}
...
try{window.__mkPageActionMenuGuardUntil=Date.now()+900;}catch(_){}};const openMenu=()=>{if(!btn.isConnected||currentPath()!==path||window.__mkAccountWorkspaceSwitching){closeMenu();return;}
loadState();const context=captureSocialAccountContext();closeTopNavMenusForPageAction();renderActionMenu(menu,btn,actions,(action)=>toggleAction(action,context),{onEditPage:async()=>{closeMenu();openPageEditModal(path);},onAdminEdits:async()=>{closeMenu();openPageEditAdminManager();}});menu.hidden=false;shield.hidden=false;markMenuGuard();positionMenu();try{requestAnimationFrame(positionMenu);}catch(_){}};const loadState=()=>{const localActions=Object.assign({},pageActionMapForPathFromLocal(path));try{if(window.MkAccountData&&typeof window.MkAccountData.getStateForPage==="function"){const state=window.MkAccountData.getStateForPage(path)||{};actions=Object.assign({},state.actions||{},localActions);}else{actions=localActions;}}catch(_){actions=localActions;}
Object.keys(actions).forEach((k)=>{if(!actions[k])delete actions[k];});updateButton();};const toggleAction=async(action,context)=>{if(!btn.isConnected||currentPath()!==path||!socialAccountContextIsCurrent(context)){closeMenu();return;}
loadState();const nextActive=!actions[action];if(nextActive&&!ecConsumeGuestAction("save_page",{source:"page-actions",path,title:pageTitle,action}))return;actions=Object.assign({},actions,{[action]:nextActive});if(!nextActive)delete actions[action];saveLocalPageAction(path,pageTitle,action,nextActive);try{if(window.MkAccountData&&typeof window.MkAccountData.scheduleAutoSync==="function")window.MkAccountData.scheduleAutoSync("page-action-toggle");}catch(_){}
...
updateSubmitState();};const restoreSubmittedText=()=>{if(!textClearedEarly)return;textClearedEarly=false;const newerDraft=String(text.value||"");text.value=newerDraft.trim()&&newerDraft.trim()!==tx?`${tx}\n\n${newerDraft}`:tx;try{text.dispatchEvent(new Event("input",{bubbles:true}));}catch(_){}
updateSubmitState();};const keepNewReplyDraft=(message)=>{const draft=String(text.value||"").trim();if(!parentId||!draft||draft===tx)return false;pendingReplyRefresh=true;let status=f.querySelector(".mk-comment-draft-status");if(!status){status=el("div","mk-comment-draft-status");status.setAttribute("role","status");formContent.appendChild(status);}
status.textContent=message;updateSubmitState();return true;};const emitOptimisticComment=()=>{if(!optimisticId||optimisticSent||typeof onDone!=="function")return;optimisticSent=true;onDone({ok:true,optimistic:true,comment:{id:optimisticId,path:currentPath(),title:title(),parentId:"",name:nm,avatar:av,avatarFrame:profileBefore.avatarFrame||"level-1",text:tx,ts:Date.now(),editedAt:0,deleted:false,pending:true,reactions:{},isOwner:true},profile:{name:nm,avatar:av,avatarFrame:profileBefore.avatarFrame||"level-1"}});};const removeOptimisticComment=()=>{if(optimisticSent&&optimisticId&&typeof onDone==="function")onDone({ok:false,removeOptimisticId:optimisticId});};emitOptimisticComment();if(optimisticSent)clearSubmittedTextEarly();const hadAccountBefore=!!String(profileBefore.accountKey||"").trim();let createdAccountFromName=false;let profileNow={name:nm,avatar:av,avatarFrame:profileBefore.avatarFrame||"level-1",accountKey:profileBefore.accountKey||""};if(!sendAnonymous&&!hadAccountBefore){const privacyChoice=defaultNewAccountPrivacy();const identity=await saveOnlineProfile(nm,av,{keepAvatar:false,privacy:privacyChoice,isPublic:false,askPrivacy:false,bio:profileBefore.bio||""});if(!identity.ok){if(isLikelyLostPostResponse(identity)){profileNow=writeLocalProfile(Object.assign({},profileBefore,{name:nm,avatar:av,avatarFrame:profileBefore.avatarFrame||"level-1"}));}else{submittingComment=false;submit.disabled=false;restoreSubmittedText();removeOptimisticComment();alertCooldown(identity);if(nameInput)nameInput.focus();return;}}else{profileNow=identity.profile||{name:nm,avatar:av,avatarFrame:"level-1",accountKey:""};createdAccountFromName=true;const nextContext=captureSocialAccountContext();if(nextContext.visitorId===formAccountContext.visitorId&&nextContext.accountKey===String(profileNow.accountKey||"").trim().toLowerCase()&&nextContext.accountKey)formAccountContext=nextContext;}}else if(!sendAnonymous){profileNow=Object.assign({},profileBefore,{name:nm,avatar:av||profileBefore.avatar||"",avatarFrame:profileBefore.avatarFrame||"level-1",accountKey:profileBefore.accountKey||""});}
if(!formIsCurrent()){submittingComment=false;restoreSubmittedText();updateSubmitState();return;}
const res=await apiPost("/comments",{path:formPath,title:formPageTitle,visitorId:getVisitorId(),parentId:parentId||"",name:profileNow.name,avatar:profileNow.avatar||av,avatarFrame:profileNow.avatarFrame||"level-1",keepAvatar:!sendAnonymous&&isImageAvatar(profileNow.avatar||av),text:tx,anonymous:sendAnonymous});submittingComment=false;submit.disabled=false;if(!socialAccountContextIsCurrent(formAccountContext)){restoreSubmittedText();return;}
if(!res||!res.ok){if(!formIsCurrent()){restoreSubmittedText();return;}
if(isLikelyLostPostResponse(res)){if(sendAnonymous)markAnonymousCommentUsedToday();if(parentId){if(keepNewReplyDraft("The reply may have been sent. Your new draft is still here; the list will refresh when you finish or cancel it."))return;f.remove();}else if(sendAnonymous||hasProfile||profileNow.name){if(!textClearedEarly)text.value="";try{text.dispatchEvent(new Event("input",{bubbles:true}));}catch(_){}
updateSubmitState();}
if(typeof onDone==="function")onDone({ok:true,reconcileOnly:true,keepOptimistic:true});return;}
restoreSubmittedText();removeOptimisticComment();const msg=res&&(res.code==="anonymous_daily_limit"||res.code==="anon_daily_limit")?anonymousCommentDailyLimitMessage():((res&&res.error)||"Could not post this comment.");window.alert(msg);return;}
...
if(!sendAnonymous&&res.comment&&res.comment.id){saveLocalComment({id:res.comment.id,path:formPath,title:formPageTitle,parentId:parentId||"",name:(res.profile&&res.profile.name)||finalProfile.name||profileNow.name,avatar:(res.profile&&res.profile.avatar)||finalProfile.avatar||profileNow.avatar||"",avatarFrame:(res.profile&&res.profile.avatarFrame)||finalProfile.avatarFrame||profileNow.avatarFrame||"level-1",text:tx,ts:Number(res.comment.ts||Date.now()),editedAt:0,deleted:false});}else if(sendAnonymous&&hasProfile&&res.comment&&res.comment.id){const anonymousComments=getLocalComments().filter((x)=>x&&x.id!==res.comment.id);anonymousComments.unshift({id:res.comment.id,path:formPath,title:formPageTitle,parentId:parentId||"",name:"Anonymous",avatar:"",avatarFrame:"level-1",text:tx,ts:Number(res.comment.ts||Date.now()),editedAt:0,deleted:false,anonymous:true,});writeLocalArray(LOCAL_COMMENTS_KEY,anonymousComments,300);activityChanged("anonymous-comment",{id:res.comment.id,path:formPath});}
if(createdAccountFromName)scheduleLocalProgressSyncForNamedGuest("comment-name-created-account");if(!formIsCurrent())return;if(parentId){if(keepNewReplyDraft("Reply published. Your new draft is still here; the list will refresh when you finish or cancel it."))return;pendingReplyRefresh=false;f.remove();}else if(sendAnonymous||hasProfile||profileNow.name||textClearedEarly){if(!textClearedEarly)text.value="";try{text.dispatchEvent(new Event("input",{bubbles:true}));}catch(_){}
updateSubmitState();}
...
return f;}
async function openReactionViewer(c,key,icon){ensureStylesOnce();const existing=document.querySelector(".mk-reaction-view-modal");if(existing){if(typeof existing.__mkReactionClose==="function")existing.__mkReactionClose();else existing.remove();}
const accountContext=captureSocialAccountContext();const page=currentPath();const modal=el("div","mk-reaction-view-modal");modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.innerHTML=`<div class="mk-reaction-view-panel" role="document">
    ...
    </div>`;document.body.appendChild(modal);const body=modal.querySelector(".mk-reaction-view-body");let closed=false;let loading=false;const isCurrent=()=>!closed&&modal.isConnected&&currentPath()===page&&socialAccountContextIsCurrent(accountContext);const close=()=>{if(closed)return;closed=true;document.removeEventListener("keydown",onKey,true);document.removeEventListener("DOMContentSwitch",close);document.removeEventListener("navigation:load",close);window.removeEventListener("pagehide",close);window.removeEventListener("mk-account-workspace-changed",close);try{closeQuickShopMenu();}catch(_){}
try{modal.remove();}catch(_){}
try{if(!document.querySelector(".mk-local-activity-modal")){document.documentElement.classList.remove("mk-local-activity-open");document.body.classList.remove("mk-local-activity-open");unlockPageBehindModal();mkLocalUnbindViewportMetrics();}}catch(_){}};modal.__mkReactionClose=close;modal.addEventListener("click",(ev)=>{if(ev.target===modal)close();});const closeBtn=modal.querySelector(".mk-reaction-view-close");if(closeBtn)closeBtn.addEventListener("click",close);const onKey=(ev)=>{if(ev.key==="Escape")close();};document.addEventListener("keydown",onKey,true);document.addEventListener("DOMContentSwitch",close);document.addEventListener("navigation:load",close);window.addEventListener("pagehide",close);window.addEventListener("mk-account-workspace-changed",close);const load=async()=>{if(loading||!isCurrent())return;loading=true;body.replaceChildren(el("div","mk-reaction-empty","Loading…"));let data=null;try{data=await apiGet(`/comment-reactions?path=${encodeURIComponent(c.path || page)}&commentId=${encodeURIComponent(c.id)}&reaction=${encodeURIComponent(key)}&visitorId=${encodeURIComponent(accountContext.visitorId)}`);}catch(_){}finally{loading=false;}
if(!isCurrent())return;body.innerHTML="";if(!data||!data.ok||!Array.isArray(data.users)||!data.users.every(u=>u&&typeof u==="object"&&!Array.isArray(u))){body.appendChild(el("div","mk-reaction-empty","Could not load reactions."));const retry=el("button","mk-comment-small-btn","Retry");retry.type="button";retry.addEventListener("click",()=>load());body.appendChild(retry);return;}
if(!data.users.length){body.appendChild(el("div","mk-reaction-empty","No reactions of this type yet."));return;}
data.users.forEach((u)=>{const row=el("div","mk-reaction-user");row.innerHTML=avatarHtml(u.name||"Anonymous",u.avatar||"",u.avatarFrame||"level-1");row.appendChild(el("span","mk-reaction-user-name",u.name||"Anonymous"));body.appendChild(row);});};await load();}
...
const commentListData=new WeakMap();function commentsResponseIsValid(data){return!!data&&data.ok!==false&&Array.isArray(data.comments)&&data.comments.every(c=>c&&typeof c==="object"&&!Array.isArray(c)&&(typeof c.id==="string"||typeof c.id==="number")&&String(c.id).trim()&&(c.text==null||typeof c.text==="string"));}
async function loadCommentsInto(list,opts){const options=opts&&typeof opts==="object"?opts:{};const path=currentPath();const accountContext=captureSocialAccountContext();if(!options.silent&&!list.children.length){list.appendChild(el("div","mk-comments-loading","Loading comments..."));}
const key=`${path}:${accountContext.visitorId}:${accountContext.accountKey}:${accountContext.owner}`;let cached=commentListData.get(list);if(!cached||cached.key!==key||cached.epoch!==commentsMemoEpoch){cached={key,epoch:commentsMemoEpoch,data:null,pending:null};commentListData.set(list,cached);}
let data=options.reuseData?cached.data:null;if(!data){if(!cached.pending)cached.pending=fetchCommentsForPath(path,{fresh:true}).finally(()=>{cached.pending=null;});try{data=await cached.pending;}catch(_){data=null;}
if(commentsResponseIsValid(data))cached.data=data;}
if(!list.isConnected||!socialAccountContextIsCurrent(accountContext)||path!==currentPath()||commentListData.get(list)!==cached||cached.epoch!==commentsMemoEpoch)return;if(!commentsResponseIsValid(data)){const oldError=list.querySelector(".mk-comments-load-error");if(oldError)oldError.remove();const error=el("div","mk-comments-empty mk-comments-load-error");error.appendChild(el("div","","Could not load comments."));const retry=el("button","mk-comment-small-btn","Retry");retry.type="button";retry.addEventListener("click",()=>{if(!list.isConnected||path!==currentPath()||!socialAccountContextIsCurrent(accountContext))return;return loadCommentsInto(list,{keepExistingOnError:true});});error.appendChild(retry);if(list.querySelector(".mk-comment-card"))list.appendChild(error);else list.replaceChildren(error);return;}
const comments=data.comments;const frag=document.createDocumentFragment();if(!comments.length){frag.appendChild(commentEmptyNode());list.replaceChildren(frag);return;}
...
if(!["ArrowDown","ArrowUp","Home","End"].includes(event.key)||sortMenu.hidden)return;const options=Array.from(sortMenu.querySelectorAll(".mk-comments-sort-option"));if(!options.length)return;event.preventDefault();const currentIndex=options.indexOf(document.activeElement);let nextIndex=currentIndex;if(event.key==="Home")nextIndex=0;else if(event.key==="End")nextIndex=options.length-1;else if(event.key==="ArrowDown")nextIndex=currentIndex<0?0:(currentIndex+1)%options.length;else nextIndex=currentIndex<0?options.length-1:(currentIndex-1+options.length)%options.length;options[nextIndex].focus();});sort.addEventListener("focusout",()=>{window.setTimeout(()=>{if(!sort.contains(document.activeElement))setSortOpen(false);},0);});section.appendChild(head);function showMainComposer(){const previousText=composer.querySelector("textarea");const draft=previousText?String(previousText.value||""):"";const restoreFocus=previousText&&document.activeElement===previousText;const selectionStart=previousText&&previousText.selectionStart;const selectionEnd=previousText&&previousText.selectionEnd;composer.innerHTML="";composer.appendChild(formNode("",(res)=>{prependCommentOptimistically(list,res,()=>loadCommentsInto(list,{silent:true,keepExistingOnError:true}));if(readLocalProfile().name&&composer.querySelector(".mk-comment-name-input"))showMainComposer();}));if(draft){const nextText=composer.querySelector("textarea");if(nextText){nextText.value=draft;try{nextText.dispatchEvent(new Event("input",{bubbles:true}));}catch(_){}
if(restoreFocus){try{nextText.focus({preventScroll:true});}catch(_){}
try{nextText.setSelectionRange(selectionStart,selectionEnd);}catch(_){}}}}}
...
function renderPublicProfileRows(items,type){let arr=Array.isArray(items)?items:[];if(type==="actions")arr=arr.filter((item)=>item&&isActiveSavedPageAction(item.action));if(!arr.length){if(type==="readiness"){return`<div class="mk-local-activity-empty">No shared readiness to show.</div>`;}
...
function renderPublicProfileSection(titleText,html){return`<details class="mk-public-profile-section"><summary tabindex="0"><span class="mk-public-profile-section-title">${escapeHtml(titleText)}</span><span class="mk-public-profile-section-count"></span></summary><div class="mk-public-profile-section-body">${html}</div></details>`;}
function updatePublicProfileSectionScrollState(root){if(!root)return;root.querySelectorAll(".mk-public-profile-section").forEach((sec)=>{const body=sec.querySelector(".mk-public-profile-section-body");if(!body)return;const rows=body.querySelectorAll(".mk-local-activity-row").length;const count=sec.querySelector(".mk-public-profile-section-count");if(count)count.textContent=rows?`${rows} shown`:"Nothing shared";const isEmpty=rows===0&&!!body.querySelector(".mk-local-activity-empty");const textLen=String(body.textContent||"").replace(/\s+/g," ").trim().length;const shouldScroll=!isEmpty&&(rows>=5||textLen>900);sec.classList.toggle("mk-public-profile-section--scroll",shouldScroll);if(!shouldScroll){try{body.scrollTop=0;}catch(_){}}});}
...
const imageVars=["--mk-public-profile-theme-header-image","--mk-public-profile-theme-bg-image","--mk-public-profile-theme-header-position","--mk-public-profile-theme-bg-position","--mk-public-profile-theme-header-blur","--mk-public-profile-theme-bg-blur","--mk-public-profile-theme-header-overlay","--mk-public-profile-theme-bg-opacity"];const paletteVars=["--mk-public-profile-palette-header","--mk-public-profile-palette-body","--mk-public-profile-palette-card","--mk-public-profile-palette-accent","--mk-public-profile-palette-border","--mk-public-profile-palette-header-fg","--mk-public-profile-palette-body-fg"];let imagePalette={};if(cfg&&cfg.type!=="palette"){try{const imageCfg=publicProfileImageThemeConfig(cfg.id);imagePalette=imageCfg&&imageCfg.interfaceTheme||{};}catch(_){}}
const mode=cfg?(cfg.type==="palette"?cfg.themeBase:imagePalette.themeBase):((document.body&&document.body.getAttribute("data-md-color-scheme"))||document.documentElement.getAttribute("data-md-color-scheme"));const dark=mode==="slate";const tone=cfg&&cfg.publicProfileTheme||{};const accent=tone.accent||imagePalette.accent||(dark?"#93c5fd":"#315d94");modal.setAttribute("data-wiki-profile-mode",dark?"dark":"light");const colors={"--wp-ground":dark?"#111d2c":"#edf1f4","--wp-paper":dark?"#1b2a3b":"#fffefd","--wp-ink":tone.bodyForeground||(dark?"#e8eef6":"#243247"),"--wp-muted":dark?"#b5c4d5":"#536277","--wp-line":dark?"#45586c":"#c5d0db","--wp-accent":accent,"--wp-primary":dark?`color-mix(in srgb, ${accent} 78%, #e8f2ff)`:`color-mix(in srgb, ${accent} 68%, #15283e)`,"--wp-primary-ink":dark?"#112033":"#ffffff"};Object.keys(colors).forEach((key)=>modal.style.setProperty(key,colors[key]));if(!cfg){modal.removeAttribute("data-public-profile-theme");modal.removeAttribute("data-public-profile-palette-theme");modal.removeAttribute("data-public-profile-palette-mode");imageVars.concat(paletteVars).forEach((name)=>modal.style.removeProperty(name));return;}
...
async function openPublicProfile(name,options){const opts=options&&typeof options==="object"?options:{};const nm=cleanProfileNameLocal(name||"");const requestedAccountKey=String(opts.accountKey||opts.account_key||opts.account||"").replace(/^user:/i,"").trim();if(!nm&&!requestedAccountKey)return;const viewerKey=()=>String((readLocalProfile()||{}).accountKey||"").replace(/^user:/i,"").trim().toLowerCase();const openedViewerKey=viewerKey();ensureStylesOnce();const existing=document.querySelector(".mk-public-profile-modal");if(existing){if(typeof existing.__mkClosePublicProfile==="function")existing.__mkClosePublicProfile();else existing.remove();}
const modal=document.createElement("div");modal.className="mk-local-activity-modal mk-public-profile-modal";modal.setAttribute("data-wiki-profile","");modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.setAttribute("aria-busy","true");modal.innerHTML=`<div class="mk-local-activity-panel" role="document"><div class="mk-local-activity-head"><div><div class="mk-local-activity-title">Public profile</div><div class="mk-local-activity-sub" role="status">Loading…</div></div><div class="mk-local-activity-head-actions"><button type="button" class="mk-public-profile-theme-shortcut mk-comment-icon-btn" hidden ${iconButtonAttrs("Public profile themes")}>${iconButtonHtml("clothes", "Public profile themes")}</button><button type="button" class="mk-local-activity-close mk-comment-icon-btn" ${iconButtonAttrs("Close")}>${iconButtonHtml("x", "Close")}</button></div></div><div class="mk-local-activity-body"></div></div>`;applyPublicProfileImageTheme(modal,"");let closed=false;let releaseFocus=()=>{};let cleanupPublicProfileCosmeticListeners=()=>{};const cosmeticTimers=[];document.body.appendChild(modal);try{document.documentElement.classList.add("mk-local-activity-open");document.body.classList.add("mk-local-activity-open");if(!mkLocalIsTouchLikeViewport())lockPageBehindModal();}catch(_){}
...
const body=modal.querySelector(".mk-local-activity-body");const sub=modal.querySelector(".mk-local-activity-sub");const refreshOnCloudSync=(ev)=>{if(viewerKey()!==openedViewerKey){close();return;}
if(!ev||!ev.detail||ev.detail.type!=="cloud-sync")return;if(body)refreshActivityLinkTitles(body);};window.addEventListener("mk-local-activity-change",refreshOnCloudSync);const close=()=>{if(closed)return;closed=true;try{closeQuickShopMenu();}catch(_){}
window.removeEventListener("mk-local-activity-change",refreshOnCloudSync);window.removeEventListener("pagehide",close);cleanupPublicProfileCosmeticListeners();cosmeticTimers.forEach((timer)=>window.clearTimeout(timer));modal.remove();releaseFocus();try{if(!document.querySelector(".mk-local-activity-modal")){document.documentElement.classList.remove("mk-local-activity-open");document.body.classList.remove("mk-local-activity-open");unlockPageBehindModal();mkLocalUnbindViewportMetrics();}}catch(_){}};modal.__mkClosePublicProfile=close;releaseFocus=bindAccountDialogFocus(modal,close,".mk-local-activity-title");window.addEventListener("pagehide",close);modal.addEventListener("click",(ev)=>{if(ev.target===modal)close();});modal.querySelector(".mk-local-activity-close").addEventListener("click",close);const profileThemeBtn=modal.querySelector(".mk-public-profile-theme-shortcut");if(profileThemeBtn){profileThemeBtn.addEventListener("click",(ev)=>{try{ev.preventDefault();ev.stopPropagation();}catch(_){}
try{openQuickShopMenu(profileThemeBtn,{categories:["Public Profile themes"],title:"Profile themes"});}catch(_){}});}
let data;try{data=await fetchPublicProfile(nm,opts);}catch(_){data=null;}
if(closed||!modal.isConnected)return;if(viewerKey()!==openedViewerKey){close();return;}
modal.setAttribute("aria-busy","false");const localForSelfPreview=readLocalProfile()||{};const isRequestedSelf=!!(requestedAccountKey&&localForSelfPreview.accountKey&&requestedAccountKey.toLowerCase()===String(localForSelfPreview.accountKey||"").toLowerCase());if((!data||!data.ok)&&(opts.selfPreview||isRequestedSelf)){const local=localForSelfPreview;const root=document.documentElement;data={ok:true,profile:Object.assign({},local,{name:local.name||nm||requestedAccountKey}),comments:[],actions:[],visits:[],level:1,totalScore:0,viewerIsConnection:false,equippedCosmetics:{profile_frame:root&&root.getAttribute?root.getAttribute("data-mk-profile-frame")||"":"",profile_background:root&&root.getAttribute?root.getAttribute("data-mk-profile-background")||"":"",public_profile_theme:root&&root.getAttribute?root.getAttribute("data-mk-public-profile-theme-item")||"":""}};}
...
try{const fx=(data.equippedCosmetics||prof.equippedCosmetics||{});const localForCosmetics=readLocalProfile()||{};const localAccountKey=String(localForCosmetics.accountKey||localForCosmetics.account_key||"").replace(/^user:/i,"").trim().toLowerCase();const profileAccountKey=String(prof.accountKey||prof.account_key||prof.account||"").replace(/^user:/i,"").trim().toLowerCase();const requestedKey=String(requestedAccountKey||"").replace(/^user:/i,"").trim().toLowerCase();const isSelfCosmeticPreview=!!(opts.selfPreview||opts.source==="shop-preview"||(localAccountKey&&profileAccountKey&&localAccountKey===profileAccountKey)||(localAccountKey&&requestedKey&&localAccountKey===requestedKey));let selfThemeChanged=false;const applyModalCosmetics=()=>{if(closed||!modal.isConnected)return;try{modal.removeAttribute("data-profile-frame");modal.removeAttribute("data-profile-background");const rootThemeItem=document.documentElement&&document.documentElement.getAttribute?document.documentElement.getAttribute("data-mk-public-profile-theme-item")||"":"";const itemId=isSelfCosmeticPreview?(selfThemeChanged?rootThemeItem:(rootThemeItem||fx.public_profile_theme||"")):(fx.public_profile_theme||"");applyPublicProfileImageTheme(modal,itemId);}catch(_){}};applyModalCosmetics();if(isSelfCosmeticPreview){const onCosmeticChange=()=>{if(closed||!modal.isConnected)return;selfThemeChanged=true;try{if(window.MkAccountData&&typeof window.MkAccountData.applyEquippedCosmetics==="function")window.MkAccountData.applyEquippedCosmetics();}catch(_){}
[0,80,220].forEach((delay)=>{try{cosmeticTimers.push(window.setTimeout(applyModalCosmetics,delay));}catch(_){}});};try{window.addEventListener("mk-shop-trial-change",onCosmeticChange);}catch(_){}
...
data=mergePublicProfileWithLocalSelfRows(data,prof,nm,requestedAccountKey,opts);const bio=cleanProfileBioLocal(prof.bio||prof.description||prof.intro||"");const xpOverride=opts&&opts.rankingXp&&typeof opts.rankingXp==="object"?opts.rankingXp:null;const displayLevel=xpOverride&&xpOverride.level!=null?xpOverride.level:(data.level||1);const displayTotalScore=xpOverride&&xpOverride.totalXp!=null?xpOverride.totalXp:(data.totalScore||0);body.innerHTML=`<div class="mk-local-profile-card"><div class="mk-local-profile-preview mk-public-profile-preview">${avatarHtml(prof.name || nm, prof.avatar || "", prof.avatarFrame || "level-1")}<div class="mk-public-profile-main"><div class="mk-local-profile-name">${escapeHtml(prof.name || nm)}</div><div class="mk-local-activity-meta"><span class="mk-public-profile-level">Level ${escapeHtml(displayLevel)}</span><span>${escapeHtml(displayTotalScore)} XP</span></div></div>${bio ? `<div class="mk-public-profile-bio">${escapeHtml(bio)}</div>` : ""}<div class="mk-public-profile-relationship"><span class="mk-public-profile-connection-status" role="status" aria-live="polite"></span><button type="button" class="mk-comment-small-btn mk-public-profile-connect" title="Add study connection" aria-label="Add study connection">${iconButtonHtml("link", "Add study connection")}</button></div></div></div><div class="mk-public-profile-shared">${isPublicProfileSelfTarget(prof, nm, requestedAccountKey, opts) ? "Your learning" : "Shared learning"}</div>${renderPublicProfileSection("Recent comments", renderPublicProfileRows(data.comments, "comments"))}${renderPublicProfileSection("Saved pages", renderPublicProfileRows(data.actions, "actions"))}${renderPublicProfileSection("Recent visits", renderPublicProfileRows(data.visits, "visits"))}${renderPublicProfileSection("Course readiness", renderPublicProfileRows(data.readiness, "readiness"))}`;updatePublicProfileSectionScrollState(body);body.querySelectorAll(".mk-public-profile-section").forEach((sec)=>{sec.addEventListener("toggle",()=>{const inner=sec.querySelector(".mk-public-profile-section-body");updatePublicProfileSectionScrollState(body);if(sec.open){body.querySelectorAll(".mk-public-profile-section[open]").forEach((other)=>{if(other!==sec)other.open=false;});if(inner)inner.scrollTop=0;try{void sec.offsetHeight;void(inner&&inner.offsetHeight);}catch(_){}
window.setTimeout(()=>{if(closed||!modal.isConnected)return;try{sec.scrollIntoView({block:"nearest",inline:"nearest"});}catch(_){}},0);}});});refreshActivityLinkTitles(body);const connectBtn=body.querySelector(".mk-public-profile-connect");if(connectBtn){const localProfile=readLocalProfile()||{};const localKey=String(localProfile.accountKey||"").replace(/^user:/i,"").trim().toLowerCase();const profileKey=String(prof.accountKey||requestedAccountKey||"").replace(/^user:/i,"").trim();let connectionStatus=String(data.connectionStatus||(data.viewerIsConnection?"accepted":"none")).toLowerCase();if(localKey&&profileKey&&localKey===profileKey.toLowerCase())connectionStatus="self";const paintConnectButton=(status)=>{const s=String(status||"none").toLowerCase();const statusLabel=body.querySelector(".mk-public-profile-connection-status");if(statusLabel)statusLabel.textContent=s==="self"?"Your profile":s==="accepted"?"Study connection":s==="incoming"?"Wants to connect with you":s==="outgoing"?"Request sent":"";connectBtn.hidden=s==="accepted"||s==="self";connectBtn.disabled=s==="outgoing";const label=s==="incoming"?"Accept study connection":s==="outgoing"?"Connection request sent":"Add study connection";connectBtn.title=label;connectBtn.setAttribute("aria-label",label);connectBtn.innerHTML=`${iconButtonHtml(s === "incoming" ? "check" : "link", label)}<span>${escapeHtml(label)}</span>`;};paintConnectButton(connectionStatus);connectBtn.addEventListener("click",async()=>{if(connectBtn.disabled||connectBtn.hidden)return;const statusLabel=body.querySelector(".mk-public-profile-connection-status");const actionHadFocus=document.activeElement===connectBtn;const ownsActionFocus=()=>{const dialogs=Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]')).filter(node=>node.isConnected&&node.getClientRects().length);return actionHadFocus&&statusLabel&&document.activeElement===statusLabel&&dialogs[dialogs.length-1]===modal;};connectBtn.setAttribute("aria-busy","true");if(statusLabel){statusLabel.removeAttribute("data-error");statusLabel.textContent=connectionStatus==="incoming"?"Accepting request…":"Sending request…";if(actionHadFocus){statusLabel.tabIndex=-1;statusLabel.focus({preventScroll:true});}}
connectBtn.disabled=true;let res;try{res=connectionStatus==="incoming"?await respondConnection(profileKey,"accept"):await requestConnection(prof.name||nm);}catch(_){res=null;}
if(closed||!modal.isConnected)return;if(viewerKey()!==openedViewerKey){close();return;}
connectBtn.disabled=false;connectBtn.setAttribute("aria-busy","false");if(!res||!res.ok){if(statusLabel){statusLabel.setAttribute("data-error","true");statusLabel.textContent=(res&&res.error)||(connectionStatus==="incoming"?"Could not accept connection request. Try again.":"Could not send connection request. Try again.");}
if(ownsActionFocus())connectBtn.focus({preventScroll:true});return;}
connectionStatus=res.status==="accepted"?"accepted":"outgoing";paintConnectButton(connectionStatus);});}}
...
let accountDialogSequence=0;function bindAccountDialogFocus(modal,close,titleSelector){const previous=document.activeElement;const previousId=previous&&previous.id;const title=modal.querySelector(titleSelector);const id=`mk-account-dialog-${++accountDialogSequence}`;if(title){title.id=`${id}-title`;modal.setAttribute("aria-labelledby",title.id);}
modal.tabIndex=-1;const isTop=()=>{const dialogs=Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]'));return dialogs[dialogs.length-1]===modal;};const ownedQuickMenu=()=>{const menu=document.getElementById("mk-quick-shop-menu");return menu&&menu.getAttribute("role")==="menu"&&menu.__mkQuickShopAnchor&&modal.contains(menu.__mkQuickShopAnchor)?menu:null;};const containsFocus=(node)=>modal.contains(node)||!!(ownedQuickMenu()&&ownedQuickMenu().contains(node));const controlsIn=(host)=>Array.from(host.querySelectorAll('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')).filter((node)=>node.getClientRects().length&&!node.closest('[hidden]'));const focusable=()=>{const items=controlsIn(modal),menu=ownedQuickMenu();if(menu){const anchorIndex=items.indexOf(menu.__mkQuickShopAnchor);const menuItems=controlsIn(menu);items.splice(anchorIndex<0?items.length:anchorIndex+1,0,...(menuItems.length?menuItems:[menu]));}
return items;};const onKey=(event)=>{if(event.isComposing||event.keyCode===229)return;if(!isTop())return;if(event.key==="Escape"){event.preventDefault();event.stopPropagation();if(ownedQuickMenu())closeQuickShopMenu({returnFocus:true,lostMenuFocus:true});else close();return;}
if(event.key!=="Tab")return;const items=focusable();const first=items[0]||modal,last=items[items.length-1]||modal;if(ownedQuickMenu()&&items.length){const index=items.indexOf(document.activeElement);event.preventDefault();(index<0?(event.shiftKey?last:first):items[(index+(event.shiftKey?-1:1)+items.length)%items.length]).focus();}else if(!containsFocus(document.activeElement)||(event.shiftKey?document.activeElement===first:document.activeElement===last)||!items.length){event.preventDefault();(event.shiftKey?last:first).focus();}};const onFocus=(event)=>{if(isTop()&&!containsFocus(event.target))(focusable()[0]||modal).focus();};document.addEventListener("keydown",onKey,true);document.addEventListener("focusin",onFocus,true);window.requestAnimationFrame(()=>{if(modal.isConnected&&isTop()&&!containsFocus(document.activeElement))(focusable()[0]||modal).focus();});return(options)=>{document.removeEventListener("keydown",onKey,true);document.removeEventListener("focusin",onFocus,true);if(ownedQuickMenu())closeQuickShopMenu();if(options&&options.restoreFocus===false)return;const canRestore=(node)=>node&&node.isConnected&&!node.disabled&&typeof node.focus==="function"&&node.getClientRects().length&&!node.closest('[hidden],[inert]')&&(typeof window.getComputedStyle!=="function"||window.getComputedStyle(node).visibility==="visible");const target=canRestore(previous)?previous:(previousId?document.getElementById(previousId):null);if(canRestore(target))target.focus();};}
function openLocalMiniModal(titleText,options){const opts=options||{};ensureStylesOnce();const modal=document.createElement("div");modal.className="mk-local-mini-modal";if(opts.accountEditor)modal.classList.add("mk-account-editor-modal","mk-account-workspace");modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.innerHTML=`
      ...
      </div>`;document.body.appendChild(modal);const panel=modal.querySelector(".mk-local-mini-panel");if(opts.accountEditor&&panel)panel.classList.add("mk-account-editor-panel");const closeCallbacks=[];let releaseFocus=()=>{};let didClose=false;let busy=false,busyControls=[];const current=()=>!didClose&&modal.isConnected&&(!opts.isCurrent||opts.isCurrent());const close=(settings)=>{if(didClose)return;const dialogs=Array.from(document.querySelectorAll('[role="dialog"][aria-modal="true"]'));const restoreFocus=current()&&dialogs[dialogs.length-1]===modal&&!(settings&&settings.restoreFocus===false);didClose=true;try{closeCallbacks.splice(0).forEach((fn)=>{try{if(typeof fn==="function")fn();}catch(_){}});}catch(_){}
try{modal.remove();}catch(_){}
releaseFocus({restoreFocus});};releaseFocus=bindAccountDialogFocus(modal,close,".mk-local-mini-title");modal.addEventListener("click",(ev)=>{if(ev.target===modal)close();});const closeBtn=modal.querySelector(".mk-local-activity-close");if(closeBtn)closeBtn.addEventListener("click",close);if(opts.accountEditor){const viewport=window.visualViewport;const updateViewport=()=>{if(!current())return;modal.style.setProperty("--wa-editor-top",`${Number(viewport && viewport.offsetTop || 0)}px`);modal.style.setProperty("--wa-editor-left",`${Number(viewport && viewport.offsetLeft || 0)}px`);modal.style.setProperty("--wa-editor-height",`${Number(viewport && viewport.height || window.innerHeight)}px`);modal.style.setProperty("--wa-editor-width",`${Number(viewport && viewport.width || window.innerWidth)}px`);};window.addEventListener("resize",updateViewport);if(viewport){viewport.addEventListener("resize",updateViewport);viewport.addEventListener("scroll",updateViewport);}
closeCallbacks.push(()=>{window.removeEventListener("resize",updateViewport);if(viewport){viewport.removeEventListener("resize",updateViewport);viewport.removeEventListener("scroll",updateViewport);}});updateViewport();}
return{modal,body:modal.querySelector(".mk-local-mini-body"),close,current,get busy(){return busy;},setBusy(button,value){busy=!!value;button.setAttribute("aria-disabled",String(busy));modal.setAttribute("aria-busy",String(busy));if(busy&&!busyControls.length){busyControls=Array.from(modal.querySelectorAll("input,textarea,select")).map(node=>[node,node.disabled]);busyControls.forEach(([node])=>{node.disabled=true;});}else if(!busy){busyControls.forEach(([node,disabled])=>{node.disabled=disabled;});busyControls=[];}},setActions:(actions)=>{(opts.accountEditor?panel:modal.querySelector(".mk-local-mini-body")).appendChild(actions);},onClose:(fn)=>{if(typeof fn==="function"){if(didClose)fn();else closeCallbacks.push(fn);}}};}
...
function renderTodayXpBlock(score){const state=todayDailyXpState(score||null);const cap=Math.max(0,Number((state&&state.cap)||(score&&score.dailyCap)||0));const counted=Math.min(cap||0,Math.max(0,Number(state&&state.counted||0)));const coinsToday=todayCurrencyEarned(score||null);const currency=accountCurrencySummary(score||null);const remaining=Math.max(0,cap-counted);const note=cap>0?(remaining<=0.05?`XP cap reached. ${currency.name} still keep earning after the cap.`:`${formatAccountXp(remaining)} XP left before today’s XP cap`):`${currency.name} have no daily cap.`;return`<div class="mk-level-summary-block mk-level-daily-block">
      <div class="mk-level-summary-head">
        <div>
          <div class="mk-level-kicker">Today (UTC)</div>
      ...
      <div class="mk-level-daily-note">${escapeHtml(note)}. ${escapeHtml(accountDayResetNote())}</div>
    </div>`;}
function renderLevelSyncBar(score,opts){const health=accountProgressSyncHealth();const message=health.error?"Not synced. New changes are saved on this device; retry in Account → Sync & learning data.":health.syncing?"Syncing account history…":health.pending?"New changes are waiting to sync.":"";return message?`<div class="mk-level-sync-status" role="status">${escapeHtml(message)}</div>`:"";}
function accountProgressSyncHealth(){try{return window.MkAccountData&&typeof window.MkAccountData.getSyncHealth==="function"?window.MkAccountData.getSyncHealth():{};}catch(_){return{};}}
function accountDayResetNote(){const now=new Date();const reset=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()+1));return`Daily totals reset at 00:00 UTC (${reset.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit", timeZoneName: "short" })} in your timezone).`;}
...
function renderAccountLevelModalBody(body,score,loading,opts){if(!body)return;bindAccountLevelOpenState(body);const openState=captureAccountLevelOpenState(body);const health=accountProgressSyncHealth();if(health.initialPending){body.innerHTML=`<div class="mk-level-loading-card" role="status"><strong>${health.error ? "Account history has not synced." : "Loading your account history…"}</strong><span>Your level and XP will appear after this device downloads the account history. This device's partial total is hidden until sync completes. ${health.error ? "Retry in Account → Sync & learning data." : ""}</span></div>`;bindLevelPanelAdaptiveHeight(body);return;}
...
window.removeEventListener("mk-account-xp-change",onXp);window.removeEventListener("mk-account-sync-health",onSyncHealth);});}catch(_){}}
async function openAvatarFramePicker(scoreHint,onSaved,options){const opts=options||{},accountContext=captureSocialAccountContext();const owned=()=>socialAccountContextIsCurrent(accountContext);if(!owned()||(opts.isCurrent&&!opts.isCurrent()))return;let curr=readLocalProfile();if(!curr.name){window.alert("Please choose a username before changing the avatar frame.");return;}
const mini=opts.openMini?opts.openMini("Choose avatar frame"):openLocalMiniModal("Choose avatar frame",opts);if(opts.onDispose)opts.onDispose(()=>mini.close({restoreFocus:false}));let saving=false;const current=()=>owned()&&mini.current()&&(!opts.isCurrent||opts.isCurrent());if(mini.modal)mini.modal.classList.add("mk-avatar-frame-modal");const framePanel=mini.modal&&mini.modal.querySelector(".mk-local-mini-panel");if(framePanel)framePanel.classList.add("mk-avatar-frame-panel");if(mini.body)mini.body.classList.add("mk-avatar-frame-body");if(mini.body)mini.body.innerHTML=`<div class="mk-local-avatar-note">Checking your current level and unlocked frames…</div>`;let score=null;try{if(scoreHint&&accountXpCacheMatchesCurrentProfile(scoreHint))score=scoreHint;}catch(_){score=scoreHint||null;}
...
if(current()){mini.close();if(typeof onSaved==="function"){try{onSaved(savedProfile||res.profile||null);}catch(_){}}}
...
function renderGuestAccountOnboarding(host,profile,refreshInfo){if(!host)return;ensureGuestAccountOnboardingStyles();const curr=profile&&typeof profile==="object"?profile:readLocalProfile();const profileView=host.__mkProfileView;const current=()=>!profileView||profileView.current();let selectedAvatar="";const card=el("div","mk-guest-account-card");const hero=el("div","mk-guest-account-hero");hero.appendChild(el("div","mk-guest-account-title","Save your learning progress and continue on another device."));card.appendChild(hero);const form=el("div","mk-guest-account-form");const nameLabel=el("label","mk-guest-account-label","Your display name");form.appendChild(nameLabel);const input=document.createElement("input");input.type="text";input.maxLength=40;input.autocomplete="nickname";input.placeholder="For example: Alex";input.value=curr.name||"";input.className="mk-guest-account-input";input.id=`mk-guest-display-name-${++accountDialogSequence}`;nameLabel.htmlFor=input.id;form.appendChild(input);form.appendChild(el("div","mk-guest-account-label","Default avatar"));const avatarRow=el("div","mk-guest-account-avatar-row");const preview=el("div","mk-guest-account-avatar-preview");const paintPreview=()=>{preview.innerHTML=avatarHtml(input.value||"You","",curr.avatarFrame||"level-1");};input.addEventListener("input",paintPreview);avatarRow.appendChild(preview);avatarRow.appendChild(el("div","mk-local-avatar-note","New accounts start with initials. Emoji avatars can be unlocked later with EORbits."));form.appendChild(avatarRow);paintPreview();const status=el("div","mk-guest-account-status","");form.appendChild(status);card.appendChild(form);const actions=el("div","mk-guest-account-actions");actions.appendChild(el("div","mk-guest-account-note","Local progress is imported after creation."));const connect=iconButton("mk-comment-small-btn","key","Connect account");connect.type="button";const scanConnect=iconButton("mk-comment-small-btn","scan","Scan QR");scanConnect.type="button";const save=iconButton("mk-comment-primary-btn","check","Create account");save.type="button";actions.appendChild(connect);actions.appendChild(save);card.appendChild(actions);connect.addEventListener("click",async()=>{if(!current()||connect.disabled)return;const nm=window.prompt("Username on the existing account:",curr.name||input.value||"");if(!nm)return;const code=window.prompt("Access code:","");if(!code)return;if(!current())return;connect.disabled=true;scanConnect.disabled=true;status.textContent="Connecting…";status.classList.remove("is-error");if(profileView)profileView.beginRename(nm);let res;try{res=await claimAccountAccessCode(nm,code,"guest-onboarding-access-code");}
catch(_){res={ok:false,error:"Could not connect. Try again."};}
if(profileView)profileView.finishRename(res);connect.disabled=false;scanConnect.disabled=false;if(res&&res.ok&&res.workspaceReloadRequired&&profileView&&profileView.ownsHost()){status.textContent="Connected. Reloading…";return;}
if(!current())return;if(!res||!res.ok){status.textContent=(res&&res.error)||"Could not connect to this account.";status.classList.add("is-error");return;}
try{window.MkGuestAccess&&window.MkGuestAccess.refreshBadges&&window.MkGuestAccess.refreshBadges();}catch(_){}
try{window.dispatchEvent(new CustomEvent("mk-guest-account-change",{detail:{hasAccount:true}}));}catch(_){}
if(typeof refreshInfo==="function")refreshInfo();});save.addEventListener("click",async()=>{if(!current()||save.disabled)return;const nm=cleanProfileNameLocal(input.value||"");if(!nm){status.textContent="Please choose a display name first.";status.classList.add("is-error");try{input.focus();}catch(_){}
return;}
save.disabled=true;connect.disabled=true;scanConnect.disabled=true;status.textContent="Creating account and importing progress...";status.classList.remove("is-error");const privacy=defaultNewAccountPrivacy();if(profileView)profileView.beginRename(nm);let res;try{res=await saveOnlineProfile(nm,"",{askPrivacy:false,privacy,isPublic:false,bio:curr.bio||""});}
catch(_){res={ok:false,error:"Could not create the account. Try again."};}
if(profileView)profileView.finishRename(res);save.disabled=connect.disabled=scanConnect.disabled=false;if(!current())return;if(!res||!res.ok){save.disabled=false;connect.disabled=false;scanConnect.disabled=false;status.textContent=(res&&res.error)||"Could not create the account.";status.classList.add("is-error");return;}
...
if(!current())return;try{window.MkGuestAccess&&window.MkGuestAccess.refreshBadges&&window.MkGuestAccess.refreshBadges();}catch(_){}
try{window.dispatchEvent(new CustomEvent("mk-guest-account-change",{detail:{hasAccount:true}}));}catch(_){}
save.disabled=false;connect.disabled=false;scanConnect.disabled=false;if(typeof refreshInfo==="function")refreshInfo();});host.appendChild(card);setTimeout(()=>{if(current()&&input.isConnected){try{input.focus();if(!input.value)input.select();}catch(_){}}},80);}
function ensureAccountShopStylesOnce(){const id="mk-account-shop-style-v1";if(document.getElementById(id))return;const st=document.createElement("style");st.id=id;st.textContent=`
      ...
      /* Store browse collections: scoped structure, inherited theme colors. */
      .mk-account-shop-card .mk-account-shop-tabs.mk-account-shop-browse-tabs{
        display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px;
      }
      .mk-account-shop-card .mk-account-shop-tabs.mk-account-shop-browse-tabs .mk-account-shop-category-tab{
        display:flex; flex-direction:column; align-items:flex-start; justify-content:center;
        min-width:0; min-height:2.65rem; width:100%; padding:.45rem .6rem;
        text-align:left; white-space:normal; line-height:1.3; gap:3px;
        font-size:.65rem; font-weight:700; border-radius:12px;
      }
      .mk-account-shop-browse-tabs .mk-account-shop-browse-count{
        display:block; font-size:.55rem; line-height:1.3; font-weight:500; opacity:.72;
      }
      .mk-account-shop-card .mk-account-shop-browse-description{
        margin:.3rem 0 0; font-size:.65rem; line-height:1.5; color:var(--md-default-fg-color--light);
      }
      .mk-account-shop-card .mk-account-shop-browse-filter{
        display:flex; align-items:center; flex-wrap:wrap; gap:8px 12px;
        font-size:.65rem; font-weight:650; color:var(--md-default-fg-color--light);
      }
      .mk-account-shop-card .mk-account-shop-browse-select{
        max-width:100%; min-width:0; min-height:40px; padding:.38rem 2rem .38rem .65rem;
        border:1px solid color-mix(in srgb,var(--md-default-fg-color) 18%,transparent);
        border-radius:9px; background:var(--md-default-bg-color); color:var(--md-default-fg-color);
        font:inherit; cursor:pointer;
      }
      .mk-account-shop-card :is(.mk-account-shop-browse-deals,.mk-account-shop-browse-page){
        appearance:none; min-height:40px; padding:.35rem .65rem; border-radius:9px;
        border:1px solid color-mix(in srgb,var(--md-default-fg-color) 18%,transparent);
        background:var(--md-default-bg-color); color:var(--md-default-fg-color);
        font:inherit; font-size:.63rem; font-weight:650; cursor:pointer;
      }
      .mk-account-shop-card .mk-account-shop-browse-deals.is-active{
        border-color:var(--md-accent-fg-color); background:color-mix(in srgb,var(--md-accent-fg-color) 10%,var(--md-default-bg-color));
      }
      .mk-account-shop-card :is(.mk-account-shop-browse-deals,.mk-account-shop-browse-page):hover:not(:disabled){
        border-color:var(--md-accent-fg-color);
      }
      .mk-account-shop-card .mk-account-shop-browse-page:disabled{ opacity:.45; cursor:default; }
      .mk-account-shop-card .mk-account-shop-browse-pager{
        display:flex; align-items:center; justify-content:flex-end; flex-wrap:wrap; gap:8px;
        border-top:1px solid color-mix(in srgb,var(--md-default-fg-color) 10%,transparent); padding-top:12px;
      }
      .mk-account-shop-card .mk-account-shop-browse-status{
        margin-right:auto; font-size:.61rem; line-height:1.5; color:var(--md-default-fg-color--light);
      }
      .mk-account-shop-card :is(.mk-account-shop-browse-select,.mk-account-shop-browse-deals,.mk-account-shop-browse-page):focus-visible{
        outline:2px solid var(--md-accent-fg-color); outline-offset:3px;
      }
      @media(max-width:720px){
        .mk-account-shop-card .mk-account-shop-tabs.mk-account-shop-browse-tabs{ grid-template-columns:repeat(2,minmax(0,1fr)); }
        .mk-account-shop-card .mk-account-shop-browse-select{ flex:1 1 180px; }
        .mk-account-shop-card .mk-account-shop-browse-status{ flex-basis:100%; }
      }
      @media(pointer:coarse){
        .mk-account-shop-card :is(.mk-account-shop-browse-select,.mk-account-shop-browse-deals,.mk-account-shop-browse-page){ min-height:44px; }
      }
    `.trim();document.head.appendChild(st);}
function openShopCategories(host){const set=new Set();try{const selected=readShopBrowseState(host).selectedCategory;if(selected){set.add(selected);return set;}}catch(_){}
try{(host||document).querySelectorAll(".mk-account-shop-category-tab[data-shop-category].is-active").forEach((node)=>{const cat=String(node.getAttribute("data-shop-category")||"");if(cat)set.add(cat);});if(set.size)return set;}catch(_){}
...
function shopDisplayCollections(){return[{key:"__mk_collection_reading__",label:"Reading & appearance",description:"Make your everyday reading space your own.",categories:["Interface themes","Header fonts","Body fonts","Mouse cursors","Top reading progress bars","Sidebar effects","Course index card skins"]},{key:"__mk_collection_study__",label:"Maps & study",description:"Optional visuals, celebrations and XP boosts. Core study tools stay free.",categories:["Concept connection effects","Finder builder effects","Finder effects","Map effects","Mastery effects","Quiz celebrations","Progress boosts"]},{key:"__mk_collection_community__",label:"Identity & community",description:"Personal touches for your profile and conversations.",categories:["Public Profile themes","Personal name labels","Comment reaction buttons","Account notification skins","Ranking effects","Comment effects","Emoji bundles"]},{key:"__mk_collection_audio__",label:"Audio & extras",description:"Optional music and interface sounds, on your terms.",categories:["Background music","Interface sound packs"]}];}
function buildShopDisplayModel(items,isOwned,discountMap){const rows=Array.isArray(items)?items:[];const owns=typeof isOwned==="function"?isOwned:()=>false;const discounts=discountMap||{};const byCat=new Map();rows.forEach((item)=>{const cat=String(item&&item.category||"Other").trim()||"Other";if(!byCat.has(cat))byCat.set(cat,[]);byCat.get(cat).push(item);});const categories=Array.from(byCat.keys());byCat.forEach((entries,cat)=>byCat.set(cat,shopItemsOwnedFirst(entries,owns)));const groups=shopDisplayCollections();const parentByCategory=new Map();groups.forEach((group)=>{group.categories.forEach((cat)=>parentByCategory.set(cat,group.key));byCat.set(group.key,shopItemsOwnedFirst(group.categories.flatMap((cat)=>byCat.get(cat)||[]),owns));});byCat.set("__mk_all__",shopItemsOwnedFirst(rows,owns));byCat.set("__mk_owned__",rows.filter(owns).sort((a,b)=>String(a.category||"").localeCompare(String(b.category||""))||String(a.title||a.id||"").localeCompare(String(b.title||b.id||""))));byCat.set("__mk_daily_discounts__",shopItemsOwnedFirst(rows.filter((item)=>Number(discounts[String(item.id||"")]||0)>0).sort((a,b)=>Number(discounts[b.id]||0)-Number(discounts[a.id]||0)),owns));return{byCat,groups,categories,parentByCategory};}
function readShopBrowseState(host){try{const card=(host||document).querySelector(".mk-account-shop-card[data-shop-selected]");if(card)return{selectedCategory:String(card.getAttribute("data-shop-selected")||""),page:Math.max(1,Number(card.getAttribute("data-shop-page")||1)||1)};}catch(_){}
return{selectedCategory:"",page:1};}
function shopBrowsePage(rows,page){const items=Array.isArray(rows)?rows:[];const pageSize=24;const pageCount=Math.max(1,Math.ceil(items.length/pageSize));const current=Math.max(1,Math.min(pageCount,Math.floor(Number(page)||1)));const start=(current-1)*pageSize;return{page:current,pageCount,total:items.length,start,items:items.slice(start,start+pageSize)};}
function scrollShopBrowseResults(host,pager){try{const scroller=host&&(host.closest(".mk-local-activity-body")||host.closest(".mk-local-mini-body"));if(!scroller||!pager||!pager.isConnected)return;const containerRect=scroller.getBoundingClientRect();const pagerRect=pager.getBoundingClientRect();const top=(Number(scroller.scrollTop)||0)+pagerRect.top-containerRect.top-12;if(Number.isFinite(top))scroller.scrollTop=Math.max(0,top);}catch(_){}}
const SHOP_BROWSE_REFRESH_PENDING=new WeakMap();function cancelPendingShopBrowseRefresh(host){const pending=SHOP_BROWSE_REFRESH_PENDING.get(host);if(!pending)return;SHOP_BROWSE_REFRESH_PENDING.delete(host);try{pending.control.removeEventListener("blur",pending.flush);}catch(_){}
try{document.removeEventListener("pointerdown",pending.pointerDown,true);}catch(_){}
try{document.removeEventListener("pointerup",pending.pointerUp,true);}catch(_){}
try{document.removeEventListener("pointercancel",pending.pointerUp,true);}catch(_){}
try{document.removeEventListener("focusin",pending.focusChanged,true);}catch(_){}
try{if(pending.timer!=null)window.clearTimeout(pending.timer);}catch(_){}}
function refreshAccountShop(host){if(!host||host.isConnected===false||!host.querySelector(".mk-account-shop-card"))return;const pending=SHOP_BROWSE_REFRESH_PENDING.get(host);if(pending&&(pending.scheduled||document.activeElement===pending.control))return;if(pending)cancelPendingShopBrowseRefresh(host);const active=document.activeElement;if(active&&host.contains(active)&&active.classList.contains("mk-account-shop-browse-select")){const deferred={control:active,scheduled:false,flush:null,pointers:new Set(),timer:null};const finish=()=>{deferred.timer=null;if(SHOP_BROWSE_REFRESH_PENDING.get(host)!==deferred||deferred.pointers.size)return;cancelPendingShopBrowseRefresh(host);refreshAccountShop(host);};deferred.flush=()=>{if(deferred.scheduled)return;deferred.scheduled=true;if(!deferred.pointers.size)Promise.resolve().then(finish);};deferred.pointerDown=(event)=>{if(host.isConnected===false||active.isConnected===false){cancelPendingShopBrowseRefresh(host);return;}
deferred.pointers.add(event.pointerId);};deferred.pointerUp=(event)=>{deferred.pointers.delete(event.pointerId);if(deferred.scheduled&&!deferred.pointers.size&&deferred.timer==null)deferred.timer=window.setTimeout(finish,0);};deferred.focusChanged=(event)=>{if(event.target!==active)deferred.flush();};SHOP_BROWSE_REFRESH_PENDING.set(host,deferred);active.addEventListener("blur",deferred.flush,{once:true});document.addEventListener("pointerdown",deferred.pointerDown,true);document.addEventListener("pointerup",deferred.pointerUp,true);document.addEventListener("pointercancel",deferred.pointerUp,true);document.addEventListener("focusin",deferred.focusChanged,true);return;}
renderAccountShop(host,{openCategories:openShopCategories(host)});}
const SHOP_BROWSE_EMOJI_PENDING=new WeakSet();function ensureShopBrowseEmojiData(host){if(__commentEmojiDataReady||SHOP_BROWSE_EMOJI_PENDING.has(host))return;SHOP_BROWSE_EMOJI_PENDING.add(host);Promise.resolve().then(()=>loadCommentEmojiData()).then(()=>{SHOP_BROWSE_EMOJI_PENDING.delete(host);if(__commentEmojiDataReady)refreshAccountShop(host);}).catch(()=>{SHOP_BROWSE_EMOJI_PENDING.delete(host);});}
function renderAccountShop(host,opts){if(!host)return;cancelPendingShopBrowseRefresh(host);ensureAccountShopStylesOnce();bindShopTrialDelegatedFallbackOnce();try{markDailyDiscountsSeen();}catch(_){}
try{maybeRefreshShopDynamicPrices(()=>{try{refreshAccountShop(host);}catch(_){}});}catch(_){}
try{maybeRefreshShopCloudBalance(()=>{try{refreshAccountShop(host);}catch(_){}});}catch(_){}
const options=opts&&typeof opts==="object"?opts:{};const previousBrowse=options.preserveOpen===false?{selectedCategory:"",page:1}:readShopBrowseState(host);const preservedOpen=options.preserveOpen===false?new Set():(options.openCategories instanceof Set?options.openCategories:openShopCategories(host));let focusKey=String(options.focusKey||"");if(!focusKey)try{const active=document.activeElement;if(active&&host.contains(active))focusKey=String(active.getAttribute("data-shop-browse-focus")||"");}catch(_){}
host.innerHTML="";const score=readWarmAccountXpSnapshot()||{};const currency=accountCurrencySummary(score);const profile=readLocalProfile()||{};const hasAccount=!!profile.accountKey;const card=el("div","mk-account-shop-card");const hero=el("div","mk-account-shop-hero");const copy=el("div","mk-account-shop-main");copy.innerHTML=`<div class="mk-account-shop-heading"><div class="mk-account-shop-kicker">${escapeHtml(currency.name)} store</div><details class="mk-account-shop-info"><summary aria-label="About the store" title="About the store">i</summary><div class="mk-account-shop-info-panel">${escapeHtml(currency.name)} come from study activity and sync with your account. Core study tools stay free. Store items are optional visuals or boosts. Items using the same slot replace each other. Try starts a 2-minute preview, once per item per day.</div></details></div>`;const balance=el("div","mk-account-shop-balance");balance.innerHTML=`<strong>${commentSvgIcon("eorbit", 18)}${escapeHtml(formatEorbits(currency.balance))}</strong><span>${escapeHtml(currency.name)} available</span>`;hero.appendChild(copy);hero.appendChild(balance);card.appendChild(hero);const api=window.MkAccountData||null;const catalog=api&&typeof api.shopCatalog==="function"?api.shopCatalog():[];const inventory=api&&typeof api.getShopInventory==="function"?api.getShopInventory():{owned:{},equipped:{},activeBoosts:{active:[]},activeTrials:[]};try{if(api&&typeof api.getActiveShopTrials==="function")inventory.activeTrials=api.getActiveShopTrials();}catch(_){}
const items=(catalog&&catalog.length?catalog:[{id:"local_map_3d",title:"3D View",price:500,category:"Map effects",description:"Adds a rotatable 3D view to concept maps."}]).filter((item)=>item&&!item.free&&Number(item.price||0)>0);if(items.some((item)=>item&&item.randomEmojiBundle)&&!__commentEmojiDataReady)ensureShopBrowseEmojiData(host);const activeBoostForItem=(itemId)=>inventory&&inventory.activeBoosts&&Array.isArray(inventory.activeBoosts.active)?inventory.activeBoosts.active.find((b)=>String(b.itemId||"")===String(itemId||"")):null;const itemById=new Map(items.map((it)=>[String(it&&it.id||""),it]).filter((pair)=>pair[0]));const activeTrialForSlot=(slot)=>{const wanted=String(slot||"").trim();if(!wanted||!(inventory&&Array.isArray(inventory.activeTrials)))return null;return inventory.activeTrials.find((row)=>{if(!row||row.muted||Number(row.expiresAt||0)<=Date.now())return false;const it=itemById.get(String(row.itemId||""));return it&&String(it.slot||"").trim()===wanted;})||null;};const isOwnedItem=(item)=>{const itemId=String(item&&item.id||"").trim();if(!itemId)return false;if(item&&item.consumable)return!!activeBoostForItem(itemId);return!!(inventory&&inventory.owned&&inventory.owned[itemId]);};const DISCOUNT_CAT="__mk_daily_discounts__";const OWNED_CAT="__mk_owned__";const discountInfo=(api&&typeof api.dailyDiscounts==="function")?api.dailyDiscounts():null;const discountMap=discountInfo&&typeof discountInfo==="object"?(discountInfo.map||{}):{};const model=buildShopDisplayModel(items,isOwnedItem,discountMap);const byCat=model.byCat;const discountedItems=byCat.get(DISCOUNT_CAT);const cats=[...model.groups.map((group)=>group.key),"__mk_all__",OWNED_CAT];const catLabel=(cat)=>{const group=model.groups.find((entry)=>entry.key===cat);return group?group.label:cat==="__mk_all__"?"All items":shopCategoryDisplayLabel(cat);};const selectedFromSet=(()=>{if(typeof options.selectedCategory==="string"&&byCat.has(options.selectedCategory))return options.selectedCategory;for(const c of preservedOpen||[])if(byCat.has(c))return c;return"";})();const selectedCat=selectedFromSet||cats.find((cat)=>(byCat.get(cat)||[]).length)||cats[0];const selectedTop=cats.includes(selectedCat)?selectedCat:model.parentByCategory.get(selectedCat)||"__mk_all__";const pageState=shopBrowsePage(byCat.get(selectedCat),options.page!=null?options.page:previousBrowse.selectedCategory===selectedCat?previousBrowse.page:1);card.setAttribute("data-shop-selected",selectedCat);card.setAttribute("data-shop-page",String(pageState.page));const tabs=el("div","mk-account-shop-tabs mk-account-shop-browse-tabs");tabs.setAttribute("role","group");tabs.setAttribute("aria-label","Store collections");cats.forEach((cat)=>{const active=selectedCat!==DISCOUNT_CAT&&cat===selectedTop;const tab=el("button","mk-account-shop-category-tab"+(active?" is-active":""),"");tab.type="button";tab.setAttribute("data-shop-category",cat);tab.setAttribute("data-shop-browse-focus",`collection:${cat}`);tab.setAttribute("aria-pressed",active?"true":"false");tab.innerHTML=`<span>${escapeHtml(catLabel(cat))}</span><small class="mk-account-shop-browse-count">${(byCat.get(cat) || []).length} items</small>`;tab.addEventListener("click",(ev)=>{try{ev.preventDefault();ev.stopPropagation();}catch(_){}
renderAccountShop(host,{selectedCategory:cat,page:1,focusKey:`collection:${cat}`});});tabs.appendChild(tab);});card.appendChild(tabs);const panel=el("div","mk-account-shop-panel");const panelHead=el("div","mk-account-shop-panel-head");const panelCopy=el("div","");const selectedHint=(()=>{const group=model.groups.find((entry)=>entry.key===selectedCat);if(group)return group.description;if(selectedCat==="__mk_all__")return"Browse every available item. Your unlocked items appear first.";if(selectedCat===OWNED_CAT)return"Your unlocked visuals and currently active boosts.";if(selectedCat===DISCOUNT_CAT)return"Today's offers, with the same prices shown throughout the store.";const rows=byCat.get(selectedCat)||[];const hit=rows.find((row)=>String(row&&row.categoryHint||"").trim());return hit?String(hit.categoryHint||"").trim():"";})();panelCopy.innerHTML=`<div class="mk-account-shop-panel-title"><span>${escapeHtml(catLabel(selectedCat))}</span></div>${selectedHint ? `<p class="mk-account-shop-browse-description">${escapeHtml(selectedHint)}</p>` : ""}`;panelHead.appendChild(panelCopy);if(discountedItems.length){const deals=el("button","mk-account-shop-browse-deals"+(selectedCat===DISCOUNT_CAT?" is-active":""),"Today's discounts");deals.type="button";deals.setAttribute("aria-pressed",selectedCat===DISCOUNT_CAT?"true":"false");deals.setAttribute("data-shop-browse-focus","discounts");deals.addEventListener("click",()=>renderAccountShop(host,{selectedCategory:DISCOUNT_CAT,page:1,focusKey:"discounts"}));panelHead.appendChild(deals);}
panel.appendChild(panelHead);if(selectedCat!==OWNED_CAT&&selectedCat!==DISCOUNT_CAT){const group=model.groups.find((entry)=>entry.key===selectedTop);const filterCategories=(group?group.categories:model.categories).filter((cat)=>byCat.has(cat));const filterLabel=el("label","mk-account-shop-browse-filter");filterLabel.appendChild(el("span","","Item type"));const filter=el("select","mk-account-shop-browse-select");filter.setAttribute("data-shop-browse-focus","filter");const all=el("option","",group?"All in this collection":"All item types");all.value=selectedTop;filter.appendChild(all);filterCategories.forEach((cat)=>{const option=el("option","",`${shopCategoryDisplayLabel(cat)} (${byCat.get(cat).length})`);option.value=cat;filter.appendChild(option);});filter.value=byCat.has(selectedCat)&&filterCategories.includes(selectedCat)?selectedCat:selectedTop;filter.addEventListener("change",()=>renderAccountShop(host,{selectedCategory:filter.value,page:1,focusKey:"filter"}));filterLabel.appendChild(filter);panel.appendChild(filterLabel);}
...
return onClick(ev);});actions.appendChild(toggle);return toggle;};const afterPurchase=(verb)=>(res)=>{if(!res||res.ok===false){const err=(res&&res.error)||"";let msg;if(err==="price_changed"){msg=`The price has changed to ${formatEorbits(Number(res.price))} ${currency.name}.\nYour account has not been charged. Review the updated price and choose ${verb || "Unlock"} again.`;refreshAccountShop(host);}else if(err==="insufficient_funds"){const bal=Number((res&&res.balance)||0);const price=Number((res&&res.price)||0);const need=Math.max(0,Number((res&&res.missing)||(price-bal)||0));msg=`Not enough ${currency.name}.\nYou have ${formatEorbits(bal)}, this costs ${formatEorbits(price)}${need ? `-${formatEorbits(need)}short` : ""}.`;}else if(err==="cloud_purchase_failed"){msg=`The purchase response could not be confirmed. Run Sync and check your Store and Activity before trying again.${res && res.message ? `\n\n${String(res.message).slice(0,1200)}` : ""}`;}else if(err==="purchase_recovery_not_ready"){msg=res.message||"The purchase request could not be saved for recovery. No new purchase was sent.";}else if(err==="already_owned"){msg=`You already own this item.`;}else if(err==="unknown_item"){msg=`This item is unavailable.`;}else if(err==="cloud_shop_state_not_ready"){msg=`Store balance is still syncing, so your ${currency.name} was not charged.\nRun Sync once, reopen the Store, and try again.${res && res.message ? `\n\nDetails:${String(res.message).slice(0,500)}` : ""}`;}else{msg=`${verb || "Unlock"} could not be confirmed: ${err || (res && res.message) || "unknown error"}.\nRun Sync and check your Store and Activity before trying again.`;}
...
refreshAccountShop(host);return;}
if(typeof liveApi.startShopTrial!=="function"){window.setTimeout(()=>{try{refreshAccountShop(host);}catch(_){}},260);return;}
...
if(actions.childNodes&&actions.childNodes.length)tile.appendChild(actions);tile.querySelectorAll("button").forEach((button,index)=>{button.setAttribute("data-shop-browse-focus",`item:${itemId}:${index}`);});grid.appendChild(tile);};let browsePager=null;if(!pageState.total){const empty=el("div","mk-account-shop-empty",selectedCat===OWNED_CAT?"Nothing unlocked yet.":"No items here right now.");panel.appendChild(empty);}else{pageState.items.forEach(renderShopItem);const pager=el("div","mk-account-shop-browse-pager");browsePager=pager;pager.setAttribute("role","group");pager.setAttribute("aria-label","Store pages");const status=el("span","mk-account-shop-browse-status",`${pageState.start + 1}–${Math.min(pageState.total, pageState.start + pageState.items.length)} of ${pageState.total} items · Page ${pageState.page} of ${pageState.pageCount}`);status.setAttribute("role","status");status.setAttribute("aria-live","polite");pager.appendChild(status);if(pageState.pageCount>1){[["previous","Previous",-1],["next","Next",1]].forEach(([key,label,offset])=>{const button=el("button","mk-account-shop-browse-page",label);button.type="button";button.disabled=offset<0?pageState.page===1:pageState.page===pageState.pageCount;button.setAttribute("data-shop-browse-focus",`page:${key}`);button.addEventListener("click",()=>renderAccountShop(host,{selectedCategory:selectedCat,page:pageState.page+offset,focusKey:`page:${key}`,scrollToResults:true}));pager.appendChild(button);});}
panel.appendChild(pager);panel.appendChild(grid);}
card.appendChild(panel);host.appendChild(card);scheduleShopTrialCountdowns(host);if(focusKey)try{const controls=Array.from(card.querySelectorAll("[data-shop-browse-focus]")).filter((node)=>!node.disabled);const target=controls.find((node)=>node.getAttribute("data-shop-browse-focus")===focusKey)||(focusKey.startsWith("page:")&&controls.find((node)=>String(node.getAttribute("data-shop-browse-focus")||"").startsWith("page:")))||controls.find((node)=>node.getAttribute("aria-pressed")==="true")||controls[0];if(target)try{target.focus({preventScroll:true});}catch(_){target.focus();}}catch(_){}
if(options.scrollToResults)scrollShopBrowseResults(host,browsePager);}
...
let __quickShopMenuCleanup=null;function closeQuickShopMenu(options){const menu=document.getElementById(QUICK_SHOP_MENU_ID);const anchor=menu&&menu.__mkQuickShopAnchor;const returnFocus=!!(options&&options.returnFocus&&menu&&(menu.contains(document.activeElement)||document.activeElement===anchor||options.lostMenuFocus));try{if(typeof __quickShopMenuCleanup==="function"){const cleanup=__quickShopMenuCleanup;__quickShopMenuCleanup=null;cleanup();}}catch(_){}
try{document.getElementById(QUICK_SHOP_MENU_ID)?.remove();}catch(_){}
if(returnFocus&&anchor&&anchor.isConnected){try{if(anchor.getClientRects().length)anchor.focus({preventScroll:true});}catch(_){}}}
...
closeQuickShopMenu();const options=opts&&typeof opts==="object"?opts:{};let keyboardOpen=false;try{keyboardOpen=!!(anchor&&anchor.isConnected&&document.activeElement===anchor&&(options.focusOnOpen===true||(options.focusOnOpen!==false&&anchor.matches(":focus-visible"))));}catch(_){}
const categories=(Array.isArray(options.categories)?options.categories:[options.category||"Profile styles"]).map((x)=>String(x||"").trim()).filter(Boolean);const menu=el("div","","");menu.id=QUICK_SHOP_MENU_ID;menu.setAttribute("role","menu");menu.tabIndex=-1;try{menu.__mkQuickShopAnchor=anchor||null;}catch(_){}
document.body.appendChild(menu);let closed=false;let focusVersion=0;let pendingActionFocus=null;const onFocusChange=()=>{focusVersion+=1;pendingActionFocus=null;};const focusMenuControl=(key)=>{if(closed||!menu.isConnected)return;const controls=Array.from(menu.querySelectorAll("button, a[href], input, select, textarea, [tabindex]")).filter(node=>!node.disabled&&node.getAttribute("tabindex")!=="-1"&&node.getClientRects().length);const target=controls.find(node=>key&&node.getAttribute("data-mk-quick-focus")===key)||controls[0]||menu;try{target.focus({preventScroll:true});}catch(_){}};const render=(selectedCategory,actionFocus)=>{if(closed||!menu.isConnected)return;const focusInside=menu.contains(document.activeElement);const restoreActionFocus=actionFocus&&actionFocus.version===focusVersion&&document.activeElement===document.body;const restoreFocus=focusInside||restoreActionFocus;const focusKey=focusInside?document.activeElement.getAttribute("data-mk-quick-focus"):restoreActionFocus&&actionFocus.key;if(actionFocus===pendingActionFocus)pendingActionFocus=null;const api=window.MkAccountData||null;const score=readWarmAccountXpSnapshot()||{};const currency=accountCurrencySummary(score);const catalog=api&&typeof api.shopCatalog==="function"?api.shopCatalog():[];const inventory=api&&typeof api.getShopInventory==="function"?api.getShopInventory():{owned:{},equipped:{},activeBoosts:{active:[]},activeTrials:[]};const allowed=new Set(categories);const items=(catalog||[]).filter((item)=>item&&!item.free&&Number(item.price||0)>0&&allowed.has(String(item.category||"")));const cats=categories.filter((cat)=>items.some((item)=>String(item.category||"")===cat));const selected=cats.includes(selectedCategory)?selectedCategory:(cats[0]||categories[0]||"");const itemById=new Map(items.map((it)=>[String(it.id||""),it]).filter((row)=>row[0]));const activeTrialForSlot=(slot)=>{const wanted=String(slot||"").trim();if(!wanted||!(inventory&&Array.isArray(inventory.activeTrials)))return null;return inventory.activeTrials.find((row)=>{if(!row||row.muted)return false;const it=itemById.get(String(row.itemId||""));return it&&String(it.slot||"").trim()===wanted;})||null;};const activeBoostForItem=(itemId)=>inventory&&inventory.activeBoosts&&Array.isArray(inventory.activeBoosts.active)?inventory.activeBoosts.active.find((b)=>String(b.itemId||"")===String(itemId||"")):null;const deactivateInterfaceThemes=()=>{const liveApi=window.MkAccountData||api;if(!liveApi)return Promise.resolve();const tasks=[];const settle=(value)=>value&&typeof value.then==="function"?value.catch(()=>null):Promise.resolve(value);if(typeof liveApi.clearShopSlot==="function"){["interface_theme","interface_theme_light","interface_theme_dark"].forEach((slot)=>{if(inventory&&inventory.equipped&&inventory.equipped[slot]){tasks.push(settle(liveApi.clearShopSlot(slot,{source:"quick-shop-base-theme"})));}});}
...
return tasks.length?Promise.all(tasks).then(()=>undefined):Promise.resolve();};menu.innerHTML="";const head=el("div","mk-quick-shop-head","");head.innerHTML=`<div class="mk-quick-shop-title">${escapeHtml(options.title || quickShopDefaultTitle(categories))}</div><div class="mk-quick-shop-balance">${commentSvgIcon("eorbit", 13)}${escapeHtml(formatEorbits(currency.balance))}</div>`;menu.appendChild(head);if(cats.length>1){const tabs=el("div","mk-quick-shop-tabs","");cats.forEach((cat)=>{const tab=el("button","mk-quick-shop-tab"+(cat===selected?" is-active":""),shopCategoryDisplayLabel(cat));tab.type="button";tab.setAttribute("data-mk-quick-focus",`category:${cat}`);tab.addEventListener("click",(ev)=>{try{ev.preventDefault();ev.stopPropagation();}catch(_){}render(cat);});tabs.appendChild(tab);});menu.appendChild(tabs);}
const grid=el("div","mk-quick-shop-grid","");const selectedItems=shopItemsOwnedFirst(items.filter((item)=>String(item.category||"")===selected),(item)=>{const itemId=String(item&&item.id||"").trim();if(!itemId)return false;if(item&&item.consumable)return!!activeBoostForItem(itemId);return!!(inventory&&inventory.owned&&inventory.owned[itemId]);});if(options.includeBaseThemes&&selected==="Interface themes"){const activeScheme=currentQuickShopPaletteScheme();[{scheme:"default",label:"Light mode",description:"Applies the built-in light interface theme."},{scheme:"slate",label:"Dark mode",description:"Applies the built-in dark interface theme."}].forEach((entry)=>{const active=activeScheme===entry.scheme;const tile=el("div","mk-quick-shop-item is-owned"+(active?" is-active":""),"");tile.innerHTML=`<div class="mk-quick-shop-item-main"><div class="mk-quick-shop-item-title"><strong>${escapeHtml(entry.label)}</strong><small>Built-in theme · Free</small></div><p class="mk-quick-shop-desc">${escapeHtml(entry.description)}</p></div>`;const actions=el("div","mk-quick-shop-actions","");const toggle=el("button","mk-quick-shop-toggle mk-quick-shop-owned-switch"+(active?" is-on":""),"");toggle.type="button";toggle.setAttribute("data-mk-quick-focus",`palette:${entry.scheme}`);toggle.setAttribute("aria-pressed",active?"true":"false");toggle.setAttribute("aria-label",active?`${entry.label} is on.`:`Turn ${entry.label} on.`);toggle.addEventListener("click",(ev)=>{try{ev.preventDefault();ev.stopPropagation();}catch(_){}
if(active)return;const focusedKey=document.activeElement===toggle&&toggle.getAttribute("data-mk-quick-focus");toggle.disabled=true;const actionFocus=focusedKey?{key:focusedKey,version:focusVersion}:null;pendingActionFocus=actionFocus;deactivateInterfaceThemes().finally(()=>{if(!switchQuickShopPaletteScheme(entry.scheme)){toggle.disabled=false;return;}
window.setTimeout(()=>render(selected,actionFocus),0);});});actions.appendChild(toggle);tile.appendChild(actions);grid.appendChild(tile);});}
...
rerender();});actions.appendChild(tryBtn);const buy=el("button","mk-comment-primary-btn"+(discountPriceMarkup?" mk-account-shop-discount-buy":""),"");buy.type="button";buy.setAttribute("data-mk-quick-focus",`item:${itemId}:buy`);buy.innerHTML=discountPriceMarkup||`<span class="mk-account-shop-btn-price">${commentSvgIcon("eorbit", 12)}${escapeHtml(formatEorbits(price))}</span>`;buy.addEventListener("click",()=>{const liveApi=window.MkAccountData||api;if(!(liveApi&&typeof liveApi.buyShopItem==="function"))return;const livePricing=typeof liveApi.shopItemPricing==="function"?liveApi.shopItemPricing(itemId):null;const chargePrice=livePricing&&Number.isFinite(Number(livePricing.final))?Math.max(0,Number(livePricing.final)):price;if(!window.confirm(`Unlock ${item.title || itemId} for ${formatEorbits(chargePrice)} ${currency.name}?`))return;liveApi.buyShopItem(itemId,{source:"quick-shop"}).then((res)=>{if(!res||res.ok===false){window.alert(res&&res.error==="insufficient_funds"?`Not enough ${currency.name}.`:"Unlock failed.");return;}
rerender();}).catch((err)=>window.alert(String(err&&err.message||err||"Unlock failed.")));});actions.appendChild(buy);}
if(actions.childNodes&&actions.childNodes.length)tile.appendChild(actions);grid.appendChild(tile);});menu.appendChild(grid);scheduleShopTrialCountdowns(menu);positionQuickShopMenu(menu,anchor);if(restoreFocus)focusMenuControl(focusKey);};render(options.selectedCategory||"");let listenersBound=false;const cleanup=()=>{closed=true;pendingActionFocus=null;if(listenersBound){try{document.removeEventListener("click",onDoc,true);}catch(_){}
try{document.removeEventListener("keydown",onKey,true);}catch(_){}
try{document.removeEventListener("focusin",onFocusChange,true);}catch(_){}
listenersBound=false;}
if(__quickShopMenuCleanup===cleanup)__quickShopMenuCleanup=null;};const onDoc=(ev)=>{focusVersion+=1;pendingActionFocus=null;const t=ev&&ev.target;if(t&&menu.contains(t))return;if(anchor&&anchor.contains&&anchor.contains(t))return;closeQuickShopMenu();};const onKey=(ev)=>{if(ev&&ev.key==="Tab"){focusVersion+=1;pendingActionFocus=null;}
if(!ev||ev.key!=="Escape"||ev.defaultPrevented)return;const focused=document.activeElement;if(focused&&!menu.contains(focused)&&focused.closest('[role="dialog"][aria-modal="true"]'))return;ev.preventDefault();ev.stopPropagation();closeQuickShopMenu({returnFocus:true,lostMenuFocus:!!(focused===document.body&&pendingActionFocus&&pendingActionFocus.version===focusVersion)});};__quickShopMenuCleanup=cleanup;if(keyboardOpen)focusMenuControl();setTimeout(()=>{if(closed||!menu.isConnected)return;document.addEventListener("click",onDoc,true);document.addEventListener("keydown",onKey,true);document.addEventListener("focusin",onFocusChange,true);listenersBound=true;},0);window.setTimeout(()=>{if(!closed&&menu.isConnected)positionQuickShopMenu(menu,anchor);},80);}
...
function mountAccountHistoryRecovery(host,section,options={}){if(!document.getElementById("mk-account-history-recovery-style")){const style=el("style");style.id="mk-account-history-recovery-style";style.textContent=`
.mk-account-sync-card .mk-account-history-recovery{min-width:0;padding:1rem;border:1px solid var(--md-default-fg-color--lighter);border-left:3px solid var(--mk-theme-link-color,var(--md-accent-fg-color));border-radius:12px;background:var(--md-default-bg-color);color:var(--md-default-fg-color);font-size:var(--mk-local-body-size,.72rem);line-height:1.55}
.mk-account-history-recovery[hidden],.mk-account-history-recovery [hidden]{display:none!important}
.mk-account-sync-card [data-sync-history-review][hidden]{display:none!important}
.mk-account-history-recovery .mk-history-recovery-title{margin:0 0 .45rem;font-size:1.08em;font-weight:750;line-height:1.35}
.mk-account-history-recovery .mk-history-recovery-copy{margin:.5rem 0;overflow-wrap:anywhere}
.mk-account-history-recovery .mk-history-recovery-list{margin:.65rem 0;padding-left:1.1rem}
.mk-account-history-recovery .mk-history-recovery-list li{margin:.45rem 0;overflow-wrap:anywhere}
.mk-account-history-recovery .mk-history-recovery-archive{padding:.65rem .75rem;border-radius:8px;background:var(--md-code-bg-color);overflow-wrap:anywhere}
.mk-account-history-recovery .mk-history-recovery-actions{display:flex;flex-wrap:wrap;gap:.45rem;margin-top:.75rem}
.mk-account-history-recovery .mk-history-recovery-button{max-width:100%;min-height:36px;padding:.5rem .7rem;border:1px solid var(--md-default-fg-color--lighter);border-radius:8px;background:var(--md-default-bg-color);color:var(--md-default-fg-color);font:inherit;font-weight:650;line-height:1.4;text-align:left;white-space:normal;cursor:pointer}
.mk-account-history-recovery .mk-history-recovery-button:hover:not(:disabled){border-color:var(--mk-theme-link-color,var(--md-accent-fg-color));color:var(--mk-theme-link-color,var(--md-accent-fg-color))}
.mk-account-history-recovery .mk-history-recovery-button:focus-visible{outline:2px solid var(--mk-theme-link-color,var(--md-accent-fg-color));outline-offset:3px}
.mk-account-history-recovery .mk-history-recovery-button:disabled{opacity:.55;cursor:wait}
.mk-account-history-recovery .mk-history-recovery-button--primary{border-color:var(--mk-theme-link-color,var(--md-accent-fg-color));font-weight:750}
.mk-account-history-recovery .mk-history-recovery-status{margin:.6rem 0 0;overflow-wrap:anywhere}
@media(pointer:coarse){.mk-account-history-recovery .mk-history-recovery-button{min-height:44px}}
`;document.head.appendChild(style);}
const box=el("div","mk-account-history-recovery");box.hidden=true;box.setAttribute("role","region");box.setAttribute("aria-label","Local history recovery");const heading=el("h3","mk-history-recovery-title","Local history needs review");const guide=el("p","mk-history-recovery-copy","This browser has quiz or mastery history that cannot be fully read. Close quizzes in other tabs, then choose a recovery action below. The original is saved and verified in this browser before readable records are repaired. You can download a separate copy first.");const list=el("ul","mk-history-recovery-list");const archive=el("p","mk-history-recovery-archive");const privacy=el("p","mk-history-recovery-copy","Downloads contain your personal learning history and stay on your device. Unreadable parts are kept only in this browser's recovery archive; download it before clearing browser data or moving devices. A later Sync now syncs readable records, not the unreadable parts.");const actions=el("div","mk-history-recovery-actions");const button=(name,action)=>{const node=el("button","mk-history-recovery-button",name);node.type="button";node.setAttribute("data-history-recovery-action",action);actions.appendChild(node);return node;};const repairAndSync=button("Preserve original, repair & retry sync","repair-sync");repairAndSync.classList.add("mk-history-recovery-button--primary");const download=button("Download original history","original");const repair=button("Preserve original & repair readable history","repair");const downloadArchive=button("Download recovery archive","archive");const review=button("Review again","review");const status=el("p","mk-history-recovery-status");status.setAttribute("role","status");status.setAttribute("aria-live","polite");status.setAttribute("aria-atomic","true");[heading,guide,list,archive,privacy,actions,status].forEach(node=>box.appendChild(node));host.appendChild(box);const context=captureSocialAccountContext();const page=currentPath();const listeners=[];let snapshot=null,issueCount=0,archiveCount=0,busy=false,destroyed=false,generation=0;const listen=(target,type,fn)=>{target.addEventListener(type,fn);listeners.push(()=>target.removeEventListener(type,fn));};const destroy=()=>{if(destroyed)return;destroyed=true;generation++;snapshot=null;listeners.splice(0).forEach(remove=>remove());box.innerHTML="";box.hidden=true;};const current=()=>{if(destroyed)return false;if(!box.isConnected||!socialAccountContextIsCurrent(context)||currentPath()!==page){destroy();return false;}
return true;};const controls=()=>{box.setAttribute("aria-busy",String(busy));download.hidden=repair.hidden=issueCount===0;download.disabled=repair.disabled=busy||!snapshot;repairAndSync.hidden=issueCount===0||typeof options.onRetrySync!=="function";repairAndSync.disabled=busy||!snapshot;downloadArchive.hidden=archiveCount===0;downloadArchive.disabled=review.disabled=busy;};const messageFor=(result,fallback)=>String(result&&result.error||fallback);const invalidate=(message)=>{generation++;busy=false;snapshot=null;status.textContent=message;controls();};const inspect=async(reviewOptions={})=>{if(!current()||busy)return;const token=++generation;busy=true;snapshot=null;if(reviewOptions.reveal)box.hidden=false;status.textContent="Reviewing local history…";controls();try{const api=window.MkAccountData;if(!api||typeof api.inspectLegacyHistoryRecovery!=="function")throw new Error("History review is not available yet. Reload this page, then try again.");const result=await api.inspectLegacyHistoryRecovery();if(!current()||token!==generation)return;if(!result||!result.ok)throw new Error(messageFor(result,"Could not review local history. Nothing was repaired."));const issues=Array.isArray(result.issues)?result.issues:[];snapshot=result.snapshot;issueCount=issues.length;archiveCount=Math.max(0,Number(result.archiveCount)||0);list.innerHTML="";issues.forEach(issue=>{const readable=Math.max(0,Number(issue.validRecords)||0);const unreadable=issue.unreadableRecords==null?"unreadable count cannot be determined":`${Math.max(0, Number(issue.unreadableRecords) || 0)} unreadable records`;list.appendChild(el("li","",`${issue.label || issue.key || "Local history"}: ${readable} readable records; ${unreadable}. ${issue.reason || "This history could not be fully read."}`));});heading.textContent=issueCount?"Local history needs review":"Original history is preserved locally";guide.hidden=list.hidden=issueCount===0;privacy.hidden=issueCount===0&&archiveCount===0;archive.hidden=archiveCount===0;archive.textContent=`${archiveCount} original history ${archiveCount === 1 ? "copy is" : "copies are"} preserved in this browser. This archive is not uploaded by Sync. Its unreadable records still need recovery.`;box.hidden=issueCount===0&&archiveCount===0&&!reviewOptions.reveal;status.textContent=reviewOptions.message||(issueCount?"Nothing has been repaired. Choose an action after reviewing the records above.":archiveCount?"Keep a downloaded archive for any further recovery.":"No unreadable local history was found.");if(!issueCount&&!archiveCount)heading.textContent="Local history review";}catch(error){if(!current()||token!==generation)return;box.hidden=false;status.textContent=`${error && error.message || error} Select Review again to retry.`;}finally{if(current()&&token===generation){busy=false;controls();if(reviewOptions.focus&&!box.hidden){try{section.open=true;box.tabIndex=-1;box.focus({preventScroll:true});box.scrollIntoView({block:"nearest",behavior:"instant"});}catch(_){}}}}};const exportHistory=async(original)=>{if(!current()||busy||original&&!snapshot)return;const token=++generation,reviewed=snapshot;busy=true;status.textContent="Preparing your local history download…";controls();try{const api=window.MkAccountData;if(!api||typeof api.exportLegacyHistoryRecovery!=="function")throw new Error("History download is not available yet.");const result=await api.exportLegacyHistoryRecovery(original?reviewed:undefined);if(!current()||token!==generation)return;if(!result||!result.ok||typeof result.text!=="string")throw new Error(messageFor(result,"Could not prepare the history download."));const url=URL.createObjectURL(new Blob([result.text],{type:"application/json;charset=utf-8"}));const link=el("a");link.href=url;link.download=original?"eor-original-history.json":"eor-history-recovery-archive.json";document.body.appendChild(link);try{link.click();}finally{link.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1000);}
status.textContent="Download requested. Check your browser's downloads and keep this personal history file safely. No data was uploaded.";}catch(error){if(!current()||token!==generation)return;snapshot=null;status.textContent=`${error && error.message || error} Select Review again before another attempt.`;}finally{if(current()&&token===generation){busy=false;controls();}}};listen(download,"click",()=>exportHistory(true));listen(downloadArchive,"click",()=>exportHistory(false));listen(review,"click",()=>inspect({reveal:true}));const repairHistory=async(retrySync)=>{if(!current()||busy||!snapshot||!issueCount)return;const token=++generation,reviewed=snapshot;busy=true;status.textContent="Preserving the original history before repairing readable records…";if(document.activeElement===repair||document.activeElement===repairAndSync){status.tabIndex=-1;status.focus({preventScroll:true});}
controls();try{const api=window.MkAccountData;if(!api||typeof api.recoverLegacyHistory!=="function")throw new Error("History repair is not available yet.");const result=await api.recoverLegacyHistory(reviewed);if(!current()||token!==generation)return;if(!result||!result.ok)throw new Error(messageFor(result,"History repair could not finish."));snapshot=null;busy=false;await inspect({reveal:true,message:`Preserved ${Math.max(0, Number(result.archived) || 0)} original history copies and checked ${Math.max(0, Number(result.recoveredRecords) || 0)} readable records. No sync was started. Download the recovery archive, then use Sync now for readable records. Unreadable parts have not been recovered.`});if(current()&&retrySync&&typeof options.onRetrySync==="function"){if(!snapshot||issueCount){status.textContent+=" Sync was not retried because the history review is not clear yet.";}else{const started=options.onRetrySync();status.textContent=started&&started.ok===false?`The original was preserved and readable history repaired. ${started.error || "Use Sync now when the current operation finishes."}`:"The original was preserved and readable history repaired. Sync retry started; follow the progress above. Unreadable parts remain in the recovery archive and are not uploaded.";}}}catch(error){if(!current()||token!==generation)return;snapshot=null;status.textContent=`${error && error.message || error} Select Review again before another repair attempt.`;}finally{if(current()&&token===generation){busy=false;controls();}}};listen(repair,"click",()=>repairHistory(false));listen(repairAndSync,"click",()=>repairHistory(true));const contextChanged=()=>{if(!current())return;};const historyChanged=()=>{if(!current()||busy||box.hidden)return;invalidate("Local history changed after this review. Select Review again before downloading originals or repairing.");};listen(section,"toggle",()=>{if(section.open)inspect();});["mk-account-workspace-changed","mk-account-login-change"].forEach(type=>listen(window,type,contextChanged));["mk-ai-quiz-sessions-changed","mk:mastery-submitted","mk-account-data-changed","mk-account-history-repaired"].forEach(type=>listen(window,type,historyChanged));listen(window,"storage",event=>{if(!current())return;if(!event.key||["concept_mastery_v1","concept_quiz_sessions_v1","mk_account_data_file_v1","mk_account_pending_events_v1","mk_account_history_recovery_v1"].includes(event.key))historyChanged();});listen(window,"pagehide",destroy);controls();return{refresh:inspect,destroy};}
function disposeAccountHistoryRecovery(host){if(host&&typeof host.__mkAccountHistoryRecoveryCleanup==="function"){host.__mkAccountHistoryRecoveryCleanup();delete host.__mkAccountHistoryRecoveryCleanup;}}
function createAccountProfileView(host){if(typeof host.__mkDisposeProfileInfo==="function")host.__mkDisposeProfileInfo();let context=captureSocialAccountContext(),active=true,editorSerial=0,editor=null,renameTarget="";const cleanups=[];const ownsHost=()=>active&&host.__mkProfileView===view&&host.isConnected&&(!host.dataset.type||host.dataset.type==="info");const current=()=>ownsHost()&&socialAccountContextIsCurrent(context);const dispose=()=>{if(!active)return;active=false;editorSerial++;if(editor)editor.close({restoreFocus:false});cleanups.splice(0).forEach(fn=>{try{fn();}catch(_){}});if(host.__mkDisposeProfileInfo===dispose)host.__mkDisposeProfileInfo=null;};const changed=()=>{if(!ownsHost()||current())return;const next=captureSocialAccountContext();if(renameTarget&&next.accountKey===renameTarget&&next.visitorId===context.visitorId)return;const focused=host.contains(document.activeElement)||!!(editor&&editor.modal.contains(document.activeElement));dispose();disposeAccountHistoryRecovery(host);host.innerHTML="";const status=el("div","mk-profile-view-status","Account changed. Reload this section.");status.setAttribute("role","status");status.tabIndex=-1;const reload=el("button","mk-comment-small-btn","Reload account");reload.type="button";reload.addEventListener("click",()=>{if(reload.isConnected&&(!host.dataset.type||host.dataset.type==="info"))renderProfileInfo(host);});host.appendChild(status);host.appendChild(reload);if(focused){try{status.focus({preventScroll:true});}catch(_){}}};const view={current,ownsHost,get context(){return context;},onDispose(fn){if(active)cleanups.push(fn);else fn();},beginEditor(){if(!current())return 0;editorSerial++;if(editor)editor.close({restoreFocus:false});return editorSerial;},editorCurrent(ticket){return!!ticket&&ticket===editorSerial&&current();},openEditor(title,ticket){const mini=openLocalMiniModal(title,{accountEditor:true,isCurrent:()=>view.editorCurrent(ticket)});editor=mini;mini.onClose(()=>{if(editor===mini){editor=null;editorSerial++;}});return mini;},beginRename(name){renameTarget=cleanProfileNameLocal(name).toLowerCase();},finishRename(result){const next=captureSocialAccountContext();if(ownsHost()&&result&&result.ok&&result.workspaceReloadRequired&&result.profile&&renameTarget&&String(result.profile.accountKey||result.profile.account_key||"").trim().toLowerCase()===renameTarget&&next.accountKey===renameTarget&&next.visitorId===context.visitorId)return false;if(ownsHost()&&result&&result.ok&&result.profile&&renameTarget&&String(result.profile.accountKey||"").trim().toLowerCase()===renameTarget&&next.accountKey===renameTarget&&next.visitorId===context.visitorId&&socialAccountContextIsCurrent(next))context=next;renameTarget="";changed();return current();}};host.__mkProfileView=view;host.__mkDisposeProfileInfo=dispose;["mk-account-workspace-changed","mk-account-login-change","mk-account-workspaces-ready","storage"].forEach(event=>{window.addEventListener(event,changed);view.onDispose(()=>window.removeEventListener(event,changed));});return view;}
function renderProfileInfo(host){if(!host)return;disposeAccountHistoryRecovery(host);ensureAccountShopStylesOnce();bindShopTrialDelegatedFallbackOnce();try{host.querySelectorAll(".mk-local-level-badge").forEach((btn)=>{if(btn&&typeof btn.__mkAccountXpCleanup==="function")btn.__mkAccountXpCleanup();});}catch(_){}
host.innerHTML="";const profile=readLocalProfile();const profileView=createAccountProfileView(host);const profileContext=profileView.context;const refreshInfo=()=>{if(!profileView.current())return;const focused=document.activeElement;const action=host.contains(focused)&&["avatar","frame","name","bio"].find(kind=>focused.classList.contains(`mk-local-${kind}-change-btn`));renderProfileInfo(host);if(action){const target=host.querySelector(`.mk-local-${action}-change-btn`);try{target&&target.focus({preventScroll:true});}catch(_){}}};if(!profile.accountKey){renderGuestAccountOnboarding(host,profile,()=>{if(profileView.ownsHost())renderProfileInfo(host);});return;}
const wrap=el("div","mk-local-profile-card mk-account-profile-stack");const profileBlock=el("div","mk-account-profile-card");const row=el("div","mk-local-profile-main-row");const avatarCell=el("div","mk-local-profile-avatar-cell");const avatarBox=el("div","mk-local-profile-avatarbox");avatarBox.innerHTML=avatarHtml(profile.name||"You",profile.avatar||"",profile.avatarFrame||"level-1");avatarCell.appendChild(avatarBox);const identityCell=el("div","mk-local-profile-identity-cell");const nameCell=el("div","mk-local-profile-name-cell");const nameBox=el("div","mk-local-profile-name",profile.name||"No username yet");const levelBtn=el("button","mk-local-level-badge");levelBtn.type="button";levelBtn.setAttribute("aria-label","Open level, XP and EORbits details");levelBtn.innerHTML=`<span class="mk-local-level-badge-fill" style="width:0%"></span><strong>Lv. …</strong><span class="mk-local-level-badge-xp">XP</span>`;levelBtn.addEventListener("click",(ev)=>{try{ev.preventDefault();ev.stopPropagation();}catch(_){}if(profileView.current())openAccountLevelModal();});const currencyBtn=el("button","mk-local-inline-change mk-account-currency-btn mk-account-qr-beside-xp","…");currencyBtn.type="button";currencyBtn.setAttribute("aria-label","Open EORbits store");currencyBtn.title="Open EORbits store";const paintCurrencyButton=(score)=>{const c=accountCurrencySummary(score||null);currencyBtn.innerHTML=`<span class="mk-account-currency-icon">${commentSvgIcon("eorbit", 16)}</span><span class="mk-account-currency-amount">${escapeHtml(formatEorbits(c.balance))}</span>`;currencyBtn.title=`${formatEorbits(c.balance)} ${c.name} available. Open store.`;};const paintLevelBadge=(score)=>{if(!score)return;if(accountProgressSyncHealth().initialPending){levelBtn.innerHTML=`<strong>Lv. —</strong><span class="mk-local-level-badge-xp">History syncing</span>`;levelBtn.title="Your level will appear after account history has synced.";paintCurrencyButton(score);return;}
const s=accountScoreSummary(score);const pct=Math.max(0,Math.min(100,Number(s.progressPct||0)));levelBtn.title=s.nextLevel?`${formatAccountXp(s.intoLevel)} / ${formatAccountXp(s.levelSpan)} XP in Level ${s.level}`:`${formatAccountXp(s.totalScore)} XP total`;levelBtn.innerHTML=`<span class="mk-local-level-badge-fill" style="width:${escapeAttr(pct)}%"></span><strong>Lv. ${escapeHtml(s.level)}</strong><span class="mk-local-level-badge-xp">${escapeHtml(formatAccountXp(s.totalScore))} XP</span>`;paintCurrencyButton(score);};if(profile.name){let initialScore=null;try{initialScore=readWarmAccountXpSnapshot()||readLiveAccountEventXpScore({fresh:false});}catch(_){initialScore=null;}
if(!initialScore){try{initialScore=readCachedAccountXp({full:true});}catch(_){initialScore=null;}}
if(initialScore){paintLevelBadge(initialScore);}else{levelBtn.innerHTML=`<span class="mk-local-level-badge-fill" style="width:0%"></span><strong>Lv. —</strong><span class="mk-local-level-badge-xp">Sync to load</span>`;levelBtn.title="Sync this device to load your XP and EORbits.";currencyBtn.innerHTML=`<span class="mk-account-currency-icon">${commentSvgIcon("eorbit", 16)}</span><span class="mk-account-currency-amount">—</span>`;currencyBtn.title="Sync this device to load your EORbits.";try{fetchAccountScore({force:false,reason:"account-panel-badge"}).then((sc)=>{if(sc&&profileView.current())paintLevelBadge(sc);}).catch(()=>{});}catch(_){}}
try{if(window.MkAccountData&&typeof window.MkAccountData.syncShopStateNow==="function"){window.MkAccountData.syncShopStateNow({reason:"account-panel-open"}).then(()=>{if(!profileView.current())return;try{const sc=readLiveAccountEventXpScore({fresh:true})||readWarmAccountXpSnapshot();if(sc){publishAccountXp(sc);paintLevelBadge(sc);}}catch(_){}}).catch(()=>{});}}catch(_){}
const onXp=(ev)=>{const score=ev&&ev.detail&&ev.detail.score;if(score&&profileView.current())paintLevelBadge(score);};window.addEventListener("mk-account-xp-change",onXp);levelBtn.__mkAccountXpCleanup=()=>window.removeEventListener("mk-account-xp-change",onXp);profileView.onDispose(levelBtn.__mkAccountXpCleanup);}else{levelBtn.disabled=true;}
nameCell.appendChild(nameBox);nameCell.appendChild(levelBtn);nameCell.appendChild(currencyBtn);identityCell.appendChild(nameCell);const bioCell=el("div","mk-local-profile-bio-cell");const bioText=el("div","mk-local-profile-bio",profile.bio||"Add a short profile intro");bioText.classList.toggle("is-empty",!profile.bio);bioCell.appendChild(bioText);row.appendChild(avatarCell);row.appendChild(identityCell);row.appendChild(bioCell);profileBlock.appendChild(row);const actionRow=el("div","mk-local-profile-action-row");const avatarChange=el("button","mk-local-inline-change mk-local-avatar-change-btn","Avatar");avatarChange.type="button";avatarChange.setAttribute("aria-label","Update avatar");const frameChange=el("button","mk-local-inline-change mk-local-frame-change-btn","Frame");frameChange.type="button";frameChange.setAttribute("aria-label","Choose avatar frame");const nameChange=el("button","mk-local-inline-change mk-local-name-change-btn","Username");nameChange.type="button";nameChange.setAttribute("aria-label","Change username");const bioChange=el("button","mk-local-inline-change mk-local-bio-change-btn","Intro");bioChange.type="button";bioChange.setAttribute("aria-label",profile.bio?"Edit profile intro":"Add profile intro");actionRow.appendChild(avatarChange);actionRow.appendChild(frameChange);actionRow.appendChild(nameChange);actionRow.appendChild(bioChange);currencyBtn.addEventListener("click",()=>{if(!profileView.current())return;try{openLocalActivity("shop");}catch(_){}});profileBlock.appendChild(actionRow);wrap.appendChild(profileBlock);frameChange.addEventListener("click",()=>{const ticket=profileView.beginEditor();if(!ticket)return;openAvatarFramePicker(__accountXpCache,refreshInfo,{accountEditor:true,isCurrent:()=>profileView.editorCurrent(ticket),onDispose:profileView.onDispose,openMini:title=>profileView.openEditor(title,ticket)}).catch(()=>{});});bioChange.addEventListener("click",()=>{const ticket=profileView.beginEditor();if(!ticket)return;const accountContext=captureSocialAccountContext();const curr=readLocalProfile();if(!curr.name){window.alert("Please choose a username before adding a profile intro.");return;}
const mini=profileView.openEditor("Profile intro",ticket);const input=document.createElement("textarea");input.maxLength=140;input.rows=3;input.value=curr.bio||"";input.placeholder="A short sentence about yourself";input.className="mk-local-profile-bio-input";input.setAttribute("aria-label","Profile intro");const note=el("div","mk-local-avatar-note","140 characters max. Appears on rankings when both Profile and User rankings are Public in Privacy settings.");const actions=el("div","mk-local-mini-actions");const cancel=iconButton("mk-comment-small-btn","x","Cancel");const save=iconButton("mk-comment-primary-btn","check","Save intro");cancel.type=save.type="button";actions.appendChild(cancel);actions.appendChild(save);mini.body.appendChild(input);mini.body.appendChild(note);mini.setActions(actions);cancel.addEventListener("click",mini.close);save.addEventListener("click",async()=>{if(!mini.current()||mini.busy||!socialAccountContextIsCurrent(accountContext))return;const bio=cleanProfileBioLocal(input.value||"");if(bio===cleanProfileBioLocal(curr.bio||"")){mini.close();return;}
mini.setBusy(save,true);let res;try{res=await saveOnlineProfile(curr.name,"",{keepAvatar:true,privacy:curr.privacy,isPublic:curr.isPublic,bio});}
catch(_){res={ok:false,error:"Could not save your intro. Try again."};}
finally{mini.setBusy(save,false);}
if(!mini.current())return;if(!res||!res.ok){alertCooldown(res);return;}
mini.close();refreshInfo();});setTimeout(()=>{if(mini.current()){try{input.focus();input.select();}catch(_){}}},0);});nameChange.addEventListener("click",async()=>{const ticket=profileView.beginEditor();if(!ticket)return;const accountContext=captureSocialAccountContext();let curr=readLocalProfile();if(isCooldownActive(curr.nameCooldownUntil))curr=await refreshProfileIfCooldownActive("name");if(!profileView.editorCurrent(ticket)||!socialAccountContextIsCurrent(accountContext))return;if(isCooldownActive(curr.nameCooldownUntil)){window.alert(`Username is locked until ${formatCooldownDate(curr.nameCooldownUntil)}.`);return;}
const mini=profileView.openEditor("Change username",ticket);const input=document.createElement("input");input.type="text";input.maxLength=40;input.autocomplete="nickname";input.value=curr.name||"";input.placeholder="Username";input.setAttribute("aria-label","New username");const actions=el("div","mk-local-mini-actions");const cancel=iconButton("mk-comment-small-btn","x","Cancel");const save=iconButton("mk-comment-primary-btn","check","Save username");cancel.type=save.type="button";actions.appendChild(cancel);actions.appendChild(save);mini.body.appendChild(input);mini.setActions(actions);cancel.addEventListener("click",mini.close);save.addEventListener("click",async()=>{if(!mini.current()||mini.busy||!socialAccountContextIsCurrent(accountContext))return;const nm=cleanProfileNameLocal(input.value||"");if(!nm){input.focus();return;}
if(nm===cleanProfileNameLocal(curr.name||"")){mini.close();return;}
if(!window.confirm(profileChangeWarningText(true,false,!curr.name)))return;if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;mini.setBusy(save,true);profileView.beginRename(nm);let res;try{try{res=await saveOnlineProfile(nm,"",{keepAvatar:true,privacy:curr.privacy,isPublic:curr.isPublic,bio:curr.bio||""});}
catch(_){res={ok:false,error:"Could not save your username. Try again."};}
profileView.finishRename(res);if(!mini.current())return;if(!res||!res.ok){alertCooldown(res);return;}
try{await syncAllAccountDataNow({force:true,reason:"profile-name-save",timeoutMs:ACCOUNT_SYNC_MANUAL_TIMEOUT_MS});}catch(_){}
if(!mini.current())return;mini.close();refreshInfo();}finally{mini.setBusy(save,false);}});setTimeout(()=>{if(mini.current()){try{input.focus();input.select();}catch(_){}}},0);});avatarChange.addEventListener("click",async()=>{const ticket=profileView.beginEditor();if(!ticket)return;const accountContext=captureSocialAccountContext();let curr=readLocalProfile();if(!curr.name){window.alert("Please choose a username before setting an avatar.");return;}
const mini=profileView.openEditor("Change avatar",ticket);let avatarMode=avatarModeFromValue(curr.avatar||"");let avatarValue=(isImageAvatar(curr.avatar)||isInitialAvatarStyleValue(curr.avatar))?"":cleanAvatarLocal(curr.avatar||"");let selectedInitialStyle=parseInitialAvatarStyle(curr.avatar||"");let selectedInitialFont=selectedInitialStyle.font.id;let selectedInitialColor=selectedInitialStyle.color.id;let selectedInitialText=selectedInitialStyle.text.id;let selectedInitialScale=selectedInitialStyle.scale.id;let selectedAvatarEmojiItem=null;let selectedAvatarEmojiTier="";let selectedFile=null;let uploadPreviewUrl="";let selectedAvatarEmojiCategory="";let selectedAvatarToneBase="";if(mini.modal)mini.modal.classList.add("mk-local-avatar-modal");const sticky=el("div","mk-local-avatar-sticky");const preview=el("div","mk-local-profile-preview mk-local-avatar-preview");const previewAvatar=el("div","mk-local-profile-avatarbox");preview.appendChild(previewAvatar);const tabs=el("div","mk-local-avatar-tabs");const modeInitials=el("button","mk-local-avatar-tab","Initials");const modeEmoji=el("button","mk-local-avatar-tab","Emoji");const modeUpload=el("button","mk-local-avatar-tab","Upload");[modeInitials,modeEmoji,modeUpload].forEach((b)=>{b.type="button";tabs.appendChild(b);});sticky.appendChild(preview);sticky.appendChild(tabs);const body=el("div","mk-local-avatar-body");const file=document.createElement("input");file.type="file";file.accept="image/jpeg,image/png,image/webp,image/gif";const actions=el("div","mk-local-mini-actions");const cancel=iconButton("mk-comment-small-btn","x","Cancel");const save=iconButton("mk-comment-primary-btn","check","Save avatar");cancel.type=save.type="button";actions.appendChild(cancel);actions.appendChild(save);mini.body.appendChild(sticky);mini.body.appendChild(body);mini.setActions(actions);const revokeUploadPreview=()=>{if(!uploadPreviewUrl)return;try{URL.revokeObjectURL(uploadPreviewUrl);}catch(_){}
uploadPreviewUrl="";};mini.onClose(revokeUploadPreview);const currentAvatar=()=>{if(avatarMode==="initials")return encodeInitialAvatarStyle(selectedInitialFont,selectedInitialColor,selectedInitialText,selectedInitialScale);if(avatarMode==="emoji")return cleanAvatarLocal(avatarValue||"");if(avatarMode==="upload")return selectedFile?uploadPreviewUrl:(isImageAvatar(curr.avatar)?curr.avatar:"");return"";};const rememberSelectedAvatarEmojiItem=(item,cp)=>{if(!item)return null;const codepoint=String(cp||item.codepoint||normaliseNotoEmojiText(item.emoji||"")).replace(/[^0-9a-f_]/gi,"").toLowerCase();selectedAvatarEmojiItem=Object.assign({},item,{codepoint});if(!avatarValue)avatarValue=selectedAvatarEmojiItem.emoji||"";const state=avatarEmojiUnlockState(selectedAvatarEmojiItem);if(!selectedAvatarEmojiTier)selectedAvatarEmojiTier=state.hasDynamic?"dynamic":"static";return selectedAvatarEmojiItem;};const refreshPreview=()=>{if(!mini.current())return;previewAvatar.innerHTML="";if(avatarMode==="emoji"&&selectedAvatarEmojiItem){const state=avatarEmojiUnlockState(selectedAvatarEmojiItem);const cards=el("div","mk-local-avatar-paywall mk-local-avatar-paywall--preview");const addCard=(tier)=>{const wantsDynamic=tier==="dynamic";const owned=wantsDynamic?state.hasDynamic:state.hasStatic;const includedStaticPreview=!wantsDynamic&&state.hasDynamic;const price=wantsDynamic?state.dynamicPrice:state.staticPrice;const src=wantsDynamic?selectedAvatarEmojiItem.animatedUrl:selectedAvatarEmojiItem.staticUrl;const card=el("div","mk-local-avatar-card"+(selectedAvatarEmojiTier===tier?" is-selected":""));card.appendChild(el("div","mk-local-avatar-card-title",wantsDynamic?"Dynamic avatar":(includedStaticPreview?"Static preview":"Static avatar")));const pv=el("div","mk-local-avatar-card-preview");const instantSrc=wantsDynamic?(selectedAvatarEmojiItem.staticUrl||src):src;pv.innerHTML=avatarHtml(curr.name||"You",instantSrc||selectedAvatarEmojiItem.emoji||"",curr.avatarFrame||"level-1");if(wantsDynamic&&selectedAvatarEmojiItem.animatedUrl&&instantSrc!==selectedAvatarEmojiItem.animatedUrl){const liveImg=pv.querySelector("img");if(liveImg){const pre=new Image();pre.decoding="async";const animatedUrl=selectedAvatarEmojiItem.animatedUrl;pre.addEventListener("load",()=>{if(mini.current()&&liveImg.isConnected){try{liveImg.src=animatedUrl;}catch(_){}}});pre.src=selectedAvatarEmojiItem.animatedUrl;}}
card.appendChild(pv);const btn=el("button","mk-local-avatar-price-btn"+(owned?" is-owned":""),"");btn.innerHTML=includedStaticPreview?"Included":(owned?"Use emoji":`<span class="mk-account-shop-btn-price">${commentSvgIcon("eorbit", 12)}${escapeHtml(formatEorbits(price))}</span>`);btn.type="button";btn.disabled=includedStaticPreview;btn.title=includedStaticPreview?"Static preview is included with your animated avatar unlock.":(owned?"Use this unlocked emoji avatar.":`Unlock for ${price} EORbits.`);if(!includedStaticPreview)btn.addEventListener("click",async()=>{if(!mini.current()||mini.busy)return;if(!owned){if(!window.confirm(`Unlock ${wantsDynamic ? "dynamic" : "static"} emoji avatar ${selectedAvatarEmojiItem.emoji || ""} for ${price} EORbits?`))return;if(!mini.current())return;mini.setBusy(save,true);let paid;try{paid=await buyEmojiUnlock(selectedAvatarEmojiItem,"avatar",tier,wantsDynamic?"avatar-emoji-dynamic-preview":"avatar-emoji-static-preview");}
catch(_){paid={ok:false};}
finally{mini.setBusy(save,false);}
if(!mini.current())return;if(!paid||paid.ok===false){window.alert((paid&&paid.error==="insufficient_funds")?`Not enough EORbits. You need ${paid.price || price}.`:"Could not unlock this emoji avatar.");return;}
...
[modeInitials,modeEmoji,modeUpload].forEach((b)=>b.classList.remove("is-active"));if(avatarMode==="initials")modeInitials.classList.add("is-active");if(avatarMode==="emoji")modeEmoji.classList.add("is-active");if(avatarMode==="upload")modeUpload.classList.add("is-active");};const renderBody=()=>{if(!mini.current())return;body.innerHTML="";if(avatarMode==="initials"){body.appendChild(el("div","mk-local-avatar-note","Initials are free. Optional font, text color, and background styles cost 20 EORbits each and apply only to initials, not emoji or uploaded images. Scale is free."));const editor=el("div","mk-local-initial-editor");const makePriceHtml=(price)=>`<span class="mk-account-shop-btn-price">${commentSvgIcon("eorbit", 12)}${escapeHtml(formatEorbits(price))}</span>`;const setInitialChoice=(type,id)=>{if(!mini.current()||mini.busy)return;if(type==="font")selectedInitialFont=id;else if(type==="text")selectedInitialText=id;else if(type==="scale")selectedInitialScale=id;else selectedInitialColor=id;};const initialStylesInventory=initialAvatarStyleInventory();const makeGroup=(title,items,selectedId,type)=>{const wrap=el("div","mk-local-initial-group");wrap.appendChild(el("div","mk-local-avatar-card-title",title));const opts=el("div","mk-local-initial-options");items.forEach((item)=>{const owned=initialAvatarStyleOwned(type,item.id,initialStylesInventory);const card=el("div","mk-local-initial-option"+(item.id===selectedId?" is-active":"")+(owned?" is-owned":""),"");const select=el("button","mk-local-initial-select","");select.type="button";if(type==="text"){const textDef=initialAvatarTextColorDef(item.id);const fg=textDef.fg||"#e5e7eb";const sampleBg=item.id==="ink"?"#f8fafc":"#111827";const sampleLetter=displayInitials(curr.name||"You").slice(0,1)||"A";select.innerHTML=`<strong>${escapeHtml(item.label)}</strong><span class="mk-local-initial-color-sample" style="color:${escapeAttr(fg)};background:${escapeAttr(sampleBg)};">${escapeHtml(sampleLetter)}</span>`;}else{select.innerHTML=`<strong>${escapeHtml(item.label)}</strong>`;}
...
refreshPreview();};const setMode=(mode)=>{if(!mini.current()||mini.busy)return;avatarMode=mode;if(mode==="initials"){avatarValue="";selectedAvatarEmojiItem=null;selectedAvatarEmojiTier="";selectedFile=null;}
...
renderBody();};modeInitials.addEventListener("click",()=>setMode("initials"));modeEmoji.addEventListener("click",()=>setMode("emoji"));modeUpload.addEventListener("click",()=>setMode("upload"));file.addEventListener("change",()=>{if(!mini.current()||mini.busy)return;selectedFile=file.files&&file.files[0]?file.files[0]:null;revokeUploadPreview();if(selectedFile&&typeof URL!=="undefined"&&typeof URL.createObjectURL==="function"){try{uploadPreviewUrl=URL.createObjectURL(selectedFile);}catch(_){uploadPreviewUrl="";}}
avatarMode="upload";renderBody();});cancel.addEventListener("click",mini.close);save.addEventListener("click",async()=>{if(!mini.current()||mini.busy||!socialAccountContextIsCurrent(accountContext))return;mini.setBusy(save,true);try{if(selectedFile&&selectedFile.size>1024*1024){window.alert("Avatar image must be 1 MB or smaller.");file.focus();return;}
...
if(selectedFile){if(isCooldownActive(curr.avatarCooldownUntil))curr=await refreshProfileIfCooldownActive("avatar");if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;if(isCooldownActive(curr.avatarCooldownUntil)){window.alert(`Custom image avatars are locked until ${formatCooldownDate(curr.avatarCooldownUntil)}.`);return;}}
...
if(!window.confirm(profileChangeWarningText(false,true,!oldAvatar)))return;if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;if(avatarMode==="emoji"&&selectedAvatarEmojiItem&&selectedAvatarEmojiTier){if(!(await ensureAvatarEmojiUnlocked(selectedAvatarEmojiItem,selectedAvatarEmojiTier))){mini.setBusy(save,false);return;}
if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;}
if(pendingInitialUnlocks.length){const total=pendingInitialUnlocks.reduce((sum,row)=>sum+Number(row[1].price||0),0);if(!window.confirm(`Unlock this initials style for ${total} EORbits?`)){mini.setBusy(save,false);return;}
if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;for(const row of pendingInitialUnlocks){const paid=await buyInitialAvatarStyle(row[0],row[1].id);if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;if(!paid||paid.ok===false){mini.setBusy(save,false);window.alert((paid&&paid.error==="insufficient_funds")?`Not enough EORbits. You need ${paid.price || row[1].price}.`:"Could not unlock this initials style.");return;}}
try{window.dispatchEvent(new CustomEvent("mk-shop-inventory-change",{detail:{source:"initial-avatar-style"}}));}catch(_){}}
if(selectedFile){if(!window.confirm("Custom image avatars cost 100 EORbits per upload and can only be changed once every 7 days. Continue to payment?")){mini.setBusy(save,false);return;}
if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;const paid=await buyCustomAvatarUpload();if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;if(!paid||paid.ok===false){mini.setBusy(save,false);window.alert((paid&&paid.error==="insufficient_funds")?`Not enough EORbits. You need ${paid.price || 100}.`:"Could not process the 100 EORbits custom avatar payment.");return;}}
let res=await saveOnlineProfile(curr.name,avatarMode==="emoji"?cleanAvatarLocal(avatarValue||""):(avatarMode==="initials"?initialAvatarValue:""),{keepAvatar:avatarMode==="upload"&&(selectedFile||(!selectedFile&&isImageAvatar(curr.avatar))),privacy:curr.privacy,isPublic:curr.isPublic,bio:curr.bio||""});if(!mini.current()||!socialAccountContextIsCurrent(accountContext))return;if(!res||!res.ok){mini.setBusy(save,false);alertCooldown(res);return;}
if(selectedFile){const uploaded=await apiUploadAvatar(selectedFile,curr.name);if(!socialAccountContextIsCurrent(accountContext))return;if(!uploaded||!uploaded.ok||!uploaded.profile){mini.setBusy(save,false);if(mini.current())alertCooldown(uploaded||{error:"Could not upload this avatar. Check the R2 binding on the hot Worker."});return;}
try{writeLocalProfile(uploaded.profile,{requirePersistence:true});}
catch(_){const failure=identityProfileFailure("The avatar was saved in the cloud, but this browser could not save it locally. Free browser storage and reload.",accountContext,true);if(mini.current())alertCooldown(failure);return;}
setIdentityRecoveryStatus("",accountContext);}
if(!mini.current())return;try{await syncAllAccountDataNow({force:true,reason:"profile-avatar-save",timeoutMs:ACCOUNT_SYNC_MANUAL_TIMEOUT_MS});}catch(_){}
if(!mini.current())return;mini.setBusy(save,false);mini.close();refreshInfo();}catch(_){if(mini.current())alertCooldown({error:"Could not save your avatar. Try again."});}
finally{mini.setBusy(save,false);}});renderBody();});const makeAccountSection=(title,subtitle,className)=>{const section=el("details",`mk-account-section ${className || ""}`.trim());const summary=el("summary","mk-account-section-summary");const copy=el("span","mk-account-section-summary-copy");copy.innerHTML=`<strong>${escapeHtml(title)}</strong>${subtitle ? `<small>${escapeHtml(subtitle)}</small>` : ""}`;const arrow=el("span","mk-account-section-arrow");summary.appendChild(copy);summary.appendChild(arrow);const body=el("div","mk-account-section-body");section.appendChild(summary);section.appendChild(body);try{section.addEventListener("toggle",()=>{mkLocalScheduleAccountSectionSafeScroll();window.setTimeout(mkLocalScheduleAccountSectionSafeScroll,90);window.setTimeout(mkLocalScheduleAccountSectionSafeScroll,260);});}catch(_){}
...
syncAutoCountdown.textContent=text;syncAutoCountdown.title=title;}catch(_){}};const updateSyncIntervalUi=(value,persist)=>{if(!profileView.current())return;const next=autoSyncSliderToIntervalUi(value==null?syncIntervalSlider.value:value);syncIntervalValue.textContent=autoSyncIntervalLabelUi(next);syncIntervalSlider.setAttribute("aria-valuetext",autoSyncIntervalLabelUi(next));syncIntervalCard.classList.toggle("is-off",next===Infinity);syncIntervalCard.title=autoSyncNoteTextUi(next);if(persist)writeAutoSyncIntervalUi(next);updateAutoSyncCountdownUi();};syncIntervalSlider.addEventListener("input",()=>updateSyncIntervalUi(syncIntervalSlider.value,false));syncIntervalSlider.addEventListener("change",()=>updateSyncIntervalUi(syncIntervalSlider.value,true));const onSyncIntervalChange=(ev)=>{if(!profileView.current())return;const ms=ev&&ev.detail?ev.detail.intervalMs:readAutoSyncIntervalUi();syncIntervalSlider.value=String(autoSyncIntervalToSliderUi(ms));updateSyncIntervalUi(syncIntervalSlider.value,false);};window.addEventListener("mk-account-auto-sync-interval-change",onSyncIntervalChange);let countdownTimer=0;const stopCountdownTicker=()=>{if(countdownTimer)window.clearInterval(countdownTimer);countdownTimer=0;};const startCountdownTicker=()=>{stopCountdownTicker();if(!profileView.current())return;updateAutoSyncCountdownUi();let live=false;try{live=!!(readLocalProfile().accountKey)&&readAutoSyncIntervalUi()!==Infinity&&!document.hidden;}catch(_){live=false;}
if(live){try{countdownTimer=window.setInterval(()=>{if(profileView.current())updateAutoSyncCountdownUi();else stopCountdownTicker();},1000);}catch(_){}}};const onCountdownVisibility=()=>startCountdownTicker();window.addEventListener("mk-account-auto-sync-timer-change",startCountdownTicker);window.addEventListener("mk-account-sync-complete",startCountdownTicker);window.addEventListener("mk-account-auto-sync-interval-change",startCountdownTicker);document.addEventListener("visibilitychange",onCountdownVisibility,{passive:true});profileView.onDispose(()=>{stopCountdownTicker();window.removeEventListener("mk-account-auto-sync-interval-change",onSyncIntervalChange);window.removeEventListener("mk-account-auto-sync-timer-change",startCountdownTicker);window.removeEventListener("mk-account-sync-complete",startCountdownTicker);window.removeEventListener("mk-account-auto-sync-interval-change",startCountdownTicker);document.removeEventListener("visibilitychange",onCountdownVisibility);});startCountdownTicker();updateSyncIntervalUi(syncIntervalSlider.value,false);syncIntervalCard.appendChild(syncIntervalHead);syncIntervalCard.appendChild(syncIntervalSlider);syncIntervalCard.appendChild(syncIntervalValue);syncIntervalCard.appendChild(syncIntervalTicks);syncIntervalCard.appendChild(syncIntervalHint);syncBox.appendChild(syncIntervalCard);const progressWrap=el("div","mk-account-sync-steps-card");const syncStepDetail=el("div","mk-account-sync-step-detail","No sync running.");const syncStepDefs=[[1,"Prepare","Prepare local data."],[2,"Download","Download cloud data."],[3,"Merge","Merge both records."],[4,"Upload","Save merged data to the cloud."],[5,"Verify","Check the cloud copy."],[6,"Repair","Fix any mismatch."],[7,"Finish","Refresh account totals."],];const syncStepList=el("ol","mk-account-sync-steps");const syncStepNodes={};syncStepDefs.forEach(([num,title,help])=>{const row=el("li","mk-account-sync-step");row.dataset.step=String(num);const icon=el("span","mk-account-sync-step-icon",String(num));const copy=el("span","mk-account-sync-step-copy");copy.innerHTML=`<strong title="${escapeAttr(help)}">${escapeHtml(title)}</strong><small>${escapeHtml(help)}</small>`;row.appendChild(icon);row.appendChild(copy);syncStepList.appendChild(row);syncStepNodes[num]={row,icon,copy};});const setSyncStepState=(step,state)=>{const node=syncStepNodes[step];if(!node)return;node.row.classList.remove("is-active","is-done","is-error");if(state)node.row.classList.add(`is-${state}`);node.icon.textContent=state==="done"?"✓":(state==="error"?"!":String(step));};const resetSyncSteps=()=>{syncStepDefs.forEach(([num])=>setSyncStepState(num,""));syncStepDetail.classList.remove("is-error");syncStepDetail.textContent="No sync running.";};const updateSyncSteps=(currentStep,mode,detail)=>{const step=Math.max(1,Math.min(syncStepDefs.length,Number(currentStep||1)));const def=syncStepDefs[step-1]||[step,`Step ${step}`,"Syncing…"];syncStepDetail.classList.toggle("is-error",mode==="error");syncStepDefs.forEach(([num])=>{if(mode==="success")setSyncStepState(num,"done");else if(mode==="error"){if(num<step)setSyncStepState(num,"done");else if(num===step)setSyncStepState(num,"error");else setSyncStepState(num,"");}else{if(num<step)setSyncStepState(num,"done");else if(num===step)setSyncStepState(num,"active");else setSyncStepState(num,"");}});if(mode==="success")syncStepDetail.textContent=detail||"All 7 steps completed. Local and cloud fingerprints matched.";else if(mode==="error")syncStepDetail.textContent=`Step ${step}/7 failed: ${detail || def[2] || "Stopped here."}`;else syncStepDetail.textContent=`Step ${step}/7: ${detail || def[2] || "Running now…"}`;};resetSyncSteps();progressWrap.appendChild(syncStepList);progressWrap.appendChild(syncStepDetail);const reviewSyncHistory=el("button","mk-comment-small-btn","Review and recover local history");reviewSyncHistory.type="button";reviewSyncHistory.hidden=true;reviewSyncHistory.setAttribute("data-sync-history-review","");progressWrap.appendChild(reviewSyncHistory);syncBox.appendChild(progressWrap);const historyRecovery=mountAccountHistoryRecovery(syncBox,syncSection.section,{onRetrySync:()=>{if(!syncAll.isConnected||syncAll.disabled)return{ok:false,error:"Wait for the current sync to finish, then use Sync now."};syncAll.click();return{ok:true};}});host.__mkAccountHistoryRecoveryCleanup=historyRecovery.destroy;reviewSyncHistory.addEventListener("click",()=>historyRecovery.refresh({reveal:true,focus:true}));renderSyncStatus();try{const initialSummary=readAccountSyncSummary();if(readLocalProfile().accountKey){const CLOUD_STATUS_TTL_MS=60000;const summaryAge=Date.now()-Number(initialSummary&&(initialSummary.updatedAt||initialSummary.finishedAt)||0);if(initialSummary&&summaryAge>=0&&summaryAge<CLOUD_STATUS_TTL_MS){renderSyncStatus(initialSummary);}else{renderSyncStatus(initialSummary,"Refreshing cloud status…");refreshAccountCloudStatusForDisplay().then((fresh)=>{const current=readAccountSyncSummary();if(shouldRenderCloudStatusSummary(fresh,current))renderSyncStatus(fresh);else renderSyncStatus(current);}).catch(()=>renderSyncStatus(readAccountSyncSummary()));}}}catch(_){}
syncSection.body.appendChild(syncBox);wrap.appendChild(syncSection.section);const devicesSection=makeAccountSection("Connected devices","","mk-account-section--devices");const deviceBox=el("div","mk-account-section-content mk-local-devices-card");const currentDeviceRow=el("div","mk-local-account-action-row mk-current-device-row");const currentDeviceText=el("div","mk-local-account-action-title");currentDeviceText.innerHTML=`<strong>This device</strong><small>${escapeHtml(getDeviceName())}</small>`;const renameDevice=iconButton("mk-comment-small-btn","edit","Rename this device");renameDevice.querySelector(".mk-comment-sr").textContent="Rename";renameDevice.type="button";currentDeviceRow.appendChild(currentDeviceText);currentDeviceRow.appendChild(renameDevice);const deviceNameStatus=el("div","mk-device-name-status");deviceNameStatus.setAttribute("role","status");deviceNameStatus.hidden=true;const deviceList=el("div","mk-local-device-list");const deviceToolbar=el("div","mk-device-list-toolbar");const deviceStatus=el("div","mk-device-list-status");deviceStatus.setAttribute("role","status");const refreshDeviceList=el("button","mk-comment-small-btn mk-device-list-refresh","Refresh devices");refreshDeviceList.type="button";deviceToolbar.appendChild(deviceStatus);deviceToolbar.appendChild(refreshDeviceList);let deviceLoadGeneration=0,deviceLoading=false,deviceRenamePending=false,deviceRefreshAfterAction=false;const devicePending=new Set();const deviceViewCurrent=()=>profileView.current();profileView.onDispose(()=>{deviceLoadGeneration++;});refreshDeviceList.addEventListener("click",()=>{if(!deviceLoading&&!devicePending.size&&deviceViewCurrent())refreshDevices();});const fromRow=el("div","mk-local-account-action-row");fromRow.appendChild(el("div","mk-local-account-action-title",profile.accountKey?"Switch to another account.":"Connect this browser to an existing account."));const connect=iconButton("mk-comment-small-btn","link",profile.accountKey?"Switch account":"Enter access code");connect.type="button";const scanAccountQr=iconButton("mk-comment-small-btn","scan","Scan QR");scanAccountQr.type="button";fromRow.appendChild(connect);const toRow=el("div","mk-local-account-action-row");toRow.appendChild(el("div","mk-local-account-action-title","Create a one-time code for another device."));const gen=iconButton("mk-comment-small-btn","link","Generate one-time code");gen.type="button";toRow.appendChild(gen);const deviceCodeOut=el("div","mk-account-code-output");deviceCodeOut.hidden=true;deviceBox.appendChild(currentDeviceRow);deviceBox.appendChild(deviceNameStatus);deviceBox.appendChild(deviceToolbar);deviceBox.appendChild(deviceList);deviceBox.appendChild(toRow);deviceBox.appendChild(deviceCodeOut);devicesSection.body.appendChild(deviceBox);wrap.appendChild(devicesSection.section);const accountSection=makeAccountSection("Account access","","mk-account-section--access");const accountBox=el("div","mk-account-section-content mk-local-account-actions");const recoveryRow=el("div","mk-local-account-action-row");recoveryRow.appendChild(el("div","mk-local-account-action-title","Create or change your permanent recovery code."));const recoveryGen=iconButton("mk-comment-small-btn","key","Create or change permanent code");recoveryGen.type="button";recoveryRow.appendChild(recoveryGen);const recoverRow=el("div","mk-local-account-action-row");recoverRow.appendChild(el("div","mk-local-account-action-title","Use a saved permanent code."));const recover=iconButton("mk-comment-small-btn","key","Use saved code");recover.type="button";recoverRow.appendChild(recover);const accessCodeOut=el("div","mk-account-code-output");accessCodeOut.hidden=true;const unlinkRow=el("div","mk-local-account-action-row");const unlinkText=el("div","mk-local-account-action-title");unlinkText.innerHTML=`<strong>Remove from this browser</strong><small>Signs out here only. Cloud data and public comments stay unchanged.</small>`;const unlinkBtn=iconButton("mk-comment-small-btn","logout","Remove account from this browser");unlinkBtn.type="button";unlinkBtn.disabled=!profile.accountKey;unlinkRow.appendChild(unlinkText);unlinkRow.appendChild(unlinkBtn);const deleteRow=el("div","mk-local-account-action-row");const deleteText=el("div","mk-local-account-action-title");deleteText.innerHTML=`<strong>Delete account</strong><small>Deletes the cloud profile and synced learning data permanently.</small>`;const deleteBtn=iconButton("mk-comment-small-btn mk-comment-danger-btn","trash","Delete account permanently");deleteBtn.type="button";deleteBtn.disabled=!profile.accountKey;deleteRow.appendChild(deleteText);deleteRow.appendChild(deleteBtn);accountBox.appendChild(fromRow);accountBox.appendChild(recoveryRow);if(!profile.accountKey)accountBox.appendChild(recoverRow);accountBox.appendChild(accessCodeOut);accountBox.appendChild(unlinkRow);accountBox.appendChild(deleteRow);accountSection.body.appendChild(accountBox);wrap.appendChild(accountSection.section);connect.addEventListener("click",async()=>{if(!profileView.current()||connect.disabled)return;const curr=readLocalProfile();const nm=window.prompt(curr.accountKey?"Username on the account you want to switch to:":"Username on the existing account:",curr.accountKey?"":(curr.name||""));if(!nm)return;const code=window.prompt("Access code:","");if(!code)return;if(!profileView.current())return;connect.disabled=true;scanAccountQr.disabled=true;profileView.beginRename(nm);let res;try{res=await claimAccountAccessCode(nm,code,"manual-access-code");}catch(_){res={ok:false,error:"Could not connect. Try again."};}
profileView.finishRename(res);connect.disabled=false;scanAccountQr.disabled=false;if(res&&res.ok&&res.workspaceReloadRequired&&profileView.ownsHost())return;if(!profileView.current())return;if(!res||!res.ok){window.alert((res&&res.error)||"Could not connect to this account. Use a fresh one-time code or the saved permanent access code for that username.");return;}
window.alert(curr.accountKey?"This browser has switched to the selected account.":"This browser is now connected to your account.");refreshInfo();});gen.addEventListener("click",async()=>{if(!profileView.current()||gen.disabled)return;gen.disabled=true;deviceCodeOut.hidden=true;const res=await createSyncCode().catch(()=>({ok:false,error:"Could not create an access code. Try again."}));gen.disabled=false;if(!profileView.current())return;deviceCodeOut.hidden=false;if(!res||!res.ok){deviceCodeOut.textContent=(res&&res.error)||"Could not create an access code.";return;}
const loginUrl=accountLoginUrlFromCode(res);deviceCodeOut.innerHTML=`<strong>${escapeHtml(res.code)}</strong><span>one-time access code, expires in 10 minutes</span>${loginUrl ? `<a class="mk-account-qr-link"href="${escapeAttr(loginUrl)}"target="_blank"rel="noreferrer">Open QR login link</a>` : ""}`;});recoveryGen.addEventListener("click",async()=>{if(!profileView.current()||recoveryGen.disabled)return;const custom=window.prompt("Enter a custom permanent access code, or leave this blank to generate one automatically. Use at least 8 letters or numbers.","");if(custom==null)return;if(String(custom||"").trim()&&String(custom||"").replace(/[^a-zA-Z0-9]/g,"").length<8){window.alert("Permanent access code must contain at least 8 letters or numbers.");return;}
if(!window.confirm("Save this as the permanent access code for this account? This replaces any old saved code."))return;if(!profileView.current())return;recoveryGen.disabled=true;accessCodeOut.hidden=true;const res=await createRecoveryCode(custom).catch(()=>({ok:false,error:"Could not save the access code. Try again."}));recoveryGen.disabled=false;if(!profileView.current())return;accessCodeOut.hidden=false;if(!res||!res.ok){accessCodeOut.textContent=(res&&res.error)||"Could not save the permanent access code.";return;}
accessCodeOut.innerHTML=`<strong>${escapeHtml(res.code)}</strong><span>save this permanent access code now. It replaces the previous saved code.</span>`;});recover.addEventListener("click",async()=>{if(!profileView.current()||recover.disabled)return;const curr=readLocalProfile();const nm=window.prompt("Username on the account:",curr.name||"");if(!nm)return;const code=window.prompt("Permanent access code:","");if(!code)return;if(!profileView.current())return;recover.disabled=true;profileView.beginRename(nm);let res;try{res=await claimAccountAccessCode(nm,code,"saved-access-code");}catch(_){res={ok:false,error:"Could not connect. Try again."};}
profileView.finishRename(res);recover.disabled=false;if(res&&res.ok&&res.workspaceReloadRequired&&profileView.ownsHost())return;if(!profileView.current())return;if(!res||!res.ok){window.alert((res&&res.error)||"Could not connect to this account.");return;}
window.alert("This browser is now connected to your account.");refreshInfo();});const setSyncButtonLoading=(loading)=>{syncAll.disabled=!!loading;syncAll.classList.toggle("is-loading",!!loading);syncAll.innerHTML=loading?`${commentSvgIcon("sync", 18)}<span>Syncing</span>`:"Sync now";updateAutoSyncCountdownUi();};syncAll.addEventListener("click",async()=>{if(!profileView.current()||syncAll.disabled)return;syncSection.section.open=true;reviewSyncHistory.hidden=true;setSyncButtonLoading(true);const started=Date.now();let lastPct=6;let lastProgressInfo=null;progressWrap.classList.remove("is-error");resetSyncSteps();updateSyncSteps(1,"active","Starting sync.");const showProgress=(info)=>{if(!profileView.current())return;const p=info&&typeof info==="object"?info:{};lastProgressInfo=p;const step=Math.max(1,Number(p.step||1));const total=Math.max(step,Number(p.total||7));const chunkIndex=Number(p.chunkIndex||0);const chunkCount=Number(p.chunkCount||0);const chunkBonus=chunkCount>0?Math.min(0.85,Math.max(0,chunkIndex/Math.max(1,chunkCount))*(100/total)*0.72):0;const rawPct=Math.max(6,Math.min(96,Math.round(((step-1)/total)*100+5+chunkBonus)));lastPct=Math.max(lastPct,rawPct);const text=p.text||accountSyncProgressText(step,total,p.label||"Syncing account data",p.extra||"");const elapsed=((Date.now()-started)/1000).toFixed(1);updateSyncSteps(step,"active",`${p.label || "Running"}${p.extra ? `· ${p.extra}` : ""}`);const detailBits=[];if(chunkCount)detailBits.push(`chunk ${chunkIndex || 0}/${chunkCount}`);if(p.eventCount!=null)detailBits.push(`${p.eventCount} events`);if(p.bytes!=null)detailBits.push(`${Math.round(Number(p.bytes || 0) / 1024)} KB`);if(p.difference)detailBits.push(`local-only ${p.difference.localOnly || 0}, cloud-only ${p.difference.cloudOnly || 0}`);syncStepDetail.textContent=`Step ${step}/7: ${text} · ${elapsed}s${detailBits.length ? `· ${detailBits.join(" · ")}` : ""}`;renderSyncStatus(readAccountSyncSummary(),`Syncing: ${text}`);};showProgress({step:1,total:7,label:"Starting account sync"});const res=await syncAllAccountDataNow({force:true,reason:"account-panel-manual",timeoutMs:ACCOUNT_SYNC_MANUAL_TIMEOUT_MS,onProgress:showProgress,}).catch((err)=>({ok:false,error:err&&err.message||"Sync failed",lastProgress:lastProgressInfo}));if(!profileView.current())return;setSyncButtonLoading(false);nameBox.textContent=readLocalProfile().name||"No username yet";if(!res||res.ok===false||res.importedOk===false||res.accountJsonOk===false){progressWrap.classList.add("is-error");const failureSummary=(res&&(res.accountJsonSummary||res.accountDataSummary))||res||readAccountSyncSummary();const fp=res&&(res.failureStep||res.lastProgress)||lastProgressInfo||failureSummary&&(failureSummary.failureStep||failureSummary.lastProgress);const atStep=fp?` at Step ${fp.step || "?"}/${fp.total || "?"} (${fp.label || "syncing"}${fp.extra ? `,${fp.extra}` : ""})`:"";const msg=(res&&res.error)||(failureSummary&&failureSummary.error)||"Sync failed. Nothing was cleared. Please try again after checking the connection.";const failedStepNum=fp&&fp.step?fp.step:7;updateSyncSteps(failedStepNum,"error",msg);syncStepDetail.textContent=`Sync failed${atStep}: ${msg}`;renderSyncStatus(failureSummary,`Sync failed${atStep}: ${msg}`);if(res&&res.code==="local_history_unreadable"||failureSummary&&failureSummary.code==="local_history_unreadable"||/Local history.*could not be safely read/i.test(String(msg))){reviewSyncHistory.hidden=false;historyRecovery.refresh({reveal:true,focus:true});}
return;}
...
const serverInfo=successSummary&&successSummary.server||{};const eventCount=serverInfo.canonicalFingerprint&&serverInfo.canonicalFingerprint.eventCount||serverInfo.cloudFingerprint&&serverInfo.cloudFingerprint.eventCount||serverInfo.localFingerprint&&serverInfo.localFingerprint.eventCount;const compactNote=successSummary&&successSummary.localCompactedCache||serverInfo.localCompactedCache?" Cloud is complete; this device keeps a local cache.":"";updateSyncSteps(7,"success",eventCount?`Final check passed: ${eventCount} events in the cloud.${compactNote}`:`Final fingerprints matched.${compactNote}`);syncStepDetail.textContent=eventCount?`All 7 steps completed · ${eventCount} events verified in the cloud.${compactNote}`:`All 7 steps completed · cloud account file verified.${compactNote}`;renderSyncStatus(successSummary);try{refreshDevices();}catch(_){}
try{fetchAccountScore({force:true,reason:"post-account-sync"}).catch(()=>null);}catch(_){}});async function refreshDevices(){if(!deviceViewCurrent())return;if(devicePending.size){deviceRefreshAfterAction=true;return;}
const generation=++deviceLoadGeneration;const current=()=>deviceViewCurrent()&&generation===deviceLoadGeneration;deviceLoading=true;deviceStatus.textContent="Loading connected devices…";deviceStatus.dataset.state="loading";refreshDeviceList.setAttribute("aria-disabled","true");refreshDeviceList.textContent="Refresh devices";let res;try{res=await fetchAccountDevices({context:profileContext,isCurrent:current});}
catch(_){res={ok:false,error:"Check your connection and try again."};}
if(!current()||!deviceList.isConnected)return;deviceLoading=false;if(devicePending.size){deviceRefreshAfterAction=true;return;}
refreshDeviceList.removeAttribute("aria-disabled");if(!res||!res.ok||!Array.isArray(res.devices)){deviceStatus.textContent=(res&&res.error)?`Could not load devices: ${res.error}`:"Could not load devices. Try again.";deviceStatus.dataset.state="error";refreshDeviceList.textContent="Retry";return;}
const active=document.activeElement;const focusInList=deviceList.contains(active);const focusedDevice=focusInList&&active.closest("[data-device-id]");const focusedId=focusedDevice&&focusedDevice.dataset.deviceId;deviceList.innerHTML="";const devices=res.devices.filter((d)=>d&&!d.revokedAt);const others=devices.filter((d)=>!d.current);deviceStatus.textContent=others.length?`${others.length} other connected ${others.length === 1 ? "device" : "devices"}.`:"No other connected devices.";deviceStatus.dataset.state=others.length?"loaded":"empty";others.forEach((d)=>{const row=el("div","mk-local-device-row");row.dataset.deviceId=String(d.visitorHash||"");const text=el("div","mk-local-device-text");text.innerHTML=`<strong>${escapeHtml(d.deviceName || "Other device")}</strong><small>Last seen: ${escapeHtml(localActivityTime(d.lastSeen || d.linkedAt || 0))}</small>`;row.appendChild(text);const status=el("div","mk-device-row-status");status.setAttribute("role","status");status.hidden=true;if(d.visitorHash){const off=iconButton("mk-comment-small-btn mk-comment-danger-btn","x","Disconnect this device");off.querySelector(".mk-comment-sr").textContent="Disconnect";off.type="button";let disconnected=false;const rowCurrent=()=>deviceViewCurrent()&&row.isConnected&&deviceList.contains(row);off.addEventListener("click",async()=>{if(!rowCurrent()||disconnected||devicePending.has(d.visitorHash))return;if(!window.confirm("Disconnect this device from your account?"))return;if(!rowCurrent()||devicePending.has(d.visitorHash))return;devicePending.add(d.visitorHash);off.setAttribute("aria-disabled","true");refreshDeviceList.setAttribute("aria-disabled","true");row.setAttribute("aria-busy","true");status.hidden=false;status.dataset.state="pending";status.textContent="Disconnecting device…";let out;try{out=await disconnectAccountDevice(d.visitorHash,{context:profileContext,isCurrent:rowCurrent});}
catch(_){out={ok:false,error:"Check your connection and try again."};}
devicePending.delete(d.visitorHash);if(!rowCurrent())return;row.setAttribute("aria-busy","false");disconnected=!!(out&&out.ok);if(disconnected){row.classList.add("is-disconnected");status.dataset.state="saved";status.textContent="Device disconnected.";off.setAttribute("aria-label","Device disconnected");off.setAttribute("data-tip","Device disconnected");const remaining=deviceList.querySelectorAll(".mk-local-device-row:not(.is-disconnected)").length;deviceStatus.textContent=remaining?`${remaining} other connected ${remaining === 1 ? "device" : "devices"}.`:"No other connected devices.";deviceStatus.dataset.state=remaining?"loaded":"empty";}else{off.removeAttribute("aria-disabled");status.dataset.state="error";status.textContent=`Could not disconnect this device. ${out && out.error || "Try again."}`;}
if(!deviceLoading&&!devicePending.size)refreshDeviceList.removeAttribute("aria-disabled");if(!devicePending.size&&deviceRefreshAfterAction){deviceRefreshAfterAction=false;refreshDevices();}});row.appendChild(off);}else{status.hidden=false;status.dataset.state="error";status.textContent="Device details are incomplete. Refresh the device list.";}
row.appendChild(status);deviceList.appendChild(row);});if(focusInList){const same=Array.from(deviceList.querySelectorAll("[data-device-id]")).find(row=>row.dataset.deviceId===focusedId);try{(same&&same.querySelector("button")||refreshDeviceList).focus({preventScroll:true});}catch(_){}}}
renameDevice.addEventListener("click",async()=>{if(!deviceViewCurrent()||!renameDevice.isConnected||deviceRenamePending)return;const nm=window.prompt("Name this device:",getDeviceName());if(nm==null||!deviceViewCurrent())return;deviceRenamePending=true;renameDevice.setAttribute("aria-disabled","true");deviceNameStatus.hidden=false;deviceNameStatus.dataset.state="pending";deviceNameStatus.textContent="Saving device name…";let out;try{out=await saveCurrentDeviceName(nm,{context:profileContext,isCurrent:deviceViewCurrent});}
catch(_){out={ok:false,error:"Check your connection and try again."};}
deviceRenamePending=false;if(!deviceViewCurrent()||!renameDevice.isConnected)return;renameDevice.removeAttribute("aria-disabled");currentDeviceText.innerHTML=`<strong>This device</strong><small>${escapeHtml(getDeviceName())}</small>`;deviceNameStatus.dataset.state=out&&out.ok?"saved":"error";deviceNameStatus.textContent=out&&out.ok?"Device name saved.":`Could not sync the device name. This browser uses “${getDeviceName()}”. ${out && out.error || "Try again."}`;});refreshDevices();unlinkBtn.addEventListener("click",async()=>{if(!profileView.current()||unlinkBtn.disabled||!profile.accountKey)return;const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext)||String(profile.accountKey).trim().toLowerCase()!==accountContext.accountKey){refreshInfo();return;}
if(!window.confirm("Remove this account from this browser? Your cloud account will not be deleted."))return;const deleteLocal=window.confirm("Also delete this account's local data from this browser?\n\nPress Cancel to keep its protected offline cache for the next login.");if(!profileView.current()||!socialAccountContextIsCurrent(accountContext))return;unlinkBtn.disabled=true;try{let res=null;try{res=await unlinkThisBrowserAccount();}catch(_){}
if(!socialAccountContextIsCurrent(accountContext)||(res&&res.error==="account_workspace_changed")){refreshInfo();return;}
const cleared=await clearLocalAccountState({clearActivity:deleteLocal});if(!cleared||!cleared.ok){window.alert(cleared&&cleared.error||"This browser could not remove the local account. Please retry.");return;}
window.alert(deleteLocal?"This browser is no longer connected, and its local account data was removed.":"This browser is no longer connected. Its local account data remains protected for the next login.");try{window.location.reload();}catch(_){refreshInfo();}}finally{unlinkBtn.disabled=!readLocalProfile().accountKey;}});deleteBtn.addEventListener("click",async()=>{if(!profileView.current()||deleteBtn.disabled||!profile.accountKey)return;const accountContext=captureSocialAccountContext();if(!socialAccountContextIsCurrent(accountContext)||String(profile.accountKey).trim().toLowerCase()!==accountContext.accountKey){refreshInfo();return;}
if(!window.confirm("Delete this account permanently? Existing comments will remain visible as ‘Deleted account’."))return;const typed=window.prompt("Type DELETE to confirm permanent account deletion:","");if(String(typed||"").trim().toUpperCase()!=="DELETE")return;if(!profileView.current()||!socialAccountContextIsCurrent(accountContext))return;deleteBtn.disabled=true;try{const res=await deleteCurrentCloudAccount();if(!socialAccountContextIsCurrent(accountContext)){refreshInfo();return;}
if(!res||!res.ok){window.alert((res&&res.error)||"Could not delete this account.");return;}
const cleared=await clearLocalAccountState({clearActivity:true});if(!cleared||!cleared.ok){window.alert("The cloud account was deleted, but this browser could not finish clearing its local account. "+String(cleared&&cleared.error||"Please retry."));return;}
window.alert("The account has been deleted. Existing comments now show as Deleted account.");try{window.location.reload();}catch(_){refreshInfo();}}finally{deleteBtn.disabled=!readLocalProfile().accountKey;}});host.appendChild(wrap);historyRecovery.refresh();}
function privacyToggle(label,desc,key,privacy){const row=el("label","mk-privacy-row");const select=document.createElement("select");select.dataset.privacyKey=key;select.className="mk-privacy-select";select.setAttribute("aria-label",label);[["private","Private"],["connections","Connections only"],["public","Public"],].forEach(([value,text])=>{const opt=document.createElement("option");opt.value=value;opt.textContent=text;select.appendChild(opt);});const p=normalisePrivacy(privacy,false);select.value=visibilityValue(p[key.replace(/Public$/,"Visibility")]||(p[key]?"public":"private"));const text=el("span","mk-privacy-text");text.innerHTML=`<strong>${escapeHtml(label)}</strong><small>${escapeHtml(desc)}</small>`;row.appendChild(select);row.appendChild(text);return row;}
...
function renderPrivacySettings(host){if(!host)return;host.innerHTML="";const profile=readLocalProfile();const accountContext=captureSocialAccountContext();const wrap=el("div","mk-privacy-card");let draftRevision=0;let saving=false;let submittedRevision=0;let loading=!!profile.accountKey;let savedKnown=false;let error="";const formIsMounted=()=>wrap.isConnected&&wrap.parentNode===host;const currentForm=()=>{if(!formIsMounted())return false;if(!socialAccountContextIsCurrent(accountContext)){renderPrivacySettings(host);return false;}
return true;};const markEdited=()=>{if(!currentForm())return;draftRevision+=1;error="";updateStatus();};wrap.appendChild(el("div","mk-local-activity-meta",profile.accountKey?"Choose who can see each part of your profile. Connections means accepted study connections.":"Save or connect a username first. Until then, activity stays private and local."));const p=normalisePrivacy(profile.privacy,profile.isPublic);let savedPrivacy=p;const list=el("div","mk-privacy-list");list.appendChild(privacyToggle("Profile name & avatar, including intro","Who can open your profile card. Your avatar and intro appear on public rankings only when this is Public.","profilePublic",p));list.appendChild(privacyToggle("User rankings","Public lists your username, XP, quiz results, lifetime shop spending and mastered-concept count. Avatar and intro also require the Profile setting to be Public.","rankingPublic",p));list.appendChild(privacyToggle("Visit history","Who can see recently visited concepts.","visitsPublic",p));list.appendChild(privacyToggle("Saved pages","Who can see Favourites, Study later, and Review later.","actionsPublic",p));list.appendChild(privacyToggle("Comments","Who can see recent comments on your profile.","commentsPublic",p));list.appendChild(privacyToggle("Readiness","Whether readiness contributes to public averages and your profile.","readinessPublic",p));list.querySelectorAll("select[data-privacy-key]").forEach(input=>{input.addEventListener("input",markEdited);input.addEventListener("change",markEdited);});wrap.appendChild(list);const quick=el("div","mk-local-profile-actions mk-privacy-actions");const allPub=el("button","mk-privacy-action-btn","All public");const allConn=el("button","mk-privacy-action-btn","Connections only");const allPriv=el("button","mk-privacy-action-btn","All private");const save=el("button","mk-privacy-action-btn mk-privacy-save","Save settings");const status=el("div","mk-privacy-status");status.setAttribute("role","status");status.setAttribute("aria-live","polite");status.setAttribute("aria-atomic","true");function updateStatus(){const dirty=JSON.stringify(readPrivacyForm(wrap))!==JSON.stringify(savedPrivacy);let state="saved",message="Saved";if(saving){state="saving";message=draftRevision!==submittedRevision?"Saving… Newer changes are still unsaved.":"Saving…";}else if(error){state="error";message=error;}
else if(!profile.accountKey){state="local";message="Connect a username to save settings.";}
else if(dirty){state="dirty";message="Unsaved changes";}
else if(loading){state="loading";message="Loading saved settings…";}
else if(!savedKnown){state="dirty";message="Save to confirm these settings.";}
status.setAttribute("data-state",state);status.textContent=message;save.disabled=saving;save.setAttribute("aria-busy",saving?"true":"false");}
allPub.type=allConn.type=allPriv.type=save.type="button";const applyPreset=value=>{if(!currentForm())return;applyPrivacyToForm(wrap,defaultPrivacy(value));markEdited();};allPub.addEventListener("click",()=>applyPreset(true));allConn.addEventListener("click",()=>applyPreset("connections"));allPriv.addEventListener("click",()=>applyPreset(false));save.addEventListener("click",async()=>{if(!currentForm()||saving)return;const next=readPrivacyForm(wrap);submittedRevision=++draftRevision;saving=true;loading=false;error="";updateStatus();let res;try{res=await savePrivacySettings(next);}
catch(_){res={ok:false,error:"Could not save privacy settings. Please try again."};}
finally{saving=false;}
if(!currentForm())return;if(!res||!res.ok){error=(res&&res.error)||"Could not save privacy settings. Please try again.";updateStatus();return;}
const hasNewerDraft=draftRevision!==submittedRevision;savedPrivacy=normalisePrivacy(res.privacy||next,false);savedKnown=true;loading=false;if(!hasNewerDraft)applyPrivacyToForm(wrap,savedPrivacy);updateStatus();try{refreshReadinessCardSoon(100);}catch(_){}});quick.appendChild(allPub);quick.appendChild(allConn);quick.appendChild(allPriv);quick.appendChild(save);wrap.appendChild(status);wrap.appendChild(quick);host.appendChild(wrap);updateStatus();const initialRevision=draftRevision;fetchPrivacySettings().then((res)=>{if(!currentForm()||submittedRevision)return;loading=false;if(res&&res.ok&&res.privacy){savedPrivacy=normalisePrivacy(res.privacy,false);savedKnown=true;if(draftRevision===initialRevision)applyPrivacyToForm(wrap,savedPrivacy);}else if(profile.accountKey){error="Could not refresh saved settings. Your current choices are kept.";}
updateStatus();}).catch(()=>{if(!currentForm()||submittedRevision)return;loading=false;if(profile.accountKey)error="Could not refresh saved settings. Your current choices are kept.";updateStatus();});}
...
async function renderConnections(host){if(!host||!host.isConnected||host.dataset.type!=="connections")return;if(typeof host.__mkDisposeConnections==="function")host.__mkDisposeConnections();const context=captureSocialAccountContext();let disposed=false,listSerial=0,searchSerial=0,searchTimer=0,draftRevision=0;let loading=false,activeActions=0,formBusy=false;const busyPeople=new Set();const wrap=el("div","mk-connections-card");const toolbar=el("div","mk-connections-toolbar");toolbar.appendChild(el("div","mk-connections-scope",context.accountKey?"Profile sections marked Connections only are shared here.":"Save or connect a username before using study connections."));const refresh=el("button","mk-comment-small-btn mk-connections-refresh","Refresh");refresh.type="button";refresh.setAttribute("aria-label","Refresh connections");toolbar.appendChild(refresh);wrap.appendChild(toolbar);const form=el("form","mk-connection-request-form");const label=el("label","mk-connection-request-label","Username");const input=document.createElement("input");input.type="text";input.placeholder="Username to connect with";input.autocomplete="off";input.setAttribute("aria-label","Username to connect with");label.appendChild(input);const send=iconButton("mk-comment-primary-btn","link","Send connection request");send.type="submit";send.dataset.primary="true";form.appendChild(label);form.appendChild(send);const requestStatus=el("div","mk-connection-request-status");requestStatus.setAttribute("role","status");requestStatus.setAttribute("aria-live","polite");const searchResults=el("div","mk-connection-search-results");const status=el("div","mk-connections-status");status.setAttribute("role","status");status.setAttribute("aria-live","polite");const retry=el("button","mk-comment-small-btn mk-connections-retry","Retry");retry.type="button";retry.hidden=true;retry.setAttribute("aria-label","Retry loading connections");const sections=el("div","mk-connections-sections");[form,requestStatus,searchResults,status,retry,sections].forEach(node=>wrap.appendChild(node));host.innerHTML="";host.appendChild(wrap);const ownsView=()=>!disposed&&host.__mkDisposeConnections===dispose&&host.isConnected&&wrap.isConnected&&host.dataset.type==="connections";const current=()=>ownsView()&&socialAccountContextIsCurrent(context);const events=["mk-account-workspace-changed","mk-account-login-change","mk-account-workspaces-ready","storage"];function dispose(){disposed=true;listSerial+=1;searchSerial+=1;window.clearTimeout(searchTimer);events.forEach(name=>window.removeEventListener(name,checkIdentity));if(host.__mkDisposeConnections===dispose)delete host.__mkDisposeConnections;}
function checkIdentity(){if(!ownsView()||socialAccountContextIsCurrent(context))return;const hadFocus=wrap.contains(document.activeElement);dispose();wrap.innerHTML="";wrap.dataset.state="stale";const message=el("div","mk-connections-status","Account changed. Reload connections.");message.setAttribute("role","status");const reload=el("button","mk-comment-small-btn mk-connections-retry","Reload");reload.type="button";reload.addEventListener("click",()=>{if(!reload.isConnected||host.dataset.type!=="connections"||window.__mkAccountWorkspaceSwitching)return;const focused=document.activeElement===reload;renderConnections(host);if(focused){try{host.querySelector(".mk-connections-refresh").focus({preventScroll:true});}catch(_){}}});wrap.appendChild(message);wrap.appendChild(reload);if(hadFocus){try{reload.focus({preventScroll:true});}catch(_){}}}
function guard(){if(!ownsView()){dispose();return false;}
checkIdentity();return current();}
function controls(){refresh.setAttribute("aria-disabled",String(loading||activeActions>0||!context.accountKey));retry.setAttribute("aria-disabled",String(loading||activeActions>0));send.setAttribute("aria-disabled",String(loading||formBusy||!context.accountKey));wrap.querySelectorAll(".mk-connection-row button, .mk-connection-search-row .mk-comment-small-btn").forEach(button=>{const row=button.closest(".mk-connection-row, .mk-connection-search-row");button.setAttribute("aria-disabled",String(loading||button.dataset.settled==="true"||row&&["busy","success"].includes(row.dataset.state)));});}
function feedback(node,state,text){node.dataset.state=state;node.textContent=text;}
function clearSearch(){window.clearTimeout(searchTimer);searchSerial+=1;searchResults.innerHTML="";}
function successText(result){return result.status==="accepted"?"You are now connected.":result.alreadyPending?"Connection request was already sent.":"Connection request sent.";}
function actionRow(row,buttons,key,output){let busy=false,done=false;return async(button,operation,pending,complete,onSuccess,confirm)=>{if(!guard()||!row.isConnected||loading||busy||done||busyPeople.has(key))return;if(confirm&&!window.confirm(confirm))return;if(!guard())return;busy=true;activeActions+=1;busyPeople.add(key);controls();row.dataset.state="busy";feedback(output,"busy",pending);buttons.forEach(btn=>btn.setAttribute("aria-disabled","true"));let result;try{result=await operation(current);}catch(_){result={ok:false};}
if(!guard())return;busy=false;activeActions-=1;busyPeople.delete(key);controls();if(!row.isConnected)return;if(!result||!result.ok){row.dataset.state="error";feedback(output,"error",result&&result.error||"Could not update connection. Try again.");buttons.forEach(btn=>btn.setAttribute("aria-disabled","false"));return;}
done=true;row.dataset.state="success";const text=typeof complete==="function"?complete(result):complete;button.textContent=text;button.setAttribute("aria-label",text);feedback(output,"success",text);buttons.filter(btn=>btn!==button).forEach(btn=>{const focused=document.activeElement===btn;btn.hidden=true;if(focused){try{button.focus({preventScroll:true});}catch(_){}}});if(onSuccess)onSuccess(result);};}
function renderSearchResults(users,query,serial){searchResults.innerHTML="";if(!users.length){searchResults.appendChild(el("div","mk-local-activity-empty mk-connection-search-empty","No matching usernames."));return;}
users.forEach((user)=>{const row=el("div","mk-connection-search-row");row.dataset.state="idle";const profileBtn=el("button","mk-connection-search-profile");profileBtn.type="button";profileBtn.innerHTML=connectionPersonHtml({otherName:user.name,otherAvatar:user.avatar,avatarFrame:user.avatarFrame},"search");const searchCurrent=()=>guard()&&row.isConnected&&serial===searchSerial&&cleanProfileNameLocal(input.value||"")===query;profileBtn.addEventListener("click",()=>{if(searchCurrent())openPublicProfile(user.name||"",{accountKey:user.accountKey||"",source:"connections-search"});});row.appendChild(profileBtn);const state=el("span","mk-connection-search-status",connectionStatusText(user.connectionStatus));row.appendChild(state);const settled=user.connectionStatus==="accepted"||user.connectionStatus==="outgoing";const action=iconButton("mk-comment-small-btn",user.connectionStatus==="incoming"?"check":"link",user.connectionStatus==="incoming"?"Accept":user.connectionStatus==="accepted"?"Connected":user.connectionStatus==="outgoing"?"Request sent":"Add connection");action.type="button";action.dataset.settled=String(settled);action.setAttribute("aria-disabled",String(settled));const output=el("div","mk-connection-feedback");output.setAttribute("role","status");const run=actionRow(row,[action],String(user.accountKey||user.name).toLowerCase(),output);action.addEventListener("click",()=>{if(settled||!searchCurrent())return;run(action,isCurrent=>user.connectionStatus==="incoming"?respondConnection(user.accountKey,"accept",{isCurrent}):requestConnection(user.name||user.accountKey,{isCurrent}),"Updating connection…",result=>result.status==="accepted"?"Connected":"Request sent",result=>{state.textContent=result.status==="accepted"?"Connected":"Request sent";});});row.appendChild(action);row.appendChild(output);searchResults.appendChild(row);});}
async function runSearch(){if(!guard()||!context.accountKey)return;const query=cleanProfileNameLocal(input.value||"");const serial=++searchSerial;if(!query){searchResults.innerHTML="";return;}
searchResults.innerHTML="";searchResults.appendChild(el("div","mk-local-activity-empty mk-connection-search-empty","Searching usernames…"));let res;try{res=await searchConnections(query);}catch(_){res={ok:false};}
if(!guard()||serial!==searchSerial||cleanProfileNameLocal(input.value||"")!==query)return;if(!res||!res.ok||!Array.isArray(res.users)||res.users.some(user=>!user||typeof user!=="object")){searchResults.innerHTML="";searchResults.appendChild(el("div","mk-local-activity-empty mk-connection-search-empty",(res&&res.error)||"Could not search usernames."));const again=el("button","mk-comment-small-btn mk-connection-search-retry","Retry search");again.type="button";again.addEventListener("click",()=>{if(guard()&&serial===searchSerial){input.focus({preventScroll:true});runSearch();}});searchResults.appendChild(again);return;}
renderSearchResults(res.users,query,serial);}
input.addEventListener("input",()=>{draftRevision+=1;clearSearch();if(!guard())return;searchTimer=window.setTimeout(runSearch,220);});form.addEventListener("submit",async(ev)=>{ev.preventDefault();if(!guard()||!context.accountKey||loading||formBusy)return;const target=cleanProfileNameLocal(input.value||"");if(!target){input.focus();return;}
const key=target.toLowerCase(),revision=draftRevision;if(busyPeople.has(key))return;formBusy=true;activeActions+=1;busyPeople.add(key);controls();feedback(requestStatus,"busy",`Sending request to ${target}…`);let res;try{res=await requestConnection(target,{isCurrent:current});}catch(_){res={ok:false};}
if(!guard())return;formBusy=false;activeActions-=1;busyPeople.delete(key);controls();if(!res||!res.ok){feedback(requestStatus,"error",res&&res.error||"Could not send connection request. Try again.");return;}
if(draftRevision===revision&&cleanProfileNameLocal(input.value||"")===target){input.value="";clearSearch();}
feedback(requestStatus,"success",`${target}: ${successText(res)}`);});function section(title,arr,kind){const group=el("section","mk-connections-section");group.dataset.kind=kind;const h=el("h3","mk-connections-heading",title);const count=el("span","mk-connections-count");let handled=0;const updateCount=()=>{count.textContent=`${arr.length - handled}${kind === "accepted" ? " connected" : " pending"}${handled ? `· ${handled}handled` : ""}`;};updateCount();h.appendChild(count);group.appendChild(h);const list=el("div","mk-connections-list");group.appendChild(list);sections.appendChild(group);if(!arr.length){list.appendChild(el("div","mk-local-activity-empty","None yet."));return;}
arr.forEach((item)=>{const row=el("div","mk-local-activity-row mk-connection-row");row.dataset.state="idle";const main=el("div","mk-local-activity-main");main.innerHTML=connectionPersonHtml(item,kind);row.appendChild(main);const actions=el("div","mk-local-activity-actions");const output=el("div","mk-connection-feedback");output.setAttribute("role","status");const handledRow=()=>{handled+=1;updateCount();};if(kind==="incoming"){const accept=iconButton("mk-comment-small-btn","check","Accept");const decline=iconButton("mk-comment-small-btn mk-comment-danger-btn","dismiss","Decline");accept.type=decline.type="button";accept.dataset.primary="true";const run=actionRow(row,[accept,decline],String(item.requesterKey||item.otherAccountKey).toLowerCase(),output);accept.addEventListener("click",()=>run(accept,isCurrent=>respondConnection(item.requesterKey,"accept",{isCurrent}),"Accepting…","Accepted",handledRow));decline.addEventListener("click",()=>run(decline,isCurrent=>respondConnection(item.requesterKey,"decline",{isCurrent}),"Declining…","Declined",handledRow));actions.appendChild(accept);actions.appendChild(decline);}else if(kind==="accepted"){const remove=el("button","mk-comment-small-btn mk-comment-danger-btn mk-connection-remove-btn");remove.type="button";remove.innerHTML=`${commentSvgIcon("trash", 16)}<span>Remove</span>`;remove.setAttribute("aria-label","Remove connection");remove.setAttribute("title","Remove connection");const run=actionRow(row,[remove],String(item.otherAccountKey).toLowerCase(),output);remove.addEventListener("click",()=>run(remove,isCurrent=>removeConnection(item.otherAccountKey,{isCurrent}),"Removing…","Removed",handledRow,"Remove this study connection?"));actions.appendChild(remove);}else{actions.appendChild(el("span","mk-local-activity-meta","pending"));}
row.appendChild(actions);row.appendChild(output);list.appendChild(row);});}
async function load(){if(!guard()||loading||activeActions||!context.accountKey)return;const serial=++listSerial;loading=true;controls();wrap.dataset.state="loading";feedback(status,"loading","Loading connections…");let data;try{data=await fetchConnections();}catch(_){data={ok:false};}
if(!guard()||serial!==listSerial)return;loading=false;controls();if(!data||!data.ok||["connections","incoming","outgoing"].some(key=>!Array.isArray(data[key])||data[key].some(item=>!item||typeof item!=="object"||Array.isArray(item)))){wrap.dataset.state="error";feedback(status,"error",data&&data.error||"Could not load connections.");retry.hidden=false;return;}
const focusedRetry=document.activeElement===retry;retry.hidden=true;if(focusedRetry){try{refresh.focus({preventScroll:true});}catch(_){}}
sections.innerHTML="";section("Incoming requests",data.incoming,"incoming");section("Study connections",data.connections,"accepted");section("Sent requests",data.outgoing,"outgoing");wrap.dataset.state="ready";feedback(status,"success","");markConnectionsSeen();}
host.__mkDisposeConnections=dispose;events.forEach(name=>window.addEventListener(name,checkIdentity));refresh.addEventListener("click",load);retry.addEventListener("click",load);controls();if(!context.accountKey){wrap.dataset.state="guest";return;}
return load();}
...
async function activateXpCapBoostVoucher(voucherId,options){const opts=options||{};const context=captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!opts.isCurrent||opts.isCurrent());const id=String(voucherId||"").trim();if(!id||!current())return opts.silent?{ok:false}:false;const res=await apiPost("/xp-voucher/activate",{visitorId:context.visitorId,voucherId:id});if(!current())return opts.silent?socialAccountChangedResult():false;if(res&&res.ok){refreshAccountXpSoon("cap-boost-voucher",80);updateNotificationBadgesSoon(120);return opts.silent?res:true;}
if(!opts.silent)window.alert((res&&res.error)||"Could not activate this voucher.");return opts.silent?(res||{ok:false,error:"Could not activate this voucher."}):false;}
async function openAnonymousCommentClaims(item,options){const opts=options||{};const context=captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context)&&(!opts.isCurrent||opts.isCurrent());if(!current())return socialAccountChangedResult();const res=await apiPost("/comments/claim-notice/seen",{visitorId:context.visitorId});if(!current())return socialAccountChangedResult();if(!res||!res.ok){if(!opts.silent)window.alert((res&&res.error)||"Could not open the anonymous comment review.");return res||{ok:false,error:"Could not open the anonymous comment review."};}
try{const activity=await fetchCloudActivity();if(!current())return socialAccountChangedResult();if(!activity||!activity.ok)return{ok:false,error:"Could not load comment review. Try again."};applyCloudActivity(activity);}catch(_){return{ok:false,error:"Could not load comment review. Try again."};}
if(!current())return socialAccountChangedResult();saveLocalFoldOpen(localFoldStateKey("activity","visits"),false);saveLocalFoldOpen(localFoldStateKey("activity","comments"),true);openLocalActivity("activity");const opened=document.querySelector('.mk-local-activity-modal[data-type="activity"]');window.setTimeout(()=>{try{if(!opened||!opened.isConnected||!socialAccountContextIsCurrent(context)||opened.dataset.type!=="activity")return;const fold=opened.querySelector('details[data-fold-key="activity:comments"]');if(fold){fold.open=true;fold.scrollIntoView({block:"start",behavior:"smooth"});}}catch(_){}},80);updateNotificationBadgesSoon(120,{force:true,reason:"anonymous-comment-claim-open"});return{ok:true};}
async function renderNotifications(host){if(!host||!host.isConnected||host.dataset.type!=="notifications")return;if(typeof host.__mkDisposeNotifications==="function")host.__mkDisposeNotifications();const context=captureSocialAccountContext();let disposed=false,serial=0,loading=false,activeActions=0,seenBusy=false;let seenDone=false,loaded=false,readOptions={};const view=el("div","mk-notifications-view");const toolbar=el("div","mk-notifications-toolbar");const count=el("div","mk-notifications-count","Recent alerts");const refresh=el("button","mk-comment-small-btn mk-notifications-refresh","Refresh");refresh.type="button";refresh.setAttribute("aria-label","Refresh notifications");toolbar.appendChild(count);toolbar.appendChild(refresh);const status=el("div","mk-notifications-status");status.setAttribute("role","status");status.setAttribute("aria-live","polite");const retry=el("button","mk-comment-small-btn mk-notifications-retry","Retry");retry.type="button";retry.hidden=true;retry.setAttribute("aria-label","Retry loading notifications");const seenStatus=el("div","mk-notifications-seen-status");seenStatus.setAttribute("role","status");seenStatus.setAttribute("aria-live","polite");const seenRetry=el("button","mk-comment-small-btn mk-notifications-seen-retry","Retry read status");seenRetry.type="button";seenRetry.hidden=true;const list=el("div","mk-notifications-list");[toolbar,status,retry,seenStatus,seenRetry,list].forEach(node=>view.appendChild(node));host.innerHTML="";host.appendChild(view);const ownsView=()=>!disposed&&host.__mkDisposeNotifications===dispose&&host.isConnected&&view.isConnected&&host.dataset.type==="notifications";const current=()=>ownsView()&&socialAccountContextIsCurrent(context);const lifecycleEvents=["mk-account-workspace-changed","mk-account-login-change","mk-account-workspaces-ready","storage"];function dispose(){disposed=true;serial+=1;lifecycleEvents.forEach(name=>window.removeEventListener(name,checkIdentity));document.removeEventListener("visibilitychange",onVisible);if(host.__mkDisposeNotifications===dispose)delete host.__mkDisposeNotifications;}
function checkIdentity(){if(!ownsView()||socialAccountContextIsCurrent(context))return;const hadFocus=view.contains(document.activeElement);dispose();view.innerHTML="";view.dataset.state="stale";const message=el("div","mk-notifications-status","Account changed. Reload notifications.");message.setAttribute("role","status");const reload=el("button","mk-comment-small-btn mk-notifications-retry","Reload");reload.type="button";reload.addEventListener("click",()=>{if(reload.isConnected&&host.dataset.type==="notifications"&&!window.__mkAccountWorkspaceSwitching){const hadFocus=document.activeElement===reload;renderNotifications(host);if(hadFocus){try{host.querySelector(".mk-notifications-refresh").focus({preventScroll:true});}catch(_){}}}});view.appendChild(message);view.appendChild(reload);if(hadFocus){try{reload.focus({preventScroll:true});}catch(_){}}}
function guard(){if(!ownsView()){dispose();return false;}
checkIdentity();return current();}
function controls(){refresh.setAttribute("aria-disabled",String(loading||activeActions>0||seenBusy));retry.setAttribute("aria-disabled",String(loading));seenRetry.setAttribute("aria-disabled",String(seenBusy));}
async function markVisibleRead(){if(!guard()||!loaded||loading||seenBusy||seenDone||document.visibilityState==="hidden")return;const epoch=serial;seenBusy=true;controls();seenStatus.dataset.state="loading";seenStatus.textContent="Updating read status…";let result;try{result=await markNotificationsSeen(Object.assign({},readOptions,{context,isCurrent:()=>current()&&serial===epoch}));}
catch(_){result={ok:false};}
if(!guard()||serial!==epoch)return;seenBusy=false;controls();if(result&&result.ok){seenDone=true;seenStatus.textContent="";seenStatus.dataset.state="success";const hadFocus=document.activeElement===seenRetry;seenRetry.hidden=true;if(hadFocus){try{refresh.focus({preventScroll:true});}catch(_){}}}else{seenStatus.dataset.state="error";seenStatus.textContent=result&&result.error||"Could not update read status.";seenRetry.hidden=false;}}
function onVisible(){if(document.visibilityState!=="hidden")markVisibleRead();}
async function load(){if(!guard()||loading||activeActions||seenBusy)return;const epoch=++serial;loading=true;loaded=false;seenDone=false;view.dataset.state="loading";status.dataset.state="loading";status.textContent="Loading notifications…";seenStatus.textContent="";seenRetry.hidden=true;list.innerHTML="";count.textContent="Recent alerts";controls();let data;try{const qs=new URLSearchParams({visitorId:context.visitorId,limit:"80"});data=await apiGet(`/notifications?${qs.toString()}`);}catch(_){data={ok:false};}
if(!guard()||serial!==epoch)return;loading=false;controls();if(!data||!data.ok||!Array.isArray(data.notifications)){view.dataset.state="error";status.dataset.state="error";status.textContent="Could not load notifications.";retry.hidden=false;return;}
const arr=data.notifications.filter(item=>item&&typeof item==="object"&&!Array.isArray(item));if(arr.length!==data.notifications.length){view.dataset.state="error";status.dataset.state="error";status.textContent="Could not load notifications.";retry.hidden=false;return;}
const retryHadFocus=document.activeElement===retry;retry.hidden=true;if(retryHadFocus){try{refresh.focus({preventScroll:true});}catch(_){}}
touchNotificationBadgeRefresh();count.textContent=`Recent alerts · ${arr.length} shown`;view.dataset.state=arr.length?"ready":"empty";status.dataset.state="success";status.textContent=arr.length?"":"No notifications yet.";const previousSeen=Math.max(Number(data.seenAt)||0,getNotificationSeenAt());readOptions={remainingUnread:arr.filter(item=>item.forceUnread).length,hadUnread:arr.some(item=>Number(item.createdAt)>previousSeen),latestNotificationAt:arr.reduce((latest,item)=>Number.isFinite(Number(item.createdAt))?Math.max(latest,Number(item.createdAt)):latest,0),};renderRows(arr,epoch);loaded=true;await markVisibleRead();}
refresh.addEventListener("click",load);retry.addEventListener("click",load);seenRetry.addEventListener("click",markVisibleRead);host.__mkDisposeNotifications=dispose;lifecycleEvents.forEach(name=>window.addEventListener(name,checkIdentity));document.addEventListener("visibilitychange",onVisible);function renderRows(arr,epoch){arr.forEach((item)=>{const row=el("div","mk-local-activity-row mk-notification-row"+(item.type==="xp_cap_boost_voucher"?" is-xp-voucher":"")+(item.type==="anonymous_comment_claim"?" is-anonymous-claim":""));row.dataset.state="idle";const feedback=el("div","mk-notification-feedback");feedback.dataset.state="idle";feedback.setAttribute("role","status");feedback.setAttribute("aria-live","polite");let busy=false,done=false;const rowCurrent=()=>guard()&&serial===epoch&&row.isConnected;async function act(button,buttons,operation,pendingText,successText,failureText,afterSuccess){if(!rowCurrent()||busy||done||loading)return;busy=true;activeActions+=1;controls();row.dataset.state=feedback.dataset.state="busy";feedback.textContent=pendingText;buttons.forEach(btn=>btn.setAttribute("aria-disabled","true"));let result;try{result=await operation(rowCurrent);}catch(_){result={ok:false};}
if(!rowCurrent())return;busy=false;activeActions-=1;controls();if(!result||!result.ok){row.dataset.state=feedback.dataset.state="error";feedback.textContent=result&&typeof result.error==="string"?result.error:failureText;buttons.forEach(btn=>btn.setAttribute("aria-disabled","false"));return;}
done=true;row.dataset.state=feedback.dataset.state="success";feedback.textContent=successText;button.textContent=successText;buttons.filter(btn=>btn!==button).forEach(btn=>{const hadFocus=document.activeElement===btn;btn.hidden=true;if(hadFocus){try{button.focus({preventScroll:true});}catch(_){}}});if(afterSuccess)afterSuccess(result);}
const main=el("div","mk-local-activity-main");const left=el("div","mk-notification-left");const line=el("div","mk-notification-line");let messageHtml="";if(item.type==="reply")messageHtml=`<span>replied to your comment</span>`;else if(item.type==="reaction")messageHtml=`<span>reacted ${escapeHtml(reactionSymbol(item.reaction))} to your comment</span>`;else if(item.type==="mention")messageHtml=`<span>mentioned you in a comment</span>`;else if(item.type==="connection_request")messageHtml=`<span>sent you a study connection request</span>`;else if(item.type==="connection_accepted")messageHtml=`<span>accepted your study connection request</span>`;else if(item.type==="connection_declined")messageHtml=`<span>declined your study connection request</span>`;else if(item.type==="connection_removed")messageHtml=`<span>removed your study connection</span>`;else if(item.type==="xp_cap_boost_voucher")messageHtml=`<span>sent you a Daily XP Cap Boost voucher</span>`;else if(item.type==="anonymous_comment_claim")messageHtml=`<span>${escapeHtml(item.notificationTitle || "found anonymous comments from this device")}</span>`;else if(item.notificationTitle||item.notificationMessage)messageHtml=`<span>${escapeHtml(item.notificationTitle || "sent you a notification")}</span>`;else messageHtml=`<span>sent you a notification</span>`;line.innerHTML=notificationActorHtml(item)+messageHtml;left.appendChild(line);if(item.path){const a=el("a","mk-local-activity-link",activityTitle(item));a.href=pageHref(item.path||"");a.setAttribute("data-path",normActivityTitlePath(item.path||""));left.appendChild(a);}
const meta=el("div","mk-local-activity-meta");meta.appendChild(el("span","",localActivityTime(item.createdAt)));if(item.type==="reaction"&&item.reaction)meta.appendChild(el("span","",reactionSymbol(item.reaction)));if(item.type==="reply")meta.appendChild(el("span","","reply"));if(item.type==="mention")meta.appendChild(el("span","","mention"));if(item.type==="connection_request")meta.appendChild(el("span","","connection request"));if(item.type==="connection_accepted")meta.appendChild(el("span","","connection accepted"));if(item.type==="connection_declined")meta.appendChild(el("span","","connection declined"));if(item.type==="connection_removed")meta.appendChild(el("span","","connection removed"));const voucherState=item.type==="xp_cap_boost_voucher"?el("span","mk-notification-voucher-state",voucherStateText(item)):null;if(voucherState)meta.appendChild(voucherState);if(item.type==="anonymous_comment_claim")meta.appendChild(el("span","",`${Math.max(1, Number(item.claimableCount || 1))} available`));const sourceLabel=notificationSourceLabel(item.notificationSource||item.voucherSource||"");if(sourceLabel)meta.appendChild(el("span","",sourceLabel));left.appendChild(meta);main.appendChild(left);if(item.type==="connection_request"&&item.actorAccountKey){const action=el("div","mk-notification-action");const accept=el("button","mk-comment-small-btn","Accept");const decline=el("button","mk-comment-small-btn mk-comment-danger-btn","Decline");accept.type=decline.type="button";accept.dataset.primary="true";const afterConnection=()=>{markConnectionsSeen();updateNotificationBadgesSoon(0,{force:true,reason:"connection-response"});};accept.addEventListener("click",()=>act(accept,[accept,decline],isCurrent=>respondConnection(item.actorAccountKey,"accept",{isCurrent}),"Accepting…","Accepted","Could not accept request. Try again.",afterConnection));decline.addEventListener("click",()=>act(decline,[accept,decline],isCurrent=>respondConnection(item.actorAccountKey,"decline",{isCurrent}),"Declining…","Declined","Could not decline request. Try again.",afterConnection));action.appendChild(accept);action.appendChild(decline);main.appendChild(action);}
if(item.type==="xp_cap_boost_voucher"&&!item.voucherUsed&&item.voucherId){const action=el("div","mk-notification-action");const activate=el("button","mk-comment-small-btn","Activate");activate.type="button";activate.title="Activate this voucher to double today's total XP cap and each action daily cap until 23:59:59 UTC.";activate.dataset.primary="true";activate.addEventListener("click",()=>runVoucher(activate));action.appendChild(activate);main.appendChild(action);}
if(item.type==="anonymous_comment_claim"){const action=el("div","mk-notification-action");const review=el("button","mk-comment-small-btn mk-comment-reveal-name",`Review ${Math.max(1, Number(item.claimableCount || 1))}`);review.type="button";review.dataset.primary="true";review.addEventListener("click",()=>act(review,[review],isCurrent=>openAnonymousCommentClaims(item,{silent:true,isCurrent}),"Opening review…","Opened","Could not open review. Try again."));action.appendChild(review);main.appendChild(action);}
...
row.appendChild(feedback);list.appendChild(row);function runVoucher(button){return act(button,[button],isCurrent=>activateXpCapBoostVoucher(item.voucherId,{silent:true,isCurrent}),"Activating…","Activated","Could not activate voucher. Try again.",()=>{if(voucherState)voucherState.textContent="Activated";});}});}
return load();}
async function revealAnonymousCommentFromActivity(item,options){if(!item||!item.id)return false;const opts=options||{};const context=opts.context||captureSocialAccountContext();const current=()=>socialAccountContextIsCurrent(context);const visible=()=>current()&&(!opts.isCurrent||opts.isCurrent());if(!visible())return false;const profile=readLocalProfile();const displayName=cleanProfileNameLocal(profile&&profile.name||"");const ok=window.confirm(`Show ${displayName || "your account name"} on this comment? This cannot be undone. The comment reward will be added once.`);if(!ok||!visible())return false;const res=await apiPost("/comments/reveal",{visitorId:context.visitorId,commentId:item.id});if(!current())return false;if(!res||!res.ok){if(visible())window.alert((res&&res.error)||"Could not reveal your name on this comment.");return false;}
...
function accountActivityRowKey(item,type){return JSON.stringify([type,String(item.id||item.path||""),type==="saved"?String(item.action||""):""]);}
function disposeAccountActivity(host){if(host&&typeof host.__mkDisposeActivityView==="function")host.__mkDisposeActivityView();}
function accountActivityFocusKey(node,view){if(!node||!view||!view.contains(node))return null;const row=node.closest("[data-mk-activity-row]");const section=node.closest("[data-fold-key]");return{row:row&&row.dataset.mkActivityRow||"",action:node.dataset&&node.dataset.mkActivityAction||"",section:section&&section.dataset.foldKey||"",summary:node.tagName==="SUMMARY"};}
function restoreAccountActivityFocus(state,key){if(!key||!state.current())return;const rows=Array.from(state.view.querySelectorAll("[data-mk-activity-row]"));const row=rows.find(node=>node.dataset.mkActivityRow===key.row);const section=Array.from(state.view.querySelectorAll("[data-fold-key]")).find(node=>node.dataset.foldKey===key.section);let target;if(row&&key.action)target=Array.from(row.querySelectorAll("[data-mk-activity-action]")).find(node=>node.dataset.mkActivityAction===key.action&&!node.hidden&&!node.disabled);if(!target&&!key.row&&key.action&&section)target=Array.from(section.querySelectorAll("[data-mk-activity-action]")).find(node=>node.dataset.mkActivityAction===key.action&&!node.hidden&&!node.disabled);if(!target&&row)target=row.querySelector(".mk-local-activity-link");if(!target&&section)target=section.querySelector("summary");if(!target){target=state.status;state.status.textContent="Activity updated.";}
if(target&&!target.getClientRects().length&&section)target=section.querySelector("summary");try{target&&target.focus({preventScroll:true});}catch(_){}}
function makeLocalActivityRow(item,type,rootHost,rootType){const row=el("div","mk-local-activity-row"+(type==="comments"?" mk-local-comment-activity-row":"")+(item.deleted?" is-deleted":"")+(type==="comments"&&item.anonymous&&!item.deleted?" is-anonymous-comment":""));const rowKey=accountActivityRowKey(item,type);row.dataset.mkActivityRow=rowKey;const context=captureSocialAccountContext();const state=rootHost&&rootHost.__mkActivityView;const current=()=>row.isConnected&&(!rootHost||rootHost.contains(row))&&(!rootHost||!rootHost.dataset.type||rootHost.dataset.type===(rootType||type))&&socialAccountContextIsCurrent(context)&&(!state||state.current());const main=el("div","mk-local-activity-main");const left=el("div","");const a=el("a","mk-local-activity-link",activityTitle(item));a.dataset.mkActivityAction="link";a.href=pageHref(item.path||"");a.setAttribute("data-path",normActivityTitlePath(item.path||""));left.appendChild(a);const meta=el("div","mk-local-activity-meta");if(type==="saved"){meta.appendChild(el("span","",pageActionLabels()[item.action]||"Saved"));const when=localActivityTime(item.ts||item.deletedAt);if(when)meta.appendChild(el("span","",when));}else{meta.appendChild(el("span","",localActivityTime(item.ts||item.deletedAt)));}
if(item.deleted)meta.appendChild(el("span","","deleted"));if(type==="comments"&&item.editedAt&&!item.deleted)meta.appendChild(el("span","","edited"));if(type==="comments"&&item.parentId)meta.appendChild(el("span","","reply"));if(type==="comments"&&item.anonymous&&!item.deleted)meta.appendChild(el("span","","anonymous · no reward yet"));left.appendChild(meta);main.appendChild(left);row.appendChild(main);if(type==="comments"){row.appendChild(el("div","mk-local-activity-text",previewText(item.text,260)));if(!item.deleted&&item.id){const actions=el("div","mk-local-activity-actions");const status=el("div","mk-activity-row-status");status.setAttribute("role","status");status.tabIndex=-1;status.hidden=true;status.dataset.mkActivityAction="status";const pending=state?state.pending:new Map();const runAction=async(button,work)=>{if(!current()||pending.has(rowKey))return;const token={};pending.set(rowKey,token);status.textContent="Updating comment…";status.hidden=false;row.setAttribute("aria-busy","true");Array.from(actions.querySelectorAll("button")).forEach(control=>{control.setAttribute("aria-disabled","true");});let done=false;try{done=await work({context,isCurrent:current});}catch(_){}
if(pending.get(rowKey)===token)pending.delete(rowKey);if(!current()){const live=rootHost&&rootHost.__mkActivityView;if(live&&live.pending===pending&&live.current()&&!pending.size)renderLocalActivityList(rootHost,rootType||type);return;}
row.setAttribute("aria-busy","false");Array.from(actions.querySelectorAll("button")).forEach(control=>{control.removeAttribute("aria-disabled");});status.textContent=done?"Comment updated.":"No change was saved. You can try again.";if(done||(state&&state.needsRefresh&&!pending.size))renderLocalActivityList(rootHost,rootType||type);};if(item.anonymous){const reveal=el("button","mk-comment-small-btn mk-comment-reveal-name","Show my name");reveal.type="button";reveal.dataset.mkActivityAction="reveal";reveal.title="Reveal your account name on this comment and receive its XP/EORbits reward once.";reveal.addEventListener("click",()=>runAction(reveal,options=>revealAnonymousCommentFromActivity(item,options)));actions.appendChild(reveal);}
const edit=iconButton("mk-comment-small-btn mk-comment-owner-btn","edit","Edit");edit.type="button";edit.dataset.mkActivityAction="edit";edit.addEventListener("click",()=>{if(!current()||pending.has(rowKey))return;const next=window.prompt("Edit your comment:",item.text||"");if(next==null)return;const tx=String(next||"").trim();if(!tx)return;runAction(edit,options=>editOwnComment(item.id,item.path||currentPath(),tx,options));});actions.appendChild(edit);const del=iconButton("mk-comment-small-btn mk-comment-owner-btn","trash","Delete from page");del.type="button";del.dataset.mkActivityAction="delete";del.addEventListener("click",()=>runAction(del,options=>deleteOwnComment(item.id,item.path||currentPath(),options)));actions.appendChild(del);row.appendChild(actions);row.appendChild(status);if(pending.has(rowKey)){status.textContent="Updating comment…";status.hidden=false;row.setAttribute("aria-busy","true");Array.from(actions.querySelectorAll("button")).forEach(control=>{control.setAttribute("aria-disabled","true");});}}}
return row;}
...
const groups=[["favorite","Favourites"],["study_later","Study later"],["review_later","Review later"],];groups.forEach(([action,label])=>{const arr=actions.filter((x)=>x.action===action);appendAccountActivitySection(host,arr,"saved",rootHost||host,rootType||"saved",{label,key:localFoldStateKey("saved-v19",action),open:false,pageSize:80});});}
function appendAccountActivitySection(host,items,type,rootHost,rootType,options){const arr=Array.isArray(items)?items:[];const context=captureSocialAccountContext();const state=rootHost&&rootHost.__mkActivityView;const opts=options||{};const pageSize=opts.pageSize||40;const sec=makeFoldSection(opts.label,arr.length,opts.open!==false,opts.key);if(state&&state.folds.has(opts.key))sec.details.open=state.folds.get(opts.key);host.appendChild(sec.details);if(!arr.length){sec.body.appendChild(el("div","mk-local-activity-empty",type==="saved"?"None yet.":type==="comments"?"No comments yet.":"No visit history yet."));return;}
let shown=0;let requested=Math.max(pageSize,state&&state.pages.get(opts.key)||0);const toolbar=el("div","mk-activity-toolbar");const count=el("span","mk-activity-page-status");count.setAttribute("role","status");count.setAttribute("aria-live","polite");const more=el("button","mk-comment-small-btn mk-activity-more"+(type==="saved"?" mk-local-saved-more":""),"Show more");more.type="button";more.dataset.mkActivityAction="more";more.setAttribute("aria-label",`Show more ${String(opts.label || "history").toLowerCase()}`);const appendBatch=()=>{toolbar.remove();const batch=arr.slice(shown,requested);appendRowsTo(sec.body,batch,type,rootHost,rootType,batch.length);shown+=batch.length;if(state)state.pages.set(opts.key,requested);count.textContent=`Showing ${shown} of ${arr.length}`;toolbar.appendChild(count);more.remove();if(shown<arr.length)toolbar.appendChild(more);sec.body.appendChild(toolbar);};more.addEventListener("click",()=>{if(!more.isConnected||!rootHost.contains(more)||!socialAccountContextIsCurrent(context)||(rootHost.dataset.type&&rootHost.dataset.type!==rootType)||(state&&!state.current()))return;const focused=document.activeElement===more;const firstNew=shown;requested+=pageSize;appendBatch();refreshActivityLinkTitles(sec.body);if(focused){const target=more.isConnected?more:sec.body.querySelectorAll(".mk-local-activity-link")[firstNew];try{target&&target.focus({preventScroll:true});}catch(_){}}});appendBatch();if(state&&state.scrollPositions.has(opts.key))sec.body.scrollTop=state.scrollPositions.get(opts.key);}
function renderCombinedActivity(host,visits,comments,pageActions){const rootHost=host.__mkActivityRoot||host;appendAccountActivitySection(host,visits,"visits",rootHost,"activity",{label:"Visit history",key:localFoldStateKey("activity","visits")});appendAccountActivitySection(host,comments,"comments",rootHost,"activity",{label:"Comments and replies",key:localFoldStateKey("activity","comments")});}
async function renderLocalActivityList(host,type){if(!host)return;disposeAccountHistoryRecovery(host);if(!["activity","saved","comments","visits"].includes(type))disposeAccountActivity(host);if(type==="info"){renderProfileInfo(host);return;}
...
const context=captureSocialAccountContext();const previous=host.__mkActivityView;if(previous&&previous.type===type&&previous.current()&&previous.pending.size){previous.needsRefresh=true;return;}
const focused=previous&&previous.current()?accountActivityFocusKey(document.activeElement,previous.view):null;const scrollTop=Number(host.scrollTop)||0;disposeAccountActivity(host);host.innerHTML="";const changed=()=>{const restoreFocus=host.contains(document.activeElement);disposeAccountActivity(host);host.__mkActivityCache=null;host.innerHTML="";const status=el("div","mk-activity-status","Account data changed. Reload this section to see the current account.");status.setAttribute("role","status");status.tabIndex=-1;host.appendChild(status);const reload=el("button","mk-comment-small-btn mk-activity-reload","Reload activity");reload.type="button";reload.addEventListener("click",()=>{if(reload.isConnected&&(!host.dataset.type||host.dataset.type===type))renderLocalActivityList(host,type);});host.appendChild(reload);if(restoreFocus){try{status.focus({preventScroll:true});}catch(_){}}};if(!host.isConnected||!socialAccountContextIsCurrent(context)){changed();return;}
let cache=host.__mkActivityCache;if(!cache||!socialAccountContextIsCurrent(cache.context))cache=host.__mkActivityCache={context,views:new Map()};let saved=cache.views.get(type);if(!saved){saved={pages:new Map(),folds:new Map(),scrollPositions:new Map(),pending:new Map()};cache.views.set(type,saved);}
const view=el("div","mk-activity-view");const status=el("div","mk-activity-status");status.setAttribute("role","status");status.tabIndex=-1;view.__mkActivityRoot=host;view.appendChild(status);host.appendChild(view);const state=Object.assign({view,status,type,context,active:true,needsRefresh:false},saved);state.current=()=>state.active&&host.__mkActivityView===state&&view.isConnected&&host.contains(view)&&(!host.dataset.type||host.dataset.type===type)&&socialAccountContextIsCurrent(context);host.__mkActivityView=state;const events=["mk-account-workspace-changed","mk-account-login-change","mk-account-workspaces-ready","storage"];const ownerChanged=()=>{if(state.active&&host.contains(view)&&!socialAccountContextIsCurrent(context))changed();};const dispose=()=>{if(!state.active)return;Array.from(view.querySelectorAll("[data-fold-key]")).forEach(section=>{state.folds.set(section.dataset.foldKey,section.open);const list=section.querySelector(".mk-local-fold-body");if(list)state.scrollPositions.set(section.dataset.foldKey,Number(list.scrollTop)||0);});state.active=false;events.forEach(event=>window.removeEventListener(event,ownerChanged));if(host.__mkDisposeActivityView===dispose)host.__mkDisposeActivityView=null;};host.__mkDisposeActivityView=dispose;events.forEach(event=>window.addEventListener(event,ownerChanged));if(type==="activity")renderCombinedActivity(view,getLocalVisitsForSync(),getLocalComments());else if(type==="saved")renderSavedPagesGrouped(view,getLocalPageActions(),host,"saved");else appendAccountActivitySection(view,type==="comments"?getLocalComments():getLocalVisitsForSync(),type,host,type,{label:type==="comments"?"Comments and replies":"Visit history",key:localFoldStateKey(type,type),pageSize:80});refreshActivityLinkTitles(host);host.scrollTop=scrollTop;restoreAccountActivityFocus(state,focused);}
...
if(panel&&panel.style){try{panel.style.minHeight="";panel.style.height="";}catch(_){}}}
function openLocalActivity(type){ensureStylesOnce();const initial=["info","privacy","notifications","connections","saved","activity","shop","visits","comments"].includes(type)?type:"info";const existing=document.querySelector(".mk-local-activity-modal");if(existing){if(typeof existing.__mkCloseDialog==="function")existing.__mkCloseDialog();else existing.remove();}
const modal=document.createElement("div");modal.className="mk-local-activity-modal mk-local-activity-modal--account mk-account-workspace";modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.innerHTML=`
      <div class="mk-local-activity-panel" role="document">
        ...
        <div class="mk-local-activity-tabs" role="tablist" aria-label="Account sections">
...
const panel=modal.querySelector(".mk-local-activity-panel");const body=modal.querySelector(".mk-local-activity-body");const titleEl=modal.querySelector(".mk-local-activity-title");const tabs=Array.from(modal.querySelectorAll(".mk-local-activity-tab"));const tabLabels={info:"Account",privacy:"Privacy",notifications:"Alerts",connections:"People",saved:"Saved",activity:"Activity",shop:"Store"};tabs.forEach((button)=>{if(!button.dataset.type)return;const label=el("span","mk-account-tab-label",tabLabels[button.dataset.type]);label.setAttribute("aria-hidden","true");button.appendChild(label);});const panelId=`mk-account-tabpanel-${++accountDialogSequence}`;body.id=panelId;body.setAttribute("role","tabpanel");body.tabIndex=0;tabs.filter((button)=>button.dataset.type).forEach((button)=>{button.id=`${panelId}-${button.dataset.type}`;button.setAttribute("role","tab");button.setAttribute("aria-controls",panelId);});const qrAction=tabs.find((button)=>button.dataset.action==="login-qr");if(qrAction)modal.querySelector(".mk-local-activity-head").insertBefore(qrAction,modal.querySelector(".mk-local-activity-close"));modal.querySelector(".mk-local-activity-tabs").addEventListener("keydown",(event)=>{if(!["ArrowLeft","ArrowRight","Home","End"].includes(event.key))return;const sectionTabs=tabs.filter((button)=>button.dataset.type);const current=sectionTabs.indexOf(document.activeElement);if(current<0)return;const index=event.key==="Home"?0:event.key==="End"?sectionTabs.length-1:(current+(event.key==="ArrowRight"?1:-1)+sectionTabs.length)%sectionTabs.length;event.preventDefault();sectionTabs[index].click();sectionTabs[index].focus();});applyStoredAccountPanelHeight(panel);try{paintShopDiscountBadges();}catch(_){}
const setType=(next)=>{if(body&&typeof body.__mkDisposeProfileInfo==="function")body.__mkDisposeProfileInfo();if(body&&typeof body.__mkDisposeNotifications==="function")body.__mkDisposeNotifications();if(body&&typeof body.__mkDisposeConnections==="function")body.__mkDisposeConnections();if(body&&typeof body.__mkDisposeActivityView==="function")body.__mkDisposeActivityView();tabs.forEach((b)=>{const active=b.dataset.type===next;b.classList.toggle("is-active",active);if(b.dataset.type){b.setAttribute("aria-selected",String(active));b.tabIndex=active?0:-1;if(active)body.setAttribute("aria-labelledby",b.id);}});if(titleEl)titleEl.textContent=localActivityTitleFor(next);if(body)body.dataset.type=next;if(modal){modal.dataset.type=next||"";modal.classList.toggle("mk-local-activity-modal--activity",next==="activity");}
if(panel)panel.dataset.type=next||"";renderLocalActivityList(body,next);try{mkLocalUpdateViewportMetrics();window.setTimeout(mkLocalScheduleViewportMetrics,80);}catch(_){}
...
cloudSyncRenderTimer=window.setTimeout(()=>{cloudSyncRenderTimer=0;const active=(tabs.find((b)=>b.classList.contains("is-active"))||{}).dataset;const t=active&&active.type||body.dataset.type||initial;if(t!=="info"&&t!=="privacy"&&t!=="connections"&&t!=="notifications")renderLocalActivityList(body,t);},350);};window.addEventListener("mk-local-activity-change",refreshOnCloudSync);let releaseFocus=()=>{};let didClose=false;const close=()=>{if(didClose)return;didClose=true;if(body&&typeof body.__mkDisposeProfileInfo==="function")body.__mkDisposeProfileInfo();if(body&&typeof body.__mkDisposeNotifications==="function")body.__mkDisposeNotifications();if(body&&typeof body.__mkDisposeConnections==="function")body.__mkDisposeConnections();if(body&&typeof body.__mkDisposeActivityView==="function")body.__mkDisposeActivityView();disposeAccountHistoryRecovery(body);try{if(cloudSyncRenderTimer)clearTimeout(cloudSyncRenderTimer);}catch(_){}
...
releaseFocus();};modal.addEventListener("click",(ev)=>{if(ev.target===modal)close();});modal.querySelector(".mk-local-activity-close").addEventListener("click",close);releaseFocus=bindAccountDialogFocus(modal,close,".mk-local-activity-title");modal.__mkCloseDialog=close;setType(initial);}
