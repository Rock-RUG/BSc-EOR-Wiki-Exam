...
return false;}
function setOpen(sec,open){if(!sec||!sec.body||!sec.h2)return;sec.body.hidden=!open;sec.h2.classList.toggle("rf-open",!!open);try{sec.h2.setAttribute("aria-expanded",open?"true":"false");}catch(_){}}
...
function enableHeadingToggles(sections,onChange){for(const sec of sections){if(!sec||!sec.h2||!sec.body)continue;if(sec.h2.getAttribute("data-rf-bound")==="1")continue;sec.h2.setAttribute("data-rf-bound","1");sec.h2.setAttribute("role","button");sec.h2.setAttribute("tabindex","0");sec.h2.setAttribute("aria-controls",sec.body.id||"");sec.h2.setAttribute("aria-expanded",sec.body.hidden?"false":"true");sec.h2.addEventListener("click",(e)=>{const a=e.target&&e.target.closest?e.target.closest("a"):null;if(a)return;preserveScrollDuring(()=>setOpen(sec,sec.body.hidden));if(onChange)onChange();});sec.h2.addEventListener("keydown",(e)=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();preserveScrollDuring(()=>setOpen(sec,sec.body.hidden));if(onChange)onChange();}});}}
...
function getPanel(host){panel=document.getElementById(PANEL_ID)||panel;if(!panel){panel=document.createElement("div");panel.id=PANEL_ID;panel.setAttribute("role","menu");panel.addEventListener("mouseenter",cancelHide);panel.addEventListener("mouseleave",scheduleHide);}
...
function renderCourses(courses){const p=getPanel(activeHost);p.innerHTML="";const ul=document.createElement("ul");for(const c of(courses||[]).slice(0,MAX)){const li=document.createElement("li");const a=document.createElement("a");a.href=c.href||"#";a.textContent=c.title;a.setAttribute("role","menuitem");li.appendChild(a);ul.appendChild(li);}
p.appendChild(ul);}
...
function openPanelFor(link){if(!link||!isLikelyTopYearLink(link))return;cancelHide();activeLink=link;activeHost=findHost(link);if(activeHost&&activeHost.classList)activeHost.classList.add(HOST_CLASS);enhanceOne(link);lockOriginalDropdown(link);const label=labelFor(link);const yearHref=stableYearHref(link,label);const p=getPanel(activeHost);p.setAttribute("aria-label",label+" courses");p.classList.add("rf-year-course-popover-open-v4");renderCourses(baseCourses(yearHref,label));positionPanel();loadCourses(yearHref,label).then((courses)=>{if(activeLink!==link)return;renderCourses(courses);requestAnimationFrame(positionPanel);}).catch(()=>{if(activeLink!==link)return;renderCourses(baseCourses(yearHref,label));requestAnimationFrame(positionPanel);});}
function cancelHide(){if(hideTimer)window.clearTimeout(hideTimer);hideTimer=0;}
function hidePanel(){cancelHide();if(panel){panel.classList.remove("rf-year-course-popover-open-v4");panel.style.transform="translate(-9999px, -9999px)";}
...
function enhanceOne(a){if(!a||a.getAttribute(DONE)==="1")return;if(!isLikelyTopYearLink(a))return;a.setAttribute(DONE,"1");a.classList.add("rf-year-course-trigger-v4");a.setAttribute("aria-haspopup","true");if(!a.querySelector(".rf-year-course-arrow-v4")){const arrow=document.createElement("span");arrow.className="rf-year-course-arrow-v4";arrow.setAttribute("aria-hidden","true");arrow.textContent="›";a.appendChild(arrow);}}
...
function bindGlobalHandlers(){if(document.documentElement.getAttribute("data-rf-year-course-global-v4")==="1")return;document.documentElement.setAttribute("data-rf-year-course-global-v4","1");document.addEventListener("mouseover",(e)=>{const a=closestYearLink(e.target);if(a)openPanelFor(a);},true);document.addEventListener("focusin",(e)=>{const a=closestYearLink(e.target);if(a)openPanelFor(a);},true);document.addEventListener("mouseout",(e)=>{const a=closestYearLink(e.target);if(!a)return;const to=e.relatedTarget;if(to&&(a.contains(to)||(panel&&panel.contains(to))||(activeHost&&activeHost.contains(to))))return;scheduleHide();},true);document.addEventListener("keydown",(e)=>{if(e.key==="Escape")hidePanel();},true);document.addEventListener("click",(e)=>{const target=e.target;if(target&&target.closest&&(target.closest("#"+PANEL_ID)||closestYearLink(target)))return;hidePanel();},true);window.addEventListener("resize",positionPanel,{passive:true});window.addEventListener("scroll",positionPanel,{passive:true,capture:true});}
