...
function onScrollActivity(ev){if(ev&&isInsidePopup(ev.target))return;if(state.focusAnchor&&document.activeElement===state.focusAnchor)return;if(ev&&ev.type==="wheel"&&Number.isFinite(ev.clientX)&&Number.isFinite(ev.clientY)){state.lastPointerX=ev.clientX;state.lastPointerY=ev.clientY;}
...
function getPopup(){let popup=document.getElementById(POPUP_ID);if(popup){popup.setAttribute("role","tooltip");return popup;}
popup=document.createElement("div");popup.id=POPUP_ID;popup.setAttribute("role","tooltip");popup.setAttribute("aria-hidden","true");popup.setAttribute("data-kind","preview");popup.setAttribute("data-placement","right");popup.innerHTML=`
...
function hidePopupNow(){clearTimers();clearDeferredIntent();cancelActiveDemand();const popup=state.popup||document.getElementById(POPUP_ID);if(popup){popup.style.left=VIEWPORT_PAD+"px";popup.style.top=VIEWPORT_PAD+"px";closePopup();}
setPreviewDescription(state.activeAnchor,false);state.activeAnchor=null;state.focusAnchor=null;state.currentAbsUrl="";state.currentRelUrl="";state.currentTitle="";state.currentKind="";state.currentAnchorRect=null;state.currentPlacement="right";state.hoverAnchor=null;state.hoverAbsUrl="";state.hoverPopup=false;state.pendingAnchor=null;state.pendingAbsUrl="";}
function scheduleHide(){clearTimeout(state.leaveTimer);state.leaveTimer=window.setTimeout(()=>{state.leaveTimer=0;const popup=getPopup();if(state.hoverPopup)return;if(state.focusAnchor&&document.activeElement===state.focusAnchor)return;if(popup&&popup.matches&&popup.matches(":hover"))return;if(state.activeAnchor&&state.hoverAnchor&&sameAnchor(state.activeAnchor,state.hoverAnchor)&&state.currentAbsUrl===state.hoverAbsUrl)return;hidePopupNow();},LEAVE_DELAY_MS);}
function beginPreview(info,opts){if(!info||!document.contains(info.anchor))return;const keyboard=!!(opts&&opts.keyboard&&state.focusAnchor===info.anchor&&document.activeElement===info.anchor);if(opts&&opts.keyboard&&!keyboard)return;if(!keyboard&&!canUseHoverPreview())return;if(state.pendingAnchor&&(state.pendingAnchor!==info.anchor||state.pendingAbsUrl!==info.absUrl))return;if(!state.hoverAnchor||state.hoverAnchor!==info.anchor||state.hoverAbsUrl!==info.absUrl)return;if(!keyboard&&scrollIsActive()){rememberDeferredIntent(info);return;}
state.pendingAnchor=null;state.pendingAbsUrl="";const switchOnly=!!(opts&&opts.switchOnly);const sameTarget=info.absUrl===state.currentAbsUrl;clearTimeout(state.leaveTimer);state.leaveTimer=0;if(state.demandEntry&&state.demandEntry.url!==info.absUrl)cancelActiveDemand();const entry=prefetch(info.absUrl,{demanded:true});state.demandEntry=entry;deferFeatureLoads();setPreviewDescription(state.activeAnchor,false);state.activeAnchor=info.anchor;setPreviewDescription(info.anchor,true);state.currentAbsUrl=info.absUrl;state.currentRelUrl=info.relUrl;state.currentTitle=info.anchor.getAttribute("title")||cleanTitle(info.anchor.textContent||"")||info.relUrl;state.currentKind="";state.currentAnchorRect=previewAnchorRect(info.anchor);const seq=++state.requestSeq;const applyResolvedPayload=(payload)=>{if(seq!==state.requestSeq)return;if(!state.activeAnchor||state.currentAbsUrl!==info.absUrl)return;if(!document.contains(state.activeAnchor)){hidePopupNow();return;}
const normalizedPayload={title:(payload&&payload.title)||state.currentTitle||"Preview",titleHtml:(payload&&payload.titleHtml)||"",kind:normalizeKind(payload&&payload.kind),html:(payload&&payload.html)||`<div class="mk-chp-empty">Preview unavailable.</div>`};state.currentKind=normalizedPayload.kind;if(state.demandEntry===entry)state.demandEntry=null;if(entry)entry.demanded=false;const popup=getPopup();popup.classList.remove("is-loading","is-swapping");openPopup();setPopupContent(normalizedPayload);schedulePosition();updateFadeVisibility();};if(!entry){applyResolvedPayload(unavailablePayload());return;}
...
function schedulePreview(info,opts){if(!(opts&&opts.keyboard)&&!canUseHoverPreview())return;clearDeferredIntent();deferFeatureLoads();state.pendingAnchor=info.anchor;state.pendingAbsUrl=info.absUrl;clearTimeout(state.enterTimer);clearTimeout(state.leaveTimer);state.leaveTimer=0;state.enterTimer=window.setTimeout(()=>{state.enterTimer=0;beginPreview(info,opts);},ENTER_DELAY_MS);}
...
function onMouseOver(ev){if(state.focusAnchor)return;if(!canUseHoverPreview())return;if(state.mouseDown)return;if(isInsidePopup(ev.target)){state.hoverPopup=true;clearTimeout(state.leaveTimer);state.leaveTimer=0;return;}
...
function onMouseMove(ev){if(state.focusAnchor){const known=Number.isFinite(state.lastPointerX)&&Number.isFinite(state.lastPointerY);const moved=pointerActuallyMoved(ev);if(!eventReportsPhysicalMovement(ev)&&!(known&&moved))return;hidePopupNow();}
if(!canUseHoverPreview())return;pointerActuallyMoved(ev);if(isInsidePopup(ev.target)){state.hoverPopup=true;clearTimeout(state.leaveTimer);state.leaveTimer=0;return;}
...
function onMouseOut(ev){if(state.focusAnchor)return;const fromInfo=qualifyAnchor(ev.target);const from=fromInfo?fromInfo.anchor:null;if(!from)return;const related=ev.relatedTarget;if(related&&from.contains&&from.contains(related))return;if(state.deferredIntent&&state.deferredIntent.anchor===from)clearDeferredIntent();if(related&&isInsidePopup(related)){state.hoverPopup=true;clearTimeout(state.leaveTimer);state.leaveTimer=0;cancelPendingPreview(from,fromInfo.absUrl);return;}
...
function setPreviewDescription(anchor,enabled){if(!anchor)return;const ids=(anchor.getAttribute("aria-describedby")||"").split(/\s+/).filter(id=>id&&id!==POPUP_ID);if(enabled)ids.push(POPUP_ID);if(ids.length)anchor.setAttribute("aria-describedby",ids.join(" "));else anchor.removeAttribute("aria-describedby");}
function onFocusIn(ev){if(state.mouseDown)return;const info=qualifyAnchor(ev.target);if(!info)return;if(state.pointerFocusAnchor===info.anchor)return;if(state.activeAnchor!==info.anchor)hidePopupNow();ensureStyles();getPopup();bindScrollForFade();observeDom();state.focusAnchor=info.anchor;state.hoverAnchor=info.anchor;state.hoverAbsUrl=info.absUrl;schedulePreview(info,{keyboard:true});}
function onFocusOut(ev){if(state.focusAnchor&&(ev.target===state.focusAnchor||state.focusAnchor.contains(ev.target)))hidePopupNow();}
function onPreviewKeyDown(ev){state.pointerFocusAnchor=null;if(ev.key!=="Escape"||(!state.activeAnchor&&!state.pendingAnchor))return;hidePopupNow();ev.preventDefault();ev.stopPropagation();}
function onPointerDown(ev){if(ev&&isInsidePopup(ev.target))return;const info=qualifyAnchor(ev&&ev.target);state.pointerFocusAnchor=info?info.anchor:null;state.mouseDown=true;hidePopupNow();}
...
state.mo=null;}
try{state.mo=new MutationObserver(()=>{if(state.pendingAnchor&&!document.contains(state.pendingAnchor)){hidePopupNow();return;}
if(!state.activeAnchor)return;if(!document.contains(state.activeAnchor))hidePopupNow();});state.mo.observe(document.body||document.documentElement,{childList:true,subtree:true});}catch(_){}}
function bindOnce(){if(state.bound)return;state.bound=true;document.addEventListener("focusin",onFocusIn,true);document.addEventListener("focusout",onFocusOut,true);document.addEventListener("keydown",onPreviewKeyDown,true);document.addEventListener("pointerdown",onPointerDown,true);document.addEventListener("pointerup",onPointerUp,true);document.addEventListener("pointercancel",onPointerUp,true);if(canUseHoverPreview()){ensureStyles();getPopup();bindScrollForFade();observeDom();document.addEventListener("mouseover",onMouseOver,true);document.addEventListener("mousemove",onMouseMove,true);document.addEventListener("mouseout",onMouseOut,true);}
window.addEventListener("scroll",onScrollActivity,{passive:true,capture:true});window.addEventListener("wheel",onScrollActivity,{passive:true});window.addEventListener("touchmove",onScrollActivity,{passive:true});window.addEventListener("scroll",onScrollOrResize,true);window.addEventListener("resize",onScrollOrResize,{passive:true});window.addEventListener("pageshow",hidePopupNow);window.addEventListener("mk:site-motion-change",()=>{syncMotionMode();hidePopupNow();});document.addEventListener("DOMContentSwitch",()=>{resetPathCaches();abortSpeculative();hidePopupNow();observeDom();});}
