...
function setModalOpen(open,anchor,btn){if(open&&!state.open&&!cmmConsumeGuestAction('map',{source:'course-diagnostics-map',title:'Course diagnostics'}))return;if(anchor)state.anchor=anchor;if(btn)state.button=btn;const targetAnchor=state.anchor||anchor||q('[data-course-mastery-map]');if(!targetAnchor)return;const{modal,panel}=ensureModal(targetAnchor);state.open=!!open;if(state.open)cmmUpdateViewportMetrics();modal.hidden=!state.open;document.documentElement.classList.toggle('cmm-modal-open',state.open);document.body.classList.toggle('cmm-modal-open',state.open);if(state.button){state.button.setAttribute('aria-expanded',state.open?'true':'false');}
