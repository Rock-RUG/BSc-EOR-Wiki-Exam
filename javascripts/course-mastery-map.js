...
const hasRating=rec.unrated!==true&&rec.m!=null&&(typeof rec.m==='number'||(typeof rec.m==='string'&&rec.m.trim()!==''));const m=hasRating&&[0,1,2,3].includes(Number(rec.m))?Number(rec.m):null;const history=Array.isArray(rec.history)?rec.history.slice():[];return{m,reviewCount:safeNum(rec.reviewCount),viewCount:safeNum(rec.viewCount),visitCount:safeNum(rec.visitCount),lastReviewed:safeNum(rec.lastReviewed),lastViewed:Math.max(safeNum(rec.lastViewed),safeNum(rec.lastSeen)),visited:!!(rec.visited||safeNum(rec.visitCount)>0||safeNum(rec.viewCount)>0||safeNum(rec.lastViewed)>0),history,};}
...
return'';}
function courseOwnershipCatalogue(docs,token,scope){const courses=new Map();for(const doc of Array.isArray(docs)?docs:[]){const match=normLoc(doc&&doc.location).match(/^(Year-\d+\/[^/]+)\/index\.html$/);if(!match)continue;for(const tag of getTagsFromDoc(doc)){const value=String(tag||'').trim().toLowerCase();if(/^[a-z][a-z0-9]*$/.test(value))courses.set(match[1]+'|'+value,{id:match[1],token:value});}}
if(scope&&scope.yearSeg&&scope.courseSeg){const id=scope.yearSeg+'/'+scope.courseSeg;courses.set(id+'|'+token,{id,token});}
return Array.from(courses.values());}
function pageMatchesCourse(page,token,scope,catalogue=[]){const tags=Array.isArray(page&&page.tags)?page.tags.map((x)=>String(x||'').trim().toLowerCase()):[];const path=normLoc(page&&page.location);const home=path.split('/').slice(0,2).join('/');const target=scope&&scope.yearSeg&&scope.courseSeg?scope.yearSeg+'/'+scope.courseSeg:'';if(!target)return false;const courses=catalogue.some(course=>course.id===home)?catalogue:catalogue.concat([{id:home,token:''}]);const shared=window.MkMapTeachingUnits;if(shared&&typeof shared.primaryCourse==='function')return shared.primaryCourse(tags,courses,home).courseId===target;return home===target;}
...
function levelShort(rec){if(!rec||(!rec.visited&&!isExplicitRating(rec)))return'•';if(!isExplicitRating(rec))return'V';if(rec.m===3)return'M';if(rec.m===2)return'C';if(rec.m===1)return'P';if(rec.m===0)return'U';return'•';}
...
const title=selected.concept.title||'';const scores=localScores&&typeof localScores==='object'?localScores:{};const renderSeq=state.seq;applyPublicScoreAverage(panel,loc,'mastery',{status:'loading'});applyPublicScoreAverage(panel,loc,'prereq',{status:'loading'});try{const bodyScores={};if(scores.mastery!=null&&Number.isFinite(Number(scores.mastery)))bodyScores.mastery=clampPct(scores.mastery);if(scores.prereq!=null&&Number.isFinite(Number(scores.prereq)))bodyScores.prereq=clampPct(scores.prereq);if(Object.keys(bodyScores).length){await cmmApiPost('/concept-score',{visitorId:cmmVisitorId(),path:loc,title,scores:bodyScores});}}catch(_){}
if(renderSeq!==state.seq)return;const[masteryAvg,prereqAvg]=await Promise.all([cmmApiGet(`/concept-score/average?path=${encodeURIComponent(loc)}&type=mastery&visitorId=${encodeURIComponent(cmmVisitorId())}`),cmmApiGet(`/concept-score/average?path=${encodeURIComponent(loc)}&type=prereq&visitorId=${encodeURIComponent(cmmVisitorId())}`),]);if(renderSeq!==state.seq)return;applyPublicScoreAverage(panel,loc,'mastery',masteryAvg&&masteryAvg.ok?masteryAvg:{status:'unavailable'});applyPublicScoreAverage(panel,loc,'prereq',prereqAvg&&prereqAvg.ok?prereqAvg:{status:'unavailable'});}
...
function buildRecentChangeCard(diagnosis){const hist=diagnosis&&diagnosis.dailyHistory?diagnosis.dailyHistory:null;const current=hist&&hist.current?hist.current:null;const compareBase=hist&&hist.compareBase?hist.compareBase:null;const changeDate=shortDateLabel(current&&current.date?current.date:todayKeyLocal());const changes=compareBase?(hist.lectureChanges||[]).filter((x)=>x&&safeNum(x.delta)!==0).slice().sort((a,b)=>{const absDiff=Math.abs(safeNum(b.delta))-Math.abs(safeNum(a.delta));if(absDiff!==0)return absDiff;if(safeNum(b.delta)!==safeNum(a.delta))return safeNum(b.delta)-safeNum(a.delta);return safeNum(a.lectureNum)-safeNum(b.lectureNum);}).slice(0,3):[];return`
    ...
    `;}
function buildConceptStateHistogram(diagnosis){const items=diagnosis&&Array.isArray(diagnosis.conceptDiagnostics)?diagnosis.conceptDiagnostics:[];const buckets=[{key:'notVisited',label:'Not visited',tone:0,count:0},{key:'visitedOnly',label:'Visited, unrated',tone:22,count:0},{key:'unknown',label:'Unknown',tone:0,count:0},{key:'fuzzy',label:'Partial',tone:25,count:0},{key:'clear',label:'Clear',tone:67,count:0},{key:'mastered',label:'Mastered',tone:96,count:0},];const byKey=new Map(buckets.map((bucket)=>[bucket.key,bucket]));items.forEach((item)=>{const rec=item&&item.concept?item.concept.record:null;const bucket=byKey.get(levelBucketKey(rec))||byKey.get('notVisited');bucket.count+=1;});const total=Math.max(1,items.length);const maxCount=Math.max(1,...buckets.map((b)=>b.count));return`
      <section class="cmm-sidecard cmm-sidecard--chart">
...
function levelLabel(rec){if(!rec)return"Not visited";if(!rec.visited&&!isExplicitRating(rec))return"Not visited";if(!isExplicitRating(rec))return"Visited";if(rec.m===3)return"Mastered";if(rec.m===2)return"Clear";if(rec.m===1)return"Partial";if(rec.m===0)return"Unknown";return rec.visited?"Visited":"Not visited";}
...
function summariseData(pages,courseToken){const masteryAll=readAllMastery();const comparable=new Map();Object.entries(masteryAll).forEach(([key,record])=>{const loc=cmmComparableMasteryLocation(key);if(!loc)return;const old=comparable.get(loc);const ts=Math.max(safeNum(record.updatedAt),safeNum(record.lastReviewed),safeNum(record.ratingClearedAt));if(!old||ts>old.ts)comparable.set(loc,{record,ts});});const lectures=new Map();const now=Date.now();const concepts=pages.map((page)=>{const match=comparable.get(cmmComparableMasteryLocation(page.location));const record=normaliseRecord(masteryAll[normLoc(page.location)]||match&&match.record);const tags=Array.isArray(page.tags)?page.tags:[];const lectureInfo=lectureInfoFromTags(tags,courseToken);const unitType=lectureInfo&&lectureInfo.unitType?lectureInfo.unitType:"lecture";const lectureNum=lectureInfo&&lectureInfo.lectureNum?lectureInfo.lectureNum:0;const pct=isExplicitRating(record)?masteryPctFromLevel(record.m):null;const lastActivity=Math.max(safeNum(record.lastViewed),safeNum(record.lastReviewed),latestTsFromHistory(record.history));const recent=!!(lastActivity&&now-lastActivity<=RECENT_WINDOW_MS);const concept={location:normLoc(page.location),title:cleanTitle(page.title),tags,lectureNum,unitType,record,pct,lastActivity,recent,};if(!lectures.has(lectureNum)){lectures.set(lectureNum,{lectureNum,unitType,concepts:[],total:0,visited:0,rated:0,weak:0,recent:0,avgPct:0,sumPct:0,});}
...
async function loadCourseMapData(anchor){const context=cmmMasteryContext();const route=currentRelPath();if(context==null)throw cmmMasteryReadError();const ready=cmmEnsureMasteryReader();if(ready)await ready;if(context!==cmmMasteryContext()||route!==currentRelPath())throw cmmMasteryReadError();const courseToken=courseTokenForPage(anchor);const scope=currentCourseScope();const key=`${courseToken}|${scope.yearSeg}|${scope.courseSeg}`;if(state.data&&state.data.key===key){state.data.summary=summariseData(state.data.pages,courseToken);return state.data;}
if(state.loadPromise&&state.loadPromise.key===key&&state.loadPromise.context===context)return state.loadPromise;const promise=(async()=>{const root=getSiteRootUrl();const url=new URL('search/search_index.json',root).toString();const request=__mkFetchSearchIndex(url,{cache:'no-cache'});const j=await request;if(context!==cmmMasteryContext()||route!==currentRelPath())throw cmmMasteryReadError();if(!j||!Array.isArray(j.docs)){[window.__mkSharedJsonPromiseMap,window.__mkMaterialSearchIndexPromises].forEach((cache)=>{if(cache&&cache[url]===request)delete cache[url];});throw new Error('Invalid course search index');}
const catalogue=courseOwnershipCatalogue(j.docs,courseToken,scope);const pages=aggregateDocsToPages(j.docs).filter((page)=>pageMatchesCourse(page,courseToken,scope,catalogue)).filter(isCoreCurriculumPage);const data={key,courseToken,scope,pages,summary:summariseData(pages,courseToken),};if(state.loadPromise===promise)state.data=data;return data;})();promise.key=key;promise.context=context;state.loadPromise=promise;try{return await promise;}finally{if(state.loadPromise===promise)state.loadPromise=null;}}
...
if(!window.__cmmEscBound){window.__cmmEscBound=true;document.addEventListener('keydown',(e)=>{if(!state.open||e.defaultPrevented)return;const modal=document.getElementById('mk-course-mastery-map-modal');const dialog=modal&&q('.cmm-modal__dialog',modal);if(!dialog)return;const active=document.activeElement;const otherDialog=active&&active.closest&&active.closest('[aria-modal="true"]');if(otherDialog&&otherDialog!==dialog)return;if(e.key==='Escape'){e.preventDefault();e.stopPropagation();setModalOpen(false);}else if(e.key==='Tab'){const controls=Array.from(dialog.querySelectorAll('button, a[href], input, select, textarea, [tabindex]')).filter(node=>!node.disabled&&node.getAttribute('tabindex')!=='-1'&&node.getClientRects().length);const first=controls[0],last=controls[controls.length-1];if(first&&(!dialog.contains(active)||(e.shiftKey?active===first:active===last))){e.preventDefault();(e.shiftKey?last:first).focus({preventScroll:true});}}});}
...
function setModalOpen(open,anchor,btn,keyboardOpen){if(open&&!state.open&&!cmmConsumeGuestAction('map',{source:'course-diagnostics-map',title:'Course diagnostics'}))return;if(anchor)state.anchor=anchor;if(btn)state.button=btn;const targetAnchor=state.anchor||anchor||q('[data-course-mastery-map]');if(!targetAnchor)return;const{modal,panel}=ensureModal(targetAnchor);const wasOpen=state.open;const focusWasInside=modal.contains(document.activeElement);state.open=!!open;if(!state.open){state.seq+=1;state.prereqReadySeq+=1;}else if(!wasOpen){state.prereqReadySeq+=1;state.prereqReadyCache=new Map();}
if(state.open)cmmUpdateViewportMetrics();else if(wasOpen)cmmInstallClickShield(520);modal.hidden=!state.open;document.documentElement.classList.toggle('cmm-modal-open',state.open);document.body.classList.toggle('cmm-modal-open',state.open);if(state.button){state.button.setAttribute('aria-expanded',state.open?'true':'false');}
...
renderMap(targetAnchor,panel);window.setTimeout(cmmUpdateViewportMetrics,60);const closeBtn=q('.cmm-modal__close',modal);if(keyboardOpen&&closeBtn){try{closeBtn.focus({preventScroll:true});}catch(_){}}else if(closeBtn&&typeof closeBtn.blur==='function'){try{closeBtn.blur();}catch(_){}}}else if(wasOpen&&focusWasInside&&state.button&&state.button.isConnected){try{state.button.focus({preventScroll:true});}catch(_){}}}
...
wrap.classList.add('course-title-actions');let btn=document.getElementById(TOGGLE_ID);if(!btn){btn=document.createElement('button');btn.id=TOGGLE_ID;btn.type='button';btn.className='cmm-h1-entry';btn.setAttribute('aria-label','Open course diagnostics');btn.setAttribute('title','Open course diagnostics');btn.innerHTML=`<span class="cmm-h1-entry__icon" aria-hidden="true">${mapSvg()}</span><span class="cmm-h1-entry__label">Course diagnostics</span>`;wrap.insertBefore(btn,wrap.firstChild);}else if(btn.parentNode!==wrap){wrap.insertBefore(btn,wrap.firstChild);}
btn.setAttribute('aria-expanded',state.open?'true':'false');cmmBindEntryHeightSyncOnce();cmmSyncEntryButtonStyle(btn);window.setTimeout(()=>cmmSyncEntryButtonStyle(btn),60);window.setTimeout(()=>cmmSyncEntryButtonStyle(btn),360);if(btn.dataset.cmmBound!=='1'){btn.dataset.cmmBound='1';btn.addEventListener('click',(event)=>{if(state.open)setModalOpen(false,anchor,btn);else setModalOpen(true,anchor,btn,!!(event&&event.detail===0));});}
...
try{window.MathJax&&window.MathJax.typesetPromise&&window.MathJax.typesetPromise([panel]).catch(()=>{});}catch(_){}}catch(error){if(seq!==state.seq)return;panel.innerHTML=`
        <div class="cmm-head">
          <div class="cmm-title" id="${PANEL_ID}-title">Course diagnostics</div>
          <div class="cmm-sub">The panel could not be built right now.</div>
        </div>
        <div class="cmm-body"><div class="cmm-error" role="status">${error && error.code === 'cmm_mastery_unavailable' ? escapeHtml(error.message) : 'Failed to load course data. Please retry.'}</div><button type="button" class="cmm-filter" data-cmm-retry="1">Retry</button></div>
      `;const retry=q('[data-cmm-retry]',panel);if(retry)retry.addEventListener('click',()=>{if(state.open&&anchor.isConnected!==false&&route===currentRelPath())renderMap(anchor,panel);});}}
...
document.addEventListener('DOMContentSwitch',mountPreservingOpen);window.addEventListener('pageshow',mountPreservingOpen);function accountChanged(){state.data=null;state.loadPromise=null;state.prereqReadyCache=new Map();state.seq+=1;if(state.open)setModalOpen(false,state.anchor,state.button);}
window.addEventListener('mk-account-login-change',accountChanged);window.addEventListener('mk-account-workspace-changed',accountChanged);window.addEventListener('storage',event=>{if(event&&(event.key==null||['mk_comment_profile_v1','mk_account_workspace_active_owner_v1','mk_hot_visitor_id_v1'].includes(event.key)))accountChanged();});})();