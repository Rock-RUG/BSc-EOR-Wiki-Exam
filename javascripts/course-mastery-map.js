...
const pages=aggregateDocsToPages(j.docs).filter((page)=>pageMatchesCourse(page,courseToken,scope)).filter(isCoreCurriculumPage);const data={key,courseToken,scope,pages,summary:summariseData(pages),};if(state.loadPromise===promise)state.data=data;return data;})();promise.key=key;promise.context=context;state.loadPromise=promise;try{return await promise;}finally{if(state.loadPromise===promise)state.loadPromise=null;}}
