...
let __csrIndexLoadPromise=null;async function loadIndex(){if(__csrIndexLoadPromise)return __csrIndexLoadPromise;__csrIndexLoadPromise=(async()=>{const root=getSiteRootUrl();const candidates=[new URL("search/search_index.json",root).toString(),new URL("search_index.json",root).toString(),];for(const url of candidates){try{const j=await __mkFetchSearchIndex(url,{cache:"no-cache"});if(j&&Array.isArray(j.docs))return j;}catch(_){}}
return{docs:[]};})();return __csrIndexLoadPromise;}
...
__csrAssistUi.scopeCacheKey=courseKey;__csrAssistUi.scopePromise=(async()=>{const indexJson=await loadIndex();const docs=indexJson.docs||[];const pages=csrAggregateDocsToPages(docs).filter((p)=>inSameCourseByKey(p.location,courseKey)).map((p)=>{const lec=lectureInfoFromTags(Array.isArray(p.tags)?p.tags:[]);return{...p,lectureText:lec?(lec.unitLabel||`Lecture ${lec.lectureNum}`):"",};});return pages;})();return __csrAssistUi.scopePromise;}
...
async function csrRefreshAssistUiNow(){const input=csrGetSearchInput();const dd=csrGetAssistDropdown();if(!input||!dd)return;const seq=++__csrAssistUi.requestSeq;const value=String(input.value||"");try{const state=await csrBuildAssistState(value,8);if(seq!==__csrAssistUi.requestSeq)return;csrRenderAssistDropdown(state.items,state);}catch(_){if(seq!==__csrAssistUi.requestSeq)return;csrHideAssistDropdown();}}
...
function init(){bind();}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init);else init();document.addEventListener("DOMContentSwitch",init);})();