...
const REVIEW_NAV_FLAG="random_review_nav_flag_v1";const CANDS_KEY="random_custom_candidates_v1";const ENTRY_KEY="random_custom_page_v1";const TOKENS_KEY="random_custom_tokens_v1";const TOKENMAP_KEY="random_custom_token_map_v1";const NAV_FLAG_KEY="random_custom_nav_flag_v1";const LAST_FACE_KEY="random_custom_last_dice_face_v1";const __crBannerDiceState={timer:null,navigating:false};function isHoverPointer(){try{return!!(window.matchMedia&&window.matchMedia("(hover: hover) and (pointer: fine)").matches);}catch(_){return false;}}
...
document.getElementById("cr-view").addEventListener("click",(e)=>{e.preventDefault();try{sessionStorage.removeItem("random_custom_nav_flag_v1");sessionStorage.removeItem("random_custom_candidates_v1");sessionStorage.removeItem("random_custom_tokens_v1");sessionStorage.removeItem("random_custom_token_map_v1");sessionStorage.removeItem("random_custom_page_v1");sessionStorage.removeItem("random_review_nav_flag_v1");}catch(_){}
window.location.reload();});document.getElementById("cr-continue").addEventListener("click",async(e)=>{e.preventDefault();if(__crBannerDiceState.navigating)return;__crBannerDiceState.navigating=true;const contBtn=document.getElementById("cr-continue");const ico=contBtn?contBtn.querySelector(".cr-dice__ico"):null;const currentId=normLoc(currentRelPath());let pool=readCandidates();pool=pool.filter((x)=>normLoc(x)!==currentId);if(window.ConceptMastery){pool=window.ConceptMastery.filterActive(pool);}
...
try{const arrivalId=newArrivalId();sessionStorage.setItem(ARRIVAL_ID_KEY,arrivalId);sessionStorage.setItem(ARRIVAL_LOC_KEY,normLoc(chosen));}catch(_){}
try{const cb=document.getElementById("cr-selftest");const want=cb?cb.checked:(sessionStorage.getItem(REVIEW_MODE_KEY)==="1");if(want){if(!crbHasShopItem(CRB_SELF_TEST_ITEM_ID)){const ok=await crbOfferUnlock(CRB_SELF_TEST_ITEM_ID,CRB_SELF_TEST_ITEM_TITLE,CRB_SELF_TEST_ITEM_PRICE,"random-banner-continue-self-test");if(!ok){if(cb)cb.checked=false;sessionStorage.removeItem(REVIEW_MODE_KEY);sessionStorage.removeItem(REVIEW_NAV_FLAG);}else{sessionStorage.setItem(REVIEW_MODE_KEY,"1");sessionStorage.setItem(REVIEW_NAV_FLAG,"1");}}else{sessionStorage.setItem(REVIEW_MODE_KEY,"1");sessionStorage.setItem(REVIEW_NAV_FLAG,"1");}}else{sessionStorage.removeItem(REVIEW_MODE_KEY);sessionStorage.removeItem(REVIEW_NAV_FLAG);}}catch(_){}
const targetUrl=toAbsoluteUrl(chosen);try{if(contBtn){contBtn.setAttribute("aria-disabled","true");contBtn.style.pointerEvents="none";contBtn.style.opacity="0.9";}}catch(_){}
let face=1;try{if(ico){if(isHoverPointer())face=freezeToRandomFace(ico,__crBannerDiceState);else face=await rollDiceOnce(ico,{frames:14,interval:55});writeLastFace(face);}}catch(_){}
await sleep(250);window.location.assign(targetUrl);});document.getElementById("cr-change").addEventListener("click",(e)=>{e.preventDefault();let entry="";try{entry=sessionStorage.getItem("random_custom_page_v1")||"";}catch(_){}
