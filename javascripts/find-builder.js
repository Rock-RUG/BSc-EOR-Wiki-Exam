...
async function fbWaitAndRunSearchFromAutofill(timeoutMs,isCurrent){const timeout=Number.isFinite(timeoutMs)?timeoutMs:2400;const startedAt=Date.now();let attempts=0;while(Date.now()-startedAt<timeout){if(typeof isCurrent==="function"&&!isCurrent())return false;attempts+=1;const scope=document.getElementById("find-builder")||document;const rb=scope.querySelector("#fb-run")||document.querySelector("#fb-run");const hasExpr=!!(state&&Array.isArray(state.expr)&&state.expr.length);if(rb&&hasExpr&&fbCanAutofillRunButton(rb)){fbDispatchSyntheticClick(rb);return true;}
await new Promise((r)=>setTimeout(r,attempts<8?32:80));}
if(typeof isCurrent==="function"&&!isCurrent())return false;return fbRunSearchFallbackFromAutofill();}
...
return req;}
async function fbAutofillFromTopSearch(rawToken){const q=String(rawToken||"").trim().replace(/\s+/g," ");if(!q)return;const now=Date.now();if(__fbAutofillStateV1.running&&__fbAutofillStateV1.last===q&&now-(__fbAutofillStateV1.t||0)<1200)return;const requestId=__fbAutofillStateV1.request=(Number(__fbAutofillStateV1.request)||0)+1;__fbAutofillStateV1.running=true;__fbAutofillStateV1.last=q;__fbAutofillStateV1.t=now;try{state.enterQuery="";state.enterQueryUsed=true;}catch(_){}
...
clearStatus();render();const sourceUrl=window.location.href;const sourceHost=document.getElementById("find-builder");const sourceExpr=state.expr;let expectedExpr=tokensToExprText(normalizeNodesToTokens(state.expr));const isCurrent=()=>requestId===__fbAutofillStateV1.request&&window.location.href===sourceUrl&&document.getElementById("find-builder")===sourceHost&&state.expr===sourceExpr&&tokensToExprText(normalizeNodesToTokens(state.expr))===expectedExpr;try{try{const has=await fbHasAnyMatchForTermCached(first);if(!isCurrent())return;if(!has){let to="";try{to=await fbSuggestPhraseForAdd(first);}catch(_){to="";}
if(!isCurrent())return;to=String(to||"").trim().replace(/\s+/g," ");if(to&&to.toLowerCase()!==first.toLowerCase()){let ok=true;try{ok=await fbHasAnyMatchForTermCached(to);}catch(_){ok=true;}
if(!isCurrent())return;if(ok){const exprBefore=snapshot().expr;fbReplaceTokenEverywhere(first,to);try{state.tokens=readTokens();}catch(_){}
clearStatus();render();const exprNow=tokensToExprText(normalizeNodesToTokens(state.expr));expectedExpr=exprNow;const swapExisted=beforePoolLower.has(String(to).toLowerCase());const tokenSwaps=swapExisted?{}:{[to]:first};const tokenAddsOnEdit=swapExisted?[first]:[];fbShowFuzzyNote(`<div class="fb-fuzzy-msg">No results as typed. Using <code>${escapeHtml(to)}</code> instead of <code>${escapeHtml(first)}</code>.</div>
...
try{await new Promise((r)=>setTimeout(r,0));if(!isCurrent())return;await fbWaitAndRunSearchFromAutofill(2600,isCurrent);}catch(_){}}finally{if(requestId===__fbAutofillStateV1.request)__fbAutofillStateV1.running=false;}}
function fbReadEnterQueryNow(){try{const params=new URLSearchParams(window.location.search);if(params.get("src")==="header_search")return(params.get("q")||"").trim();if(params.get("src")==="header_search_done")return"";}catch(_){}
try{const raw=sessionStorage.getItem(FB_ENTER_PAYLOAD_KEY_V2)||"";const s=String(raw||"").trim();if(s&&s.startsWith("{")){const obj=JSON.parse(s);const q=String(obj&&obj.q||"").trim();if(q)return q;}
...
async function fbHasAnyMatchForTerm(rawTerm){const termLower=String(rawTerm||"").trim().toLowerCase();if(!termLower)return false;const pages=await fbGetPageDocsCached();for(const d of pages){const extraTags=Array.isArray(d.tags)?d.tags.join(" "):"";const extraAliases=Array.isArray(d.aliases)?d.aliases.join(" "):"";const hay=fbNormaliseForSearch(`${d.title} ${d.text} ${d.location} ${extraTags} ${extraAliases}`);if(fbTermMatchesHay(termLower,hay,d))return true;}
return false;}
const __fbHasAnyMatchCache=new Map();async function fbHasAnyMatchForTermCached(rawTerm){const k=String(rawTerm||"").trim().replace(/\s+/g," ").toLowerCase();if(!k)return false;if(__fbHasAnyMatchCache.has(k))return!!__fbHasAnyMatchCache.get(k);const v=await fbHasAnyMatchForTerm(rawTerm);__fbHasAnyMatchCache.set(k,!!v);return!!v;}
...
if(typeof isCurrent==="function"&&!isCurrent())return false;if(!to)continue;if(/^[A-Z]/.test(from))to=to.charAt(0).toUpperCase()+to.slice(1);if(!to||to===from)continue;swaps.push([from,to]);swapMapUndo[to]=from;}
if(!swaps.length)return false;if(typeof isCurrent==="function"&&!isCurrent())return false;swaps.forEach(([from,to])=>fbReplaceTokenEverywhere(from,to));state.tokens=readTokens();clearStatus();render();const norm2=normalizeNodesToTokens(state.expr);const expr2=tokensToExprText(norm2);const msg=swaps.map(([a,b])=>`<code>${escapeHtml(b)}</code> instead of <code>${escapeHtml(a)}</code>`).join(", ");fbShowFuzzyNote(`<div class="fb-fuzzy-msg">No results as typed. Using ${msg}.</div>
       <div class="fb-fuzzy-actions">
         <button type="button" class="fb-fuzzy-btn fb-fuzzy-btn--undo" data-fb-fuzzy-act="edit">Edit as typed</button>
...
function closeMaterialSearchOverlayFast(){if(isHeaderSearchBusy())return;const toggle=document.querySelector('input.md-toggle[data-md-toggle="search"]')||document.querySelector('input#__search');if(toggle)toggle.checked=false;const input=document.querySelector('input[data-md-component="search-query"]');if(input){if(input.value){input.value="";input.dispatchEvent(new Event("input",{bubbles:true}));}
...
`;}
let __fbThemeForegroundObserver=null;let __fbThemeForegroundFrame=0;let __fbThemeForegroundTimer=0;function fbFinderComputedColor(value){const legacy=fbParseComputedColor(value);if(legacy)return legacy;const text=String(value||"").trim();const match=/^color\(srgb\s+([^/]+?)(?:\s*\/\s*([^)]*))?\)$/i.exec(text);if(!match)return null;const parts=match[1].trim().split(/\s+/);if(parts.length!==3)return null;const channel=(part)=>/%$/.test(part)?Number(part.slice(0,-1))/100:Number(part);const numbers=parts.map(channel);const alpha=match[2]==null?1:channel(match[2].trim());if(![...numbers,alpha].every(Number.isFinite))return null;const clamp=(number)=>Math.max(0,Math.min(1,number));return{r:clamp(numbers[0])*255,g:clamp(numbers[1])*255,b:clamp(numbers[2])*255,a:clamp(alpha)};}
function fbFinderVisibleSurface(panel,readStyle=getComputedStyle){let node=panel;let color=null;while(node){let layer=null;try{layer=fbFinderComputedColor(readStyle(node).backgroundColor);}catch(_){}
...
function fbFinderSurfacePalette(surface){const light={r:248,g:250,b:252,a:1};const dark={r:15,g:23,b:42,a:1};const useLight=fbContrastRatio(light,surface)>=fbContrastRatio(dark,surface);return useLight?{strong:"rgba(248,250,252,.96)",normal:"rgba(248,250,252,.86)",muted:"rgba(248,250,252,.68)",disabled:"rgba(248,250,252,.48)",}:{strong:"rgba(15,23,42,.95)",normal:"rgba(15,23,42,.84)",muted:"rgba(15,23,42,.63)",disabled:"rgba(15,23,42,.46)",};}
function fbSyncThemeForeground(){const body=document.body;const panel=document.querySelector("#find-builder .fb-panel");if(!body||!panel)return;const root=panel.closest&&panel.closest('.wiki-finder');const styles=new Map();const readStyle=(node)=>{if(!styles.has(node))styles.set(node,getComputedStyle(node));return styles.get(node);};const surfaces=root?Array.from(root.querySelectorAll('.search-hero, #find-builder .fb-panel, #find-builder .fb-board__inner, #search-input, '+'.fb-tokensearch-dropdown, #find-builder .fb-actions .fb-btn, #find-builder .fb-tokens__clear, '+'#search-form .fb-cta-btn')):[body];const updates=surfaces.map(node=>{const palette=fbFinderSurfacePalette(fbFinderVisibleSurface(root?node:panel,readStyle));if(root&&node.matches){const authoredProperty=node.matches('.fb-panel,.fb-board__inner')?'--mk-fb-panel-ink':node.matches('.fb-btn,.fb-cta-btn')?'--mk-fb-button-ink':'';const authored=authoredProperty?readStyle(node).getPropertyValue(authoredProperty).trim():'';if(authored&&authored!=='currentColor'){palette.strong=palette.normal=palette.muted=authored;palette.disabled=authored;}}
return{node,palette};});for(const{node,palette}of updates){const values={'--mk-finder-fg-strong':palette.strong,'--mk-finder-fg':palette.normal,'--mk-finder-fg-muted':palette.muted,'--mk-finder-fg-disabled':palette.disabled,'--mk-finder-text':palette.normal,};if(root){values['--wf-fg']=palette.strong;values['--wf-muted']=palette.muted;}
...
return;}
function readTokens(){let arr=readJsonFrom(sessionStorage,TOKENS_KEY,null);if(!Array.isArray(arr))arr=readJsonFrom(localStorage,TOKENS_KEY,[]);const norm=(s)=>String(s||"").trim().replace(/\s+/g," ");const out=Array.isArray(arr)?arr.filter((token)=>typeof token==="string").map(norm).filter(Boolean).filter((t)=>!fbIsGarbageToken(t)):[];try{const raw2=JSON.stringify(out);const rawSess=sessionStorage.getItem(TOKENS_KEY)||"";const rawLoc=localStorage.getItem(TOKENS_KEY)||"";if((rawSess&&rawSess!==raw2)||(rawLoc&&rawLoc!==raw2)){writeTokens(out);}}catch(_){}
return out;}
function writeTokens(tokens){writeJsonTo(sessionStorage,TOKENS_KEY,tokens||[]);writeJsonTo(localStorage,TOKENS_KEY,tokens||[]);}
function fbSanitizeExpr(value){if(!Array.isArray(value))return[];return value.filter((node)=>node&&typeof node==="object"&&!Array.isArray(node)&&((node.t==="TERM"&&typeof node.v==="string"&&node.v.trim())||(node.t==="OP"&&(node.v==="AND"||node.v==="OR"))||node.t==="LP"||node.t==="RP")).map((node)=>{const copy={...node};if(!fbValidNodeId(copy.__fbid))delete copy.__fbid;else copy.__fbid=copy.__fbid.trim();return copy;});}
function readExpr(){let v=readJsonFrom(sessionStorage,EXPR_KEY,null);if(!Array.isArray(v))v=readJsonFrom(localStorage,EXPR_KEY,[]);return fbSanitizeExpr(v);}
...
function fbDismissTokenInputSuggestions(){__fbTokenInputUi.seq+=1;__fbTokenInputUi.refreshEpoch=(Number(__fbTokenInputUi.refreshEpoch)||0)+1;__fbTokenInputUi.items=[];fbHideTokenInputDropdown();const wrap=fbGetTokenInputWrap();const suffix=wrap&&wrap.querySelector(".fb-tokensearch-suffix");if(suffix){suffix.textContent="";suffix.style.display="none";}}
...
if(seq!==__fbTokenInputUi.seq||fbGetTokenInputEl()!==input||fbGetTokenInputWrap()!==wrap||fbNormaliseTokenInputValue(input.value||"")!==raw)return;if(mirror)mirror.textContent=raw;const canShowInline=!!payload.inlineSuffix&&document.activeElement===input&&fbTokenInputCanInline(tokenPart);if(suffix){suffix.textContent=canShowInline?payload.inlineSuffix:"";suffix.style.display=canShowInline?"inline":"none";}
...
function fbRefreshTokenInputUiSoon(delayMs){const ms=Number(delayMs)||0;const input=fbGetTokenInputEl();const epoch=Number(__fbTokenInputUi.refreshEpoch)||0;window.setTimeout(()=>{if(epoch!==(Number(__fbTokenInputUi.refreshEpoch)||0)||fbGetTokenInputEl()!==input)return;try{fbRefreshTokenInputUi();}catch(_){}},ms);}
...
return;}
if(!visible)return;if(ev.key==="ArrowDown"){ev.preventDefault();ev.stopPropagation();idx=Math.min(items.length-1,idx+1);fbSetTokenInputActiveIndex(idx);return;}
if(ev.key==="ArrowUp"){ev.preventDefault();ev.stopPropagation();idx=Math.max(0,idx-1);fbSetTokenInputActiveIndex(idx);return;}},true);}
...
function restore(prev){state.expr=fbSanitizeExpr(prev&&prev.expr);try{fbEnsureExprNodeIds(state.expr);}catch(_){}
...
let __fbNodeSeq=1;function fbValidNodeId(id){if(typeof id!=="string")return false;const match=/^n(\d+)$/.exec(id.trim());return!!match&&Number.isSafeInteger(Number(match[1]))&&Number(match[1])>0;}
function fbNextNodeId(used){const seen=used&&typeof used.has==="function"?used:null;let id="";do{if(!Number.isSafeInteger(__fbNodeSeq)||__fbNodeSeq<1)__fbNodeSeq=1;id="n"+__fbNodeSeq;__fbNodeSeq=__fbNodeSeq>=Number.MAX_SAFE_INTEGER?1:__fbNodeSeq+1;}while(seen&&seen.has(id));if(seen)seen.add(id);return id;}
function fbEnsureNodeId(obj){if(!obj)return obj;const id=typeof obj.__fbid==="string"?obj.__fbid.trim():"";if(!fbValidNodeId(id))obj.__fbid=fbNextNodeId();return obj;}
function fbEnsureExprNodeIds(nodes){const arr=Array.isArray(nodes)?nodes:[];const used=new Set();let maxSeen=0;for(const n of arr){if(!n||typeof n!=="object"||Array.isArray(n))continue;const id=typeof n.__fbid==="string"?n.__fbid.trim():"";const m=/^n(\d+)$/.exec(id);if(m&&fbValidNodeId(id)&&!used.has(id)){used.add(id);maxSeen=Math.max(maxSeen,Number(m[1])||0);}}
if(__fbNodeSeq<=maxSeen)__fbNodeSeq=maxSeen>=Number.MAX_SAFE_INTEGER?1:maxSeen+1;const kept=new Set();for(const n of arr){if(!n||typeof n!=="object"||Array.isArray(n))continue;const id=typeof n.__fbid==="string"?n.__fbid.trim():"";if(fbValidNodeId(id)&&!kept.has(id)){kept.add(id);continue;}
n.__fbid=fbNextNodeId(used);kept.add(n.__fbid);}
return arr;}
...
const root=getSiteRootUrl();const url=new URL("search/search_index.json",root);const key=url.toString();const request=__mkFetchSearchIndex(key);try{const j=await request;if(!j||!Array.isArray(j.docs))throw new Error("Failed to load search index");__fb_index_cache=j;return __fb_index_cache;}catch(error){for(const map of[window.__mkSharedJsonPromiseMap,window.__mkMaterialSearchIndexPromises]){if(map&&map[key]===request)delete map[key];}
throw error;}}
...
function fbSimpleExpr(value){const text=String(value||"").replace(/[\u0000-\u001f\u007f]/g," ").trim().replace(/\s+/g," ");return text?tokensToExprText([{k:"TERM",v:text}]):"";}
function fbEnsureSimpleSearchUi(){const editor=document.getElementById("wf-query-editor"),form=document.getElementById("search-form");if(!editor||!form||document.getElementById("fb-simple-search"))return;const ui=document.createElement("div");ui.className="fb-search-modes";ui.innerHTML=`<form id="fb-simple-search" role="search" aria-label="Find a concept">
      <label for="fb-simple-input">Concept, course or lecture</label>
      <div class="fb-simple-search__row"><input id="fb-simple-input" type="search" maxlength="500" autocomplete="off" placeholder="Try chain rule or m1c-lecture02"><button type="submit">Search</button></div>
      <p id="fb-simple-status" role="status" aria-live="polite">Type a topic and press Enter.</p>
      </form><button type="button" id="fb-search-mode" aria-controls="search-form find-builder" aria-expanded="false">Advanced query · AND, OR, saved keywords</button><p id="fb-advanced-draft-note" hidden>Advanced query keeps its own draft and undo history. Your simple search results remain available; run an advanced query to replace them.</p>`;form.insertAdjacentElement("beforebegin",ui);const input=ui.querySelector("#fb-simple-input"),simple=ui.querySelector("#fb-simple-search"),toggle=ui.querySelector("#fb-search-mode"),message=ui.querySelector("#fb-simple-status");input.value=state.simpleQuery||(state.expr.length===1&&state.expr[0].t==="TERM"?state.expr[0].v:"");const setMode=(advanced,focusInput)=>{editor.dataset.fbSearchMode=advanced?"advanced":"simple";simple.hidden=advanced;ui.querySelector('#fb-advanced-draft-note').hidden=!advanced;toggle.setAttribute("aria-expanded",String(advanced));toggle.textContent=advanced?"← Simple search":"Advanced query · AND, OR, saved keywords";const title=editor.querySelector(".wf-editor-heading h2");if(title)title.textContent=advanced?"Build an advanced query":"Find a concept";editor.setAttribute("aria-label",advanced?"Build an advanced query":"Find a concept");if(advanced)fbPublishDraftExpr();else{const api=window.__findSearchV2;if(api&&api.setDraftExpr)api.setDraftExpr(fbSimpleExpr(input.value));}
if(focusInput)(advanced?document.getElementById("search-input"):input)?.focus();};toggle.addEventListener("click",()=>setMode(editor.dataset.fbSearchMode!=="advanced",true));input.addEventListener("input",()=>{state.simpleQuery=input.value;const api=window.__findSearchV2;if(api&&api.setDraftExpr)api.setDraftExpr(fbSimpleExpr(input.value));});simple.addEventListener("submit",event=>{event.preventDefault();event.stopPropagation();const expression=fbSimpleExpr(input.value),api=window.__findSearchV2;if(!expression){message.textContent="Enter a concept, course code or lecture tag to search.";input.focus();return;}
if(!api||typeof api.runLogicExpr!=="function"){message.textContent="Search is still loading. Try again shortly.";return;}
if(!fbConsumeGuestAction("search",{query:expression,source:"concept-finder-simple",dedupeMs:2500}))return;state.runRequest=(Number(state.runRequest)||0)+1;state.simpleQuery=input.value;if(api.setDraftExpr)api.setDraftExpr(expression);const accepted=api.runLogicExpr(expression);message.textContent=accepted===false?"Search could not run. Try again.":"";});setMode(state.expr.length>1,false);}
function fbMoveQueryNode(index,direction){const next=index+direction;if(!Number.isInteger(index)||![-1,1].includes(direction)||index<0||next<0||index>=state.expr.length||next>=state.expr.length)return false;pushHistory();const moved=state.expr.splice(index,1)[0];state.expr.splice(next,0,moved);state.cursor=next+1;setStatus("ok",nodeLabel(moved)+" moved to position "+(next+1)+". Run search to apply the new order.");render();const host=document.getElementById("find-builder");const control=host&&host.querySelector('[data-move-index="'+next+'"][data-move="'+direction+'"]:not(:disabled)');const fallback=host&&host.querySelector('[data-del="'+next+'"]');if(control||fallback)(control||fallback).focus();return true;}
...
const host=ensureFindBuilderHost();if(!host)return;state.cursor=clamp(state.cursor,0,state.expr.length);const hasExpr=state.expr.length>0;const hasTokenList=state.tokens.length>0;const canUndo=state.history.length>0;const canClear=hasExpr;const canRun=hasExpr;const canRedo=state.future.length>0;const statusHtml=state.lastMsg?`<div id="fb-status" role="status" aria-live="polite" aria-atomic="true" tabindex="-1" class="fb-status ${
    ...
    `;if(i<state.expr.length){const n=state.expr[i];const txt=escapeHtml(nodeLabel(n));let cls="fb-chip";if(n.t==="TERM")cls+=" fb-chip--term";else if(n.t==="OP")cls+=" fb-chip--op";else if(n.t==="LP"||n.t==="RP")cls+=" fb-chip--paren";const __nid=String(n.__fbid||"");const __hideChip=!!(state.animHideNodeIds&&__nid&&state.animHideNodeIds.has(__nid));const __chipStyle=__hideChip?`style="opacity:0; visibility:hidden; transform:scale(.98);"`:"";boardHtml+=`
        <span class="${cls}" data-chip="${i}" data-expr-idx="${i}" data-fb-node="${escapeHtml(n.__fbid || '')}" ${__chipStyle}>
          ...
          <span class="fb-chip__controls">
          <button class="fb-chip__move" type="button" data-move-index="${i}" data-move="-1" ${i === 0 ? "disabled" : ""} aria-label="Move ${txt} at position ${i + 1} left">←</button>
          <button class="fb-chip__move" type="button" data-move-index="${i}" data-move="1" ${i === state.expr.length - 1 ? "disabled" : ""} aria-label="Move ${txt} at position ${i + 1} right">→</button>
          <button class="fb-chip__x" type="button" data-del="${i}" aria-label="Delete ${txt} at position ${i + 1}">×</button>
          </span>
        </span>
      `;}}
const isMobile=!!(window.matchMedia&&window.matchMedia("(max-width: 600px)").matches);const ghostLineHtml=`
...
fbSuppressEmptyHints({pool:true,board:!!panelHasTerm,delayMs:FB_DISSOLVE_MS_DEFAULT+FB_HINT_DELAY_AFTER_DISSOLVE_MS});pushHistory();state.tokens=[];writeTokens([]);if(panelHasTerm){state.expr=[];state.cursor=0;fbClearEmptySearchResults();}
...
fbSuppressEmptyHints({board:true,delayMs:FB_DISSOLVE_MS_DEFAULT+FB_HINT_DELAY_AFTER_DISSOLVE_MS});pushHistory();state.expr=[];state.cursor=0;fbClearEmptySearchResults();clearStatus();window.setTimeout(()=>{render();focusPanelSoon();},20);});}
...
try{fbPlayBoardFlip(boardFirst);}catch(_){}});}
host.querySelectorAll(".fb-slot").forEach((btn)=>{btn.addEventListener("click",()=>{const i=Number(btn.getAttribute("data-slot"));if(!Number.isFinite(i))return;state.cursor=clamp(i,0,state.expr.length);render();});});host.querySelectorAll("[data-move-index]").forEach(btn=>btn.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();if(!btn.disabled)fbMoveQueryNode(Number(btn.getAttribute("data-move-index")),Number(btn.getAttribute("data-move")));}));host.querySelectorAll("[data-fb-example]").forEach(btn=>btn.addEventListener("click",()=>{pushHistory();state.expr=[TERM("m1c"),OP("AND"),TERM("limit")];state.cursor=state.expr.length;clearStatus();render();document.getElementById("fb-run")?.focus();}));host.querySelectorAll('button[data-del]').forEach((btn)=>{btn.addEventListener("click",(ev)=>{ev.stopPropagation();const restoreDeleteFocus=document.activeElement===btn;const i=Number(btn.getAttribute("data-del"));if(!Number.isFinite(i))return;let chip=null;try{chip=btn.closest?btn.closest(".fb-chip"):null;}catch(_){}
...
deleteAtIndex(i);clearStatus();window.setTimeout(()=>{if(!host.isConnected)return;const restoreFocus=restoreDeleteFocus&&(document.activeElement===btn||document.activeElement===document.body);render();if(restoreFocus){const target=host.querySelector('[data-del="'+Math.max(0,Math.min(i,state.expr.length-1))+'"]')||host.querySelector('[data-fb-example]')||document.getElementById('search-input');target?.focus({preventScroll:true});}},20);});});const btnAnd=host.querySelector("#fb-and");if(btnAnd)
...
function tryBeginFromEventTarget(tgt,clientX,clientY){if(tgt&&tgt.closest&&tgt.closest("button"))return false;const chip=tgt&&tgt.closest?tgt.closest(".fb-chip"):null;if(!chip)return false;if(!withinBoard(chip))return false;const idx=Number(chip.getAttribute("data-expr-idx"));if(!Number.isFinite(idx))return false;const node=state.expr[idx];if(!node||node.t!=="TERM")return false;if(!String(node.v||"").trim())return false;if(!String(node.v||"").trim())return false;if(drag)cleanup(false);drag={fromExprIdx:idx,startX:clientX,startY:clientY,lastX:clientX,lastY:clientY,active:false,ghost:null,offsetX:0,offsetY:0,raf:null,toExprIdx:null,dropEl:null,};document.addEventListener("mousemove",onMove,true);document.addEventListener("mouseup",onUp,true);document.addEventListener("touchmove",onTouchMove,{capture:true,passive:false});document.addEventListener("touchend",onTouchEnd,true);document.addEventListener("touchcancel",onTouchEnd,true);window.addEventListener("blur",onCancel,true);document.addEventListener("visibilitychange",onVis,true);return true;}
host.addEventListener("mousedown",(ev)=>{if(ev.button!==0)return;tryBeginFromEventTarget(ev.target,ev.clientX,ev.clientY);});host.addEventListener("touchstart",(ev)=>{const xy=getTouchXY(ev);if(!xy)return;tryBeginFromEventTarget(ev.target,xy.x,xy.y);},{passive:true});host.addEventListener("dragstart",(ev)=>{if(ev.target&&ev.target.closest&&ev.target.closest(".fb-chip"))ev.preventDefault();});})();const runBtn=host.querySelector("#fb-run");if(runBtn&&state.lastMsg)runBtn.setAttribute("aria-describedby","fb-status");if(runBtn){runBtn.addEventListener("click",async()=>{if(!state.expr.length)return;const norm=normalizeNodesToTokens(state.expr);const exprText=tokensToExprText(norm);const requestId=state.runRequest=(Number(state.runRequest)||0)+1;const sourceUrl=window.location.href;const isCurrent=()=>state.runRequest===requestId&&window.location.href===sourceUrl&&document.getElementById("find-builder")===host&&tokensToExprText(normalizeNodesToTokens(state.expr))===exprText;try{const raw=(state.fuzzyNote&&state.fuzzyNote.payload)?state.fuzzyNote.payload:"";if(raw){const data=JSON.parse(raw);if(data&&data.forExprText&&data.forExprText!==exprText){fbHideFuzzyNote();}}}catch(_){}
const v=validateTokens(norm);if(!v.ok){setStatus("bad","Query not run. "+v.msg);render();const status=host.querySelector("#fb-status");if(status)status.focus();return;}
if(!fbConsumeGuestAction("search",{query:exprText,source:"concept-finder-run",dedupeMs:2500}))return;try{const terms=norm.filter(t=>t.k==="TERM").map(t=>String(t.v||"").trim()).filter(Boolean);const uniqueTerms=Array.from(new Set(terms.map(s=>s.toLowerCase())));const hasLogic=norm.some(t=>t.k==="OP"||t.k==="LP"||t.k==="RP");const stream=norm.map(t=>{if(t.k==="TERM")return{term:String(t.v||"").trim().toLowerCase()};if(t.k==="OP")return t.v;if(t.k==="LP")return"(";if(t.k==="RP")return")";return null;}).filter(Boolean);const indexJson=await fbLoadIndex();if(!isCurrent())return;const pageDocs=fbAggregateDocsToPages((indexJson&&indexJson.docs)?indexJson.docs:[]);const rpn=fbToRPN(stream);const termCounts=new Map();for(const t of uniqueTerms)termCounts.set(t,0);let combinedCount=0;for(const d of pageDocs){const extraTags=Array.isArray(d.tags)?d.tags.join(" "):"";const extraAliases=Array.isArray(d.aliases)?d.aliases.join(" "):"";const hay=fbNormaliseForSearch(`${d.title} ${d.text} ${d.location} ${extraTags} ${extraAliases}`);const termMatches=(termLower)=>fbTermMatchesHay(termLower,hay,d);for(const t of uniqueTerms){if(termMatches(t))termCounts.set(t,(termCounts.get(t)||0)+1);}
...
fbHideFuzzyNote();const rawInputForHistory=fbNormaliseTokenInputValue(input.value||"").trim().replace(/\s+/g," ");const rawTokens=parseTokensFromInput(input.value);if(!rawTokens.length)return;const mode=isPool?"pool":"insert";const doInsert=mode==="insert";const sourceUrl=window.location.href;const sourceHost=document.getElementById("find-builder");const sourceExpr=state.expr;const sourcePoolGeneration=Number(state.poolGeneration)||0;const submittedInput=input.value;const isCurrent=()=>window.location.href===sourceUrl&&document.getElementById("find-builder")===sourceHost&&document.getElementById("search-input")===input&&(Number(state.poolGeneration)||0)===sourcePoolGeneration&&(!doInsert||state.expr===sourceExpr);(async()=>{try{fbEnsureExprNodeIds(state.expr);}catch(_){}
...
if(rawInputForHistory)fbAddTokenInputHistory(rawInputForHistory);const inputUnchanged=input.value===submittedInput;if(inputUnchanged){input.value="";try{fbHideTokenInputDropdown();}catch(_){}}
clearStatus();render();if(inputUnchanged&&Date.now()<Number(window.__fbTokenInputEnterBlurUntilV4||0)){try{fbBlurTokenInputAfterEnterAdd(input);}catch(_){}}
...
if(doInsert&&inputUnchanged){if(Date.now()<Number(window.__fbTokenInputEnterBlurUntilV4||0)){try{fbBlurTokenInputAfterEnterAdd(input);}catch(_){}}else if(keepTokenInputFocus)fbRefocusTokenInputAfterAdd();else focusPanelSoon();}})();},true);}
