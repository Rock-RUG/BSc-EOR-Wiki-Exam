...
function lpNearbyCloseEmptyHoles(data,width,height,hiddenPixels=[],maxSpan=Infinity,featherSpan=0){if(!(width>2&&height>2)||!data||data.length<width*height*4)return 0;const count=width*height,state=new Uint8Array(count),protectedPixels=new Uint8Array(count);for(const point of hiddenPixels){const x=Math.floor(point.x),y=Math.floor(point.y);if(x>=0&&x<width&&y>=0&&y<height)protectedPixels[y*width+x]=1;}
const queue=new Int32Array(count),coreThreshold=12;let filled=0;for(let start=0;start<count;start++){if(state[start]||data[start*4+3]===255)continue;let head=0,tail=1,exterior=false,protectedHole=false,hasCore=false;let minX=width,maxX=0,minY=height,maxY=0;let coreMinX=width,coreMaxX=0,coreMinY=height,coreMaxY=0;queue[0]=start;state[start]=1;while(head<tail){const index=queue[head++],x=index%width,y=(index-x)/width;if(!x||!y||x===width-1||y===height-1)exterior=true;if(protectedPixels[index])protectedHole=true;minX=Math.min(minX,x);maxX=Math.max(maxX,x);minY=Math.min(minY,y);maxY=Math.max(maxY,y);if(data[index*4+3]<=coreThreshold){hasCore=true;coreMinX=Math.min(coreMinX,x);coreMaxX=Math.max(coreMaxX,x);coreMinY=Math.min(coreMinY,y);coreMaxY=Math.max(coreMaxY,y);}
for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){if(!dx&&!dy)continue;const nx=x+dx,ny=y+dy;if(nx<0||nx>=width||ny<0||ny>=height)continue;const next=ny*width+nx;if(!state[next]&&data[next*4+3]<255){state[next]=1;queue[tail++]=next;}}}
const featherLimit=Math.max(0,Number(featherSpan)||0);if(exterior||protectedHole||!hasCore||coreMaxX-coreMinX+1>maxSpan||coreMaxY-coreMinY+1>maxSpan||maxX-minX+1>maxSpan+2*featherLimit||maxY-minY+1>maxSpan+2*featherLimit)continue;for(let i=0;i<tail;i++){const at=queue[i]*4;data[at]=data[at+1]=data[at+2]=data[at+3]=255;}
filled++;}
return filled;}
...
return[{...area,d,bounds:box,label,frontiers:[],borders:(area.borders||[]).map(run=>({...run,d:lpNearbyClipPath(run.d,box,false)})).filter(run=>run.d)}];});return{...world,source:world,areas,reveal,revealKey:key};}
...
return"full";}
function lpMapTabHelperText(kind,isPhone){const phone=typeof isPhone==="boolean"?isPhone:!!__lpIsPhoneTouch();if(String(kind||"")==="local"){const controls=phone?"Tap a nearby concept to choose Expand map or Open page.":"Hover a nearby concept to choose Expand map or Open page.";return controls+" Drag freely to explore. Expansions last until you close the map."+(window.__mkExamMode?"":" Nearby starts around the current page. Knowledge masking hides unfamiliar titles. With a connected account, an expansion that reveals new concepts gives a chance to find treasure, with or without masking. Each expanded concept gives one chance per UTC day. Crown/gold/silver/bronze chances: 1/1,000, 1/100, 1/20, 1/5; daily limits: 1, 1, 2, 5, including unopened chests. Open before midnight UTC for XP, EORbits and possible shop items; owned items become EORbits. Full rules: Site guide → Map treasures.");}
