...
return result;}
function lpMapProjectedTerritoryView(engine){return['W','H','camera','rx','ry','roll','panX','panY'].map(key=>(Number(engine&&engine[key])||0).toFixed(3)).join('|');}
function lpMapProjectedTerritoryGuard(engine){if(engine.__lpDisposeProjectedTerritory)return;const modal=engine.modal,listeners=[];const stop=()=>{if(engine.__lpCancelProjectedTerritoryReveal)engine.__lpCancelProjectedTerritoryReveal();const exploration=engine.state&&engine.state.__lpExploration;const pending=exploration&&exploration.pendingProjectedTerritoryReveal;if(pending&&pending.engine===engine)exploration.pendingProjectedTerritoryReveal=null;};const bind=(target,name,fn=stop)=>{if(!target||!target.addEventListener)return;target.addEventListener(name,fn,{capture:true,passive:true});listeners.push(()=>target.removeEventListener(name,fn,true));};['pointerdown','touchstart','wheel','keydown'].forEach(name=>bind(modal,name));['resize','orientationchange','pagehide','lp:map-view-mode-change','mk:site-motion-change','mk:motionchange'].forEach(name=>bind(window,name));bind(document,'visibilitychange',()=>{if(document.hidden)stop();});try{const media=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)');if(media&&media.addEventListener)bind(media,'change',()=>{if(media.matches)stop();});}catch(_){}
engine.__lpDisposeProjectedTerritory=()=>{stop();listeners.splice(0).forEach(remove=>remove());engine.__lpProjectedTerritoryPaint=null;engine.__lpDisposeProjectedTerritory=null;};}
function lpMapPaintProjectedTerritories(engine,svg,geometry,projected){if(!engine||engine.kind!=='local'||!geometry)return;const exploration=engine.state&&engine.state.__lpExploration;const pending=exploration&&exploration.pendingProjectedTerritoryReveal;const modal=engine.modal;const moving=!!(modal&&modal.classList.contains('lp-webgl3d-dragging'))||!!(engine.state&&engine.state.dragging);const open=!!(engine.__observing&&modal&&modal.__lpWebgl3dEngine===engine&&modal.classList.contains('lp-open')&&svg.isConnected!==false);if(open&&pending&&!pending.engine)pending.engine=engine;if(open)lpMapProjectedTerritoryGuard(engine);else if(engine.__lpDisposeProjectedTerritory)engine.__lpDisposeProjectedTerritory();const previousPaint=engine.__lpProjectedTerritoryPaint;if((!open||moving||lpMotionReduced())&&engine.__lpCancelProjectedTerritoryReveal)engine.__lpCancelProjectedTerritoryReveal();if(pending&&pending.engine===engine&&(!open||moving||lpMotionReduced()))exploration.pendingProjectedTerritoryReveal=null;if(previousPaint&&previousPaint.signature===geometry.signature&&!geometry.transient&&previousPaint.layer.parentNode===svg&&!(pending&&pending.engine===engine))return;if(engine.__lpCancelProjectedTerritoryReveal)engine.__lpCancelProjectedTerritoryReveal();if(previousPaint)previousPaint.layer.remove();engine.__lpProjectedTerritoryPaint=null;if(geometry.transient)return;const field=document.createElementNS('http://www.w3.org/2000/svg','g');field.classList.add('lp-webgl3d-territory-field');field.setAttribute('aria-hidden','true');field.setAttribute('pointer-events','none');svg.insertBefore(field,svg.firstChild);const next=lpMapPaintTerritories(field,geometry.regions);lpMapWorldSurfaceProjection(next,engine);engine.__lpProjectedTerritoryPaint={signature:geometry.signature,layer:field};const view=lpMapProjectedTerritoryView(engine);const request=exploration&&exploration.pendingProjectedTerritoryReveal;if(request&&request.engine===engine){exploration.pendingProjectedTerritoryReveal=null;const anchor=projected.get(request.anchor);const oldAreas=request.previous||[];if(open&&!moving&&!lpMotionReduced()&&anchor&&oldAreas.length&&geometry.regions.length&&request.view===view){const old=lpMapPaintTerritories(field,oldAreas);lpMapWorldSurfaceProjection(old,engine);const all=geometry.regions.concat(oldAreas),left=Math.min(...all.map(area=>area.bounds.left)),right=Math.max(...all.map(area=>area.bounds.right));const top=Math.min(...all.map(area=>area.bounds.top)),bottom=Math.max(...all.map(area=>area.bounds.bottom));const added=new Set(request.added||[]);const emergence=Array.from(projected.values()).filter(point=>added.has(point.key)).map(point=>({key:point.key,x:point.x,y:point.y,el:point.node&&point.node.el}));lpMapAnimateTerritoryReveal(field,next,old,{x:anchor.x,y:anchor.y},{left,right,top,bottom,width:right-left,height:bottom-top},modal,emergence);const finish=modal.__lpCancelTerritoryReveal;if(typeof finish==='function'){let timer=0,done=false;const cancel=()=>{if(done)return;done=true;if(timer)window.clearTimeout(timer);finish();if(engine.__lpCancelProjectedTerritoryReveal===cancel)engine.__lpCancelProjectedTerritoryReveal=null;};engine.__lpCancelProjectedTerritoryReveal=cancel;timer=window.setTimeout(cancel,1200);}}}
const retained=new Set(((engine.ctx&&engine.ctx.baseNodes)||[]).map(lpCanonKey));const committedOnly=!retained.size||Array.from(projected.keys()).every(key=>retained.has(key));if(exploration&&open&&committedOnly)exploration.projectedTerritorySnapshot={regions:geometry.regions,view};}
...
function lpMapExpansionRevealItems(modal,center,items){const delays=new Map(),effects=new Map();let elapsed=0,finished=false;const farthest=Math.max(1,...items.map(item=>Math.hypot(item.x-center.x,item.y-center.y)));for(const item of items)delays.set(lpCanonKey(item.key),820*Math.hypot(item.x-center.x,item.y-center.y)/farthest);const add=(el,delay)=>{if(!el||finished||effects.has(el)||typeof el.animate!=='function')return;try{const computed=Number.parseFloat(window.getComputedStyle(el).opacity);const authored=Number.parseFloat(el.style.opacity||(el.getAttribute&&el.getAttribute('opacity'))||'1');const opacity=Math.max(0,Math.min(1,Number.isFinite(computed)?computed:(Number.isFinite(authored)?authored:1)));const animation=el.animate([{opacity:0,visibility:'hidden'},{opacity,visibility:'visible'}],{duration:180,delay,fill:'both',easing:'ease-out'});animation.pause();animation.currentTime=elapsed;effects.set(el,animation);}catch(_){}};const controller={edge(el,from,to){const a=delays.get(lpCanonKey(from)),b=delays.get(lpCanonKey(to));if(a===undefined&&b===undefined)return;add(el,Math.max(a||0,b||0));},tick(now){elapsed=now;for(const[el,animation]of effects){if(el.isConnected===false){animation.cancel();effects.delete(el);}
else animation.currentTime=now;}},finish(){if(finished)return;finished=true;effects.forEach(animation=>animation.cancel());effects.clear();if(modal.__lpExpansionReveal===controller)modal.__lpExpansionReveal=null;}};items.forEach(item=>add(item.el,delays.get(lpCanonKey(item.key))));modal.__lpExpansionReveal=controller;return controller;}
function lpMapAnimateTerritoryReveal(svg,nextLayer,oldLayer,center,bounds,modal,emergence=[]){if(modal.__lpCancelTerritoryReveal)modal.__lpCancelTerritoryReveal();if(!oldLayer||!center||!bounds||lpMotionReduced()){if(oldLayer)oldLayer.remove();return;}
const ns='http://www.w3.org/2000/svg';const uid=`lp-territory-reveal-${lpMapAnimateTerritoryReveal.serial = (lpMapAnimateTerritoryReveal.serial || 0) + 1}`;const defs=document.createElementNS(ns,'defs');const blur=document.createElementNS(ns,'filter');blur.id=uid+'-soft';blur.setAttribute('filterUnits','userSpaceOnUse');const box={x:bounds.left-80,y:bounds.top-80,width:bounds.width+160,height:bounds.height+160};for(const[key,value]of Object.entries(box))blur.setAttribute(key,value);const gaussian=document.createElementNS(ns,'feGaussianBlur');gaussian.setAttribute('stdDeviation','18');blur.appendChild(gaussian);defs.appendChild(blur);const circles=[];for(const[name,field,wave]of[['new','black','white'],['old','white','black']]){const mask=document.createElementNS(ns,'mask');mask.id=uid+'-'+name;mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');for(const[key,value]of Object.entries(box))mask.setAttribute(key,value);const rect=document.createElementNS(ns,'rect');for(const[key,value]of Object.entries(box))rect.setAttribute(key,value);rect.setAttribute('fill',field);rect.style.setProperty('fill',field,'important');mask.appendChild(rect);const circle=document.createElementNS(ns,'circle');circle.setAttribute('cx',center.x);circle.setAttribute('cy',center.y);circle.setAttribute('r','0');circle.setAttribute('fill',wave);circle.style.setProperty('fill',wave,'important');circle.setAttribute('filter',`url(#${blur.id})`);mask.appendChild(circle);circles.push(circle);defs.appendChild(mask);}
svg.insertBefore(defs,svg.firstChild);oldLayer.setAttribute('mask',`url(#${uid}-old)`);nextLayer.setAttribute('mask',`url(#${uid}-new)`);const ripple=document.createElementNS(ns,'circle');ripple.setAttribute('class','lp-map-expansion-ripple');ripple.setAttribute('aria-hidden','true');ripple.setAttribute('cx',center.x);ripple.setAttribute('cy',center.y);ripple.setAttribute('r','0');ripple.setAttribute('fill','none');ripple.setAttribute('vector-effect','non-scaling-stroke');ripple.style.pointerEvents='none';svg.appendChild(ripple);const radius=Math.max(...[bounds.left,bounds.right].flatMap(x=>[bounds.top,bounds.bottom].map(y=>Math.hypot(x-center.x,y-center.y))))+80;let frame=0,start=null,finished=false;const items=emergence.length?lpMapExpansionRevealItems(modal,center,emergence):null;const listeners=[];const finish=()=>{if(finished)return;finished=true;if(frame)window.cancelAnimationFrame(frame);frame=0;nextLayer.removeAttribute('mask');oldLayer.remove();defs.remove();ripple.remove();if(items)items.finish();listeners.splice(0).forEach(remove=>remove());if(modal.__lpCancelTerritoryReveal===finish)modal.__lpCancelTerritoryReveal=null;};modal.__lpCancelTerritoryReveal=finish;const bind=(target,name,callback=finish)=>{if(!target||!target.addEventListener)return;target.addEventListener(name,callback,{capture:true,passive:true});listeners.push(()=>target.removeEventListener(name,callback,true));};['pointerdown','touchstart','wheel','keydown'].forEach(name=>bind(modal,name));['resize','orientationchange','pagehide','lp:map-view-mode-change','mk:site-motion-change','mk:motionchange'].forEach(name=>bind(window,name));bind(document,'visibilitychange',()=>{if(document.hidden)finish();});try{const media=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)');if(media&&media.addEventListener)bind(media,'change',()=>{if(media.matches)finish();});}catch(_){}
const step=now=>{frame=0;if(finished)return;if(!nextLayer.isConnected||!modal.classList.contains('lp-open')||lpMotionReduced())return finish();if(start===null)start=now;const progress=Math.min(1,Math.max(0,(now-start)/1000));const reach=radius*progress;circles.forEach(circle=>circle.setAttribute('r',reach));ripple.setAttribute('r',reach);ripple.style.opacity=String(Math.sin(Math.PI*progress)*.65);if(items)items.tick(progress*1000);if(progress>=1)return finish();frame=window.requestAnimationFrame(step);};frame=window.requestAnimationFrame(step);}
...
defs.appendChild(silhouette);landClip=`url(#${silhouette.id})`;const filter=document.createElementNS(ns,'filter');filter.id=uid+'-soft';filter.setAttribute('filterUnits','userSpaceOnUse');for(const[key,value]of Object.entries(box))filter.setAttribute(key,value);const blur=document.createElementNS(ns,'feGaussianBlur');blur.setAttribute('stdDeviation','12');filter.appendChild(blur);defs.appendChild(filter);const mask=document.createElementNS(ns,'mask');mask.id=uid+'-frontier';mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');mask.style.setProperty('mask-type','luminance','important');for(const[key,value]of Object.entries(box))mask.setAttribute(key,value);const field=document.createElementNS(ns,'rect');for(const[key,value]of Object.entries(box))field.setAttribute(key,value);field.setAttribute('fill','white');field.style.setProperty('fill','white','important');mask.appendChild(field);const coast=document.createElementNS(ns,'g');coast.id=uid+'-coast';coast.classList.add('lp-course-frontier-fade');coast.setAttribute('filter',`url(#${filter.id})`);for(const run of frontiers){const fade=document.createElementNS(ns,'path');fade.classList.add('lp-course-frontier-mask');fade.setAttribute('d',run.d||run);fade.setAttribute('fill','none');fade.setAttribute('stroke','black');fade.style.setProperty('fill','none','important');fade.style.setProperty('stroke','black','important');fade.setAttribute('stroke-width','56');fade.setAttribute('stroke-linecap','round');fade.setAttribute('stroke-linejoin','round');coast.appendChild(fade);}
...
const exploration=mapState.__lpExploration;const nb=window.MkMapExploration.union(exploration,getNeighbourhood(graph,cur,backHop,fwdHop,relMode));const reveal=exploration.pendingReveal;if(reveal&&modal.classList.contains('lp-webgl3d')&&exploration.projectedTerritorySnapshot){const snapshot=exploration.projectedTerritorySnapshot;exploration.pendingProjectedTerritoryReveal={anchor:reveal.anchor,added:reveal.added,previous:snapshot.regions,view:snapshot.view};}
try{if(relMode!=="off"){const coreLocs=(nb&&nb.core)?nb.core:[];const coreSet2=new Set((coreLocs||[]).map(normLoc));const extraLocs=(nb&&nb.nodes)?(nb.nodes||[]).filter((x)=>x&&!coreSet2.has(normLoc(x))):[];const limCore=(relMode==="all")?22:14;const limExtra=(relMode==="all")?14:8;try{if(modal.__lpRelatedPrefetchTimer)window.clearTimeout(modal.__lpRelatedPrefetchTimer);}catch(_){}
...
const territoryLayer=lpMapPaintTerritories(svg,geometry.regions);exploration.pendingReveal=null;if(reveal&&!modal.classList.contains('lp-webgl3d')){const oldLayer=lpMapPaintTerritories(svg,reveal.previous);const allAreas=geometry.regions.concat(reveal.previous);const left=Math.min(...allAreas.map(area=>area.bounds.left));const right=Math.max(...allAreas.map(area=>area.bounds.right));const top=Math.min(...allAreas.map(area=>area.bounds.top));const bottom=Math.max(...allAreas.map(area=>area.bounds.bottom));const added=new Set(reveal.added||[]);const emergence=Array.from(nodeEls,([key,el])=>({key,el,...pos.get(key)})).filter(item=>added.has(lpCanonKey(item.key)));lpMapAnimateTerritoryReveal(svg,territoryLayer,oldLayer,reveal.center,{left,right,top,bottom,width:right-left,height:bottom-top},modal,emergence);if(modal.__lpExpansionReveal)edgeEls.forEach(edge=>modal.__lpExpansionReveal.edge(edge,edge.dataset.from,edge.dataset.to));}
exploration.territorySnapshot=geometry.regions;vp.insertBefore(svg,vp.firstChild);const hiSvg=document.createElementNS("http://www.w3.org/2000/svg","svg");hiSvg.setAttribute("viewBox",`0 0 ${W} ${H}`);hiSvg.setAttribute("width",String(W));hiSvg.setAttribute("height",String(H));hiSvg.classList.add("lp-map-svg-hi");hiSvg.style.pointerEvents="none";const hiDefs=document.createElementNS("http://www.w3.org/2000/svg","defs");hiSvg.appendChild(hiDefs);const hiMaskId=__lpSvgUid("lpNodeMaskHi");hiSvg.__lpMaskId=hiMaskId;const __lpDesktopSingleLayerHi=true;if(__lpDesktopSingleLayerHi){try{hiSvg.dataset.lpSingleLayer="1";}catch(_){}
...
function hideLocalMapModal(){lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpCancelTerritoryReveal)modal.__lpCancelTerritoryReveal();try{if(modal.__lpStopZoomGesture)modal.__lpStopZoomGesture();}catch(_){}
