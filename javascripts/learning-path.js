...
if(vp.__lpWorldWindowTimer)clearTimeout(vp.__lpWorldWindowTimer);vp.__lpWorldWindowTimer=setTimeout(()=>{vp.__lpWorldWindowTimer=0;if(!vp.isConnected)return;const live=readCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||Math.abs(lpWorldWindowFor(live).s/committed.s-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);},160);}
...
const mathNodeEls=Array.from(nodeEls.values()).filter((el)=>{try{const titleEl=el&&el.querySelector?el.querySelector('.lp-node-title'):null;return lpNodeTitleNeedsTypeset(titleEl);}catch(_){return false;}});if(mathNodeEls.length)requestAnimationFrame(()=>{Promise.resolve().then(()=>typesetMathSafe(mathNodeEls.map((el)=>(el&&el.querySelector)?(el.querySelector('.lp-node-title')||el):el))).then(()=>{mathNodeEls.forEach((el)=>{try{lpCacheRenderedNodeTitle(graph,el);}catch(_){}});}).catch(()=>{});});if(exploration.focusAfterExpand){const target=nodeEls.get(exploration.focusAfterExpand),point=pos.get(exploration.focusAfterExpand);exploration.focusAfterExpand=null;if(point){st.tx=stageW/2-point.x*st.scale;st.ty=stageH/2-point.y*st.scale;}
if(modal.__lpApplyTransform)modal.__lpApplyTransform({skipClamp:true});if(target)target.focus({preventScroll:true});clearFocus();}
...
function startDrag(e){if(!vp||!panSurface)return;if((e.pointerType||"mouse")==="mouse"&&typeof e.button==="number"&&e.button!==0)return;const t=e.target;if(t&&t.closest&&t.closest('.lp-mctrl, .lp-mzoom, .lp-close, [data-lp-fullscreen], .lp-node-action, .lp-map-chests, [data-lp-map-dir-menu], .lp-view-toggle, canvas.lp-webgl3d-canvas'))return;lpMarkLocalMapInteractionBusy(320);const isMobile=isTouchGestureDevice();if(isMobile&&(e.pointerType==="touch"||e.pointerType==="pen")){if(typeof e.pointerId==="number"){activePtrs.set(e.pointerId,{x:e.clientX,y:e.clientY});}
...
const hardTouchGestureCleanup=(e)=>{try{const remainingTouches=e&&e.touches?Number(e.touches.length)||0:0;if(remainingTouches>0)return;activePtrs.clear();pinch=null;gestureCamera.flush(()=>applyTransform({skipClamp:true}));if(!dragging&&!pendingNodeDrag&&!st.__lpZoomDragging){try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",false);}catch(_){}
...
lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpCancelGestureFrame)modal.__lpCancelGestureFrame();if(modal.__lpViewportResizeCleanup)modal.__lpViewportResizeCleanup();try{if(modal.__lpStopZoomGesture)modal.__lpStopZoomGesture();}catch(_){}
