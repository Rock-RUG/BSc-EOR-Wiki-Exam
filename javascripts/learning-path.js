...
layer.appendChild(path);for(const run of area.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-course-border');border.setAttribute('d',run.d||run);border.dataset.lpCourse=area.course;border.style.setProperty('--lp-territory-hue',String(hue));layer.appendChild(border);}
...
label.appendChild(span);});layer.appendChild(label);}
svg.insertBefore(layer,svg.firstChild);return layer;}
...
const mapBtn=panel.querySelector('[data-lp-open-map]');if(mapBtn){mapBtn.__lpOpen=()=>showLocalMapModal(graph);lpBindTap(mapBtn,mapBtn.__lpOpen);}
