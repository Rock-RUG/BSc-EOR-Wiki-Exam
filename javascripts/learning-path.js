...
function lpWebgl3dBindCanvas(engine){if(!engine||!engine.canvas||engine.canvas.__lpWebgl3dPointerBound)return;const canvas=engine.canvas;canvas.__lpWebgl3dPointerBound=true;canvas.setAttribute('aria-label','Rotatable 3D knowledge map canvas');canvas.tabIndex=-1;let drag=null;const stop=(e)=>{try{if(e&&e.cancelable)e.preventDefault();if(e&&e.stopPropagation)e.stopPropagation();}catch(_){}};canvas.addEventListener('pointerdown',(e)=>{if(e&&e.button!=null&&e.button!==0)return;stop(e);lpMarkLocalMapInteractionBusy(360);const cur=engine.modal&&engine.modal.__lp3d?engine.modal.__lp3d:lp3dDefaultRot(engine.kind);drag={id:e.pointerId,x:Number(e.clientX)||0,y:Number(e.clientY)||0,rx:Number.isFinite(Number(cur.rx))?Number(cur.rx):lp3dDefaultRot(engine.kind).rx,ry:Number.isFinite(Number(cur.ry))?Number(cur.ry):lp3dDefaultRot(engine.kind).ry,};try{canvas.setPointerCapture(e.pointerId);}catch(_){}
try{engine.modal.classList.add('lp-webgl3d-dragging');}catch(_){}},true);canvas.addEventListener('pointermove',(e)=>{if(!drag||(e.pointerId!=null&&drag.id!==e.pointerId))return;stop(e);lpMarkLocalMapInteractionBusy(240);const dx=(Number(e.clientX)||0)-drag.x;const dy=(Number(e.clientY)||0)-drag.y;lp3dSetModalRotation(engine.modal,engine.kind,drag.rx-dy*0.22,drag.ry+dx*0.22);},true);const end=(e)=>{if(!drag)return;stop(e);lpMarkLocalMapInteractionBusy(180);try{canvas.releasePointerCapture(drag.id);}catch(_){}
drag=null;try{engine.modal.classList.remove('lp-webgl3d-dragging');}catch(_){}
try{lpWebgl3dFlushRotation(engine);}catch(_){}};canvas.addEventListener('pointerup',end,true);canvas.addEventListener('pointercancel',end,true);}
...
const relMode="all";modal.dataset.lpRelMode=relMode;const nb=getNeighbourhood(graph,cur,backHop,fwdHop,relMode);try{if(relMode!=="off"){const coreLocs=(nb&&nb.core)?nb.core:[];const coreSet2=new Set((coreLocs||[]).map(normLoc));const extraLocs=(nb&&nb.nodes)?(nb.nodes||[]).filter((x)=>x&&!coreSet2.has(normLoc(x))):[];const limCore=(relMode==="all")?22:14;const limExtra=(relMode==="all")?14:8;__lpPrefetchRelatedForLocs(graph,coreLocs,null,limCore);__lpPrefetchRelatedForLocs(graph,extraLocs,null,limExtra);}}catch(_){}
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();try{const drawnLocs=new Set();for(const loc of nodes){if(!loc)continue;drawnLocs.add(normLoc(loc));drawnLocs.add(lpCanonKey(loc));}
...
let __lpZoomInputRaf=0;let __lpZoomInputValue=null;const __lpBeginZoomDrag=(e)=>{lpMarkLocalMapInteractionBusy(360);clearHoverNow();markGestureScale();st.__lpZoomDragging=true;setSliderZoomDragMode(true);try{st.__lpZoomAnchor=getStageCenter();}catch(_){st.__lpZoomAnchor=null;}
...
e.stopPropagation();},{passive:true});const __lpEndZoomDrag=()=>{lpMarkLocalMapInteractionBusy(180);if(__lpZoomInputRaf){try{cancelAnimationFrame(__lpZoomInputRaf);}catch(_){}
