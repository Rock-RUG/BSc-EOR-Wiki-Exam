...
const uid=`lp-course-field-${lpMapPaintTerritories.serial = (lpMapPaintTerritories.serial || 0) + 1}`;const defs=document.createElementNS(ns,'defs');layer.appendChild(defs);const borderPaint=[];const labelPaint=[];let regionIndex=0;for(const area of areas||[]){const hue=courseHues.get(area.course);const path=document.createElementNS(ns,'path');path.classList.add('lp-course-territory');path.setAttribute('d',area.d);path.setAttribute('fill-rule','evenodd');path.style.setProperty('--lp-territory-hue',String(hue));path.dataset.lpCourse=area.course;const frontiers=(area.frontiers||[]).filter(run=>run&&(run.d||typeof run==='string'));if(frontiers.length&&area.bounds){const id=`${uid}-${regionIndex++}`;const box={x:area.bounds.left-80,y:area.bounds.top-80,width:area.bounds.right-area.bounds.left+160,height:area.bounds.bottom-area.bounds.top+160};const filter=document.createElementNS(ns,'filter');filter.id=id+'-soft';filter.setAttribute('filterUnits','userSpaceOnUse');for(const[key,value]of Object.entries(box))filter.setAttribute(key,value);const blur=document.createElementNS(ns,'feGaussianBlur');blur.setAttribute('stdDeviation','12');filter.appendChild(blur);defs.appendChild(filter);const mask=document.createElementNS(ns,'mask');mask.id=id+'-frontier';mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');for(const[key,value]of Object.entries(box))mask.setAttribute(key,value);const field=document.createElementNS(ns,'rect');for(const[key,value]of Object.entries(box))field.setAttribute(key,value);field.setAttribute('fill','white');field.style.setProperty('fill','white','important');mask.appendChild(field);for(const run of frontiers){const fade=document.createElementNS(ns,'path');fade.classList.add('lp-course-frontier-mask');fade.setAttribute('d',run.d||run);fade.setAttribute('fill','none');fade.setAttribute('stroke','black');fade.style.setProperty('fill','none','important');fade.style.setProperty('stroke','black','important');fade.setAttribute('stroke-width','56');fade.setAttribute('stroke-linecap','round');fade.setAttribute('stroke-linejoin','round');fade.setAttribute('filter',`url(#${filter.id})`);mask.appendChild(fade);}
for(const run of(area.boundaryRuns||[]).filter(run=>run.kind==='border')){const keep=document.createElementNS(ns,'path');keep.classList.add('lp-course-border-mask-keep');keep.setAttribute('d',run.d);keep.style.setProperty('fill','none','important');keep.style.setProperty('stroke','white','important');keep.setAttribute('stroke-width','56');keep.setAttribute('stroke-linecap','round');keep.setAttribute('stroke-linejoin','round');keep.setAttribute('filter',`url(#${filter.id})`);mask.appendChild(keep);}
defs.appendChild(mask);path.setAttribute('mask',`url(#${mask.id})`);}
layer.appendChild(path);for(const run of area.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-course-border');border.setAttribute('d',run.d||run);border.dataset.lpCourse=area.course;border.style.setProperty('--lp-territory-hue',String(hue));borderPaint.push(border);}
...
return;}}
const t=e.target;if(t&&t.closest){if(t.closest("a.lp-node")){if(e.pointerType==="touch"||e.pointerType==="pen"){pendingNodeDrag={pid:(typeof e.pointerId==="number")?e.pointerId:null,sx:Number(e.clientX)||0,sy:Number(e.clientY)||0};activePid=pendingNodeDrag.pid;lx=Number(e.clientX)||0;ly=Number(e.clientY)||0;return;}
return;}
