...
return out;}
function lpNearbyWorld(world,nodes,previous){if(!world||!nodes.length||!nodes.every(id=>world.has(id)))return null;const key=Array.from(new Set(nodes.map(lpCanonKey))).sort().join('\n');if(previous&&previous.source===world&&previous.revealKey===key)return previous;const reveal=lpNearbyReveal(world,nodes),box=reveal.bounds;const courses=new Set(nodes.map(id=>world.courseOf(id)));const areas=world.areas.flatMap(area=>{const b=area.bounds;if(b.right<box.left||b.left>box.right||b.bottom<box.top||b.top>box.bottom)return[];const d=lpNearbyClipPath(area.d,box,true);if(!d)return[];const label=area.label;const nearLabel=label&&courses.has(area.course)&&nodes.some(id=>{const p=world.point(id);return world.courseOf(id)===area.course&&Math.hypot(p.x-label.x,p.y-label.y)<100;});return[{...area,d,bounds:box,label:nearLabel?label:null,frontiers:[],borders:(area.borders||[]).map(run=>({...run,d:lpNearbyClipPath(run.d,box,false)})).filter(run=>run.d)}];});return{...world,source:world,areas,reveal,revealKey:key};}
...
function lpMapGeometryOptions(nodes){const courseLabels={},labelSizes={};for(const node of nodes){if(courseLabels[node.course])continue;const label=__lpCourseLabelFromRelPath(node.id)||node.course.split('/').pop().replace(/-/g,' ');courseLabels[node.course]=label;const lines=lpMapCourseLabelLines(label);labelSizes[node.course]={width:Math.min(320,Math.max(120,...lines.map(line=>line.length*11+20))),height:lines.length>1?68:48};}
...
function lpNearbyRevealMask(defs,uid,reveal){const ns='http://www.w3.org/2000/svg',b=reveal.bounds;const width=b.right-b.left,height=b.bottom-b.top;const mask=document.createElementNS(ns,'mask');mask.id=uid+'-nearby';mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');mask.style.setProperty('mask-type','luminance','important');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height}))mask.setAttribute(k,v);if(!reveal.alphaUrl){try{const canvas=document.createElement('canvas');const scale=Math.min(1,1024/Math.max(width,height));canvas.width=Math.max(1,Math.ceil(width*scale));canvas.height=Math.max(1,Math.ceil(height*scale));const ctx=canvas.getContext('2d');if(ctx){ctx.scale(scale,scale);ctx.translate(-b.left,-b.top);const points=reveal.hull,radius=reveal.radius,inner=radius-reveal.feather;const stops=gradient=>{gradient.addColorStop(0,'rgba(255,255,255,1)');gradient.addColorStop(inner/radius,'rgba(255,255,255,1)');gradient.addColorStop(1,'rgba(255,255,255,0)');return gradient;};if(points.length>2){ctx.beginPath();points.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.closePath();ctx.fillStyle='white';ctx.fill();}
for(let i=0;i<points.length;i++){const a=points[i],c=points[(i+1)%points.length];const length=Math.hypot(c.x-a.x,c.y-a.y);if(length){const nx=(c.y-a.y)/length*radius,ny=(a.x-c.x)/length*radius;ctx.fillStyle=stops(ctx.createLinearGradient(a.x,a.y,a.x+nx,a.y+ny));ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(c.x,c.y);ctx.lineTo(c.x+nx,c.y+ny);ctx.lineTo(a.x+nx,a.y+ny);ctx.closePath();ctx.fill();}
ctx.fillStyle=stops(ctx.createRadialGradient(a.x,a.y,0,a.x,a.y,radius));ctx.beginPath();ctx.arc(a.x,a.y,radius,0,2*Math.PI);ctx.fill();}
...
if(reveal.alphaUrl){const image=document.createElementNS(ns,'image');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height,href:reveal.alphaUrl,preserveAspectRatio:'none'}))image.setAttribute(k,v);mask.appendChild(image);}else{const single=reveal.hull.length===1;const path=document.createElementNS(ns,single?'circle':'path');if(single){path.setAttribute('cx',reveal.hull[0].x);path.setAttribute('cy',reveal.hull[0].y);path.setAttribute('r',reveal.radius/2);}
else path.setAttribute('d','M'+reveal.hull.map(p=>`${p.x} ${p.y}`).join('L')+'Z');path.setAttribute('fill','white');path.setAttribute('stroke','white');path.setAttribute('stroke-width',reveal.radius*(single?1:2));path.setAttribute('stroke-linecap','round');path.setAttribute('stroke-linejoin','round');mask.appendChild(path);}
...
if(!area.label)continue;const label=document.createElementNS(ns,'text');label.classList.add('lp-course-territory-label');label.setAttribute('x',area.label.x);label.setAttribute('y',area.label.y);label.setAttribute('text-anchor','middle');label.setAttribute('dominant-baseline','middle');label.dataset.lpCourse=area.course;label.setAttribute('aria-label',area.label.text);const lines=lpMapCourseLabelLines(area.label.text);lines.forEach((line,index)=>{const span=document.createElementNS(ns,'tspan');span.setAttribute('x',area.label.x);span.setAttribute('y',area.label.y+(index-(lines.length-1)/2)*24);span.textContent=line;if(line.length*10>area.label.width-12){span.setAttribute('textLength',area.label.width-12);span.setAttribute('lengthAdjust','spacingAndGlyphs');}
label.appendChild(span);});labelPaint.push(label);}
if(parchmentRim)(land||layer).appendChild(parchmentRim);borderPaint.forEach(border=>(land||layer).appendChild(border));let labelTarget=paintOptions.labels||layer;if(paintOptions.reveal&&paintOptions.labels&&labelPaint.length){const labelDefs=document.createElementNS(ns,'defs');labelTarget.appendChild(labelDefs);const labelGroup=document.createElementNS(ns,'g');labelGroup.setAttribute('mask',lpNearbyRevealMask(labelDefs,uid+'-labels',paintOptions.reveal));labelTarget.appendChild(labelGroup);labelTarget=labelGroup;}
labelPaint.forEach(label=>labelTarget.appendChild(label));svg.insertBefore(layer,svg.firstChild);return layer;}
...
function lpWorldLabelScale(modal,scale){const layer=modal&&modal.__lpWorldTerrain&&modal.__lpWorldTerrain.labels;if(!layer||!layer.isConnected)return;const text=Math.max(1,Math.min(4,.62/(Number(scale)||1))).toFixed(2);if(layer.__lpLabelScale===text)return;layer.__lpLabelScale=text;layer.style.setProperty('--lp-world-label-scale',text);}
...
vp.insertBefore(terrain.svg,vp.firstChild);vp.insertBefore(terrain.labels,terrain.svg.nextSibling);vp.__lpWorldWindow=null;return terrain.svg;}
...
const live=vp.__lpWorldReadCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||Math.abs(lpWorldWindowFor(live).s/(committed.sharpScale||committed.s)-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);};vp.__lpWorldWindowTimer=setTimeout(settle,160);}
...
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();const world=lpNearbyWorld(lpConceptWorld(),nodes,exploration.terrainWorld);exploration.terrainWorld=world;const worldMode=!!world;const GEO=worldMode?lpWorldGeometry(world):window.MkMapGeometry;modal.dataset.lpWorld=worldMode?(world.layout||"1"):"";const worldFootprint=null;modal.classList.toggle("lp-world-fog-on",!!worldFootprint);lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);try{const drawnLocs=new Set();for(const loc of nodes){if(!loc)continue;drawnLocs.add(normLoc(loc));drawnLocs.add(lpCanonKey(loc));}
...
for(let i=0;i<count;i++)extraEls.push(createNodeEl(extras[i].loc,true,{deferMeasure:true,decorate3d}));for(let i=0;i<count;i++)measureNodeEl(extras[i].loc,extraEls[i]);if(worldMode){for(let i=0;i<count;i++)focusExtra.nodes.set(extras[i].loc,extraEls[i]);return;}
...
function clearFocus(){const hadPreview=st2.__lpFocusedLoc||st2.__lpHoverLoc||st2.__lpRevealFocusLoc;cancelPreviewLeave();hideNodeAction();if(!hadPreview)return;try{const revealLoc=st2&&st2.__lpRevealFocusLoc;if(revealLoc&&lpRevealNavIsActive(revealLoc))return;}catch(_){}
...
if(st2.__lpRenderId!==renderId)return;__lpStopAllFlows();removeExtraFocusNodes(loc);st2.__lpHoverLoc=loc;const locVisual=nodeVisual.get(loc)||lpMapVisualForLoc(loc);const locRec=getMastery(loc);const locMastery=(locRec&&typeof locRec.m==="number")?locRec.m:null;const isExpandable=true;const ctx=lpMapShownDirectSets(classifyDirectSets(loc),false);const directCurEdges=lpDirectEdgesToCurrent(loc);const focusSet=new Set([loc,cur,...ctx.pres,...ctx.deps,...ctx.rels]);for(const e0 of directCurEdges){if(e0&&e0.from)focusSet.add(e0.from);if(e0&&e0.to)focusSet.add(e0.to);}
