...
if(typeof ctx.getImageData==='function'&&typeof ctx.putImageData==='function'){const image=ctx.getImageData(0,0,canvas.width,canvas.height);const hidden=(reveal.hiddenPoints||[]).map(p=>({x:(p.x-b.left)*scale,y:(p.y-b.top)*scale}));lpNearbyCloseEmptyHoles(image.data,canvas.width,canvas.height,hidden);ctx.putImageData(image,0,0);}
