...
if(vp.__lpWorldWindowTimer)clearTimeout(vp.__lpWorldWindowTimer);const settle=()=>{vp.__lpWorldWindowTimer=0;if(!vp.isConnected)return;if(lpLocalMapPointerHeld()){vp.__lpWorldWindowTimer=setTimeout(settle,160);return;}
const live=readCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||committed.gesture||Math.abs(lpWorldWindowFor(live).s/committed.s-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);};vp.__lpWorldWindowTimer=setTimeout(settle,160);}
...
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();const world=lpConceptWorld();const worldMode=false;const GEO=worldMode?lpWorldGeometry(world):window.MkMapGeometry;modal.dataset.lpWorld=worldMode?(world.layout||"1"):"";const worldFootprint=worldMode&&lpExplorationMode()?lpWorldFootprint(world):null;modal.classList.toggle("lp-world-fog-on",!!worldFootprint);lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);try{const drawnLocs=new Set();for(const loc of nodes){if(!loc)continue;drawnLocs.add(normLoc(loc));drawnLocs.add(lpCanonKey(loc));}
...
const expansionAnchor=exploration.anchor&&exploration.positions&&exploration.positions.get(exploration.anchor);if(exploration.positions&&exploration.positions.size){const fresh=nodes.filter(id=>!exploration.positions.has(id));for(const[id,point]of exploration.positions)if(pos.has(id))pos.set(id,{...point});if(expansionAnchor)fresh.forEach((id,index)=>{const angle=index*2.399963229728653;const radius=230+70*Math.sqrt(index);pos.set(id,{x:expansionAnchor.x+Math.cos(angle)*radius,y:expansionAnchor.y+Math.sin(angle)*radius});});}
const geometryInput=geometryNodes();const geometryOptions=lpMapGeometryOptions(geometryInput);let geometry=GEO.layout(geometryInput,edges.map(orientEdge),{...geometryOptions,courseGrouping:false,previous:exploration.positions,fixedIds:exploration.positions&&exploration.positions.size?Array.from(exploration.positions.keys()):[cur]});if(exploration.focusAfterExpand&&expansionAnchor&&geometry.routes.some(route=>route.status==='blocked')){const ring=new Set(getNeighbourhood(graph,exploration.anchor,1,1,'all').nodes);const movable=new Set();for(const route of geometry.routes)if(route.status==='blocked'){for(const id of[route.from,route.to])if(id!==cur&&id!==exploration.anchor&&ring.has(id)&&exploration.positions.has(id))movable.add(id);}
...
for(const e of edges)lpDrawBaseEdge(e);if(!worldMode)lpMapPaintTerritories(svg,geometry.regions,{groupMask:true,coastStep:24,simple:true});vp.insertBefore(svg,vp.firstChild);if(worldMode){lpMountWorldTerrain(modal,vp,world,W,H);lpWorldClaimsPaint(svg,world,worldFootprint);lpWorldFogPaint(modal,vp,world,worldFootprint,nodes,cur);lpWorldLabelsSync(modal,world,worldFootprint);}
...
const hardTouchGestureCleanup=(e)=>{try{const remainingTouches=e&&e.touches?Number(e.touches.length)||0:0;if(remainingTouches>0)return;if(!dragging&&!pinch&&!pendingNodeDrag&&!activePtrs.size&&!modal.hasAttribute('data-lp-gesturing'))return;activePtrs.clear();pinch=null;gestureCamera.flush(()=>applyTransform({skipClamp:true}));if(!dragging&&!pendingNodeDrag&&!st.__lpZoomDragging){try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",false);}catch(_){}
