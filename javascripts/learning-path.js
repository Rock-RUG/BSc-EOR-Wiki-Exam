...
elements.forEach((el)=>{const loc=normLoc(el.getAttribute('data-lp-loc')||'');if(!loc)return;const base=lpWebgl3dNodeBaseFromElement(el);let x=base.x;let y=base.y;let z=kind==='route'?lp3dDepthForRouteNode(el,state):lp3dDepthForLocalNode(loc,cur,level,coreSet);if(kind==='local'){const rank=localRanks.get(el)||{ordinal:0,total:1};const session=state.__lpExploration;const cloudCache=session&&(session.cloudPositions||(session.cloudPositions=new Map()));const key=lpCanonKey(loc);const cloud=(cloudCache&&cloudCache.get(key))||lpWebgl3dLocalCloudPoint(engine,loc,base,rank.ordinal,rank.total,cur,level,coreSet);if(cloudCache)cloudCache.set(key,{x:cloud.x,y:cloud.y,z:cloud.z});x=cloud.x;y=cloud.y;z=cloud.z;}
...
const svg=lpWebgl3dEnsureSvgOverlay(engine);if(!svg)return false;svg.innerHTML='';const ns=lpWebgl3dSvgNs();const defs=document.createElementNS(ns,'defs');const uid=`lp-webgl3d-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;const markerColors={prereq:'rgb(16,185,129)',dependent:'rgb(96,165,250)',route:'rgb(255,255,255)',default:'rgb(255,255,255)'};Object.keys(markerColors).forEach((k)=>{lpWebgl3dMakeArrowMarker(defs,`${uid}-${k}`,markerColors[k],k==='route'?0.86:1);const marker=defs.lastChild;if(marker&&marker.firstChild)marker.firstChild.setAttribute('data-lp-edge-marker',k==='default'?'base':k);});svg.appendChild(defs);const projected=lpWebgl3dProjectedNodes(engine);const localGeometry=engine.kind==='local'?lpMapLocalProjectedGeometry(engine,projected):null;if(localGeometry)lpMapPaintTerritories(svg,localGeometry.regions);if(engine.kind==='route'){const view=engine.modal&&engine.modal.__lpH1View;const semantic=lpH1StudyRouteSemanticScale(engine.modal,Number(view&&view.scale)||1);lpRouteContextPaint(svg,engine.contextEdges,engine.state,projected,`${uid}-context`,semantic);}
...
function lpMapAnimateTerritoryReveal(svg,nextLayer,oldLayer,center,bounds,modal){if(modal.__lpCancelTerritoryReveal)modal.__lpCancelTerritoryReveal();if(!oldLayer||!center||!bounds||lpMotionReduced()){if(oldLayer)oldLayer.remove();return;}
...
svg.insertBefore(defs,svg.firstChild);oldLayer.setAttribute('mask',`url(#${uid}-old)`);nextLayer.setAttribute('mask',`url(#${uid}-new)`);const radius=Math.max(...[bounds.left,bounds.right].flatMap(x=>[bounds.top,bounds.bottom].map(y=>Math.hypot(x-center.x,y-center.y))))+80;let frame=0,start=null,finished=false;const finish=()=>{if(finished)return;finished=true;if(frame)window.cancelAnimationFrame(frame);frame=0;nextLayer.removeAttribute('mask');oldLayer.remove();defs.remove();if(modal.__lpCancelTerritoryReveal===finish)modal.__lpCancelTerritoryReveal=null;};modal.__lpCancelTerritoryReveal=finish;const step=now=>{frame=0;if(finished)return;if(!nextLayer.isConnected||!modal.classList.contains('lp-open')||lpMotionReduced())return finish();if(start===null)start=now;const progress=Math.min(1,Math.max(0,(now-start)/720));const reach=radius*(1-Math.pow(1-progress,2));circles.forEach(circle=>circle.setAttribute('r',reach));if(progress>=1)return finish();frame=window.requestAnimationFrame(step);};frame=window.requestAnimationFrame(step);}
...
const territoryLayer=lpMapPaintTerritories(svg,geometry.regions);const reveal=exploration.pendingReveal;exploration.pendingReveal=null;if(reveal&&!modal.classList.contains('lp-webgl3d')){const oldLayer=lpMapPaintTerritories(svg,reveal.previous);const allAreas=geometry.regions.concat(reveal.previous);const left=Math.min(...allAreas.map(area=>area.bounds.left));const right=Math.max(...allAreas.map(area=>area.bounds.right));const top=Math.min(...allAreas.map(area=>area.bounds.top));const bottom=Math.max(...allAreas.map(area=>area.bounds.bottom));lpMapAnimateTerritoryReveal(svg,territoryLayer,oldLayer,reveal.center,{left,right,top,bottom,width:right-left,height:bottom-top},modal);}
...
if(exploration.territorySnapshot&&exploration.territorySnapshot.length){exploration.pendingReveal={center:{...pos.get(loc)},previous:exploration.territorySnapshot};}
