...
function lpInstallLocalMapPointerWatch(){if(window.__lpLocalMapPointerWatchInstalled)return;window.__lpLocalMapPointerWatchInstalled=true;const ids=(window.__lpLocalMapPointerIds=window.__lpLocalMapPointerIds||new Set());const touchIds=new Set();const sync=()=>{window.__lpLocalMapPointerDown=ids.size;};const down=(e)=>{try{const t=e&&e.target;if(!t||!t.closest||!t.closest("#lp-map-modal, #lp-h1sg-modal"))return;ids.add(e.pointerId==null?-1:e.pointerId);if(e.pointerType==='touch')touchIds.add(e.pointerId==null?-1:e.pointerId);sync();lpMarkLocalMapInteractionBusy(320);}catch(_){}};const up=(e)=>{try{ids.delete(e&&e.pointerId==null?-1:e.pointerId);touchIds.delete(e&&e.pointerId==null?-1:e.pointerId);sync();lpMarkLocalMapInteractionBusy(200);}catch(_){ids.clear();sync();}};try{window.addEventListener("pointerdown",down,{passive:true,capture:true});}catch(_){}
...
const touchEnd=e=>{if(e.touches&&e.touches.length)return;for(const id of touchIds)ids.delete(id);touchIds.clear();sync();};for(const type of['touchend','touchcancel'])window.addEventListener(type,touchEnd,{passive:true,capture:true});try{window.addEventListener("blur",()=>{ids.clear();touchIds.clear();sync();},{passive:true});}catch(_){}}
function lpLocalMapInteractionIsBusy(modal){try{const root=modal||document.getElementById("lp-map-modal");if(!root||!root.classList||!root.classList.contains("lp-open"))return false;if(lpLocalMapPointerHeld())return true;if(lpLocalMapInteractionNow()<(Number(window.__lpLocalMapInteractionBusyUntil)||0))return true;if(root.hasAttribute("data-lp-expanding")||root.hasAttribute("data-lp-dragging")||root.hasAttribute("data-lp-slider-zooming")||root.hasAttribute("data-lp-gesturing")||root.classList.contains("lp-webgl3d-dragging")||root.classList.contains("lp-route-animating"))return true;const state=window.__lpMapState;return!!(state&&state.__lpZoomDragging);}catch(_){return false;}}
...
function lp3dMapAccessMessage(){const next='Open Account → Store to check purchase and trial options.';const api=lp3dShopApi();if(!api)return'Account is not ready to check 3D access yet. '+next;const locked='3D Local Map requires ownership or an active trial. ';try{const pricing=typeof api.shopItemPricing==='function'?api.shopItemPricing(LP_3D_LOCAL_MAP_ITEM_ID):null;const price=pricing&&pricing.final!=null?Number(pricing.final):NaN;if(!Number.isFinite(price)||price<0)return locked+next;const number=value=>Number(value).toLocaleString('en',{maximumFractionDigits:2});let message=locked+`Current Store price: ${number(price)} EORbits. `;const account=window.MkLocalActivity;const currency=account&&typeof account.currencySummary==='function'?account.currencySummary():null;const rawBalance=currency&&currency.balance;const balance=rawBalance!=null?Number(rawBalance):NaN;if(Number.isFinite(balance)&&balance>=0){message+=`Your balance: ${number(balance)} EORbits. `;if(balance<price)message+=`You need ${number(Math.ceil((price - balance) * 100) / 100)} more. `;}
return message+next;}catch(_){return locked+next;}}
function lp3dSyncAccessFeedback(modal,show){if(!modal||!modal.querySelector)return;const btn=modal.querySelector('[data-lp-view-mode="3d"]');let note=modal.querySelector('[data-lp-map-access-status]');if(lp3dOwnsLocalMapItem()){if(note){note.hidden=true;note.textContent='';}
if(btn){btn.title='Use the rotatable WebGL 3D map';btn.setAttribute('aria-label',btn.title);}
return;}
const message=lp3dMapAccessMessage();if(btn){btn.title=message;btn.setAttribute('aria-label',message);}
if(!show&&(!note||note.hidden))return;if(!note){const host=modal.querySelector('.lp-map-context-copy');if(!host)return;note=document.createElement('span');note.className='lp-map-access-status';note.setAttribute('data-lp-map-access-status','');host.appendChild(note);}
note.textContent=message;note.hidden=false;}
function lp3dInstallViewToggle(modal,kind){if(!modal||!modal.querySelector)return;if(!LP_3D_MAP_ENABLED){try{modal.querySelectorAll('.lp-view-toggle').forEach((wrap)=>wrap.remove());}catch(_){}
return;}
lp3dEnsureStyles();try{const box=modal.querySelector('.lp-mbox')||modal;let wrap=modal.querySelector('.lp-view-toggle');if(!wrap){wrap=document.createElement('div');wrap.className='lp-view-toggle';wrap.setAttribute('aria-label','Switch map view');wrap.innerHTML=`
        ...
        `;box.appendChild(wrap);const stop=(e)=>{try{if(e&&e.cancelable)e.preventDefault();if(e&&e.stopPropagation)e.stopPropagation();if(e&&typeof e.stopImmediatePropagation==='function')e.stopImmediatePropagation();}catch(_){}};wrap.addEventListener('click',(e)=>{const btn=e&&e.target&&e.target.closest?e.target.closest('[data-lp-view-mode]'):null;if(!btn)return;stop(e);const next=btn.getAttribute('data-lp-view-mode')==='2d'?'2d':'3d';if(next==='3d'&&!lp3dOwnsLocalMapItem()){lp3dSyncAccessFeedback(modal,true);return;}
const note=modal.querySelector('[data-lp-map-access-status]');if(note){note.hidden=true;note.textContent='';}
...
elements.forEach((el)=>{const loc=normLoc(el.getAttribute('data-lp-loc')||'');if(!loc)return;const base=lpWebgl3dNodeBaseFromElement(el);let x=base.x;let y=base.y;let z=kind==='route'?lp3dDepthForRouteNode(el,state):lp3dDepthForLocalNode(loc,cur,level,coreSet);if(kind==='local'){const rank=localRanks.get(el)||{ordinal:0,total:1};const session=state.__lpExploration;const cloudCache=session&&(session.cloudPositions||(session.cloudPositions=new Map()));const key=lpCanonKey(loc);const atlasSource=session&&session.positions&&session.positions.size&&Number.isFinite(base.x)&&Number.isFinite(base.y)?[base.x,base.y,Number(engine.W)||0,Number(engine.H)||0].join('|'):'';const cached=cloudCache&&cloudCache.get(key);const cloud=cached&&(!atlasSource||cached.atlasSource===atlasSource)?cached:lpWebgl3dLocalCloudPoint(engine,loc,base,rank.ordinal,rank.total,cur,level,coreSet);if(cloudCache)cloudCache.set(key,{x:cloud.x,y:cloud.y,z:cloud.z,...(atlasSource?{atlasSource}:{})});x=cloud.x;y=cloud.y;z=cloud.z;}
...
function lpMapPaintEdge(path,role,type,addCasing){if(!path||!path.setAttribute)return;path.setAttribute('data-lp-edge-role',role);path.setAttribute('data-lp-edge-type',type);if(!addCasing||role==='base'||type==='related'||!path.parentNode||path.__lpStaticCasing)return;if(path.classList.contains('lp-is-hidden')||path.style.opacity==='0')return;const casing=document.createElementNS('http://www.w3.org/2000/svg','path');casing.classList.add('lp-map-edge-casing');casing.setAttribute('data-lp-edge-decoration','casing');casing.setAttribute('data-lp-edge-role',role);casing.setAttribute('data-lp-edge-type',type);casing.setAttribute('aria-hidden','true');casing.setAttribute('pointer-events','none');casing.setAttribute('fill','none');['d','mask','clip-path','transform','pathLength','stroke-dasharray','stroke-dashoffset','opacity'].forEach((name)=>{const value=path.getAttribute(name);if(value!==null)casing.setAttribute(name,value);});['opacity','strokeDasharray','strokeDashoffset','visibility','display'].forEach((name)=>{if(path.style[name])casing.style[name]=path.style[name];});const width=parseFloat(path.style.strokeWidth||path.getAttribute('stroke-width'))||2.3;casing.setAttribute('stroke','var(--wiki-map-edge-casing, rgba(5,12,28,.8))');casing.setAttribute('stroke-width',String(width+3));casing.setAttribute('stroke-linecap','round');casing.setAttribute('stroke-linejoin','round');path.parentNode.insertBefore(casing,path);path.__lpStaticCasing=casing;}
function lpRouteContextModel(world,state){if(!world||!state||!state.graph)return null;const graph=state.graph;const cached=state.__lpContextModel;if(cached&&cached.world===world&&cached.graph===graph&&cached.prereqOf===graph.prereqOf&&cached.positions===state.positions)return cached;const nodes=new Map();Array.from(world.querySelectorAll('.lp-node[data-lp-loc]')).forEach(el=>{const loc=normLoc(el.getAttribute('data-lp-loc')||'');const key=lpCanonKey(loc);if(key&&!nodes.has(key))nodes.set(key,{loc,key,el});});const seen=new Set(),incident=[],rest=[];const target=lpCanonKey(state.target||'');for(const dependent of nodes.values()){for(const raw of getPrereqs(graph,dependent.loc)){const fromKey=lpCanonKey(raw),toKey=dependent.key;if(!fromKey||fromKey===toKey||!nodes.has(fromKey))continue;const key=`${fromKey}>${toKey}`;if(seen.has(key))continue;seen.add(key);const edge={from:nodes.get(fromKey).loc,to:dependent.loc,fromKey,toKey,key};(fromKey===target||toKey===target?incident:rest).push(edge);}}
const model={world,graph,prereqOf:graph.prereqOf,positions:state.positions,nodes,edges:incident.concat(rest),shown:0,eligible:0};state.__lpContextModel=model;return model;}
function lpRouteContextPaint(svg,model,state,points,markerId,labelScale=1){if(!svg||!model||!model.edges.length||!points)return;const ns='http://www.w3.org/2000/svg';const group=document.createElementNS(ns,'g');group.setAttribute('class','lp-route-context-layer');group.setAttribute('data-lp-edge-decoration','context');group.setAttribute('aria-hidden','true');group.setAttribute('pointer-events','none');const defs=document.createElementNS(ns,'defs');lpWebgl3dMakeArrowMarker(defs,markerId,'var(--wiki-map-edge-context, #aabed0)',1);if(defs.lastChild&&defs.lastChild.firstChild)defs.lastChild.firstChild.setAttribute('data-lp-edge-marker','context');group.appendChild(defs);const path=Array.isArray(state&&state.path)?state.path:[];const selected=new Set();for(let i=0;i<path.length-1;i++)selected.add(`${lpCanonKey(path[i])}>${lpCanonKey(path[i + 1])}`);const available=new Set();for(const[key,node]of model.nodes){const el=node.el;if(points.has(key)&&!el.hidden&&!el.classList.contains('is-filter-faded'))available.add(key);}
let eligible=0,shown=0;for(const edge of model.edges){if(selected.has(edge.key)||!available.has(edge.fromKey)||!available.has(edge.toKey))continue;const rawA=points.get(edge.fromKey),rawB=points.get(edge.toKey);const A=labelScale===1?rawA:{x:rawA.x,y:rawA.y,w:rawA.w*labelScale,h:rawA.h*labelScale};const B=labelScale===1?rawB:{x:rawB.x,y:rawB.y,w:rawB.w*labelScale,h:rawB.h*labelScale};const p1=lpWebgl3dClipPillEdge(A,B,7),p2=lpWebgl3dClipPillEdge(B,A,7);const distance=Math.hypot(p2.x-p1.x,p2.y-p1.y);if(distance<8)continue;eligible++;if(shown>=512)continue;const line=document.createElementNS(ns,'path');line.setAttribute('class','lp-route-context-edge');line.setAttribute('data-lp-edge-role','context');line.setAttribute('data-lp-edge-type','context');line.setAttribute('data-lp-context-from',edge.from);line.setAttribute('data-lp-context-to',edge.to);const bend=(lpWebgl3dHash01(edge.key)<.5?-1:1)*12*Math.min(1,distance/260);line.setAttribute('d',lpWebgl3dCurvedD(p1,p2,bend));line.setAttribute('fill','none');line.setAttribute('stroke','var(--wiki-map-edge-context, #aabed0)');line.setAttribute('stroke-width','1.15');line.setAttribute('opacity',path.length>1?'.16':'.38');line.setAttribute('stroke-linecap','round');line.setAttribute('stroke-linejoin','round');line.setAttribute('marker-end',`url(#${markerId})`);group.appendChild(line);shown++;}
model.shown=shown;model.eligible=eligible;if(shown)svg.appendChild(group);}
function lpRouteContextPaint2d(svg,world,state){const model=lpRouteContextModel(world,state);if(!model)return;const points=new Map();const modal=world.closest&&world.closest('#lp-h1sg-modal');const view=modal&&modal.__lpH1View;const semantic=lpH1StudyRouteSemanticScale(modal,Number(view&&view.scale)||1);for(const[key,node]of model.nodes){const xy=state.positions&&state.positions.get(node.loc);if(!xy)continue;points.set(key,{x:xy.x,y:xy.y,w:(Number(node.el.offsetWidth)||120)*semantic,h:(Number(node.el.offsetHeight)||36)*semantic});}
if(!svg.querySelector('.lp-map-route-paper')&&!(modal&&modal.dataset&&modal.dataset.lpWorld))lpMapPaintRouteSurface(svg,points.values());lpRouteContextPaint(svg,model,state,points,`lp-route-context-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);}
function lpRouteContextRefresh2d(modal){const state=modal&&modal.__lpH1StudyState;if(!state||(state.path&&state.path.length>1)||modal.classList.contains('lp-webgl3d'))return;const world=modal.__lpH1Viewport;const svg=world&&world.querySelector('.lp-h1sg-overlay');if(!svg)return;svg.querySelectorAll('.lp-route-context-layer').forEach(layer=>layer.remove());lpRouteContextPaint2d(svg,world,state);lpMapSyncRouteContext(modal);}
function lpMapLocalProjectedGeometry(engine,projected){if(!window.MkMapGeometry||engine.kind!=='local')return null;const input=Array.from(projected.values(),p=>({id:p.key,x:p.x,y:p.y,width:p.w,height:p.h,course:lpMapCourseKey(p.loc)}));const edges=(engine.edges||[]).map(e=>({from:lpCanonKey(e.from),to:lpCanonKey(e.to),type:e.type}));const retained=new Set(((engine.ctx&&engine.ctx.baseNodes)||[]).map(lpCanonKey));const signature=input.map(n=>`${n.id}:${n.x.toFixed(1)},${n.y.toFixed(1)},${n.width.toFixed(1)},${n.height.toFixed(1)}`).join('|')+edges.map(e=>`${e.from}>${e.to}:${e.type}`).join('|')+'#retained:'+Array.from(retained).sort().join('|');const cached=engine.__lpProjectedGeometry;if(cached&&cached.signature===signature&&!cached.transient)return cached;const moving=!!(engine.modal&&engine.modal.classList.contains('lp-webgl3d-dragging'))||!!(engine.state&&engine.state.dragging);const options=lpMapGeometryOptions(input);const full=!moving&&(!cached||engine.__lpGeometrySettledSignature===signature);const routed=window.MkMapGeometry.route(input,edges,{...options,directOnly:!full});const regionInput=retained.size?input.map(node=>retained.has(node.id)?node:{...node,course:''}):input;const areas=full?window.MkMapGeometry.regions(regionInput,options).regions:[];const result={signature,routes:routed.routes,regions:areas,transient:!full};engine.__lpProjectedGeometry=result;if(!full&&engine.__lpGeometryScheduledSignature!==signature){if(engine.__lpGeometryTimer)window.clearTimeout(engine.__lpGeometryTimer);engine.__lpGeometryScheduledSignature=signature;const ticket=window.setTimeout(()=>{if(engine.__lpGeometryTimer!==ticket||engine.__lpGeometryScheduledSignature!==signature)return;engine.__lpGeometryTimer=0;engine.__lpGeometryScheduledSignature='';if((engine.modal&&engine.modal.classList.contains('lp-webgl3d-dragging'))||(engine.state&&engine.state.dragging))return;if(!engine.__observing||!engine.modal||engine.modal.__lpWebgl3dEngine!==engine||!engine.modal.classList.contains('lp-open'))return;engine.__lpGeometrySettledSignature=signature;engine.__lpGeometryScheduledSignature='';lpWebgl3dDraw(engine);},120);engine.__lpGeometryTimer=ticket;}
return result;}
function lpMapPaintProjectedTerritories(engine,svg,geometry,projected){if(!engine||engine.kind!=='local'||!geometry)return;const modal=engine.modal;const open=!!(engine.__observing&&modal&&modal.__lpWebgl3dEngine===engine&&modal.classList.contains('lp-open')&&svg.isConnected!==false);const previousPaint=engine.__lpProjectedTerritoryPaint;if(!open){if(engine.__lpDisposeProjectedTerritory)engine.__lpDisposeProjectedTerritory();return;}
if(!engine.__lpDisposeProjectedTerritory)engine.__lpDisposeProjectedTerritory=()=>{const paint=engine.__lpProjectedTerritoryPaint;if(paint)paint.layer.remove();engine.__lpProjectedTerritoryPaint=null;engine.__lpDisposeProjectedTerritory=null;};if(previousPaint&&previousPaint.signature===geometry.signature&&!geometry.transient&&previousPaint.layer.parentNode===svg)return;if(previousPaint)previousPaint.layer.remove();engine.__lpProjectedTerritoryPaint=null;if(geometry.transient)return;const field=document.createElementNS('http://www.w3.org/2000/svg','g');field.classList.add('lp-webgl3d-territory-field');field.setAttribute('aria-hidden','true');field.setAttribute('pointer-events','none');svg.insertBefore(field,svg.firstChild);const next=lpMapPaintTerritories(field,geometry.regions);lpMapWorldSurfaceProjection(next,engine);engine.__lpProjectedTerritoryPaint={signature:geometry.signature,layer:field};}
function lpMapSeparateProjectedLabels(engine){if(engine.kind!=='local'||!window.MkMapGeometry)return;const projected=lpWebgl3dProjectedNodes(engine);const input=Array.from(projected.values(),p=>({id:p.key,x:p.x,y:p.y,width:p.w,height:p.h,course:lpMapCourseKey(p.loc)}));const placementOptions={...lpMapGeometryOptions(input),edges:(engine.edges||[]).map(edge=>({from:lpCanonKey(edge.from),to:lpCanonKey(edge.to),type:edge.type})),fixedIds:[lpCanonKey(engine.ctx&&engine.ctx.cur)]};let placed=window.MkMapGeometry.place(input,placementOptions);if(placed.stats.straightRemaining>0){const retry=window.MkMapGeometry.place(placed.nodes,{...placementOptions,previous:placed.nodes,maxStraightCandidates:4096});if(retry.stats.straightRemaining<placed.stats.straightRemaining)placed=retry;}
for(const point of placed.nodes){const p=projected.get(point.id);if(!p||!p.node||!p.node.el)continue;const el=p.node.el;el.__lpWebglScreenX=point.x;el.__lpWebglScreenY=point.y;el.style.left=`${point.x}px`;el.style.top=`${point.y}px`;}}
...
try{engine.modal?.__lpMapChests?.updateLayout();}catch(_){}}
...
function lpWebgl3dObserve(engine){if(!engine||!engine.viewport||engine.__observing)return;engine.__observing=true;let timer=0;let disposed=false;const schedule=()=>{if(disposed||timer)return;timer=window.setTimeout(()=>{timer=0;try{if(!disposed&&engine.modal&&engine.modal.__lpWebgl3dEngine===engine)lpWebgl3dRebuild(engine);}catch(_){}},80);};engine.__acknowledgeInputRebuild=()=>{if(disposed)return;if(timer)window.clearTimeout(timer);timer=0;try{if(engine.__mo)engine.__mo.takeRecords();}catch(_){}};engine.__disposeLifecycle=()=>{if(disposed)return;disposed=true;try{if(engine.__lpDisposeProjectedTerritory)engine.__lpDisposeProjectedTerritory();}catch(_){}
try{if(engine.__cancelPointerDrag)engine.__cancelPointerDrag();}catch(_){}
if(timer)window.clearTimeout(timer);if(engine.__lpGeometryTimer)window.clearTimeout(engine.__lpGeometryTimer);engine.__lpGeometryTimer=0;timer=0;try{if(engine.__mo)engine.__mo.disconnect();}catch(_){}
...
engine[key]=0;});engine.__observing=false;engine.__schedule=null;engine.__acknowledgeInputRebuild=null;};try{const mo=new MutationObserver((records)=>{const relevant=(records||[]).some((record)=>{const target=record&&record.target;if(target&&target.nodeType===1&&target.closest&&target.closest('.lp-webgl3d-overlay, .lp-map-presentation-svg'))return false;if(record&&record.type==='childList'){const changed=[...(record.addedNodes||[]),...(record.removedNodes||[])];if(changed.length&&changed.every(node=>node&&node.nodeType===1&&node.matches&&(node.matches('svg.lp-map-presentation-svg')||node.matches('path.lp-map-edge-casing[data-lp-edge-decoration="casing"]'))))return false;}
return true;});if(relevant)schedule();});mo.observe(engine.viewport,{childList:true,subtree:true,attributes:true,attributeFilter:['data-lp-visual','data-lp-unvisited','data-lp-rel-only','data-lp-loc']});if(document.documentElement)mo.observe(document.documentElement,{attributes:true,attributeFilter:['data-mk-map-style']});engine.__mo=mo;}catch(_){}
...
const __mkFetchJsonShared=__mkInstallSharedJsonFetch();function __mkRunIdle(task,timeoutMs){const fn=typeof task==="function"?task:function(){};const timeout=Math.max(0,Number(timeoutMs)||0);if(typeof window.__mkRunWhenInteractionIdle==="function"){return window.__mkRunWhenInteractionIdle(function(){try{fn();}catch(_){}},timeout);}
try{if(typeof window.requestIdleCallback==="function"){return window.requestIdleCallback(()=>{try{fn();}catch(_){}},timeout?{timeout}:undefined);}}catch(_){}
...
return st;}
function lpLimitDependentsForMap(graph,loc){return uniq(getDependents(graph,loc)).sort((a,b)=>{const ka=lpCanonKey(a),kb=lpCanonKey(b);return ka<kb?-1:ka>kb?1:0;}).slice(0,10);}
function lpExplorationMode(){return lpFogEnabled();}
function lpConceptExploreState(loc){const rec=getMastery(loc);const m=(rec&&typeof rec.m==="number")?rec.m:null;const rated=lpHasExplicitMastery(rec,m);const visited=lpWasVisited(loc);return{visited,rated,explored:visited&&rated,claimed:visited&&rated&&m>=2};}
function lpMapExpandGate(loc){if(lpCanonKey(loc)===lpCanonKey(currentRelPath()))return"";const state=lpConceptExploreState(loc);if(state.explored)return"";return state.visited?"rate":"visit";}
try{localStorage.removeItem("lp_map_keep_expansions_v1");localStorage.removeItem("lp_map_kept_expansions_v1");}catch(_){}
...
const __lpNodeTitleTypesetQueue=new Set();let __lpNodeTitleTypesetDraining=false;function lpMapGestureActive(){try{const roots=[document.getElementById("lp-map-modal"),document.getElementById("lp-h1sg-modal")];for(const root of roots){if(!root||!root.classList||!root.classList.contains("lp-open"))continue;if(lpLocalMapPointerHeld())return true;if(root.hasAttribute("data-lp-dragging")||root.hasAttribute("data-lp-slider-zooming")||root.hasAttribute("data-lp-gesturing")||root.classList.contains("lp-webgl3d-dragging")||root.classList.contains("lp-route-animating"))return true;const state=window.__lpMapState;if(state&&state.__lpZoomDragging)return true;}
...
#lp-side-panel{ font-size:max(13px, .65rem); }
...
#lp-side-panel .lp-head .lp-title{
  ...
  white-space:nowrap;
      ...
      #lp-side-panel .lp-empty{ opacity:1; font-size:1em; line-height:1.5; padding:.35rem .2rem; }
...
@media (max-width: 768px){
  #lp-map-modal .lp-mzoom{
    ...
    width: min(640px, calc(100% - 24px));
      ...
      #lp-map-modal.lp-opera-map-perf[data-lp-gesturing],
      #lp-map-modal.lp-opera-map-perf[data-lp-slider-zooming]{
        backdrop-filter:none !important;
        -webkit-backdrop-filter:none !important;
      }
      ...
      #lp-map-modal.lp-opera-map-perf[data-lp-world=""][data-lp-gesturing] .lp-mapviewport svg,
      #lp-map-modal.lp-opera-map-perf[data-lp-world=""][data-lp-slider-zooming] .lp-mapviewport svg{
        display:none !important;
      }
      #lp-map-modal.lp-opera-map-perf[data-lp-gesturing] a.lp-node::before,
      #lp-map-modal.lp-opera-map-perf[data-lp-slider-zooming] a.lp-node::before{
        display:none !important;
      }
      #lp-map-modal.lp-opera-map-perf[data-lp-gesturing] a.lp-node::after,
      #lp-map-modal.lp-opera-map-perf[data-lp-slider-zooming] a.lp-node::after{
        box-shadow:none !important;
        background:rgba(255,255,255,.055) !important;
      }
      /* The pill's own three elements, not "a.lp-node *": a trailing universal
         selector makes every toggle of lp-mobile-gesturing restyle the whole
         modal, on every device, not only under this Opera fallback. */
      #lp-map-modal.lp-opera-map-perf[data-lp-gesturing] a.lp-node,
      #lp-map-modal.lp-opera-map-perf[data-lp-gesturing] a.lp-node :is(.lp-node-label, .lp-readiness-badge, .lp-node-title),
      #lp-map-modal.lp-opera-map-perf[data-lp-slider-zooming] a.lp-node,
      #lp-map-modal.lp-opera-map-perf[data-lp-slider-zooming] a.lp-node :is(.lp-node-label, .lp-readiness-badge, .lp-node-title){
        transition:none !important;
        animation:none !important;
        filter:none !important;
        -webkit-filter:none !important;
...
function __lpEnsureRelatedFromBody(graph,loc,onUpdate){try{if(!graph)return Promise.resolve([]);const key=normLoc(loc);if(!key||!isConceptPage(key))return Promise.resolve([]);const cached=__lpGetRelatedCache(graph,key);if(cached)return Promise.resolve(cached);if(graph.related&&Array.isArray(graph.related[key]))return Promise.resolve(graph.related[key]);if(!graph.__lpRelatedFetchPromises)graph.__lpRelatedFetchPromises=new Map();if(graph.__lpRelatedFetchPromises.has(key))return graph.__lpRelatedFetchPromises.get(key);const url=toAbsolutePageUrl(key);const p=__lpRunRelatedFetch(()=>__lpFetchRelatedPageHtml(url).then(async(html2)=>{await lpWaitForLocalMapInteractionIdle(15000);await new Promise((resolve)=>__mkRunIdle(resolve,1200));try{const doc=new DOMParser().parseFromString(String(html2||""),"text/html");const h1=doc.querySelector("article.md-content__inner h1")||doc.querySelector("h1");const h1t=cleanTitle(h1?(h1.textContent||""):"");const h1html=lpExtractRenderableTitleHtmlFromHeading(h1);if(h1t){__lpSetTitleCache(graph,key,h1t,true);__lpForceGraphNodeTitle(graph,key,h1t);}
...
const LP_MAP_PAN_KEEP_SHARE=0.35;const LP_MAP_PAN_KEEP_MIN=64;function lpMapPanKeep(scaledSpan,stageSpan){const span=Math.min(Number(scaledSpan)||0,Number(stageSpan)||0);if(!(span>0))return 0;return Math.min(span,Math.max(LP_MAP_PAN_KEEP_MIN,span*LP_MAP_PAN_KEEP_SHARE));}
function lpMapFitCamera(records,area,minScale,maxScale,semanticScale){const rows=(records||[]).filter((r)=>r&&[r.x,r.y,r.w,r.h].every(Number.isFinite)&&r.w>0&&r.h>0);if(!rows.length||!area||!(area.width>0)||!(area.height>0)||!(minScale>0)||!(maxScale>=minScale))return null;const boundsAt=(scale)=>{const semantic=Math.max(1,Number(semanticScale&&semanticScale(scale))||1);let left=Infinity,top=Infinity,right=-Infinity,bottom=-Infinity;rows.forEach((r)=>{left=Math.min(left,r.x-r.w*semantic/2);right=Math.max(right,r.x+r.w*semantic/2);top=Math.min(top,r.y-r.h*semantic/2);bottom=Math.max(bottom,r.y+r.h*semantic/2);});return{left,top,right,bottom};};const fits=(scale)=>{const b=boundsAt(scale);return(b.right-b.left)*scale<=area.width&&(b.bottom-b.top)*scale<=area.height;};const limited=!fits(minScale);let low=minScale,high=maxScale;if(!limited&&fits(high))low=high;else if(!limited){for(let i=0;i<18;i++){const mid=(low+high)/2;if(fits(mid))low=mid;else high=mid;}}
const bounds=boundsAt(low);return{scale:low,tx:area.left+area.width/2-(bounds.left+bounds.right)*low/2,ty:area.top+area.height/2-(bounds.top+bounds.bottom)*low/2,limited,count:rows.length};}
function lpMapFitArea(modal,route,view){const stage=modal&&modal.querySelector('.lp-mapstage');if(!stage)return null;const rect=stage.getBoundingClientRect();const width=Number(stage.clientWidth)||Number(rect.width)||0;const height=Number(stage.clientHeight)||Number(rect.height)||0;if(!(width>0)||!(height>0))return null;if(route){view.stageW=width;view.stageH=height;}
else{view.__lpStageW=width;view.__lpStageH=height;}
const safe=route?lpH1StudyStartSafeInsets(modal):lpLocalMapSafeInsets(modal,width,height);let bottom=Math.max(24,Number(safe.bottom)||0);const launch=route&&modal.querySelector('[data-lp-h1sg-dock-launch]');if(launch&&!launch.hidden){const launchRect=launch.getBoundingClientRect();if(launchRect.height>0&&launchRect.top<rect.bottom)bottom=Math.max(bottom,rect.bottom-launchRect.top+12);}
const left=Math.max(24,Number(safe.left)||0)+12;const top=Math.max(24,Number(safe.top)||0)+12;const right=Math.max(24,Number(safe.right)||0)+12;return{left,top,width:Math.max(1,width-left-right),height:Math.max(1,height-top-bottom-12)};}
function lpMapFitRecords(modal,viewport,view,route,path){const engine=modal.classList.contains('lp-webgl3d')&&modal.__lpWebgl3dEngine;if(engine)lpWebgl3dFlushRotation(engine);const stageRect=modal.querySelector('.lp-mapstage').getBoundingClientRect();const scale=Math.max(.001,Number(view.scale)||1);const semantic=route?lpH1StudyRouteSemanticScale(modal,scale):1;const selected=path&&path.length>1?new Set(path.map(lpCanonKey)):null;const nodes=Array.from(viewport.querySelectorAll('.lp-node[data-lp-loc]')).filter((node)=>!selected||selected.has(lpCanonKey(node.getAttribute('data-lp-loc')||'')));const culled=nodes.filter((node)=>node.classList.contains('lp-pan-culled'));const windowed=nodes.filter((node)=>node.classList.contains('lp-node-offscreen'));culled.forEach((node)=>node.classList.remove('lp-pan-culled'));windowed.forEach((node)=>node.classList.remove('lp-node-offscreen'));try{return nodes.map((node)=>{const key=lpCanonKey(node.getAttribute('data-lp-loc')||'');const rect=node.getBoundingClientRect();if(!(rect.width>0)||!(rect.height>0))return null;const projectedNode=engine&&engine.nodeMap&&engine.nodeMap.get(key);const point=projectedNode?lpWebgl3dProject(engine,projectedNode):null;const x=point?point.x:(rect.left-stageRect.left+rect.width/2-(Number(view.tx)||0))/scale;const y=point?point.y:(rect.top-stageRect.top+rect.height/2-(Number(view.ty)||0))/scale;return{loc:key,x,y,w:rect.width/scale/semantic,h:rect.height/scale/semantic};}).filter(Boolean);}finally{culled.forEach((node)=>node.classList.add('lp-pan-culled'));windowed.forEach((node)=>node.classList.add('lp-node-offscreen'));}}
function lpMapFitView(modal,options){if(!modal||!modal.classList.contains('lp-open'))return false;const route=modal.id==='lp-h1sg-modal';const viewport=route?lpH1StudyStartViewport(modal):modal.querySelector('[data-map-viewport]');const view=route?lpH1StudyStartEnsureView(modal):window.__lpMapState;if(!viewport||!view||(route&&modal.__lpH1LaunchBusy))return false;if(typeof modal.__lpStopZoomGesture==='function')modal.__lpStopZoomGesture();const state=route&&modal.__lpH1StudyState;const path=state&&Array.isArray(state.path)?state.path:[];if(route){lpH1StudyRouteAnimStop(modal);if(path.length>1)lpH1StudyStartRenderSelection(viewport,state,{staticView:true});}
const records=lpMapFitRecords(modal,viewport,view,route,path);const minScale=lpMapMinActualScale();const maxScale=Math.min(lpMapMaxActualScale(),lpMapDefaultActualScale());const semantic=route?(scale)=>lpH1StudyRouteSemanticScale(modal,scale):null;let fit=lpMapFitCamera(records,lpMapFitArea(modal,route,view),minScale,maxScale,semantic);if(!fit)return false;const phoneSized=lpMapUsesMobileVisualScale()||!!(window.matchMedia&&window.matchMedia('(max-width: 600px)').matches);const target=options&&options.initial&&route&&phoneSized&&path.length<2&&lpMapDisplayPctFromScale(fit.scale)<70&&state&&records.find(row=>row.loc===lpCanonKey(state.target||''));const status=modal.querySelector('[data-lp-map-fit-status]');if(status){const initiallyLimited=fit.limited;status.hidden=false;status.textContent=target?'Current concept view. Zoom out for the overview.':fit.limited?`Minimum zoom (${lpMapDisplayPctFromScale(minScale)}%). Drag to explore the rest.`:(path.length>1?'Selected path fitted.':'Map fitted.');if(!target)fit=lpMapFitCamera(records,lpMapFitArea(modal,route,view),minScale,maxScale,semantic)||fit;if(!target&&fit.limited&&!initiallyLimited){status.textContent=`Minimum zoom (${lpMapDisplayPctFromScale(minScale)}%). Drag to explore the rest.`;fit=lpMapFitCamera(records,lpMapFitArea(modal,route,view),minScale,maxScale,semantic)||fit;}}
if(target){const area=lpMapFitArea(modal,route,view);const scale=Math.min(maxScale,Math.max(minScale,lpMapDefaultActualScale()*.7));fit={...fit,scale,tx:area.left+area.width/2-target.x*scale,ty:area.top+area.height/2-target.y*scale};}
view.scale=fit.scale;view.tx=fit.tx;view.ty=fit.ty;view.userMoved=true;if(route){view.forceCenter=false;modal.__lpH1PreserveViewUntil=Date.now()+900;lpH1StudyStartApplyTransform(modal,{skipClamp:true});lpH1StudyStartCullForDrag(modal,true,true);if(path.length<2)lpRouteContextRefresh2d(modal);}else{view.__lpForceCenter=false;if(typeof modal.__lpApplyTransform==='function')modal.__lpApplyTransform({skipClamp:true});}
return true;}
function lpMapScheduleInitialOverview(modal,state){const previous=modal&&modal.__lpInitialOverview;if(previous&&typeof previous.cancel==='function')previous.cancel();if(!modal||!state||!modal.classList.contains('lp-open'))return false;const view=lpH1StudyStartEnsureView(modal);const viewport=lpH1StudyStartViewport(modal);const stage=lpH1StudyStartStage(modal);const untouched=()=>modal.__lpH1StudyState===state&&modal.__lpH1View===view&&!view.userMoved&&!(state.path&&state.path.length)&&!state.query&&!modal.__lpH1LaunchBusy&&!(modal.__lpH1RouteAnim&&!modal.__lpH1RouteAnim.cancelled);if(!viewport||!stage||!untouched()||!state.positions||state.positions.size<2)return false;const width=stage.clientWidth,height=stage.clientHeight;if(!(width>0&&height>0))return false;const is3d=modal.classList.contains('lp-webgl3d');const engine=is3d?modal.__lpWebgl3dEngine:null;const events=['pointerdown','touchstart','wheel','keydown','input'];let frame=0,stopped=false;const ticket={cancel};function cancel(){if(stopped)return;stopped=true;if(frame)cancelAnimationFrame(frame);events.forEach(name=>modal.removeEventListener(name,cancel,true));if(modal.__lpInitialOverview===ticket)modal.__lpInitialOverview=null;}
modal.__lpInitialOverview=ticket;events.forEach(name=>modal.addEventListener(name,cancel,{capture:true,passive:true}));frame=requestAnimationFrame(()=>{frame=0;const current=!stopped&&modal.__lpInitialOverview===ticket&&modal.classList.contains('lp-open')&&untouched()&&lpH1StudyStartViewport(modal)===viewport&&lpH1StudyStartStage(modal)===stage&&stage.clientWidth===width&&stage.clientHeight===height&&modal.classList.contains('lp-webgl3d')===is3d&&(!is3d||(modal.__lpWebgl3dEngine===engine&&engine&&engine.viewport===viewport&&engine.nodeMap&&engine.nodeMap.size));cancel();if(current)lpMapFitView(modal,{initial:true});});return true;}
function lpMapSyncRouteContext(modal){const state=modal&&modal.__lpH1StudyState;const copy=modal&&modal.querySelector('[data-lp-map-context]');const endpoints=modal&&modal.querySelector('[data-lp-map-route-endpoints]');if(!copy||!endpoints)return;const path=state&&Array.isArray(state.path)?state.path:[];const hasPath=path.length>1;const model=state&&state.__lpContextModel;const budgetNote=model&&model.eligible>model.shown?` Showing ${model.shown} of ${model.eligible} connections.`:'';const text=(hasPath?`${path.length} concepts · Start guided study to begin.`:'Select a node to show a learning path.')+budgetNote;if(copy.textContent!==text)copy.textContent=text;const legend=modal.querySelector('.lp-map-flow-key');if(legend){const selectedKey=legend.querySelector&&legend.querySelector('.lp-map-key-selected');if(selectedKey)selectedKey.hidden=!hasPath;legend.setAttribute('aria-label',hasPath?'Thin arrows show prerequisite connections. The highlighted selected path runs from Start to Goal.':'Thin arrows show prerequisite connections, from prerequisite to dependent. Select a node to choose a learning path.');}
endpoints.hidden=!hasPath;const visibleTitle=(loc)=>{const node=lpH1StudyStartFindNodeEl(modal,loc);if(!node||lpH1StudyRouteNodeMasked(node,state)||node.classList.contains('is-route-revealing'))return'Unknown concept';const title=node.querySelector('.lp-node-title');const text=String(title&&(title.innerText||title.textContent)||'').replace(/\s+/g,' ').trim();return text.length>72?text.slice(0,69)+'…':(text||'Concept');};const labels=hasPath?`Start: ${visibleTitle(path[0])} → Goal: ${visibleTitle(path[path.length - 1])}`:'';if(endpoints.textContent!==labels){endpoints.textContent=labels;endpoints.title=labels;}
const fit=modal.querySelector('[data-lp-map-fit]');if(fit){fit.setAttribute('aria-label',hasPath?'Fit selected learning path in view':'Fit all map nodes in view');fit.setAttribute('title',hasPath?'Zoom out until the selected learning path fits on screen':'Zoom out until every concept fits on screen');const label=fit.querySelector&&fit.querySelector('.lp-map-fit-txt');const labelText=hasPath?'Fit path':'Fit all';if(label&&label.textContent!==labelText)label.textContent=labelText;}}
function lpMapSyncRouteContextForNode(node,state){const path=state&&Array.isArray(state.path)?state.path:[];if(!node)return;const key=lpCanonKey(node.getAttribute('data-lp-loc')||'');const endpoint=path.length<2?'':(key===lpCanonKey(path[0])?'start':(key===lpCanonKey(path[path.length-1])?'goal':''));const visible=!!(endpoint&&node.classList.contains('is-route')&&!node.classList.contains('lp-dim')&&!node.classList.contains('is-route-revealing')&&!lpH1StudyRouteNodeMasked(node,state));if(node.setAttribute&&node.removeAttribute){if(visible)node.setAttribute('data-lp-route-endpoint',endpoint);else node.removeAttribute('data-lp-route-endpoint');}
if(path.length<2||!node.closest)return;if(key===lpCanonKey(path[0])||key===lpCanonKey(path[path.length-1]))lpMapSyncRouteContext(node.closest('#lp-h1sg-modal'));}
function lpMapClearFitStatus(modal){const status=modal&&modal.querySelector('[data-lp-map-fit-status]');if(status){status.hidden=true;status.textContent='';}}
function lpLocalMapResetView(modal,st){const state=st||window.__lpMapState;if(!modal||!state)return;lpMapClearFitStatus(modal);const stageW=Math.max(0,Number(state.__lpStageW)||0);const stageH=Math.max(0,Number(state.__lpStageH)||0);const safe=lpLocalMapSafeInsets(modal,stageW,stageH);const centerX=Number.isFinite(safe.centerX)?safe.centerX:(stageW/2);const centerY=Number.isFinite(safe.centerY)?safe.centerY:(stageH/2);const target=lpLocalMapPreferredTargetPoint(modal,state);const scale=lpMapDefaultActualScale();state.scale=scale;state.tx=centerX-(Number(target.x)||0)*scale;state.ty=centerY-(Number(target.y)||0)*scale;state.userMoved=false;state.__lpForceCenter=false;}
...
function lpScheduleLiteMathKatexRefresh(){if(lpKatexAvailable())return;if(__lpLiteMathKatexRefreshBound)return;__lpLiteMathKatexRefreshBound=true;let ran=false;const idle=task=>typeof window.__mkRunWhenInteractionIdle==='function'?window.__mkRunWhenInteractionIdle(task,500):window.setTimeout(task,200);const runOnce=()=>{if(ran||!lpKatexAvailable())return;idle(()=>{if(ran)return;ran=true;__lpLiteMathKatexRefreshBound=false;try{lpRefreshLiteMathTitleSpans();}catch(_){}});};const failed=()=>{ran=true;__lpLiteMathKatexRefreshBound=false;};idle(()=>{try{Promise.resolve(window.__mkWarmMathRuntime?.()).then(()=>lpKatexAvailable()?runOnce():failed(),failed);}
catch(_){failed();}});try{window.addEventListener("mk:math-runtime-ready",runOnce,{once:true});}catch(_){}
...
function lpSetMapNodeTitleContent(titleSpan,rawTitle,visual,renderedHtml){if(!titleSpan)return;const raw=String(rawTitle||"").trim();const html=String(renderedHtml||"").trim();titleSpan.title=visual&&visual.hideTitle?"":raw;try{titleSpan.setAttribute("data-lp-raw-title",raw);}catch(_){}
...
function lpMapCourseKey(loc){return normLoc(loc||'').split('/').slice(0,2).join('/');}
function lpMapCourseLabelLines(text){const title=String(text||'').trim();const colon=title.indexOf(':');if(colon>0&&colon<12)return[title.slice(0,colon),title.slice(colon+1).trim()];if(title.length<=22)return[title];const words=title.split(/\s+/);let split=1,distance=Infinity;for(let i=1;i<words.length;i++){const candidate=Math.abs(words.slice(0,i).join(' ').length-words.slice(i).join(' ').length);if(candidate<distance){split=i;distance=candidate;}}
return[words.slice(0,split).join(' '),words.slice(split).join(' ')].filter(Boolean);}
const LP_WORLD_URL="assets/concept-world.json";const LP_WORLD_PAD=420;let __lpConceptWorld=null,__lpConceptWorldPromise=null;function lpWorldPathSegments(d){const segments=[];for(const m of String(d||"").matchAll(/([MLZ])\s*(?:(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?))?/g)){if(m[1]==="Z")segments.push({type:"Z"});else segments.push({type:m[1],x:Number(m[2]),y:Number(m[3])});}
return segments;}
function lpBuildConceptWorld(raw){const grid=raw&&raw.grid,box=raw&&raw.bounds;if(!raw||raw.version!==1||!grid||grid.kind!=="hex-odd-r"||!box||!raw.concepts||!Array.isArray(raw.courses))return null;const cw=Number(grid.cellWidth),rh=Number(grid.rowHeight);if(!(cw>0)||!(rh>0)||![box.left,box.top,box.right,box.bottom].every(Number.isFinite))return null;const ox=LP_WORLD_PAD-box.left,oy=LP_WORLD_PAD-box.top;const points=new Map();for(const[loc,cell]of Object.entries(raw.concepts)){if(!Array.isArray(cell)||!Number.isInteger(cell[0])||!Number.isInteger(cell[1]))continue;points.set(lpCanonKey(loc),{x:cell[0]*cw+(cell[1]&1?cw/2:0)+ox,y:cell[1]*rh+oy});}
const shift=d=>String(d||"").replace(/(-?\d+(?:\.\d+)?) (-?\d+(?:\.\d+)?)/g,(_,x,y)=>`${Number(x) + ox} ${Number(y) + oy}`);const run=r=>{const d=shift(r&&r.d);return{d,segments:lpWorldPathSegments(d),neighbour:r&&r.neighbour||""};};const provinces=new Map();for(const unit of raw.provinces||[]){const b=unit.bounds||{};if(!unit.id||!unit.courseId||!unit.d||![b.left,b.right,b.top,b.bottom].every(Number.isFinite))continue;if(!provinces.has(unit.courseId))provinces.set(unit.courseId,[]);provinces.get(unit.courseId).push({id:unit.id,d:shift(unit.d),borders:(unit.borders||[]).map(run),bounds:{left:b.left+ox,right:b.right+ox,top:b.top+oy,bottom:b.bottom+oy},label:unit.label?{...unit.label,x:unit.label.x+ox,y:unit.label.y+oy,box:unit.label.box?{left:unit.label.box.left+ox,right:unit.label.box.right+ox,top:unit.label.box.top+oy,bottom:unit.label.box.bottom+oy}:null}:null});}
const areas=raw.courses.map(course=>{const text=__lpCourseLabelFromRelPath(course.id+"/index.html")||course.name||String(course.id).split("/").pop();const lines=lpMapCourseLabelLines(text);const b=course.bounds||{};return{id:"world:"+course.id,course:course.id,hue:Number(course.hue),fillRule:"evenodd",d:shift(course.d),bounds:{left:b.left+ox,right:b.right+ox,top:b.top+oy,bottom:b.bottom+oy},borders:(course.borders||[]).map(run),frontiers:(course.frontiers||[]).map(run),provinces:(provinces.get(course.id)||[]).sort((a,b)=>a.id.localeCompare(b.id)).map((unit,index)=>({...unit,tone:index%3})),label:course.label?{x:course.label.x+ox,y:course.label.y+oy,text,course:course.id,width:Math.min(360,Math.max(140,...lines.map(line=>line.length*13+24))),height:lines.length>1?68:48}:null,};});const hues=new Map(raw.courses.map(course=>[course.id,Number(course.hue)]));return{layout:String(raw.layout||""),width:Math.ceil(box.right-box.left+2*LP_WORLD_PAD),height:Math.ceil(box.bottom-box.top+2*LP_WORLD_PAD),cellWidth:cw,rowHeight:rh,point:loc=>points.get(lpCanonKey(loc))||null,has:loc=>points.has(lpCanonKey(loc)),areas,locs:Object.keys(raw.concepts).filter(loc=>points.has(lpCanonKey(loc))),courseOf:loc=>raw.conceptCourses?.[lpCanonKey(loc)]||lpCanonKey(loc).split("/").slice(0,2).join("/"),hueOf:course=>hues.get(course),};}
function lpConceptWorld(){return __lpConceptWorld;}
function lpNearbyReveal(world,nodes,edges=[]){const byId=new Map();for(const id of nodes){const p=world.point(id),key=String(id);if(p&&!byId.has(key))byId.set(key,{x:p.x,y:p.y});}
const points=[...byId.values()],segments=[],seen=new Set();for(const edge of edges){const from=String(edge.from),to=String(edge.to);const a=byId.get(from),b=byId.get(to),key=[from,to].sort().join('\n');if(!a||!b||from===to||seen.has(key))continue;seen.add(key);segments.push({a,b});}
const radius=180,corridorRadius=90,feather=48;const bounds={left:Math.min(...points.map(p=>p.x))-radius,right:Math.max(...points.map(p=>p.x))+radius,top:Math.min(...points.map(p=>p.y))-radius,bottom:Math.max(...points.map(p=>p.y))+radius};const hiddenPoints=(world.locs||[]).filter(id=>!byId.has(lpCanonKey(id))).map(id=>world.point(id)).filter(p=>p&&p.x>=bounds.left&&p.x<=bounds.right&&p.y>=bounds.top&&p.y<=bounds.bottom);return{points,segments,radius,corridorRadius,feather,bounds,hiddenPoints};}
function lpNearbyCloseEmptyHoles(data,width,height,hiddenPixels=[]){if(!(width>2&&height>2)||!data||data.length<width*height*4)return 0;const count=width*height,state=new Uint8Array(count),protectedPixels=new Uint8Array(count);for(const point of hiddenPixels){const x=Math.floor(point.x),y=Math.floor(point.y);for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++)
if(x+dx>=0&&x+dx<width&&y+dy>=0&&y+dy<height)
protectedPixels[(y+dy)*width+x+dx]=1;}
const queue=new Int32Array(count);let filled=0;for(let start=0;start<count;start++){if(state[start]||data[start*4+3]===255)continue;let head=0,tail=1,exterior=false,protectedHole=false;queue[0]=start;state[start]=1;while(head<tail){const index=queue[head++],x=index%width,y=(index-x)/width;if(!x||!y||x===width-1||y===height-1)exterior=true;if(protectedPixels[index])protectedHole=true;for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){if(!dx&&!dy)continue;const nx=x+dx,ny=y+dy;if(nx<0||nx>=width||ny<0||ny>=height)continue;const next=ny*width+nx;if(!state[next]&&data[next*4+3]<255){state[next]=1;queue[tail++]=next;}}}
if(exterior||protectedHole)continue;for(let i=0;i<tail;i++){const at=queue[i]*4;data[at]=data[at+1]=data[at+2]=data[at+3]=255;}
filled++;}
return filled;}
function lpNearbyClipPath(d,box,closed){const tokens=String(d||'').match(/[MLZ]|-?\d+(?:\.\d+)?/gi)||[];const chains=[];let chain=[];for(let i=0;i<tokens.length;){const type=tokens[i++].toUpperCase();if(type==='Z')continue;if(type==='M'&&chain.length){chains.push(chain);chain=[];}
if(type==='M'||type==='L')chain.push({x:Number(tokens[i++]),y:Number(tokens[i++])});}
if(chain.length)chains.push(chain);const edges=[['x',box.left,1],['x',box.right,-1],['y',box.top,1],['y',box.bottom,-1]];const coord=p=>`${Math.round(p.x*100)/100} ${Math.round(p.y*100)/100}`;let out='';for(let points of chains){if(closed){for(const[axis,bound,sign]of edges){const input=points;points=[];if(!input.length)break;let a=input[input.length-1],aIn=sign*(a[axis]-bound)>=0;for(const b of input){const bIn=sign*(b[axis]-bound)>=0;if(aIn!==bIn){const t=(bound-a[axis])/(b[axis]-a[axis]);points.push({x:a.x+t*(b.x-a.x),y:a.y+t*(b.y-a.y)});}
if(bIn)points.push(b);a=b;aIn=bIn;}}
if(points.length>=3)out+='M'+points.map(coord).join('L')+'Z';}else{for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i];let lo=0,hi=1;for(const[axis,bound,sign]of edges){const value=sign*(a[axis]-bound),delta=sign*(b[axis]-a[axis]);if(!delta){if(value<0)hi=-1;}
else if(delta>0)lo=Math.max(lo,-value/delta);else hi=Math.min(hi,-value/delta);}
if(lo<=hi)out+='M'+coord({x:a.x+(b.x-a.x)*lo,y:a.y+(b.y-a.y)*lo})+'L'+coord({x:a.x+(b.x-a.x)*hi,y:a.y+(b.y-a.y)*hi});}}}
return out;}
function lpNearbyWorld(world,nodes,previous,edges=[]){if(!world||!nodes.length||!nodes.every(id=>world.has(id)))return null;const ids=Array.from(new Set(nodes.map(lpCanonKey))).sort(),visible=new Set(ids);const links=new Set(edges.filter(e=>visible.has(lpCanonKey(e.from))&&visible.has(lpCanonKey(e.to))&&lpCanonKey(e.from)!==lpCanonKey(e.to)).map(e=>[lpCanonKey(e.from),lpCanonKey(e.to)].sort().join('\t')));const key=ids.join('\n')+'\n--links--\n'+[...links].sort().join('\n');if(previous&&previous.source===world&&previous.revealKey===key)return previous;const reveal=lpNearbyReveal(world,ids,[...links].map(pair=>{const[from,to]=pair.split('\t');return{from,to};})),box=reveal.bounds;const labels=[];const areas=world.areas.flatMap(area=>{const b=area.bounds;if(b.right<box.left||b.left>box.right||b.bottom<box.top||b.top>box.bottom)return[];const d=lpNearbyClipPath(area.d,box,true);if(!d)return[];const members=ids.filter(id=>world.courseOf(id)===area.course).map(id=>world.point(id));let label=area.label?{...area.label}:null;if(area.label&&members.length){let cluster=[];for(const point of members){const near=members.filter(p=>Math.hypot(p.x-point.x,p.y-point.y)<=600);if(near.length>cluster.length)cluster=near;}
const xs=cluster.map(p=>p.x).sort((a,b)=>a-b);const preferred={x:xs[Math.floor(xs.length/2)],y:Math.min(...cluster.map(p=>p.y))-176};const halfW=area.label.width*2,halfH=area.label.height*2;const obstacles=ids.map(id=>{const p=world.point(id);return{x:p.x,y:p.y,width:340,height:44};}).concat(labels);let best=Infinity;for(const point of members)for(const offset of[-176,176]){const candidate={x:point.x,y:point.y+offset};const overlap=obstacles.reduce((sum,p)=>sum+Math.max(0,halfW+p.width/2-Math.abs(p.x-candidate.x))*Math.max(0,halfH+p.height/2+16-Math.abs(p.y-candidate.y)),0);const score=overlap*1000+Math.hypot(candidate.x-preferred.x,candidate.y-preferred.y);if(score<best){best=score;label={...area.label,...candidate};}}
labels.push({x:label.x,y:label.y,width:halfW*2,height:halfH*2});}
const provinces=(area.provinces||[]).flatMap(unit=>{const b=unit.bounds;if(b.right<box.left||b.left>box.right||b.bottom<box.top||b.top>box.bottom)return[];const d=lpNearbyClipPath(unit.d,box,true);return d?[{...unit,d,borders:(unit.borders||[]).map(run=>({...run,d:lpNearbyClipPath(run.d,box,false)})).filter(run=>run.d)}]:[];});return[{...area,d,bounds:box,label,frontiers:(area.frontiers||[]).map(run=>({...run,d:lpNearbyClipPath(run.d,box,false)})).filter(run=>run.d),provinces,borders:(area.borders||[]).map(run=>({...run,d:lpNearbyClipPath(run.d,box,false)})).filter(run=>run.d)}];});reveal.frontiers=areas.flatMap(area=>area.frontiers||[]);return{...world,source:world,areas,reveal,revealKey:key};}
function lpLoadConceptWorld(){if(__lpConceptWorld)return Promise.resolve(__lpConceptWorld);if(!__lpConceptWorldPromise){__lpConceptWorldPromise=(async()=>{try{const raw=await __mkFetchJsonShared(new URL(LP_WORLD_URL,getSiteRootUrl()).toString());__lpConceptWorld=lpBuildConceptWorld(raw);}catch(_){__lpConceptWorld=null;}
return __lpConceptWorld;})().then(world=>{if(!world){__lpConceptWorldPromise=null;try{delete window.__mkSharedJsonPromiseMap[new URL(LP_WORLD_URL,getSiteRootUrl()).toString()];}catch(_){}}
return world;});}
return __lpConceptWorldPromise;}
function lpPillExit(node,toward,gap){const hw=Math.max(1,Number(node.width)/2||1),hh=Math.max(1,Number(node.height)/2||1);let dx=toward.x-node.x,dy=toward.y-node.y;const len=Math.hypot(dx,dy)||1;dx/=len;dy/=len;const r=Math.min(hw,hh),core=Math.max(0,hw-r);let t=Math.abs(dy)>1e-9?r/Math.abs(dy):Infinity;if(!(Math.abs(dx*t)<=core)){const cx=Math.sign(dx)*core,b=dx*cx;t=b+Math.sqrt(Math.max(0,b*b-(cx*cx-r*r)));}
return{x:node.x+dx*(t+gap),y:node.y+dy*(t+gap)};}
function lpWorldGeometry(world){const num=v=>Math.round(v*10)/10;const place=input=>({nodes:(input||[]).map(n=>{const p=world.point(n.id);return p?{...n,x:p.x,y:p.y}:{...n};}),issues:[],stats:{}});const route=(input,edges,options)=>{const gap=Number.isFinite(Number(options&&options.endpointGap))?Number(options.endpointGap):14;const byId=new Map((input||[]).map(n=>[n.id,n]));const routes=(edges||[]).map((edge,index)=>{const from=byId.get(edge&&edge.from),to=byId.get(edge&&edge.to);const base={index,from:edge&&edge.from,to:edge&&edge.to};if(!from||!to||from.id===to.id)return{...base,status:"invalid",points:[],segments:[],d:""};const a=lpPillExit(from,to,gap),b=lpPillExit(to,from,gap);if((b.x-a.x)*(to.x-from.x)+(b.y-a.y)*(to.y-from.y)<=0)return{...base,status:"blocked",reason:"endpoint-overlap",points:[],segments:[],d:""};const segments=[{type:"M",x:a.x,y:a.y},{type:"L",x:b.x,y:b.y}];return{...base,status:"ok",points:[a,b],segments,d:`M ${num(a.x)} ${num(a.y)} L ${num(b.x)} ${num(b.y)}`,length:Math.hypot(b.x-a.x,b.y-a.y)};});return{routes,issues:[],stats:{}};};const regions=()=>({regions:world.areas,courses:[],borders:[],frontiers:[],issues:[],stats:{}});const layout=(input,edges,options)=>{const nodes=place(input).nodes;return{nodes,routes:route(nodes,edges,options).routes,regions:world.areas,courses:[],borders:[],frontiers:[],issues:[],stats:{},bounds:{left:0,top:0,right:world.width,bottom:world.height,width:world.width,height:world.height}};};return{version:1,world:true,layout,place,route,regions};}
function lpMapGeometryOptions(nodes){const courseLabels={},labelSizes={};for(const node of nodes){if(courseLabels[node.course])continue;const label=__lpCourseLabelFromRelPath(node.id)||node.course.split('/').pop().replace(/-/g,' ');courseLabels[node.course]=label;const lines=lpMapCourseLabelLines(label);labelSizes[node.course]={width:Math.min(360,Math.max(140,...lines.map(line=>line.length*13+24))),height:lines.length>1?68:48};}
return{cellSize:18,clearance:12,nodeGap:60,regionPadding:16,regionReach:220,routeMode:'straight',endpointGap:14,courseLabels,labelSizes};}
function lpMapParchmentFrontierPaths(frontiers){const distance=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);const key=p=>`${p.x.toFixed(4)},${p.y.toFixed(4)}`;const chains=[];for(const run of frontiers){if(!Array.isArray(run.segments))return[];let start,from;const points=[];for(const s of run.segments){if(s.type==='M'){start=from={x:s.x,y:s.y};points.push(from);continue;}
if(!from)return[];const to=s.type==='Z'?start:s;const length=s.type==='C'?distance(from,{x:s.c1x,y:s.c1y})+Math.hypot(s.c2x-s.c1x,s.c2y-s.c1y)+distance({x:s.c2x,y:s.c2y},to):s.type==='Q'?distance(from,{x:s.cx,y:s.cy})+distance({x:s.cx,y:s.cy},to):distance(from,to);const steps=Math.max(1,Math.ceil(length/4));for(let i=1;i<=steps;i++){const t=i/steps,u=1-t;const p=s.type==='C'?{x:u*u*u*from.x+3*u*u*t*s.c1x+3*u*t*t*s.c2x+t*t*t*to.x,y:u*u*u*from.y+3*u*u*t*s.c1y+3*u*t*t*s.c2y+t*t*t*to.y}:s.type==='Q'?{x:u*u*from.x+2*u*t*s.cx+t*t*to.x,y:u*u*from.y+2*u*t*s.cy+t*t*to.y}:{x:from.x+(to.x-from.x)*t,y:from.y+(to.y-from.y)*t};if(!Number.isFinite(p.x)||!Number.isFinite(p.y))return[];if(distance(points[points.length-1],p)>.001)points.push(p);}
from=to;}
if(points.length>1)chains.push(points);}
const links=new Map();chains.forEach((points,index)=>[points[0],points[points.length-1]].forEach(p=>{const at=key(p);if(!links.has(at))links.set(at,[]);links.get(at).push(index);}));if([...links.values()].some(list=>list.length!==2))return[];const used=new Set(),rings=[];for(let index=0;index<chains.length;index++){if(used.has(index))continue;let points=chains[index].slice();used.add(index);while(key(points[0])!==key(points[points.length-1])){const next=links.get(key(points[points.length-1])).find(i=>!used.has(i));if(next===undefined)return[];let chain=chains[next];used.add(next);if(key(chain[0])!==key(points[points.length-1]))chain=chain.slice().reverse();points.push(...chain.slice(1));}
points.pop();if(points.length<3)continue;const signed=points.reduce((sum,p,i)=>{const q=points[(i+1)%points.length];return sum+p.x*q.y-p.y*q.x;},0);if(signed<0)points.reverse();let first=0;points.forEach((p,i)=>{if(p.x<points[first].x||(p.x===points[first].x&&p.y<points[first].y))first=i;});points=points.slice(first).concat(points.slice(0,first));points.push(points[0]);const lengths=[0];for(let i=1;i<points.length;i++)lengths.push(lengths[i-1]+distance(points[i-1],points[i]));const total=lengths[lengths.length-1];let segment=1;const at=offset=>{const d=((offset%total)+total)%total;while(segment>1&&lengths[segment-1]>d)segment--;while(segment<lengths.length-1&&lengths[segment]<d)segment++;const a=points[segment-1],b=points[segment],t=(d-lengths[segment-1])/(lengths[segment]-lengths[segment-1]||1);return{x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t};};const noise=(x,y)=>{const n=Math.sin(x*127.1+y*311.7)*43758.5453;return n-Math.floor(n);};const samples=[];const add=(d,depth=0)=>{const p=at(d),a=at(d-1.5),b=at(d+1.5),length=distance(a,b)||1,nx=-(b.y-a.y)/length,ny=(b.x-a.x)/length;const inset=4.8+1.1*Math.sin(p.x*.019+p.y*.027)+.7*Math.sin(p.x*.067-p.y*.051)+2.2*(2*noise(Math.round(p.x*2),Math.round(p.y*2))-1)+depth;samples.push({outer:{x:p.x-nx*6,y:p.y-ny*6},inner:{x:p.x+nx*inset,y:p.y+ny*inset}});};let nextNotch=55+150*noise(points[0].x,points[0].y);for(let d=0;d<total;){const p=at(d);if(d>=nextNotch&&d+12<total){const width=3+7*noise(p.x+41,p.y-9),depth=12+14*noise(p.x-23,p.y+61),lean=.32+.36*noise(p.x+9,p.y+17);add(d);add(d+width*lean,depth);add(d+width);d+=width+2;nextNotch=d+115+290*noise(p.x-31,p.y-19);}else{add(d);d+=2+4*noise(Math.round(p.x),Math.round(p.y));}}
if(samples.length>2){const xy=p=>`${p.x.toFixed(2)} ${p.y.toFixed(2)}`;const triangles=[];const triangle=(a,b,c)=>{if((b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x)<0)[b,c]=[c,b];triangles.push(`M${xy(a)}L${xy(b)}L${xy(c)}Z`);};samples.forEach((a,i)=>{const b=samples[(i+1)%samples.length];triangle(a.outer,b.outer,b.inner);triangle(a.outer,b.inner,a.inner);});rings.push({edge:'M'+samples.map(s=>xy(s.inner)).join('L')+'Z',cut:triangles.join('')});}}
return rings.sort((a,b)=>a.edge<b.edge?-1:a.edge>b.edge?1:0);}
function lpMapPaintWorldSurfaces(layer,defs,uid,box,clip,mask){const ns='http://www.w3.org/2000/svg';let root='';try{root=lpMapPaintTerritories.paperRoot||getSiteRootUrl();lpMapPaintTerritories.paperRoot=root;}catch(_){}
for(const paper of['default','ink','parchment','cyber','nordic','ocean','celestial']){const pattern=document.createElementNS(ns,'pattern');pattern.id=uid+'-surface-'+paper;pattern.classList.add('lp-map-world-pattern');pattern.dataset.lpPaper=paper;pattern.setAttribute('patternUnits','userSpaceOnUse');pattern.setAttribute('x','0');pattern.setAttribute('y','0');const size=paper==='default'?160:1024;pattern.setAttribute('width',String(size));pattern.setAttribute('height',String(size));const base=document.createElementNS(ns,'rect');base.setAttribute('width',String(size));base.setAttribute('height',String(size));base.style.setProperty('fill',paper==='default'?'var(--lp-paper-base, var(--wiki-map-canvas, #14283a))':'var(--lp-paper-base, var(--atlas-paper, #e8e0cc))','important');pattern.appendChild(base);if(paper==='default'){for(const major of[false,true]){const grid=document.createElementNS(ns,'path');grid.classList.add(major?'lp-map-world-grid-major':'lp-map-world-grid');grid.setAttribute('d',major?'M0 160V0H160':[32,64,96,128].map(n=>`M${n} 0V160M0 ${n}H160`).join(''));grid.style.setProperty('fill','none','important');grid.style.setProperty('stroke',major?'var(--wiki-map-grid-major, rgba(150,190,215,.2))':'var(--wiki-map-grid, rgba(150,190,215,.14))','important');grid.setAttribute('stroke-width',major?'1':'.6');pattern.appendChild(grid);}}else if(root){const photo=document.createElementNS(ns,'image');photo.classList.add('lp-map-paper-photo');photo.dataset.lpPaper=paper;photo.setAttribute('href',root+`assets/map-styles/paper-${paper}.jpg`);photo.setAttribute('width','1024');photo.setAttribute('height','1024');photo.setAttribute('preserveAspectRatio','none');pattern.appendChild(photo);}
defs.appendChild(pattern);const surface=document.createElementNS(ns,'rect');surface.classList.add('lp-map-world-surface');if(paper==='parchment')surface.classList.add('lp-parchment-paper');surface.dataset.lpPaper=paper;surface.setAttribute('display',paper==='default'?'inline':'none');surface.setAttribute('pointer-events','none');for(const[key,value]of Object.entries(box))surface.setAttribute(key,value);surface.style.setProperty('fill',`url(#${pattern.id})`,'important');if(clip)surface.setAttribute('clip-path',clip);if(mask)surface.setAttribute('mask',mask);layer.appendChild(surface);}}
function lpMapWorldSurfaceProjection(layer,engine){if(!layer||!engine||typeof lpWebgl3dProject!=='function')return;if(layer.querySelector('.lp-map-world-projection'))return;const x=(Number(engine.W)||1)/2,y=(Number(engine.H)||1)/2;const cam=Number(engine.camera)||LP_WEBGL3D_CAMERA;const samples=[{x,y,z:0},{x:x+1,y,z:0},{x,y:y+1,z:0}].map(p=>{const q=lpWebgl3dProject(engine,p),d=(cam-q.z)/cam;return{x:q.x*d,y:q.y*d,d};});const[a,b,c]=samples,h=[b.x-a.x,c.x-a.x,0,b.y-a.y,c.y-a.y,0,b.d-a.d,c.d-a.d,0];h[2]=a.x-h[0]*x-h[1]*y;h[5]=a.y-h[3]*x-h[4]*y;h[8]=a.d-h[6]*x-h[7]*y;if(!h.every(Number.isFinite))return;const surfaces=Array.from(layer.querySelectorAll('.lp-map-world-surface'));if(!surfaces.length)return;const first=surfaces[0],sx=Number(first.getAttribute('x')),sy=Number(first.getAttribute('y'));const sw=Number(first.getAttribute('width')),sh=Number(first.getAttribute('height'));let nodes=Array.from(engine.nodes||[]).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(!nodes.length)nodes=[{x:0,y:0},{x:Number(engine.W)||1,y:Number(engine.H)||1}];const margin=Math.max(320,Math.min(720,cam*.4));const left=Math.floor(Math.min(...nodes.map(p=>p.x)))-margin,top=Math.floor(Math.min(...nodes.map(p=>p.y)))-margin;const width=Math.ceil(Math.max(...nodes.map(p=>p.x)))-left+margin,height=Math.ceil(Math.max(...nodes.map(p=>p.y)))-top+margin;if(!(width>0&&height>0))return;const rasterScale=Math.min(1,2048/width,2048/height);const rasterWidth=width*rasterScale,rasterHeight=height*rasterScale;const ns='http://www.w3.org/2000/svg',carrier=document.createElementNS(ns,'foreignObject');carrier.classList.add('lp-map-world-projection');carrier.setAttribute('x',String(sx));carrier.setAttribute('y',String(sy));carrier.setAttribute('width',String(sw));carrier.setAttribute('height',String(sh));carrier.setAttribute('pointer-events','none');carrier.style.setProperty('overflow','visible','important');for(const key of['clip-path','mask'])if(first.getAttribute(key))carrier.setAttribute(key,first.getAttribute(key));const plane=document.createElementNS('http://www.w3.org/1999/xhtml','div');plane.classList.add('lp-map-world-plane');plane.style.setProperty('position','absolute');plane.style.setProperty('left','0');plane.style.setProperty('top','0');plane.style.setProperty('width',`${rasterWidth}px`);plane.style.setProperty('height',`${rasterHeight}px`);plane.style.setProperty('transform-origin','0 0');plane.style.setProperty('pointer-events','none');const u0=h[0]-sx*h[6],u1=h[1]-sx*h[7],u2=h[2]-sx*h[8];const v0=h[3]-sy*h[6],v1=h[4]-sy*h[7],v2=h[5]-sy*h[8];const matrix=[u0/rasterScale,v0/rasterScale,0,h[6]/rasterScale,u1/rasterScale,v1/rasterScale,0,h[7]/rasterScale,0,0,1,0,u0*left+u1*top+u2,v0*left+v1*top+v2,0,h[6]*left+h[7]*top+h[8]];plane.style.setProperty('transform',`matrix3d(${matrix.join(',')})`);plane.dataset.lpWorldLeft=String(left);plane.dataset.lpWorldTop=String(top);plane.dataset.lpRasterScale=String(rasterScale);const sheet=document.createElementNS(ns,'svg');sheet.setAttribute('width',String(rasterWidth));sheet.setAttribute('height',String(rasterHeight));sheet.setAttribute('viewBox',`${left} ${top} ${width} ${height}`);sheet.style.setProperty('overflow','visible','important');layer.insertBefore(carrier,first);carrier.appendChild(plane);plane.appendChild(sheet);for(const surface of surfaces){surface.removeAttribute('clip-path');surface.removeAttribute('mask');surface.setAttribute('x',String(left));surface.setAttribute('y',String(top));surface.setAttribute('width',String(width));surface.setAttribute('height',String(height));surface.remove();sheet.appendChild(surface);}}
function lpMapPaintRouteSurface(svg,points,engine){const nodes=Array.from(points||[]).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(!nodes.length)return null;const pad=180,left=Math.min(...nodes.map(p=>p.x-(p.w||p.width||160)/2))-pad;const top=Math.min(...nodes.map(p=>p.y-(p.h||p.height||48)/2))-pad;const right=Math.max(...nodes.map(p=>p.x+(p.w||p.width||160)/2))+pad;const bottom=Math.max(...nodes.map(p=>p.y+(p.h||p.height||48)/2))+pad;const ns='http://www.w3.org/2000/svg',layer=document.createElementNS(ns,'g');layer.classList.add('lp-map-route-paper');layer.setAttribute('aria-hidden','true');layer.setAttribute('pointer-events','none');const defs=document.createElementNS(ns,'defs');layer.appendChild(defs);const uid=`lp-route-paper-${lpMapPaintRouteSurface.serial = (lpMapPaintRouteSurface.serial || 0) + 1}`;lpMapPaintWorldSurfaces(layer,defs,uid,{x:left,y:top,width:right-left,height:bottom-top},'','');if(engine)lpMapWorldSurfaceProjection(layer,engine);svg.insertBefore(layer,svg.firstChild);return layer;}
function lpNearbyFeatherCoast(data,width,height,reveal,scale){if(!reveal.frontiers?.length||width<1||height<1)return;const distance=new Float32Array(width*height);const reach=Math.max(1,reveal.feather*scale);distance.fill(reach);const b=reveal.bounds;for(const run of reveal.frontiers){let previous=null;for(const match of String(run.d||run).matchAll(/([ML])\s*(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)/g)){const point={x:(Number(match[2])-b.left)*scale,y:(Number(match[3])-b.top)*scale};if(match[1]==='L'&&previous){const steps=Math.max(1,Math.ceil(Math.max(Math.abs(point.x-previous.x),Math.abs(point.y-previous.y))*2));for(let i=0;i<=steps;i++){const x=Math.floor(previous.x+(point.x-previous.x)*i/steps),y=Math.floor(previous.y+(point.y-previous.y)*i/steps);if(x>=0&&x<width&&y>=0&&y<height)distance[y*width+x]=0;}}
previous=point;}}
const diagonal=Math.SQRT2;for(let y=0;y<height;y++)for(let x=0;x<width;x++){const i=y*width+x;if(x)distance[i]=Math.min(distance[i],distance[i-1]+1);if(y){distance[i]=Math.min(distance[i],distance[i-width]+1);if(x)distance[i]=Math.min(distance[i],distance[i-width-1]+diagonal);if(x+1<width)distance[i]=Math.min(distance[i],distance[i-width+1]+diagonal);}}
for(let y=height-1;y>=0;y--)for(let x=width-1;x>=0;x--){const i=y*width+x;if(x+1<width)distance[i]=Math.min(distance[i],distance[i+1]+1);if(y+1<height){distance[i]=Math.min(distance[i],distance[i+width]+1);if(x)distance[i]=Math.min(distance[i],distance[i+width-1]+diagonal);if(x+1<width)distance[i]=Math.min(distance[i],distance[i+width+1]+diagonal);}
const t=Math.min(1,distance[i]/reach);data[i*4+3]=Math.round(data[i*4+3]*t*t*(3-2*t));}}
function lpNearbyRevealMask(defs,uid,reveal){const ns='http://www.w3.org/2000/svg',b=reveal.bounds;const width=b.right-b.left,height=b.bottom-b.top;const mask=document.createElementNS(ns,'mask');mask.id=uid+'-nearby';mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');mask.style.setProperty('mask-type','luminance','important');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height}))mask.setAttribute(k,v);if(!reveal.alphaUrl){try{const canvas=document.createElement('canvas');const scale=Math.min(1,1024/Math.max(width,height));canvas.width=Math.max(1,Math.ceil(width*scale));canvas.height=Math.max(1,Math.ceil(height*scale));const ctx=canvas.getContext('2d');if(ctx){ctx.scale(scale,scale);ctx.translate(-b.left,-b.top);const stops=(gradient,radius)=>{gradient.addColorStop(0,'rgba(255,255,255,1)');gradient.addColorStop(Math.max(0,radius-reveal.feather)/radius,'rgba(255,255,255,1)');gradient.addColorStop(1,'rgba(255,255,255,0)');return gradient;};for(const{a,b:c}of reveal.segments){const length=Math.hypot(c.x-a.x,c.y-a.y),radius=reveal.corridorRadius;if(!length)continue;for(const side of[-1,1]){const nx=(c.y-a.y)/length*radius*side,ny=(a.x-c.x)/length*radius*side;ctx.fillStyle=stops(ctx.createLinearGradient(a.x,a.y,a.x+nx,a.y+ny),radius);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(c.x,c.y);ctx.lineTo(c.x+nx,c.y+ny);ctx.lineTo(a.x+nx,a.y+ny);ctx.closePath();ctx.fill();}}
for(const p of reveal.points){const radius=reveal.radius;ctx.fillStyle=stops(ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,radius),radius);ctx.beginPath();ctx.arc(p.x,p.y,radius,0,2*Math.PI);ctx.fill();}
if(typeof ctx.getImageData==='function'&&typeof ctx.putImageData==='function'){const image=ctx.getImageData(0,0,canvas.width,canvas.height);const hidden=(reveal.hiddenPoints||[]).map(p=>({x:(p.x-b.left)*scale,y:(p.y-b.top)*scale}));lpNearbyCloseEmptyHoles(image.data,canvas.width,canvas.height,hidden);lpNearbyFeatherCoast(image.data,canvas.width,canvas.height,reveal,scale);ctx.putImageData(image,0,0);}
reveal.alphaUrl=canvas.toDataURL('image/png');reveal.alphaSize={width:canvas.width,height:canvas.height};ctx.globalCompositeOperation='source-out';ctx.fillStyle='white';ctx.fillRect(b.left,b.top,width,height);reveal.edgeAlphaUrl=canvas.toDataURL('image/png');}}catch(_){}}
if(reveal.alphaUrl){const image=document.createElementNS(ns,'image');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height,href:reveal.alphaUrl,preserveAspectRatio:'none'}))image.setAttribute(k,v);mask.appendChild(image);}else{for(const p of reveal.points){const circle=document.createElementNS(ns,'circle');for(const[k,v]of Object.entries({cx:p.x,cy:p.y,r:reveal.radius,fill:'white'}))circle.setAttribute(k,v);mask.appendChild(circle);}
if(reveal.segments.length){const path=document.createElementNS(ns,'path');path.setAttribute('d',reveal.segments.map(({a,b})=>`M${a.x} ${a.y}L${b.x} ${b.y}`).join(' '));path.setAttribute('fill','none');path.setAttribute('stroke','white');path.setAttribute('stroke-width',reveal.corridorRadius*2);path.setAttribute('stroke-linecap','round');mask.appendChild(path);}}
defs.appendChild(mask);if(reveal.edgeAlphaUrl)try{const root=lpMapPaintTerritories.paperRoot||getSiteRootUrl();lpMapPaintTerritories.paperRoot=root;const band=document.createElementNS(ns,'mask');band.id=uid+'-paper-band';band.classList.add('lp-map-frontier-paper-band');band.setAttribute('maskUnits','userSpaceOnUse');band.setAttribute('maskContentUnits','userSpaceOnUse');band.style.setProperty('mask-type','alpha','important');const edge=document.createElementNS(ns,'image');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height})){band.setAttribute(k,v);edge.setAttribute(k,v);}
edge.setAttribute('href',reveal.edgeAlphaUrl);edge.setAttribute('preserveAspectRatio','none');band.appendChild(edge);defs.appendChild(band);const pattern=document.createElementNS(ns,'pattern');pattern.id=uid+'-paper';pattern.classList.add('lp-map-frontier-paper-pattern');for(const[k,v]of Object.entries({patternUnits:'userSpaceOnUse',x:0,y:0,width:512,height:512}))pattern.setAttribute(k,v);for(const paper of['ink','parchment','cyber','nordic','ocean','celestial']){const image=document.createElementNS(ns,'image');image.classList.add('lp-map-frontier-paper');image.dataset.lpPaper=paper;for(const[k,v]of Object.entries({href:root+`assets/map-styles/edge-${paper}.svg`,width:512,height:512,display:'none'}))image.setAttribute(k,v);pattern.appendChild(image);}
defs.appendChild(pattern);const grain=document.createElementNS(ns,'rect');grain.classList.add('lp-map-frontier-grain');for(const[k,v]of Object.entries({x:b.left,y:b.top,width,height,display:'none',mask:`url(#${band.id})`}))grain.setAttribute(k,v);grain.style.setProperty('fill',`url(#${pattern.id})`,'important');grain.style.setProperty('opacity','var(--atlas-edge-grain, .85)');mask.appendChild(grain);}catch(_){}
return`url(#${mask.id})`;}
function lpMapPaintTerritories(svg,areas,options){const ns='http://www.w3.org/2000/svg';const paintOptions=options||{};const layer=document.createElementNS(ns,'g');layer.classList.add('lp-course-territories');layer.setAttribute('aria-hidden','true');const courses=Array.from(new Set((areas||[]).map(area=>area.course))).sort();const hues=[192,35,282,142,345,75];const courseHues=lpMapPaintTerritories.courseHues||(lpMapPaintTerritories.courseHues=new Map());for(const area of areas||[])if(area&&Number.isFinite(area.hue))courseHues.set(area.course,area.hue);for(const course of courses){if(!courseHues.has(course)){const index=courseHues.size;courseHues.set(course,index<hues.length?hues[index]:(220+(index-hues.length)*137.508)%360);}}
const uid=`lp-course-field-${lpMapPaintTerritories.serial = (lpMapPaintTerritories.serial || 0) + 1}`;const defs=document.createElementNS(ns,'defs');layer.appendChild(defs);const frontiers=(areas||[]).flatMap(area=>area.frontiers||[]).filter(run=>run&&(run.d||typeof run==='string'));const bounds=(areas||[]).map(area=>area.bounds).filter(box=>box&&[box.left,box.top,box.right,box.bottom].every(Number.isFinite));let frontierMask='';let parchmentRim=null;let worldBox=null,landClip='';if(paintOptions.reveal){const b=paintOptions.reveal.bounds;worldBox={x:b.left,y:b.top,width:b.right-b.left,height:b.bottom-b.top};frontierMask=lpNearbyRevealMask(defs,uid,paintOptions.reveal);const silhouette=document.createElementNS(ns,'clipPath');silhouette.id=uid+'-land';silhouette.setAttribute('clipPathUnits','userSpaceOnUse');for(const area of areas||[]){const path=document.createElementNS(ns,'path');path.setAttribute('d',area.d);path.setAttribute('clip-rule','evenodd');silhouette.appendChild(path);}
defs.appendChild(silhouette);landClip=`url(#${silhouette.id})`;}else if(frontiers.length&&bounds.length){const left=Math.min(...bounds.map(box=>box.left))-80;const top=Math.min(...bounds.map(box=>box.top))-80;const box={x:left,y:top,width:Math.max(...bounds.map(box=>box.right))+80-left,height:Math.max(...bounds.map(box=>box.bottom))+80-top};worldBox=box;const silhouette=document.createElementNS(ns,'clipPath');silhouette.id=uid+'-land';silhouette.setAttribute('clipPathUnits','userSpaceOnUse');for(const area of areas||[]){const land=document.createElementNS(ns,'path');land.setAttribute('d',area.d);land.setAttribute('clip-rule','evenodd');silhouette.appendChild(land);}
defs.appendChild(silhouette);landClip=`url(#${silhouette.id})`;const mask=document.createElementNS(ns,'mask');mask.id=uid+'-frontier';mask.setAttribute('maskUnits','userSpaceOnUse');mask.setAttribute('maskContentUnits','userSpaceOnUse');mask.style.setProperty('mask-type','luminance','important');for(const[key,value]of Object.entries(box))mask.setAttribute(key,value);const field=document.createElementNS(ns,'rect');for(const[key,value]of Object.entries(box))field.setAttribute(key,value);field.setAttribute('fill','white');field.style.setProperty('fill','white','important');mask.appendChild(field);const coast=document.createElementNS(ns,'g');coast.id=uid+'-coast';coast.classList.add('lp-course-frontier-fade');const coastData=frontiers.map(run=>run.d||run).join(' ');const soft=document.createElementNS(ns,'filter');soft.id=uid+'-soft-coast';soft.setAttribute('filterUnits','userSpaceOnUse');soft.setAttribute('primitiveUnits','userSpaceOnUse');soft.setAttribute('data-lp-window-filter','');for(const[key,value]of Object.entries(box))soft.setAttribute(key,value);const blur=document.createElementNS(ns,'feGaussianBlur');blur.setAttribute('stdDeviation','12');soft.appendChild(blur);defs.appendChild(soft);const fade=document.createElementNS(ns,'path');fade.classList.add('lp-course-frontier-mask');fade.setAttribute('d',coastData);fade.setAttribute('fill','none');fade.setAttribute('stroke','black');fade.style.setProperty('fill','none','important');fade.style.setProperty('stroke','black','important');fade.setAttribute('stroke-width','64');fade.setAttribute('stroke-linecap','round');fade.setAttribute('stroke-linejoin','round');coast.appendChild(fade);coast.setAttribute('filter',`url(#${soft.id})`);mask.appendChild(coast);try{const root=lpMapPaintTerritories.paperRoot||getSiteRootUrl();lpMapPaintTerritories.paperRoot=root;const band=document.createElementNS(ns,'mask');band.id=uid+'-paper-band';band.classList.add('lp-map-frontier-paper-band');band.setAttribute('maskUnits','userSpaceOnUse');band.setAttribute('maskContentUnits','userSpaceOnUse');band.style.setProperty('mask-type','alpha','important');for(const[key,value]of Object.entries(box))band.setAttribute(key,value);const reuse=document.createElementNS(ns,'use');reuse.setAttribute('href',`#${coast.id}`);band.appendChild(reuse);defs.appendChild(band);const pattern=document.createElementNS(ns,'pattern');pattern.id=uid+'-paper';pattern.classList.add('lp-map-frontier-paper-pattern');pattern.setAttribute('patternUnits','userSpaceOnUse');pattern.setAttribute('x','0');pattern.setAttribute('y','0');pattern.setAttribute('width','512');pattern.setAttribute('height','512');for(const paper of['ink','parchment','cyber','nordic','ocean','celestial']){const image=document.createElementNS(ns,'image');image.classList.add('lp-map-frontier-paper');image.dataset.lpPaper=paper;image.setAttribute('href',root+`assets/map-styles/edge-${paper}.svg`);image.setAttribute('width','512');image.setAttribute('height','512');image.setAttribute('display','none');pattern.appendChild(image);}
defs.appendChild(pattern);const grain=document.createElementNS(ns,'rect');grain.classList.add('lp-map-frontier-grain');for(const[key,value]of Object.entries(box))grain.setAttribute(key,value);grain.setAttribute('display','none');grain.style.setProperty('fill',`url(#${pattern.id})`,'important');grain.style.setProperty('opacity','var(--atlas-edge-grain, .85)');grain.setAttribute('mask',`url(#${band.id})`);mask.appendChild(grain);}catch(_){}
const torn=paintOptions.simple?[]:lpMapParchmentFrontierPaths(frontiers);if(torn.length){layer.classList.add('lp-parchment-ready');const cut=document.createElementNS(ns,'g');cut.classList.add('lp-parchment-cut');cut.setAttribute('display','none');const coastPath=torn.map(run=>run.edge).join('');const tear=document.createElementNS(ns,'path');tear.setAttribute('d',torn.map(run=>run.cut).join(''));tear.setAttribute('fill-rule','nonzero');tear.style.setProperty('fill','black','important');tear.style.setProperty('stroke','none','important');cut.appendChild(tear);mask.appendChild(cut);parchmentRim=document.createElementNS(ns,'g');parchmentRim.classList.add('lp-parchment-rim');parchmentRim.setAttribute('display','none');parchmentRim.setAttribute('clip-path',`url(#${silhouette.id})`);parchmentRim.setAttribute('mask',`url(#${mask.id})`);for(const[name,width,opacity]of[['age',40,.009],['age',36,.011],['age',32,.013],['age',28,.015],['age',24,.017],['age',20,.019],['age',16,.021],['age',12,.023],['age',8,.025],['age',4,.027],['depth',1.1,.18],['fiber',.6,.13]]){const rim=document.createElementNS(ns,'path');rim.classList.add('lp-parchment-'+name);rim.setAttribute('d',coastPath);rim.setAttribute('fill','none');rim.setAttribute('stroke-width',String(width));rim.setAttribute('stroke-linejoin','round');rim.style.setProperty('opacity',String(opacity));parchmentRim.appendChild(rim);}}
defs.appendChild(mask);frontierMask=`url(#${mask.id})`;}
const borderPaint=[];const labelPaint=[];const unitLabelPaint=[];const land=paintOptions.groupMask&&frontierMask?document.createElementNS(ns,'g'):null;if(land){land.classList.add('lp-course-land');land.setAttribute('mask',frontierMask);layer.appendChild(land);if(parchmentRim)parchmentRim.removeAttribute('mask');}
const landMask=land?'':frontierMask;if(worldBox)lpMapPaintWorldSurfaces(land||layer,defs,uid,worldBox,landClip,landMask);for(const area of areas||[]){const hue=courseHues.get(area.course);const path=document.createElementNS(ns,'path');path.classList.add('lp-course-territory');path.setAttribute('d',area.d);path.setAttribute('fill-rule','evenodd');path.style.setProperty('--lp-territory-hue',String(hue));path.dataset.lpCourse=area.course;if(landMask)path.setAttribute('mask',landMask);(land||layer).appendChild(path);for(const unit of area.provinces||[]){const region=document.createElementNS(ns,'path');region.classList.add('lp-unit-territory');region.setAttribute('d',unit.d);region.setAttribute('fill-rule','evenodd');region.dataset.lpUnit=unit.id;region.dataset.lpCourse=area.course;region.style.setProperty('--lp-territory-hue',String(hue));region.style.setProperty('--lp-unit-lightness',['34%','60%','86%'][unit.tone||0]);if(landMask)region.setAttribute('mask',landMask);(land||layer).appendChild(region);if(unit.label&&unit.label.text){const label=document.createElementNS(ns,'text');label.classList.add('lp-unit-territory-label');label.dataset.lpUnit=unit.id;label.setAttribute('x',String(unit.label.x));label.setAttribute('y',String(unit.label.y));label.setAttribute('text-anchor','middle');label.setAttribute('dominant-baseline','middle');label.setAttribute('aria-hidden','true');label.textContent=unit.label.text;label.style.setProperty('font-family','var(--atlas-font, "Palatino Linotype", Georgia, serif)');label.style.setProperty('font-weight','600');label.style.setProperty('letter-spacing','.5px');label.style.setProperty('fill','var(--wiki-map-ink, var(--md-default-fg-color))');label.style.setProperty('stroke','var(--wiki-map-canvas, var(--md-default-bg-color))');label.style.setProperty('stroke-width','1.2px');label.style.setProperty('paint-order','stroke fill');label.style.setProperty('opacity','.38');label.style.setProperty('pointer-events','none');unitLabelPaint.push(label);}
for(const run of unit.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-unit-border');border.setAttribute('d',run.d);border.dataset.lpUnit=unit.id;border.style.setProperty('--lp-territory-hue',String(hue));if(landMask)border.setAttribute('mask',landMask);borderPaint.push(border);}}
for(const run of area.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-course-border');border.setAttribute('d',run.d||run);border.dataset.lpCourse=area.course;border.style.setProperty('--lp-territory-hue',String(hue));if(landMask)border.setAttribute('mask',landMask);borderPaint.push(border);}
if(!area.label)continue;const label=document.createElementNS(ns,'text');label.classList.add('lp-course-territory-label');label.setAttribute('x',area.label.x);label.setAttribute('y',area.label.y);label.setAttribute('text-anchor','middle');label.setAttribute('dominant-baseline','middle');label.dataset.lpCourse=area.course;label.setAttribute('aria-label',area.label.text);const lines=lpMapCourseLabelLines(area.label.text);lines.forEach((line,index)=>{const span=document.createElementNS(ns,'tspan');span.setAttribute('x',area.label.x);span.setAttribute('y',area.label.y+(index-(lines.length-1)/2)*32);span.textContent=line;label.appendChild(span);});labelPaint.push(label);}
if(parchmentRim)(land||layer).appendChild(parchmentRim);borderPaint.forEach(border=>(land||layer).appendChild(border));const labelTarget=paintOptions.labels||layer;unitLabelPaint.forEach(label=>labelTarget.appendChild(label));labelPaint.forEach(label=>labelTarget.appendChild(label));svg.insertBefore(layer,svg.firstChild);return layer;}
let __lpMotionAccessMemo=null;try{['mk-shop-inventory-change','mk-shop-trial-change','mk-account-login-change','mk-account-workspace-changed','storage'].forEach(name=>window.addEventListener(name,()=>{__lpMotionAccessMemo=null;}));}catch(_){}
function lpLocalMapMotionAccess(modal){const now=Date.now();const memo=__lpMotionAccessMemo;if(memo&&memo.modal===modal&&now-memo.at<30000)return memo.value;const value=lpLocalMapAnimationsEnabled(modal);__lpMotionAccessMemo={modal,at:now,value};return value;}
function lpWorldFootprint(world){const out={total:world.locs.length,seen:0,explored:0,claimed:0,cells:[],courses:new Map()};for(const loc of world.locs){const state=lpConceptExploreState(loc);if(!state.visited&&!state.rated)continue;out.seen++;if(state.explored)out.explored++;if(state.claimed)out.claimed++;out.cells.push({loc,state});const course=world.courseOf(loc);out.courses.set(course,(out.courses.get(course)||0)+1);}
return out;}
function lpWorldClaimsPaint(svg,world,footprint){if(!footprint||!footprint.claimed)return null;const ns='http://www.w3.org/2000/svg',half=world.cellWidth/2,rise=world.rowHeight/3;const byCourse=new Map();for(const{loc,state}of footprint.cells){const p=state.claimed&&world.point(loc);if(!p)continue;const course=world.courseOf(loc);const hex=`M${p.x} ${p.y - 2 * rise}L${p.x + half} ${p.y - rise}L${p.x + half} ${p.y + rise}L${p.x} ${p.y + 2 * rise}L${p.x - half} ${p.y + rise}L${p.x - half} ${p.y - rise}Z`;byCourse.set(course,(byCourse.get(course)||'')+hex);}
const group=document.createElementNS(ns,'g');group.classList.add('lp-world-claims');group.setAttribute('aria-hidden','true');for(const[course,d]of byCourse){const path=document.createElementNS(ns,'path');path.classList.add('lp-world-claim');path.setAttribute('d',d);path.dataset.lpCourse=course;path.style.setProperty('--lp-territory-hue',String(world.hueOf(course)));group.appendChild(path);}
svg.insertBefore(group,svg.firstChild);return group;}
function lpCssColorRgb(value){const text=String(value||'').trim();let m=/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.exec(text);if(m){const hex=m[1].length===3?m[1].split('').map(c=>c+c).join(''):m[1];return[0,2,4].map(i=>parseInt(hex.slice(i,i+2),16));}
m=/^rgba?\(\s*([\d.]+)[\s,]+([\d.]+)[\s,]+([\d.]+)/i.exec(text);return m?[m[1],m[2],m[3]].map(Number):null;}
const LP_WORLD_FOG_SCALE=1/8;function lpWorldFogHole(target,point,ox,oy,strength,rx,ry){if(!point)return;const f=LP_WORLD_FOG_SCALE,r=rx*f;target.save();target.translate((point.x-ox)*f,(point.y-oy)*f);target.scale(1,ry/rx);const g=target.createRadialGradient(0,0,0,0,0,r);g.addColorStop(0,`rgba(0,0,0,${strength})`);g.addColorStop(.5,`rgba(0,0,0,${strength})`);g.addColorStop(1,'rgba(0,0,0,0)');target.fillStyle=g;target.beginPath();target.arc(0,0,r,0,Math.PI*2);target.fill();target.restore();}
function lpWorldFogPaint(modal,vp,world,footprint,shownLocs,cur){let canvas=vp.querySelector(':scope > canvas.lp-world-fog');if(!footprint){if(canvas)canvas.remove();return null;}
if(!canvas){canvas=document.createElement('canvas');canvas.className='lp-world-fog';canvas.setAttribute('aria-hidden','true');const base=vp.querySelector(':scope > svg.lp-map-svg:not(.lp-map-terrain)');vp.insertBefore(canvas,base?base.nextSibling:vp.firstChild);}
const f=LP_WORLD_FOG_SCALE;const width=Math.max(1,Math.ceil(world.width*f)),height=Math.max(1,Math.ceil(world.height*f));const css=getComputedStyle(modal);const rgb=lpCssColorRgb(css.getPropertyValue('--atlas-backing'))||lpCssColorRgb(css.getPropertyValue('--wiki-map-canvas'))||[12,24,36];const colour=String(css.getPropertyValue('--lp-world-fog')||'').trim()||`rgba(${rgb.join(',')},.8)`;let base=canvas.__lpFogBase;if(!base||base.footprint!==footprint||base.colour!==colour||base.width!==width||base.height!==height){const layer=document.createElement('canvas');layer.width=width;layer.height=height;const bctx=layer.getContext('2d');if(!bctx)return canvas;bctx.fillStyle=colour;bctx.fillRect(0,0,width,height);bctx.globalCompositeOperation='destination-out';for(const{loc,state}of footprint.cells)lpWorldFogHole(bctx,world.point(loc),0,0,state.claimed?1:state.explored?.9:.65,320,170);lpWorldFogHole(bctx,world.point(cur),0,0,1,380,200);base=canvas.__lpFogBase={footprint,colour,width,height,layer};}
canvas.__lpFogState={world,base,colour,shown:Array.from(shownLocs||[],loc=>world.point(loc)).filter(Boolean)};if(vp.__lpWorldWindow)lpWorldFogDraw(canvas,vp.__lpWorldWindow);return canvas;}
function lpWorldFogDraw(canvas,win){const state=canvas&&canvas.__lpFogState;if(!state||!win)return;const f=LP_WORLD_FOG_SCALE,base=state.base;const density=win.gesture?.5:1;const fw=Math.round(win.w*f),fh=Math.round(win.h*f);const cw=Math.max(1,Math.round(fw*density)),ch=Math.max(1,Math.round(fh*density));if(canvas.width!==cw)canvas.width=cw;if(canvas.height!==ch)canvas.height=ch;lpWorldWindowPlace(canvas,win);const ctx=canvas.getContext('2d');if(!ctx)return;ctx.setTransform(1,0,0,1,0,0);ctx.globalCompositeOperation='source-over';ctx.clearRect(0,0,cw,ch);ctx.setTransform(density,0,0,density,0,0);ctx.fillStyle=state.colour;ctx.fillRect(0,0,fw,fh);const ox=Math.round(win.x*f),oy=Math.round(win.y*f);const sx=Math.max(0,ox),sy=Math.max(0,oy);const ex=Math.min(base.width,ox+fw),ey=Math.min(base.height,oy+fh);if(ex>sx&&ey>sy){ctx.clearRect(sx-ox,sy-oy,ex-sx,ey-sy);ctx.drawImage(base.layer,sx,sy,ex-sx,ey-sy,sx-ox,sy-oy,ex-sx,ey-sy);}
ctx.globalCompositeOperation='destination-out';const x0=win.x-320,y0=win.y-200,x1=win.x+win.w+320,y1=win.y+win.h+200;for(const p of state.shown){if(p.x<x0||p.x>x1||p.y<y0||p.y>y1)continue;lpWorldFogHole(ctx,p,win.x,win.y,.5,230,120);}
ctx.globalCompositeOperation='source-over';}
function lpWorldLabelsSync(modal,world,footprint){const layer=modal.__lpWorldTerrain&&modal.__lpWorldTerrain.labels;if(!layer)return;layer.querySelectorAll('.lp-course-territory-label').forEach(label=>{const course=label.dataset.lpCourse||'';label.dataset.lpExplored=footprint&&!(footprint.courses.get(course)>0)?'0':'1';});lpWorldLabelScale(modal,window.__lpMapState&&window.__lpMapState.scale);}
function lpWorldLabelScale(modal,scale){const layer=modal&&modal.__lpWorldTerrain&&modal.__lpWorldTerrain.labels;if(!layer||!layer.isConnected)return;if(layer.__lpVisibleLabelsPlaced)return;const text=Math.max(1,Math.min(4,.62/(Number(scale)||1))).toFixed(2);if(layer.__lpLabelScale===text)return;layer.__lpLabelScale=text;layer.style.setProperty('--lp-world-label-scale',text);}
function lpVisibleCoursePathBounds(area){if(area.__lpLabelBounds)return area.__lpLabelBounds;const numbers=String(area.d||'').match(/-?(?:\d+\.?\d*|\.\d+)(?:e[-+]?\d+)?/gi)||[];let left=Infinity,right=-Infinity,top=Infinity,bottom=-Infinity;for(let i=0;i+1<numbers.length;i+=2){const x=Number(numbers[i]),y=Number(numbers[i+1]);if(!Number.isFinite(x)||!Number.isFinite(y))continue;left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}
area.__lpLabelBounds=Number.isFinite(left)?{left,right,top,bottom}:area.bounds;return area.__lpLabelBounds;}
function lpVisibleCourseLabelPlacement(areas,reveal,camera,ctx,maxProbes=1500){if(!camera||!ctx||typeof Path2D!=='function')return[];const budget=Math.max(24,Math.floor(Number(maxProbes)||1500));const scale=Math.max(.001,Number(camera.scale)||1);const viewport={left:-(Number(camera.tx)||0)/scale,top:-(Number(camera.ty)||0)/scale};viewport.right=viewport.left+(Number(camera.stageW)||0)/scale;viewport.bottom=viewport.top+(Number(camera.stageH)||0)/scale;if(!(viewport.right>viewport.left&&viewport.bottom>viewport.top))return[];const visible=(point,points,segments)=>{if(!reveal)return true;const radius=reveal.radius||180,corridor=reveal.corridorRadius||90;for(const p of points)if(Math.hypot(point.x-p.x,point.y-p.y)<=radius)return true;for(const seg of segments){const dx=seg.b.x-seg.a.x,dy=seg.b.y-seg.a.y;const t=Math.max(0,Math.min(1,((point.x-seg.a.x)*dx+(point.y-seg.a.y)*dy)/(dx*dx+dy*dy||1)));if(Math.hypot(point.x-seg.a.x-t*dx,point.y-seg.a.y-t*dy)<=corridor)return true;}
return false;};const placements=[];for(const area of areas||[]){if(!area.label||!area.bounds||!area.d)continue;const b=lpVisibleCoursePathBounds(area);const left=Math.max(viewport.left,area.bounds.left,b.left),right=Math.min(viewport.right,area.bounds.right,b.right);const top=Math.max(viewport.top,area.bounds.top,b.top),bottom=Math.min(viewport.bottom,area.bounds.bottom,b.bottom);if(!(right>left&&bottom>top))continue;const radius=reveal&&(reveal.radius||180),corridor=reveal&&(reveal.corridorRadius||90);const nearPoints=reveal?(reveal.points||[]).filter(p=>p.x+radius>=left&&p.x-radius<=right&&p.y+radius>=top&&p.y-radius<=bottom):[];const nearSegments=reveal?(reveal.segments||[]).filter(seg=>Math.max(seg.a.x,seg.b.x)+corridor>=left&&Math.min(seg.a.x,seg.b.x)-corridor<=right&&Math.max(seg.a.y,seg.b.y)+corridor>=top&&Math.min(seg.a.y,seg.b.y)-corridor<=bottom):[];if(reveal&&!nearPoints.length&&!nearSegments.length)continue;const path=area.__lpLabelPath||(area.__lpLabelPath=new Path2D(area.d));const pixelW=(right-left)*scale,pixelH=(bottom-top)*scale;const primaryBudget=Math.max(9,Math.floor(budget*.8));let columns=Math.max(3,Math.ceil(pixelW/18)),rows=Math.max(3,Math.ceil(pixelH/18));if(columns*rows>primaryBudget){const shrink=Math.sqrt(columns*rows/primaryBudget);columns=Math.max(3,Math.floor(columns/shrink));rows=Math.max(3,Math.floor(rows/shrink));while(columns*rows>primaryBudget){if(columns>=rows&&columns>3)columns--;else if(rows>3)rows--;else break;}}
const dx=(right-left)/columns,dy=(bottom-top)/rows;let count=0,sumX=0,sumY=0,minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity;const samples=[];const add=point=>{if(!ctx.isPointInPath(path,point.x,point.y,'evenodd')||!visible(point,nearPoints,nearSegments))return;samples.push(point);sumX+=point.x;sumY+=point.y;count++;minX=Math.min(minX,point.x);maxX=Math.max(maxX,point.x);minY=Math.min(minY,point.y);maxY=Math.max(maxY,point.y);};for(let row=0;row<rows;row++)for(let col=0;col<columns;col++){add({x:left+(col+.5)*dx,y:top+(row+.5)*dy});}
const fallbackOnly=!count;if(fallbackOnly){const remaining=budget-columns*rows;const perEdge=Math.floor(remaining/4);const insetX=Math.min((right-left)/4,.5/scale);const insetY=Math.min((bottom-top)/4,.5/scale);for(let i=0;i<perEdge&&!count;i++){const t=(i+.5)/perEdge;add({x:left+insetX,y:top+(bottom-top)*t});add({x:right-insetX,y:top+(bottom-top)*t});add({x:left+(right-left)*t,y:top+insetY});add({x:left+(right-left)*t,y:bottom-insetY});}}
if(!count)continue;const centroid={x:sumX/count,y:sumY/count};let anchor=samples[0],best=Infinity;for(const point of samples){const distance=(point.x-centroid.x)**2+(point.y-centroid.y)**2;if(distance<best){best=distance;anchor=point;}}
const areaPx=fallbackOnly?Math.min(900,count*dx*dy*scale*scale):count*dx*dy*scale*scale;const lines=typeof lpMapCourseLabelLines==='function'?lpMapCourseLabelLines(area.label.text):[String(area.label.text||'')];let lineEm=Math.max(...lines.map(line=>String(line).length*.55),1);if(typeof ctx.measureText==='function'){ctx.font='600 20px Georgia';lineEm=Math.max(...lines.map(line=>ctx.measureText(line).width/20),1);}
const shownWidth=Math.max(dx,maxX-minX+dx)*scale;const shownHeight=Math.max(dy,maxY-minY+dy)*scale;const fitWidth=shownWidth*.9/lineEm;const fitHeight=shownHeight*.9/(lines.length*1.08);const edgeWidth=Math.max(0,2*Math.min(anchor.x-viewport.left,viewport.right-anchor.x)*scale-12);const edgeHeight=Math.max(0,2*Math.min(anchor.y-viewport.top,viewport.bottom-anchor.y)*scale-12);const fontPx=Math.max(9,Math.min(64,8+Math.sqrt(areaPx)*.075,fitWidth,fitHeight,edgeWidth/lineEm,edgeHeight/(lines.length*1.08)));const anchorPx=(anchor.x-viewport.left)*scale;const lineWidthPx=fontPx*lineEm;const textAnchor=anchorPx-lineWidthPx/2<6?'start':anchorPx+lineWidthPx/2>camera.stageW-6?'end':'middle';placements.push({course:area.course,x:anchor.x,y:anchor.y,fontPx,areaPx,textAnchor});}
return placements;}
function lpWorldCourseLabelPlacement(areas,reveal,ctx,maxProbes=1500,fontFamily='Georgia',fontStyle='normal',fontWeight='600'){if(!reveal||!reveal.bounds)return(areas||[]).filter(area=>area.label).map(area=>({course:area.course,x:area.label.x,y:area.label.y,fontWorld:26,lines:lpMapCourseLabelLines(area.label.text),areaPx:0}));const b=reveal.bounds,camera={scale:1,tx:-b.left,ty:-b.top,stageW:b.right-b.left,stageH:b.bottom-b.top};const rough=new Map(lpVisibleCourseLabelPlacement(areas,reveal,camera,ctx,maxProbes).map(p=>[p.course,p]));const solid=point=>{const radius=Math.max(0,reveal.radius-reveal.feather*.5-2);const corridor=Math.max(0,reveal.corridorRadius-reveal.feather*.5-2);for(const p of reveal.points||[])if((point.x-p.x)**2+(point.y-p.y)**2<=radius**2)return true;for(const seg of reveal.segments||[]){const dx=seg.b.x-seg.a.x,dy=seg.b.y-seg.a.y;const t=Math.max(0,Math.min(1,((point.x-seg.a.x)*dx+(point.y-seg.a.y)*dy)/(dx*dx+dy*dy||1)));if((point.x-seg.a.x-t*dx)**2+(point.y-seg.a.y-t*dy)**2<=corridor**2)return true;}
return false;};const out=[];for(const area of areas||[]){const initial=rough.get(area.course);if(!initial||!area.label||!area.d)continue;const path=area.__lpLabelPath||(area.__lpLabelPath=new Path2D(area.d));const edges=area.__lpLabelEdges||(area.__lpLabelEdges=(()=>{const out=[];let first=null,previous=null;for(const part of lpWorldPathSegments(area.d)){if(part.type==='M'){first=previous=part;continue;}
const next=part.type==='Z'?first:part;if(previous&&next)out.push([previous,next]);previous=next;}
return out;})());const words=String(area.label.text||'').trim().split(/\s+/);const ordinary=lpMapCourseLabelLines(area.label.text);const variants=[ordinary];if(words.length>2)variants.push(words);let best=null;const offsets=[0,-32,32,-64,64];for(const lines of variants){for(let font=Math.min(64,Math.floor(initial.fontPx));font>=8;font-=2){ctx.font=`${fontStyle} ${fontWeight} ${font}px ${fontFamily}`;const widths=lines.map(line=>{const measured=typeof ctx.measureText==='function'?ctx.measureText(line):null;const width=measured?Math.max(measured.width,(measured.actualBoundingBoxLeft||0)+(measured.actualBoundingBoxRight||0)):line.length*font*.6;return width+line.length*1.5;});const halfW=Math.max(...widths)/2+3,halfH=lines.length*font*1.08/2+3;const fits=(x,y)=>{if(x-halfW<b.left||x+halfW>b.right||y-halfH<b.top||y+halfH>b.bottom)return false;const left=x-halfW,right=x+halfW,top=y-halfH,bottom=y+halfH;for(const[a,z]of edges){if(Math.max(a.x,z.x)<left||Math.min(a.x,z.x)>right||Math.max(a.y,z.y)<top||Math.min(a.y,z.y)>bottom)continue;let lo=0,hi=1,dx=z.x-a.x,dy=z.y-a.y;for(const[p,q]of[[-dx,a.x-left],[dx,right-a.x],[-dy,a.y-top],[dy,bottom-a.y]]){if(!p){if(q<0)hi=-1;}
else{const t=q/p;if(p<0)lo=Math.max(lo,t);else hi=Math.min(hi,t);}}
if(lo<=hi)return false;}
const cols=Math.max(2,Math.ceil(halfW*2/6)),rows=Math.max(2,Math.ceil(halfH*2/6));for(let row=0;row<=rows;row++)for(let col=0;col<=cols;col++){const point={x:x-halfW+col*2*halfW/cols,y:y-halfH+row*2*halfH/rows};if(!ctx.isPointInPath(path,point.x,point.y,'evenodd')||!solid(point))return false;}
return true;};for(const oy of offsets)for(const ox of offsets){const x=initial.x+ox,y=initial.y+oy;if(fits(x,y)){best={course:area.course,x,y,fontWorld:font,lines,areaPx:initial.areaPx};break;}}
if(best)break;}
if(best)break;}
if(best)out.push(best);}
return out;}
function lpWorldProvinceLabelPlacement(areas,reveal,ctx,courseLabels,nodePoints=[],fontFamily='Georgia',fontStyle='normal',fontWeight='600'){if(!ctx||typeof Path2D!=='function')return[];const intersects=(a,b,pad=0)=>a.left<b.right+pad&&a.right>b.left-pad&&a.top<b.bottom+pad&&a.bottom>b.top-pad;const obstacles=(nodePoints||[]).map(p=>({left:p.x-(p.width||340)/2,right:p.x+(p.width||340)/2,top:p.y-(p.height||68)/2,bottom:p.y+(p.height||68)/2}));for(const p of courseLabels||[]){if(!p||!p.lines)continue;ctx.font=`${fontStyle} ${fontWeight} ${p.fontWorld}px ${fontFamily}`;const width=Math.max(...p.lines.map(line=>Math.max(ctx.measureText(line).width,line.length*p.fontWorld*.75)+line.length*1.5))+20;const height=p.lines.length*p.fontWorld*1.08+12;obstacles.push({left:p.x-width/2,right:p.x+width/2,top:p.y-height/2,bottom:p.y+height/2});}
const solid=point=>{if(!reveal)return true;const radius=Math.max(0,reveal.radius-reveal.feather*.5-2);const corridor=Math.max(0,reveal.corridorRadius-reveal.feather*.5-2);for(const p of reveal.points||[])if((point.x-p.x)**2+(point.y-p.y)**2<=radius**2)return true;for(const seg of reveal.segments||[]){const dx=seg.b.x-seg.a.x,dy=seg.b.y-seg.a.y;const t=Math.max(0,Math.min(1,((point.x-seg.a.x)*dx+(point.y-seg.a.y)*dy)/(dx*dx+dy*dy||1)));if((point.x-seg.a.x-t*dx)**2+(point.y-seg.a.y-t*dy)**2<=corridor**2)return true;}
return false;};const placed=[];for(const area of areas||[])for(const unit of area.provinces||[]){const label=unit.label,bounds=unit.bounds;if(!label||!label.text||!bounds||!unit.d)continue;const path=unit.__lpUnitLabelPath||(unit.__lpUnitLabelPath=new Path2D(unit.d));const edges=unit.__lpUnitLabelEdges||(unit.__lpUnitLabelEdges=(()=>{const out=[];let first=null,previous=null;for(const part of lpWorldPathSegments(unit.d)){if(part.type==='M'){first=previous=part;continue;}
const next=part.type==='Z'?first:part;if(previous&&next)out.push([previous,next]);previous=next;}
return out;})());const near=(reveal&&reveal.points||[]).filter(p=>p.x>=bounds.left-180&&p.x<=bounds.right+180&&p.y>=bounds.top-180&&p.y<=bounds.bottom+180).sort((a,b)=>(a.x-label.x)**2+(a.y-label.y)**2-(b.x-label.x)**2-(b.y-label.y)**2).slice(0,6);const seeds=[{x:label.x,y:label.y},...near];const offsets=[[0,0],[0,-70],[0,70],[-85,0],[85,0],[-85,-70],[85,-70],[-85,70],[85,70],[0,-125],[0,125]];let found=null;for(const font of[Math.min(26,Number(label.fontSize)||26),22,18]){if(font<16||(found&&font<=found.fontWorld))continue;ctx.font=`normal 600 ${font}px ${fontFamily}`;const width=Math.max(ctx.measureText(label.text).width,label.text.length*font*.75)+label.text.length*.5+16;const height=font*1.3+16;for(const seed of seeds)for(const[dx,dy]of offsets){const x=seed.x+dx,y=seed.y+dy;const box={left:x-width/2,right:x+width/2,top:y-height/2,bottom:y+height/2};if(box.left<bounds.left||box.right>bounds.right||box.top<bounds.top||box.bottom>bounds.bottom||(reveal&&(box.left<reveal.bounds.left||box.right>reveal.bounds.right||box.top<reveal.bounds.top||box.bottom>reveal.bounds.bottom))||obstacles.some(o=>intersects(box,o,8)))continue;let crosses=false;for(const[a,b]of edges){if(Math.max(a.x,b.x)<box.left||Math.min(a.x,b.x)>box.right||Math.max(a.y,b.y)<box.top||Math.min(a.y,b.y)>box.bottom)continue;let lo=0,hi=1;const ex=b.x-a.x,ey=b.y-a.y;for(const[p,q]of[[-ex,a.x-box.left],[ex,box.right-a.x],[-ey,a.y-box.top],[ey,box.bottom-a.y]]){if(!p){if(q<0)hi=-1;}
else{const t=q/p;if(p<0)lo=Math.max(lo,t);else hi=Math.min(hi,t);}}
if(lo<=hi){crosses=true;break;}}
if(crosses||!ctx.isPointInPath(path,x,y,'evenodd'))continue;const cols=Math.max(2,Math.ceil(width/8)),rows=Math.max(2,Math.ceil(height/8));let valid=true;for(let row=0;row<=rows&&valid;row++)for(let col=0;col<=cols;col++){const point={x:box.left+col*width/cols,y:box.top+row*height/rows};if(!ctx.isPointInPath(path,point.x,point.y,'evenodd')||!solid(point)){valid=false;break;}}
if(valid){found={id:unit.id,x,y,fontWorld:font,box};break;}}
if(found)break;}
if(found){placed.push(found);obstacles.push(found.box);}}
return placed;}
function lpWorldLabelsPlace(vp,camera){const modal=vp&&vp.__lpWorldLabelModal;const terrain=modal&&modal.__lpWorldTerrain;if(!terrain||!terrain.labels||!terrain.labels.isConnected||!terrain.world)return;const ctx=terrain.__lpLabelHitContext||(terrain.__lpLabelHitContext=document.createElement('canvas').getContext('2d'));if(!ctx)return;const labelNode=terrain.labels.querySelector('.lp-course-territory-label');const style=labelNode&&typeof getComputedStyle==='function'?getComputedStyle(labelNode):null;const fontFamily=style&&style.fontFamily||'Georgia';const fontStyle=style&&style.fontStyle||'normal',fontWeight=style&&style.fontWeight||'600';const fontKey=`${terrain.world.revealKey || ''}|${terrain.labelScopeKey || ''}|${fontFamily}|${fontStyle}|${fontWeight}|${style && style.letterSpacing || ''}`;if(terrain.__lpLabelLayoutKey===fontKey){terrain.labels.__lpVisibleLabelsPlaced=true;return;}
const touch=!!(window.matchMedia&&window.matchMedia('(pointer: coarse), (max-width: 900px)').matches);const coursePlaces=lpWorldCourseLabelPlacement(terrain.world.areas,terrain.world.reveal,ctx,touch?700:1500,fontFamily,fontStyle,fontWeight);const placed=new Map(coursePlaces.map(p=>[p.course,p]));for(const label of terrain.labels.querySelectorAll('.lp-course-territory-label')){const p=placed.get(label.dataset.lpCourse);label.style.display=p?'':'none';if(!p)continue;const fontSize=p.fontWorld;label.style.fontSize=`${fontSize}px`;label.setAttribute('text-anchor','middle');label.setAttribute('x',String(p.x));label.setAttribute('y',String(p.y));while(label.querySelectorAll('tspan').length<p.lines.length){const span=document.createElementNS('http://www.w3.org/2000/svg','tspan');label.appendChild(span);}
while(label.querySelectorAll('tspan').length>p.lines.length)label.lastElementChild.remove();const spans=label.querySelectorAll('tspan');spans.forEach((span,i)=>{span.setAttribute('x',String(p.x));span.setAttribute('y',String(p.y+(i-(spans.length-1)/2)*fontSize*1.08));span.textContent=p.lines[i];});label.dataset.lpVisibleArea=String(Math.round(p.areaPx));label.dataset.lpVisibleFont=String(p.fontWorld);}
const domPoints=typeof vp.querySelectorAll==='function'?Array.from(vp.querySelectorAll(':scope > .lp-node')).map(node=>({x:parseFloat(node.style.left),y:parseFloat(node.style.top),width:node.offsetWidth||340,height:node.offsetHeight||68})).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y)):[];const provincePlaces=new Map(lpWorldProvinceLabelPlacement(terrain.world.areas,terrain.world.reveal,ctx,coursePlaces,domPoints.length?domPoints:terrain.labelPoints||terrain.world.reveal?.points||[],fontFamily,fontStyle,fontWeight).map(p=>[p.id,p]));for(const label of terrain.labels.querySelectorAll('.lp-unit-territory-label')){const p=provincePlaces.get(label.dataset.lpUnit);label.style.display=p?'':'none';if(!p)continue;label.style.fontSize=`${p.fontWorld}px`;label.setAttribute('x',String(p.x));label.setAttribute('y',String(p.y));}
terrain.labels.__lpVisibleLabelsPlaced=true;terrain.__lpLabelLayoutKey=fontKey;}
function lpWorldExploreStatus(modal,footprint){const treasure=modal.querySelector('[data-lp-map-treasure-note]');if(treasure){const note='Treasure: expand a concept to reveal new neighbours for a chance to find a chest. See Map tips.';if(treasure.textContent!==note)treasure.textContent=note;}
const status=modal.querySelector('[data-lp-map-explore-status]');if(!status)return;status.hidden=!footprint;const text=footprint?`Knowledge masking: ${footprint.explored} explored · ${footprint.claimed} claimed of ${footprint.total} concepts.`:'';if(status.textContent!==text)status.textContent=text;}
function lpMountWorldTerrain(modal,vp,world,width,height){let terrain=modal.__lpWorldTerrain;if(!terrain||terrain.world!==world||terrain.width!==width||terrain.height!==height){const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('viewBox',`0 0 ${width} ${height}`);svg.setAttribute('width',String(width));svg.setAttribute('height',String(height));svg.setAttribute('aria-hidden','true');svg.classList.add('lp-map-svg','lp-map-terrain');const labels=document.createElementNS('http://www.w3.org/2000/svg','svg');labels.setAttribute('viewBox',`0 0 ${width} ${height}`);labels.setAttribute('width',String(width));labels.setAttribute('height',String(height));labels.setAttribute('aria-hidden','true');labels.classList.add('lp-world-labels');lpMapPaintTerritories(svg,world.areas,{groupMask:true,simple:true,labels,reveal:world.reveal});terrain=modal.__lpWorldTerrain={world,width,height,svg,labels};}
vp.insertBefore(terrain.svg,vp.firstChild);vp.insertBefore(terrain.labels,terrain.svg.nextSibling);vp.__lpWorldLabelModal=modal;terrain.labels.__lpVisibleLabelsPlaced=false;vp.__lpWorldWindow=null;return terrain.svg;}
function lpFrameCameraQueue(apply){let raf=0;const cancel=()=>{if(raf)cancelAnimationFrame(raf);raf=0;};return{schedule(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;apply();});},flush(settle){cancel();(settle||apply)();},cancel};}
const LP_WORLD_WINDOW_MARGIN=.25;const LP_WORLD_WINDOW_RESCALE=1.6;function lpWorldWindowLayers(vp){if(!vp)return[];const selector=vp.__lpWorldWindowScope==='route'?':scope > svg.lp-map-terrain, :scope > svg.lp-world-labels, :scope > svg.lp-h1sg-overlay':':scope > svg.lp-map-terrain, :scope > svg.lp-world-labels, :scope > svg.lp-map-svg, :scope > svg.lp-map-svg-hi';return Array.from(vp.querySelectorAll(selector));}
function lpWorldWindowFor(camera){const s=Math.max(.001,Number(camera.scale)||1),tx=Number(camera.tx)||0,ty=Number(camera.ty)||0;const vw=Math.max(1,Number(camera.stageW)||1)/s,vh=Math.max(1,Number(camera.stageH)||1)/s;const vx=-tx/s,vy=-ty/s,m=LP_WORLD_WINDOW_MARGIN,grid=1/LP_WORLD_FOG_SCALE;const x=Math.floor((vx-vw*m)/grid)*grid,y=Math.floor((vy-vh*m)/grid)*grid;const w=Math.ceil((vx+vw*(1+m)-x)/grid)*grid,h=Math.ceil((vy+vh*(1+m)-y)/grid)*grid;return{s,vx,vy,vw,vh,x,y,w,h};}
function lpWorldWindowPlace(el,win){const style=el.style,round=v=>Math.round(v*1000)/1000;style.inset='auto';style.left='0px';style.top='0px';style.width=`${Math.ceil(win.w * win.s)}px`;style.height=`${Math.ceil(win.h * win.s)}px`;style.transformOrigin='0 0';style.transform=`translate(${round(win.x)}px, ${round(win.y)}px) scale(${round(1 / win.s)})`;}
function lpWorldWindowMasks(svg,win){if(!svg||!win||typeof svg.querySelectorAll!=='function')return;for(const mask of svg.querySelectorAll('mask[maskUnits="userSpaceOnUse"], filter[data-lp-window-filter]')){let box=mask.__lpWorldMaskBounds;if(!box){const raw=['x','y','width','height'].map(key=>mask.getAttribute(key));if(raw.some(value=>value==null||!Number.isFinite(Number(value))))continue;const[x,y,w,h]=raw.map(Number);box=mask.__lpWorldMaskBounds={x,y,w,h};}
const pad=mask.getAttribute('data-lp-window-filter')!=null?36:0;const x=Math.max(box.x,win.x-pad),y=Math.max(box.y,win.y-pad);mask.setAttribute('x',String(x));mask.setAttribute('y',String(y));mask.setAttribute('width',String(Math.max(0,Math.min(box.x+box.w,win.x+win.w+pad)-x)));mask.setAttribute('height',String(Math.max(0,Math.min(box.y+box.h,win.y+win.h+pad)-y)));}}
function lpWorldWindowCommit(vp,camera,gesture=false){if(!vp)return null;const rasterCamera=gesture?{...camera,scale:camera.scale*.5,tx:camera.stageW*.25+camera.tx*.5,ty:camera.stageH*.25+camera.ty*.5}:camera;const win=lpWorldWindowFor(rasterCamera);win.gesture=gesture;win.cameraScale=Number(camera.scale)||1;win.sharpScale=gesture&&vp.__lpWorldWindow?(vp.__lpWorldWindow.sharpScale||vp.__lpWorldWindow.cameraScale||vp.__lpWorldWindow.s):win.cameraScale;for(const el of lpWorldWindowLayers(vp)){el.setAttribute('viewBox',`${win.x} ${win.y} ${win.w} ${win.h}`);el.setAttribute('width',String(Math.ceil(win.w*win.s)));el.setAttribute('height',String(Math.ceil(win.h*win.s)));el.style.overflow='hidden';el.style.contain='paint';lpWorldWindowPlace(el,win);lpWorldWindowMasks(el,win);}
const fog=vp.querySelector(':scope > canvas.lp-world-fog');if(fog)lpWorldFogDraw(fog,win);vp.__lpWorldWindow=win;return win;}
function lpWorldWindowCull(vp,win,revealOnly=false){if(!vp||!win)return;const near=260,far=640;const outside=(x0,y0,x1,y1,pad)=>x1<win.x-pad||y1<win.y-pad||x0>win.x+win.w+pad||y0>win.y+win.h+pad;for(const node of vp.querySelectorAll(':scope > a.lp-node[data-lp-loc]')){const x=parseFloat(node.style.left),y=parseFloat(node.style.top);if(!Number.isFinite(x)||!Number.isFinite(y))continue;const hidden=node.classList.contains('lp-node-offscreen');if(!revealOnly&&!hidden&&outside(x,y,x,y,far))node.classList.add('lp-node-offscreen');else if(hidden&&!outside(x,y,x,y,near))node.classList.remove('lp-node-offscreen');}
for(const path of vp.querySelectorAll(':scope > svg.lp-map-svg:not(.lp-map-terrain) path.lp-edge')){const box=path.__lpBox;if(!box)continue;const hidden=path.classList.contains('lp-edge-offscreen');if(!revealOnly&&!hidden&&outside(box[0],box[1],box[2],box[3],far))path.classList.add('lp-edge-offscreen');else if(hidden&&!outside(box[0],box[1],box[2],box[3],near))path.classList.remove('lp-edge-offscreen');}}
function lpWorldWindowFollow(vp,readCamera){if(!vp||!vp.isConnected||typeof readCamera!=='function')return;const camera=readCamera();if(!camera)return;const win=vp.__lpWorldWindow,now=lpWorldWindowFor(camera);const held=lpLocalMapPointerHeld();const ratio=win?now.s/(win.cameraScale||win.s):0,slack=win?Math.min(win.w,win.h)*.02:0;const resized=!!win&&(Math.abs(now.vw*now.s-win.vw*win.s)>1||Math.abs(now.vh*now.s-win.vh*win.s)>1);const inside=!!win&&now.vx>=win.x+slack&&now.vy>=win.y+slack&&now.vx+now.vw<=win.x+win.w-slack&&now.vy+now.vh<=win.y+win.h-slack;if(resized||!inside||(!held&&(ratio>LP_WORLD_WINDOW_RESCALE||ratio<1/LP_WORLD_WINDOW_RESCALE))){const next=lpWorldWindowCommit(vp,camera,held);if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,next,true);}
vp.__lpWorldWindowTouched=lpLocalMapInteractionNow();vp.__lpWorldReadCamera=readCamera;if(vp.__lpWorldWindowTimer)return;const settle=()=>{vp.__lpWorldWindowTimer=0;if(!vp.isConnected)return;const wait=160-(lpLocalMapInteractionNow()-vp.__lpWorldWindowTouched);if(lpLocalMapPointerHeld()||wait>0){vp.__lpWorldWindowTimer=setTimeout(settle,Math.max(16,wait>0?wait:160));return;}
const live=vp.__lpWorldReadCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||Math.abs(lpWorldWindowFor(live).s/(committed.sharpScale||committed.s)-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);if(vp.__lpWorldLabelModal&&typeof lpWorldLabelsPlace==='function')lpWorldLabelsPlace(vp,live);};vp.__lpWorldWindowTimer=setTimeout(settle,160);}
function lpLocalMapUpdateStageSize(modal,st,width,height){const oldW=Number(st.__lpStageW)||0,oldH=Number(st.__lpStageH)||0;const nextW=Number(width)||0,nextH=Number(height)||0;if(!(nextW>0)||!(nextH>0)||(oldW===nextW&&oldH===nextH))return false;if(oldW>0&&oldH>0&&st.userMoved&&!st.__lpForceCenter){st.tx=(Number(st.tx)||0)+(nextW-oldW)/2;st.ty=(Number(st.ty)||0)+(nextH-oldH)/2;}
st.__lpStageW=nextW;st.__lpStageH=nextH;return true;}
function lpLocalMapResizeCamera(modal){if(!modal||modal.style.display==='none'||!modal.classList.contains('lp-open'))return false;const stage=modal.querySelector('.lp-mapstage'),st=window.__lpMapState;if(!stage||!st)return false;const rect=stage.getBoundingClientRect();const width=Math.floor(Number(stage.clientWidth)||Number(rect.width)||0);const height=Math.floor(Number(stage.clientHeight)||Number(rect.height)||0);if(!lpLocalMapUpdateStageSize(modal,st,width,height))return false;if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();if(st.__lpForceCenter||!st.userMoved)lpLocalMapResetView(modal,st);if(typeof modal.__lpApplyTransform==='function')modal.__lpApplyTransform({skipClamp:true});return true;}
function lpBindLocalMapViewportResize(modal){if(!modal||modal.__lpViewportResizeCleanup)return;const stage=modal.querySelector('.lp-mapstage');if(!stage)return;let raf=0,observer=null,disposed=false;const schedule=()=>{if(disposed||raf||modal.style.display==='none'||!modal.classList.contains('lp-open'))return;raf=requestAnimationFrame(()=>{raf=0;lpLocalMapResizeCamera(modal);});};if(typeof ResizeObserver==='function'){observer=new ResizeObserver(schedule);observer.observe(stage);}
window.addEventListener('resize',schedule,{passive:true});window.addEventListener('orientationchange',schedule,{passive:true});modal.__lpViewportResizeCleanup=()=>{disposed=true;if(raf)cancelAnimationFrame(raf);raf=0;if(observer)observer.disconnect();window.removeEventListener('resize',schedule);window.removeEventListener('orientationchange',schedule);modal.__lpViewportResizeCleanup=null;};}
function lpBindMapOptions(modal){const box=modal&&modal.querySelector('[data-lp-map-options]');if(!box)return;const fog=box.querySelector('[data-lp-fog-switch]');if(fog)fog.checked=lpFogEnabled();if(box.dataset.lpBound==='1')return;box.dataset.lpBound='1';if(fog)fog.addEventListener('change',()=>lpSetFogEnabled(!!fog.checked));box.addEventListener('pointerdown',event=>event.stopPropagation());}
function lpAttachMapChests(modal){if(!modal||!window.MkMapChests||window.__mkExamMode)return null;const stage=modal.querySelector('.lp-mapstage');const st=window.__lpMapState;if(!stage||!st||!st.__lpExploration)return null;const ui=window.MkMapChests.attach({modal,stage,ownerKey:lpCanonKey(currentRelPath()),exploration:st.__lpExploration,projectPoint(point){const engine=modal.__lpWebgl3dEngine;const p=engine&&modal.classList.contains('lp-webgl3d')?lpWebgl3dProject(engine,{...point,z:0}):point;const scale=Number(st.scale)||1;return{x:p.x*scale+(Number(st.tx)||0),y:p.y*scale+(Number(st.ty)||0)};},unprojectPoint(screen){const scale=Number(st.scale)||1;let p={x:(screen.x-(Number(st.tx)||0))/scale,y:(screen.y-(Number(st.ty)||0))/scale};for(let i=0;i<16;i++){const q=this.projectPoint(p),dx=screen.x-q.x,dy=screen.y-q.y;if(Math.hypot(dx,dy)<.05)return p;const x=this.projectPoint({x:p.x+1,y:p.y}),y=this.projectPoint({x:p.x,y:p.y+1});const a=x.x-q.x,b=y.x-q.x,c=x.y-q.y,d=y.y-q.y,det=a*d-b*c;if(!Number.isFinite(det)||Math.abs(det)<1e-8)return null;p={x:p.x+(d*dx-b*dy)/det,y:p.y+(a*dy-c*dx)/det};}
return null;},getView(nodeId){const box=stage.getBoundingClientRect();const safe=lpLocalMapSafeInsets(modal,box.width,box.height);const visual=window.visualViewport;const left=visual?Number(visual.offsetLeft)||0:0;const top=visual?Number(visual.offsetTop)||0:0;const width=Number(visual&&visual.width)||Number(window.innerWidth)||box.width;const height=Number(visual&&visual.height)||Number(window.innerHeight)||box.height;const bounds={left:Math.max(safe.left,left-box.left+8),right:Math.min(box.width-safe.right,left+width-box.left-8),top:Math.max(safe.usableTop+8,top-box.top+8),bottom:Math.min(safe.usableBottom-8,top+height-box.top-8)};if(bounds.right<=bounds.left||bounds.bottom<=bounds.top)return null;const occupied=[];let anchor=null;modal.querySelectorAll('.lp-node[data-lp-loc], .lp-course-territory-label, .lp-node-action:not([hidden]), .lp-mctrl, .lp-mzoom, .lp-view-toggle, .lp-3d-rotctl').forEach(el=>{if(el.hidden||el.isConnected===false)return;const r=el.getBoundingClientRect();if(!(r.width>0&&r.height>0))return;const rect={left:r.left-box.left,right:r.right-box.left,top:r.top-box.top,bottom:r.bottom-box.top};occupied.push(rect);if(nodeId&&el.dataset&&lpCanonKey(el.dataset.lpLoc)===lpCanonKey(nodeId))anchor={x:(rect.left+rect.right)/2,y:(rect.top+rect.bottom)/2};});return{bounds,occupied,anchor};}});modal.__lpMapChests=ui;return ui;}
function lpLocalExpansionMotionAllowed(modal){return!document.documentElement.classList.contains('mk-motion-off')&&!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)&&!modal.classList.contains('lp-webgl3d')&&!modal.classList.contains('lp-map-3d');}
function lpCaptureLocalExpansion(modal,vp,positions){if(!lpLocalExpansionMotionAllowed(modal)||typeof vp.animate!=='function')return null;const layers=lpWorldWindowLayers(vp).filter(el=>!el.classList.contains('lp-map-svg-hi')).map(el=>{const copy=el.cloneNode(true),ids=new Map();for(const item of[copy,...copy.querySelectorAll('[id]')])if(item.id){const id=item.id;ids.set(id,`${id}-expansion-old`);item.id=ids.get(id);}
for(const item of[copy,...copy.querySelectorAll('*')])for(const attr of Array.from(item.attributes)){let value=attr.value;for(const[id,replacement]of ids)value=value.split(`url(#${id})`).join(`url(#${replacement})`);if((attr.name==='href'||attr.name==='xlink:href')&&ids.has(value.slice(1))&&value[0]==='#')value=`#${ids.get(value.slice(1))}`;if(value!==attr.value)item.setAttribute(attr.name,value);}
copy.setAttribute('aria-hidden','true');copy.classList.add('lp-expansion-old');return copy;});return{positions:new Map(positions),layers};}
function lpAnimateLocalExpansion(modal,vp,st,snapshot,positions,nodes,current,anchor){const target=positions.get(anchor);if(!target)return;if(!snapshot||!lpLocalExpansionMotionAllowed(modal)){modal.__lpApplyTransform?.({skipClamp:true});return;}
const oldCurrent=snapshot.positions.get(current),newCurrent=positions.get(current);const dx=oldCurrent&&newCurrent?newCurrent.x-oldCurrent.x:0;const dy=oldCurrent&&newCurrent?newCurrent.y-oldCurrent.y:0;const animations=[];let raf=0,start=null,done=false;const animate=(el,frames,opts)=>animations.push(el.animate(frames,{duration:1350,easing:'cubic-bezier(.3,.5,.3,1)',...opts}));const layers=lpWorldWindowLayers(vp);for(const old of snapshot.layers){old.style.translate=`${dx}px ${dy}px`;vp.appendChild(old);animate(old,[{opacity:1},{opacity:0}],{fill:'forwards'});}
const win=vp.__lpWorldWindow;const touchSurface=!!(window.matchMedia&&window.matchMedia('(pointer: coarse), (max-width: 900px)').matches);for(const layer of layers){const frames=[{opacity:0},{opacity:1}];if(!touchSurface&&win&&(layer.classList.contains('lp-map-terrain')||layer.querySelector('.lp-course-territories'))){const x=(target.x-win.x)*win.s,y=(target.y-win.y)*win.s;frames[0].clipPath=`circle(0px at ${x}px ${y}px)`;frames[1].clipPath=`circle(${Math.hypot(win.w, win.h) * win.s}px at ${x}px ${y}px)`;}
animate(layer,frames);}
for(const[id,el]of nodes){const point=positions.get(id),before=snapshot.positions.get(id);if(!point)continue;if(before){const x=before.x+dx-point.x,y=before.y+dy-point.y;if(x||y)animate(el,[{translate:`${x}px ${y}px`},{translate:'0px 0px'}]);}else{const x=target.x-point.x,y=target.y-point.y;const delay=Math.min(320,Math.hypot(x,y)*.36);animate(el,[{opacity:0,translate:`${x * .12}px ${y * .12}px`},{opacity:1,translate:'0px 0px'}],{duration:1000,delay,fill:'backwards'});}}
modal.setAttribute('data-lp-expanding','');const finish=()=>{if(done)return;done=true;if(raf)cancelAnimationFrame(raf);for(const animation of animations)animation.cancel();for(const layer of snapshot.layers)layer.remove();modal.removeAttribute('data-lp-expanding');modal.__lpCancelExpansion=null;modal.__lpApplyTransform?.({skipClamp:true});};modal.__lpCancelExpansion=finish;const step=time=>{if(done||!modal.classList.contains('lp-open')||!vp.isConnected){finish();return;}
if(!lpLocalExpansionMotionAllowed(modal)){finish();return;}
if(start===null)start=time;if(time-start<1400)raf=requestAnimationFrame(step);else finish();};raf=requestAnimationFrame(step);}
function renderLocalMapModal(graph){const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();if(modal.__lpCancelGestureFrame)modal.__lpCancelGestureFrame();lpSyncLocalMapAnimationAccess(modal);lpEnsureMapRedesignPatchStyles();const stage=modal.querySelector(".lp-mapstage");if(!stage)return;stage.scrollTop=0;stage.scrollLeft=0;modal.__lpRelatedDirty=false;const vp=stage.querySelector("[data-map-viewport]")||stage;if(vp.__lpWorldWindowTimer)clearTimeout(vp.__lpWorldWindowTimer);vp.__lpWorldWindowTimer=0;vp.__lpWorldWindow=null;vp.__lpLocalWindow=false;modal.dataset.lpLocalWindow='';const previousState=window.__lpMapState;if(previousState&&typeof previousState.__lpDisposeNodeAction==='function')previousState.__lpDisposeNodeAction();vp.innerHTML="";const backHop=1;const fwdHop=1;const cur=currentRelPath();if(!cur)return;const depPickState=(window.__lpDependentPickCacheState=window.__lpDependentPickCacheState||{current:""});if(depPickState.current!==cur){window.__lpDependentPickCache=new Map();depPickState.current=cur;}
const relMode="all";modal.dataset.lpRelMode=relMode;const mapState=window.__lpMapState||(window.__lpMapState={scale:lpMapDefaultActualScale(),tx:0,ty:0});const owner=lpCanonKey(cur);if(!window.MkMapExploration.sameOwner(mapState.__lpExploration,owner)||mapState.__lpExploration.graphOwner!==graph){mapState.__lpExploration=window.MkMapExploration.create(owner,lpCanonKey);mapState.__lpExploration.graphOwner=graph;}
const exploration=mapState.__lpExploration;const baseNeighbourhood=getNeighbourhood(graph,cur,backHop,fwdHop,relMode);const nb=window.MkMapExploration.union(exploration,baseNeighbourhood);try{if(relMode!=="off"){const coreLocs=(nb&&nb.core)?nb.core:[];const coreSet2=new Set((coreLocs||[]).map(normLoc));const extraLocs=(nb&&nb.nodes)?(nb.nodes||[]).filter((x)=>x&&!coreSet2.has(normLoc(x))):[];const limCore=(relMode==="all")?22:14;const limExtra=(relMode==="all")?14:8;try{if(modal.__lpRelatedPrefetchTimer)window.clearTimeout(modal.__lpRelatedPrefetchTimer);}catch(_){}
...
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();const world=lpNearbyWorld(lpConceptWorld(),nodes,exploration.terrainWorld,edges);exploration.terrainWorld=world;const worldMode=!!world;const GEO=worldMode?lpWorldGeometry(world):window.MkMapGeometry;modal.dataset.lpWorld=worldMode?(world.layout||"1"):"";const worldFootprint=null;modal.classList.toggle("lp-world-fog-on",!!worldFootprint);lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);try{const drawnLocs=new Set();for(const loc of nodes){if(!loc)continue;drawnLocs.add(normLoc(loc));drawnLocs.add(lpCanonKey(loc));}
modal.__lpRenderedLocSet=drawnLocs;}catch(_){modal.__lpRenderedLocSet=null;}
const coreSet=new Set((nb&&nb.core)?nb.core:nodes);const prereqCache=new Map(),depCache=new Map(),relCache=new Map();const degPre=new Map();const degRel=new Map();for(const e of edges){if(!e)continue;const t=e.type||"prereq";if(t==="related"){degRel.set(e.from,(degRel.get(e.from)||0)+1);degRel.set(e.to,(degRel.get(e.to)||0)+1);}else{degPre.set(e.from,(degPre.get(e.from)||0)+1);degPre.set(e.to,(degPre.get(e.to)||0)+1);}}
const rect=stage.getBoundingClientRect();const stageW=Math.max(320,Math.floor(rect.width));const stageH=Math.max(260,Math.floor(rect.height));const base=1.28;const byN=Math.sqrt(Math.max(1,nodes.length))/5.8;const maxHop=Math.max(backHop,fwdHop);const byHop=Math.max(0,maxHop-1)*0.18;const worldScale=Math.max(1.28,Math.min(3.0,base+byN+byHop));const layoutSpanRaw=Math.max(stageW,stageH);const layoutSpanBasis=Math.max(860,Math.min(980,Math.floor((stageW+stageH)/2)));const layoutSpan=Math.min(layoutSpanRaw,layoutSpanBasis);const worldHere=worldMode?world.point(cur):null;let W=worldMode?world.width:exploration.world?exploration.world.width:Math.floor(layoutSpan*worldScale);let H=worldMode?world.height:exploration.world?exploration.world.height:Math.floor(layoutSpan*worldScale);exploration.world={width:W,height:H};const previousCurrent=exploration.positions&&exploration.positions.get(cur);let cx=worldMode?Math.round(worldHere.x):previousCurrent?previousCurrent.x:Math.floor(W/2);let cy=worldMode?Math.round(worldHere.y):previousCurrent?previousCurrent.y:Math.floor(H/2);vp.style.inset="auto";vp.style.left="0";vp.style.top="0";vp.style.width=`${W}px`;vp.style.height=`${H}px`;const st=window.__lpMapState||(window.__lpMapState={scale:lpMapDefaultActualScale(),tx:0,ty:0});lpLocalMapUpdateStageSize(modal,st,stageW,stageH);st.__lpWorldCX=cx;st.__lpWorldCY=cy;st.__lpCurrentX=cx;st.__lpCurrentY=cy;st.__lpWorldW=W;st.__lpWorldH=H;if(st.__lpForceCenter||!st.userMoved){lpLocalMapResetView(modal,st);if(modal.__lpApplyTransform)modal.__lpApplyTransform();}
...
(function(){if(worldMode)return;const others=nodes.filter((p)=>p!==cur);if(!others.length)return;const minDim=Math.min(W,H);const baseOrbit=Math.max(168,Math.min(280,minDim*0.17));const ringStep=Math.max(102,minDim*0.07);const maxOrbit=Math.max(baseOrbit+ringStep,minDim*0.31);const startAng=-Math.PI/2;const pres=[];const deps=[];const rels=[];const same=[];for(const p of others){const lv=Number(level.get(p)||0);if(lv<0)pres.push(p);else if(lv>0)deps.push(p);else if(relSet(cur).has(p))rels.push(p);else same.push(p);}
...
if(moved===0)break;}})();const NODE_NEIGHBOUR_PAD=18;const SHELL_PAD=NODE_NEIGHBOUR_PAD;const MIN_GAP=Math.max(102,Math.min(138,380/Math.sqrt(nodes.length+1)));const DIRECT_EDGE_GAP=Math.max(148,Math.min(188,MIN_GAP+42));const ITER=reusePlacedMap?0:180;const CLAMP_PAD=10;const connectedPairSet=new Set();for(const e of edges){if(!e||!e.from||!e.to)continue;connectedPairSet.add(lpPairKey(e.from,e.to));}
...
if(moved===0)break;}
(function(){if(reusePlacedMap)return;const adj=new Map();for(const n of nodes)adj.set(n,new Set());for(const e of edges){if(!e)continue;const a=e.from,b=e.to;if(!a||!b)continue;if(!adj.has(a))adj.set(a,new Set());if(!adj.has(b))adj.set(b,new Set());adj.get(a).add(b);adj.get(b).add(a);}
...
moved2++;}}
const C=clampPoint(a,A.x,A.y);A.x=C.x;A.y=C.y;}
if(moved2===0)break;}}})();function geometryNodes(){return Array.from(pos,([id,point])=>{const size=dims.get(id)||{hw:80,hh:18};return{id,x:point.x,y:point.y,width:size.hw*2,height:size.hh*2,course:lpMapCourseKey(id)};}).filter(node=>nodeEls.has(node.id));}
const expansionAnchor=exploration.anchor&&exploration.positions&&exploration.positions.get(exploration.anchor);if(!worldMode&&exploration.positions&&exploration.positions.size){const fresh=nodes.filter(id=>!exploration.positions.has(id));for(const[id,point]of exploration.positions)if(pos.has(id))pos.set(id,{...point});if(expansionAnchor)fresh.forEach((id,index)=>{const angle=index*2.399963229728653;const radius=230+70*Math.sqrt(index);pos.set(id,{x:expansionAnchor.x+Math.cos(angle)*radius,y:expansionAnchor.y+Math.sin(angle)*radius});});}
const geometryInput=geometryNodes();const geometryOptions=lpMapGeometryOptions(geometryInput);let geometry=GEO.layout(geometryInput,edges.map(orientEdge),{...geometryOptions,courseGrouping:false,previous:exploration.positions,fixedIds:exploration.positions&&exploration.positions.size?Array.from(exploration.positions.keys()):[cur]});if(!worldMode&&exploration.focusAfterExpand&&expansionAnchor&&geometry.routes.some(route=>route.status==='blocked')){const ring=new Set(getNeighbourhood(graph,exploration.anchor,1,1,'all').nodes);const movable=new Set();for(const route of geometry.routes)if(route.status==='blocked'){for(const id of[route.from,route.to])if(id!==cur&&id!==exploration.anchor&&ring.has(id)&&exploration.positions.has(id))movable.add(id);}
if(movable.size){const candidate=GEO.layout(geometry.nodes,edges.map(orientEdge),{...geometryOptions,courseGrouping:false,previous:exploration.positions,fixedIds:Array.from(exploration.positions.keys()).filter(id=>!movable.has(id))});const improved=candidate.routes.filter(r=>r.status==='blocked').length<geometry.routes.filter(r=>r.status==='blocked').length;const safe=geometry.routes.every((r,i)=>r.status!=='ok'||candidate.routes[i]?.status==='ok');const bounded=candidate.nodes.every(p=>!movable.has(p.id)||Math.hypot(p.x-exploration.positions.get(p.id).x,p.y-exploration.positions.get(p.id).y)<=230);if(improved&&safe&&bounded&&!candidate.issues.some(issue=>/overlap|invalid/.test(issue.code)))geometry=candidate;}}
for(const point of geometry.nodes)pos.set(point.id,{x:point.x,y:point.y});if(!worldMode&&expansionAnchor){const pad=300;const dx=Math.max(0,pad-Math.min(...geometry.nodes.map(p=>p.x-p.width/2)));const dy=Math.max(0,pad-Math.min(...geometry.nodes.map(p=>p.y-p.height/2)));if(dx||dy){for(const point of pos.values()){point.x+=dx;point.y+=dy;}
for(const point of prefPos.values()){point.x+=dx;point.y+=dy;}
const shifted=GEO.layout(geometryNodes(),edges.map(orientEdge),{...geometryOptions,courseGrouping:false,fixedIds:nodes});geometry.routes=shifted.routes;geometry.regions=shifted.regions;geometry.issues=shifted.issues;st.tx-=dx*st.scale;st.ty-=dy*st.scale;cx+=dx;cy+=dy;}
W=Math.max(W+dx,...Array.from(pos,([id,point])=>point.x+(dims.get(id)?.hw||80)+pad));H=Math.max(H+dy,...Array.from(pos,([id,point])=>point.y+(dims.get(id)?.hh||18)+pad));exploration.world={width:W,height:H};st.__lpWorldW=W;st.__lpWorldH=H;st.__lpWorldCX=st.__lpCurrentX=cx;st.__lpWorldCY=st.__lpCurrentY=cy;vp.style.width=`${W}px`;vp.style.height=`${H}px`;}
exploration.positions=new Map(Array.from(pos,([key,point])=>[key,{...point}]));const routedBase=new Map(geometry.routes.map((route,index)=>[lpFocusEdgeKey({...route,type:(edges[index]||{}).type}),route]));modal.dataset.lpGeometryIssues=String(geometry.issues.length);for(const p of nodes){const el=nodeEls.get(p);const xy=pos.get(p);if(!el||!xy)continue;el.style.left=`${Math.round(xy.x)}px`;el.style.top=`${Math.round(xy.y)}px`;lp3dDecorateLocalNodeElement(el,p,cur,level,coreSet);}
lp3dDecorateLocalMap(modal,graph,{cur,level,coreSet,baseNodes:nodes.slice()});function prereqSet(x){if(!prereqCache.has(x))prereqCache.set(x,new Set(getPrereqs(graph,x)||[]));return prereqCache.get(x);}
function depSet(x){if(!depCache.has(x))depCache.set(x,new Set(getDependents(graph,x)||[]));return depCache.get(x);}
function relSet(x){if(!relCache.has(x)){const s=new Set(getRelated(graph,x)||[]);for(const p of(getPrereqs(graph,x)||[]))s.delete(p);for(const d of(getDependents(graph,x)||[]))s.delete(d);s.delete(x);relCache.set(x,s);}
...
function makeEdgeArrowMarker(id,fill){const marker=document.createElementNS("http://www.w3.org/2000/svg","marker");marker.setAttribute("id",id);marker.setAttribute("markerWidth","9");marker.setAttribute("markerHeight","9");marker.setAttribute("refX","9");marker.setAttribute("refY","3");marker.setAttribute("orient","auto");marker.setAttribute("markerUnits","strokeWidth");const mpath=document.createElementNS("http://www.w3.org/2000/svg","path");mpath.setAttribute('data-lp-edge-marker','base');mpath.setAttribute("d","M0,0 L9,3 L0,6 Z");const parsed=__lpParseRgbAlpha(fill);mpath.setAttribute("fill",parsed.rgb);mpath.setAttribute("fill-opacity","1");mpath.setAttribute("stroke",parsed.rgb);mpath.setAttribute("stroke-opacity","1");mpath.setAttribute("stroke-width","0.6");mpath.setAttribute("stroke-linejoin","round");marker.appendChild(mpath);defs.appendChild(marker);return mpath;}
...
function lpDrawBaseEdge(e){const o=orientEdge(e);const A=pos.get(o.from);const B=pos.get(o.to);if(!A||!B)return null;const da=dims.get(o.from)||{hw:80,hh:18};const db=dims.get(o.to)||{hw:80,hh:18};const P1=clipToEdge(A.x,A.y,B.x,B.y,da.hw+NODE_NEIGHBOUR_PAD,da.hh+NODE_NEIGHBOUR_PAD,PAD);const P2=clipToEdge(B.x,B.y,A.x,A.y,db.hw+NODE_NEIGHBOUR_PAD,db.hh+NODE_NEIGHBOUR_PAD,PAD);const segLen=Math.hypot(P2.x-P1.x,P2.y-P1.y);const t=hash01(`${o.from}→${o.to}`);const baseB=(t<0.5?-1:1)*(8+16*Math.abs(t-0.5)*2);const damp=Math.max(0.35,Math.min(1,segLen/220));const bend=baseB*damp;const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.classList.add("lp-edge");const safeRoute=routedBase.get(lpFocusEdgeKey(o));if(!safeRoute||safeRoute.status!=='ok')return null;path.setAttribute("d",safeRoute.d);path.setAttribute("fill","none");path.dataset.from=o.from;path.dataset.to=o.to;path.__lpBox=[Math.min(A.x,B.x)-40,Math.min(A.y,B.y)-40,Math.max(A.x,B.x)+40,Math.max(A.y,B.y)+40];path.dataset.type=o.type||e.type||"prereq";if(path.dataset.type==="related"){path.classList.add("lp-rel-edge");path.setAttribute("stroke",EDGE_DEFAULT);path.setAttribute("stroke-width","1.7");const relOp=relMode==="all"?0.42:0.24;path.setAttribute("opacity",String(relOp*EDGE_DEFAULT_ALPHA));path.setAttribute("stroke-dasharray","6 6");path.setAttribute("stroke-dashoffset","0");}else{path.setAttribute("stroke",EDGE_DEFAULT);path.setAttribute("stroke-width","1.8");path.setAttribute("opacity",String(0.78*EDGE_DEFAULT_ALPHA));const markerId=__lpSvgUid(`lpArrowE${edgeEls.length}`);path.dataset.lpMarkerId=markerId;path.__lpMarkerPath=makeEdgeArrowMarker(markerId,EDGE_DEFAULT);path.setAttribute("marker-end",`url(#${markerId})`);}
svg.appendChild(path);lpMapPaintEdge(path,'base',path.dataset.type==='related'?'related':'base',false);edgeEls.push(path);addIncident(o.from,path);addIncident(o.to,path);return path;}
for(const e of edges)lpDrawBaseEdge(e);if(!worldMode)lpMapPaintTerritories(svg,geometry.regions,{groupMask:true,simple:true});vp.insertBefore(svg,vp.firstChild);if(worldMode){lpMountWorldTerrain(modal,vp,world,W,H);}
lpWorldExploreStatus(modal,worldFootprint);const hiSvg=document.createElementNS("http://www.w3.org/2000/svg","svg");hiSvg.setAttribute("viewBox",`0 0 ${W} ${H}`);hiSvg.setAttribute("width",String(W));hiSvg.setAttribute("height",String(H));hiSvg.classList.add("lp-map-svg-hi");hiSvg.style.pointerEvents="none";const hiDefs=document.createElementNS("http://www.w3.org/2000/svg","defs");hiSvg.appendChild(hiDefs);const hiMaskId=__lpSvgUid("lpNodeMaskHi");hiSvg.__lpMaskId=hiMaskId;const __lpDesktopSingleLayerHi=true;if(__lpDesktopSingleLayerHi){try{hiSvg.dataset.lpSingleLayer="1";}catch(_){}
try{hiSvg.__lpDisableMask="1";}catch(_){}}
const hiMask=document.createElementNS("http://www.w3.org/2000/svg","mask");hiMask.setAttribute("id",hiMaskId);hiMask.setAttribute("maskUnits","userSpaceOnUse");hiMask.setAttribute("x","0");hiMask.setAttribute("y","0");hiMask.setAttribute("width",String(W));hiMask.setAttribute("height",String(H));const hiAll=document.createElementNS("http://www.w3.org/2000/svg","rect");hiAll.setAttribute("x","0");hiAll.setAttribute("y","0");hiAll.setAttribute("width",String(W));hiAll.setAttribute("height",String(H));hiAll.setAttribute("fill","white");hiMask.appendChild(hiAll);const hiCuts=document.createElementNS("http://www.w3.org/2000/svg","g");hiCuts.setAttribute("data-hi-cuts","1");hiMask.appendChild(hiCuts);hiDefs.appendChild(hiMask);const HI_PRE_MARKER_ID=__lpSvgUid("lpHiArrowPre");const HI_POST_MARKER_ID=__lpSvgUid("lpHiArrowPost");const HI_DEF_MARKER_ID=__lpSvgUid("lpHiArrowDef");(function initHiMarkers(){const mk=(id,color,type)=>{const marker=document.createElementNS("http://www.w3.org/2000/svg","marker");marker.setAttribute("id",id);marker.setAttribute("markerWidth","9");marker.setAttribute("markerHeight","9");marker.setAttribute("refX","9");marker.setAttribute("refY","3");marker.setAttribute("orient","auto");marker.setAttribute("markerUnits","strokeWidth");const mpath=document.createElementNS("http://www.w3.org/2000/svg","path");mpath.setAttribute('data-lp-edge-marker',type);mpath.setAttribute("d","M0,0 L9,3 L0,6 Z");const parsed=__lpParseRgbAlpha(color);mpath.setAttribute("fill",parsed.rgb);mpath.setAttribute("fill-opacity","1");mpath.setAttribute("stroke",parsed.rgb);mpath.setAttribute("stroke-opacity","1");mpath.setAttribute("stroke-width","0.6");mpath.setAttribute("stroke-linejoin","round");marker.appendChild(mpath);hiDefs.appendChild(marker);};mk(HI_PRE_MARKER_ID,PRE_COLOR,'prereq');mk(HI_POST_MARKER_ID,POST_COLOR,'dependent');mk(HI_DEF_MARKER_ID,EDGE_DEFAULT,'base');})();const hiBaseG=document.createElementNS("http://www.w3.org/2000/svg","g");hiBaseG.setAttribute("data-hi-static-edges","1");hiBaseG.setAttribute("mask",`url(#${hiMaskId})`);hiSvg.appendChild(hiBaseG);const hiG=document.createElementNS("http://www.w3.org/2000/svg","g");hiG.setAttribute("data-hi-edges","1");hiSvg.appendChild(hiG);vp.appendChild(hiSvg);function __lpEdgeIsBaselineVisible(baseEl){if(!baseEl)return false;const typ=baseEl.dataset.type||"prereq";if(typ==="related")return relMode==="all";const from=baseEl.dataset.from;const to=baseEl.dataset.to;const isCoreEdge=!!(from&&to&&coreSet&&coreSet.has(from)&&coreSet.has(to));return!(relMode==="dim"&&!isCoreEdge);}
...
hiBaseG.appendChild(p);lpMapPaintEdge(p,'base',typ==='related'?'related':'base',false);}}
...
gWrap.appendChild(p2);lpMapPaintEdge(p2,'focus',typ==='related'?'related':(baseEl.dataset.to===loc?'prereq':'dependent'),typ!=='related');hiG.appendChild(gWrap);if(typ!=="related")__lpStartFlow(p2,gWrap);}}
const st2=window.__lpMapState||(window.__lpMapState={scale:lpMapDefaultActualScale(),tx:0,ty:0});const renderId=(st2.__lpRenderId=(st2.__lpRenderId||0)+1);let measuredEdgesRaf=0;let measuredNodesObserver=null;function syncMeasuredEdgeGeometry(){if(st2.__lpRenderId!==renderId||modal.classList.contains('lp-webgl3d')||modal.hidden||(modal.style&&modal.style.display==='none'))return false;let changed=false;for(const[loc,el]of nodeEls){if(!el.isConnected||(el.classList&&el.classList.contains('lp-node-offscreen')))continue;const next={hw:Math.max(34,el.offsetWidth/2),hh:Math.max(14,el.offsetHeight/2)};const old=dims.get(loc);if(!old||Math.abs(old.hw-next.hw)>.25||Math.abs(old.hh-next.hh)>.25){dims.set(loc,next);changed=true;}}
if(!changed)return false;const paths=edgeEls.concat(Array.from(hiG.querySelectorAll('path.lp-edge[data-from][data-to]')));const edgeData=paths.map(el=>({from:el.dataset.from,to:el.dataset.to,type:el.dataset.type}));const result=GEO.route(geometryNodes(),edgeData,geometryOptions);result.routes.forEach((route,index)=>{const el=paths[index];if(!el)return;const casing=el.__lpStaticCasing;if(route.status!=='ok'){el.style.display='none';if(casing)casing.style.display='none';return;}
el.style.removeProperty('display');if(casing){casing.style.removeProperty('display');if(casing.getAttribute('d')!==route.d)casing.setAttribute('d',route.d);}
if(el.getAttribute('d')!==route.d){el.setAttribute('d',route.d);const flow=__lpFlow.get(el);if(flow){flow.arrow.remove();__lpFlow.delete(el);__lpStartFlow(el,el.parentNode);}}});modal.dataset.lpGeometryIssues=String(result.issues.length);return true;}
function scheduleMeasuredEdgeGeometry(){if(measuredEdgesRaf||st2.__lpRenderId!==renderId)return;measuredEdgesRaf=window.requestAnimationFrame(()=>{measuredEdgesRaf=0;syncMeasuredEdgeGeometry();});}
function __lpReduceMotion(){return lpMotionReduced()||!lpLocalMapMotionAccess(modal);}
...
function __lpStartRelDash(path,reducedMotion){if(typeof reducedMotion==='boolean'?reducedMotion:__lpReduceMotion())return;if(!path||__lpRelDash.has(path))return;try{path.classList.add('lp-rel-anim');path.style.strokeDasharray='6 6';path.style.willChange='stroke-dashoffset';}catch(_){}
...
function __lpStartFlow(path,container,reducedMotion){if(typeof reducedMotion==='boolean'?reducedMotion:__lpReduceMotion())return;if(!path||__lpFlow.has(path))return;const svgRoot=path.ownerSVGElement;if(!svgRoot)return;const arrow=document.createElementNS("http://www.w3.org/2000/svg","path");arrow.setAttribute("d","M0 0 L12 5.2 L0 10.4 Z");arrow.classList.add("lp-flow-arrow");arrow.style.pointerEvents="none";arrow.style.willChange="transform, opacity";try{const mid=svgRoot.__lpMaskId;const disableMask=!!(svgRoot&&svgRoot.__lpDisableMask==="1");if(mid&&!disableMask)arrow.setAttribute("mask",`url(#${mid})`);}catch(_){}
(container&&container.ownerSVGElement===svgRoot?container:svgRoot).appendChild(arrow);const straight=/^\s*M\s*([-+\d.eE]+)[\s,]+([-+\d.eE]+)\s*L\s*([-+\d.eE]+)[\s,]+([-+\d.eE]+)\s*$/.exec(path.getAttribute('d')||'');const coordinates=straight&&straight.slice(1).map(Number);const endpoints=coordinates&&coordinates.every(Number.isFinite)?[{x:coordinates[0],y:coordinates[1]},{x:coordinates[2],y:coordinates[3]}]:null;const total=Math.max(1,endpoints?Math.hypot(endpoints[1].x-endpoints[0].x,endpoints[1].y-endpoints[0].y):path.getTotalLength());const isMobileMap=(()=>{try{return __lpIsMobileMapModal();}catch(_){return false;}})();const isMobileSmoothPreview=(()=>{try{return!!(modal&&modal.classList&&modal.classList.contains('lp-mobile-focus-preview'));}catch(_){return false;}})();const isDesktopSmoothPreview=(()=>{try{return!!(modal&&modal.classList&&modal.classList.contains('lp-desktop-focus-preview'));}catch(_){return false;}})();const useSmoothFlow=isMobileMap||isMobileSmoothPreview||isDesktopSmoothPreview;const sampleCap=isDesktopSmoothPreview?Math.max(160,Math.floor(__LP_FLOW_SAMPLES*0.8)):(useSmoothFlow?Math.max(96,Math.floor(__LP_FLOW_SAMPLES*0.5)):__LP_FLOW_SAMPLES);const n=endpoints?1:Math.max(isDesktopSmoothPreview?48:(useSmoothFlow?32:48),Math.min(sampleCap,Math.ceil(total/(isDesktopSmoothPreview?2:(useSmoothFlow?3:2)))));const pts=endpoints||new Array(n+1);for(let i=0;!endpoints&&i<=n;i++){const L=(total*i)/n;const p=path.getPointAtLength(L);pts[i]={x:p.x,y:p.y};}
...
function lpMapShownDirectSets(ctx,canExplore){if(canExplore)return ctx;const shown=(x)=>nodeEls.has(x)&&!focusExtra.nodes.has(x);return{pres:new Set(Array.from(ctx.pres).filter(shown)),deps:new Set(Array.from(ctx.deps).filter(shown)),rels:new Set(Array.from(ctx.rels).filter(shown))};}
function lpRootNeighbourhood(loc){const ctx=classifyDirectSets(loc);return{nodes:[loc,...ctx.pres,...ctx.deps,...ctx.rels],core:[loc,...ctx.pres,...ctx.deps],edges:buildFocusContextEdges(loc,ctx),level:new Map([[loc,0],...Array.from(ctx.pres,key=>[key,-1]),...Array.from(ctx.deps,key=>[key,1]),...Array.from(ctx.rels,key=>[key,0])])};}
function classifyDirectSets(loc){const pres=new Set(getPrereqs(graph,loc)||[]);const deps=new Set(getDependents(graph,loc)||[]);const rels=new Set((getRelated(graph,loc)||[]).filter((x)=>!pres.has(x)&&!deps.has(x)&&x!==loc));return{pres,deps,rels};}
...
function createNodeEl(loc,extra,opts){const xy=pos.get(loc)||{x:cx,y:cy};const rawVisual=lpMapVisualForLoc(loc);const visual=(loc===cur)?{...rawVisual,blurPx:0,hideTitle:false,fog:false}:rawVisual;nodeVisual.set(loc,visual);const a=document.createElement("a");a.className="lp-node"+(extra?" lp-extra":"");a.dataset.lpLoc=loc;a.href=toAbsoluteUrl(loc);a.setAttribute("data-mk-no-hover-preview","1");const label=document.createElement("span");label.className="lp-node-label";lpBuildNodeLabel(label,loc,visual);a.appendChild(label);lpApplyMapReadinessFill(a,loc);a.dataset.lpTitle=__lpRepairTitleMathFromLoc(loc,nodeTitle(graph,loc));a.dataset.lpVisual=visual.key;if(visual.mastered)a.classList.add("lp-node-mastered");if(lpShouldMaskNode(visual))a.dataset.lpUnvisited="1";if(loc===cur)a.classList.add("is-cur");a.style.color="";lpMapSetTitleFilter(a,lpNodeVisualFilter(loc,0));a.style.left=`${Math.round(xy.x)}px`;a.style.top=`${Math.round(xy.y)}px`;a.style.transform="translate(-50%, -50%)";lp3dDecorateLocalNodeElement(a,loc,cur,level,coreSet,opts&&opts.decorate3d);vp.appendChild(a);lpQueueNodeTitleTypeset(a);nodeEls.set(loc,a);if(!(opts&&opts.deferMeasure))measureNodeEl(loc,a);return a;}
...
function placeFocusExtras(loc,ctx){const anchor=pos.get(loc)||pos.get(cur)||{x:cx,y:cy};const fixedFocusIds=new Set([...baseNodeSet,loc]);const occupied=[];for(const n of fixedFocusIds){const p0=pos.get(n);const d0=dims.get(n);if(p0&&d0)occupied.push({x:p0.x,y:p0.y,d:d0});}
const extras=[];for(const child of Array.from(ctx.pres).filter((n)=>!baseNodeSet.has(n)))extras.push({loc:child,tag:'pre'});for(const child of Array.from(ctx.deps).filter((n)=>!baseNodeSet.has(n)))extras.push({loc:child,tag:'dep'});for(const child of Array.from(ctx.rels).filter((n)=>!baseNodeSet.has(n)))extras.push({loc:child,tag:'rel'});if(!extras.length)return;extras.sort((a,b)=>{const ta=titleTie(a.loc);const tb=titleTie(b.loc);if(ta!==tb)return ta.localeCompare(tb,undefined,{sensitivity:'base'});return h01Stable(`focus-extra-sort|${loc}|${a.tag}|${a.loc}`)-h01Stable(`focus-extra-sort|${loc}|${b.tag}|${b.loc}`);});const count=extras.length;const startAng=-Math.PI/2+((h01Stable(`focus-extra-phase|${loc}`)-0.5)*0.45);const baseRadius=214;const ringStep=50;const perRing=Math.max(6,Math.min(10,Math.ceil(count/2)));const extraEls=[];const decorate3d=lp3dMapEnabled();if(worldMode)for(const item of extras){const q=world.point(item.loc);if(q)pos.set(item.loc,{x:q.x,y:q.y});}
for(let i=0;i<count;i++){const extraEl=createNodeEl(extras[i].loc,true,{deferMeasure:true,decorate3d});if(typeof st2!=='undefined'&&st2.__lpTemporaryPreviewLoc===loc)extraEl.classList.add('lp-temporary-neighbour');extraEls.push(extraEl);}
for(let i=0;i<count;i++)measureNodeEl(extras[i].loc,extraEls[i]);if(worldMode){for(let i=0;i<count;i++)focusExtra.nodes.set(extras[i].loc,extraEls[i]);return;}
const signature=JSON.stringify([Array.from(fixedFocusIds).sort().map(id=>[id,pos.get(id),dims.get(id)]),extras.map(item=>[item.loc,item.tag,dims.get(item.loc)]),edges.concat(buildFocusContextEdges(loc,ctx)).map(edge=>[edge.from,edge.to,edge.type])]);const layouts=focusExtra.layouts||(focusExtra.layouts=new Map());const cached=layouts.get(loc);if(cached&&cached.signature===signature){for(let i=0;i<count;i++){const id=extras[i].loc,point=cached.positions.get(id),el=extraEls[i];pos.set(id,{...point});el.style.left=`${point.x}px`;el.style.top=`${point.y}px`;focusExtra.nodes.set(id,el);}
return;}
for(let i=0;i<count;i++){const item=extras[i];const child=item.loc;const a=extraEls[i];const d=dims.get(child)||{hw:80,hh:18};const ringIdx=Math.floor(i/perRing);const slotIdx=i%perRing;const slotsThisRing=Math.min(perRing,count-ringIdx*perRing);const slotAng=startAng+((Math.PI*2)*slotIdx/Math.max(1,slotsThisRing));const jitterAng=(h01Stable(`focus-extra-ang|${loc}|${item.tag}|${child}`)-0.5)*Math.min(0.24,(Math.PI*2)/Math.max(14,slotsThisRing*3));let radius=baseRadius+ringIdx*ringStep;if(item.tag==='rel')radius+=14;else if(item.tag==='pre')radius-=8;const radialJitter=(h01Stable(`focus-extra-r|${loc}|${item.tag}|${child}`)-0.5)*18;let ang=slotAng+jitterAng;let pt={x:anchor.x+Math.cos(ang)*(radius+radialJitter),y:anchor.y+Math.sin(ang)*(radius+radialJitter*0.8),};if(!window.MkMapGeometry)pt=clampFocusPoint(pt,d);for(let tries=0;tries<10&&overlapsExisting(pt,d,occupied,50);tries++){radius+=16;ang+=0.28+tries*0.05;pt={x:anchor.x+Math.cos(ang)*radius,y:anchor.y+Math.sin(ang)*radius,};if(!window.MkMapGeometry)pt=clampFocusPoint(pt,d);}
...
const placed=GEO.place(geometryNodes(),{...geometryOptions,courseGrouping:true,preferCoursePeers:true,expansionAnchor:anchor,expansionRadius:Math.max(640,320*Math.sqrt(count)),previous:exploration.positions,edges:edges.concat(buildFocusContextEdges(loc,ctx)).map(orientEdge),fixedIds:Array.from(fixedFocusIds)});for(const point of placed.nodes){if(fixedFocusIds.has(point.id))continue;pos.set(point.id,{x:point.x,y:point.y});const el=nodeEls.get(point.id);if(el){el.style.left=`${point.x}px`;el.style.top=`${point.y}px`;}}
layouts.delete(loc);layouts.set(loc,{signature,positions:new Map(extras.map(item=>[item.loc,{...pos.get(item.loc)}]))});if(layouts.size>16)layouts.delete(layouts.keys().next().value);}
function renderContextEdges(contextEdges,loc,keepSet){const reducedMotion=__lpReduceMotion();__lpHiClear();try{hiBaseG.innerHTML="";}catch(_){}
__lpHiUpdateMask(keepSet);if(!worldMode)syncMeasuredEdgeGeometry();const routedFocusResult=GEO.route(geometryNodes(),contextEdges.map(orientEdge),geometryOptions);const routedFocus=new Map(routedFocusResult.routes.map((route,index)=>[lpFocusEdgeKey({...route,type:(contextEdges[index]||{}).type}),route]));for(const e0 of(contextEdges||[])){const o=orientEdge(e0);const incidentToLoc=o.from===loc||o.to===loc;if(!incidentToLoc)continue;const A=pos.get(o.from);const B=pos.get(o.to);if(!A||!B)continue;const da=dims.get(o.from)||{hw:80,hh:18};const db=dims.get(o.to)||{hw:80,hh:18};const P1=clipToEdge(A.x,A.y,B.x,B.y,da.hw+NODE_NEIGHBOUR_PAD,da.hh+NODE_NEIGHBOUR_PAD);const P2=clipToEdge(B.x,B.y,A.x,A.y,db.hw+NODE_NEIGHBOUR_PAD,db.hh+NODE_NEIGHBOUR_PAD);const t=hash01(`${o.from}→${o.to}`);const segLen=Math.hypot(P2.x-P1.x,P2.y-P1.y);const baseB=(t<0.5?-1:1)*(8+16*Math.abs(t-0.5)*2);const damp=Math.max(0.35,Math.min(1,segLen/220));const safeRoute=routedFocus.get(lpFocusEdgeKey(o));if(!safeRoute||safeRoute.status!=='ok')continue;const dAttr=safeRoute.d;const p2=document.createElementNS("http://www.w3.org/2000/svg","path");p2.classList.add("lp-edge");p2.setAttribute("d",dAttr);p2.setAttribute("fill","none");try{p2.dataset.from=o.from;p2.dataset.to=o.to;p2.dataset.type=o.type||"prereq";}catch(_){}
if(!__lpDesktopSingleLayerHi)p2.setAttribute("mask",`url(#${hiMaskId})`);const gWrap=document.createElementNS("http://www.w3.org/2000/svg","g");gWrap.classList.add("lp-hi-edgewrap");gWrap.style.opacity=incidentToLoc?"0.92":"0.78";if(o.type==="related"){p2.classList.add("lp-rel-edge");p2.style.strokeWidth=incidentToLoc?"2.2":"1.8";p2.style.strokeDasharray="6 6";p2.style.strokeDashoffset="0";p2.style.stroke=incidentToLoc?REL_COLOR:EDGE_DEFAULT;if(incidentToLoc&&!reducedMotion){p2.classList.add("lp-rel-anim");__lpStartRelDash(p2,reducedMotion);}}else{p2.style.strokeWidth=incidentToLoc?"2.3":"1.85";let col=EDGE_DEFAULT;let mid=HI_DEF_MARKER_ID;if(incidentToLoc){if(o.to===loc){col=PRE_COLOR;mid=HI_PRE_MARKER_ID;}
else if(o.from===loc){col=POST_COLOR;mid=HI_POST_MARKER_ID;}}
p2.style.stroke=col;p2.setAttribute("marker-end",`url(#${mid})`);}
gWrap.appendChild(p2);lpMapPaintEdge(p2,'focus',o.type==='related'?'related':(o.to===loc?'prereq':'dependent'),o.type!=='related');hiG.appendChild(gWrap);if(incidentToLoc&&o.type!=="related")__lpStartFlow(p2,gWrap,reducedMotion);}}
...
const curEl=nodeEls.get(cur);if(curEl){curEl.classList.add('lp-keep');curEl.style.opacity='1';lpMapSetTitleFilter(curEl,lpNodeVisualFilter(cur,0));}
const liveTarget=lpRevealCurrentElement(loc,target);if(liveTarget){liveTarget.classList.add('lp-focus','lp-keep');liveTarget.style.opacity='1';lpMapSetTitleFilter(liveTarget,lpNodeVisualFilter(loc,0));}
...
function lpClearDesktopHoverInterference(){if(!lpIsDesktopLocalMapHover())return;try{if(modal&&modal.toggleAttribute){modal.toggleAttribute('data-lp-slider-zooming',false);modal.toggleAttribute('data-lp-gesturing',false);}}catch(_){}
...
stage.querySelectorAll('.lp-node-action').forEach(item=>item.remove());const nodeAction=document.createElement('div');nodeAction.className='lp-node-action';nodeAction.id='lp-nearby-node-actions';nodeAction.hidden=true;nodeAction.setAttribute('role','group');nodeAction.setAttribute('aria-label','Concept actions');const expandAction=document.createElement('button');expandAction.type='button';expandAction.className='lp-node-action-btn';expandAction.dataset.lpNodeExpand='';expandAction.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg><span>Expand map</span>';const openAction=document.createElement('a');openAction.className='lp-node-action-btn';openAction.dataset.lpNodeOpen='';openAction.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg><span>Open page</span>';nodeAction.appendChild(expandAction);nodeAction.appendChild(openAction);stage.appendChild(nodeAction);let actionLoc=null,actionNode=null;let leaveTimer=0;function hideNodeAction(){if(nodeAction.hidden&&!actionNode)return;nodeAction.hidden=true;if(actionNode){actionNode.setAttribute('aria-expanded','false');actionNode.removeAttribute('data-lp-actions-open');}
actionLoc=null;actionNode=null;}
function positionNodeAction(){if(nodeAction.hidden||!actionNode||!actionNode.isConnected)return;const box=actionNode.getBoundingClientRect(),area=stage.getBoundingClientRect();const width=Math.min(280,Math.max(0,area.width-16)),height=44;const left=Math.max(8,Math.min(area.width-width-8,(box.left+box.right)/2-area.left-width/2));const below=box.bottom-area.top+8;const top=below+height<=area.height-72?below:Math.max(8,box.top-area.top-height-8);nodeAction.style.width=`${width}px`;nodeAction.style.left=`${left}px`;nodeAction.style.top=`${top}px`;}
function showNodeAction(loc,el,focusButton){if(st2.__lpRenderId!==renderId||!modal.classList.contains('lp-open')||!el||!el.isConnected)return;if(lpCanonKey(loc)===lpCanonKey(cur)){hideNodeAction();return;}
cancelPreviewLeave();if(actionNode&&actionNode!==el)hideNodeAction();actionLoc=loc;actionNode=el;openAction.href=toAbsoluteUrl(loc);const ring=getNeighbourhood(graph,loc,1,1,'all');const shown=new Set(nodes.map(lpCanonKey));const seenEdges=new Set(edges.map(e=>lpFocusEdgeKey(orientEdge(e))));const addsContent=ring.nodes.some(id=>!shown.has(lpCanonKey(id)))||ring.edges.some(e=>!seenEdges.has(lpFocusEdgeKey(orientEdge(e))));expandAction.disabled=!addsContent;expandAction.querySelector('span').textContent=addsContent?'Expand map':'Expanded';el.setAttribute('aria-expanded','true');el.setAttribute('data-lp-actions-open','');nodeAction.hidden=false;positionNodeAction();if(focusButton)(expandAction.disabled?openAction:expandAction).focus({preventScroll:true});}
function expandNodeMap(){const loc=actionLoc;if(!loc||st2.__lpRenderId!==renderId||!modal.classList.contains('lp-open'))return;const ring=getNeighbourhood(graph,loc,1,1,'all');const shown=new Set(nodes.map(lpCanonKey));const addedNodeIds=ring.nodes.filter(id=>!shown.has(lpCanonKey(id))).map(normLoc);if(!window.MkMapExploration.expand(exploration,loc,ring))return;exploration.anchor=loc;exploration.focusAfterExpand=loc;exploration.transition=lpCaptureLocalExpansion(modal,vp,exploration.positions);lpMarkLocalMapInteractionBusy(1200);st2.userMoved=true;st2.__lpForceCenter=false;hideNodeAction();renderLocalMapModal(graph);if(addedNodeIds.length)modal.__lpMapChests?.expanded({nodeId:normLoc(loc),addedNodeIds,trigger:'manual'});}
expandAction.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();expandNodeMap();});openAction.addEventListener('click',event=>{if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;event.preventDefault();event.stopPropagation();if(actionLoc&&st2.__lpRenderId===renderId)lpNavigate(toAbsoluteUrl(actionLoc),actionLoc);});nodeAction.addEventListener('pointerenter',cancelPreviewLeave,{passive:true});nodeAction.addEventListener('pointerleave',event=>{if(event.pointerType&&event.pointerType!=='mouse')return;schedulePreviewLeave();},{passive:true});nodeAction.addEventListener('focusin',cancelPreviewLeave);nodeAction.addEventListener('focusout',event=>{if(!nodeAction.contains(event.relatedTarget))schedulePreviewLeave();});nodeAction.addEventListener('keydown',event=>{if(event.key!=='Escape')return;event.preventDefault();event.stopPropagation();const node=actionNode;if(node)node.focus({preventScroll:true});clearFocus();});function cancelPreviewLeave(){if(leaveTimer)window.clearTimeout(leaveTimer);leaveTimer=0;}
function schedulePreviewLeave(){cancelPreviewLeave();leaveTimer=window.setTimeout(()=>{leaveTimer=0;if(st2.__lpRenderId!==renderId)return;if(window.MkExplain?.active||nodeAction.contains(document.activeElement))return;clearFocus();},180);}
st2.__lpPositionNodeAction=null;st2.__lpHideNodeAction=hideNodeAction;st2.__lpDisposeNodeAction=()=>{cancelPreviewLeave();st2.__lpTemporaryPreviewLoc=null;hideNodeAction();nodeAction.remove();if(measuredNodesObserver)measuredNodesObserver.disconnect();if(measuredEdgesRaf)window.cancelAnimationFrame(measuredEdgesRaf);measuredEdgesRaf=0;};function bindNodeInteractions(el,loc){if(!el||el.dataset.lpHoverBound==="1")return;el.dataset.lpHoverBound="1";if(measuredNodesObserver)measuredNodesObserver.observe(el);el.setAttribute('role','button');if(lpCanonKey(loc)!==lpCanonKey(cur)){el.setAttribute('aria-expanded','false');el.setAttribute('aria-controls',nodeAction.id);}
el.setAttribute('aria-description',lpCanonKey(loc)===lpCanonKey(cur)?'Current concept. Preview its connections.':'Show actions to expand this map or open the concept page.');let down=null;let moved=false;const live=()=>st2.__lpRenderId===renderId&&el.isConnected;const refresh3d=()=>{const engine=modal.__lpWebgl3dEngine;if(!engine||engine.kind!=='local')return;engine.hoverKey=lpCanonKey(loc);lpWebgl3dRebuild(engine);};const preview=()=>{if(!live()||st2.dragging||modal.classList.contains('lp-webgl3d-dragging')||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0))return;cancelPreviewLeave();lpMarkLocalMapInteractionBusy(180);const focusChanged=st2.__lpFocusedLoc!==loc;if(focusChanged)applyFocus(loc);if(lpNodeShouldRunRevealPreview(el,loc))lpStartRevealPreview(el,loc);if(focusChanged||(modal.__lpWebgl3dEngine&&modal.__lpWebgl3dEngine.hoverKey!==lpCanonKey(loc)))refresh3d();showNodeAction(loc,el,false);};const retain=(temporary=false)=>{if(!live())return;cancelPreviewLeave();lpPauseRevealPlayback(loc,el);if(lpCanonKey(loc)===lpCanonKey(cur))return;if(temporary){st2.__lpTemporaryPreviewLoc=loc;applyFocus(loc);}
preview();showNodeAction(loc,el,true);};el.addEventListener('pointerenter',event=>{if(event.pointerType&&event.pointerType!=='mouse')return;preview();},{passive:true});el.addEventListener('pointerleave',event=>{if(event.pointerType&&event.pointerType!=='mouse')return;if(st2.__lpHoverLoc!==loc)return;lpPauseRevealPlayback(loc,el);schedulePreviewLeave();},{passive:true});el.addEventListener('focus',preview);el.addEventListener('blur',schedulePreviewLeave);el.addEventListener('pointerdown',event=>{down={id:event.pointerId,x:event.clientX,y:event.clientY,pointerType:event.pointerType};moved=false;},{passive:true});el.addEventListener('pointermove',event=>{if(down&&down.id===event.pointerId&&Math.hypot(event.clientX-down.x,event.clientY-down.y)>10)moved=true;},{passive:true});el.addEventListener('pointercancel',()=>{moved=true;down=null;});el.addEventListener('click',event=>{if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;event.preventDefault();event.stopImmediatePropagation();const dragged=moved||!!st2.dragging||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0);const fineHover=!!(window.matchMedia&&window.matchMedia('(hover: hover) and (pointer: fine)').matches)||(typeof lpIsDesktopLocalMapHover==='function'&&lpIsDesktopLocalMapHover());const mouseClick=fineHover&&(event.pointerType==='mouse'||(down&&down.pointerType==='mouse'));down=null;if(!dragged)retain(mouseClick);moved=false;},true);el.addEventListener('keydown',event=>{if(!live())return;if(event.key==='Escape'){if(st2.__lpFocusedLoc||st2.__lpHoverLoc){event.preventDefault();event.stopPropagation();clearFocus();}
return;}
if(event.key!=='Enter'&&event.key!==' ')return;event.preventDefault();event.stopPropagation();retain();});}
st2.__lpShowNodeAction=showNodeAction;let baselinePainted=false,focusPainted=false;function applyBaseline(){if(st2.__lpRenderId!==renderId)return;st2.__lpTemporaryPreviewLoc=null;if(!worldMode)syncMeasuredEdgeGeometry();try{const revealLoc=st2&&st2.__lpRevealFocusLoc;if(revealLoc&&!st2.__lpApplyingRevealFocus&&lpRevealNavIsActive(revealLoc)){lpScheduleRevealEdgeFocusRepaint(revealLoc,lpRevealCurrentElement(revealLoc,null));return;}}catch(_){}
__lpStopAllFlows();removeExtraFocusNodes();try{__lpHiClear();}catch(_){}
if(worldMode&&baselinePainted&&!focusPainted){try{__lpHiUpdateMask();}catch(_){}
try{__lpSyncStaticHiEdges();}catch(_){}
st2.__lpFocusedLoc=null;st2.__lpHoverLoc=null;return;}
for(const el of edgeEls)lpBaselineEdge(el);try{__lpHiUpdateMask();}catch(_){}
try{__lpSyncStaticHiEdges();}catch(_){}
for(const[loc,el]of nodeEls.entries()){lpApplyBaselineNodeVisual(el,loc);}
baselinePainted=true;focusPainted=false;st2.__lpFocusedLoc=null;st2.__lpHoverLoc=null;}
function lpBaselineEdge(el){{el.classList.remove("lp-rel-anim");const typ=el.dataset.type||"prereq";el.style.removeProperty("filter");el.style.removeProperty("stroke-width");el.style.removeProperty("opacity");el.style.removeProperty("stroke");el.style.removeProperty("stroke-dasharray");el.style.removeProperty("stroke-dashoffset");if(typ==="related"){try{el.removeAttribute("marker-end");}catch(_){}
el.style.stroke=EDGE_DEFAULT;el.style.opacity="0.28";el.style.strokeWidth="1.7";el.style.strokeDasharray="6 6";el.style.strokeDashoffset="0";el.style.filter=lpEdgeDimFilter();}else{el.classList.add("lp-dim");setEdgeArrowPaint(el,EDGE_DEFAULT,0.28);el.style.stroke=EDGE_DEFAULT;el.style.opacity="0.28";el.style.strokeWidth="1.6";el.style.filter=lpEdgeDimFilter();}}}
function clearFocus(){const hadPreview=st2.__lpFocusedLoc||st2.__lpHoverLoc||st2.__lpRevealFocusLoc;cancelPreviewLeave();hideNodeAction();st2.__lpTemporaryPreviewLoc=null;if(!hadPreview)return;try{const revealLoc=st2&&st2.__lpRevealFocusLoc;if(revealLoc&&lpRevealNavIsActive(revealLoc))return;}catch(_){}
...
applyBaseline();const engine=modal.__lpWebgl3dEngine;if(engine&&engine.kind==='local'){engine.hoverKey='';lpWebgl3dRebuild(engine);}}
...
lpClearDesktopHoverInterference();if(st2.__lpHoverLoc===loc&&st2.__lpFocusedLoc===loc&&st2.__lpPaintedTemporaryPreviewLoc===st2.__lpTemporaryPreviewLoc)return;}
if(st2.__lpRenderId!==renderId)return;__lpStopAllFlows();removeExtraFocusNodes(loc);st2.__lpHoverLoc=loc;st2.__lpPaintedTemporaryPreviewLoc=st2.__lpTemporaryPreviewLoc||null;const locVisual=nodeVisual.get(loc)||lpMapVisualForLoc(loc);const locRec=getMastery(loc);const locMastery=(locRec&&typeof locRec.m==="number")?locRec.m:null;const isExpandable=true;const ctx=lpMapShownDirectSets(classifyDirectSets(loc),st2.__lpTemporaryPreviewLoc===loc);const directCurEdges=lpDirectEdgesToCurrent(loc);const focusSet=new Set([loc,cur,...ctx.pres,...ctx.deps,...ctx.rels]);for(const e0 of directCurEdges){if(e0&&e0.from)focusSet.add(e0.from);if(e0&&e0.to)focusSet.add(e0.to);}
...
if(!isExpandable&&loc!==cur){applyBaseline();const curEl=nodeEls.get(cur);if(curEl){curEl.classList.add("lp-keep");curEl.style.opacity="1";lpMapSetTitleFilter(curEl,lpNodeVisualFilter(cur,0));}
const selfOnly=nodeEls.get(loc);if(selfOnly){selfOnly.classList.add("lp-focus","lp-keep");selfOnly.style.opacity="1";lpMapSetTitleFilter(selfOnly,lpNodeVisualFilter(loc,0));}
renderContextEdges(directCurEdges,loc,new Set([loc,cur]));focusPainted=true;st2.__lpFocusedLoc=loc;st2.__lpHoverLoc=loc;return;}
focusPainted=true;for(const[k,el]of nodeEls.entries()){el.style.color="";el.classList.remove("lp-focus","lp-dim","lp-keep");if(keep.has(k)){el.classList.add("lp-keep");el.style.opacity="1";lpMapSetTitleFilter(el,lpNodeVisualFilter(k,0));const visual=nodeVisual.get(k)||lpMapVisualForLoc(k);if(visual.mastered)el.style.color="";}else{lpApplyDimNodeVisual(el,k);}}
const self=nodeEls.get(loc);if(self){self.classList.add("lp-focus","lp-keep");self.style.opacity="1";lpMapSetTitleFilter(self,lpNodeVisualFilter(loc,0));}
...
applyBaseline();});if(typeof ResizeObserver==='function')measuredNodesObserver=new ResizeObserver(scheduleMeasuredEdgeGeometry);for(const[loc,el]of nodeEls.entries()){bindNodeInteractions(el,loc);}
...
if(!stage.dataset.lpFocusBound){stage.dataset.lpFocusBound="1";stage.addEventListener("pointerleave",event=>{if(event.pointerType&&event.pointerType!=='mouse')return;if(window.MkExplain?.active||document.activeElement?.closest('.lp-node-action')||event.relatedTarget?.closest?.('[data-wiki-explain-ui]'))return;const st=window.__lpMapState;if(st&&typeof st.__lpClearFocus==="function")st.__lpClearFocus();});stage.addEventListener("pointerdown",(e)=>{try{const t=e&&e.target;if(t&&t.closest){if(t.closest("a.lp-node"))return;if(t.closest(".lp-mctrl"))return;if(t.closest(".lp-mzoom"))return;if(t.closest(".lp-node-action"))return;if(t.closest(".lp-map-chests"))return;if(t.closest(".lp-close"))return;if(t.closest("[data-lp-fullscreen]"))return;}}catch(_){}
const st=window.__lpMapState;if(st&&typeof st.__lpClearFocus==="function")st.__lpClearFocus();});}
const mathNodeEls=Array.from(nodeEls.values()).filter((el)=>{try{const titleEl=el&&el.querySelector?el.querySelector('.lp-node-title'):null;return lpNodeTitleNeedsTypeset(titleEl);}catch(_){return false;}});if(mathNodeEls.length)requestAnimationFrame(()=>{Promise.resolve().then(()=>typesetMathSafe(mathNodeEls.map((el)=>(el&&el.querySelector)?(el.querySelector('.lp-node-title')||el):el))).then(()=>{mathNodeEls.forEach((el)=>{try{lpCacheRenderedNodeTitle(graph,el);}catch(_){}});}).catch(()=>{});});const expansionFocus=exploration.focusAfterExpand;if(expansionFocus){const target=nodeEls.get(exploration.focusAfterExpand),point=pos.get(exploration.focusAfterExpand);exploration.focusAfterExpand=null;if(modal.__lpApplyTransform)modal.__lpApplyTransform({skipClamp:true});if(target)target.focus({preventScroll:true});clearFocus();}
modal.__lpRenderedFor=lpCanonKey(cur);modal.__lpRenderedGraph=graph||null;modal.__lpRenderedStageW=stageW;modal.__lpRenderedStageH=stageH;modal.__lpRenderedFogEnabled=lpFogEnabled();modal.__lpRenderedMasterySignature=lpCurrentMasterySignature();const localWindow=!worldMode&&!modal.classList.contains('lp-webgl3d')&&!modal.classList.contains('lp-map-3d');vp.__lpLocalWindow=localWindow;modal.dataset.lpLocalWindow=localWindow?'1':'';if(worldMode||localWindow){const cam=window.__lpMapState||{};const win=lpWorldWindowCommit(vp,{scale:cam.scale,tx:cam.tx,ty:cam.ty,stageW:Number(cam.__lpStageW)||stageW,stageH:Number(cam.__lpStageH)||stageH});if(worldMode)lpWorldWindowCull(vp,win);}
if(expansionFocus){lpAnimateLocalExpansion(modal,vp,st,exploration.transition,pos,nodeEls,cur,expansionFocus);exploration.transition=null;}
try{const chests=lpAttachMapChests(modal);chests?.updateLayout();}catch(_){}}
...
let lpFocusedMap=null;let lpMapReturnFocus=null;let lpMapKeyboardBound=false;function lpMapOwnedStyleMenu(modal){const menu=document.getElementById('mk-quick-shop-menu');return menu&&menu.__mkQuickShopAnchor&&modal.contains(menu.__mkQuickShopAnchor)?menu:null;}
function lpMountMapStyleShortcut(modal){if(!modal||window.__mkExamMode||modal.querySelector('[data-lp-map-style]'))return;const close=modal.querySelector('.lp-close');if(!close)return;modal.__lpCloseForStyleStore=()=>{if(modal.id==='lp-h1sg-modal')hideH1StudyStartModal();else hideLocalMapModal();};const button=document.createElement('button');button.type='button';button.className='lp-map-style-shortcut';button.setAttribute('data-lp-map-style','');button.setAttribute('aria-label','Map styles');button.setAttribute('aria-haspopup','dialog');button.setAttribute('aria-expanded','false');button.title='Map styles';button.innerHTML=typeof window.MkStyleClothesIcon==='function'?window.MkStyleClothesIcon():'';close.parentNode.insertBefore(button,close);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const root=document.documentElement;if(window.__mkExamMode||root.getAttribute('data-mk-startup-account')==='off'||root.classList.contains('mk-startup-account-off'))return;lpMapTipsCollapse(modal);const options={anchor:button,categories:['Map effects'],slots:['map_style'],title:'Map styles',includeDefaultMapStyle:true,focusOnOpen:true};button.focus({preventScroll:true});if(window.MkLocalActivity&&typeof window.MkLocalActivity.openQuickShop==='function')window.MkLocalActivity.openQuickShop(button,options);else window.dispatchEvent(new CustomEvent('mk-open-quick-shop',{detail:options}));});}
function lpMapFocusable(modal){return Array.from(modal.querySelectorAll('a[href], button, input, select, textarea, [tabindex]')).filter((el)=>!el.disabled&&el.tabIndex>=0&&!el.closest('[hidden], [aria-hidden="true"]')&&el.getClientRects().length>0&&window.getComputedStyle(el).visibility!=='hidden');}
function lpMapFocusOpen(modal,opener){if(lpFocusedMap!==modal){const active=opener||document.activeElement;if(active&&active!==document.body&&active!==document.documentElement&&typeof active.closest==='function'&&!active.closest('#lp-map-modal, #lp-h1sg-modal'))lpMapReturnFocus=active;}
lpFocusedMap=modal;if(!lpMapKeyboardBound){lpMapKeyboardBound=true;document.addEventListener('keydown',(event)=>{if(event.isComposing||event.keyCode===229||event.defaultPrevented)return;if(event.target&&event.target.closest&&event.target.closest(".lp-map-chest-dialog"))return;const host=lpFocusedMap;if(!host||!host.isConnected||!host.classList.contains('lp-open'))return;const otherDialog=event.target&&event.target.closest&&event.target.closest('[role="dialog"][aria-modal="true"]');if(otherDialog&&!host.contains(otherDialog))return;const styleMenu=lpMapOwnedStyleMenu(host);if(styleMenu&&event.key==='Escape'){event.preventDefault();event.stopPropagation();if(styleMenu.__mkQuickShopClose)styleMenu.__mkQuickShopClose({returnFocus:true});return;}
...
if(event.key!=='Tab')return;const controls=lpMapFocusable(host);if(styleMenu){controls.splice(controls.indexOf(styleMenu.__mkQuickShopAnchor)+1,0,...lpMapFocusable(styleMenu));const index=controls.indexOf(document.activeElement);event.preventDefault();const next=index<0?(event.shiftKey?controls.length-1:0):(index+(event.shiftKey?-1:1)+controls.length)%controls.length;const target=controls[next];if(target){target.focus({preventScroll:true});if(styleMenu.contains(target)){const item=target.getBoundingClientRect(),panel=styleMenu.getBoundingClientRect();const top=panel.top+(styleMenu.clientTop||0)+8;const bottom=panel.top+(styleMenu.clientTop||0)+(styleMenu.clientHeight||panel.height)-8;const delta=item.top<top?item.top-top:item.bottom>bottom?item.bottom-bottom:0;if(delta)styleMenu.scrollTop=(Number(styleMenu.scrollTop)||0)+delta;}}
return;}
const activeIndex=controls.indexOf(document.activeElement);if(activeIndex<0||(event.shiftKey&&activeIndex===0)||(!event.shiftKey&&activeIndex===controls.length-1)){event.preventDefault();const target=event.shiftKey?controls[controls.length-1]:controls[0];if(target)target.focus({preventScroll:true});}},true);}
...
function lpMapFocusClose(modal){const menu=lpMapOwnedStyleMenu(modal);if(menu&&menu.__mkQuickShopClose)menu.__mkQuickShopClose();if(lpFocusedMap!==modal)return;lpFocusedMap=null;const target=lpMapReturnFocus;lpMapReturnFocus=null;if(target&&target.isConnected&&target.getClientRects().length)target.focus({preventScroll:true});}
...
modal.innerHTML=`
        ...
        <div class="lp-map-topbar">
          <div class="lp-map-toprow">
            <div class="lp-map-tabs" role="tablist" aria-label="Map views">
              <button type="button" class="lp-map-tab" data-lp-map-tab="toHere" role="tab" aria-label="Prerequisites" title="Prerequisites" aria-selected="false"><span class="lp-tab-label-long">Prerequisites</span><span class="lp-tab-label-short">Before</span></button>
              ...
              <button type="button" class="lp-map-tab" data-lp-map-tab="fromHere" role="tab" aria-label="Dependents" title="Dependents" aria-selected="false"><span class="lp-tab-label-long">Dependents</span><span class="lp-tab-label-short">After</span></button>
            </div>
            <button type="button" class="lp-map-tipbtn" data-lp-map-tip-toggle aria-label="Map tips" aria-expanded="false">
            ...
            <div class="lp-map-options" data-lp-map-options>
              <label class="lp-map-option" title="Knowledge masking">
                <span class="lp-map-option-text">Masking</span>
                <span class="lp-ios-switch">
                  <input type="checkbox" data-lp-fog-switch aria-label="Turn knowledge masking on or off">
    ...
    `;document.body.appendChild(modal);lpInstallBlankLongPressSelectionGuard(modal);modal.__lpOpen=opener=>showLocalMapModal(graph,opener);modal.addEventListener("click",(e)=>{const tab=e.target&&e.target.closest?e.target.closest('[data-lp-map-tab]'):null;if(!tab)return;e.preventDefault();e.stopPropagation();const kind=tab.getAttribute('data-lp-map-tab')||'local';if(kind==='local'){lpSyncMapTabs(modal,'local');lpMapTipsCollapse(modal);return;}
...
lpMapTipsSet(modal,'local');lpMapTipsCollapse(modal);lpBindMapOptions(modal);lpBindTap(modal.querySelector(".lp-close"),()=>{hideLocalMapModal();});lpBindTap(modal.querySelector("[data-lp-fullscreen]"),()=>{if(window.__lpMapState&&typeof window.__lpMapState.__lpClearFocus==="function"){window.__lpMapState.__lpClearFocus();}
const isOn=modal.classList.toggle("lp-full");const btn=modal.querySelector("[data-lp-fullscreen]");if(btn)lpSetFullscreenIcon(btn,isOn);const st=window.__lpMapState||(window.__lpMapState={scale:lpMapDefaultActualScale(),tx:0,ty:0});st.__lpForceCenter=true;st.userMoved=false;const isDesktopFine=!!(window.matchMedia&&window.matchMedia("(pointer: fine)").matches);if(modal.dataset.lpWorld){requestAnimationFrame(()=>{if(modal.style.display==="none")return;if(!lpLocalMapResizeCamera(modal)){lpLocalMapResetView(modal,st);if(modal.__lpApplyTransform)modal.__lpApplyTransform({skipClamp:true});}});return;}
if(isDesktopFine){if(typeof __lpStopAllFlows==="function"){try{__lpStopAllFlows();}catch(_){}}
...
function applyTransform(opts){if(!vp)return;const skipRangeSync=!!(opts&&opts.skipRangeSync);const skipClamp=!!(opts&&opts.skipClamp);const syncZoomRange=!!(opts&&opts.syncZoomRange);const perfMode=lpLocalMapPerfActive();if(!skipClamp)clampPan();const tx=Number(st.tx)||0;const ty=Number(st.ty)||0;const sc=Number(st.scale)||1;const useCompositorTransform=lpLocalMapTouchCompositorActive()||!!(modal&&modal.hasAttribute&&modal.hasAttribute("data-lp-dragging"));const transformText=lp3dViewportTransform(modal,'local',tx,ty,sc,useCompositorTransform);const useProxy=!!(st.__lpZoomDragging&&st.__lpUseZoomProxy&&st.__lpZoomProxy&&st.__lpZoomProxy.isConnected);const target=useProxy?st.__lpZoomProxy:vp;if(target&&target.style.transform!==transformText){target.style.transform=transformText;if(typeof st.__lpHideNodeAction==='function')st.__lpHideNodeAction();try{modal.__lpMapChests?.updateLayout();}catch(_){}}
if(modal&&modal.dataset&&(modal.dataset.lpWorld||modal.dataset.lpLocalWindow==='1'))lpWorldWindowFollow(vp,readWorldCamera);lpWorldLabelScale(modal,sc);const labelText=`${lpMapDisplayPctFromScale(sc)}%`;if(zoomLabel&&zoomLabel.textContent!==labelText){if(!perfMode||!skipRangeSync){zoomLabel.textContent=labelText;}else{const now=(typeof performance!=="undefined"&&performance.now)?performance.now():Date.now();if(!st.__lpLastOperaZoomLabelAt||now-st.__lpLastOperaZoomLabelAt>140){st.__lpLastOperaZoomLabelAt=now;zoomLabel.textContent=labelText;}}}
...
if(modal&&modal.dataset&&(modal.dataset.lpWorld||modal.dataset.lpLocalWindow==='1'))lpWorldWindowFollow(vp,readWorldCamera);}
const gestureCamera=lpFrameCameraQueue(()=>{if(!modal.classList.contains('lp-open'))return;applyTransformFast({skipClamp:true});const pct=lpMapDisplayPctFromScale(st.scale);if(zoomLabel&&zoomLabel.textContent!==`${pct}%`)zoomLabel.textContent=`${pct}%`;if(zoomRange&&!st.__lpZoomDragging&&Number(zoomRange.value)!==pct){zoomRange.value=String(pct);lpUpdateZoomRangeVisual(zoomRange,pct,LP_MAP_ZOOM_MIN_PCT,LP_MAP_ZOOM_MAX_PCT);}});modal.__lpCancelGestureFrame=()=>{gestureCamera.cancel();if(__lpTransformRaf)cancelAnimationFrame(__lpTransformRaf);__lpTransformRaf=0;__lpTransformSkipRangeSync=__lpTransformSkipClamp=__lpTransformSyncZoomRange=false;for(const pid of activePtrs.keys()){try{panSurface.releasePointerCapture(pid);}catch(_){}}
if(activePid!=null){try{panSurface.releasePointerCapture(activePid);}catch(_){}}
activePtrs.clear();pinch=null;dragging=false;pendingNodeDrag=null;activePid=null;clearMobileGestureRestore();modal.removeAttribute('data-lp-dragging');modal.removeAttribute('data-lp-gesturing');};function readWorldCamera(){const stageEl=modal.querySelector('.lp-mapstage');return{scale:st.scale,tx:st.tx,ty:st.ty,stageW:Number(st.__lpStageW)||(stageEl&&stageEl.clientWidth)||0,stageH:Number(st.__lpStageH)||(stageEl&&stageEl.clientHeight)||0};}
...
function setMobileGestureMode(on,opts){try{if(!__lpIsMobileMapModal()&&!__lpIsIPadLikeTouch()&&!lpLocalMapPerfActive()&&!lpMapDocSurfaceIsTouch()&&!(modal&&modal.classList&&modal.classList.contains("lp-touch-compositor")))return;const immediate=!!(opts&&opts.immediate);clearMobileGestureRestore();if(on){modal.toggleAttribute('data-lp-gesturing',true);return;}
if(immediate){modal.toggleAttribute('data-lp-gesturing',false);return;}
__lpGestureRestoreTimer=window.setTimeout(()=>{__lpGestureRestoreTimer=0;__lpGestureRestoreRaf=requestAnimationFrame(()=>{__lpGestureRestoreRaf=0;const pointersStillActive=activePtrs&&typeof activePtrs.size==='number'?activePtrs.size>0:false;if(dragging||pendingNodeDrag||pinch||pointersStillActive)return;modal.toggleAttribute('data-lp-gesturing',false);});},120);}catch(_){}}
...
return{x:0,y:0};}
function zoomAboutScreenPoint(nextScale,anchor,opts){if(!vp)return;if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();const oldScale=Number(st.scale)||1;const clamped=Math.min(lpMapMaxActualScale(),Math.max(lpMapMinActualScale(),Number(nextScale)||oldScale));if(!isFinite(clamped)||clamped===oldScale)return;const a=anchor||getStageCenter();const ax=Number(a.x)||0;const ay=Number(a.y)||0;const wx=(ax-st.tx)/oldScale;const wy=(ay-st.ty)/oldScale;st.scale=clamped;st.tx=ax-wx*clamped;st.ty=ay-wy*clamped;if(opts&&opts.defer)scheduleTransform({skipRangeSync:!!opts.skipRangeSync,skipClamp:!!opts.skipClamp});else applyTransform({skipRangeSync:!!(opts&&opts.skipRangeSync),skipClamp:!!(opts&&opts.skipClamp)});}
...
function setSliderZoomDragMode(on){try{if(!modal)return;modal.toggleAttribute("data-lp-slider-zooming",!!on);}catch(_){}}
...
try{modal.toggleAttribute("data-lp-gesturing",false);modal.toggleAttribute("data-lp-slider-zooming",false);}catch(_){}
...
let __lpZoomInputRaf=0;let __lpZoomInputValue=null;let __lpZoomPointerHeld=false;const __lpBeginZoomDrag=(e)=>{if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();__lpZoomPointerHeld=true;lpMarkLocalMapInteractionBusy(360);clearHoverNow();markGestureScale();st.__lpZoomDragging=true;setSliderZoomDragMode(true);try{st.__lpZoomAnchor=getStageCenter();}catch(_){st.__lpZoomAnchor=null;}
...
const ZOOM_BTN_STEP=0.1;lpBindTap(btnZoomDec,()=>{clearHoverNow();zoomAboutScreenPoint((Number(st.scale)||1)-lpMapZoomStepActualScale(),getStageCenter());st.userMoved=true;});lpBindTap(btnZoomInc,()=>{clearHoverNow();zoomAboutScreenPoint((Number(st.scale)||1)+lpMapZoomStepActualScale(),getStageCenter());st.userMoved=true;});lpBindTap(modal.querySelector("[data-zoom-reset]"),()=>{if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();clearHoverNow();lpMapClearFitStatus(modal);st.scale=lpMapDefaultActualScale();st.userMoved=false;centerView();applyTransform();});const stageEl=modal.querySelector(".lp-mapstage");const panSurface=stageEl||vp;let dragging=false;let pendingNodeDrag=null;let activePid=null;let lx=0,ly=0;const NODE_DRAG_THRESHOLD=8;const activePtrs=new Map();let pinch=null;function isTouchGestureDevice(){try{if(navigator&&typeof navigator.maxTouchPoints==="number"&&navigator.maxTouchPoints>0)return true;}catch(_){}
...
function endDrag(opts){if(__lpTransformRaf)cancelAnimationFrame(__lpTransformRaf);__lpTransformRaf=0;__lpTransformSkipRangeSync=__lpTransformSkipClamp=__lpTransformSyncZoomRange=false;gestureCamera.flush(()=>applyTransform({skipClamp:true}));lpMarkLocalMapInteractionBusy(180);dragging=false;const pidToRelease=activePid;activePid=null;if(pidToRelease!=null&&panSurface&&typeof panSurface.releasePointerCapture==="function"){try{panSurface.releasePointerCapture(pidToRelease);}catch(_){}}
requestAnimationFrame(()=>{if(dragging||pendingNodeDrag||pinch||(activePtrs&&activePtrs.size>0))return;try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",false);}catch(_){}
setMobileGestureMode(false,Object.assign({immediate:!lpLocalMapLiveGestureFast()},opts||{}));schedulePostZoomRestore();});}
function startDrag(e){if(!vp||!panSurface)return;if((e.pointerType||"mouse")==="mouse"&&typeof e.button==="number"&&e.button!==0)return;const t=e.target;if(t&&t.closest&&t.closest('.lp-mctrl, .lp-mzoom, .lp-close, [data-lp-fullscreen], .lp-node-action, .lp-map-chests, [data-lp-map-dir-menu], .lp-view-toggle, canvas.lp-webgl3d-canvas'))return;if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();lpMarkLocalMapInteractionBusy(320);const isMobile=isTouchGestureDevice();if(isMobile&&(e.pointerType==="touch"||e.pointerType==="pen")){if(typeof e.pointerId==="number"){activePtrs.set(e.pointerId,{x:e.clientX,y:e.clientY});}
if(activePtrs.size===2){pendingNodeDrag=null;dragging=false;activePid=null;const ids=Array.from(activePtrs.keys());const p1=activePtrs.get(ids[0]);const p2=activePtrs.get(ids[1]);if(p1&&p2){const r0=(stageEl&&typeof stageEl.getBoundingClientRect==="function")?stageEl.getBoundingClientRect():(vp&&typeof vp.getBoundingClientRect==="function"?vp.getBoundingClientRect():null);clearHoverNow();markGestureScale();try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",true);}catch(_){}
setMobileGestureMode(true);const m0=mid(p1,p2);const ax0=r0?(m0.x-r0.left):m0.x;const ay0=r0?(m0.y-r0.top):m0.y;const s0=(st.scale||1);const tx0=(st.tx||0);const ty0=(st.ty||0);const wx0=(ax0-tx0)/s0;const wy0=(ay0-ty0)/s0;pinch={id1:ids[0],id2:ids[1],d0:dist(p1,p2),s0,wx0,wy0,stageLeft:r0?(Number(r0.left)||0):0,stageTop:r0?(Number(r0.top)||0):0,};try{panSurface.setPointerCapture(ids[0]);}catch(_){}
try{panSurface.setPointerCapture(ids[1]);}catch(_){}
if(e.cancelable)e.preventDefault();e.stopPropagation();}
return;}}
if(t&&t.closest){if(t.closest("a.lp-node")){if(e.pointerType==="touch"||e.pointerType==="pen"){pendingNodeDrag={pid:(typeof e.pointerId==="number")?e.pointerId:null,sx:Number(e.clientX)||0,sy:Number(e.clientY)||0};activePid=pendingNodeDrag.pid;lx=Number(e.clientX)||0;ly=Number(e.clientY)||0;return;}
return;}}
clearHoverNow();markGestureScale();try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",true);}catch(_){}
setMobileGestureMode(true);dragging=true;st.userMoved=true;activePid=(typeof e.pointerId==="number")?e.pointerId:null;lx=e.clientX;ly=e.clientY;if(activePid!=null&&typeof panSurface.setPointerCapture==="function"){try{panSurface.setPointerCapture(activePid);}catch(_){}}
if(e.cancelable)e.preventDefault();e.stopPropagation();}
function moveDrag(e){const contact=activePtrs.get(e.pointerId);if(contact){contact.x=e.clientX;contact.y=e.clientY;}
if(pinch&&(e.pointerType==="touch"||e.pointerType==="pen")&&typeof e.pointerId==="number"){if(e.pointerId===pinch.id1||e.pointerId===pinch.id2){const p1=activePtrs.get(pinch.id1);const p2=activePtrs.get(pinch.id2);if(p1&&p2){lpMarkLocalMapInteractionBusy(240);const d1=dist(p1,p2);const ratio=d1/Math.max(1e-6,pinch.d0);const rawScale=(pinch.s0||1)*ratio;const nextScale=Math.min(lpMapMaxActualScale(),Math.max(lpMapMinActualScale(),Number(rawScale)||(pinch.s0||lpMapDefaultActualScale())));const m=mid(p1,p2);const ax=m.x-(Number(pinch.stageLeft)||0);const ay=m.y-(Number(pinch.stageTop)||0);st.scale=nextScale;st.tx=ax-(Number(pinch.wx0)||0)*nextScale;st.ty=ay-(Number(pinch.wy0)||0)*nextScale;st.userMoved=true;gestureCamera.schedule();}
if(e.cancelable)e.preventDefault();e.stopPropagation();return;}}
if(!dragging){if(pendingNodeDrag){if(pendingNodeDrag.pid!=null&&typeof e.pointerId==="number"&&e.pointerId!==pendingNodeDrag.pid)return;const dx0=e.clientX-pendingNodeDrag.sx;const dy0=e.clientY-pendingNodeDrag.sy;if((dx0*dx0+dy0*dy0)<(NODE_DRAG_THRESHOLD*NODE_DRAG_THRESHOLD))return;clearHoverNow();markGestureScale();try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",true);}catch(_){}
setMobileGestureMode(true);dragging=true;st.userMoved=true;st.__lpSuppressNextNodeClick=true;if(activePid!=null&&panSurface&&typeof panSurface.setPointerCapture==="function"){try{panSurface.setPointerCapture(activePid);}catch(_){}}
lx=e.clientX;ly=e.clientY;pendingNodeDrag=null;}else{return;}}
if(activePid!=null&&typeof e.pointerId==="number"&&e.pointerId!==activePid)return;lpMarkLocalMapInteractionBusy(220);const dx=e.clientX-lx;const dy=e.clientY-ly;lx=e.clientX;ly=e.clientY;st.tx+=dx;st.ty+=dy;st.userMoved=true;gestureCamera.schedule();if(e.cancelable)e.preventDefault();e.stopPropagation();}
function stopDrag(e){if((e&&(e.pointerType==="touch"||e.pointerType==="pen"))&&typeof e.pointerId==="number"){activePtrs.delete(e.pointerId);if(pinch&&(e.pointerId===pinch.id1||e.pointerId===pinch.id2)){pinch=null;if(activePtrs.size===1&&!dragging&&!pendingNodeDrag){const rid=Array.from(activePtrs.keys())[0];const rp=activePtrs.get(rid);if(rp&&vp){clearHoverNow();dragging=true;st.userMoved=true;activePid=rid;lx=rp.x;ly=rp.y;try{panSurface.setPointerCapture(rid);}catch(_){}}}}
if(activePtrs.size<2){try{panSurface&&panSurface.releasePointerCapture&&panSurface.releasePointerCapture(e.pointerId);}catch(_){}}}
...
const hardTouchGestureCleanup=(e)=>{try{const remainingTouches=e&&e.touches?Number(e.touches.length)||0:0;if(remainingTouches>0)return;if(!dragging&&!pinch&&!pendingNodeDrag&&!activePtrs.size&&!modal.hasAttribute('data-lp-gesturing'))return;const captured=Array.from(activePtrs.keys());if(activePid!=null)captured.push(activePid);activePtrs.clear();pinch=null;dragging=false;pendingNodeDrag=null;activePid=null;for(const id of captured){try{panSurface.releasePointerCapture(id);}catch(_){}}
gestureCamera.flush(()=>applyTransform({skipClamp:true}));if(!dragging&&!pendingNodeDrag&&!st.__lpZoomDragging){try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",false);}catch(_){}
setMobileGestureMode(false,{immediate:!lpLocalMapLiveGestureFast()});schedulePostZoomRestore();}}catch(_){}};modal.addEventListener("touchend",hardTouchGestureCleanup,{passive:true,capture:true});modal.addEventListener("touchcancel",hardTouchGestureCleanup,{passive:true,capture:true});if(window.PointerEvent){panSurface?.addEventListener("pointerdown",startDrag,{passive:false});panSurface?.addEventListener("pointermove",moveDrag,{passive:false});panSurface?.addEventListener("pointerup",stopDrag,{passive:false});panSurface?.addEventListener("pointercancel",stopDrag,{passive:false});panSurface?.addEventListener("lostpointercapture",stopDrag,{passive:true});window.addEventListener("pointerup",stopDrag,{passive:false});window.addEventListener("pointercancel",stopDrag,{passive:false});}else{panSurface?.addEventListener("mousedown",startDrag);window.addEventListener("mousemove",moveDrag);window.addEventListener("mouseup",stopDrag);}
...
function lpMapDocIOSBottomOcclusionPx(safeBottom){if(!lpMapDocSurfaceIsTouch()||!lpMapDocSurfaceIsIOS())return 0;try{const vv=window.visualViewport;const layoutH=Math.max(1,Number(window.innerHeight)||Number(document.documentElement&&document.documentElement.clientHeight)||1);const vvBottom=vv?((Number(vv.offsetTop)||0)+(Number(vv.height)||0)):layoutH;const visualGap=vv?Math.max(0,Math.round(layoutH-vvBottom)):0;let screenH=0;try{screenH=Math.max(Number(window.screen&&window.screen.height)||0,Number(window.screen&&window.screen.width)||0);}catch(_){screenH=0;}
const safe=Math.max(0,safeBottom==null?lpMapDocSafeBottomPx():safeBottom);const screenGap=screenH>0?Math.max(0,Math.round(screenH-layoutH-safe)):0;const raw=Math.max(visualGap,screenGap);if(raw<40){try{if(__lpIsPhoneTouch()||__lpIsMobileMapModal())return 112;}catch(_){}
return 0;}
return lpMapDocClamp(raw,64,320);}catch(_){return 0;}}
function lpUpdateMapDocumentSurface(modal){try{if(!modal||!modal.classList||!modal.classList.contains("lp-open"))return;if(!modal.classList.contains("lp-doc-surface"))return;const vv=window.visualViewport;const layoutW=Math.max(1,Number(window.innerWidth)||Number(document.documentElement&&document.documentElement.clientWidth)||1);const layoutH=Math.max(1,Number(window.innerHeight)||Number(document.documentElement&&document.documentElement.clientHeight)||1);const vvLeft=vv?(Number(vv.offsetLeft)||0):0;const vvTop=vv?(Number(vv.offsetTop)||0):0;const vvW=vv&&Number(vv.width)?Number(vv.width):layoutW;const vvH=vv&&Number(vv.height)?Number(vv.height):layoutH;const vvBottom=vvTop+vvH;const safeBottom=lpMapDocSafeBottomPx();const safeStrip=Math.max(safeBottom,vv?Math.max(0,Math.round(layoutH-vvBottom)):0,lpMapDocIOSBottomOcclusionPx(safeBottom));const visibleBottom=vv?Math.max(0,vvBottom):layoutH;const extraTail=lpMapDocSurfaceIsIOS()?56:28;const layoutBottom=Math.max(layoutH,visibleBottom)+Math.max(0,safeStrip)+extraTail;const docLeft=lpMapDocScrollX()+vvLeft;const docTop=lpMapDocScrollY()+vvTop;const docHeight=Math.max(120,Math.ceil(layoutBottom-vvTop));const visibleHeight=Math.max(120,Math.ceil(vvH||layoutH));const hiddenTail=Math.max(0,Math.ceil(docHeight-visibleHeight));const setMetric=(key,value)=>{if(modal.style.getPropertyValue(key)!==value)modal.style.setProperty(key,value);};setMetric("--lp-map-doc-left",lpMapDocPx(docLeft));setMetric("--lp-map-doc-top",lpMapDocPx(docTop));setMetric("--lp-map-doc-width",lpMapDocPx(vvW||layoutW));setMetric("--lp-map-doc-height",lpMapDocPx(docHeight));setMetric("--lp-map-visible-height",lpMapDocPx(visibleHeight));setMetric("--lp-map-ios-hidden-tail",lpMapDocPx(hiddenTail));const dockGap=lpMapDocSurfaceIsIOS()?8:12;const zoomBottom=Math.max(8,Math.ceil(hiddenTail+dockGap));setMetric("--lp-map-zoom-bottom",lpMapDocPx(zoomBottom));}catch(_){}}
...
function lpApplyLocalMapPerfMode(modal,nodeCount,edgeCount){try{if(!modal||!modal.classList)return false;const operaPerf=lpIsOperaMapPerfMode();const prefersReduced=!!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches);const lowCore=Number(navigator&&navigator.hardwareConcurrency||8)<=4;const lowMemory=Number(navigator&&navigator.deviceMemory||8)<=4;const n=Number(nodeCount)||0;const e=Number(edgeCount)||0;const worldMap=!!(modal.dataset&&modal.dataset.lpWorld);const on=!!(operaPerf||prefersReduced||(!worldMap&&(lowCore||lowMemory)&&(n>55||e>95))||(!worldMap&&(n>85||e>150)));modal.classList.toggle("lp-opera-map-perf",on);if(on)modal.setAttribute('data-lp-opera-perf',operaPerf?'opera':'auto');else modal.removeAttribute('data-lp-opera-perf');return on;}catch(_){return false;}}
async function showLocalMapModal(graph,opener){if(!lpAnyMapModalOpen()&&!lpConsumeGuestAction("map",{source:"learning-path-local-map",title:"Concept maps"}))return;lpResetRevealSession();const modal=ensureMapModal(graph);if(!modal)return;const worldOpen={};modal.__lpWorldOpen=worldOpen;lpSyncLocalMapAnimationAccess(modal);lpLocalMapHideDirectionMenu(modal);const helper=modal&&modal.querySelector?modal.querySelector('[data-lp-map-helper-panel]'):null;if(helper)helper.textContent=lpMapTabHelperText('local',__lpIsPhoneTouch());lpSyncMapTabs(modal,'local');lpMapTipsSet(modal,'local');lpMapTipsCollapse(modal);const isMobile=__lpIsMobileMapModal();const touchCompositor=!!(isMobile||lpMapDocSurfaceIsTouch());try{modal.classList.toggle("lp-touch-compositor",!!touchCompositor);if(touchCompositor)modal.setAttribute("data-lp-touch-compositor","1");else modal.removeAttribute("data-lp-touch-compositor");}catch(_){}
lpApplyLocalMapPerfMode(modal,0,0);const isDesktopPc=!!(window.matchMedia&&window.matchMedia('(min-width: 901px) and (hover: hover) and (pointer: fine)').matches);lpReparentZoomGroup(modal);if(isMobile||isDesktopPc)modal.classList.add("lp-full");else modal.classList.remove("lp-full");modal.classList.add("lp-open");modal.style.display="flex";lpInstallLocalMapPointerWatch();lpMarkLocalMapInteractionBusy(700);lpRecordXpActivity("map_open",{source:"show-local-map-modal",eventName:"showLocalMapModal"});try{document.dispatchEvent(new CustomEvent("mk:map-opened",{detail:{source:"show-local-map-modal"}}));}catch(_){}
...
const btn=modal.querySelector("[data-lp-fullscreen]");if(btn)lpSetFullscreenIcon(btn,modal.classList.contains("lp-full"));lpMapFocusOpen(modal,opener);const stage=modal.querySelector('.lp-mapstage');const curKey=lpCanonKey(currentRelPath());if(!lpConceptWorld()){if(stage)stage.style.visibility='hidden';await lpLoadConceptWorld();if(!modal.classList.contains('lp-open')||modal.__lpWorldOpen!==worldOpen||curKey!==lpCanonKey(currentRelPath()))return;}
lpMarkLocalMapInteractionBusy(700);const stageWNow=stage?Math.max(0,Math.round(stage.clientWidth||stage.getBoundingClientRect().width||0)):0;const stageHNow=stage?Math.max(0,Math.round(stage.clientHeight||stage.getBoundingClientRect().height||0)):0;const masterySigNow=lpCurrentMasterySignature();const worldKey=String(lpConceptWorld()?.layout||"");const canReuse=!!(stage&&curKey&&String(modal.dataset.lpWorld||"")===worldKey&&modal.__lpRenderedGraph===(graph||null)&&modal.__lpRenderedFor===curKey&&!modal.__lpRelatedDirty&&modal.__lpRenderedFogEnabled===lpFogEnabled()&&modal.__lpRenderedMasterySignature===masterySigNow&&stage.querySelector('[data-map-viewport] .lp-node[data-lp-loc]')&&Math.abs((Number(modal.__lpRenderedStageW)||0)-stageWNow)<=20&&Math.abs((Number(modal.__lpRenderedStageH)||0)-stageHNow)<=20);const preserveView=!!(window.__lpMapState&&modal.__lpRenderedFor===curKey&&window.__lpMapState.userMoved);if(window.__lpMapState&&!preserveView){window.__lpMapState.scale=lpMapDefaultActualScale();window.__lpMapState.__lpForceCenter=true;window.__lpMapState.userMoved=false;}
if(canReuse){try{lpAttachMapChests(modal)?.updateLayout();}catch(_){}
if(stage)stage.style.visibility='';const engine=modal.__lpWebgl3dEngine;if(engine){lpWebgl3dObserve(engine);lpWebgl3dDraw(engine);}
requestAnimationFrame(()=>{try{if(window.__lpMapState&&!preserveView)lpLocalMapResetView(modal,window.__lpMapState);modal.__lpApplyTransform&&modal.__lpApplyTransform();}catch(_){}});return;}
if(stage)stage.style.visibility='';lp3dActivateModal(modal,'local');lpBindMapOptions(modal);renderLocalMapModal(graph);try{if(usingDocSurface){lpUpdateMapDocumentSurface(modal);requestAnimationFrame(()=>{try{lpUpdateMapDocumentSurface(modal);if(window.__lpMapState&&!preserveView)lpLocalMapResetView(modal,window.__lpMapState);modal.__lpApplyTransform&&modal.__lpApplyTransform();}catch(_){}});}}catch(_){}}
function hideLocalMapModal(){try{document.getElementById("lp-map-modal")?.__lpMapChests?.suspend();}catch(_){}
lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpCancelExpansion)modal.__lpCancelExpansion();if(modal.__lpCancelGestureFrame)modal.__lpCancelGestureFrame();if(modal.__lpViewportResizeCleanup)modal.__lpViewportResizeCleanup();try{if(modal.__lpStopZoomGesture)modal.__lpStopZoomGesture();}catch(_){}
...
function lpDiscardLocalMapExpansion(modal){const state=window.__lpMapState;if(!state||!state.__lpExploration||!window.MkMapExploration.roots(state.__lpExploration).length)return;if(state.__lpDisposeNodeAction)state.__lpDisposeNodeAction();state.__lpExploration=null;state.__lpRenderId=(Number(state.__lpRenderId)||0)+1;state.userMoved=false;state.__lpForceCenter=true;modal.__lpRenderedFor='';modal.__lpRenderedGraph=null;}
...
function mountMobileSheet(panel,graph){const{backdrop,root}=ensureMobileSheetShell();const body=root.querySelector("[data-lp-ms-body]");const btnToggle=root.querySelector("[data-lp-ms-toggle]");const btnOpenMap=root.querySelector("[data-lp-ms-openmap]");const head=root.querySelector(".lp-msheet-head");lpInstallMobileSheetSelectionShield();lpInstallMobileSheetClickContainmentShield();for(const control of[btnOpenMap,btnToggle]){if(control&&!control.__lpWorldPreload){control.__lpWorldPreload=true;try{control.addEventListener('pointerdown',()=>{try{lpLoadConceptWorld();}catch(_){}},{once:true,passive:true});}catch(_){}}}
if(body){body.innerHTML="";body.appendChild(panel);lpBindMobileSectionSummaryTaps(panel);}
...
lpBindMobileTapAction(btnToggle,"lpMsToggleTapBound",()=>{const open=root.classList.contains("lp-expanded");lpSetMobileSheetExpanded(!open);},{movePx:12,tapMs:760});lpBindMobileTapAction(btnOpenMap,"lpMsOpenMapTapBound",()=>{showLocalMapModal(graph,btnOpenMap);},{movePx:12,tapMs:760});}
...
function lpEmitPanelShellMounted(panel,loc){try{const rel=normLoc(loc||currentRelPath());if(!panel||panel.__lpShellMountedRel===rel)return;panel.__lpShellMountedRel=rel;window.__lpShellMountedRel=rel;window.dispatchEvent(new CustomEvent("lp:shell-mounted",{detail:{relPath:window.__lpShellMountedRel,panelId:panel&&panel.id||""}}));}catch(_){}}
...
const mapBtn=panel.querySelector('[data-lp-open-map]');if(mapBtn){mapBtn.__lpOpen=()=>showLocalMapModal(graph,mapBtn);lpBindTap(mapBtn,mapBtn.__lpOpen);const preloadWorld=()=>{try{lpLoadConceptWorld();}catch(_){}};try{for(const type of['pointerenter','pointerdown'])mapBtn.addEventListener(type,preloadWorld,{once:true,passive:true});mapBtn.addEventListener('focus',preloadWorld,{once:true});}catch(_){}}
const gpsBtn=panel.querySelector('[data-lp-open-gps]');hideRedundantBodySections();return panel;}
...
function lpH1StudyStartApplyTransformFast(modal,opts){const vp=lpH1StudyStartViewport(modal);if(!vp)return;const view=lpH1StudyStartEnsureView(modal);if(!(opts&&opts.skipClamp))lpH1StudyStartClampPan(modal);const scale=Number(view.scale)||1;const transformText=lp3dViewportTransform(modal,'route',Number(view.tx)||0,Number(view.ty)||0,scale,true);if(vp.style.transform!==transformText)vp.style.transform=transformText;lpH1StudyWorldWindowFollow(modal,vp);if(!(opts&&opts.gesture))lpH1StudySyncScaleMeta(modal,scale,!!(opts&&opts.syncZoomMeta));const state=modal&&modal.__lpH1StudyState;if(state&&state.positions instanceof Map&&state.positions.size>48){lpH1StudyStartCullForDrag(modal,true,false);}}
function lpH1StudyWorldWindowFollow(modal,vp){if(!modal||!vp||!modal.dataset||!modal.dataset.lpWorld||!vp.querySelector(':scope > svg.lp-map-terrain'))return;vp.__lpWorldWindowScope='route';lpWorldWindowFollow(vp,()=>{const view=lpH1StudyStartEnsureView(modal);return{scale:view.scale,tx:view.tx,ty:view.ty,stageW:Number(view.stageW)||0,stageH:Number(view.stageH)||0};});}
function lpH1StudyStartCullForDrag(modal,active,force){try{const viewport=lpH1StudyStartViewport(modal);const state=modal&&modal.__lpH1StudyState;const view=modal&&lpH1StudyStartEnsureView(modal);if(!viewport||!state||!view||!(state.positions instanceof Map))return;if(!active){const nodes=viewport.querySelectorAll('.lp-node[data-lp-loc]');nodes.forEach((nodeEl)=>{if(nodeEl.classList.contains('lp-pan-culled'))nodeEl.classList.remove('lp-pan-culled');});view.__lpCullBounds=null;return;}
const scale=Math.max(0.05,Number(view.scale)||1);const tx=Number(view.tx)||0;const ty=Number(view.ty)||0;const bounds=view.__lpCullBounds,guard=160/scale;if(!force&&bounds&&-tx/scale-guard>=bounds.left&&-ty/scale-guard>=bounds.top&&(view.stageW-tx)/scale+guard<=bounds.right&&(view.stageH-ty)/scale+guard<=bounds.bottom)return;const margin=400/scale;const left=(-tx/scale)-margin;const top=(-ty/scale)-margin;const right=((Number(view.stageW)||0)-tx)/scale+margin;const bottom=((Number(view.stageH)||0)-ty)/scale+margin;view.__lpCullBounds={left,top,right,bottom};const nodes=viewport.querySelectorAll('.lp-node[data-lp-loc]');nodes.forEach((nodeEl)=>{const loc=normLoc(nodeEl.getAttribute('data-lp-loc')||'');const pos=state.positions.get(loc);const x=Number(pos&&pos.x);const y=Number(pos&&pos.y);const culled=Number.isFinite(x)&&Number.isFinite(y)?(x<left||x>right||y<top||y>bottom):false;if(nodeEl.classList.contains('lp-pan-culled')!==culled)nodeEl.classList.toggle('lp-pan-culled',culled);});}catch(_){}}
...
function lpH1StudyStartZoomAboutFast(modal,nextScale,anchor,opts){const view=lpH1StudyStartEnsureView(modal);const oldScale=Number(view.scale)||1;const clamped=Math.min(lpMapMaxActualScale(),Math.max(lpMapMinActualScale(),Number(nextScale)||oldScale));if(!isFinite(clamped)||clamped===oldScale)return;const a=anchor||lpH1StudyStartGetStageCenter(modal);const ax=Number(a.x)||0;const ay=Number(a.y)||0;const wx=(ax-(Number(view.tx)||0))/oldScale;const wy=(ay-(Number(view.ty)||0))/oldScale;view.scale=clamped;view.tx=ax-wx*clamped;view.ty=ay-wy*clamped;if(opts&&opts.deferGesture&&modal.__lpScheduleGestureFrame)modal.__lpScheduleGestureFrame();else lpH1StudyStartApplyTransformFast(modal,{skipClamp:!!(opts&&opts.skipClamp),syncZoomMeta:true});}
...
lpH1StudyRouteAnimSetNavArrow(modal,false);if(anim.cancelled)return;if(!useNav3d&&entry&&entry.pathEl&&okList.every(Boolean)){lpMapPaintEdge(entry.pathEl,'route','route',true);}
if(useNav3d&&okList.some((ok)=>!ok)){lpWebgl3dRouteSetNavProgress(modal,i,1,i+1);}
...
function lpH1StudyStartRenderSelection(world,state,opts){if(!world||!state)return;const modal=(world&&world.closest)?world.closest('#lp-h1sg-modal'):lpRouteMapActiveModal();if(!(opts&&opts.staticView))lpMapClearFitStatus(modal);const cfg=lpRouteMapConfig(state.mode);const graph=state.graph||(modal&&modal.__lpGraph)||window.__lpLearningPathGraph||null;const pathLocs=Array.isArray(state.path)?state.path.map((loc)=>normLoc(loc)).filter(Boolean):[];const pathSet=new Set(pathLocs.map((loc)=>lpCanonKey(loc)).filter(Boolean));const startKey=lpCanonKey(state.startLoc||"");const targetKey=lpCanonKey(state.target||"");const hasQuery=!!lpH1StudyStartNormQuery(state.query||"");const hasPath=pathLocs.length>0;const hasAnimPath=pathLocs.length>=2;const animKey=hasPath?`${cfg.mode}|${pathLocs.map((loc) => lpCanonKey(loc)).join('>')}`:'';const explicitPlay=!!(modal&&modal.__lpH1ExplicitRoutePlay);if(modal)modal.__lpH1ExplicitRoutePlay=false;const canAnimate=!!(!(opts&&opts.staticView)&&hasAnimPath&&modal&&(explicitPlay||!lpMotionReduced())&&(explicitPlay||modal.__lpH1RenderedAnimKey!==animKey));if(modal)modal.__lpH1RenderedAnimKey=animKey;const visiblePathKeys=canAnimate?new Set():new Set(pathSet);if(hasPath&&!canAnimate&&lpMotionReduced()&&!(opts&&opts.staticView)){const revealed=lpH1StudyRouteRevealSetForState(state);pathLocs.forEach((loc)=>{const key=lpCanonKey(loc);if(key)revealed.add(key);});}
...
svg.innerHTML="";lpRouteContextPaint2d(svg,world,state);if(hasAnimPath){const H1_EDGE_COLOR="rgb(255,255,255)";const H1_EDGE_ALPHA=0.92;const H1_NODE_PAD=18;function h1SvgUid(prefix){const seed=`${prefix}|${Date.now()}|${Math.random()}|${state.target || ""}|${pathLocs.join("→")}`;let h=2166136261;for(let i=0;i<seed.length;i++){h^=seed.charCodeAt(i);h=Math.imul(h,16777619);}
return`${prefix}-${(h >>> 0).toString(36)}`;}
...
lpMapSyncRouteContext(modal);if(svg&&world.__lpWorldWindow)lpWorldWindowMasks(svg,world.__lpWorldWindow);}
...
function lpH1StudyStartWorldLayout(tree,graph){const world=lpConceptWorld();if(!world||!tree||!Array.isArray(tree.layers))return null;const locs=uniq(tree.layers.flatMap(layer=>(layer&&layer.nodes)||[]).map(normLoc).filter(Boolean));const root=normLoc(tree.target||'');if(root&&!locs.includes(root))locs.push(root);if(!locs.length||!locs.every(loc=>world.has(loc)))return null;const primary=lpH1StudyStartPrimaryTree(graph,tree);const positions=new Map(locs.map(loc=>{const p=world.point(loc);return[loc,{x:p.x,y:p.y}];}));const here=world.point(root)||positions.values().next().value;return{positions,world,worldW:world.width,worldH:world.height,cx:here.x,cy:here.y,parentOf:primary&&primary.parentOf instanceof Map?primary.parentOf:new Map(),childrenByParent:primary&&primary.childrenByParent,subtreeSize:primary&&primary.subtreeSize,desiredAngles:primary&&primary.desiredAngles,};}
function lpH1StudyStartPrepKey(targetLoc,hostW,hostH,mode){const world=lpConceptWorld();return`${lpRouteMapMode(mode)}|d${lpRouteMapMobileDistanceScale()}|${lpCanonKey(targetLoc || currentRelPath())}|${lpH1StudyStartPrepBucket(hostW, hostH)}|w${world ? world.layout || '1' : ''}`;}
function lpH1StudyStartPrepare(graph,targetLoc,hostW,hostH,mode){const cfg=lpRouteMapConfig(mode);const tree=lpH1StudyStartTree(graph,targetLoc||currentRelPath(),cfg.mode);const target=tree.target||normLoc(targetLoc||currentRelPath());const targetTitle=cleanTitle(nodeTitle(graph,target)||target||'');let layout=lpH1StudyStartWorldLayout(tree,graph);try{if(!layout)layout=lpH1StudyStartLayout(tree,graph,hostW,hostH);}catch(_){layout=null;}
if(!lpH1StudyStartLayoutLooksValid(layout,tree)){layout=lpH1StudyStartLegacyLayout(tree,graph,hostW,hostH);}
const positions=layout&&layout.positions instanceof Map?layout.positions:new Map();const layoutParentOf=layout&&layout.parentOf instanceof Map?layout.parentOf:new Map();const worldW=Math.max(960,Math.round(Number(layout&&layout.worldW)||hostW||960));const worldH=Math.max(960,Math.round(Number(layout&&layout.worldH)||hostH||960));const cx=Math.round(Number(layout&&layout.cx)||(worldW/2));const cy=Math.round(Number(layout&&layout.cy)||(worldH/2));return{graph:graph||null,target,targetTitle,helperText:cfg.helperText,tree,layout,positions,layoutParentOf,worldW,worldH,cx,cy,hostW:Math.max(320,Number(hostW)||320),hostH:Math.max(420,Number(hostH)||420),world:layout&&layout.world||null,cacheKey:lpH1StudyStartPrepKey(target,hostW,hostH,cfg.mode),cacheBucket:lpH1StudyStartPrepBucket(hostW,hostH),mode:cfg.mode,};}
          ...
          <div class="lp-h1sg-topbar">
            <div class="lp-h1sg-tophead">
              <div class="lp-h1sg-tabs" role="tablist" aria-label="Map views">
                <button type="button" class="lp-h1sg-tab" data-lp-map-tab="toHere" role="tab" aria-label="Prerequisites" title="Prerequisites" aria-selected="false"><span class="lp-tab-label-long">Prerequisites</span><span class="lp-tab-label-short">Before</span></button>
                ...
                <button type="button" class="lp-h1sg-tab" data-lp-map-tab="fromHere" role="tab" aria-label="Dependents" title="Dependents" aria-selected="false"><span class="lp-tab-label-long">Dependents</span><span class="lp-tab-label-short">After</span></button>
              </div>
              <button type="button" class="lp-h1sg-tipbtn" data-lp-map-tip-toggle aria-label="Map tips" aria-expanded="false">
...
const dirBtn=e.target&&e.target.closest?e.target.closest('[data-lp-h1sg-dir]'):null;if(dirBtn){e.preventDefault();e.stopPropagation();return;}});window.addEventListener("keydown",(e)=>{if(e.isComposing||e.keyCode===229||e.defaultPrevented)return;if(e.key==="Escape"&&modal.style.display!=="none")hideH1StudyStartModal();});let dragState=null;const DRAG_THRESHOLD=8;const routeGestureCamera=lpFrameCameraQueue(()=>{if(!modal.classList.contains('lp-open'))return;lpH1StudyStartApplyTransformFast(modal,{skipClamp:true,gesture:true});});modal.__lpCancelGestureFrame=()=>{routeGestureCamera.cancel();const stage=lpH1StudyStartStage(modal);if(dragState&&stage){try{stage.releasePointerCapture(dragState.pid);}catch(_){}}
dragState=null;pinchState=null;clearTimeout(routeResizeTimer);modal.removeAttribute('data-lp-dragging');};modal.__lpScheduleGestureFrame=routeGestureCamera.schedule;function lpRouteSetDragCompositor(on){try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",!!on);}catch(_){}
...
function lpRouteScheduleTransform(){routeGestureCamera.schedule();}
function lpRouteFlushTransform(opts){routeGestureCamera.flush(()=>lpH1StudyStartApplyTransform(modal,opts));}
...
return null;};modal.addEventListener('touchstart',(e)=>{if(!modal.classList.contains('lp-open'))return;const stage=lpH1StudyStartStage(modal);if(!stage)return;if(!e.touches||e.touches.length<2)return;const target=e.target;if(target&&target.closest&&(target.closest('.lp-h1sg-topbar')||target.closest('.lp-mzoom')||target.closest('.lp-close')||target.closest('[data-lp-h1sg-fullscreen]')||target.closest('[data-lp-h1sg-dir-menu]')))return;const t1=e.touches[0],t2=e.touches[1];const rect=stage.getBoundingClientRect();const mx=((t1.clientX+t2.clientX)/2)-rect.left;const my=((t1.clientY+t2.clientY)/2)-rect.top;const view=lpH1StudyStartEnsureView(modal);const scale0=Math.max(lpMapMinActualScale(),Number(view.scale)||lpMapDefaultActualScale());lpRouteSetDragCompositor(true);pinchState={id1:t1.identifier,id2:t2.identifier,d0:Math.max(1e-6,Math.hypot(t2.clientX-t1.clientX,t2.clientY-t1.clientY)),s0:scale0,stageLeft:rect.left,stageTop:rect.top,wx0:(mx-(Number(view.tx)||0))/scale0,wy0:(my-(Number(view.ty)||0))/scale0};dragState=null;if(e.cancelable)e.preventDefault();},{passive:false,capture:true});modal.addEventListener('touchmove',(e)=>{if(!pinchState||!modal.classList.contains('lp-open'))return;const t1=routeTouchById(e.touches,pinchState.id1);const t2=routeTouchById(e.touches,pinchState.id2);if(!t1||!t2)return;const mx=((t1.clientX+t2.clientX)/2)-pinchState.stageLeft;const my=((t1.clientY+t2.clientY)/2)-pinchState.stageTop;const d1=Math.max(1e-6,Math.hypot(t2.clientX-t1.clientX,t2.clientY-t1.clientY));const view=lpH1StudyStartEnsureView(modal);const nextScale=Math.min(lpMapMaxActualScale(),Math.max(lpMapMinActualScale(),(Number(pinchState.s0)||lpMapDefaultActualScale())*(d1/Math.max(1e-6,Number(pinchState.d0)||1))));view.scale=nextScale;view.tx=mx-(Number(pinchState.wx0)||0)*nextScale;view.ty=my-(Number(pinchState.wy0)||0)*nextScale;view.userMoved=true;lpRouteScheduleTransform();if(e.cancelable)e.preventDefault();},{passive:false,capture:true});const endPinch=()=>{if(!pinchState)return;pinchState=null;lpRouteFlushTransform({skipClamp:false});requestAnimationFrame(()=>{if(!dragState&&!pinchState)lpRouteSetDragCompositor(false);});};modal.addEventListener('touchend',endPinch,{passive:true,capture:true});modal.addEventListener('touchcancel',endPinch,{passive:true,capture:true});function keepResizeFocusClear(previous){const view=lpH1StudyStartEnsureView(modal);const state=modal.__lpH1StudyState;if(!state||!(state.positions instanceof Map)||!(previous.stageW>0)||!(previous.stageH>0))return false;const loc=lpH1StudyStartPreferredCenterLoc(modal);const pos=loc&&state.positions.get(loc);if(!pos)return false;const scale=Number(view.scale)||1;const oldX=Number(pos.x)*scale+previous.tx;const oldY=Number(pos.y)*scale+previous.ty;if(oldX<0||oldX>previous.stageW||oldY<0||oldY>previous.stageH)return false;const stage=lpH1StudyStartStage(modal);const stageRect=stage&&stage.getBoundingClientRect?stage.getBoundingClientRect():null;if(!stageRect)return false;const node=lpH1StudyStartFindNodeEl(modal,loc);const rect=node&&node.getBoundingClientRect?node.getBoundingClientRect():null;const centerX=rect&&rect.width>0?(rect.left+rect.right)/2-stageRect.left:Number(pos.x)*scale+view.tx;const centerY=rect&&rect.height>0?(rect.top+rect.bottom)/2-stageRect.top:Number(pos.y)*scale+view.ty;const halfW=rect&&rect.width>0?rect.width/2:48;const halfH=rect&&rect.height>0?rect.height/2:18;const safe=lpH1StudyStartSafeInsets(modal);const minX=(Number(safe.left)||28)+halfW+8;const maxX=view.stageW-(Number(safe.right)||28)-halfW-8;const minY=(Number(safe.top)||24)+halfH+8;const maxY=view.stageH-(Number(safe.bottom)||24)-halfH-8;const desiredX=minX<=maxX?Math.max(minX,Math.min(maxX,centerX)):view.stageW/2;const desiredY=minY<=maxY?Math.max(minY,Math.min(maxY,centerY)):(minY+maxY)/2;const dx=desiredX-centerX,dy=desiredY-centerY;if(Math.abs(dx)<1&&Math.abs(dy)<1)return false;view.tx+=dx;view.ty+=dy;return true;}
let routeResizeTimer=0;const resizeRoute=()=>{clearTimeout(routeResizeTimer);routeResizeTimer=window.setTimeout(()=>{if(modal.style.display==='none'||!modal.classList.contains('lp-open'))return;if(dragState||pinchState){resizeRoute();return;}
const stage=lpH1StudyStartStage(modal),view=lpH1StudyStartEnsureView(modal);if(!stage)return;const width=stage.clientWidth,height=stage.clientHeight;if(Math.abs(width-view.stageW)<2&&Math.abs(height-view.stageH)<2)return;lpH1StudyRouteAnimStop(modal);const previous={stageW:view.stageW,stageH:view.stageH,tx:view.tx,ty:view.ty};view.tx+=(width-view.stageW)/2;view.ty+=(height-view.stageH)/2;view.stageW=width;view.stageH=height;lpRouteFlushTransform({skipClamp:true});if(keepResizeFocusClear(previous))lpRouteFlushTransform({skipClamp:true});},120);};window.addEventListener('resize',resizeRoute,{passive:true});if(typeof ResizeObserver==='function'){const routeResizeObserver=new ResizeObserver(resizeRoute);const routeStage=lpH1StudyStartStage(modal);if(routeStage)routeResizeObserver.observe(routeStage);}
return modal;}
function hideH1StudyStartModal(){const modal=lpRouteMapActiveModal();if(!modal)return;if(modal.__lpCancelGestureFrame)modal.__lpCancelGestureFrame();if(modal.__lpInitialOverview)modal.__lpInitialOverview.cancel();try{lpH1StudyRouteAnimStop(modal);}catch(_){}
...
function __lpMarkObservedContentRefs(){try{window.__lpObservedRel=currentRelPath();window.__lpObservedArticleInner=document.querySelector("article.md-content__inner")||null;window.__lpObservedMainContainer=document.querySelector("main.md-main")||null;}catch(_){window.__lpObservedRel="";window.__lpObservedArticleInner=null;window.__lpObservedMainContainer=null;}}
...
function scheduleMain(){if(window.__lpMainScheduled)return;window.__lpMainScheduled=true;__lpMarkObservedContentRefs();try{const rel=currentRelPath();if(isConceptPage(rel)){syncSecondaryTocVisibility(false);mountPanelShell(rel);}}catch(_){}
setTimeout(()=>{window.__lpMainScheduled=false;__lpRunMainSerial();},0);}
...
function restoreGuidedRouteFromHistory(){const state=lpGpsReadRouteState();const loc=currentRelPath();if(state&&lpGpsRouteIndexForLoc(state,loc)>=0)lpGpsWriteRouteTicket(loc);scheduleMain();}
window.addEventListener("popstate",restoreGuidedRouteFromHistory);window.addEventListener("pageshow",(e)=>{try{if(e&&e.persisted)restoreGuidedRouteFromHistory();}catch(_){}});const mo=new MutationObserver(()=>{const rel=currentRelPath();if(!isConceptPage(rel)){window.__lpObservedRel=rel;window.__lpObservedArticleInner=null;window.__lpObservedMainContainer=null;unmountMobileSheet();return;}
const currentInner=document.querySelector("article.md-content__inner")||null;const currentMain=document.querySelector("main.md-main")||null;const relChanged=window.__lpObservedRel!==rel;const innerChanged=currentInner!==(window.__lpObservedArticleInner||null);const mainChanged=currentMain!==(window.__lpObservedMainContainer||null);if(!relChanged&&!innerChanged&&!mainChanged)return;__lpReleaseStaleSearchUiIfSafe();scheduleMain();});try{const target=document.querySelector("main.md-main")||document.body;mo.observe(target,{childList:true,subtree:true});}catch(_){}
try{window.MkLearningPathUI=Object.assign(window.MkLearningPathUI||{},{isMobileMapModal:__lpIsMobileMapModal,});}catch(_){}})();(function installMapPresentation(){if(window.MkMapPresentation)return;const active=new Map();const reduce=window.matchMedia?window.matchMedia('(prefers-reduced-motion: reduce)'):null;function lpPresentationMotionBlocked(){if(document.hidden||(reduce&&reduce.matches))return true;if(document.documentElement.classList.contains('mk-motion-off'))return true;try{if(window.MkSiteMotion&&typeof window.MkSiteMotion.isReduced==='function'&&window.MkSiteMotion.isReduced())return true;}catch(_){}
return false;}
function lpPresentationOpen(modal){return!!(modal&&modal.isConnected&&modal.classList.contains('lp-open')&&modal.getAttribute('aria-hidden')!=='true');}
function lpPresentationStop(modal){const session=active.get(modal);if(!session)return;active.delete(modal);if(session.frame)window.cancelAnimationFrame(session.frame);if(session.timer)window.clearTimeout(session.timer);session.observers.forEach(observer=>observer.disconnect());session.listeners.forEach(([target,type,fn])=>target.removeEventListener(type,fn,true));session.animations.forEach(animation=>{try{animation.cancel();}catch(_){}});session.decorations.forEach(node=>{try{node.remove();}catch(_){}});}
function lpPresentationStopAll(){Array.from(active.keys()).forEach(lpPresentationStop);}
function lpPresentationLegend(modal){const host=modal.querySelector('.lp-map-context-copy');if(!host||host.querySelector('.lp-map-flow-key'))return;const legend=document.createElement('span');legend.className='lp-map-flow-key';legend.setAttribute('role','img');if(modal.id==='lp-map-modal'){legend.setAttribute('aria-label','Knowledge flow: prerequisites to this concept to dependents. Related concepts are undirected.');legend.innerHTML='<span class="lp-map-key-before">Prerequisites</span><span aria-hidden="true">→</span><span>Here</span><span aria-hidden="true">→</span><span class="lp-map-key-after">Dependents</span><span class="lp-map-key-related">· Related</span>';}else{const hasPath=!!(modal.__lpH1StudyState&&modal.__lpH1StudyState.path&&modal.__lpH1StudyState.path.length>1);legend.setAttribute('aria-label',hasPath?'Thin arrows show prerequisite connections. The highlighted selected path runs from Start to Goal.':'Thin arrows show prerequisite connections, from prerequisite to dependent. Select a node to choose a learning path.');legend.innerHTML='<span class="lp-map-key-context">Connections</span><span class="lp-map-key-selected"'+(hasPath?'':' hidden')+'>Selected path</span>';}
host.appendChild(legend);}
function lpPresentationPlay(modal){if(!modal)return false;lpPresentationStop(modal);lpPresentationLegend(modal);return false;}
document.addEventListener('mk:map-opened',event=>{const source=event&&event.detail&&event.detail.source;const id=source==='show-local-map-modal'?'lp-map-modal':(source==='show-guided-study-map'?'lp-h1sg-modal':'');if(id)Promise.resolve().then(()=>lpPresentationPlay(document.getElementById(id)));});document.addEventListener('visibilitychange',()=>{if(document.hidden)lpPresentationStopAll();});['pagehide','DOMContentSwitch','resize','orientationchange','mk-shop-trial-change','mk-shop-inventory-change'].forEach(type=>window.addEventListener(type,lpPresentationStopAll));['mk:site-motion-change','mk:motionchange'].forEach(type=>window.addEventListener(type,()=>{if(lpPresentationMotionBlocked())lpPresentationStopAll();}));if(reduce&&reduce.addEventListener)reduce.addEventListener('change',()=>{if(reduce.matches)lpPresentationStopAll();});window.MkMapPresentation=Object.freeze({play:lpPresentationPlay,stop:lpPresentationStop,activeCount:()=>active.size});})();(function(){try{if(document.getElementById('lp-concept-connections-polish-v1'))return;const st=document.createElement('style');st.id='lp-concept-connections-polish-v1';st.textContent=`
...
`;(document.head||document.documentElement).appendChild(st);}
function syncTabLabels(root){if(!root||!root.querySelectorAll)return;const labels={toHere:{long:'Prerequisites',short:'Before'},local:{long:'Nearby',short:'Nearby'},fromHere:{long:'Dependents',short:'After'}};const setLabel=(btn,label)=>{try{if(!btn||!label)return;const longLabel=document.createElement('span');longLabel.className='lp-tab-label-long';longLabel.textContent=String(label.long||'');const shortLabel=document.createElement('span');shortLabel.className='lp-tab-label-short';shortLabel.textContent=String(label.short||'');btn.replaceChildren(longLabel,shortLabel);}catch(_){}};root.querySelectorAll('[data-lp-map-tab="toHere"]').forEach((btn)=>setLabel(btn,labels.toHere));root.querySelectorAll('[data-lp-map-tab="local"]').forEach((btn)=>setLabel(btn,labels.local));root.querySelectorAll('[data-lp-map-tab="fromHere"]').forEach((btn)=>setLabel(btn,labels.fromHere));}
...
function boot(){ensureStyles();bindDrawerToggle();bindGlobals();syncDrawerClass();requestAnimationFrame(syncViewportVars);}
...
document.addEventListener("DOMContentSwitch",ensureStyle);})();(function(){const STYLE_ID='lp-mobile-map-header-offset-v2';const ROOT_VAR='--lp-mobile-top-stack-offset';let rafId=0;function isPhoneLike(){try{return!!(window.matchMedia&&(window.matchMedia('(max-width: 900px)').matches||window.matchMedia('(hover: none) and (pointer: coarse)').matches));}catch(_){return false;}}
...
function run(){ensureStyle();scheduleApply();bindModalObserver('lp-map-modal');bindModalObserver('lp-h1sg-modal');}
...
#lp-map-modal.lp-ipad-map-ui[data-lp-gesturing],
#lp-map-modal.lp-ipad-map-ui[data-lp-slider-zooming]{
  background:rgba(0,0,0,.46) !important;
  backdrop-filter:blur(6px) !important;
  -webkit-backdrop-filter:blur(6px) !important;
}
#lp-map-modal.lp-ipad-map-ui[data-lp-gesturing] .lp-fog-layer,
#lp-map-modal.lp-ipad-map-ui[data-lp-slider-zooming] .lp-fog-layer{
  display:block !important;
...
try{window.setTimeout(applyNow,600);}catch(_){}})();(function(){"use strict";const STYLE_ID="lp-map-drag-compositor-style-v14";function install(){if(document.getElementById(STYLE_ID))return;const st=document.createElement("style");st.id=STYLE_ID;st.textContent=`
/* Touch layers keep the same paint/compositing properties from map opening
   through finger-up. Promoting them on every first move stalls that frame. */
...
#lp-map-modal.lp-touch-compositor .lp-mapviewport svg,
#lp-map-modal.lp-touch-compositor .lp-mapviewport a.lp-node,
#lp-h1sg-modal.lp-touch-compositor .lp-mapviewport svg,
#lp-h1sg-modal.lp-touch-compositor .lp-mapviewport a.lp-node,
...
@media (hover: hover) and (pointer: fine){
  #lp-map-modal[data-lp-dragging] .lp-mapviewport,
  #lp-h1sg-modal[data-lp-dragging] .lp-mapviewport{
    transition:none !important;
    will-change:transform !important;
    contain:layout style !important;
    backface-visibility:hidden !important;
  ...
  #lp-map-modal[data-lp-dragging] .lp-mapviewport svg,
  #lp-map-modal[data-lp-dragging] .lp-mapviewport a.lp-node,
  #lp-h1sg-modal[data-lp-dragging] .lp-mapviewport svg,
  #lp-h1sg-modal[data-lp-dragging] .lp-mapviewport a.lp-node{
    backface-visibility:hidden !important;
    -webkit-backface-visibility:hidden !important;
  }
}
...
/* Only transitions are suppressed here: an in-flight transition fights the
   ...
   The transitions in question are hover effects, so this is a mouse rule: on a
   touch screen it only restyled every pill and both its pseudo-elements at the
   start and the end of each drag. */
@media (hover: hover) and (pointer: fine){
  #lp-map-modal[data-lp-dragging] .lp-mapviewport a.lp-node,
  #lp-map-modal[data-lp-dragging] .lp-mapviewport a.lp-node::before,
  #lp-map-modal[data-lp-dragging] .lp-mapviewport a.lp-node::after{
    transition:none !important;
  }
}
/* Links never transition (path.lp-edge sets transition:none for good) and the
   land has none, so no "svg path" here: naming every path made each drag start
   and end restyle all of them, well over a thousand on a long history. */
/* Nodes cannot be interacted with mid-pan anyway (the pointer is captured by the
   ...
   Hover and the grabbing cursor belong to a mouse. Both properties inherit, so
   on a touch screen they only made every drag start and end restyle the whole
   map; there the captured pointer already keeps taps off the pills. */
@media (hover: hover) and (pointer: fine){
  #lp-map-modal[data-lp-dragging] .lp-mapviewport a.lp-node{
    pointer-events:none !important;
  }
  #lp-map-modal[data-lp-dragging],
  #lp-h1sg-modal[data-lp-dragging]{
    cursor:grabbing !important;
  }
  #lp-h1sg-modal[data-lp-dragging] .lp-node{
    pointer-events:none !important;
  }
}
@media (hover: hover) and (pointer: fine){
  #lp-h1sg-modal[data-lp-dragging] .lp-node{ transition:none !important; }
}
#lp-h1sg-modal.lp-map-virtualized[data-lp-dragging] .lp-mapviewport{
  will-change:auto !important;
  contain:layout style !important;
}
...
try{document.addEventListener("mk:map-opened",syncOpenMap);}catch(_){}})();(function(){try{if(document.getElementById('lp-h1-route-mobile-end-frame-v29'))return;const st=document.createElement('style');st.id='lp-h1-route-mobile-end-frame-v29';st.textContent=`
      article.md-content__inner .lp-h1-routebar{
