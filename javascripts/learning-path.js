...
const radius=180,corridorRadius=90,feather=48;const bounds={left:Math.min(...points.map(p=>p.x))-radius,right:Math.max(...points.map(p=>p.x))+radius,top:Math.min(...points.map(p=>p.y))-radius,bottom:Math.max(...points.map(p=>p.y))+radius};return{points,segments,radius,corridorRadius,feather,bounds};}
...
return placements;}
function lpWorldLabelsPlace(vp,camera){const modal=vp&&vp.__lpWorldLabelModal;const terrain=modal&&modal.__lpWorldTerrain;if(!terrain||!terrain.labels||!terrain.labels.isConnected||!terrain.world)return;const ctx=terrain.__lpLabelHitContext||(terrain.__lpLabelHitContext=document.createElement('canvas').getContext('2d'));if(!ctx)return;const touch=!!(window.matchMedia&&window.matchMedia('(pointer: coarse), (max-width: 900px)').matches);const placed=new Map(lpVisibleCourseLabelPlacement(terrain.world.areas,terrain.world.reveal,camera,ctx,touch?700:1500).map(p=>[p.course,p]));const scale=Math.max(.001,Number(camera.scale)||1);for(const label of terrain.labels.querySelectorAll('.lp-course-territory-label')){const p=placed.get(label.dataset.lpCourse);label.style.display=p?'':'none';if(!p)continue;const fontSize=p.fontPx/scale;label.style.fontSize=`${fontSize}px`;label.setAttribute('text-anchor',p.textAnchor);label.setAttribute('x',String(p.x));label.setAttribute('y',String(p.y));const lines=label.querySelectorAll('tspan');lines.forEach((span,i)=>{span.setAttribute('x',String(p.x));span.setAttribute('y',String(p.y+(i-(lines.length-1)/2)*fontSize*1.08));});label.dataset.lpVisibleArea=String(Math.round(p.areaPx));label.dataset.lpVisibleFont=String(Math.round(p.fontPx*10)/10);}
terrain.labels.__lpVisibleLabelsPlaced=true;}
