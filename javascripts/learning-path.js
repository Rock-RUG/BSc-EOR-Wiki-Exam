...
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();const world=lpConceptWorld();const worldMode=!!(world&&nodes.length&&nodes.every(loc=>world.has(loc)));const GEO=worldMode?lpWorldGeometry(world):window.MkMapGeometry;modal.dataset.lpWorld=worldMode?(world.layout||"1"):"";const worldFootprint=worldMode&&lpExplorationMode()?lpWorldFootprint(world):null;modal.classList.toggle("lp-world-fog-on",!!worldFootprint);lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);try{const drawnLocs=new Set();for(const loc of nodes){if(!loc)continue;drawnLocs.add(normLoc(loc));drawnLocs.add(lpCanonKey(loc));}
...
function prereqSet(x){if(!prereqCache.has(x))prereqCache.set(x,new Set(getPrereqs(graph,x)||[]));return prereqCache.get(x);}
...
function lpRevealLearnedNeighbourhoods(session,base){const known=lpConceptWorld();const fresh=[];if(!known)return fresh;const api=window.MkMapExploration;const candidates=known.locs;const learned=candidates.filter(loc=>lpCanonKey(loc)!==lpCanonKey(cur)&&known.has(loc)&&!lpMapExpandGate(loc));api.retain(session,learned);const opened=new Set(api.roots(session).map(lpCanonKey));const shown=new Set((api.union(session,base).nodes||[]).map(lpCanonKey));for(const loc of learned){const key=lpCanonKey(loc);const ring=lpRootNeighbourhood(loc),placed=item=>known.has(item);ring.nodes=ring.nodes.filter(placed);ring.core=ring.core.filter(placed);ring.edges=ring.edges.filter(edge=>placed(edge.from)&&placed(edge.to));ring.level=new Map(Array.from(ring.level).filter(([item])=>placed(item)));if(!api.expand(session,key,ring))continue;const added=ring.nodes.filter(item=>!shown.has(lpCanonKey(item)));for(const item of added)shown.add(lpCanonKey(item));if(!opened.has(key))fresh.push({nodeId:normLoc(loc),addedNodeIds:added.filter(item=>lpCanonKey(item)!==key).map(normLoc)});}
return fresh;}
...
const nodeAction=document.createElement("div");nodeAction.className="lp-node-action";nodeAction.hidden=true;const gateNote=document.createElement("span");gateNote.className="lp-node-gate";gateNote.hidden=true;nodeAction.appendChild(gateNote);stage.querySelectorAll('.lp-node-action').forEach(item=>item.remove());stage.appendChild(nodeAction);let actionOwner="";let leaveTimer=0;function cancelPreviewLeave(){if(leaveTimer)window.clearTimeout(leaveTimer);leaveTimer=0;}
function positionNodeAction(){const anchor=nodeEls.get(actionOwner);if(!anchor||!anchor.isConnected||nodeAction.hidden)return;const r=anchor.getBoundingClientRect();const host=stage.getBoundingClientRect();const w=nodeAction.offsetWidth||154;const h=nodeAction.offsetHeight||44;nodeAction.style.left=`${Math.max(8, Math.min(host.width - w - 8, r.left - host.left + (r.width - w) / 2))}px`;const below=r.bottom-host.top+6;nodeAction.style.top=`${Math.max(8, below + h < host.height - 8 ? below : r.top - host.top - h - 6)}px`;}
function showNodeAction(loc){cancelPreviewLeave();if(lpCanonKey(loc)===lpCanonKey(cur)){actionOwner='';nodeAction.hidden=true;return;}
actionOwner=loc;nodeAction.dataset.lpOwner=lpCanonKey(loc);const gate=lpMapExpandGate(loc);const text=gate?lpNodeRevealHint(loc,gate):"";gateNote.hidden=!text;gateNote.textContent=text;nodeAction.classList.toggle("is-gated",!!gate);nodeAction.hidden=!text;if(text)positionNodeAction();}
function lpNodeRevealHint(loc,gate){let hidden=0;try{const shown=new Set(Array.from(nodeEls.keys(),lpCanonKey));for(const key of lpRootNeighbourhood(loc).nodes)if(!shown.has(lpCanonKey(key)))hidden+=1;}catch(_){}
const what=hidden?`${hidden} more concept${hidden === 1 ? "" : "s"} around it`:"the map around it";return gate==="rate"?`Rate this concept to reveal ${what}.`:`Open and rate this concept to reveal ${what}.`;}
function schedulePreviewLeave(){cancelPreviewLeave();leaveTimer=window.setTimeout(()=>{leaveTimer=0;if(st2.__lpRenderId!==renderId)return;clearFocus();},90);}
nodeAction.addEventListener('pointerenter',cancelPreviewLeave);nodeAction.addEventListener('pointerleave',schedulePreviewLeave);nodeAction.addEventListener('focusin',cancelPreviewLeave);nodeAction.addEventListener('focusout',event=>{if(!nodeAction.contains(event.relatedTarget))schedulePreviewLeave();});nodeAction.addEventListener('pointerdown',event=>event.stopPropagation());st2.__lpPositionNodeAction=positionNodeAction;st2.__lpHideNodeAction=()=>{nodeAction.hidden=true;};st2.__lpDisposeNodeAction=()=>{cancelPreviewLeave();if(measuredNodesObserver)measuredNodesObserver.disconnect();if(measuredEdgesRaf)window.cancelAnimationFrame(measuredEdgesRaf);measuredEdgesRaf=0;nodeAction.remove();};function bindNodeInteractions(el,loc){if(!el||el.dataset.lpHoverBound==="1")return;el.dataset.lpHoverBound="1";if(measuredNodesObserver)measuredNodesObserver.observe(el);el.setAttribute('role','button');el.removeAttribute('aria-expanded');el.setAttribute('aria-description',lpCanonKey(loc)===lpCanonKey(cur)?'Current concept. Preview its connections.':'Preview connections; activate to open this concept. Rating it reveals the map around it.');let down=null;let moved=false;const live=()=>st2.__lpRenderId===renderId&&el.isConnected;const refresh3d=()=>{const engine=modal.__lpWebgl3dEngine;if(!engine||engine.kind!=='local')return;engine.hoverKey=lpCanonKey(loc);lpWebgl3dRebuild(engine);positionNodeAction();};const preview=()=>{if(!live()||st2.dragging||modal.classList.contains('lp-webgl3d-dragging')||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0))return;cancelPreviewLeave();lpMarkLocalMapInteractionBusy(180);const focusChanged=st2.__lpFocusedLoc!==loc;if(focusChanged)applyFocus(loc);showNodeAction(loc);if(lpNodeShouldRunRevealPreview(el,loc))lpStartRevealPreview(el,loc);if(focusChanged||(modal.__lpWebgl3dEngine&&modal.__lpWebgl3dEngine.hoverKey!==lpCanonKey(loc)))refresh3d();};const retain=()=>{if(!live())return;cancelPreviewLeave();lpPauseRevealPlayback(loc,el);if(lpCanonKey(loc)===lpCanonKey(cur)){showNodeAction(loc);return;}
lpNavigate(toAbsoluteUrl(loc),loc);};el.addEventListener('pointerenter',event=>{if(event.pointerType&&event.pointerType!=='mouse')return;preview();},{passive:true});el.addEventListener('pointerleave',()=>{if(st2.__lpHoverLoc!==loc)return;lpPauseRevealPlayback(loc,el);schedulePreviewLeave();},{passive:true});el.addEventListener('focus',preview);el.addEventListener('blur',event=>{if(!nodeAction.contains(event.relatedTarget))schedulePreviewLeave();});el.addEventListener('pointerdown',event=>{down={id:event.pointerId,x:event.clientX,y:event.clientY};moved=false;},{passive:true});el.addEventListener('pointermove',event=>{if(down&&down.id===event.pointerId&&Math.hypot(event.clientX-down.x,event.clientY-down.y)>10)moved=true;},{passive:true});el.addEventListener('pointercancel',()=>{moved=true;down=null;});el.addEventListener('click',event=>{if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;event.preventDefault();event.stopImmediatePropagation();const dragged=moved||!!st2.dragging||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0);down=null;if(!dragged)retain();moved=false;},true);el.addEventListener('keydown',event=>{if(!live())return;if(event.key==='Escape'){if(st2.__lpFocusedLoc||st2.__lpHoverLoc||!nodeAction.hidden){event.preventDefault();event.stopPropagation();clearFocus();}
return;}
...
function lpAppendWorldExpansion(focusLoc){if(!worldMode||st2.__lpRenderId!==renderId)return false;const next=window.MkMapExploration.union(exploration,baseNeighbourhood);const nextNodes=next.nodes||[];if(!nextNodes.every(loc=>world.has(loc)))return false;__lpStopAllFlows();removeExtraFocusNodes();try{__lpHiClear();}catch(_){}
const shown=new Set(nodes);const fresh=nextNodes.filter(loc=>!shown.has(loc));if(next.level)next.level.forEach((value,key)=>level.set(key,value));for(const key of(next.core||[]))coreSet.add(key);for(const p of fresh){const q=world.point(p);pos.set(p,{x:q.x,y:q.y});prefPos.set(p,{x:q.x,y:q.y});}
const created=[];for(const p of fresh){nodes.push(p);baseNodeSet.add(p);if(lpCreateBaseNode(p))created.push(p);}
for(const p of created)measureNodeEl(p,nodeEls.get(p));for(const p of nodes){const el=nodeEls.get(p);if(el)lpTagBaseNode(el,p);}
const drawn=new Set(edges.map(e=>lpFocusEdgeKey(orientEdge(e))));const freshEdges=[];for(const e of(next.edges||[])){const key=lpFocusEdgeKey(orientEdge(e));if(drawn.has(key))continue;drawn.add(key);freshEdges.push(e);}
if(freshEdges.length){const routed=GEO.route(geometryNodes(),freshEdges.map(orientEdge),geometryOptions);routed.routes.forEach((route,index)=>routedBase.set(lpFocusEdgeKey({...route,type:(freshEdges[index]||{}).type}),route));for(const e of freshEdges){edges.push(e);const path=lpDrawBaseEdge(e);if(path)lpBaselineEdge(path);}}
for(const p of created){const el=nodeEls.get(p);lpApplyBaselineNodeVisual(el,p);bindNodeInteractions(el,p);}
exploration.positions=new Map(Array.from(pos,([key,point])=>[key,{...point}]));lpWorldFogPaint(modal,vp,world,worldFootprint,nodes,cur);try{const drawnLocs=modal.__lpRenderedLocSet;if(drawnLocs)for(const p of fresh){drawnLocs.add(normLoc(p));drawnLocs.add(lpCanonKey(p));}}catch(_){}
lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);if(created.length){const mathEls=created.map(p=>nodeEls.get(p)).filter(el=>{try{return lpNodeTitleNeedsTypeset(el.querySelector('.lp-node-title'));}catch(_){return false;}});if(mathEls.length)requestAnimationFrame(()=>{Promise.resolve().then(()=>typesetMathSafe(mathEls.map(el=>el.querySelector('.lp-node-title')||el))).then(()=>{mathEls.forEach(el=>{try{lpCacheRenderedNodeTitle(graph,el);}catch(_){}});}).catch(()=>{});});}
st2.__lpHoverLoc=null;st2.__lpFocusedLoc=null;applyFocus(focusLoc);return true;}
function clearFocus(){const hadPreview=st2.__lpFocusedLoc||st2.__lpHoverLoc||!nodeAction.hidden||st2.__lpRevealFocusLoc;cancelPreviewLeave();nodeAction.hidden=true;actionOwner="";if(!hadPreview)return;try{const revealLoc=st2&&st2.__lpRevealFocusLoc;if(revealLoc&&lpRevealNavIsActive(revealLoc))return;}catch(_){}
...
if(st2.__lpRenderId!==renderId)return;__lpStopAllFlows();removeExtraFocusNodes(loc);st2.__lpHoverLoc=loc;const locVisual=nodeVisual.get(loc)||lpMapVisualForLoc(loc);const locRec=getMastery(loc);const locMastery=(locRec&&typeof locRec.m==="number")?locRec.m:null;const isExpandable=true;const ctx=lpMapShownDirectSets(classifyDirectSets(loc),!lpMapExpandGate(loc));const directCurEdges=lpDirectEdgesToCurrent(loc);const focusSet=new Set([loc,cur,...ctx.pres,...ctx.deps,...ctx.rels]);for(const e0 of directCurEdges){if(e0&&e0.from)focusSet.add(e0.from);if(e0&&e0.to)focusSet.add(e0.to);}
        ...
        <div class="lp-map-topbar">
          ...
          <div class="lp-map-context">
            <div class="lp-map-context-copy" role="status" aria-live="polite" aria-atomic="true">
              <span data-lp-map-context>Drag to explore. Click a concept to open it.</span>
              ${window.__mkExamMode ? '' : '<span class="lp-map-context-note" data-lp-map-treasure-note>Treasure: turn on Knowledge masking to uncover the map by learning. See Map tips.</span>'}
              <span class="lp-map-context-note" data-lp-map-explore-status hidden></span>
              <span class="lp-map-context-note" data-lp-map-fit-status hidden></span>
            </div>
            <div class="lp-map-options" data-lp-map-options>
              <label class="lp-map-option" title="Fog of war: the map opens around the concepts you have opened and rated.">
                <span class="lp-map-option-text">Knowledge masking</span>
...
if(modal&&modal.dataset&&modal.dataset.lpWorld)lpWorldWindowFollow(vp,readWorldCamera);}
function readWorldCamera(){const stageEl=modal.querySelector('.lp-mapstage');return{scale:st.scale,tx:st.tx,ty:st.ty,stageW:Number(st.__lpStageW)||(stageEl&&stageEl.clientWidth)||0,stageH:Number(st.__lpStageH)||(stageEl&&stageEl.clientHeight)||0};}
...
function endDrag(opts){lpMarkLocalMapInteractionBusy(180);dragging=false;const pidToRelease=activePid;activePid=null;if(pidToRelease!=null&&panSurface&&typeof panSurface.releasePointerCapture==="function"){try{panSurface.releasePointerCapture(pidToRelease);}catch(_){}}
...
const hardTouchGestureCleanup=(e)=>{try{const remainingTouches=e&&e.touches?Number(e.touches.length)||0:0;if(remainingTouches>0)return;activePtrs.clear();pinch=null;if(!dragging&&!pendingNodeDrag&&!st.__lpZoomDragging){try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",false);}catch(_){}
...
if(stage)stage.style.visibility='';lp3dActivateModal(modal,'local');lpBindMapOptions(modal);renderLocalMapModal(graph);if(worldLoading&&typeof worldLoading.then==='function')worldLoading.then(world=>{if(world&&modal.classList.contains('lp-open')&&!modal.dataset.lpWorld)renderLocalMapModal(graph);}).catch(error=>{try{console.error('[concept map] world render failed',error);}catch(_){}});try{if(usingDocSurface){lpUpdateMapDocumentSurface(modal);requestAnimationFrame(()=>{try{lpUpdateMapDocumentSurface(modal);if(window.__lpMapState&&!preserveView)lpLocalMapResetView(modal,window.__lpMapState);modal.__lpApplyTransform&&modal.__lpApplyTransform();}catch(_){}});}}catch(_){}}
function hideLocalMapModal(){try{document.getElementById("lp-map-modal")?.__lpMapChests?.suspend();}catch(_){}
lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpViewportResizeCleanup)modal.__lpViewportResizeCleanup();try{if(modal.__lpStopZoomGesture)modal.__lpStopZoomGesture();}catch(_){}
...
return"full";}
function lpMapTabHelperText(kind,isPhone){const phone=typeof isPhone==="boolean"?isPhone:!!__lpIsPhoneTouch();if(String(kind||"")==="local"){const controls=phone?"Tap a concept to open its page.":"Hover to preview connections. Click a concept to open its page.";return controls+" Drag freely to explore."+(phone?" Before shows prerequisites; After shows dependents.":"")+(window.__mkExamMode?"":" The map shows your current page's neighbours and the surroundings of every concept you have opened and rated. Knowledge masking adds fog; switching it off keeps the same learning boundary. Concepts rated Clear or Mastered light up in their course colour. Treasure needs Knowledge masking, a connected account and a concept whose ring the map has just revealed. Each concept gives one chance per UTC day. Crown/gold/silver/bronze chances: 1/1,000, 1/100, 1/20, 1/5; daily limits: 1, 1, 2, 5, including unopened chests. Open before midnight UTC for XP, EORbits and possible shop items; owned items become EORbits. Full rules: Site guide → Map treasures.");}
...
function lpH1StudyStartApplyTransformFast(modal,opts){const vp=lpH1StudyStartViewport(modal);if(!vp)return;const view=lpH1StudyStartEnsureView(modal);if(!(opts&&opts.skipClamp))lpH1StudyStartClampPan(modal);const scale=Number(view.scale)||1;const transformText=lp3dViewportTransform(modal,'route',Number(view.tx)||0,Number(view.ty)||0,scale,true);if(vp.style.transform!==transformText)vp.style.transform=transformText;lpH1StudyWorldWindowFollow(modal,vp);lpH1StudySyncScaleMeta(modal,scale,!!(opts&&opts.syncZoomMeta));const state=modal&&modal.__lpH1StudyState;if(state&&state.positions instanceof Map&&state.positions.size>48){lpH1StudyStartCullForDrag(modal,true,false);}}
function lpH1StudyWorldWindowFollow(modal,vp){if(!modal||!vp||!modal.dataset||!modal.dataset.lpWorld||!vp.querySelector(':scope > svg.lp-map-terrain'))return;vp.__lpWorldWindowScope='route';lpWorldWindowFollow(vp,()=>{const view=lpH1StudyStartEnsureView(modal),stage=lpH1StudyStartStage(modal);return{scale:view.scale,tx:view.tx,ty:view.ty,stageW:(stage&&stage.clientWidth)||0,stageH:(stage&&stage.clientHeight)||0};});}
...
function hideH1StudyStartModal(){const modal=lpRouteMapActiveModal();if(!modal)return;if(modal.__lpInitialOverview)modal.__lpInitialOverview.cancel();try{lpH1StudyRouteAnimStop(modal);}catch(_){}
