...
function lpMapGeometryOptions(nodes){const courseLabels={},labelSizes={};for(const node of nodes){if(courseLabels[node.course])continue;const label=__lpCourseLabelFromRelPath(node.id)||node.course.split('/').pop().replace(/-/g,' ');courseLabels[node.course]=label;labelSizes[node.course]={width:Math.min(300,Math.max(112,label.length*7.4)),height:40};}
return{cellSize:18,clearance:12,nodeGap:60,regionPadding:16,regionReach:220,courseLabels,labelSizes};}
function lpMapPaintTerritories(svg,areas){const ns='http://www.w3.org/2000/svg';const layer=document.createElementNS(ns,'g');layer.classList.add('lp-course-territories');layer.setAttribute('aria-hidden','true');const courses=Array.from(new Set((areas||[]).map(area=>area.course))).sort();const hues=[192,35,282,142,345,75];for(const area of areas||[]){const hue=hues[courses.indexOf(area.course)%hues.length];const path=document.createElementNS(ns,'path');path.classList.add('lp-course-territory');path.setAttribute('d',area.d);path.setAttribute('fill-rule','evenodd');path.style.setProperty('--lp-territory-hue',String(hue));path.dataset.lpCourse=area.course;layer.appendChild(path);if(!area.label)continue;const label=document.createElementNS(ns,'text');label.classList.add('lp-course-territory-label');label.setAttribute('x',area.label.x);label.setAttribute('y',area.label.y);label.setAttribute('text-anchor','middle');label.setAttribute('dominant-baseline','middle');label.dataset.lpCourse=area.course;label.textContent=area.label.text;label.setAttribute('textLength',Math.min(area.label.width-8,area.label.text.length*7.1));label.setAttribute('lengthAdjust','spacingAndGlyphs');layer.appendChild(label);}
...
const placed=window.MkMapGeometry.place(geometryNodes(),{...geometryOptions,fixedIds:Array.from(baseNodeSet)});for(const point of placed.nodes){if(baseNodeSet.has(point.id))continue;pos.set(point.id,{x:point.x,y:point.y});const el=nodeEls.get(point.id);if(el){el.style.left=`${point.x}px`;el.style.top=`${point.y}px`;}}}
...
function showNodeAction(loc){cancelPreviewLeave();actionOwner=loc;nodeAction.dataset.lpOwner=lpCanonKey(loc);openConcept.href=toAbsoluteUrl(loc);nodeAction.hidden=false;positionNodeAction();}
