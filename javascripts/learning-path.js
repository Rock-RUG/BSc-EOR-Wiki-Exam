...
function lp3dInstallRotationControls(modal,kind){return;if(!modal||!modal.querySelector||!lp3dMapEnabled())return;try{const box=modal.querySelector('.lp-mbox')||modal;let ctl=modal.querySelector('.lp-3d-rotctl');if(!ctl){ctl=document.createElement('div');ctl.className='lp-3d-rotctl';ctl.setAttribute('aria-label','3D rotation controls');ctl.innerHTML=`
        ...
        `;box.appendChild(ctl);const stop=(e)=>{try{if(e&&e.cancelable)e.preventDefault();if(e&&e.stopPropagation)e.stopPropagation();if(e&&typeof e.stopImmediatePropagation==='function')e.stopImmediatePropagation();}catch(_){}};ctl.addEventListener('click',(e)=>{const btn=e&&e.target&&e.target.closest?e.target.closest('[data-lp-3d-rot]'):null;if(!btn)return;stop(e);const activeKind=ctl.dataset.lp3dKind||'local';const cur=modal.__lp3d||lp3dDefaultRot(activeKind);const action=btn.getAttribute('data-lp-3d-rot')||'';let rx=Number(cur.rx);let ry=Number(cur.ry);if(!Number.isFinite(rx)||!Number.isFinite(ry)){const d=lp3dDefaultRot(activeKind);rx=d.rx;ry=d.ry;}
...
lp3dSetModalRotation(modal,activeKind,rx,ry);},true);let drag=null;const dragHandle=ctl.querySelector('[data-lp-3d-drag]')||ctl;dragHandle.addEventListener('pointerdown',(e)=>{stop(e);const activeKind=ctl.dataset.lp3dKind||'local';const cur=modal.__lp3d||lp3dDefaultRot(activeKind);drag={id:e.pointerId,kind:activeKind,x:Number(e.clientX)||0,y:Number(e.clientY)||0,rx:Number.isFinite(Number(cur.rx))?Number(cur.rx):lp3dDefaultRot(activeKind).rx,ry:Number.isFinite(Number(cur.ry))?Number(cur.ry):lp3dDefaultRot(activeKind).ry,};try{dragHandle.setPointerCapture(e.pointerId);}catch(_){}
try{ctl.classList.add('is-dragging');}catch(_){}},true);dragHandle.addEventListener('pointermove',(e)=>{if(!drag||(e.pointerId!=null&&drag.id!==e.pointerId))return;stop(e);const dx=(Number(e.clientX)||0)-drag.x;const dy=(Number(e.clientY)||0)-drag.y;lp3dSetModalRotation(modal,drag.kind,drag.rx-dy*0.18,drag.ry+dx*0.18);},true);const endDrag=(e)=>{if(!drag)return;stop(e);try{dragHandle.releasePointerCapture(drag.id);}catch(_){}
drag=null;try{ctl.classList.remove('is-dragging');}catch(_){}};dragHandle.addEventListener('pointerup',endDrag,true);dragHandle.addEventListener('pointercancel',endDrag,true);ctl.addEventListener('pointerdown',stop,true);ctl.addEventListener('touchstart',stop,{capture:true,passive:false});}
...
function lpWebgl3dSetRotation(engine,rx,ry){if(!engine)return;engine.rx=lp3dClampNumber(rx,LP_3D_ROT_X_MIN,LP_3D_ROT_X_MAX,lp3dDefaultRot(engine.kind).rx);engine.ry=lp3dClampNumber(ry,LP_3D_ROT_Y_MIN,LP_3D_ROT_Y_MAX,lp3dDefaultRot(engine.kind).ry);lpWebgl3dDraw(engine);}
function lpWebgl3dBindCanvas(engine){if(!engine||!engine.canvas||engine.canvas.__lpWebgl3dPointerBound)return;const canvas=engine.canvas;canvas.__lpWebgl3dPointerBound=true;canvas.setAttribute('aria-label','Rotatable 3D knowledge map canvas');canvas.tabIndex=-1;let drag=null;const stop=(e)=>{try{if(e&&e.cancelable)e.preventDefault();if(e&&e.stopPropagation)e.stopPropagation();}catch(_){}};canvas.addEventListener('pointerdown',(e)=>{if(e&&e.button!=null&&e.button!==0)return;stop(e);const cur=engine.modal&&engine.modal.__lp3d?engine.modal.__lp3d:lp3dDefaultRot(engine.kind);drag={id:e.pointerId,x:Number(e.clientX)||0,y:Number(e.clientY)||0,rx:Number.isFinite(Number(cur.rx))?Number(cur.rx):lp3dDefaultRot(engine.kind).rx,ry:Number.isFinite(Number(cur.ry))?Number(cur.ry):lp3dDefaultRot(engine.kind).ry,};try{canvas.setPointerCapture(e.pointerId);}catch(_){}
try{engine.modal.classList.add('lp-webgl3d-dragging');}catch(_){}},true);canvas.addEventListener('pointermove',(e)=>{if(!drag||(e.pointerId!=null&&drag.id!==e.pointerId))return;stop(e);const dx=(Number(e.clientX)||0)-drag.x;const dy=(Number(e.clientY)||0)-drag.y;lp3dSetModalRotation(engine.modal,engine.kind,drag.rx-dy*0.22,drag.ry+dx*0.22);},true);const end=(e)=>{if(!drag)return;stop(e);try{canvas.releasePointerCapture(drag.id);}catch(_){}
drag=null;try{engine.modal.classList.remove('lp-webgl3d-dragging');}catch(_){}};canvas.addEventListener('pointerup',end,true);canvas.addEventListener('pointercancel',end,true);}
function lpWebgl3dObserve(engine){if(!engine||!engine.viewport||engine.__observing)return;engine.__observing=true;let timer=0;const schedule=()=>{if(timer)return;timer=window.setTimeout(()=>{timer=0;try{if(engine.modal&&engine.modal.__lpWebgl3dEngine===engine)lpWebgl3dRebuild(engine);}catch(_){}},80);};try{const mo=new MutationObserver((records)=>{const relevant=(records||[]).some((record)=>{const target=record&&record.target;if(target&&target.nodeType===1&&target.closest&&target.closest('.lp-webgl3d-overlay'))return false;return true;});if(relevant)schedule();});mo.observe(engine.viewport,{childList:true,subtree:true,attributes:true,attributeFilter:['data-lp-visual','data-lp-unvisited','data-lp-rel-only','data-lp-loc']});engine.__mo=mo;}catch(_){}
...
let __lpMasteryCacheSignature=null;let __lpMasteryCacheAll=null;let __lpMasteryRecordCache=new Map();function lpInvalidateMasteryCache(){__lpMasteryCacheSignature=null;__lpMasteryCacheAll=null;__lpMasteryRecordCache=new Map();}
...
if(!all||typeof all!=="object"||Array.isArray(all))return null;__lpMasteryCacheSignature=sig;__lpMasteryCacheAll=all;__lpMasteryRecordCache=new Map();return all;}
function getMastery(loc){if(!window.ConceptMastery)return null;const key=normLoc(loc);if(!key)return null;const all=lpReadMasteryAllCached();if(all){if(__lpMasteryRecordCache.has(key))return __lpMasteryRecordCache.get(key);const raw=all[key]||all[key.replace(/\/+$/,"")]||null;const rec=raw?(typeof window.ConceptMastery._normaliseRecord==="function"?window.ConceptMastery._normaliseRecord(raw):raw):null;__lpMasteryRecordCache.set(key,rec);return rec;}
return window.ConceptMastery.get(key);}
function lpCurrentMasterySignature(){try{return String(localStorage.getItem(MASTERY_KEY)||"");}catch(_){return"";}}
function lpWasVisited(loc){const rec=getMastery(loc);if(!rec)return false;return!!(rec.visited||(Number(rec.visitCount)||0)>0||(Number(rec.viewCount)||0)>0||(Number(rec.lastViewed)||0)>0);}
try{if(!window.__lpMasteryCacheInvalidationInstalled){window.__lpMasteryCacheInvalidationInstalled=true;window.addEventListener("conceptMasteryChanged",lpInvalidateMasteryCache);window.addEventListener("storage",(e)=>{try{if(e&&e.key===MASTERY_KEY)lpInvalidateMasteryCache();}catch(_){}});}}catch(_){}
...
function __lpRawMathTitleFromGraphNode(graph,key){try{const node=__lpReadGraphNodeForLoc(graph,key)||null;if(!node||typeof node!=="object")return"";const vals=[node.title,node.name,node.label].map((x)=>cleanTitle(x||"")).filter(Boolean);for(const v of vals)if(__lpMapHasMath(v))return v;return"";}catch(_){return"";}}
function __lpShouldBlockTitleMutation(graph,key,incomingTitle){try{const incoming=cleanTitle(incomingTitle||"");if(!incoming)return true;const sourceMath=__lpRawMathTitleFromGraphNode(graph,key);if(sourceMath&&!__lpMapHasMath(incoming))return true;if(!__lpMapHasMath(incoming)&&__lpTitleTextLooksBrokenForLoc(key,incoming))return true;}catch(_){}
return false;}
...
return sanitized;}
function __lpReadGraphNodeForLoc(graph,key){try{const nodes=graph&&graph.nodes;const k=normLoc(key);const ck=lpCanonKey(k);if(!nodes||!k)return null;if(nodes instanceof Map){if(nodes.has(k))return nodes.get(k);for(const[kk,vv]of nodes.entries())if(lpCanonKey(kk)===ck)return vv;return null;}
if(Object.prototype.hasOwnProperty.call(nodes,k))return nodes[k];const bare=k.replace(/\/+$/g,"");const slash=bare?`${bare}/`:"";if(bare&&Object.prototype.hasOwnProperty.call(nodes,bare))return nodes[bare];if(slash&&Object.prototype.hasOwnProperty.call(nodes,slash))return nodes[slash];for(const kk of Object.keys(nodes))if(lpCanonKey(kk)===ck)return nodes[kk];}catch(_){}
return null;}
...
else if(m===1){bucket="fuzzy";label="Fuzzy";}
...
if(meta.m==null)parts.push('Not rated yet.');else if(meta.m===0)parts.push("Currently marked Unknown.");else if(meta.m===1)parts.push('Currently marked Fuzzy.');else if(meta.m===2)parts.push('Currently marked Clear.');else if(meta.m===3)parts.push('Currently marked Mastered.');return parts.join(' ');}
...
function showKnowledgeGpsModal(graph,targetLoc){ensureGpsStylesOnce();ensureKnowledgeGpsModalStylesOnce();const modal=ensureKnowledgeGpsModal(graph);modal.__lpRender(graph,targetLoc);modal.style.display='flex';modal.classList.add('lp-open');modal.setAttribute('aria-hidden','false');setLpModalScrollLock(true);}
function hideKnowledgeGpsModal(){const modal=document.getElementById('lp-gps-modal');if(!modal||modal.style.display==='none')return;modal.classList.remove('lp-open');modal.setAttribute('aria-hidden','true');modal.style.display='none';setLpModalScrollLock(false);}
...
function injectStylesOnce(){if(document.getElementById("lp-style-v2"))return;const st=document.createElement("style");st.id="lp-style-v2";st.textContent=`
      ...
      #lp-side-panel.lp-pending{ opacity:0; pointer-events:none; }
...
#lp-mobile-sheet #lp-side-panel .lp-name math{
  ...
  margin-bottom:0 !important;
      ...
      /* modal */
      #lp-map-modal{
        position:fixed; inset:0; z-index:2147483300; display:none;
...
@media (max-width: 768px){
  #lp-map-modal .lp-mzoom{
    ...
    width: min(640px, calc(100% - 24px));
      ...
      #lp-map-modal.lp-slider-zooming .lp-fog-layer{
        opacity: 0 !important;
        filter: none !important;
        -webkit-filter: none !important;
      }
      #lp-map-modal.lp-slider-zooming .lp-mapviewport svg .lp-flow-arrow{
        display: none !important;
      }
      #lp-map-modal.lp-slider-zooming .lp-mapviewport svg path.lp-rel-edge.lp-rel-anim{
        animation: none !important;
      }
      #lp-map-modal.lp-slider-zooming a.lp-node::after{
        box-shadow: none !important;
      }
      @media (min-width: 901px) and (hover: hover) and (pointer: fine){
...
function __lpGetTitleCache(graph,key){try{const k=normLoc(key);const ck=lpCanonKey(k);const c=graph&&graph.__lpTitleByLoc;if(!c||!k)return"";const candidates=[];const addCandidate=(locKey,value)=>{const t=cleanTitle(value||"");if(!t)return;if(lpCanonKey(locKey)!==ck)return;candidates.push({loc:locKey,title:t,score:__lpScoreTitleCandidate(locKey,t)});};if(c instanceof Map){if(c.has(k))addCandidate(k,c.get(k));const kBare=k.replace(/\/+$/g,"");const kSlash=kBare?(kBare+"/"):"";if(kBare&&c.has(kBare))addCandidate(kBare,c.get(kBare));if(kSlash&&c.has(kSlash))addCandidate(kSlash,c.get(kSlash));for(const[kk,vv]of c.entries())addCandidate(kk,vv);}else{if(Object.prototype.hasOwnProperty.call(c,k))addCandidate(k,c[k]);const kBare=k.replace(/\/+$/g,"");const kSlash=kBare?(kBare+"/"):"";if(kBare&&Object.prototype.hasOwnProperty.call(c,kBare))addCandidate(kBare,c[kBare]);if(kSlash&&Object.prototype.hasOwnProperty.call(c,kSlash))addCandidate(kSlash,c[kSlash]);for(const kk of Object.keys(c))addCandidate(kk,c[kk]);}
...
function __lpSetTitleCache(graph,key,title,force){try{const k=normLoc(key);const incoming=cleanTitle(title||"");if(__lpShouldBlockTitleMutation(graph,k,incoming))return;const t=__lpRepairTitleMathFromLoc(k,incoming);if(!k||!t)return;const c=__lpEnsureTitleCache(graph);if(!c)return;const prevBest=cleanTitle(__lpGetTitleCache(graph,k)||"");const nextScore=__lpScoreTitleCandidate(k,t);const prevScore=__lpScoreTitleCandidate(k,prevBest);if(!force&&prevBest&&nextScore<prevScore)return;if(prevBest&&lpHasMathMarkup(prevBest)&&!lpHasMathMarkup(t))return;if(c instanceof Map){c.set(k,t);for(const[kk]of c.entries()){if(lpCanonKey(kk)===lpCanonKey(k))c.set(kk,t);}}else{c[k]=t;for(const kk of Object.keys(c)){if(lpCanonKey(kk)===lpCanonKey(k))c[kk]=t;}}}catch(_){}}
function __lpForceGraphNodeTitle(graph,key,title){try{const k=normLoc(key);const ck=lpCanonKey(k);const incoming=cleanTitle(title||"");if(__lpShouldBlockTitleMutation(graph,k,incoming))return;const t=__lpRepairTitleMathFromLoc(k,incoming);if(!graph||!graph.nodes||!t||!k)return;let hit=false;for(const kk of Object.keys(graph.nodes)){if(lpCanonKey(kk)!==ck)continue;const oldNode=graph.nodes[kk];const nextNode=(oldNode&&typeof oldNode==="object")?oldNode:{};const oldTitle=cleanTitle((nextNode&&(nextNode.title||nextNode.name||nextNode.label))||"");if(oldTitle&&lpHasMathMarkup(oldTitle)&&!lpHasMathMarkup(t)){hit=true;continue;}
...
if(!hit)graph.nodes[k]={title:t};}catch(_){}}
...
function renderLocalMapModal(graph){const modal=document.getElementById("lp-map-modal");if(!modal)return;lpSyncLocalMapAnimationAccess(modal);lpEnsureMapRedesignPatchStyles();const stage=modal.querySelector(".lp-mapstage");if(!stage)return;const vp=stage.querySelector("[data-map-viewport]")||stage;vp.innerHTML="";const backHop=1;const fwdHop=1;const cur=currentRelPath();if(!cur)return;const depPickState=(window.__lpDependentPickCacheState=window.__lpDependentPickCacheState||{current:""});if(depPickState.current!==cur){window.__lpDependentPickCache=new Map();depPickState.current=cur;}
const relMode="all";modal.dataset.lpRelMode=relMode;const nb=getNeighbourhood(graph,cur,backHop,fwdHop,relMode);try{if(relMode!=="off"){const coreLocs=(nb&&nb.core)?nb.core:[];const coreSet2=new Set((coreLocs||[]).map(normLoc));const extraLocs=(nb&&nb.nodes)?(nb.nodes||[]).filter((x)=>x&&!coreSet2.has(normLoc(x))):[];const onUpdate=()=>{const modal2=document.getElementById("lp-map-modal");if(!modal2||!modal2.classList.contains("lp-open"))return;const stR=window.__lpMapState;if(!stR)return;if(stR.__lpRelFetchRaf)return;stR.__lpRelFetchRaf=requestAnimationFrame(()=>{stR.__lpRelFetchRaf=0;renderLocalMapModal(graph);});};const limCore=(relMode==="all")?22:14;const limExtra=(relMode==="all")?14:8;__lpPrefetchRelatedForLocs(graph,coreLocs,onUpdate,limCore);__lpPrefetchRelatedForLocs(graph,extraLocs,onUpdate,limExtra);}}catch(_){}
const nodes=nb.nodes||[];const edges=nb.edges||[];const level=nb.level||new Map();const coreSet=new Set((nb&&nb.core)?nb.core:nodes);const degPre=new Map();const degRel=new Map();for(const e of edges){if(!e)continue;const t=e.type||"prereq";if(t==="related"){degRel.set(e.from,(degRel.get(e.from)||0)+1);degRel.set(e.to,(degRel.get(e.to)||0)+1);}else{degPre.set(e.from,(degPre.get(e.from)||0)+1);degPre.set(e.to,(degPre.get(e.to)||0)+1);}}
...
function ensureMapModal(graph){let modal=document.getElementById("lp-map-modal");ensureLocalMapModalUiStyles();if(!modal){modal=document.createElement("div");modal.id="lp-map-modal";modal.setAttribute("aria-hidden","true");modal.style.display="none";try{modal.style.setProperty("z-index","2147483300","important");}catch(_){}
modal.innerHTML=`
...
const curNode=lpStrictNodeFromEvent(modal,e,'#lp-map-modal a.lp-node.is-cur[href]')||(function(){const pt=lpEventPoint(e);return pt?lpNodeFromPoint(modal,pt.x,pt.y,'#lp-map-modal a.lp-node.is-cur[href]'):null;})()||(t&&t.closest?t.closest('#lp-map-modal a.lp-node.is-cur[href]'):null);if(!curNode)return;if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;if(e.cancelable)e.preventDefault();e.stopPropagation();if(typeof e.stopImmediatePropagation==='function')e.stopImmediatePropagation();lpLocalMapShowDirectionMenu(modal,curNode);},true);modal.addEventListener("click",(e)=>{if(e.target===modal)hideLocalMapModal();});window.addEventListener("keydown",(e)=>{if(e.key==="Escape"&&modal.style.display!=="none"){const menu=modal.querySelector('[data-lp-map-dir-menu]');if(menu&&!menu.hidden){lpLocalMapHideDirectionMenu(modal);return;}
hideLocalMapModal();}});}
...
let __lpZoomInputRaf=0;let __lpZoomInputValue=null;const __lpBeginZoomDrag=(e)=>{clearHoverNow();markGestureScale();st.__lpZoomDragging=true;setSliderZoomDragMode(true);try{st.__lpZoomAnchor=getStageCenter();}catch(_){st.__lpZoomAnchor=null;}
...
function endDrag(opts){dragging=false;const pidToRelease=activePid;activePid=null;if(pidToRelease!=null&&vp&&typeof vp.releasePointerCapture==="function"){try{vp.releasePointerCapture(pidToRelease);}catch(_){}}
...
function startDrag(e){if(!vp)return;if((e.pointerType||"mouse")==="mouse"&&typeof e.button==="number"&&e.button!==0)return;const isMobile=isTouchGestureDevice();if(isMobile&&(e.pointerType==="touch"||e.pointerType==="pen")){if(typeof e.pointerId==="number"){activePtrs.set(e.pointerId,{x:e.clientX,y:e.clientY});}
...
if(activePid!=null&&typeof e.pointerId==="number"&&e.pointerId!==activePid)return;const dx=e.clientX-lx;const dy=e.clientY-ly;lx=e.clientX;ly=e.clientY;st.tx+=dx;st.ty+=dy;st.userMoved=true;applyTransformFast({skipClamp:true});if(e.cancelable)e.preventDefault();e.stopPropagation();}
...
function lpBindMapDocumentSurfaceMetricsOnce(){if(window.__lpMapDocSurfaceMetricsBoundV8)return;window.__lpMapDocSurfaceMetricsBoundV8=true;let raf=0;const update=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;lpUpdateMapDocumentSurface(document.getElementById("lp-map-modal"));});};try{window.addEventListener("resize",update,{passive:true});}catch(_){window.addEventListener("resize",update);}
...
const operaPerf=lpIsOperaMapPerfMode();const broadPerf=(()=>{try{const nodeCount=graph&&graph.nodes?Object.keys(graph.nodes).length:0;const edgeCount=Array.isArray(graph&&graph.edges)?graph.edges.length:0;const lowCore=Number(navigator&&navigator.hardwareConcurrency||8)<=4;const lowMemory=Number(navigator&&navigator.deviceMemory||8)<=4;const prefersReduced=!!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches);return!!(operaPerf||prefersReduced||((lowCore||lowMemory)&&(nodeCount>55||edgeCount>95))||(nodeCount>85||edgeCount>150));}catch(_){return!!operaPerf;}})();try{modal.classList.toggle("lp-opera-map-perf",!!broadPerf);if(broadPerf)modal.setAttribute('data-lp-opera-perf',operaPerf?'opera':'auto');else modal.removeAttribute('data-lp-opera-perf');}catch(_){}
const isDesktopPc=!!(window.matchMedia&&window.matchMedia('(min-width: 901px) and (hover: hover) and (pointer: fine)').matches);lpReparentZoomGroup(modal);if(isMobile||isDesktopPc)modal.classList.add("lp-full");else modal.classList.remove("lp-full");modal.classList.add("lp-open");modal.style.display="flex";lpRecordXpActivity("map_open",{source:"show-local-map-modal",eventName:"showLocalMapModal"});try{document.dispatchEvent(new CustomEvent("mk:map-opened",{detail:{source:"show-local-map-modal"}}));}catch(_){}
try{modal.style.setProperty("z-index","2147483300","important");}catch(_){}
...
const btn=modal.querySelector("[data-lp-fullscreen]");if(btn)lpSetFullscreenIcon(btn,modal.classList.contains("lp-full"));const stage=modal.querySelector('.lp-mapstage');const curKey=lpCanonKey(currentRelPath());const stageWNow=stage?Math.max(0,Math.round(stage.clientWidth||stage.getBoundingClientRect().width||0)):0;const stageHNow=stage?Math.max(0,Math.round(stage.clientHeight||stage.getBoundingClientRect().height||0)):0;const masterySigNow=lpCurrentMasterySignature();const canReuse=!!(stage&&curKey&&modal.__lpRenderedGraph===(graph||null)&&modal.__lpRenderedFor===curKey&&modal.__lpRenderedFogEnabled===lpFogEnabled()&&modal.__lpRenderedMasterySignature===masterySigNow&&stage.querySelector('[data-map-viewport] .lp-node[data-lp-loc]')&&Math.abs((Number(modal.__lpRenderedStageW)||0)-stageWNow)<=20&&Math.abs((Number(modal.__lpRenderedStageH)||0)-stageHNow)<=20);if(window.__lpMapState){window.__lpMapState.scale=lpMapDefaultActualScale();window.__lpMapState.__lpForceCenter=true;window.__lpMapState.userMoved=false;}
if(canReuse){if(stage)stage.style.visibility='';requestAnimationFrame(()=>{try{if(window.__lpMapState)lpLocalMapResetView(modal,window.__lpMapState);modal.__lpApplyTransform&&modal.__lpApplyTransform();}catch(_){}});return;}
if(stage)stage.style.visibility='hidden';try{lp3dActivateModal(modal,'local');renderLocalMapModal(graph);const titleEls=Array.from(modal.querySelectorAll('.lp-mapstage .lp-node .lp-node-title'));const pendingEls=titleEls.filter((el)=>lpNodeTitleNeedsTypeset(el));if(pendingEls.length){await typesetMathSafe(pendingEls);pendingEls.forEach((titleEl)=>{try{const host=titleEl&&titleEl.closest?titleEl.closest('.lp-node[data-lp-loc]'):null;if(host)lpCacheRenderedNodeTitle(graph,host);}catch(_){}});renderLocalMapModal(graph);}}finally{if(stage)stage.style.visibility='';try{if(usingDocSurface){lpUpdateMapDocumentSurface(modal);requestAnimationFrame(()=>{try{lpUpdateMapDocumentSurface(modal);if(window.__lpMapState)lpLocalMapResetView(modal,window.__lpMapState);modal.__lpApplyTransform&&modal.__lpApplyTransform();}catch(_){}});}}catch(_){}}}
function hideLocalMapModal(){lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.classList.contains("lp-doc-surface")){try{document.documentElement.classList.remove("lp-modal-open");}catch(_){}
...
const btn=modal.querySelector("[data-lp-fullscreen]");if(btn)lpSetFullscreenIcon(btn,false);modal.style.display="none";modal.setAttribute("aria-hidden","true");}
...
function lpH1StudyStartRerenderPreservingView(modal,graph,targetLoc,mode){try{if(!modal||!graph||!modal.classList.contains('lp-open'))return false;const anim=modal.__lpH1RouteAnim;if(anim&&!anim.cancelled)return false;const view=lpH1StudyStartEnsureView(modal);const saved={scale:Number(view.scale)||lpMapDefaultActualScale(),tx:Number(view.tx)||0,ty:Number(view.ty)||0,userMoved:!!view.userMoved,forceCenter:!!view.forceCenter};modal.__lpH1PreserveViewUntil=Date.now()+900;lpH1StudyStartRenderModal(graph,targetLoc||currentRelPath(),null,mode||modal.dataset.lpMode||LP_ROUTE_MAP_MODE.TO_HERE);const next=lpH1StudyStartEnsureView(modal);next.scale=saved.scale;next.tx=saved.tx;next.ty=saved.ty;next.userMoved=saved.userMoved;next.forceCenter=saved.forceCenter;lpH1StudyStartApplyTransform(modal,{skipClamp:true});return true;}catch(_){return false;}}
function ensureH1StudyStartModal(){let modal=document.getElementById("lp-h1sg-modal");if(!modal){modal=document.createElement("div");modal.id="lp-h1sg-modal";modal.setAttribute("aria-hidden","true");modal.style.display="none";modal.innerHTML=`
        <div class="lp-mbox" role="dialog" aria-modal="true" aria-label="Concept maps">
          ...
          <button class="lp-h1sg-docklaunch" type="button" hidden data-lp-h1sg-dock-launch title="Unlock Guided Routes to start guided study">Start guided study</button>
          <div class="lp-mzoom">
...
const dirBtn=e.target&&e.target.closest?e.target.closest('[data-lp-h1sg-dir]'):null;if(dirBtn){e.preventDefault();e.stopPropagation();return;}});window.addEventListener("keydown",(e)=>{if(e.key==="Escape"&&modal.style.display!=="none")hideH1StudyStartModal();});let dragState=null;const DRAG_THRESHOLD=8;let __lpRouteDragRaf=0;let __lpRouteDragSkipRangeSync=false;let __lpRouteDragSkipClamp=false;function lpRouteSetDragCompositor(on){try{modal&&modal.classList&&modal.classList.toggle("lp-map-dragging",!!on);}catch(_){}
...
function ensureH1StudyStartStyles(){if(document.getElementById("lp-h1sg-style-v8f"))return;const st=document.createElement("style");st.id="lp-h1sg-style-v8f";st.textContent=`
      ...
      /* Final route-map readability pass:
         route-highlight = not dimmed.  It should not mean glow/brightness, and
         in light mode the route node text must remain dark on a white pill. */
      #lp-h1sg-modal .lp-node.lp-route-anim-focus,
      #lp-h1sg-modal.lp-webgl3d .lp-node.lp-route-anim-focus,
...
return total;}
function __lpCourseTokenFromRelPath(path){try{const p=String(path||"").replace(/^\/+/,"");const segs=p.split("/").filter(Boolean);if(segs.length<2)return"";let courseSeg=String(segs[1]||"").replace(/^\d+[a-z]-/i,"");const words=courseSeg.split(/[-_\s]+/).map((x)=>String(x||"").trim()).filter(Boolean);if(!words.length)return"";if(/^math$/i.test(words[0]||"")&&words[1]&&/^[IVXL]+$/i.test(words[1])){const n=__lpRomanToInt(words[1]);const rest=words.slice(2).filter((w)=>!/^(for|and|of|the|to|in|eor)$/i.test(w));const suffix=rest.map((w)=>w.charAt(0).toLowerCase()).join("");return n&&suffix?`m${n}${suffix}`:"";}
...
async function __lpLoadSearchIndexOnce(){if(__lpSearchIndexPromise)return __lpSearchIndexPromise;__lpSearchIndexPromise=(async()=>{const root=getSiteRootUrl();const url=new URL("search/search_index.json",root).toString();const j=await __mkFetchJsonShared(url).catch(()=>null);return(j&&Array.isArray(j.docs))?j.docs:[];})();return __lpSearchIndexPromise;}
...
const docs=await __lpLoadSearchIndexOnce();const map=__lpBuildSearchIndexLectureMap(docs);window.__lpSearchIndexLectureMap=map;return map;}catch(_){return new Map();}}
async function __lpPrimeTitlesFromSearchIndex(graph){try{const docs=await __lpLoadSearchIndexOnce();const map=__lpBuildSearchIndexTitleMap(docs);window.__lpSearchIndexPageTitleMap=map;for(const[loc,title]of map.entries()){if(!loc||!title)continue;__lpSetTitleCache(graph,loc,title,true);__lpForceGraphNodeTitle(graph,loc,title);}
...
try{const g=graphRef||window.__lpLearningPathGraph;const modal=document.getElementById("lp-map-modal");if(modal)modal.__lpRenderedMasterySignature="";if(g&&modal&&modal.classList&&modal.classList.contains("lp-open")){if(!window.__lpMasteryMapRefreshRaf){window.__lpMasteryMapRefreshRaf=requestAnimationFrame(()=>{window.__lpMasteryMapRefreshRaf=0;try{renderLocalMapModal(g);}catch(_){}});}}}catch(_){}
...
requestAnimationFrame(()=>{try{renderLocalMapModal(g);}catch(_){}
...
st.__lpApplyFocus(restoreLoc);}catch(_){}});});};const syncOpenRouteMapForReveal=(key)=>{const modal=document.getElementById('lp-h1sg-modal');if(!modal||!modal.classList.contains('lp-open'))return;const state=modal.__lpH1StudyState;if(!state)return;try{lpH1StudyRouteRevealSetForState(state).add(key);}catch(_){}
...
if(!window.__lpRelatedPanelRefreshInstalled){window.__lpRelatedPanelRefreshInstalled=true;window.addEventListener("lp:related-cache-updated",()=>{});}
mountTopWhyLine();const h1Tools=mountH1Tools(graph);const h1RouteBar=mountH1RouteBar(graph);const panel=mountPanel(graph);await typesetMathSafe([panel,h1Tools]);if(h1RouteBar&&h1RouteBar.__lpTypesetPromise&&typeof h1RouteBar.__lpTypesetPromise.then==="function"){await h1RouteBar.__lpTypesetPromise;}else{await typesetMathSafe([h1RouteBar]);}
...
window.__lpCourseLinePromise=Promise.resolve(courseLine);lpMarkPanelReady(panel,rel);try{const relForPrime=rel;__mkRunIdle(async()=>{try{if(normLoc(currentRelPath())!==normLoc(relForPrime))return;await __lpPrimeSidebarRelated(graph,relForPrime,{deep:false});}catch(_){}},420);}catch(_){}
...
try{window.addEventListener("pageshow",boot,{passive:true});}catch(_){}})();(function(){const STYLE_ID='lp-map-v3-mobile-zoom-and-layer-fix';function ensureStyles(){if(document.getElementById(STYLE_ID))return;const st=document.createElement('style');st.id=STYLE_ID;st.textContent=`
#lp-map-modal,
#lp-h1sg-modal{
...
`;(document.head||document.documentElement).appendChild(st);}
function localMapOpen(){try{const modal=document.getElementById('lp-map-modal');return!!(modal&&modal.classList&&modal.classList.contains('lp-open')&&modal.style.display!=='none');}catch(_){return false;}}
function clickThroughGuard(e){try{if(!localMapOpen())return;const modal=document.getElementById('lp-map-modal');if(!modal)return;const target=e&&e.target;if(target&&modal.contains(target))return;if(e&&e.cancelable)e.preventDefault();if(e){e.stopPropagation();if(typeof e.stopImmediatePropagation==='function')e.stopImmediatePropagation();}
...
try{window.addEventListener("pageshow",ensureStyle,{passive:true});}catch(_){}})();(function(){"use strict";const BUILD="lp-desktop-panel-right-shift-v4-remount-stable";const STYLE_ID="lp-desktop-panel-right-shift-v4";const ROOT_SHIFT_VAR="--lp-desktop-panel-right-shift-current";const PANEL_SHIFT_VAR="--lp-panel-right-shift";const OLD_STYLE_IDS=["lp-desktop-panel-right-shift-v1","lp-desktop-panel-right-shift-v2","lp-desktop-panel-right-shift-v3"];const TARGET_SCROLLBAR_GAP_PX=30;const TARGET_VIEWPORT_GAP_PX=52;const MAX_SHIFT_PX=96;const MIN_PANEL_WIDTH_PX=180;const SHIFT_EPSILON_PX=0.5;if(window.__lpDesktopPanelRightShiftPatchBuild===BUILD){try{if(typeof window.__lpDesktopPanelRightShiftApplyNow==="function"){window.__lpDesktopPanelRightShiftApplyNow();}else if(typeof window.__lpDesktopPanelRightShiftSchedule==="function"){window.__lpDesktopPanelRightShiftSchedule();}}catch(_){}
return;}
...
return;}
try{const rect=panel.getBoundingClientRect();if(!rect||rect.width<MIN_PANEL_WIDTH_PX)return;const currentShift=readCurrentShiftPx(panel);const naturalRight=Number(rect.right)-currentShift;const limit=panelRightLimit(panel);const targetGap=limit&&limit.source==="viewport"?TARGET_VIEWPORT_GAP_PX:TARGET_SCROLLBAR_GAP_PX;const gap=(limit&&Number(limit.right))-naturalRight;const shift=clamp(Math.round(gap-targetGap),0,MAX_SHIFT_PX);const rootShift=rootShiftPx();if(Math.abs(shift-rootShift)>=SHIFT_EPSILON_PX){writeShiftPx(panel,shift);}else{try{if(panel.style&&panel.style.getPropertyValue(PANEL_SHIFT_VAR))panel.style.removeProperty(PANEL_SHIFT_VAR);}catch(_){}}}catch(_){}}
...
/* Local map (Prerequisites / Nearby / Dependents) pan parity with the route map.
   Mastered/know pills run an infinite box-shadow keyframe with up to 150px blur
   radii; box-shadow is not compositable, so every one of them repainted on every
   frame *inside* the promoted pan layer and forced tile re-rasters for the whole
   drag. Dense maps have dozens of them, which is exactly why pointer input felt
   laggy there and not on small maps. Freeze them (and the dashed related-edge
   motion) for the duration of the gesture only; lp-slider-zooming already does
   the same during slider zoom. */
#lp-map-modal.lp-map-dragging .lp-mapviewport a.lp-node,
...
#lp-map-modal.lp-map-dragging .lp-mapviewport svg .lp-flow-arrow{
  display:none !important;
}
...
#lp-h1sg-modal.lp-map-dragging .lp-h1sg-overlay{
  visibility:hidden !important;
}
...
#lp-h1sg-modal.lp-map-dragging .lp-node::before,
#lp-h1sg-modal.lp-map-dragging .lp-node::after{
  animation-play-state:paused !important;
  box-shadow:none !important;
  filter:none !important;
}
...
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",install,{once:true});else install();})();(function(){try{if(document.getElementById('lp-h1-route-currentcolor-v28'))return;const st=document.createElement('style');st.id='lp-h1-route-currentcolor-v28';st.textContent=`
    ...
    `;(document.head||document.documentElement).appendChild(st);}catch(_){}})();(function(){"use strict";const STYLE_ID="lp-local-map-default-blur-v72";const FOG_KEY="lp_map_fog_enabled_v1";function ensureStyle(){try{if(document.getElementById(STYLE_ID))return;const st=document.createElement("style");st.id=STYLE_ID;st.textContent=`
#lp-map-modal,
#lp-map-modal.lp-open,
#lp-map-modal.lp-full,
...
function syncOpenMap(){try{ensureStyle();const modal=document.getElementById("lp-map-modal");if(!modal||!modal.classList||!modal.classList.contains("lp-open"))return;modal.style.setProperty("background","rgba(0,0,0,.46)","important");modal.style.setProperty("-webkit-backdrop-filter","blur(6px) saturate(1.04)","important");modal.style.setProperty("backdrop-filter","blur(6px) saturate(1.04)","important");}catch(_){}}
