...
if(!svg.querySelector('.lp-map-route-paper'))lpMapPaintRouteSurface(svg,points.values());lpRouteContextPaint(svg,model,state,points,`lp-route-context-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);}
...
function lpMapPaintTerritories(svg,areas){const ns='http://www.w3.org/2000/svg';const layer=document.createElementNS(ns,'g');layer.classList.add('lp-course-territories');layer.setAttribute('aria-hidden','true');const courses=Array.from(new Set((areas||[]).map(area=>area.course))).sort();const hues=[192,35,282,142,345,75];const courseHues=lpMapPaintTerritories.courseHues||(lpMapPaintTerritories.courseHues=new Map());for(const course of courses){if(!courseHues.has(course)){const index=courseHues.size;courseHues.set(course,index<hues.length?hues[index]:(220+(index-hues.length)*137.508)%360);}}
...
const borderPaint=[];const labelPaint=[];if(worldBox)lpMapPaintWorldSurfaces(layer,defs,uid,worldBox,landClip,frontierMask);for(const area of areas||[]){const hue=courseHues.get(area.course);const path=document.createElementNS(ns,'path');path.classList.add('lp-course-territory');path.setAttribute('d',area.d);path.setAttribute('fill-rule','evenodd');path.style.setProperty('--lp-territory-hue',String(hue));path.dataset.lpCourse=area.course;if(frontierMask)path.setAttribute('mask',frontierMask);layer.appendChild(path);for(const run of area.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-course-border');border.setAttribute('d',run.d||run);border.dataset.lpCourse=area.course;border.style.setProperty('--lp-territory-hue',String(hue));if(frontierMask)border.setAttribute('mask',frontierMask);borderPaint.push(border);}
...
if(parchmentRim)layer.appendChild(parchmentRim);borderPaint.forEach(border=>layer.appendChild(border));labelPaint.forEach(label=>layer.appendChild(label));svg.insertBefore(layer,svg.firstChild);return layer;}
...
const reusePlacedMap=!!(window.MkMapGeometry&&exploration.positions&&exploration.positions.size);(function(){if(reusePlacedMap)return;const adj=new Map();for(const n of nodes)adj.set(n,new Set());for(const e of edges){if(!e||!e.from||!e.to)continue;if(!adj.has(e.from))adj.set(e.from,new Set());if(!adj.has(e.to))adj.set(e.to,new Set());adj.get(e.from).add(e.to);adj.get(e.to).add(e.from);}
...
if(!changed)return false;const paths=edgeEls.concat(Array.from(hiG.querySelectorAll('path.lp-edge[data-from][data-to]')));const edgeData=paths.map(el=>({from:el.dataset.from,to:el.dataset.to,type:el.dataset.type}));const result=window.MkMapGeometry.route(geometryNodes(),edgeData,geometryOptions);result.routes.forEach((route,index)=>{const el=paths[index];if(!el)return;const casing=el.__lpStaticCasing;if(route.status!=='ok'){el.style.display='none';if(casing)casing.style.display='none';return;}
...
function __lpReduceMotion(){return lpMotionReduced()||!lpLocalMapAnimationsEnabled(modal);}
...
const nodeAction=document.createElement("div");nodeAction.className="lp-node-action";nodeAction.hidden=true;const openConcept=document.createElement("a");openConcept.className="lp-node-open";openConcept.textContent="Open concept ↗";openConcept.setAttribute("data-mk-no-hover-preview","1");nodeAction.appendChild(openConcept);stage.querySelectorAll('.lp-node-action').forEach(item=>item.remove());stage.appendChild(nodeAction);let actionOwner="";let leaveTimer=0;function cancelPreviewLeave(){if(leaveTimer)window.clearTimeout(leaveTimer);leaveTimer=0;}
...
actionOwner=loc;nodeAction.dataset.lpOwner=lpCanonKey(loc);openConcept.href=toAbsoluteUrl(loc);nodeAction.hidden=false;positionNodeAction();}
...
st2.__lpShowNodeAction=showNodeAction;function applyBaseline(){if(st2.__lpRenderId!==renderId)return;syncMeasuredEdgeGeometry();try{const revealLoc=st2&&st2.__lpRevealFocusLoc;if(revealLoc&&!st2.__lpApplyingRevealFocus&&lpRevealNavIsActive(revealLoc)){lpScheduleRevealEdgeFocusRepaint(revealLoc,lpRevealCurrentElement(revealLoc,null));return;}}catch(_){}
...
if(st2.__lpRenderId!==renderId)return;__lpStopAllFlows();removeExtraFocusNodes(loc);st2.__lpHoverLoc=loc;const locVisual=nodeVisual.get(loc)||lpMapVisualForLoc(loc);const locRec=getMastery(loc);const locMastery=(locRec&&typeof locRec.m==="number")?locRec.m:null;const isExpandable=true;const ctx=isExpandable?classifyDirectSets(loc):{pres:new Set(),deps:new Set(),rels:new Set()};const directCurEdges=lpDirectEdgesToCurrent(loc);const focusSet=new Set([loc,cur,...ctx.pres,...ctx.deps,...ctx.rels]);for(const e0 of directCurEdges){if(e0&&e0.from)focusSet.add(e0.from);if(e0&&e0.to)focusSet.add(e0.to);}
        ...
        <div class="lp-map-topbar">
          ...
          <div class="lp-map-context">
            ...
            <div class="lp-map-context-copy" role="status" aria-live="polite" aria-atomic="true">
              ...
              ${window.__mkExamMode ? '' : '<span class="lp-map-context-note">Nearby treasure: expand nodes with a connected account. See Map tips.</span>'}
...
const mapBtn=panel.querySelector('[data-lp-open-map]');if(mapBtn){mapBtn.__lpOpen=()=>showLocalMapModal(graph,mapBtn);lpBindTap(mapBtn,mapBtn.__lpOpen);}
