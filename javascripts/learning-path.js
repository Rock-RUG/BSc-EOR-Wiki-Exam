...
const exploration=mapState.__lpExploration;const nb=window.MkMapExploration.union(exploration,getNeighbourhood(graph,cur,backHop,fwdHop,relMode));try{if(relMode!=="off"){const coreLocs=(nb&&nb.core)?nb.core:[];const coreSet2=new Set((coreLocs||[]).map(normLoc));const extraLocs=(nb&&nb.nodes)?(nb.nodes||[]).filter((x)=>x&&!coreSet2.has(normLoc(x))):[];const limCore=(relMode==="all")?22:14;const limExtra=(relMode==="all")?14:8;try{if(modal.__lpRelatedPrefetchTimer)window.clearTimeout(modal.__lpRelatedPrefetchTimer);}catch(_){}
...
function hideLocalMapModal(){lpResetRevealSession();lpLocalMapHideDirectionMenu(document.getElementById("lp-map-modal"));window.__lpDependentPickCache=new Map();if(window.__lpDependentPickCacheState)window.__lpDependentPickCacheState.current="";const modal=document.getElementById("lp-map-modal");if(!modal)return;if(modal.__lpViewportResizeCleanup)modal.__lpViewportResizeCleanup();try{if(modal.__lpStopZoomGesture)modal.__lpStopZoomGesture();}catch(_){}
