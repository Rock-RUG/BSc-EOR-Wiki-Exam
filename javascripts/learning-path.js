...
const torn=lpMapParchmentFrontierPaths(frontiers);if(torn.length){layer.classList.add('lp-parchment-ready');const cut=document.createElementNS(ns,'g');cut.classList.add('lp-parchment-cut');cut.setAttribute('display','none');const coastPath=torn.map(run=>run.edge).join('');const tear=document.createElementNS(ns,'path');tear.setAttribute('d',torn.map(run=>run.cut).join(''));tear.setAttribute('fill-rule','nonzero');tear.style.setProperty('fill','black','important');tear.style.setProperty('stroke','none','important');cut.appendChild(tear);mask.appendChild(cut);const silhouette=document.createElementNS(ns,'clipPath');silhouette.id=uid+'-land';silhouette.setAttribute('clipPathUnits','userSpaceOnUse');for(const area of areas||[]){const land=document.createElementNS(ns,'path');land.setAttribute('d',area.d);land.setAttribute('clip-rule','evenodd');silhouette.appendChild(land);}
defs.appendChild(silhouette);try{const root=lpMapPaintTerritories.paperRoot||getSiteRootUrl();lpMapPaintTerritories.paperRoot=root;const paperPattern=document.createElementNS(ns,'pattern');paperPattern.id=uid+'-parchment-paper';paperPattern.setAttribute('patternUnits','userSpaceOnUse');paperPattern.setAttribute('x','0');paperPattern.setAttribute('y','0');paperPattern.setAttribute('width','1024');paperPattern.setAttribute('height','1024');const backing=document.createElementNS(ns,'rect');backing.setAttribute('width','1024');backing.setAttribute('height','1024');backing.style.setProperty('fill','#dfbd7d','important');paperPattern.appendChild(backing);const photo=document.createElementNS(ns,'image');photo.setAttribute('href',root+'assets/map-styles/paper-parchment.jpg');photo.setAttribute('width','1024');photo.setAttribute('height','1024');photo.setAttribute('preserveAspectRatio','none');paperPattern.appendChild(photo);defs.appendChild(paperPattern);parchmentPaper=document.createElementNS(ns,'rect');parchmentPaper.classList.add('lp-parchment-paper');parchmentPaper.setAttribute('display','none');for(const[key,value]of Object.entries(box))parchmentPaper.setAttribute(key,value);parchmentPaper.style.setProperty('fill',`url(#${paperPattern.id})`,'important');parchmentPaper.setAttribute('clip-path',`url(#${silhouette.id})`);parchmentPaper.setAttribute('mask',`url(#${mask.id})`);}catch(_){}
parchmentRim=document.createElementNS(ns,'g');parchmentRim.classList.add('lp-parchment-rim');parchmentRim.setAttribute('display','none');parchmentRim.setAttribute('clip-path',`url(#${silhouette.id})`);parchmentRim.setAttribute('mask',`url(#${mask.id})`);for(const[name,width,opacity]of[['age',40,.009],['age',36,.011],['age',32,.013],['age',28,.015],['age',24,.017],['age',20,.019],['age',16,.021],['age',12,.023],['age',8,.025],['age',4,.027],['depth',1.1,.18],['fiber',.6,.13]]){const rim=document.createElementNS(ns,'path');rim.classList.add('lp-parchment-'+name);rim.setAttribute('d',coastPath);rim.setAttribute('fill','none');rim.setAttribute('stroke-width',String(width));rim.setAttribute('stroke-linejoin','round');rim.style.setProperty('opacity',String(opacity));parchmentRim.appendChild(rim);}}
defs.appendChild(mask);frontierMask=`url(#${mask.id})`;}
const borderPaint=[];const labelPaint=[];if(parchmentPaper)layer.appendChild(parchmentPaper);for(const area of areas||[]){const hue=courseHues.get(area.course);const path=document.createElementNS(ns,'path');path.classList.add('lp-course-territory');path.setAttribute('d',area.d);path.setAttribute('fill-rule','evenodd');path.style.setProperty('--lp-territory-hue',String(hue));path.dataset.lpCourse=area.course;if(frontierMask)path.setAttribute('mask',frontierMask);layer.appendChild(path);for(const run of area.borders||[]){const border=document.createElementNS(ns,'path');border.classList.add('lp-course-border');border.setAttribute('d',run.d||run);border.dataset.lpCourse=area.course;border.style.setProperty('--lp-territory-hue',String(hue));if(frontierMask)border.setAttribute('mask',frontierMask);borderPaint.push(border);}
...
function lpMountMapStyleShortcut(modal){if(!modal||window.__mkExamMode||modal.querySelector('[data-lp-map-style]'))return;const row=modal.querySelector('.lp-map-toprow, .lp-h1sg-tophead');if(!row)return;modal.__lpCloseForStyleStore=()=>{if(modal.id==='lp-h1sg-modal')hideH1StudyStartModal();else hideLocalMapModal();};const button=document.createElement('button');button.type='button';button.className='lp-map-style-shortcut';button.setAttribute('data-lp-map-style','');button.setAttribute('aria-label','Map styles');button.setAttribute('aria-haspopup','dialog');button.setAttribute('aria-expanded','false');button.title='Map styles';button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="m8 3-6 4 3 5 3-2v11h8V10l3 2 3-5-6-4c0 4-8 4-8 0Z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/></svg><span>Map styles</span>';const filter=row.querySelector('.lp-h1sg-filter');row.insertBefore(button,filter||null);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const root=document.documentElement;if(window.__mkExamMode||root.getAttribute('data-mk-startup-account')==='off'||root.classList.contains('mk-startup-account-off'))return;lpMapTipsCollapse(modal);const options={anchor:button,categories:['Map effects'],slots:['map_style'],title:'Map styles',includeDefaultMapStyle:true,focusOnOpen:true};button.focus({preventScroll:true});if(window.MkLocalActivity&&typeof window.MkLocalActivity.openQuickShop==='function')window.MkLocalActivity.openQuickShop(button,options);else window.dispatchEvent(new CustomEvent('mk-open-quick-shop',{detail:options}));});}
