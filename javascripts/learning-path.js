...
function __lpSetTitleCache(graph,key,title,force,batch){try{const k=normLoc(key);const incoming=cleanTitle(title||"");if(__lpShouldBlockTitleMutation(graph,k,incoming,batch&&batch.nodes))return;const t=__lpRepairTitleMathFromLoc(k,incoming);if(!k||!t)return;const c=__lpEnsureTitleCache(graph);if(!c)return;const aliases=batch&&batch.cache;const prevBest=cleanTitle(__lpGetTitleCache(graph,k,aliases)||"");const nextScore=__lpScoreTitleCandidate(k,t);const prevScore=__lpScoreTitleCandidate(k,prevBest);if(!force&&prevBest&&nextScore<prevScore)return;if(prevBest&&lpHasMathMarkup(prevBest)&&!lpHasMathMarkup(t))return;if(c instanceof Map){c.set(k,t);for(const kk of aliases?__lpIndexTitleAlias(aliases,k):c.keys()){if(lpCanonKey(kk)===lpCanonKey(k))c.set(kk,t);}}else{c[k]=t;for(const kk of aliases?__lpIndexTitleAlias(aliases,k):Object.keys(c)){if(lpCanonKey(kk)===lpCanonKey(k))c[kk]=t;}}}catch(_){}}
...
try{document.body&&document.body.classList.remove("lp-modal-open");}catch(_){}}else{setLpModalScrollLock(false);}
lpExitMapDocumentSurface(modal);lpInstallClickShield(560);modal.classList.remove("lp-open");modal.classList.remove("lp-full");modal.style.display="none";modal.setAttribute("aria-hidden","true");modal.__lpH1PendingToken='';}
