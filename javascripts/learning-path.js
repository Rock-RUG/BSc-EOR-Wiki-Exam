...
lpBindMobileTapAction(btnToggle,"lpMsToggleTapBound",()=>{const open=root.classList.contains("lp-expanded");lpSetMobileSheetExpanded(!open);},{movePx:12,tapMs:760});lpBindMobileTapAction(btnOpenMap,"lpMsOpenMapTapBound",()=>{showLocalMapModal(graph,btnOpenMap);},{movePx:12,tapMs:760});fogSwitch&&fogSwitch.addEventListener("change",()=>{const enabled=!!fogSwitch.checked;lpSetFogEnabled(enabled);const modalNow=document.getElementById("lp-map-modal");if(modalNow&&modalNow.classList.contains("lp-open")){requestAnimationFrame(()=>renderLocalMapModal(graph));}});}
