...
function lpNearbyWorld(world,nodes,previous){if(!world||!nodes.every(id=>world.has(id)))return null;const courses=new Set(nodes.map(id=>world.courseOf(id)));const areas=world.areas.filter(area=>courses.has(area.course));if(previous&&previous.source===world&&previous.areas.length===areas.length&&areas.every((area,i)=>area===previous.areas[i]))return previous;return{...world,source:world,areas};}
...
if(parchmentRim)(land||layer).appendChild(parchmentRim);borderPaint.forEach(border=>(land||layer).appendChild(border));labelPaint.forEach(label=>(paintOptions.labels||layer).appendChild(label));svg.insertBefore(layer,svg.firstChild);return layer;}
...
function lpWorldExploreStatus(modal,footprint){const treasure=modal.querySelector('[data-lp-map-treasure-note]');if(treasure){const note=lpExplorationMode()?'Treasure: open and rate a concept, and the map reveals its surroundings. See Map tips.':'Treasure: turn on Knowledge masking to uncover the map by learning. See Map tips.';if(treasure.textContent!==note)treasure.textContent=note;}
...
const live=vp.__lpWorldReadCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||committed.gesture||Math.abs(lpWorldWindowFor(live).s/committed.s-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);};vp.__lpWorldWindowTimer=setTimeout(settle,160);}
...
for(const e of edges)lpDrawBaseEdge(e);if(!worldMode)lpMapPaintTerritories(svg,geometry.regions,{groupMask:true,coastStep:24,simple:true});vp.insertBefore(svg,vp.firstChild);if(worldMode){lpMountWorldTerrain(modal,vp,world,W,H);}
...
function lpRevealLearnedNeighbourhoods(session,base){if(!lpExplorationMode()||!lpConceptExploreState(cur).explored)return[];const key=lpCanonKey(cur);if(session.localReveal===key)return[];const direct=new Set(lpRootNeighbourhood(cur).nodes.map(lpCanonKey));const added=(base.nodes||[]).filter(loc=>lpCanonKey(loc)!==key&&direct.has(lpCanonKey(loc)));if(!added.length)return[];session.localReveal=key;return[{nodeId:normLoc(cur),addedNodeIds:added.map(normLoc)}];}
...
try{const chests=lpAttachMapChests(modal);chests?.updateLayout();if(chests&&freshReveals.length&&modal.classList.contains('lp-open')){for(const reveal of freshReveals)if(reveal.addedNodeIds.length)chests.expanded(reveal);}}catch(_){}}
...
if(activePtrs.size===2){pendingNodeDrag=null;dragging=false;activePid=null;const ids=Array.from(activePtrs.keys());const p1=activePtrs.get(ids[0]);const p2=activePtrs.get(ids[1]);if(p1&&p2){clearHoverNow();markGestureScale();try{modal&&modal.toggleAttribute&&modal.toggleAttribute("data-lp-dragging",true);}catch(_){}
...
return"full";}
function lpMapTabHelperText(kind,isPhone){const phone=typeof isPhone==="boolean"?isPhone:!!__lpIsPhoneTouch();if(String(kind||"")==="local"){const controls=phone?"Tap a concept to open its page.":"Hover to preview connections. Click a concept to open its page.";return controls+" Drag freely to explore."+(phone?" Before shows prerequisites; After shows dependents.":"")+(window.__mkExamMode?"":" Nearby shows only the concepts around the page you opened it from. Visit another concept to see its neighbourhood. Knowledge masking hides unfamiliar titles without adding more nodes. Your learning record is kept. Treasure needs Knowledge masking, a connected account, and an opened and rated current concept. Each concept gives one chance per UTC day. Crown/gold/silver/bronze chances: 1/1,000, 1/100, 1/20, 1/5; daily limits: 1, 1, 2, 5, including unopened chests. Open before midnight UTC for XP, EORbits and possible shop items; owned items become EORbits. Full rules: Site guide → Map treasures.");}
return phone?"Before shows prerequisites; After shows dependents. Search using the field above, or tap a node to find a learning path. Tap Start guided study to begin along the path.":"Click a node to find a learning path. Click Start guided study to begin along the path.";}
