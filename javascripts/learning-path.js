...
function lpLocalMapInteractionIsBusy(modal){try{const root=modal||document.getElementById("lp-map-modal");if(!root||!root.classList||!root.classList.contains("lp-open"))return false;if(lpLocalMapPointerHeld())return true;if(lpLocalMapInteractionNow()<(Number(window.__lpLocalMapInteractionBusyUntil)||0))return true;if(root.hasAttribute("data-lp-dragging")||root.hasAttribute("data-lp-slider-zooming")||root.hasAttribute("data-lp-gesturing")||root.classList.contains("lp-webgl3d-dragging")||root.classList.contains("lp-route-animating"))return true;const state=window.__lpMapState;return!!(state&&state.__lpZoomDragging);}catch(_){return false;}}
...
function lpWorldFogDraw(canvas,win){const state=canvas&&canvas.__lpFogState;if(!state||!win)return;const f=LP_WORLD_FOG_SCALE,base=state.base;const cw=Math.max(1,Math.round(win.w*f)),ch=Math.max(1,Math.round(win.h*f));if(canvas.width!==cw)canvas.width=cw;if(canvas.height!==ch)canvas.height=ch;lpWorldWindowPlace(canvas,win);const ctx=canvas.getContext('2d');if(!ctx)return;ctx.setTransform(1,0,0,1,0,0);ctx.globalCompositeOperation='source-over';ctx.clearRect(0,0,cw,ch);ctx.fillStyle=state.colour;ctx.fillRect(0,0,cw,ch);const ox=Math.round(win.x*f),oy=Math.round(win.y*f);const sx=Math.max(0,ox),sy=Math.max(0,oy);const ex=Math.min(base.width,ox+cw),ey=Math.min(base.height,oy+ch);if(ex>sx&&ey>sy){ctx.clearRect(sx-ox,sy-oy,ex-sx,ey-sy);ctx.drawImage(base.layer,sx,sy,ex-sx,ey-sy,sx-ox,sy-oy,ex-sx,ey-sy);}
...
function lpWorldWindowFollow(vp,readCamera){if(!vp||!vp.isConnected||typeof readCamera!=='function')return;const camera=readCamera();if(!camera)return;const win=vp.__lpWorldWindow,now=lpWorldWindowFor(camera);const ratio=win?now.s/win.s:0,slack=win?Math.min(win.w,win.h)*.02:0;const resized=!!win&&(Math.abs(now.vw*now.s-win.vw*win.s)>1||Math.abs(now.vh*now.s-win.vh*win.s)>1);const inside=!!win&&now.vx>=win.x+slack&&now.vy>=win.y+slack&&now.vx+now.vw<=win.x+win.w-slack&&now.vy+now.vh<=win.y+win.h-slack;if(resized||!inside||ratio>LP_WORLD_WINDOW_RESCALE||ratio<1/LP_WORLD_WINDOW_RESCALE){const next=lpWorldWindowCommit(vp,camera);if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,next,true);}
...
const live=readCamera(),committed=vp.__lpWorldWindow;if(!live)return;const settled=!committed||Math.abs(lpWorldWindowFor(live).s/committed.s-1)>=.001?lpWorldWindowCommit(vp,live):committed;if(vp.__lpWorldWindowScope!=='route'&&!vp.__lpLocalWindow)lpWorldWindowCull(vp,settled);};vp.__lpWorldWindowTimer=setTimeout(settle,160);}
...
function lpAnimateLocalExpansion(modal,vp,st,snapshot,positions,nodes,current,anchor){const target=positions.get(anchor);if(!target)return;const endX=st.__lpStageW/2-target.x*st.scale,endY=st.__lpStageH/2-target.y*st.scale;if(!snapshot||!lpLocalExpansionMotionAllowed(modal)){st.tx=endX;st.ty=endY;modal.__lpApplyTransform?.({skipClamp:true});return;}
const oldCurrent=snapshot.positions.get(current),newCurrent=positions.get(current);const dx=oldCurrent&&newCurrent?newCurrent.x-oldCurrent.x:0;const dy=oldCurrent&&newCurrent?newCurrent.y-oldCurrent.y:0;const animations=[],startX=st.tx,startY=st.ty;let raf=0,start=null,done=false;const animate=(el,frames,opts)=>animations.push(el.animate(frames,{duration:560,easing:'cubic-bezier(.2,.7,.25,1)',...opts}));const layers=lpWorldWindowLayers(vp);for(const old of snapshot.layers){old.style.translate=`${dx}px ${dy}px`;vp.appendChild(old);animate(old,[{opacity:1},{opacity:0}],{fill:'forwards'});}
...
if(!lpLocalExpansionMotionAllowed(modal)){st.tx=endX;st.ty=endY;finish();return;}
if(start===null)start=time;const t=Math.min(1,(time-start)/600),eased=1-Math.pow(1-t,3);st.tx=startX+(endX-startX)*eased;st.ty=startY+(endY-startY)*eased;modal.__lpApplyExpansionFrame?.();if(t<1)raf=requestAnimationFrame(step);else finish();};raf=requestAnimationFrame(step);}
...
modal.__lpApplyTransform=applyTransform;modal.__lpApplyExpansionFrame=()=>{vp.style.transform=lp3dViewportTransform(modal,'local',st.tx,st.ty,st.scale,false);};function getStageCenter(){if(typeof st.__lpStageW==="number"&&typeof st.__lpStageH==="number"){return{x:st.__lpStageW/2,y:st.__lpStageH/2};}
