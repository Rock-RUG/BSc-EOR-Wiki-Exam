...
function lpMapExpandGate(loc){if(!lpExplorationMode())return"";if(lpCanonKey(loc)===lpCanonKey(currentRelPath()))return"";const state=lpConceptExploreState(loc);if(state.explored)return"";return state.visited?"rate":"visit";}
const LP_MAP_KEEP_KEY="lp_map_keep_expansions_v1";const LP_MAP_KEPT_KEY="lp_map_kept_expansions_v1";function lpMapKeepExpansions(){try{return localStorage.getItem(LP_MAP_KEEP_KEY)==="1";}catch(_){return false;}}
function lpReadKeptExpansionsAll(){try{const data=JSON.parse(localStorage.getItem(LP_MAP_KEPT_KEY)||"null");return data&&data.v===1&&data.owners&&typeof data.owners==="object"?data:{v:1,owners:{}};}catch(_){return{v:1,owners:{}};}}
function lpReadKeptExpansions(owner){const entry=lpReadKeptExpansionsAll().owners[lpCanonKey(owner)];return entry&&Array.isArray(entry.roots)?entry.roots.filter(root=>typeof root==="string"&&root):[];}
function lpWriteKeptExpansions(owner,roots){try{const data=lpReadKeptExpansionsAll();const key=lpCanonKey(owner);if(!key)return;if(roots&&roots.length)data.owners[key]={at:Date.now(),roots:Array.from(new Set(roots)).slice(-80)};else delete data.owners[key];const owners=Object.entries(data.owners).sort((a,b)=>(Number(b[1]&&b[1].at)||0)-(Number(a[1]&&a[1].at)||0));data.owners=Object.fromEntries(owners.slice(0,60));localStorage.setItem(LP_MAP_KEPT_KEY,JSON.stringify(data));}catch(_){}}
function lpSetMapKeepExpansions(on){try{localStorage.setItem(LP_MAP_KEEP_KEY,on?"1":"0");if(!on)localStorage.removeItem(LP_MAP_KEPT_KEY);}catch(_){}
if(on){try{const st=window.__lpMapState,session=st&&st.__lpExploration;if(session&&session.owner)lpWriteKeptExpansions(session.owner,window.MkMapExploration.roots(session));}catch(_){}}
try{document.querySelectorAll("[data-lp-map-keep]").forEach(el=>{el.checked=!!on;});}catch(_){}}
...
const LP_WORLD_FOG_SCALE=1/8;function lpWorldFogPaint(modal,vp,world,footprint,shownLocs,cur){let canvas=vp.querySelector(':scope > canvas.lp-world-fog');if(!footprint){if(canvas)canvas.remove();return null;}
...
const f=LP_WORLD_FOG_SCALE;const width=Math.max(1,Math.ceil(world.width*f)),height=Math.max(1,Math.ceil(world.height*f));if(canvas.width!==width)canvas.width=width;if(canvas.height!==height)canvas.height=height;canvas.style.width=`${world.width}px`;canvas.style.height=`${world.height}px`;const ctx=canvas.getContext('2d');if(!ctx)return canvas;const css=getComputedStyle(modal);const rgb=lpCssColorRgb(css.getPropertyValue('--atlas-backing'))||lpCssColorRgb(css.getPropertyValue('--wiki-map-canvas'))||[12,24,36];const colour=String(css.getPropertyValue('--lp-world-fog')||'').trim()||`rgba(${rgb.join(',')},.8)`;const paintHole=(target,loc,strength,rx,ry)=>{const p=world.point(loc);if(!p)return;const r=rx*f;target.save();target.translate(p.x*f,p.y*f);target.scale(1,ry/rx);const g=target.createRadialGradient(0,0,0,0,0,r);g.addColorStop(0,`rgba(0,0,0,${strength})`);g.addColorStop(.5,`rgba(0,0,0,${strength})`);g.addColorStop(1,'rgba(0,0,0,0)');target.fillStyle=g;target.beginPath();target.arc(0,0,r,0,Math.PI*2);target.fill();target.restore();};let base=canvas.__lpFogBase;if(!base||base.footprint!==footprint||base.colour!==colour||base.width!==width||base.height!==height){const layer=document.createElement('canvas');layer.width=width;layer.height=height;const bctx=layer.getContext('2d');if(!bctx)return canvas;bctx.fillStyle=colour;bctx.fillRect(0,0,width,height);bctx.globalCompositeOperation='destination-out';for(const{loc,state}of footprint.cells)paintHole(bctx,loc,state.claimed?1:state.explored?.9:.65,320,170);paintHole(bctx,cur,1,380,200);base=canvas.__lpFogBase={footprint,colour,width,height,layer};}
ctx.globalCompositeOperation='copy';ctx.drawImage(base.layer,0,0);ctx.globalCompositeOperation='destination-out';for(const loc of shownLocs||[])paintHole(ctx,loc,.5,230,120);ctx.globalCompositeOperation='source-over';return canvas;}
...
function lpWorldExploreStatus(modal,footprint){const treasure=modal.querySelector('[data-lp-map-treasure-note]');if(treasure){const note=lpExplorationMode()?'Treasure: grow the map from concepts you have opened and rated. See Map tips.':'Treasure: turn on Knowledge masking and grow the map from concepts you have learned. See Map tips.';if(treasure.textContent!==note)treasure.textContent=note;}
...
vp.insertBefore(terrain.svg,vp.firstChild);vp.insertBefore(terrain.labels,terrain.svg.nextSibling);return terrain.svg;}
...
function lpBindMapOptions(modal){const box=modal&&modal.querySelector('[data-lp-map-options]');if(!box)return;const fog=box.querySelector('[data-lp-fog-switch]'),keep=box.querySelector('[data-lp-map-keep]');if(fog)fog.checked=lpFogEnabled();if(keep)keep.checked=lpMapKeepExpansions();if(box.dataset.lpBound==='1')return;box.dataset.lpBound='1';if(fog)fog.addEventListener('change',()=>lpSetFogEnabled(!!fog.checked));if(keep)keep.addEventListener('change',()=>lpSetMapKeepExpansions(!!keep.checked));box.addEventListener('pointerdown',event=>event.stopPropagation());}
...
function lpRestoreKeptExpansions(session){const known=lpConceptWorld();if(!known||!lpMapKeepExpansions())return;for(const loc of lpReadKeptExpansions(cur)){if(!known.has(loc)||lpMapExpandGate(loc))continue;try{window.MkMapExploration.expand(session,lpCanonKey(loc),lpRootNeighbourhood(loc));}catch(_){}}}
...
const nodeAction=document.createElement("div");nodeAction.className="lp-node-action";nodeAction.hidden=true;const openConcept=document.createElement("a");openConcept.className="lp-node-open";openConcept.textContent="Open concept ↗";openConcept.setAttribute("data-mk-no-hover-preview","1");nodeAction.appendChild(openConcept);const gateNote=document.createElement("span");gateNote.className="lp-node-gate";gateNote.hidden=true;nodeAction.appendChild(gateNote);stage.querySelectorAll('.lp-node-action').forEach(item=>item.remove());stage.appendChild(nodeAction);let actionOwner="";let leaveTimer=0;function cancelPreviewLeave(){if(leaveTimer)window.clearTimeout(leaveTimer);leaveTimer=0;}
...
actionOwner=loc;nodeAction.dataset.lpOwner=lpCanonKey(loc);openConcept.href=toAbsoluteUrl(loc);const gate=lpMapExpandGate(loc);gateNote.hidden=!gate;gateNote.textContent=gate==="visit"?"Open and rate this concept to explore beyond it.":gate==="rate"?"Rate this concept on its page to explore beyond it.":"";nodeAction.classList.toggle("is-gated",!!gate);nodeAction.hidden=false;positionNodeAction();}
...
nodeAction.addEventListener('pointerenter',cancelPreviewLeave);nodeAction.addEventListener('pointerleave',schedulePreviewLeave);nodeAction.addEventListener('focusin',cancelPreviewLeave);nodeAction.addEventListener('focusout',event=>{if(!nodeAction.contains(event.relatedTarget))schedulePreviewLeave();});nodeAction.addEventListener('pointerdown',event=>event.stopPropagation());openConcept.addEventListener('click',event=>{if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;event.preventDefault();event.stopPropagation();lpNavigate(openConcept.href,actionOwner);});st2.__lpPositionNodeAction=positionNodeAction;st2.__lpHideNodeAction=()=>{nodeAction.hidden=true;};st2.__lpDisposeNodeAction=()=>{cancelPreviewLeave();if(measuredNodesObserver)measuredNodesObserver.disconnect();if(measuredEdgesRaf)window.cancelAnimationFrame(measuredEdgesRaf);measuredEdgesRaf=0;nodeAction.remove();};function bindNodeInteractions(el,loc){if(!el||el.dataset.lpHoverBound==="1")return;el.dataset.lpHoverBound="1";if(measuredNodesObserver)measuredNodesObserver.observe(el);el.setAttribute('role','button');el.setAttribute('aria-expanded',String(window.MkMapExploration.roots(exploration).includes(lpCanonKey(loc))));el.setAttribute('aria-description',lpCanonKey(loc)===lpCanonKey(cur)?'Current concept. Preview its connections.':'Preview connections; activate to keep them on this map. Use Open concept to visit the page.');let down=null;let moved=false;const live=()=>st2.__lpRenderId===renderId&&el.isConnected;const refresh3d=()=>{const engine=modal.__lpWebgl3dEngine;if(!engine||engine.kind!=='local')return;engine.hoverKey=lpCanonKey(loc);lpWebgl3dRebuild(engine);positionNodeAction();};const preview=()=>{if(!live()||st2.dragging||modal.classList.contains('lp-webgl3d-dragging')||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0))return;cancelPreviewLeave();lpMarkLocalMapInteractionBusy(180);const focusChanged=st2.__lpFocusedLoc!==loc;if(focusChanged)applyFocus(loc);showNodeAction(loc);if(lpNodeShouldRunRevealPreview(el,loc))lpStartRevealPreview(el,loc);if(focusChanged||(modal.__lpWebgl3dEngine&&modal.__lpWebgl3dEngine.hoverKey!==lpCanonKey(loc)))refresh3d();};const retain=()=>{if(!live())return;cancelPreviewLeave();lpPauseRevealPlayback(loc,el);if(lpMapExpandGate(loc)){showNodeAction(loc);el.classList.remove('lp-node-gate-nudge');void el.offsetWidth;el.classList.add('lp-node-gate-nudge');return;}
const neighborhood=lpRootNeighbourhood(loc);const beforeNodes=new Set(window.MkMapExploration.union(exploration,baseNeighbourhood).nodes.map(lpCanonKey));const changed=window.MkMapExploration.expand(exploration,lpCanonKey(loc),neighborhood);if(!changed){showNodeAction(loc);return;}
if(lpMapKeepExpansions())lpWriteKeptExpansions(cur,window.MkMapExploration.roots(exploration));const addedNodeIds=window.MkMapExploration.union(exploration,baseNeighbourhood).nodes.filter(key=>!beforeNodes.has(lpCanonKey(key))).map(normLoc);exploration.positions=new Map(Array.from(pos,([key,point])=>[key,{...point}]));st2.userMoved=true;st2.__lpForceCenter=false;const keyboard=document.activeElement===el;if(!lpAppendWorldExpansion(loc))renderLocalMapModal(graph);else try{lpAttachMapChests(modal)?.updateLayout();}catch(_){}
if(addedNodeIds.length&&modal.classList.contains('lp-open')&&st2.__lpExploration===exploration){try{lpAttachMapChests(modal)?.expanded({nodeId:normLoc(loc),addedNodeIds});}catch(_){}}
const active=Array.from(vp.querySelectorAll('a.lp-node[data-lp-loc]')).find(item=>lpCanonKey(item.dataset.lpLoc)===lpCanonKey(loc));if(keyboard&&active)active.focus({preventScroll:true});if(typeof st2.__lpShowNodeAction==='function')st2.__lpShowNodeAction(loc);};el.addEventListener('pointerenter',event=>{if(event.pointerType&&event.pointerType!=='mouse')return;preview();},{passive:true});el.addEventListener('pointerleave',()=>{if(st2.__lpHoverLoc!==loc)return;lpPauseRevealPlayback(loc,el);schedulePreviewLeave();},{passive:true});el.addEventListener('focus',preview);el.addEventListener('blur',event=>{if(!nodeAction.contains(event.relatedTarget))schedulePreviewLeave();});el.addEventListener('pointerdown',event=>{down={id:event.pointerId,x:event.clientX,y:event.clientY};moved=false;},{passive:true});el.addEventListener('pointermove',event=>{if(down&&down.id===event.pointerId&&Math.hypot(event.clientX-down.x,event.clientY-down.y)>10)moved=true;},{passive:true});el.addEventListener('pointercancel',()=>{moved=true;down=null;});el.addEventListener('click',event=>{if(event.ctrlKey||event.metaKey||event.shiftKey||event.altKey)return;event.preventDefault();event.stopImmediatePropagation();const dragged=moved||!!st2.dragging||Date.now()<(Number(st2.__lpSuppressNodeClickUntil)||0);down=null;if(!dragged)retain();moved=false;},true);el.addEventListener('keydown',event=>{if(!live())return;if(event.key==='Escape'){if(st2.__lpFocusedLoc||st2.__lpHoverLoc||!nodeAction.hidden){event.preventDefault();event.stopPropagation();clearFocus();}
return;}
...
lpApplyLocalMapPerfMode(modal,nodes.length,edges.length);for(const[key,el]of nodeEls){if(lpCanonKey(key)===lpCanonKey(focusLoc))el.setAttribute('aria-expanded','true');}
if(created.length){const mathEls=created.map(p=>nodeEls.get(p)).filter(el=>{try{return lpNodeTitleNeedsTypeset(el.querySelector('.lp-node-title'));}catch(_){return false;}});if(mathEls.length)requestAnimationFrame(()=>{Promise.resolve().then(()=>typesetMathSafe(mathEls.map(el=>el.querySelector('.lp-node-title')||el))).then(()=>{mathEls.forEach(el=>{try{lpCacheRenderedNodeTitle(graph,el);}catch(_){}});}).catch(()=>{});});}
        ...
        <div class="lp-map-topbar">
          ...
          <div class="lp-map-context">
            ...
            <div class="lp-map-context-copy" role="status" aria-live="polite" aria-atomic="true">
              ...
              ${window.__mkExamMode ? '' : '<span class="lp-map-context-note" data-lp-map-treasure-note>Treasure: turn on Knowledge masking and grow the map from concepts you have learned. See Map tips.</span>'}
            ...
            <div class="lp-map-options" data-lp-map-options>
              <label class="lp-map-option" title="Fog of war: the map grows only from concepts you have opened and rated.">
                <input type="checkbox" data-lp-fog-switch><span>Knowledge masking</span>
              </label>
              <label class="lp-map-option" title="Reopen this concept's map with the concepts you kept.">
                <input type="checkbox" data-lp-map-keep><span>Keep expansions</span>
              </label>
            </div>
          </div>
...
return"full";}
function lpMapTabHelperText(kind,isPhone){const phone=typeof isPhone==="boolean"?isPhone:!!__lpIsPhoneTouch();if(String(kind||"")==="local"){const controls=phone?"Tap a node to add its connections. Keep tapping to extend the map. Use Open concept to visit a page.":"Hover to preview connections. Click a node to keep them and extend your map. Use Open concept to visit a page.";return controls+" Drag freely; use the Fit icon to bring the map back into view."+(phone?" Before shows prerequisites; After shows dependents.":"")+(window.__mkExamMode?"":" With Knowledge masking on, the map works as fog of war: it grows only from concepts you have opened and rated, and concepts rated Clear or Mastered light up in their course colour. Treasure needs Knowledge masking, a connected account and at least one new concept added. Each concept gives one chance per UTC day. Crown/gold/silver/bronze chances: 1/1,000, 1/100, 1/20, 1/5; daily limits: 1, 1, 2, 5, including unopened chests. Open before midnight UTC for XP, EORbits and possible shop items; owned items become EORbits. Full rules: Site guide → Map treasures.");}
