...
function renderStatCards(s,overviewOpen){const acc=s.attempted?`${Math.round((s.correct / s.attempted) * 100)}%`:"-";return`
      <details class="mm-overview" id="mm-overview" ${overviewOpen ? "open" : ""}><summary>Learning overview <span>${s.total} concepts</span></summary><div class="mm-stat-grid">
        ...
        ${window.__mkExamMode ? "" : `<div class="mm-stat-card"><span>AI checks</span><strong>${escapeHtml(String(s.aiChecks))}</strong></div><div class="mm-stat-card"><span>Practice answers</span><strong>${escapeHtml(acc)}</strong><small>Self-reported · ${escapeHtml(String(s.correct))}/${escapeHtml(String(s.attempted))}</small></div>`}
      </div></details>`;}
