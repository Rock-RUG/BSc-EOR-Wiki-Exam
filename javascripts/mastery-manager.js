...
function cardHtml(item,index){const href=hrefForLoc(item.loc);const histCount=Array.isArray(item.rec.history)?item.rec.history.length:0;const statusClass=mClass(item.m);return`
      <article class="mm-card ${statusClass}" data-loc="${escapeHtml(item.loc)}">
        ...
        ${window.__mkExamMode ? "" : `<div class="mm-expand-row ${item.aiChecks ? "has-ai" : ""}"id="mm-ai-${index}"data-panel-body="ai"hidden><div class="mm-detail-body"data-lazy-panel="ai"></div></div>`}
      </article>`;}
function renderStatCards(s,overviewOpen){const acc=s.attempted?`${Math.round((s.correct / s.attempted) * 100)}%`:"-";return`
      <details class="mm-overview" id="mm-overview" ${overviewOpen ? "open" : ""}><summary>Learning overview <span>${s.total} concepts</span></summary><div class="mm-stat-grid">
        ...
        ${window.__mkExamMode ? "" : `<div class="mm-stat-card"><span>AI checks</span><strong>${escapeHtml(String(s.aiChecks))}</strong></div><div class="mm-stat-card"><span>Practice answers</span><strong>${escapeHtml(acc)}</strong><small>Self-reported · ${s.attempted?`${escapeHtml(String(s.correct))}/${escapeHtml(String(s.attempted))}`:"No recorded scores"}${s.unavailableScores?` · ${s.unavailableScores} ${s.unavailableScores === 1 ? "check has" : "checks have"} an incomplete score`:""}</small></div>`}
      </div></details>`;}
