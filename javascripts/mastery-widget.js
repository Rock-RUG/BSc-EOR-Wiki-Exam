...
function mwSyncAiQuizButton(btn){if(!btn)return false;if(window.__mkExamMode){btn.hidden=true;btn.disabled=true;mwSyncAiQuizResumeHint(btn,null);return false;}
...
const unavailableLabel=known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading";const nextHidden=!hasApi;const nextDisabled=!available&&!loadFailed;const resumeResult=!!(attempt&&attempt.canResumeResult);const nextLabel=resumeResult?"Resume AI result":loadFailed?"Retry AI concept check":available?label:unavailableLabel;btn.setAttribute("data-aiq-resume-result",resumeResult?"1":"0");btn.setAttribute("data-aiq-load-failed",loadFailed?"1":"0");if(btn.hidden!==nextHidden)btn.hidden=nextHidden;if(btn.disabled!==nextDisabled)btn.disabled=nextDisabled;if(btn.title!==nextLabel)btn.title=nextLabel;if(btn.getAttribute("aria-label")!==nextLabel)btn.setAttribute("aria-label",nextLabel);mwSyncAiQuizResumeHint(btn,hasApi?attempt:null);mwNormaliseAiQuizButtonLabel(btn);return hasApi;}
function mwEnsureInlineAiQuizButton(host,state){if(window.__mkExamMode)return false;if(!host||!host.querySelector)return false;let available=false;let loadFailed=false;let hasApi=false;let known=false;try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));loadFailed=!!(api&&typeof api.getState==="function"&&api.getState().loadFailed);}catch(_){available=false;}
const unavailableLabel=known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading";const existingBtn=host.querySelector("button");if(!hasApi){if(existingBtn)mwSyncAiQuizResumeHint(existingBtn,null);try{if(existingBtn&&existingBtn.dataset&&existingBtn.dataset.mwAiqFallback==="1")existingBtn.remove();}catch(_){}
return false;}
