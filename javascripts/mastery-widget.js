...
function mwEnsureInlineAiQuizButton(host,state){if(!host||!host.querySelector)return false;let available=false;let hasApi=false;let known=false;let cooldown=null;try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));if(api&&typeof api.getCooldownState==="function")cooldown=api.getCooldownState();if(cooldown&&cooldown.active)available=false;}catch(_){available=false;}
...
try{if(window.MasteryManager&&typeof window.MasteryManager.open==="function"){window.MasteryManager.open();}}catch(_){}}
...
function hideSectionsNow(){clearHideTimer();state.titleMenuActiveSection="";syncSections();}
...
function hideSectionsSoon(delayMs){clearHideTimer();state.titleMenuHideTimer=window.setTimeout(()=>{state.titleMenuHideTimer=0;state.titleMenuActiveSection="";syncSections();},Math.max(0,Number(delayMs)||60));}
const supportsHover=!!(window.matchMedia&&window.matchMedia("(hover: hover) and (pointer: fine)").matches);function bindSection(btn,host,name){if(!btn||!host)return;btn.addEventListener("mouseenter",()=>{if(!supportsHover||btn.disabled)return;showSection(name);});btn.addEventListener("mouseleave",()=>{if(!supportsHover)return;hideSectionsSoon(56);});host.addEventListener("mouseenter",()=>{clearHideTimer();});host.addEventListener("mouseleave",()=>{if(!supportsHover)return;hideSectionsSoon(40);});btn.addEventListener("focus",()=>{if(btn.disabled)return;showSection(name);});btn.addEventListener("click",(e)=>{if(btn.disabled)return;if(supportsHover)return;e.preventDefault();e.stopPropagation();state.titleMenuActiveSection=state.titleMenuActiveSection===name?"":name;syncSections();});}
bindSection(readyBtn,readinessHost,"readiness");bindSection(recapBtn,recapHost,"recap");menu.addEventListener("mouseenter",()=>{clearHideTimer();});menu.addEventListener("mouseleave",()=>{if(!supportsHover)return;hideSectionsNow();});refreshMenuWidth();place();try{if(typeof state.renderReadinessNow==="function")state.renderReadinessNow(state.readinessData);else if(typeof state.bindReadinessTriggersNow==="function")state.bindReadinessTriggersNow();}catch(_){}
...
state.readinessHideTimer=window.setTimeout(()=>{state.readinessHideTimer=0;if(state.readinessPointerInside)return;mwHideReadinessPopover();},70);}
...
state.recapHideTimer=window.setTimeout(()=>{state.recapHideTimer=0;if(state.recapPointerInside)return;mwHideRecapPopover();},70);}
