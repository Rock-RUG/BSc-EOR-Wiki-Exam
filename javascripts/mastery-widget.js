...
function mwSyncAiQuizCooldownHint(btn,cooldown){const inTitleMenu=!!(btn.classList&&btn.classList.contains("mw-title-menu-btn--ai"));const host=inTitleMenu?(btn.closest&&btn.closest(".mw-title-menu")):btn.parentNode;if(!host)return;let hint=host.querySelector(".mw-aiq-cooldown");if(window.__mkExamMode||!cooldown||!cooldown.active){if(hint)hint.remove();return;}
if(!hint){hint=document.createElement("span");hint.className="mw-aiq-cooldown";hint.setAttribute("role","status");hint.setAttribute("aria-live","polite");hint.setAttribute("aria-atomic","true");host.appendChild(hint);}
const text=cooldown.canResumeResult?"Your answers are saved. Resume the result to finish saving your score.":`${inTitleMenu ? "AI concept check · " : ""}Available in ${mwFormatAiQuizCooldown(cooldown.remainingMs)}`;if(hint.textContent!==text)hint.textContent=text;}
function mwSyncAiQuizButton(btn){if(!btn)return false;if(window.__mkExamMode){btn.hidden=true;btn.disabled=true;mwSyncAiQuizCooldownHint(btn,null);return false;}
let available=false;let loadFailed=false;let hasApi=false;let known=false;let cooldown=null;const label=MW_AIQ_LABEL;try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));loadFailed=!!(api&&typeof api.getState==="function"&&api.getState().loadFailed);if(api&&typeof api.getCooldownState==="function")cooldown=api.getCooldownState();if(cooldown&&cooldown.active&&!cooldown.canResumeResult){available=false;loadFailed=false;}}catch(_){available=false;}
const unavailableLabel=cooldown&&cooldown.active?`AI concept check available in ${mwFormatAiQuizCooldown(cooldown.remainingMs)}`:(known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading");const nextHidden=!hasApi;const nextDisabled=!available&&!loadFailed;const resumeResult=!!(cooldown&&cooldown.canResumeResult);const nextLabel=resumeResult?"Resume AI result":loadFailed?"Retry AI concept check":available?label:unavailableLabel;btn.setAttribute("data-aiq-resume-result",resumeResult?"1":"0");btn.setAttribute("data-aiq-load-failed",loadFailed?"1":"0");if(btn.hidden!==nextHidden)btn.hidden=nextHidden;if(btn.disabled!==nextDisabled)btn.disabled=nextDisabled;if(btn.title!==nextLabel)btn.title=nextLabel;if(btn.getAttribute("aria-label")!==nextLabel)btn.setAttribute("aria-label",nextLabel);mwSyncAiQuizCooldownHint(btn,hasApi?cooldown:null);mwNormaliseAiQuizButtonLabel(btn);return hasApi;}
function mwEnsureInlineAiQuizButton(host,state){if(window.__mkExamMode)return false;if(!host||!host.querySelector)return false;let available=false;let loadFailed=false;let hasApi=false;let known=false;let cooldown=null;try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));loadFailed=!!(api&&typeof api.getState==="function"&&api.getState().loadFailed);if(api&&typeof api.getCooldownState==="function")cooldown=api.getCooldownState();if(cooldown&&cooldown.active&&!cooldown.canResumeResult){available=false;loadFailed=false;}}catch(_){available=false;}
const unavailableLabel=cooldown&&cooldown.active?`AI concept check available in ${mwFormatAiQuizCooldown(cooldown.remainingMs)}`:(known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading");const existingBtn=host.querySelector("button");if(!hasApi){if(existingBtn)mwSyncAiQuizCooldownHint(existingBtn,null);try{if(existingBtn&&existingBtn.dataset&&existingBtn.dataset.mwAiqFallback==="1")existingBtn.remove();}catch(_){}
return false;}
