...
function mwVerifiedCheckHtml(kind){const k=mwNormaliseVerificationKind(kind);return`<span class="mw-ai-verified${k === "stale" ? " is-stale" : ""}" aria-hidden="true"><svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" focusable="false"><path d="M13 4.5 6.5 11 3 7.5"/></svg></span>`;}
...
function mwRecapIconHtml(size){const s=Number(size)||18;return mwInkSvg(`<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>`);}
function mwAiQuizIconHtml(size){const s=Number(size)||18;return mwInkSvg(`<svg width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M14 2v6a2 2 0 0 0 .25.96l5.5 10.08A2 2 0 0 1 18 22H6a2 2 0 0 1-1.75-2.96l5.5-10.08A2 2 0 0 0 10 8V2"/><path d="M6.45 15h11.1"/><path d="M8.5 2h7"/></svg>`);}
...
function mwNormaliseAiQuizButtonLabel(root){const targets=[];try{if(root&&root.matches&&root.matches(".mw-aiq-btn, [data-aiq-widget-host='1'] button, .mw-title-menu-btn--ai")){targets.push(root);}
...
targets.forEach((btn)=>{if(!btn)return;const label=btn.__mwAiOpening?"Opening AI quiz…":btn.getAttribute("data-aiq-coming-soon")==="1"?MW_AIQ_UNAVAILABLE_LABEL:btn.getAttribute("data-aiq-resume-result")==="1"?"Resume AI result":btn.getAttribute("data-aiq-load-failed")==="1"?"Retry AI concept check":MW_AIQ_LABEL;let wroteLabelNode=false;try{const labelNode=btn.querySelector&&btn.querySelector(".mw-aiq-btn-label, [data-aiq-label], .aiq-label");if(labelNode&&String(labelNode.textContent||"").trim()!==label){labelNode.textContent=label;wroteLabelNode=true;}else if(labelNode){wroteLabelNode=true;}}catch(_){}
if(!wroteLabelNode){try{Array.from(btn.childNodes||[]).forEach((node)=>{if(node&&node.nodeType===Node.TEXT_NODE&&/(?:AI\s+(concept|mastery)\s+check|Resume AI result)/i.test(node.textContent||"")){node.textContent=String(node.textContent||"").replace(/(?:Retry\s+)?AI\s+(?:concept|mastery)\s+check|Resume AI result/ig,label);}});}catch(_){}}});}
...
function mwSyncAiQuizResumeHint(btn,attempt){const inTitleMenu=!!(btn.classList&&btn.classList.contains("mw-title-menu-btn--ai"));const host=inTitleMenu?(btn.closest&&btn.closest(".mw-title-menu")):btn.parentNode;if(!host)return;let hint=host.querySelector(".mw-aiq-resume-hint");if(window.__mkExamMode||!attempt||!attempt.canResumeResult){if(hint)hint.remove();return;}
if(!hint){hint=document.createElement("span");hint.className="mw-aiq-resume-hint";hint.setAttribute("role","status");hint.setAttribute("aria-live","polite");hint.setAttribute("aria-atomic","true");host.appendChild(hint);}
const text="Your answers are saved. Resume the result to finish saving your score.";if(hint.textContent!==text)hint.textContent=text;}
function mwAiQuizEntryEnabled(){if(window.__mkExamMode)return false;try{const prefs=window.MkStartupPrefs;if(prefs&&typeof prefs.isEnabled==="function"&&prefs.isEnabled("aiQuiz")===false)return false;}catch(_){}
return true;}
function mwSyncAiQuizButton(btn){if(!btn)return false;if(!mwAiQuizEntryEnabled()){btn.hidden=true;btn.disabled=true;mwSyncAiQuizResumeHint(btn,null);return false;}
let available=false;let loadFailed=!!btn.__mwAiLoadFailed;let hasApi=false;const eligibility=document.querySelector('meta[name="mk-concept-ai-mcq"]');const excluded=!!(eligibility&&eligibility.content==="false");let known=excluded;let attempt=null;const label=MW_AIQ_LABEL;if(!excluded)try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));loadFailed=loadFailed||!!(api&&typeof api.getState==="function"&&api.getState().loadFailed);if(api&&typeof api.getAttemptState==="function")attempt=api.getAttemptState();}catch(_){available=false;}
const unavailableLabel=known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading";const nextHidden=false;const nextDisabled=!!btn.__mwAiOpening||(known&&!available&&!loadFailed);const resumeResult=!!(attempt&&attempt.canResumeResult);const nextLabel=resumeResult?"Resume AI result":loadFailed?"Retry AI concept check":available?label:unavailableLabel;btn.setAttribute("data-aiq-coming-soon",known&&!available&&!loadFailed&&!resumeResult?"1":"0");btn.setAttribute("data-aiq-resume-result",resumeResult?"1":"0");btn.setAttribute("data-aiq-load-failed",loadFailed?"1":"0");if(btn.hidden!==nextHidden)btn.hidden=nextHidden;if(btn.disabled!==nextDisabled)btn.disabled=nextDisabled;if(btn.title!==nextLabel)btn.title=nextLabel;if(btn.getAttribute("aria-label")!==nextLabel)btn.setAttribute("aria-label",nextLabel);mwSyncAiQuizResumeHint(btn,hasApi?attempt:null);mwNormaliseAiQuizButtonLabel(btn);return hasApi;}
function mwEnsureInlineAiQuizButton(host,state){if(!host||!host.querySelector)return false;host.hidden=!mwAiQuizEntryEnabled();if(host.hidden){host.classList.remove("mw-aiq-host--reserve");host.dataset.mwAiqReserveResolved="1";return false;}
let available=false;let loadFailed=false;let hasApi=false;let known=false;try{const api=mwGetAiQuizApi();hasApi=!!(api&&typeof api.open==="function");known=!!(api&&typeof api.availabilityKnown==="function"&&api.availabilityKnown());available=!!(api&&typeof api.open==="function"&&(typeof api.isAvailable!=="function"||api.isAvailable()));loadFailed=!!(api&&typeof api.getState==="function"&&api.getState().loadFailed);}catch(_){available=false;}
const unavailableLabel=known?MW_AIQ_UNAVAILABLE_LABEL:"AI mastery check loading";const existingBtn=host.querySelector("button");if(existingBtn){mwSyncAiQuizButton(existingBtn);mwNormaliseAiQuizButtonLabel(host);return true;}
const btn=document.createElement("button");btn.type="button";btn.className="mw-aiq-btn";btn.dataset.mwAiqFallback="1";btn.setAttribute("data-aiq-load-failed",loadFailed?"1":"0");btn.innerHTML=`${mwAiQuizIconHtml(18)}<span class="mw-aiq-btn-label">${loadFailed ? "Retry AI concept check" : MW_AIQ_LABEL}</span>`;btn.disabled=!available&&!loadFailed;btn.title=loadFailed?"Retry AI concept check":available?MW_AIQ_LABEL:unavailableLabel;btn.setAttribute("aria-label",btn.title);mwBindTouchSafeButton(btn,()=>{mwDemandAiQuiz(btn,"mastery-widget");});host.appendChild(btn);mwSyncAiQuizButton(btn);mwNormaliseAiQuizButtonLabel(host);return true;}
async function mwDemandAiQuiz(btn,source){if(!btn||btn.__mwAiOpening)return;mwSyncAiQuizButton(btn);if(btn.disabled||btn.hidden)return;const path=window.location.pathname;const article=document.querySelector('article.md-content__inner');btn.__mwAiOpening=true;btn.__mwAiLoadFailed=false;btn.disabled=true;btn.setAttribute('aria-busy','true');mwNormaliseAiQuizButtonLabel(btn);try{let api=mwGetAiQuizApi();if(!api||typeof api.open!=='function'){const loader=window.MkStartupPrefs;if(!loader||typeof loader.ensureFeature!=='function'||!await loader.ensureFeature('aiQuiz'))return;api=mwGetAiQuizApi();}
if(path!==window.location.pathname||article!==document.querySelector('article.md-content__inner')||!btn.isConnected||!mwAiQuizEntryEnabled()||!api||typeof api.open!=='function')return;if(source==='title-menu')mwCloseTitleMenu();const opened=mwOpenAiQuizWithHistoryRepair(api,{source});if(opened&&typeof opened.catch==='function')opened.catch(()=>{});}catch(_){btn.__mwAiLoadFailed=true;}finally{btn.__mwAiOpening=false;btn.removeAttribute('aria-busy');mwSyncAiQuizButton(btn);}}
function mwOpenAiQuizFromTitleMenu(btn){mwDemandAiQuiz(btn,'title-menu');}
...
function scLevelLabel(m){if(m===3)return"Mastered";if(m===2)return"Clear";if(m===1)return"Partial";if(m===0)return"Unknown";return"Not rated";}
...
async function scReadPrereqReadiness(conceptId){try{const cm=window.ConceptMastery;if(!cm||typeof cm.readinessOf!=="function")return null;const data=await cm.readinessOf(conceptId,{maxDepth:2}).catch(()=>null);if(!data||data.status!=="ok")return null;if(Number(data.selectedCount||data.totalAvailable||0)<=0)return null;if(!["number","string"].includes(typeof data.pct)||String(data.pct).trim()==="")return null;const pct=Number(data.pct);if(!Number.isFinite(pct))return null;return Object.assign({},data,{pct:Math.max(0,Math.min(100,Math.round(pct)))});}catch(_){return null;}}
function scJudge(level,data){if(!data||!["number","string"].includes(typeof data.pct)||String(data.pct).trim()==="")return null;const m=Number(level);const pct=Number(data&&data.pct);if(![0,1,2,3].includes(m)||!Number.isFinite(pct))return null;if(m===3&&pct<65){return{suspicious:true,direction:"high",reason:`You selected Mastered, but prerequisite readiness is only ${pct}%.`};}
if(m===2&&pct<30){return{suspicious:true,direction:"high",reason:`You selected Clear, but prerequisite readiness is only ${pct}%.`};}
if(m===1&&pct>=85){return{suspicious:true,direction:"low",reason:`You selected Partial, but prerequisite readiness is already ${pct}%.`};}
...
let scCloseDialog=null;function scShowDialog(level,data,judgement){scEnsureStyle();if(scCloseDialog)scCloseDialog("cancel");const returnFocus=document.activeElement;const fallbackFocus=__mw_title_menu_anchor;return new Promise((resolve)=>{let modal=document.querySelector(".mw-selfcheck-modal");if(modal)modal.remove();modal=document.createElement("div");modal.className="mw-selfcheck-modal";modal.setAttribute("role","dialog");modal.setAttribute("aria-modal","true");modal.setAttribute("aria-labelledby","mw-selfcheck-title");modal.setAttribute("aria-describedby","mw-selfcheck-reason");modal.setAttribute("tabindex","-1");const pct=Number(data&&data.pct);const selectedLabel=scLevelLabel(Number(level));const reason=judgement&&judgement.reason?judgement.reason:"This self-assessment does not match the current prerequisite readiness signal.";modal.innerHTML=`
          <div class="mw-selfcheck-panel" role="document">
            <div class="mw-selfcheck-head">
              <div class="mw-selfcheck-title" id="mw-selfcheck-title">Check your self-assessment</div>
            </div>
            <div class="mw-selfcheck-body">
              <div class="mw-selfcheck-copy" id="mw-selfcheck-reason">
          ...
          </div>`;let settled=false;const cleanup=(value)=>{if(settled)return;settled=true;try{document.removeEventListener("keydown",onKey,true);}catch(_){}
...
if(scCloseDialog===cleanup)scCloseDialog=null;const opener=returnFocus&&returnFocus.isConnected&&returnFocus.getClientRects().length?returnFocus:fallbackFocus;if(opener&&opener.isConnected&&opener.getClientRects().length){try{opener.focus({preventScroll:true});}catch(_){opener.focus();}}
...
const btn=ev.target&&ev.target.closest?ev.target.closest("button[data-action]"):null;if(!btn)return;ev.preventDefault();ev.stopPropagation();cleanup(btn.getAttribute("data-action")||"cancel");});const onKey=(ev)=>{if(ev.key==="Escape"){ev.preventDefault();ev.stopPropagation();cleanup("cancel");}else if(ev.key==="Tab"){const controls=Array.from(modal.querySelectorAll("button:not([disabled])"));const first=controls[0]||modal;const last=controls[controls.length-1]||modal;if(!modal.contains(document.activeElement)||(ev.shiftKey?document.activeElement===first:document.activeElement===last)){ev.preventDefault();(ev.shiftKey?last:first).focus();}}};scCloseDialog=cleanup;document.addEventListener("keydown",onKey,true);document.body.appendChild(modal);try{document.documentElement.classList.add("mw-selfcheck-open");}catch(_){}
try{if(document.body)document.body.classList.add("mw-selfcheck-open");}catch(_){}
const firstControl=modal.querySelector("button[data-action='continue']")||modal;try{firstControl.focus({preventScroll:true});}catch(_){firstControl.focus();}
...
function mwResolveSectionLineColor(){const prefs=window.MkStartupPrefs;return prefs&&typeof prefs.masterySectionLineColor==="function"?prefs.masterySectionLineColor():"";}
...
try{const enabled=()=>!window.MkStartupPrefs||typeof window.MkStartupPrefs.isEnabled!=="function"||window.MkStartupPrefs.isEnabled('mastery')!==false;if(!enabled())return;if(window.MasteryManager&&typeof window.MasteryManager.open==="function"){window.MasteryManager.open();}else if(window.MkStartupPrefs&&window.MkStartupPrefs.ensureFeature){window.MkStartupPrefs.ensureFeature('mastery').then((loaded)=>{if(loaded===false||!enabled())return;if(window.MasteryManager&&typeof window.MasteryManager.open==="function")window.MasteryManager.open();}).catch(()=>{});}}catch(_){}}
...
function mwCloseTitleMenu(options){const anchor=__mw_title_menu_anchor;try{if(typeof __mw_title_menu_close==="function")__mw_title_menu_close();}catch(_){}
__mw_title_menu_close=null;__mw_title_menu_anchor=null;if(options&&options.returnFocus&&anchor&&anchor.isConnected&&anchor.getClientRects().length){try{anchor.focus({preventScroll:true});}catch(_){anchor.focus();}}}
...
try{if(typeof window.ConceptMastery.rate==="function"){window.ConceptMastery.rate(state.conceptId,m,state.meta,{source:"title-menu",visitId:mwVisitId(state.conceptId),});}else if(typeof window.ConceptMastery.set==="function"){window.ConceptMastery.set(state.conceptId,m,state.meta);}}catch(_){}});levels.appendChild(btn);});const sep=document.createElement("span");sep.className="mw-title-menu-sep";sep.setAttribute("aria-hidden","true");const readyBtn=document.createElement("button");readyBtn.type="button";readyBtn.className="mw-title-menu-btn mw-title-menu-btn--ready";readyBtn.title="Prerequisite readiness";readyBtn.setAttribute("aria-label","Prerequisite readiness");readyBtn.setAttribute("aria-expanded","false");readyBtn.setAttribute("data-mw-ready-trigger","1");readyBtn.innerHTML=`<span class="mw-ready-chip-pct">${mwReadinessDisplayText(state && state.readinessData)}</span>`;readyBtn.disabled=!(state&&state.readinessData&&state.readinessData.status==="ok");const recapBtn=document.createElement("button");recapBtn.type="button";recapBtn.className="mw-title-menu-btn mw-title-menu-btn--recap";recapBtn.title="Mastery recap";recapBtn.setAttribute("aria-label","Mastery recap");recapBtn.setAttribute("aria-expanded","false");recapBtn.innerHTML=mwRecapIconHtml(18);const aiQuizBtn=document.createElement("button");aiQuizBtn.type="button";aiQuizBtn.className="mw-title-menu-btn mw-title-menu-btn--ai";aiQuizBtn.innerHTML=mwAiQuizIconHtml(18);mwSyncAiQuizButton(aiQuizBtn);mwBindTouchSafeButton(aiQuizBtn,()=>{if(aiQuizBtn.disabled||aiQuizBtn.hidden)return;mwOpenAiQuizFromTitleMenu(aiQuizBtn);});const manageBtn=document.createElement("button");manageBtn.type="button";manageBtn.className="mw-title-menu-btn mw-title-menu-btn--manage";manageBtn.title="Manage mastery";manageBtn.setAttribute("aria-label","Manage mastery");manageBtn.innerHTML=mwGearHtml();manageBtn.addEventListener("click",(e)=>{e.preventDefault();e.stopPropagation();mwCloseTitleMenu({returnFocus:true});mwOpenMasteryManager();});const toggleBtn=document.createElement("button");toggleBtn.type="button";toggleBtn.className="mw-title-menu-btn mw-title-menu-btn--toggle";const currentlyHidden=!!mwReadWidgetHidden();const hasExplicitRatingNow=!!(state&&state.hasExplicitRating);try{state.widgetHidden=currentlyHidden;}catch(_){}
...
window.setTimeout(()=>{try{renderWidget();}catch(_){}},0);});const extras=document.createElement("div");extras.className="mw-title-menu-extras";extras.appendChild(readyBtn);extras.appendChild(aiQuizBtn);extras.appendChild(recapBtn);extras.appendChild(manageBtn);if(!hasExplicitRatingNow)extras.appendChild(toggleBtn);const top=document.createElement("div");top.className="mw-title-menu-top";top.appendChild(levels);top.appendChild(sep);top.appendChild(extras);const readinessHost=document.createElement("div");readinessHost.className="mw-title-menu-section";readinessHost.setAttribute("data-mw-ready-inline","1");readinessHost.hidden=true;const recapHost=document.createElement("div");recapHost.className="mw-title-menu-section mw-recap";recapHost.setAttribute("data-mw-recap","menu");recapHost.hidden=true;menu.appendChild(top);mwSyncAiQuizButton(aiQuizBtn);menu.appendChild(readinessHost);menu.appendChild(recapHost);document.body.appendChild(menu);menu.dataset.mwTitleMenu="1";__mw_title_menu_anchor=anchorEl;try{anchorEl.setAttribute("aria-expanded","true");}catch(_){}
state.titleMenuEl=menu;state.titleMenuReadyBtn=readyBtn;state.titleMenuRecapBtn=recapBtn;state.titleMenuReadinessHost=readinessHost;state.titleMenuRecapHost=recapHost;state.titleMenuActiveSection="";state.titleMenuPinnedSection="";state.titleMenuHideTimer=0;const refreshMenuWidth=()=>{return mwApplyTitleMenuAdaptiveWidth(menu,top,[readinessHost,recapHost]);};state.titleMenuRefreshWidth=refreshMenuWidth;const place=()=>{const r=anchorEl.getBoundingClientRect();const desiredTop=Math.max(8,r.bottom+8);const availableBelow=Math.max(0,window.innerHeight-desiredTop-8);let topRowHeight=52;try{const topRect=top&&top.getBoundingClientRect?top.getBoundingClientRect():null;if(topRect&&topRect.height)topRowHeight=Math.ceil(topRect.height);}catch(_){}
const sectionMax=Math.max(0,Math.min(290,availableBelow-topRowHeight-28));try{menu.style.setProperty("--mw-title-menu-available-below",`${Math.floor(availableBelow)}px`);menu.style.setProperty("--mw-title-menu-section-max",`${Math.floor(sectionMax)}px`);}catch(_){}
...
bindSection(readyBtn,readinessHost,"readiness");bindSection(recapBtn,recapHost,"recap");menu.addEventListener("mouseenter",()=>{clearHideTimer();});menu.addEventListener("focusin",clearHideTimer);menu.addEventListener("focusout",()=>hideSectionsSoon(MW_MENU_HOVER_GRACE_MS));menu.addEventListener("mouseleave",()=>{if(!supportsHover)return;hideSectionsSoon(MW_MENU_HOVER_GRACE_MS);});refreshMenuWidth();place();try{if(typeof state.renderReadinessNow==="function")state.renderReadinessNow(state.readinessData);else if(typeof state.bindReadinessTriggersNow==="function")state.bindReadinessTriggersNow();}catch(_){}
try{if(typeof state.renderRecapNow==="function")state.renderRecapNow(place);}catch(_){}
syncSections();refreshMenuWidth();place();const onDoc=(e)=>{if(menu.contains(e.target)||anchorEl.contains(e.target))return;mwCloseTitleMenu();};const onKey=(e)=>{if(e.key!=="Escape")return;if(e.target&&e.target.closest&&e.target.closest('[aria-modal="true"]'))return;e.preventDefault();mwCloseTitleMenu({returnFocus:true});};const onAiQuizAvailability=()=>{try{mwSyncAiQuizButton(aiQuizBtn);refreshMenuWidth();place();}catch(_){}};document.addEventListener("pointerdown",onDoc,true);document.addEventListener("keydown",onKey,true);window.addEventListener("resize",place);window.addEventListener("scroll",place,true);window.addEventListener("aiq:ready",onAiQuizAvailability);window.addEventListener("aiq:availability-change",onAiQuizAvailability);window.addEventListener("mk-ai-quiz-sessions-changed",onAiQuizAvailability);__mw_title_menu_close=()=>{clearHideTimer();try{document.removeEventListener("pointerdown",onDoc,true);}catch(_){}
...
try{window.removeEventListener("mk-ai-quiz-sessions-changed",onAiQuizAvailability);}catch(_){}
...
function mwCompactMarkup(){return`
      <div class="mw-row" aria-label="Mastery quick actions">
        ...
        <button type="button" class="mw-pill" data-m="1" title="Partial" aria-label="Partial">
...
function mwFinishOnReducedMotion(finish){const onChange=()=>{if(mwMotionReduced())finish();};let media=null;try{window.addEventListener("mk:site-motion-change",onChange);window.addEventListener("mk:motionchange",onChange);media=window.matchMedia&&window.matchMedia("(prefers-reduced-motion: reduce)");if(media&&typeof media.addEventListener==="function")media.addEventListener("change",onChange);else if(media&&typeof media.addListener==="function")media.addListener(onChange);}catch(_){}
return()=>{try{window.removeEventListener("mk:site-motion-change",onChange);}catch(_){}
try{window.removeEventListener("mk:motionchange",onChange);}catch(_){}
try{if(media&&typeof media.removeEventListener==="function")media.removeEventListener("change",onChange);else if(media&&typeof media.removeListener==="function")media.removeListener(onChange);}catch(_){}};}
function mwAnimateGhost(ghost,fromPt,toPt,opts){const duration=Math.max(180,Math.min(1200,Number(opts&&opts.duration)||420));const easing=String((opts&&opts.easing)||"cubic-bezier(.22, 1, .36, 1)");if(!ghost||!fromPt||!toPt)return Promise.resolve();if(mwMotionReduced()){ghost.style.left=toPt.x+"px";ghost.style.top=toPt.y+"px";return Promise.resolve();}
try{mwSyncFlyLayerHeaderClip(ghost.closest?ghost.closest(".mw-fly-layer"):null);}catch(_){}
ghost.style.left=fromPt.x+"px";ghost.style.top=fromPt.y+"px";const dx=toPt.x-fromPt.x;const dy=toPt.y-fromPt.y;const control={x:fromPt.x+dx*0.45,y:fromPt.y+dy*0.25-60,};const mid1=mwBezierQuad(fromPt,control,toPt,0.55);const mid2=mwBezierQuad(fromPt,control,toPt,0.82);const base="translate(-50%, -50%)";const kf=[{transform:`${base} translate(0px, 0px) scale(1)`,opacity:1,offset:0},{transform:`${base} translate(${dx * 0.06}px, -18px) scale(1.10)`,opacity:1,offset:0.14},{transform:`${base} translate(${mid1.x - fromPt.x}px, ${mid1.y - fromPt.y}px) scale(1.04)`,opacity:1,offset:0.58},{transform:`${base} translate(${mid2.x - fromPt.x}px, ${mid2.y - fromPt.y}px) scale(1.01)`,opacity:1,offset:0.82},{transform:`${base} translate(${dx}px, ${dy}px) scale(1)`,opacity:1,offset:1},];if(ghost.animate){const anim=ghost.animate(kf,{duration,easing,fill:"forwards",});return new Promise((resolve)=>{let done=false;let timer=0;let releaseMotion=()=>{};const finish=()=>{if(done)return;done=true;releaseMotion();if(mwMotionReduced()){try{anim.finish();}catch(_){try{anim.cancel();}catch(_){}
ghost.style.transform=kf[kf.length-1].transform;}}
try{if(timer)window.clearTimeout(timer);}catch(_){}
resolve();};releaseMotion=mwFinishOnReducedMotion(finish);try{timer=window.setTimeout(finish,duration+260);}catch(_){}
...
layer.appendChild(ghost);return ghost;}
function mwAnimateTitleTranslate(titleEl,measure,duration,easing){if(!titleEl||!measure||!measure.titleBefore||!measure.titleAfter)return Promise.resolve();if(mwMotionReduced()){mwResetTitleFlight(titleEl);return Promise.resolve();}
const dx=(Number(measure.titleBefore.left)||0)-(Number(measure.titleAfter.left)||0);if(Math.abs(dx)<1){mwResetTitleFlight(titleEl);return Promise.resolve();}
...
return new Promise((resolve)=>{let done=false;let timer=0;let raf=0;let releaseMotion=()=>{};const finish=()=>{if(done)return;done=true;releaseMotion();try{cancelAnimationFrame(raf);}catch(_){}
try{if(timer)window.clearTimeout(timer);}catch(_){}
...
releaseMotion=mwFinishOnReducedMotion(finish);timer=window.setTimeout(finish,ms+140);raf=requestAnimationFrame(()=>{if(done)return;if(mwMotionReduced()){finish();return;}
try{titleEl.style.transition=`transform ${ms}ms ${String(easing || "cubic-bezier(.22, 1, .36, 1)")}`;titleEl.style.transform="translateX(0px)";}catch(_){finish();}});});}
...
function mwClearCollapsedBoxLine(box){if(!box)return;try{const wasCollapsed=box.classList.contains("mw-collapsed-line");box.classList.remove("mw-collapsed-line");box.removeAttribute("data-mw-collapsed-kind");box.removeAttribute("data-mw-collapsed-interactive");box.removeAttribute("aria-hidden");box.style.pointerEvents="";if(wasCollapsed)mwClearCollapseStyles(box);}catch(_){}}
...
function mwSyncHiddenWidget(state,hasExplicitRating,rec){if(!state||!state.widgetHidden)return false;const box=state.box;const compact=state.compact;state.dismissed=true;state.forceExpanded=false;state.needsRatingCue=false;state.dismissLineKind=hasExplicitRating?mwDismissLineKindForLevel(rec.m):"hidden";state.dismissLineInteractive=!hasExplicitRating;if(box){box.classList.remove("mw-docked");if(state.keepCollapsedDom){mwClearDismissedLineState(state,{animate:false});mwApplyCollapsedBoxLine(box,state.dismissLineKind,state.dismissLineInteractive);box.classList.remove("mw-source-hidden","mw-dismissed");}else{mwClearCollapsedBoxLine(box);mwApplyDismissedLineState(state,{animate:false});box.classList.add("mw-source-hidden","mw-dismissed");}}
if(compact){compact.classList.remove("is-visible");compact.classList.add("mw-dismissed");}
return true;}
...
return Promise.resolve();}
return new Promise((resolve)=>{let done=false;let raf=0;let startTs=0;let safetyTimer=0;let releaseMotion=()=>{};const finish=()=>{if(done)return;done=true;releaseMotion();try{cancelAnimationFrame(raf);}catch(_){}
...
try{entry.node.style.transform=`translate3d(0, ${innerY.toFixed(2)}px, 0)`;}catch(_){}});};const tick=(ts)=>{if(done)return;if(mwMotionReduced()){finish();return;}
if(!startTs)startTs=ts||performance.now();const p=clamp01(((ts||performance.now())-startTs)/ms);paint(p);if(p>=1)finish();else raf=requestAnimationFrame(tick);};releaseMotion=mwFinishOnReducedMotion(finish);if(mwMotionReduced()){finish();return;}
try{safetyTimer=window.setTimeout(finish,ms+700);}catch(_){}
try{raf=requestAnimationFrame((ts)=>{if(done)return;if(mwMotionReduced()){finish();return;}
paint(0);raf=requestAnimationFrame(tick);});}catch(_){finish();}});}
...
await mwDelay(preFadeMs);const reducedBeforeLaunch=mwMotionReduced();if(reducedBeforeLaunch){mwResetTitleFlight(sc.titleText);for(const source of[sourceScope,sourceBox]){if(source&&source.classList)source.classList.remove("mw-preflight","mw-flight-launch");}}
if(!reducedBeforeLaunch&&!cleanCollapse){if(sourceScope&&sourceScope.classList){sourceScope.classList.add("mw-flight-launch");}
if(sourceBox&&sourceBox.classList&&sourceBox!==sourceScope){sourceBox.classList.add("mw-flight-launch");}}
if(!reducedBeforeLaunch&&cleanCollapse){const sourcePill=iconSourceEl&&iconSourceEl.closest?(iconSourceEl.closest(".mw-pill")||iconSourceEl):iconSourceEl;restoreCollapseOverlay=collapseSource?mwCreateCollapseVisualOverlay(collapseSource,{excludeNode:sourcePill}):(()=>{});restoreCleanSource=mwTemporarilyHideForFlight(sourcePill);}
ghostBadge=!reducedBeforeLaunch&&iconSourceEl?mwCreateGhost("badge",m,iconSourceEl,badge):null;if(ghostBadge&&badgeStartPt){ghostBadge.style.left=badgeStartPt.x+"px";ghostBadge.style.top=badgeStartPt.y+"px";}
...
function mwEnsureRatingStatus(){let status=document.getElementById("mw-rating-status");if(!status){status=document.createElement("div");status.id="mw-rating-status";status.className="mk-visually-hidden";status.setAttribute("role","status");status.setAttribute("aria-live","polite");status.setAttribute("aria-atomic","true");document.body.appendChild(status);}
return status;}
function mwAnnounceRatingChange(event,conceptId){const detail=event&&event.detail;if(!detail||normLoc(detail.conceptId||detail.path||"")!==conceptId)return;if(!["widget","title-menu","ai-mcq"].includes(detail.source))return;if(detail.level==null||![0,1,2,3].includes(Number(detail.level)))return;mwEnsureRatingStatus().textContent=`Rated: ${mwLevelLabel(Number(detail.level))}.`;}
function mwCloseLevelInfo(){const dialog=document.getElementById("mw-level-info-dialog");if(!dialog)return;dialog.__mwReturnFocus=false;if(dialog.open&&typeof dialog.close==="function")dialog.close();dialog.remove();}
function mwOpenLevelInfo(trigger){mwCloseLevelInfo();const dialog=document.createElement("dialog");dialog.id="mw-level-info-dialog";dialog.setAttribute("aria-labelledby","mw-level-info-title");dialog.setAttribute("aria-modal","true");dialog.__mwReturnFocus=true;dialog.innerHTML=`
      <div class="mw-level-info-head">
        <h2 id="mw-level-info-title">Understanding levels</h2>
        <button type="button" class="mw-level-info-close" aria-label="Close understanding levels" autofocus><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
      </div>
      <dl class="mw-level-info-list">
        <dt>Mastered (M)</dt><dd>You can use the concept independently, solve a new problem with it, and explain why your approach works.</dd>
        <dt>Clear (C)</dt><dd>You can explain the main idea in your own words and follow standard examples, but may still need guidance to apply it independently.</dd>
        <dt>Partial (P)</dt><dd>You recognise some of the idea, but gaps or confusing steps keep you from explaining or using it reliably.</dd>
        <dt>Unknown (U)</dt><dd>You have not learned the concept yet, or cannot explain its central idea. Start with the definition and a basic example.</dd>
      </dl>
      <p class="mw-level-info-note">Choose the level that reflects your understanding now. These are your own learning notes, not course grades. A concept with no rating is separate from Unknown. “Not assessed” means none of the prerequisites has an explicit rating yet.</p>`;dialog.querySelector(".mw-level-info-close").addEventListener("click",()=>dialog.close());dialog.addEventListener("click",(ev)=>{if(ev.target!==dialog)return;const r=dialog.getBoundingClientRect();if(ev.clientX<r.left||ev.clientX>r.right||ev.clientY<r.top||ev.clientY>r.bottom)dialog.close();});dialog.addEventListener("close",()=>{dialog.remove();if(dialog.__mwReturnFocus&&trigger&&trigger.isConnected)trigger.focus({preventScroll:true});},{once:true});document.body.appendChild(dialog);dialog.showModal();}
function renderWidget(){mwCloseLevelInfo();if(typeof window.__mw_widget_cleanup==="function"){try{window.__mw_widget_cleanup();}catch(_){}
window.__mw_widget_cleanup=null;}
try{mwClearFlyGhosts();}catch(_){}
if(window.MkStartupPrefs&&typeof window.MkStartupPrefs.isEnabled==="function"&&window.MkStartupPrefs.isEnabled("mastery")===false)return;const rel=currentRelPath();if(!isConceptPage(rel))return;if(!window.ConceptMastery)return;const inner=document.querySelector("article.md-content__inner");if(!inner)return;const existing=document.getElementById("mw-mastery");if(existing)existing.remove();const existingCompact=document.getElementById("mw-mastery-compact");if(existingCompact)existingCompact.remove();const existingAnchor=inner.querySelector(":scope > .mw-anchor");if(existingAnchor)existingAnchor.remove();const existingReadinessPopover=document.getElementById("mw-ready-popover");if(existingReadinessPopover)existingReadinessPopover.remove();ensureStyles();mwEnsureReadinessStyles();const conceptId=normLoc(rel);mwEnsureRatingStatus().textContent="";const title=normaliseMathDelimitersToDollar(((document.querySelector("h1")&&document.querySelector("h1").textContent)||document.title||"").trim());const course=mwResolveCourseFromTagOrFallback(conceptId);const meta={title,course,coursePath:conceptId.split("/").slice(0,-1).join("/"),};const state={conceptId,meta,ratedThisVisit:false,needsRatingCue:false,hasExplicitRating:false,currentM:null,aiVerificationKind:"",pendingFly:null,pendingBadgeFade:null,badgeFadeAnimating:false,dockMode:"normal",lastBoxHeight:0,raf:0,anchor:null,box:null,compact:null,observer:null,compactPos:{mode:"auto",left:0,top:0},compactDrag:null,anchorPageTop:0,dismissed:false,dismissLineKind:"default",dismissLineInteractive:false,keepCollapsedDom:false,flyAnimating:false,destroyers:[],searchObserver:null,searchSyncRaf:0,searchDockModeBeforeSearch:"normal",searchCompactHoldUntil:0,readinessSeq:0,readinessData:null,readinessPopover:null,readinessOpen:false,readinessPinned:false,readinessAnchor:null,readinessPointerInside:false,readinessHideTimer:0,recapData:null,recapExpandedWidget:false,recapExpandedMenu:false,recapPopover:null,recapOpen:false,recapPinned:false,recapAnchor:null,recapPointerInside:false,recapHideTimer:0,widgetHidden:mwReadWidgetHidden(),forceExpanded:mwConsumeWidgetForceOpen(conceptId),};function mwReadinessFlattenItemsForCalc(data){const groups=data&&data.groups?data.groups:{};return[].concat(Array.isArray(groups.readyNow)?groups.readyNow:[]).concat(Array.isArray(groups.reviewLightly)?groups.reviewLightly:[]).concat(Array.isArray(groups.learnFirst)?groups.learnFirst:[]);}
        ...
        <div class="mw-ready-calc" data-mk-no-hover-preview="1">
          ...
          <div class="mw-ready-score-card">
            <div class="mw-ready-score-main">
              <span class="mw-ready-score-number">${mwReadinessDisplayText(data)}</span>
              <span class="mw-ready-score-label">current page readiness</span>
            </div>
            <div class="mw-ready-formula">
              <span>${mwReadinessUnassessed(data) ? "After you assess a prerequisite, calculated as" : "Calculated as"}</span>
              <span class="mw-ready-formula-code">Σ influence × mastery score</span>
            </div>
            <div class="mw-ready-scale">Mastered/Clear = 100%, Partial = 50%, Unknown/Not visited = 0%.</div>
          </div>
...
const seq=++state.readinessSeq;const initial=mwBootstrapReadiness(conceptId);if(initial)hiddenRenderReadiness(initial);else if(!state.readinessData)hiddenRenderReadiness({status:"loading"});const summary=await window.ConceptMastery.readinessOf(conceptId,{maxDepth:2}).catch(()=>({status:"unavailable"}));if(seq!==state.readinessSeq)return;if(!summary||summary.status!=="unavailable"||!state.readinessData)hiddenRenderReadiness(summary);}
...
const anchor=document.createElement("div");anchor.className="mw-anchor";const box=document.createElement("div");box.id="mw-mastery";box.className="md-typeset";const firstReadiness=mwBootstrapReadiness(conceptId);box.innerHTML=`
      <div class="mw-head">
        <div class="mw-title"><span class="mw-title-label">My understanding</span><button type="button" class="mw-level-info" aria-label="About understanding levels" aria-haspopup="dialog"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg></button></div>
        <div class="mw-head-right">
          <button type="button" class="mw-ready-chip" data-mw-ready-trigger="1" aria-haspopup="dialog" aria-expanded="false" disabled>
            ...
            <span class="mw-ready-chip-pct">${firstReadiness ? mwReadinessDisplayText(firstReadiness) : "..."}</span>
          </button>
      ...
      <div class="mw-row">
        ...
        <button type="button" class="mw-pill" data-m="1" title="Partial">
          <span class="mw-emo">${mwSvgForLevel(1, 18)}</span><span class="mw-lab">Partial</span>
        </button>
    ...
    `;const levelInfoBtn=box.querySelector(".mw-level-info");levelInfoBtn.addEventListener("click",()=>mwOpenLevelInfo(levelInfoBtn));const compact=document.createElement("div");compact.id="mw-mastery-compact";compact.className="md-typeset";compact.innerHTML=mwCompactMarkup();state.anchor=anchor;state.box=box;state.compact=compact;try{mwSyncSectionLineColor(state);}catch(_){}
...
function bindManage(scope){const manageBtn=scope?scope.querySelector("[data-mw-manage='1']"):null;if(!manageBtn||manageBtn.dataset.mwBound)return;manageBtn.dataset.mwBound="1";manageBtn.addEventListener("click",(e)=>{e.preventDefault();mwHideReadinessPopover();mwHideRecapPopover();mwOpenMasteryManager();});}
...
pop.hidden=false;pop.style.left="12px";pop.style.top="12px";pop.style.maxHeight="";const rect=anchorEl.getBoundingClientRect();const verticalRect=state.box&&state.box.contains(anchorEl)?state.box.getBoundingClientRect():rect;const pad=12;const gap=8;const vw=Math.max(1,Number(window.innerWidth)||Number(document.documentElement&&document.documentElement.clientWidth)||1);const vh=Math.max(1,Number(window.innerHeight)||Number(document.documentElement&&document.documentElement.clientHeight)||1);let topPad=pad;try{topPad=Math.max(pad,Number(mwHeaderOcclusionBottom&&mwHeaderOcclusionBottom())+8||pad);}catch(_){topPad=pad;}
const width=Math.min(pop.offsetWidth||520,Math.max(120,vw-pad*2));const preferLeft=rect.right-width;let left=Math.max(pad,Math.min(vw-width-pad,preferLeft));if(!Number.isFinite(left))left=pad;const defaultMaxHeight=430;const belowTop=Math.max(topPad,verticalRect.bottom+gap);const belowSpace=Math.max(0,vh-belowTop-pad);const aboveSpace=Math.max(0,verticalRect.top-topPad-gap);const shouldFlipAbove=belowSpace<150&&aboveSpace>belowSpace+80&&verticalRect.top>vh*0.55;let top=belowTop;let maxH=Math.max(96,Math.min(defaultMaxHeight,belowSpace||defaultMaxHeight));if(shouldFlipAbove){maxH=Math.max(96,Math.min(defaultMaxHeight,aboveSpace));const naturalH=Math.min(maxH,Math.max(96,Number(pop.scrollHeight)||Number(pop.offsetHeight)||maxH));top=Math.max(topPad,verticalRect.top-gap-naturalH);}
...
function mwApplyReadinessColor(el,data){if(!el||!el.style)return;if(!data||data.status!=='ok'||mwReadinessUnassessed(data)){el.style.removeProperty('--mw-ready-rgb');el.style.removeProperty('--mw-ready-fill');el.style.removeProperty('--mw-ready-fill-hover');el.style.removeProperty('--mw-ready-pop-fill');return;}
...
function mwRefreshReadinessChips(){const data=state.readinessData;const ok=!!(data&&data.status==="ok");const pctText=mwReadinessDisplayText(data);mwGetReadinessRoots(state).forEach((root)=>{if(!root)return;root.querySelectorAll("[data-mw-ready-trigger]").forEach((btn)=>{const pctEl=btn.querySelector(".mw-ready-chip-pct");if(pctEl)pctEl.textContent=pctText;btn.disabled=!ok;btn.setAttribute("aria-expanded",state.readinessOpen?"true":"false");mwApplyReadinessColor(btn,data);});});}
...
const seq=++state.readinessSeq;const initial=mwBootstrapReadiness(conceptId);if(initial)renderReadiness(initial);const summary=await window.ConceptMastery.readinessOf(conceptId,{maxDepth:2}).catch(()=>({status:"unavailable"}));if(seq!==state.readinessSeq)return;if(!summary||summary.status!=="unavailable"||!state.readinessData)renderReadiness(summary);}
...
pop.hidden=false;pop.style.left="12px";pop.style.top="12px";pop.style.maxHeight="";const rect=anchorEl.getBoundingClientRect();const verticalRect=state.box&&state.box.contains(anchorEl)?state.box.getBoundingClientRect():rect;const pad=12;const gap=8;const vw=Math.max(1,Number(window.innerWidth)||Number(document.documentElement&&document.documentElement.clientWidth)||1);const vh=Math.max(1,Number(window.innerHeight)||Number(document.documentElement&&document.documentElement.clientHeight)||1);let topPad=pad;try{topPad=Math.max(pad,Number(mwHeaderOcclusionBottom&&mwHeaderOcclusionBottom())+8||pad);}catch(_){topPad=pad;}
const width=Math.min(pop.offsetWidth||360,Math.max(120,vw-pad*2));const preferLeft=rect.right-width;let left=Math.max(pad,Math.min(vw-width-pad,preferLeft));if(!Number.isFinite(left))left=pad;const defaultMaxHeight=320;const belowTop=Math.max(topPad,verticalRect.bottom+gap);const belowSpace=Math.max(0,vh-belowTop-pad);const aboveSpace=Math.max(0,verticalRect.top-topPad-gap);const shouldFlipAbove=belowSpace<130&&aboveSpace>belowSpace+80&&verticalRect.top>vh*0.55;let top=belowTop;let maxH=Math.max(88,Math.min(defaultMaxHeight,belowSpace||defaultMaxHeight));if(shouldFlipAbove){maxH=Math.max(88,Math.min(defaultMaxHeight,aboveSpace));const naturalH=Math.min(maxH,Math.max(88,Number(pop.scrollHeight)||Number(pop.offsetHeight)||maxH));top=Math.max(topPad,verticalRect.top-gap-naturalH);}
pop.style.maxHeight=Math.floor(maxH)+"px";pop.style.left=Math.round(left)+"px";pop.style.top=Math.round(top)+"px";}
...
try{window.addEventListener("mk-ai-quiz-sessions-changed",syncInlineAiq);}catch(_){}
...
try{window.removeEventListener("mk-ai-quiz-sessions-changed",syncInlineAiq);}catch(_){}});}}}
...
if(mwSyncHiddenWidget(state,hasExplicitRating,rec)){mwApplyTitleState(hasExplicitRating?rec.m:null,hasExplicitRating,{aiVerified:state.aiVerificationKind});scheduleDockSync();scheduleSearchSuppressionSync();return;}
...
try{anchor.remove();}catch(_){}});window.__mw_widget_cleanup=function(){mwCloseLevelInfo();state.readinessSeq+=1;const arr=state.destroyers.slice();state.destroyers.length=0;arr.forEach((fn)=>{try{fn();}catch(_){}});state.keepCollapsedDom=false;try{box.classList.remove("mw-docked","mw-source-hidden","mw-needs-rating","mw-reduced-motion","mw-managed-to-title","mw-dismissed");}catch(_){}
...
function mwRandomSetMode(mode){mwRandomCancelAiArrival();const m=(mode==="ai"||mode==="self")?mode:"normal";try{sessionStorage.setItem(MW_RANDOM_MODE_KEY,m);if(m==="self")sessionStorage.setItem(MW_SELF_TEST_MODE_KEY,"1");else sessionStorage.removeItem(MW_SELF_TEST_MODE_KEY);if(m!=="ai")sessionStorage.removeItem(MW_RANDOM_AI_NAV_FLAG);}catch(_){}
...
function mwRandomExit(ev){mwRandomCancelAiArrival();if(ev){ev.preventDefault();ev.stopPropagation();}
...
let __mw_random_ai_arrival=null;function mwRandomCancelAiArrival(){const request=__mw_random_ai_arrival;if(request&&request.cancel)request.cancel();}
function mwRandomAiArrivalEnabled(){if(window.__mkExamMode)return false;const prefs=window.MkStartupPrefs;return!prefs||typeof prefs.isEnabled!=="function"||(prefs.isEnabled("mastery")!==false&&prefs.isEnabled("aiQuiz")!==false);}
function mwRandomAiArrivalIdentity(){try{const profile=JSON.parse(localStorage.getItem("mk_comment_profile_v1")||"{}");const key=String(profile&&(profile.accountKey||profile.account_key||profile.nameKey||profile.name_key)||"").replace(/^user:/i,"").trim().toLowerCase();const workspaces=window.MkAccountWorkspaces;const owner=workspaces&&typeof workspaces.activeOwner==="function"?String(workspaces.activeOwner()):localStorage.getItem("mk_account_workspace_active_owner_v1")||(key?"account:"+key:"guest");return JSON.stringify([owner,key,localStorage.getItem("mk_hot_visitor_id_v1")||""]);}catch(_){return"";}}
function mwRandomAutoOpenAiCheck(){const host=document.querySelector("article.md-content__inner");const path=currentRelPath();const identity=mwRandomAiArrivalIdentity();let arrival="";try{arrival=sessionStorage.getItem(MW_RANDOM_ARRIVAL_ID_KEY)||"";if(!host||!isConceptPage(path)||!mwRandomIsActiveOnCurrentPage()||!mwRandomAiArrivalEnabled()||mwRandomReadMode()!=="ai"||!arrival||sessionStorage.getItem(MW_RANDOM_AI_NAV_FLAG)!=="1"||sessionStorage.getItem(MW_RANDOM_AI_OPENED_KEY)===arrival){mwRandomCancelAiArrival();return;}}catch(_){mwRandomCancelAiArrival();return;}
const previous=__mw_random_ai_arrival;if(previous&&previous.arrival===arrival&&previous.path===path&&previous.host===host&&previous.identity===identity)return;mwRandomCancelAiArrival();const request={arrival,path,host,identity,opening:false,failed:false,timer:0,notice:null};__mw_random_ai_arrival=request;const current=()=>{try{return __mw_random_ai_arrival===request&&host.isConnected&&document.querySelector("article.md-content__inner")===host&&currentRelPath()===path&&mwRandomAiArrivalIdentity()===identity&&mwRandomAiArrivalEnabled()&&mwRandomReadMode()==="ai"&&mwRandomIsActiveOnCurrentPage()&&sessionStorage.getItem(MW_RANDOM_ARRIVAL_ID_KEY)===arrival&&sessionStorage.getItem(MW_RANDOM_AI_NAV_FLAG)==="1";}catch(_){return false;}};const stopPolling=()=>{if(request.timer)window.clearInterval(request.timer);request.timer=0;};const cancel=()=>{stopPolling();["aiq:ready","mk:feature-scripts-settled"].forEach(name=>window.removeEventListener(name,onReady));["pagehide","DOMContentSwitch","mk-account-workspace-changed"].forEach(name=>window.removeEventListener(name,cancel));if(request.notice)request.notice.remove();request.notice=null;if(__mw_random_ai_arrival===request)__mw_random_ai_arrival=null;};request.cancel=cancel;const accept=()=>{if(!current()||window.__mkAccountWorkspaceSwitching){cancel();return;}
try{sessionStorage.setItem(MW_RANDOM_AI_OPENED_KEY,arrival);sessionStorage.removeItem(MW_RANDOM_AI_NAV_FLAG);}catch(_){}
cancel();};const showRetry=(message)=>{if(!current()){cancel();return;}
if(request.notice)return;const notice=document.createElement("div");notice.id="mw-random-ai-arrival-retry";notice.className="mw-random-session-panel";const status=document.createElement("p");status.setAttribute("role","status");status.textContent=message;const retry=document.createElement("button");retry.type="button";retry.className="mw-random-session-btn";retry.textContent="Retry AI quiz";retry.addEventListener("click",()=>{if(!current()){cancel();return;}
cancel();mwRandomAutoOpenAiCheck();});const dismiss=document.createElement("button");dismiss.type="button";dismiss.className="mw-random-session-btn";dismiss.textContent="Keep reading";dismiss.addEventListener("click",()=>{if(current()){try{sessionStorage.removeItem(MW_RANDOM_AI_NAV_FLAG);}catch(_){}}
cancel();});notice.append(status,retry,document.createTextNode(" "),dismiss);const heading=host.querySelector("h1");if(heading)heading.insertAdjacentElement("afterend",notice);else host.prepend(notice);request.notice=notice;};const fail=()=>{if(!current()){cancel();return;}
request.opening=false;request.failed=true;stopPolling();showRetry("The AI quiz could not be opened. Retry, or keep reading this concept.");};const dialogOpen=api=>{try{if(typeof api.getState==="function")return!!api.getState().open;const modal=document.getElementById("aiq-modal");if(!modal||!modal.isConnected||modal.hidden||(modal.getAttribute&&modal.getAttribute("aria-hidden")==="true"))return false;const style=window.getComputedStyle?window.getComputedStyle(modal):null;return(!style||(style.display!=="none"&&style.visibility!=="hidden"&&style.visibility!=="collapse"))&&(!modal.getClientRects||modal.getClientRects().length>0);}catch(_){return false;}};const tryOpen=()=>{if(!current()){cancel();return;}
if(request.opening||request.failed||window.__mkAccountWorkspaceSwitching)return;const api=mwGetAiQuizApi();if(!api||typeof api.open!=="function")return;if(dialogOpen(api)){accept();return;}
request.opening=true;stopPolling();try{const out=mwOpenAiQuizWithHistoryRepair(api,{source:"random-ai-mode"});if(dialogOpen(api)){accept();Promise.resolve(out).catch(()=>{});return;}
Promise.resolve(out).then(ok=>{if(!current()){cancel();return;}
if(ok===true||dialogOpen(api))accept();else fail();},fail);}catch(_){fail();}};function onReady(ev){if(ev&&ev.type==="mk:feature-scripts-settled"&&(!ev.detail||ev.detail.key!=="aiQuiz"))return;tryOpen();}
["aiq:ready","mk:feature-scripts-settled"].forEach(name=>window.addEventListener(name,onReady));["pagehide","DOMContentSwitch","mk-account-workspace-changed"].forEach(name=>window.addEventListener(name,cancel));tryOpen();if(__mw_random_ai_arrival!==request||request.opening||request.failed)return;const prefs=window.MkStartupPrefs;if(prefs&&typeof prefs.ensureFeature==="function"){try{Promise.resolve(prefs.ensureFeature("aiQuiz")).then(loaded=>{if(!current()){cancel();return;}
if(loaded===false){cancel();return;}
tryOpen();},fail);}catch(_){fail();}}
if(__mw_random_ai_arrival!==request||request.opening||request.failed)return;let polls=0;request.timer=window.setInterval(()=>{tryOpen();if(++polls>=60&&current()&&!request.opening&&!request.failed){stopPolling();showRetry("The AI quiz is still loading. You can retry or keep reading.");}},250);}
function mwRandomSessionBoot(){mwRandomRenderPanel();let sessionActive=false;let aiArrival=false;try{const arrived=isConceptPage(currentRelPath())&&mwRandomIsActiveOnCurrentPage();sessionActive=arrived&&mwRandomReadCandidates().length>0;aiArrival=arrived&&mwRandomReadMode()==="ai";}catch(_){}
if(aiArrival){mwRandomInstallAiResultObserver();mwRandomAutoOpenAiCheck();}else mwRandomCancelAiArrival();if(!sessionActive){try{if(window.__mwRandomPanelObserver)window.__mwRandomPanelObserver.disconnect();}catch(_){}
window.__mwRandomPanelObserver=null;window.__mwRandomPanelObserverInstalled=false;return;}
try{if(!window.__mwRandomPanelObserverInstalled&&window.MutationObserver){window.__mwRandomPanelObserverInstalled=true;let raf=0;const obs=new MutationObserver(()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;try{mwRandomHideLegacyPanels();}catch(_){}
