...
targets.forEach((btn)=>{if(!btn)return;const label=btn.getAttribute("data-aiq-resume-result")==="1"?"Resume AI result":btn.getAttribute("data-aiq-load-failed")==="1"?"Retry AI concept check":MW_AIQ_LABEL;let wroteLabelNode=false;try{const labelNode=btn.querySelector&&btn.querySelector(".mw-aiq-btn-label, [data-aiq-label], .aiq-label");if(labelNode&&String(labelNode.textContent||"").trim()!==label){labelNode.textContent=label;wroteLabelNode=true;}else if(labelNode){wroteLabelNode=true;}}catch(_){}
...
function mwAiQuizEntryEnabled(){if(window.__mkExamMode)return false;try{const prefs=window.MkStartupPrefs;if(prefs&&typeof prefs.isEnabled==="function"&&prefs.isEnabled("aiQuiz")===false)return false;const meta=document.querySelector('meta[name="mk-concept-ai-mcq"]');if(meta&&meta.content==="false")return false;}catch(_){}
return true;}
