...
targets.forEach((btn)=>{if(!btn)return;const label=btn.getAttribute("data-aiq-coming-soon")==="1"?MW_AIQ_UNAVAILABLE_LABEL:btn.getAttribute("data-aiq-resume-result")==="1"?"Resume AI result":btn.getAttribute("data-aiq-load-failed")==="1"?"Retry AI concept check":MW_AIQ_LABEL;let wroteLabelNode=false;try{const labelNode=btn.querySelector&&btn.querySelector(".mw-aiq-btn-label, [data-aiq-label], .aiq-label");if(labelNode&&String(labelNode.textContent||"").trim()!==label){labelNode.textContent=label;wroteLabelNode=true;}else if(labelNode){wroteLabelNode=true;}}catch(_){}
...
const btn=document.createElement("button");btn.type="button";btn.className="mw-aiq-btn";btn.dataset.mwAiqFallback="1";btn.setAttribute("data-aiq-load-failed",loadFailed?"1":"0");btn.innerHTML=`${mwAiQuizIconHtml(18)}<span class="mw-aiq-btn-label">${loadFailed ? "Retry AI concept check" : MW_AIQ_LABEL}</span>`;btn.disabled=!available&&!loadFailed;btn.title=loadFailed?"Retry AI concept check":available?MW_AIQ_LABEL:unavailableLabel;btn.setAttribute("aria-label",btn.title);mwBindTouchSafeButton(btn,()=>{const api=mwGetAiQuizApi();if(!api||typeof api.open!=="function")return;try{const out=mwOpenAiQuizWithHistoryRepair(api,{source:"mastery-widget"});if(out&&typeof out.catch==="function")out.catch(()=>{});}catch(_){}});host.appendChild(btn);mwSyncAiQuizButton(btn);mwNormaliseAiQuizButtonLabel(host);return true;}
function mwOpenAiQuizFromTitleMenu(btn){if(btn&&(!mwSyncAiQuizButton(btn)||btn.disabled||btn.hidden))return;const api=mwGetAiQuizApi();if(!api||typeof api.open!=="function")return;mwCloseTitleMenu();try{const out=mwOpenAiQuizWithHistoryRepair(api,{source:"title-menu"});if(out&&typeof out.catch==="function")out.catch(()=>{});}catch(_){}}
