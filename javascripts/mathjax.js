...
function loadScriptOnce(id,src,testFn){try{if(typeof testFn==="function"&&testFn())return Promise.resolve(true);let script=document.getElementById(id);if(scriptLoadPromises.has(script))return scriptLoadPromises.get(script);const owned=!script;if(owned){script=document.createElement("script");script.id=id;script.async=true;script.defer=true;script.src=src;}
const pending=new Promise((resolve)=>{let settled=false;const finish=(ok)=>{if(settled)return;settled=true;if(!ok&&owned&&script.parentNode){try{script.parentNode.removeChild(script);}catch(_){}}
resolve(ok);};const loaded=()=>{try{finish(typeof testFn!=="function"||!!testFn());}catch(_){finish(false);}};if(owned){script.onload=loaded;script.onerror=()=>finish(false);}
else{script.addEventListener("load",loaded,{once:true});script.addEventListener("error",()=>finish(false),{once:true});}
if(owned){try{(document.head||document.documentElement).appendChild(script);}catch(_){finish(false);}}});scriptLoadPromises.set(script,pending);return pending;}catch(_){return Promise.resolve(false);}}
function ensureKatex(){if(katexReadyPromise)return katexReadyPromise;katexReadyPromise=(async()=>{ensureCss();ensureRuntimeStyle();const haveKatex=()=>!!window.katex;const haveAuto=()=>typeof window.renderMathInElement==="function";if(KATEX_JS&&!(await loadScriptOnce(KATEX_SCRIPT_ID,KATEX_JS,haveKatex))&&externalFallbackAllowed()&&KATEX_JS!==KATEX_JS_FALLBACK){await loadScriptOnce(KATEX_SCRIPT_ID+"-cdn",KATEX_JS_FALLBACK,haveKatex);}
...
const macrosReady=!MACROS_JS||await loadScriptOnce(MACROS_SCRIPT_ID,MACROS_JS,()=>!!window.MK_KATEX_MACROS);return!!(window.katex&&haveAuto()&&macrosReady);})().then((ok)=>{if(!ok)katexReadyPromise=null;return ok;},()=>{katexReadyPromise=null;return false;});return katexReadyPromise;}
