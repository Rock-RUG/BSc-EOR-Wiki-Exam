...
function hasMathText(root){try{const txt=String(root&&root.textContent||"");if(!txt)return false;return/\\\(|\\\)|\\\[|\\\]|\$\$|(^|[^\\])\$|\\begin\{|\\end\{|\\(?:frac|sum|prod|int|lim|infty|mathbb|mathcal|mathbf|boldsymbol|mathrm|operatorname|displaystyle|to|mapsto|colon|subseteq|geq|leq|alpha|beta|gamma|delta|epsilon|lambda|mu|sigma|theta|omega)\b/.test(txt);}catch(_){return false;}}
...
function typesetPromise(elements){const roots=asArray(elements&&elements.length!==0?elements:document.body);return ensureKatex().then(()=>Promise.all(roots.map(renderRootNow))).then(()=>undefined);}
function pickRenderRoot(node){try{if(!node||node.nodeType!==1)return null;if(node.matches&&node.matches(DYNAMIC_ROOT_SELECTOR))return node;if(node.closest){const close=node.closest(DYNAMIC_ROOT_SELECTOR);if(close)return close;}
