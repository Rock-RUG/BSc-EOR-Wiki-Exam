...
function applyEarlyInterfaceTheme(){try{if(window.MkThemeFirstPaint&&typeof window.MkThemeFirstPaint.apply==="function"){window.MkThemeFirstPaint.apply();return;}
installEarlyInterfaceThemeStyle();const scheme=inferMaterialSchemeEarly();const accountOff=html.classList.contains("mk-startup-account-off")||html.getAttribute("data-mk-startup-account")==="off";if(accountOff){ACCOUNT_THEME_ATTRS.forEach((attr)=>html.removeAttribute(attr));ACCOUNT_THEME_VARS.forEach((name)=>html.style.removeProperty(name));html.setAttribute("data-mk-color-scheme",scheme==="slate"?"slate":"default");return;}
...
function isWithinInputWatch(){return safeNow()<currentGuardUntil;}
...
scrollGuardExpiryTimer=0;}
const remaining=Math.max(0,currentGuardUntil-safeNow());scrollGuardExpiryTimer=window.setTimeout(function(){scrollGuardExpiryTimer=0;if(isWithinInputWatch()){scheduleScrollGuardExpiry();return;}
