...
function openLocalActivity(kind){const type=kind||"visits";try{closeOpenPanel({returnFocus:true});if(window.MkLocalActivity&&typeof window.MkLocalActivity.open==="function"){window.MkLocalActivity.open(type);closeOpenPanel();return;}
...
if(data.random||data.custom){const randomItems=[];if(data.random)randomItems.push({label:"Random Concept",href:makeRandomModeHref(data.random.href,"unvisited"),guestAction:"random",randomDirect:true});if(data.random)randomItems.push({label:"Random Route",href:makeRandomRouteHref(data.random.href),guestAction:"guided_study",randomDirect:true});if(data.random&&!window.__mkExamMode)randomItems.push({label:"Random AI Quiz",href:makeRandomModeHref(data.random.href,"ai-untested"),guestAction:"random",randomDirect:true});if(data.custom)randomItems.push({label:"Concept Finder",href:data.custom.href,guestAction:"concept_finder"});else if(data.course)randomItems.push({label:"Course Random",href:data.course.href,guestAction:"random"});const randomPanel=ensurePanel(IDS.randomPanel);fillPanel(randomPanel,randomItems);const randomBtn=makeTrigger("Explore",IDS.randomPanel);setTriggerActive(randomBtn,randomItems);bindTrigger(randomBtn,randomPanel);right.appendChild(randomBtn);}
