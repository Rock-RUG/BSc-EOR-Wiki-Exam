...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(m==="spenders"){return p==="today"?"Daily spent":p==="7d"?"Weekly spent":p==="30d"?"Monthly spent":"Total spent";}
if(m==="eor_rating")return"EOR Rating";if(m==="popular"){if(p==="today")return"Daily score";if(p==="7d")return"Weekly score";if(p==="30d")return"Monthly score";return"Total score";}
...
function metricValue(item,metric){if(!item)return 0;if(metric==="spenders")return`${formatTrendingNumber(item.spent != null ? item.spent : item.score)} EOR Bits`;if(metric==="eor_rating")return formatTrendingInteger(item.rating!=null?item.rating:item.score);if(metric==="popular"||metric==="lively"){const v=item.score!=null?item.score:item.count;return Number.isInteger(Number(v))?String(Number(v)):String(Number(v||0).toFixed(1)).replace(/\.0$/,"");}
...
async function fetchHot({metric,period,limit,offset}){if(metric==="users")return fetchUserRankings({period,limit,offset});const cacheKey=hotCacheKey(metric,period,limit,offset);const request={};latestRankingRequests.set(cacheKey,request);const url=new URL(API_BASE+(metric==="eor_rating"?"/arena/rating/leaderboard":"/hot"));if(metric!=="eor_rating"){url.searchParams.set("metric",metric);url.searchParams.set("period",period);}
url.searchParams.set("limit",String(limit));url.searchParams.set("offset",String(offset));url.searchParams.set("fresh",String(Date.now()));url.searchParams.set("r",Math.random().toString(36).slice(2));const resp=await fetch(url.toString(),{cache:"no-store",headers:{"Cache-Control":"no-cache","Pragma":"no-cache"}}).catch(()=>null);const data=resp&&resp.ok?await resp.json().catch(()=>null):null;if(data&&data.ok!==false&&Array.isArray(data.items)&&(!isMilestoneMetric(metric)||(data.metric===metric&&data.period_used===period))){const fresh={items:data.items,total:typeof data.total==="number"?data.total:data.items.length,fetchedAt:Date.now(),};if(latestRankingRequests.get(cacheKey)===request)writeHotCache(metric,period,limit,offset,fresh);return fresh;}
...
function updateMetaHead(){if(metaHeadEl)metaHeadEl.textContent=periodMetricLabel(state.period,metric);}
function setActiveTab(){tabs.querySelectorAll(".trending-tab").forEach((btn)=>{btn.classList.toggle("is-active",btn.dataset.period===state.period);btn.setAttribute("aria-pressed",btn.dataset.period===state.period?"true":"false");});updateMetaHead();}
