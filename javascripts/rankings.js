...
if(m==="eor_rating"){return p==="today"?"Change today":p==="7d"?"Change this week":p==="30d"?"Change in 30 days":"EOR Rating";}
...
const[lectureMap,titleMap]=items.length&&!isTrendingUserMetric(metric)?await Promise.all([loadLectureMapOnce().catch(()=>null),loadTitleMapOnce().catch(()=>null),]):[null,null];if(token!==loadToken)return;list.innerHTML="";if(!items.length){const emptyCopy=period==="honors"?fetchStatus.refreshFailed?"Monthly honours could not load. Use Refresh to try again.":"No public monthly honours to display yet. Only champions with public rankings appear here.":fetchStatus&&fetchStatus.refreshFailed?"Could not refresh ranking":metric==="eor_rating"?(period==="all"?"No public EOR Ratings yet. A rating appears after your first rated answer, duel or weekend puzzle.":"No rated results in this period yet."):"No data yet";list.appendChild(el("li","trending-empty",emptyCopy));}else{items.forEach((it,idx)=>{if(period==="honors"){list.appendChild(monthlyHonorRow(it,metric));return;}
