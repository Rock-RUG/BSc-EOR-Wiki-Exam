...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(p==="honors")return m==="spenders"?"Spent that month":m==="eor_rating"?"Rating gained":"XP that month";if(m==="spenders"){return p==="today"?"Daily spent":p==="7d"?"Weekly spent":p==="30d"?"Monthly spent":"Total spent";}
...
function monthlyHonorGapRow(outcome){const row=el("li","trending-item trending-user-item trending-honor-item trending-honor-gap");row.dataset.month=outcome.month;row.appendChild(monthlyHonorStamp(outcome.month));const profile=el("div","trending-user-profile");const placeholder=el("span","trending-honor-gap-avatar");placeholder.setAttribute("aria-hidden","true");profile.appendChild(placeholder);const main=el("div","trending-user-main");main.appendChild(el("span","trending-honor-gap-title","No champion"));main.appendChild(el("span","trending-honor-gap-reason",Number(outcome.learners)>0?"Nobody's rating rose this month.":"No rated results this month."));profile.appendChild(main);row.appendChild(profile);row.appendChild(el("span","trending-user-period-xp trending-honor-value","—"));return row;}
