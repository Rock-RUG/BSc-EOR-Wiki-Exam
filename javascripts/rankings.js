...
.trending-honor-gaps{ list-style:none; margin:.4rem 0 0; padding:.55rem .1rem 0; font-size:.68rem; line-height:1.5; opacity:.72; border-top:1px dashed color-mix(in srgb,var(--md-default-fg-color) 14%,transparent); }
...
function boardNumber(item,metric){if(!item)return null;if(metric==="eor_rating")return Number(item.periodGain!=null?item.periodGain:item.rating);if(metric==="spenders")return Number(item.spent!=null?item.spent:item.score);if(metric==="users")return Number(userPeriodXp(item));return null;}
...
const CONCEPT_TRENDING_METRICS=[{key:"views",title:"Most viewed"},{key:"popular",title:"Most popular"},{key:"tested",title:"Most tested"},];const ACCOUNT_TRENDING_METRICS=[{key:"spenders",title:"Biggest Spenders"},{key:"eor_rating",title:"EOR Rating"},{key:"users",title:"Most active users"},];const TRENDING_METRICS=CONCEPT_TRENDING_METRICS.concat(ACCOUNT_TRENDING_METRICS);function visibleTrendingMetrics(){return accountFeatureEnabled()?TRENDING_METRICS:CONCEPT_TRENDING_METRICS;}
...
const[lectureMap,titleMap]=items.length&&!isTrendingUserMetric(metric)?await Promise.all([loadLectureMapOnce().catch(()=>null),loadTitleMapOnce().catch(()=>null),]):[null,null];if(token!==loadToken)return;list.innerHTML="";const gapNote=period==="honors"&&honorGaps.length?el("li","trending-honor-gaps",`No champion: ${honorGaps.map(o => `${monthlyHonorMonthLabel(o.month)} (${o.learners?"nobody's rating rose":"no rated results"})`).join(" · ")}.`):null;if(!items.length){const emptyCopy=period==="honors"?fetchStatus.refreshFailed?"Monthly honours could not load. Use Refresh to try again.":"No public monthly honours to display yet. Only champions with public rankings appear here.":fetchStatus&&fetchStatus.refreshFailed?"Could not refresh ranking":metric==="eor_rating"?(period==="all"?"No public EOR Ratings yet. A rating appears after your first rated answer, duel or weekend puzzle.":"No rated results in this period yet."):"No data yet";list.appendChild(el("li","trending-empty",emptyCopy));if(gapNote)list.appendChild(gapNote);}else{items.forEach((it,idx)=>{if(period==="honors"){list.appendChild(monthlyHonorRow(it,metric));return;}
