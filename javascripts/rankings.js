...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(p==="honors")return m==="spenders"?"Spent that month":m==="eor_rating"?"Rating gained":"XP that month";if(m==="spenders"){return p==="today"?"Daily spent":p==="7d"?"7-day spent":p==="30d"?"30-day spent":"Total spent";}
...
const CONCEPT_TRENDING_METRICS=[{key:"views",title:"Most viewed"},{key:"popular",title:"Most popular"},{key:"tested",title:"Most tested"},];const ACCOUNT_TRENDING_METRICS=[{key:"users",title:"Most active users"},{key:"eor_rating",title:"EOR Rating"},{key:"spenders",title:"Biggest Spenders"},];const TRENDING_METRICS=CONCEPT_TRENDING_METRICS.concat(ACCOUNT_TRENDING_METRICS);function visibleTrendingMetrics(){return accountFeatureEnabled()?TRENDING_METRICS:CONCEPT_TRENDING_METRICS;}
