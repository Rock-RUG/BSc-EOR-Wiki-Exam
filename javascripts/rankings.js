...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(p==="honors")return m==="spenders"?"Spent that month":m==="eor_rating"?"Rating gained":m==="verified_exploration"?"Exploration points":"XP that month";if(m==="verified_exploration")return"Exploration points this month";if(m==="collection_value")return p==="all"?"Collection value":p==="today"?"Change today":p==="7d"?"Change in 7 days":"Change in 30 days";if(m==="spenders"){return p==="today"?"Daily spent":p==="7d"?"7-day spent":p==="30d"?"30-day spent":"Total spent";}
...
function explorationSecondaryText(item){const concepts=Math.max(0,Math.round(Number(item&&item.concepts)||0));const rechecked=Math.max(0,Math.round(Number(item&&item.rechecked)||0));const courses=Math.max(0,Math.round(Number(item&&item.courses)||0));const parts=[`${formatTrendingNumber(concepts)} concept${concepts === 1 ? "" : "s"}`];if(rechecked)parts.push(`${formatTrendingNumber(rechecked)} rechecked`);if(courses)parts.push(`${formatTrendingNumber(courses)} course${courses === 1 ? "" : "s"}`);return parts.join(" · ");}
...
if(metric==="verified_exploration"){return`${formatTrendingNumber(item.score)} pts`;}
...
function isTrendingUserMetric(metric){return metric==="users"||metric==="quiz_correct"||metric==="verified_exploration"||isMilestoneMetric(metric);}
...
async function fetchHot({metric,period,limit,offset}){if(metric==="users")return fetchUserRankings({period,limit,offset});const cacheKey=hotCacheKey(metric,period,limit,offset);const request={};latestRankingRequests.set(cacheKey,request);const url=new URL(API_BASE+(metric==="eor_rating"?"/arena/rating/leaderboard":metric==="verified_exploration"?"/arena/exploration/board":"/hot"));if(metric!=="eor_rating"&&metric!=="verified_exploration")url.searchParams.set("metric",metric);url.searchParams.set("period",period);url.searchParams.set("limit",String(limit));url.searchParams.set("offset",String(offset));url.searchParams.set("fresh",String(Date.now()));url.searchParams.set("r",Math.random().toString(36).slice(2));const resp=await fetch(url.toString(),{cache:"no-store",headers:{"Cache-Control":"no-cache","Pragma":"no-cache"}}).catch(()=>null);const data=resp&&resp.ok?await resp.json().catch(()=>null):null;if(metric==="eor_rating"&&data&&Object.prototype.hasOwnProperty.call(data,"prizes"))applyPrizeTeaser(data.prizes);if(data&&data.ok!==false&&Array.isArray(data.items)&&(!isMilestoneMetric(metric)||(data.metric===metric&&data.period_used===period))){const fresh={items:data.items,historyComplete:data.historyComplete,trackedFrom:data.trackedFrom,total:typeof data.total==="number"?data.total:data.items.length,fetchedAt:Date.now(),};if(latestRankingRequests.get(cacheKey)===request)writeHotCache(metric,period,limit,offset,fresh);return fresh;}
...
function monthlyHonorValue(item,metric){const meta=el("span","trending-user-period-xp trending-honor-value");if(metric==="eor_rating")meta.innerHTML=trendingValueWithUnit(formatRatingGain(item.score)," gained");else if(metric==="verified_exploration")meta.innerHTML=trendingValueWithUnit(formatTrendingNumber(item.score)," pts");else if(metric==="spenders")meta.innerHTML=trendingValueWithUnit(formatTrendingNumber(item.score)," EOR Bits");else meta.innerHTML=trendingValueWithUnit(formatTrendingNumber(item.score)," XP");return meta;}
...
function monthlyHonorGapReason(outcome,metric){const anyone=Number(outcome&&outcome.learners)>0;if(metric==="eor_rating")return anyone?"Nobody's rating rose this month.":"No rated results this month.";if(metric==="verified_exploration")return anyone?"No eligible exploration this month.":"Nobody passed a concept check this month.";if(metric==="spenders")return anyone?"No eligible spending this month.":"Nobody spent EOR Bits this month.";return anyone?"No eligible XP this month.":"Nobody earned XP this month.";}
...
const CONCEPT_TRENDING_METRICS=[{key:"views",title:"Most viewed"},{key:"popular",title:"Most popular"},{key:"tested",title:"Most tested"},];const ACCOUNT_TRENDING_METRICS=[{key:"users",title:"Most active users"},{key:"eor_rating",title:"EOR Rating"},{key:"verified_exploration",title:"Verified exploration"},{key:"collection_value",title:"Collection Value"},];const TRENDING_METRICS=CONCEPT_TRENDING_METRICS.concat(ACCOUNT_TRENDING_METRICS);function visibleTrendingMetrics(){return accountFeatureEnabled()?TRENDING_METRICS:CONCEPT_TRENDING_METRICS;}
function normaliseTrendingMetric(metric){let key=String(metric||"").toLowerCase();if(key==="spenders"||key==="biggest_spenders")key="collection_value";if(key==="comments"||key==="buzz"||key==="buzzing"||key==="liveliness")key="lively";if(key==="saved_pages"||key==="saves"||key==="most_saved")key="saved";if(key==="most_tested"||key==="ai_quiz_tested"||key==="quiz_tested"||key==="ai_quiz_sessions")key="tested";if(["exploration","explorers","verified_explorers","explored","map_explorers"].includes(key))key="verified_exploration";if(["quiz_correct","ai_quiz_correct","aiquiz_correct","quiz_correct_answers","ai_quiz_correct_answers","mastery_explorers","rating","elo"].includes(key))key="eor_rating";return visibleTrendingMetrics().some((m)=>m.key===key)?key:"views";}
