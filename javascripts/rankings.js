...
.trending-block[data-metric="quiz_correct"]{
...
.trending-block[data-metric="quiz_correct"] .trending-list{
...
.trending-block[data-metric="quiz_correct"] .trending-block-header{
  display:grid;
...
.trending-block[data-metric="quiz_correct"] .trending-tabs{
  min-width:0;
}
.trending-block[data-metric="users"] .trending-metahead,
.trending-block[data-metric="quiz_correct"] .trending-metahead{
  justify-self:end;
  text-align:right;
...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(m==="popular"){if(p==="today")return"Daily score";if(p==="7d")return"Weekly score";if(p==="30d")return"Monthly score";return"Total score";}
...
function metricValue(item,metric){if(!item)return 0;if(metric==="popular"||metric==="lively"){const v=item.score!=null?item.score:item.count;return Number.isInteger(Number(v))?String(Number(v)):String(Number(v||0).toFixed(1)).replace(/\.0$/,"");}
...
function isTrendingUserMetric(metric){return metric==="users"||metric==="quiz_correct";}
...
function userPeriodXp(item){const v=firstDefinedValue(item,["periodScore","periodXp","earnedXp","earnedXP","score","count","xp"]);const n=Number(v||0);return Number.isFinite(n)?n:0;}
...
function openTrendingPublicProfile(input){try{if(!window.MkLocalActivity||typeof window.MkLocalActivity.openPublicProfile!=="function")return;const item=input&&typeof input==="object"?input:null;const name=item?userDisplayName(item):String(input||"");const accountKey=item?String(item.accountKey||item.account_key||"").trim():"";let selfPreview=false;try{const me=readTrendingLocalProfile();const meKey=String(me&&me.accountKey||"").trim().toLowerCase();selfPreview=!!(accountKey&&meKey&&accountKey.toLowerCase()===meKey);}catch(_){}
const payload=item?{accountKey,account_key:accountKey,selfPreview,source:"rankings",rankingProfile:item?{accountKey,name,avatar:userAvatarValue(item),avatarFrame:userAvatarFrameValue(item),bio:userIntroText(item),intro:userIntroText(item),equippedCosmetics:item.equippedCosmetics||{},rankingEffect:item.rankingEffect||""}:null,rankingXp:{totalXp:formatTrendingNumber(userTotalXp(item)),periodXp:formatTrendingNumber(userPeriodXp(item)),level:userLevel(item),progressPct:userLevelProgressPct(item,userTotalXp(item),userLevel(item)),accountKey,source:"rankings"}}:null;window.MkLocalActivity.openPublicProfile(name||accountKey||"",payload||undefined);}catch(_){}}
function getSiteRootUrl(){const script=document.querySelector('script[src*="assets/javascripts/bundle"]');const link=document.querySelector('link[href*="assets/stylesheets/main"]')||document.querySelector('link[href*="assets/stylesheets"]');const attr=script?script.getAttribute("src"):(link?link.getAttribute("href"):null);const assetUrl=attr?new URL(attr,document.baseURI):new URL(document.baseURI);const p=assetUrl.pathname;const idx=p.indexOf("/assets/");if(idx>=0)return assetUrl.origin+p.slice(0,idx+1);const base=new URL(document.baseURI);if(!base.pathname.endsWith("/"))base.pathname+="/";return base.origin+base.pathname;}
function safePath(loc){const s0=String(loc||"");return(s0.split("#")[0]||s0).replace(/^\/+/,"");}
function asStringList(x){if(!x)return[];if(Array.isArray(x))return x.map(String).filter(Boolean);if(typeof x==="string")return[x];return[];}
let __indexDocsPromise=null;function loadIndexDocsOnce(){if(__indexDocsPromise)return __indexDocsPromise;__indexDocsPromise=(async()=>{const root=getSiteRootUrl();const url=new URL("search/search_index.json",root).toString();const j=await __mkFetchSearchIndex(url,{cache:"no-cache"}).catch(()=>null);return j&&Array.isArray(j.docs)?j.docs:[];})();return __indexDocsPromise;}
let __validPathSetPromise=null;function loadValidPathSetOnce(){if(__validPathSetPromise)return __validPathSetPromise;__validPathSetPromise=(async()=>{const docs=await loadIndexDocsOnce().catch(()=>[]);const set=new Set();for(const d of docs){const loc=safePath(d&&d.location);if(loc)set.add(loc);}
return set;})();return __validPathSetPromise;}
let __titleMapPromise=null;function loadTitleMapOnce(){if(__titleMapPromise)return __titleMapPromise;__titleMapPromise=(async()=>{const docs=await loadIndexDocsOnce().catch(()=>[]);const map=new Map();for(const d of docs){const loc=safePath(d&&d.location);if(!loc)continue;const title=cleanTitle(d&&d.title);if(!title||titleLooksLikePathForTrending(title))continue;const key=titleLookupKey(loc);if(key&&!map.has(key))map.set(key,title);if(key.endsWith(".html"))map.set(key.slice(0,-5),title);else map.set(key+".html",title);}
return map;})();return __titleMapPromise;}
...
function rankingMemoryCacheUsable(metric,cached,nowTs){if(!cached)return false;if(!isTrendingUserMetric(metric))return true;const status=cached.status||{};if(status.stale||status.refreshFailed)return false;return rankingCacheTimestampFresh(cached.fetchedAt,USER_RANKING_MEMORY_TTL_MS,nowTs);}
...
function writeUserBoardCache(period,limit,offset,data){try{if(!data||!Array.isArray(data.items)||!data.items.length)return;const items=dedupeUserRankingItems(data.items);if(!items.length)return;localStorage.setItem(userBoardCacheKey(period,limit,offset),JSON.stringify({ts:Date.now(),data:{items,total:data.total||items.length}}));}catch(_){}}
...
async function fetchHot({metric,period,limit,offset}){if(metric==="users")return fetchUserRankings({period,limit,offset});const url=new URL(API_BASE+"/hot");url.searchParams.set("metric",metric);url.searchParams.set("period",period);url.searchParams.set("limit",String(limit));url.searchParams.set("offset",String(offset));url.searchParams.set("fresh",String(Date.now()));url.searchParams.set("r",Math.random().toString(36).slice(2));const resp=await fetch(url.toString(),{cache:"no-store",headers:{"Cache-Control":"no-cache","Pragma":"no-cache"}}).catch(()=>null);const data=resp&&resp.ok?await resp.json().catch(()=>null):null;if(data&&Array.isArray(data.items)){const fresh={items:data.items,total:typeof data.total==="number"?data.total:data.items.length,fetchedAt:Date.now(),};writeHotCache(metric,period,limit,offset,fresh);return fresh;}
...
let __lectureMapPromise=null;function loadLectureMapOnce(){if(__lectureMapPromise)return __lectureMapPromise;__lectureMapPromise=(async()=>{const docs=await loadIndexDocsOnce().catch(()=>[]);const map=new Map();for(const d of docs){const loc=safePath(d&&d.location);if(!loc||map.has(loc))continue;const info=unitInfoFromTags(getTagsFromDoc(d));if(info&&info.unitNum)map.set(loc,info);}
return map;})();return __lectureMapPromise;}
...
const CONCEPT_TRENDING_METRICS=[{key:"views",title:"Most viewed"},{key:"popular",title:"Most popular"},{key:"lively",title:"Most lively"},{key:"saved",title:"Most saved"},{key:"tested",title:"Most tested"},];const ACCOUNT_TRENDING_METRICS=[{key:"users",title:"Most active users"},{key:"quiz_correct",title:"AI Quiz Lovers"},];const TRENDING_METRICS=CONCEPT_TRENDING_METRICS.concat(ACCOUNT_TRENDING_METRICS);function visibleTrendingMetrics(){return accountFeatureEnabled()?TRENDING_METRICS:CONCEPT_TRENDING_METRICS;}
...
let __allTimeValidPromise=null;async function loadAllTimeValidItems(options){if(options&&options.forceFresh)__allTimeValidPromise=null;if(__allTimeValidPromise)return __allTimeValidPromise;__allTimeValidPromise=(async()=>{const validSet=await loadValidPathSetOnce().catch(()=>null);const out=[];const seen=new Set();const CHUNK=50;let offset=0;let total=Infinity;let guard=0;while(offset<total&&guard<200){guard++;const r=await fetchHot({metric,period:"all",limit:CHUNK,offset}).catch(()=>({items:[],total:0,refreshFailed:true,stale:true,}));if(options&&typeof options.onResult==="function"){try{options.onResult(r);}catch(_){}}
const chunk=Array.isArray(r.items)?r.items:[];if(typeof r.total==="number"&&r.total>0)total=r.total;if(!chunk.length)break;for(const it of chunk){const p=isTrendingUserMetric(metric)?String((it&&(it.accountKey||it.name||it.path))||""):safePath(it&&it.path);if(!p)continue;if(!isTrendingUserMetric(metric)){if(isUtilityPath(p))continue;if(!isExistingPagePath(p,validSet))continue;}
if(seen.has(p))continue;seen.add(p);out.push(it);if(out.length>=ALL_TIME_CAP)return out;}
offset+=chunk.length;}
return out;})();return __allTimeValidPromise;}
async function load(options){const token=++loadToken;const loadOptions=options&&typeof options==="object"?options:{};const forceFresh=!!loadOptions.forceFresh;const periodConfig=PERIODS.find((x)=>x.key===state.period)||PERIODS[1];const limit=periodConfig.limit;const cachedCandidate=periodCache.get(state.period)||null;const cached=rankingMemoryCacheUsable(metric,cachedCandidate,Date.now())?cachedCandidate:null;if(cachedCandidate&&!cached){periodCache.delete(state.period);if(state.period==="all")__allTimeValidPromise=null;}
...
let items=[];let fetchedAt=cached?cached.fetchedAt:0;const usedCache=!!cached&&!forceFresh;let fetchStatus=cached&&cached.status?cached.status:{};if(state.period==="all"){let allItems;if(cached&&!forceFresh){allItems=Array.isArray(cached.allItems)?cached.allItems:[];}else{let fetchOk=true;allItems=await loadAllTimeValidItems({forceFresh,onResult:(r)=>{if(r&&(r.refreshFailed||r.stale||r.cached)){fetchStatus={refreshFailed:!!r.refreshFailed,stale:!!r.stale,cached:!!r.cached};if(r.cachedAt)fetchedAt=Number(r.cachedAt||0)||fetchedAt;}}}).catch(()=>{fetchOk=false;fetchStatus={refreshFailed:true,stale:true};return[];});const cleanAllItems=Array.isArray(allItems)?allItems:[];if(!fetchedAt&&!(fetchStatus&&fetchStatus.refreshFailed&&!cleanAllItems.length))fetchedAt=Date.now();if(fetchOk&&!(fetchStatus&&fetchStatus.refreshFailed&&!Array.isArray(allItems))){if(!(fetchStatus&&fetchStatus.refreshFailed&&!cleanAllItems.length))periodCache.set(state.period,{fetchedAt,allItems:cleanAllItems,status:fetchStatus});}}
const patchedAllItems=dedupeUserRankingItems(metric==="users"?patchCurrentUserRankingItems(Array.isArray(allItems)?allItems:[],state.period,{preferLocalOwnScore:!!(fetchStatus&&(fetchStatus.refreshFailed||fetchStatus.stale))}):(Array.isArray(allItems)?allItems:[])).slice(0,ALL_TIME_CAP);state.total=Math.min(ALL_TIME_CAP,patchedAllItems.length);if(state.offset>=state.total)state.offset=Math.max(0,state.total-limit);items=patchedAllItems.slice(state.offset,state.offset+limit);}else{let rawItems;if(cached&&!forceFresh){rawItems=Array.isArray(cached.rawItems)?cached.rawItems:[];}else{const oversample=Math.max(limit,60);let fetchOk=true;const r=await fetchHot({metric,period:state.period,limit:oversample,offset:0,}).catch(()=>{fetchOk=false;return{items:[],total:0,refreshFailed:true,stale:true};});rawItems=Array.isArray(r.items)?r.items:[];fetchStatus={refreshFailed:!!(r&&r.refreshFailed),stale:!!(r&&r.stale),cached:!!(r&&r.cached)};fetchedAt=Number(r&&(r.fetchedAt||r.cachedAt)||0)||0;if(!fetchedAt&&!(fetchStatus.refreshFailed&&!rawItems.length))fetchedAt=Date.now();if(fetchOk&&!(r&&r.stale&&!rawItems.length))periodCache.set(state.period,{fetchedAt,rawItems,status:fetchStatus});}
const validSet=await loadValidPathSetOnce().catch(()=>null);items=dedupeUserRankingItems(metric==="users"?patchCurrentUserRankingItems(rawItems,state.period,{preferLocalOwnScore:!!(fetchStatus&&(fetchStatus.refreshFailed||fetchStatus.stale))}):rawItems).filter((it)=>{if(isTrendingUserMetric(metric))return!!(it&&(it.name||it.title||it.username||it.displayName));const p=safePath(it&&it.path);if(!p)return false;if(isUtilityPath(p))return false;if(!isExistingPagePath(p,validSet))return false;return true;}).slice(0,limit);state.total=items.length;state.offset=0;}
const[lectureMap,titleMap]=items.length?await Promise.all([loadLectureMapOnce().catch(()=>null),loadTitleMapOnce().catch(()=>null),]):[null,null];if(token!==loadToken)return;list.innerHTML="";if(!items.length){list.appendChild(el("li","trending-empty",fetchStatus&&fetchStatus.refreshFailed?"Could not refresh ranking":"No data yet"));}else{items.forEach((it,idx)=>{const li=el("li",isTrendingUserMetric(metric)?"trending-item trending-user-item":"trending-item");if(isTrendingUserMetric(metric)){const fx=String((it&&(it.rankingEffect||(it.equippedCosmetics&&it.equippedCosmetics.ranking_effect)))||"").trim();if(fx)li.setAttribute("data-ranking-effect",fx);try{const me=readTrendingLocalProfile();const meKey=String(me.accountKey||"").trim().toLowerCase();if(meKey&&String(it&&(it.accountKey||it.account_key||"")||"").trim().toLowerCase()===meKey){li.classList.add("mk-trending-current-user");li.setAttribute("data-current-user","true");}}catch(_){}}
const rank=el("span","trending-rank",String(state.offset+idx+1));li.appendChild(rank);if(isTrendingUserMetric(metric)){const name=userDisplayName(it);const level=userLevel(it);const totalXp=userTotalXp(it);const intro=userIntroText(it);const levelPct=formatTrendingPctStyle(userLevelProgressPct(it,totalXp,level));const profile=el("div","trending-user-profile");profile.innerHTML=`${trendingUserAvatarHtml(it, name)}<div class="trending-user-main"><div class="trending-user-name-row"><a href="#" class="trending-user-name">${escapeTrendingHtml(name)}</a><span class="trending-user-level" title="${escapeTrendingHtml(levelPct)}% complete in this level"><span class="trending-user-level-fill" style="width:${levelPct}%"></span><strong>Lv. ${escapeTrendingHtml(level)}</strong></span><span class="trending-user-total-xp" title="Total experience">Total XP ${escapeTrendingHtml(formatTrendingXp(totalXp))}</span></div><div class="trending-user-intro${intro ? "" : " is-empty"}">${escapeTrendingHtml(intro || (it.profileIntroVisibility === "private" ? "Profile intro is private." : "No public profile intro."))}</div></div>`;const open=(ev)=>{ev.preventDefault();openTrendingPublicProfile(it);};const nameLink=profile.querySelector(".trending-user-name");if(nameLink)nameLink.addEventListener("click",open);profile.addEventListener("dblclick",open);li.appendChild(profile);const meta=el("span",metric==="quiz_correct"?"trending-user-period-xp trending-quiz-correct-score":"trending-user-period-xp");if(metric==="quiz_correct"){meta.innerHTML=`${escapeTrendingHtml(formatQuizCorrectMetric(it))}<span class="trending-quiz-wrong">${escapeTrendingHtml(formatQuizWrongMetric(it))}</span>`;}else{meta.textContent=formatTrendingXp(userPeriodXp(it));}
