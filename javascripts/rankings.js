...
function periodMetricLabel(period,metric){const m=String(metric||"views");const p=String(period||"7d");if(p==="honors")return"";if(m==="spenders"){return p==="today"?"Daily spent":p==="7d"?"Weekly spent":p==="30d"?"Monthly spent":"Total spent";}
if(m==="eor_rating"){return p==="today"?"Gained today":p==="7d"?"Gained this week":p==="30d"?"Gained in 30 days":"EOR Rating";}
if(m==="popular"){if(p==="today")return"Daily score";if(p==="7d")return"Weekly score";if(p==="30d")return"Monthly score";return"Total score";}
...
function metricValue(item,metric){if(!item)return 0;if(metric==="spenders")return`${formatTrendingNumber(item.spent != null ? item.spent : item.score)} EOR Bits`;if(metric==="eor_rating"){if(item.periodGain!=null)return formatRatingGain(item.periodGain);return formatTrendingInteger(item.rating!=null?item.rating:item.score);}
...
function isMilestoneMetric(metric){return metric==="spenders"||metric==="eor_rating";}
function formatRatingChange(value){const n=Math.round(Number(value)||0);return n>0?`+${n} this week`:n<0?`−${Math.abs(n)} this week`:"No change this week";}
function formatRatingGain(value){const n=Math.round(Number(value)||0);return n>0?`+${n}`:n<0?`−${Math.abs(n)}`:"0";}
function ratingSecondaryLine(item){if(item&&item.periodGain!=null)return`Rating ${formatTrendingInteger(item.rating)}`;return formatRatingChange(item&&item.ratingChange7d);}
...
async function fetchHot({metric,period,limit,offset}){if(metric==="users")return fetchUserRankings({period,limit,offset});const cacheKey=hotCacheKey(metric,period,limit,offset);const request={};latestRankingRequests.set(cacheKey,request);const url=new URL(API_BASE+(metric==="eor_rating"?"/arena/rating/leaderboard":"/hot"));if(metric!=="eor_rating")url.searchParams.set("metric",metric);url.searchParams.set("period",period);url.searchParams.set("limit",String(limit));url.searchParams.set("offset",String(offset));url.searchParams.set("fresh",String(Date.now()));url.searchParams.set("r",Math.random().toString(36).slice(2));const resp=await fetch(url.toString(),{cache:"no-store",headers:{"Cache-Control":"no-cache","Pragma":"no-cache"}}).catch(()=>null);const data=resp&&resp.ok?await resp.json().catch(()=>null):null;if(data&&data.ok!==false&&Array.isArray(data.items)&&(!isMilestoneMetric(metric)||(data.metric===metric&&data.period_used===period))){const fresh={items:data.items,total:typeof data.total==="number"?data.total:data.items.length,fetchedAt:Date.now(),};if(latestRankingRequests.get(cacheKey)===request)writeHotCache(metric,period,limit,offset,fresh);return fresh;}
...
return{items:data.entries,total:data.total,firstEligibleMonth:/^\d{4}-(0[1-9]|1[0-2])$/.test(String(data.firstEligibleMonth||""))?data.firstEligibleMonth:"",historyFirstMonth:/^\d{4}-(0[1-9]|1[0-2])$/.test(String(data.historyFirstMonth||""))?data.historyFirstMonth:"",graceHours:Number.isFinite(data.graceHours)&&data.graceHours>=0?data.graceHours:null,fetchedAt:Date.now(),};}
...
function monthlyHonorRow(item,metric){const row=el("li","trending-item trending-user-item trending-honor-item");row.dataset.month=item.month;const stamp=el("div","trending-rank trending-honor-stamp");const trophy=el("span","trending-honor-trophy");trophy.setAttribute("aria-hidden","true");trophy.innerHTML='<svg viewBox="0 0 48 48" focusable="false" aria-hidden="true"><path class="trending-honor-trophy-handle" d="M13 10H6v7c0 8 6 12 13 12M35 10h7v7c0 8-6 12-13 12"/><path class="trending-honor-trophy-cup" d="M13 7h22v11c0 9-5 14-11 14S13 27 13 18Z"/><path class="trending-honor-trophy-base" d="M21 31h6v7h7v5H14v-5h7Z"/><path class="trending-honor-trophy-star" d="m24 12 2.1 4.3 4.7.7-3.4 3.3.8 4.7-4.2-2.2-4.2 2.2.8-4.7-3.4-3.3 4.7-.7Z"/></svg>';stamp.appendChild(trophy);const month=el("time","trending-honor-month",monthlyHonorMonthLabel(item.month));month.setAttribute("datetime",item.month);stamp.appendChild(month);row.appendChild(stamp);const profile=el("div","trending-user-profile");const avatar=el("div","trending-honor-avatar");avatar.innerHTML=trendingUserAvatarHtml(item,item.name);profile.appendChild(avatar);const main=el("div","trending-user-main");const name=el("a","trending-user-name",item.name);name.href="#";name.addEventListener("click",event=>{event.preventDefault();const oldError=main.querySelector(".trending-profile-error");if(oldError)oldError.remove();openTrendingPublicProfile(Object.assign({},item,{monthlyHonor:true}),name);});main.appendChild(name);const coWinners=Math.max(1,Math.floor(Number(item.coWinnerCount)||1));main.appendChild(el("span","trending-honor-title",coWinners>1?`Joint monthly champion · ${coWinners} winners`:"Monthly champion"));profile.appendChild(main);row.appendChild(profile);return row;}
...
const CONCEPT_TRENDING_METRICS=[{key:"views",title:"Most viewed"},{key:"popular",title:"Most popular"},{key:"tested",title:"Most tested"},];const ACCOUNT_TRENDING_METRICS=[{key:"spenders",title:"Biggest Spenders"},{key:"eor_rating",title:"EOR Rating"},{key:"users",title:"Most active users"},{key:"quiz_correct",title:"AI Quiz Lovers"},];const TRENDING_METRICS=CONCEPT_TRENDING_METRICS.concat(ACCOUNT_TRENDING_METRICS);function visibleTrendingMetrics(){return accountFeatureEnabled()?TRENDING_METRICS:CONCEPT_TRENDING_METRICS;}
function normaliseTrendingMetric(metric){let key=String(metric||"").toLowerCase();if(key==="comments"||key==="buzz"||key==="buzzing"||key==="liveliness")key="lively";if(key==="saved_pages"||key==="saves"||key==="most_saved")key="saved";if(key==="most_tested"||key==="ai_quiz_tested"||key==="quiz_tested"||key==="ai_quiz_sessions")key="tested";if(key==="ai_quiz_correct"||key==="aiquiz_correct"||key==="quiz_correct_answers"||key==="ai_quiz_correct_answers")key="quiz_correct";if(key==="mastery_explorers"||key==="rating"||key==="elo")key="eor_rating";return visibleTrendingMetrics().some((m)=>m.key===key)?key:"views";}
...
const PERIODS=[{key:"today",label:"Top 10 Today",shortLabel:"Today",limit:10},{key:"7d",label:"Top 10 This week",shortLabel:"This week",limit:10},{key:"30d",label:"Top 10 Last 30 days",shortLabel:"Last 30 days",limit:10},{key:"all",label:"Top 100 All time",shortLabel:"All time",limit:10},];const ALL_TIME_CAP=100;function el(tag,cls,text){const node=document.createElement(tag);if(cls)node.className=cls;if(text!=null)node.textContent=text;return node;}
...
return out;}
function buildBlock({title,metric,deferInitialLoad}){const block=el("section","trending-block");block.dataset.metric=metric||"views";if(isTrendingUserMetric(metric))block.classList.add("trending-user-block");const ratingBoard=metric==="eor_rating";const initialPeriod=ratingBoard?"all":"7d";const boardPeriods=PERIODS.map((p)=>(ratingBoard&&p.key==="all"?{...p,label:"Top 100 current EOR Ratings",shortLabel:"Current"}:p));if(isTrendingUserMetric(metric))boardPeriods.splice(3,0,{key:"honors",label:"Monthly honours: past calendar-month champions",shortLabel:"Monthly honours",limit:10});const header=el("div","trending-block-header");header.appendChild(el("h2","trending-block-title",title));const tabs=el("div","trending-tabs");tabs.setAttribute("role","group");tabs.setAttribute("aria-label","Time period");boardPeriods.forEach((p)=>{const btn=el("button","trending-tab",p.shortLabel);btn.setAttribute("aria-label",p.label);btn.type="button";btn.dataset.period=p.key;tabs.appendChild(btn);});header.appendChild(tabs);const colHeadEl=el("div","trending-colhead");colHeadEl.appendChild(el("span","trending-colhead-spacer","#"));colHeadEl.appendChild(el("span","trending-colhead-left",isTrendingUserMetric(metric)?"Learner":"Concept"));if(!isTrendingUserMetric(metric))colHeadEl.appendChild(el("span","trending-colhead-course","Course · unit"));const metaHeadEl=el("span","trending-metahead trending-colhead-right",periodMetricLabel(initialPeriod,metric));colHeadEl.appendChild(metaHeadEl);const list=el("ol","trending-list");const footer=el("div","trending-footer");const prev=el("button","trending-page-btn");prev.type="button";prev.setAttribute("aria-label","Previous page");prev.textContent="←";const pages=el("div","trending-pages");const next=el("button","trending-page-btn");next.type="button";next.setAttribute("aria-label","Next page");next.textContent="→";footer.appendChild(prev);footer.appendChild(pages);footer.appendChild(next);const boardMeta=el("div","trending-board-meta");const boardMetaTime=el("span","trending-board-time","");boardMetaTime.setAttribute("role","status");boardMetaTime.setAttribute("aria-live","polite");const boardRefresh=el("button","trending-board-refresh");boardRefresh.type="button";boardRefresh.setAttribute("aria-label","Refresh this ranking");boardRefresh.title="Refresh this ranking from the cloud";boardRefresh.innerHTML="<span class=\"trending-board-refresh-icon\" aria-hidden=\"true\">↻</span><span class=\"trending-board-refresh-text\">Refresh</span>";boardMeta.appendChild(boardMetaTime);boardMeta.appendChild(boardRefresh);block.appendChild(header);block.appendChild(boardMeta);let metricNote=metric==="users"?"XP earned in the selected period. Includes learners with public rankings.":"";if(metric==="spenders"){metricNote="EOR Bits spent, including gifts sent. Received gifts and credits are excluded; refunds leave original spending counted. Today uses UTC; week and month cover the last 7 and 30 calendar days. Public rankings update after cloud sync.";}
if(metric==="eor_rating"){metricNote="EOR Rating is an Elo-style score that starts at 1200. Each rated result is a game against an opponent rating: AI quiz questions count as 1000 (basic), 1200 (standard) or 1400 (challenge), duels use the other player's rating, and each official weekend puzzle counts as 1500. Beating a stronger opponent gains more; losing to a weaker one costs more. Duels move the rating further than practice, and a weekend run counts as five puzzle games, so events move it furthest. Current ranks by the standing rating; today, week and month rank by the rating gained inside that window, using UTC and the last 7 and 30 calendar days. Only your first answer to each quiz question counts, up to 20 answers a UTC day; the same two players are rated at most three times a week; unfinished duels with no result change nothing. Learners appear after 10 rated results. Quiz answers are checked in your browser, so treat the rating as a friendly learning signal, not an exam grade. Public rankings update after cloud sync.";}
if(metric==="quiz_correct"){metricNote="Ranked by correct answers in public AI quiz practice; wrong answers are shown alongside.";}
const honorMetricNote={users:"The monthly score is XP recorded during that UTC calendar month.",quiz_correct:"The monthly score is correct answers in quiz sessions completed during that UTC calendar month.",spenders:"The monthly score is actual EOR Bits charges recorded during that UTC calendar month, including gifts sent.",eor_rating:"The monthly score is the EOR Rating gained during that UTC calendar month, for learners with at least 10 rated results by the end of it. A month spent losing rating scores zero rather than a negative.",}[metric]||"";let honorDetails="Honours recognise completed UTC calendar months, and every tied winner shares the title. "+honorMetricNote+" Awards appear after each month closes and results are finalised. Only champions with public rankings are shown; private winners are not replaced.";let metricNoteElement=null;let metricToggleElement=null;if(metricNote){const note=el("p","trending-user-ranking-note",metricNote);metricNoteElement=note;note.id=`trending-metric-note-${metric}`;note.hidden=true;const toggle=el("button","trending-metric-toggle","How it counts");metricToggleElement=toggle;toggle.type="button";toggle.setAttribute("aria-label","How it counts");toggle.setAttribute("aria-controls",note.id);toggle.setAttribute("aria-expanded","false");const icon=el("span","trending-metric-toggle-icon","+");icon.setAttribute("aria-hidden","true");toggle.appendChild(icon);toggle.addEventListener("click",()=>{const expanded=toggle.getAttribute("aria-expanded")!=="true";toggle.setAttribute("aria-expanded",String(expanded));icon.textContent=expanded?"−":"+";note.hidden=!expanded;});boardMeta.appendChild(toggle);boardMeta.appendChild(note);}
...
block.appendChild(colHeadEl);block.appendChild(list);block.appendChild(footer);const state={metric,period:initialPeriod,offset:0,total:0,};const periodCache=new Map();const honorsPageCache=new Map();let loadToken=0;function formatBoardTime(ts){const d=new Date(Number(ts)||Date.now());const pad=(n)=>String(n).padStart(2,"0");return`${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`;}
...
function setActiveTab(){const wasHonors=block.classList.contains("trending-honors-active");block.classList.toggle("trending-honors-active",state.period==="honors");if(honorNote)honorNote.hidden=state.period!=="honors";if(metricNoteElement){metricNoteElement.textContent=state.period==="honors"?honorDetails:metricNote;if(wasHonors!==(state.period==="honors")){metricNoteElement.hidden=true;metricToggleElement.setAttribute("aria-expanded","false");metricToggleElement.querySelector(".trending-metric-toggle-icon").textContent="+";}}
