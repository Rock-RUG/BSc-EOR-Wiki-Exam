...
(function () {
  ...
  const kindLabel = item => item.kind === "badge" ? "Badge" : item.kind === "honor" ? "Monthly trophy" : "Achievement";
  ...
  function render(ui) {
    ...
    const filtered = all.filter(item => (ui.kind === "all" || item.kind === ui.kind || (ui.kind === "achievement" && !["badge", "honor"].includes(item.kind))) && (ui.category === "all" || item.category === ui.category) && (ui.filter === "all" || (ui.filter === "earned" ? earned(item) : !earned(item))) && (!ui.query || `${item.title} ${item.description}`.toLowerCase().includes(ui.query.toLowerCase())));
    ...
    ui.dialog.querySelector(".rw-results-count").textContent = `${filtered.length} ${filtered.length === 1 ? "reward" : "rewards"}`;
    ui.dialog.querySelector(".rw-grid").innerHTML = filtered.map(item => collectionCard(item, data.featured.includes(item.id))).join("") || `<div class="rw-empty"><span aria-hidden="true">✧</span><h3>${emptyHonors ? "The next chapter is still being written" : "Nothing in this view yet"}</h3><p>${emptyHonors ? `Monthly trophies celebrate completed UTC calendar months.${/^\d{4}-\d{2}$/.test(data.season?.firstMonth || "") ? ` The first eligible month is ${esc(monthLabel(data.season.firstMonth))}.` : " The archive starts with the first full month of this system."} Winners are saved after at least 48 hours, once synced results have been checked. Tied leaders share the honour.` : "Try a different filter, or take your next small learning step."}</p></div>`;
  ...
  function open() {
    ...
    dialog.innerHTML = `<div class="rw-shell"><header class="rw-header"><div><span class="rw-eyebrow">The learning collection</span><h1 id="rw-title">Your cabinet of curiosities</h1></div><button type="button" class="rw-icon-button" data-close aria-label="Close collection">×</button></header><div class="rw-scroll"><section class="rw-hero"><div><span class="rw-eyebrow">A little progress. A lasting keepsake.</span><h2>Your learning,<br>collected.</h2><p>Follow your curiosity. Find your rhythm.<br>Turn the things you learn into a collection that is yours.</p><div class="rw-counts"></div></div><div class="rw-hero-art" aria-hidden="true">${medal({ kind: "badge", rarity: "epic", icon: "compass", earned: true })}${medal({ kind: "honor", rarity: "legendary", earned: true })}${medal({ rarity: "rare", icon: "book", earned: true })}</div></section><div class="rw-status-row"><p class="rw-status" role="status" aria-live="polite"></p><button type="button" class="rw-button" data-refresh>↻ Refresh</button></div><section class="rw-showcase" aria-label="Your profile showcase"><div class="rw-section-heading"><h2>The showcase</h2><span>Three places. Your favourite milestones.</span></div><div class="rw-showcase-items"></div></section><section class="rw-next" aria-label="Suggested next reward" hidden></section><section class="rw-catalog" aria-label="Reward collection"><div class="rw-collection-tabs" role="group" aria-label="Reward type"><button type="button" data-kind="all" aria-pressed="true">Everything</button><button type="button" data-kind="achievement" aria-pressed="false">Achievements</button><button type="button" data-kind="badge" aria-pressed="false">Badges</button><button type="button" data-kind="honor" aria-pressed="false">Monthly trophies</button></div><div class="rw-filters"><label><span>Search collection</span><input type="search" placeholder="Find a reward…" data-search></label><label><span>Collection</span><select data-filter="category"><option value="all">Every collection</option></select></label><label><span>Progress</span><select data-filter="filter"><option value="all">All progress</option><option value="earned">Collected</option><option value="locked">Still to discover</option></select></label><label><span>Sort</span><select data-filter="sort"><option value="next">Closest first</option><option value="recent">Recently collected</option><option value="name">Name A–Z</option></select></label></div><p class="rw-results-count" aria-live="polite"></p><div class="rw-grid"></div></section><details class="rw-rules"><summary>How the collection works</summary><p>Achievements celebrate small steps. Badges mark bigger commitments. Monthly trophies honour the leaders of completed UTC calendar months, with at least 48 hours for cloud sync and shared trophies for ties. A month stays open until all synced results are checked; once awarded, its trophies are permanent.</p><p>Your cloud learning unlocks rewards automatically. Earned rewards are kept even if you later revise your self-ratings. Public displays respect the privacy of the learning behind each reward. Older activity only counts where reliable records exist; missing history never becomes an invented award.</p><p>There is no deadline on achievements and no penalty for taking a break. Wiki activity and self-ratings are learning records, not academic certifications. These rewards do not add XP or spendable EORbits.</p></details></div></div>`;
  ...
  function mountProfile(host, data, options = {}) {
    ...
    section.setAttribute("aria-label", "Honours and collection");
    section.innerHTML = `<div class="rw-profile-heading"><h3>Honours & collection</h3><span>${items.length} collected${honors.length ? ` · ${honors.length} monthly ${honors.length === 1 ? "trophy" : "trophies"}` : ""}</span></div>${shown.length ? `<div class="rw-profile-medals">${shown.map(item => `<div class="rw-profile-medal">${medal(item)}<strong>${esc(item.title)}</strong><span>${esc(item.description)}</span></div>`).join("")}</div>` : ""}${honors.length ? `<details class="rw-profile-honors" ${honors.length <= 3 ? "open" : ""}><summary>Monthly honours · ${honors.length}</summary>${honors.map(item => `<div>${medal(item)}<span><strong>${esc(item.title)}</strong><span>${esc(item.description)}</span></span></div>`).join("")}</details>` : ""}${options.self ? '<button type="button" class="rw-button" data-collection>Open my collection ↗</button>' : ""}`;
    if (remaining.length) {
      const more = document.createElement("details");
      more.className = "rw-profile-more";
      more.innerHTML = `<summary>Explore all ${items.length} collected rewards</summary><div class="rw-profile-medals">${remaining.map(item => `<div class="rw-profile-medal">${medal(item)}<strong>${esc(item.title)}</strong><span>${esc(item.description)}</span></div>`).join("")}</div>`;
