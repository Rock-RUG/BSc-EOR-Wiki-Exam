...
let __srMainGeneration=0;let __srActiveContext=null;function srSearchContextCurrent(context){return!!context&&context===__srActiveContext&&context.generation===__srMainGeneration&&isOnFindPage()&&context.path===window.location.pathname&&context.container.isConnected&&document.getElementById("search-results")===context.container;}
function srFinderWorkspace(container){const root=container&&container.closest?container.closest('.wiki-finder'):null;if(!root)return null;if(root.__wfWorkspace)return root.__wfWorkspace;const editor=root.querySelector('#wf-query-editor');const bar=root.querySelector('[data-wf-bar]');if(!editor||!bar)return null;const workspace={root,editor,bar,container,view:'editor',hasResults:false,ready:false,scroll:{editor:null,results:null},focus:{editor:null,results:null},editButton:root.querySelector('[data-wf-edit]'),resultsButton:root.querySelector('[data-wf-results]'),};root.__wfWorkspace=workspace;const current=()=>root.isConnected&&document.getElementById('search-results')===container;workspace.editButton.addEventListener('click',()=>{if(current())srFinderShowView(workspace,'editor',true);});container.addEventListener('click',event=>{if(current()&&event.target.closest('.sr-empty-edit'))workspace.editButton.click();});workspace.resultsButton.addEventListener('click',()=>{if(current()&&workspace.hasResults)srFinderShowView(workspace,'results',true);});return workspace;}
function srFinderRefreshWorkspace(container){const workspace=srFinderWorkspace(container);if(!workspace)return;const{root,editor,bar,editButton,resultsButton}=workspace;const wide=!!(window.matchMedia&&window.matchMedia('(min-width: 1200px)').matches);const editing=workspace.view==='editor';const active=document.activeElement;root.dataset.wfView=workspace.view;root.dataset.wfHasResults=workspace.hasResults?'true':'false';editor.hidden=!editing;container.hidden=editing&&(workspace.hasResults?!wide:workspace.ready);bar.hidden=!workspace.hasResults;editButton.hidden=editing;resultsButton.hidden=!editing;editButton.setAttribute('aria-expanded',editing?'true':'false');resultsButton.setAttribute('aria-expanded',editing?'false':'true');if(active&&editor.hidden&&editor.contains(active))editButton.focus({preventScroll:true});else if(active&&container.hidden&&container.contains(active))resultsButton.focus({preventScroll:true});const data=srResultSummaryData();const applied=root.querySelector('[data-wf-applied]');const count=root.querySelector('[data-wf-count]');const dirty=root.querySelector('[data-wf-dirty]');if(applied){applied.textContent=srReadableQuery(data.applied);applied.title=data.applied;}
if(count)count.textContent=`(${data.matched})`;if(dirty)dirty.hidden=!data.dirty;}
function srFinderShowView(workspace,next,explicit){if(!workspace||(next==='results'&&!workspace.hasResults))return;const previous=workspace.view;if(previous!==next){workspace.scroll[previous]=Math.max(0,Number(window.scrollY)||0);const area=previous==='editor'?workspace.editor:workspace.container;if(area.contains(document.activeElement))workspace.focus[previous]=document.activeElement;workspace.view=next;}
srFinderRefreshWorkspace(workspace.container);if(previous===next&&!explicit)return;const area=next==='editor'?workspace.editor:workspace.container;const saved=explicit?workspace.focus[next]:null;const simpleMode=workspace.editor.dataset.fbSearchMode==='simple';const savedVisible=saved&&(!simpleMode||!(saved.closest&&saved.closest('#search-form,#find-builder')));const target=savedVisible&&saved.isConnected&&!saved.disabled&&area.contains(saved)?saved:next==='editor'?workspace.editor.querySelector(simpleMode?'#fb-simple-input':'#search-input'):workspace.container;if(target&&target.focus)target.focus({preventScroll:true});const top=explicit&&workspace.scroll[next]!==null?workspace.scroll[next]:null;try{if(top!==null)window.scrollTo({top,behavior:'instant'});else workspace.bar.scrollIntoView({block:'start',behavior:'instant'});}catch(_){}
srScheduleStableResize();}
function srFinderCommitWorkspace(container){const workspace=srFinderWorkspace(container);if(!workspace)return;workspace.hasResults=!!String(state.currentExpr||'').trim();workspace.ready=true;workspace.scroll.results=null;workspace.focus.results=null;if(!workspace.hasResults)srFinderShowView(workspace,'editor',false);else if(!srResultSummaryData().dirty)srFinderShowView(workspace,'results',false);else srFinderRefreshWorkspace(container);}
function srCancelStableResize(context=__srActiveContext){if(!context||context.resizeFrame==null)return;const frame=context.resizeFrame;context.resizeFrame=null;if(typeof window.cancelAnimationFrame==="function")window.cancelAnimationFrame(frame);}
function srScheduleStableResize(){const context=__srActiveContext;if(!srSearchContextCurrent(context)||!context.ready||context.resizeFrame!=null)return;const frame=window.requestAnimationFrame(()=>{if(context.resizeFrame!==frame)return;context.resizeFrame=null;if(srSearchContextCurrent(context)&&context.ready){srFinderRefreshWorkspace(context.container);if(!context.container.hidden)srStableMeasure(context.container);}});context.resizeFrame=frame;}
function srBindStableResize(){const previous=window.__mkFinderStableResizeCleanup;if(typeof previous==="function")previous();window.addEventListener("resize",srScheduleStableResize,{passive:true});window.__mkFinderStableResizeCleanup=()=>{window.removeEventListener("resize",srScheduleStableResize);srCancelStableResize();};}
function srRenderIndexError(container,context){container.innerHTML=`<div class="sr-loading" role="status">Could not load the search index. Check your connection and retry. <button type="button" class="md-button" data-sr-retry>Retry</button></div>`;const retry=container.querySelector("[data-sr-retry]");if(retry)retry.addEventListener("click",()=>{if(srSearchContextCurrent(context))main(context).catch(e=>console.warn("search-results:",e));});}
...
function srEnsureAiUnlocked(source){return Promise.resolve(!window.__mkExamMode);}
const LAST_FACE_KEY="random_custom_last_dice_face_v1";let __srDiceState={timer:null,navigating:false};let __srRandomNavigation=null;let __srRandomNavigationSequence=0;function srCancelRandomNavigation(){const request=__srRandomNavigation;__srRandomNavigation=null;++__srRandomNavigationSequence;if(request)request.timers.forEach(id=>window.clearTimeout(id));srResetRandomButtonUi(document);}
function srBeginRandomNavigation(container,button){srCancelRandomNavigation();const request={url:String(window.location.href),container,button,timers:[]};__srRandomNavigation=request;__srDiceState.navigating=true;return request;}
...
function srLearningHits(hits){return(Array.isArray(hits)?hits:[]).filter(hit=>{const tags=(hit&&hit.doc&&hit.doc.tags)||[];return!(Array.isArray(tags)?tags:String(tags).split(/[\s,]+/)).some(tag=>/^curriculum-(enrichment|retired)$/i.test(String(tag).trim()));});}
function srStudySetContextError(context){if(!srSearchContextCurrent(context))return{ok:false,code:'stale_context'};if(context.failed)return{ok:false,code:'index_failed'};if(!context.ready)return{ok:false,code:'not_ready'};return null;}
function srStudySetLocation(raw,root){try{const value=String(raw||'').trim();if(!value||/[\\?#\u0000-\u001f\u007f]/.test(value)||/%2f/i.test(value))return'';const decoded=decodeURIComponent(value);if(/[\\%?#\u0000-\u001f\u007f]/.test(decoded)||decoded.split('/').some(part=>part==='.'||part==='..'))return'';const url=new URL(value,root);if(url.origin!==root.origin||url.username||url.password||url.search||url.hash||!url.pathname.startsWith(root.pathname))return'';const loc=url.pathname.slice(root.pathname.length);if(decodeURIComponent(loc).includes(':'))return'';return/\.html$/i.test(loc)&&isConceptPageLocation(loc)?loc:'';}catch(_){return'';}}
function srStudySetPage(doc,root){if(!doc)return null;const loc=srStudySetLocation(doc.location,root);if(!loc)return null;const rawTags=Array.isArray(doc.tags)?doc.tags:String(doc.tags||'').split(/[\s,]+/);const tags=Array.from(new Set(rawTags.filter(tag=>typeof tag==='string').map(tag=>tag.trim()).filter(Boolean)));const curriculumStatus=tags.some(tag=>/^curriculum-retired$/i.test(tag))?'retired':tags.some(tag=>/^curriculum-enrichment$/i.test(tag))?'enrichment':'current';return{loc,title:String(doc.title||'Untitled'),tags,curriculumStatus};}
function srPublishedStudySetPages(){const root=new URL(getSiteRootUrl());const pages=new Map();for(const doc of state.pageDocs||[]){const page=srStudySetPage(doc,root);if(page&&!pages.has(page.loc))pages.set(page.loc,page);}
return{root,pages};}
function srGetPublishedConceptsSnapshot(context){const error=srStudySetContextError(context);if(error)return error;return{ok:true,version:1,capturedAt:Date.now(),items:Array.from(srPublishedStudySetPages().pages.values())};}
function srGetSelectionSnapshot(context,options){const error=srStudySetContextError(context);if(error)return error;const includeEnrichment=!!(options&&options.includeEnrichment===true);const sorted=getSortedHits(srFilteredHits(state.hits));const selected=sorted.filter(hit=>hit&&hit.doc&&state.selectedMap&&state.selectedMap[hit.doc.location]===true);const eligible=includeEnrichment?selected:srLearningHits(selected);const{root,pages}=srPublishedStudySetPages();const seen=new Set(),items=[];for(const hit of eligible){const loc=srStudySetLocation(hit.doc.location,root),page=pages.get(loc);if(!page||seen.has(loc)||page.curriculumStatus==='retired'||(!includeEnrichment&&page.curriculumStatus==='enrichment'))continue;seen.add(loc);items.push(page);}
...
function srRenderMasteryFilterHtml(){const f=state.masteryFilter||srDefaultMasteryFilter();const items=[["mastered","Mastered"],["clear","Clear"],["fuzzy","Partial"],["unknown","Unknown"],["unrated","Unrated"],["unvisited","Unvisited"],];return`
...
async function srFetchHot({metric,period,limit,offset}){const url=new URL(SR_VIEWS_API_BASE+"/hot");url.searchParams.set("metric",metric);url.searchParams.set("period",period);url.searchParams.set("limit",String(limit));url.searchParams.set("offset",String(offset));const resp=await fetch(url.toString());if(!resp||!resp.ok)throw new Error("Could not load view counts");const data=await resp.json();if(!data||!Array.isArray(data.items))throw new Error("Invalid view counts response");return{items:data.items,total:data&&typeof data.total==="number"?data.total:0,};}
...
__srViews30dMap=m;return m;})();try{return await __srViews30dPromise;}finally{__srViews30dPromise=null;}}
...
function srRenderSortDropdownHtml(){const label=srSortLabel();const opts=[{id:"best",label:"Most relevant",icon:"target"},{id:"views30d",label:"Most viewed (30d)",icon:"fire"},{id:"lecture-asc",label:"Course · Lecture/Week ↑",icon:"up"},{id:"lecture-desc",label:"Course · Lecture/Week ↓",icon:"down"},{id:"title-asc",label:"Title A → Z",icon:"az"},{id:"title-desc",label:"Title Z → A",icon:"za"},];const menu=opts.map(o=>{const active=srIsActiveOption(o.id)?"is-active":"";return`
    ...
    <div class="mk-sort" data-mk-sort-root="1">
      <button type="button" class="mk-sortbtn" aria-controls="csr-sort-options" aria-expanded="false">
      ...
      <div class="mk-sortmenu" role="group" aria-label="Sort results" id="csr-sort-options" hidden>
        ${menu}
      </div>
    </div>
  `.trim();}
...
const opt=e.target&&e.target.closest?e.target.closest(".mk-sortopt"):null;if(opt&&root&&container.contains(root)){e.preventDefault();e.stopPropagation();const id=opt.getAttribute("data-mk-sort")||"";const next=srSortKeyDirFromOption(id);srSetSortExplicit(next.key,next.dir);state.viewsError="";srCloseMenu(root);renderResults(container,".mk-sortbtn");if(next.key==="views30d"){const context=__srActiveContext;srLoadViews30dMapOnce().then(()=>{if(srSearchContextCurrent(context)&&(state.sortKey||"best")==="views30d")renderResults(container);}).catch(()=>{if(!srSearchContextCurrent(context)||(state.sortKey||"best")!=="views30d")return;state.viewsError="View counts could not be loaded. Select Most viewed (30d) again to retry.";renderResults(container);});}
return;}});document.addEventListener("click",(e)=>{const open=container.querySelector('.mk-sortbtn[aria-expanded="true"]');if(!open)return;const root=open.closest('.mk-sort[data-mk-sort-root="1"]');if(!root)return;if(e.target&&root.contains(e.target))return;srCloseMenu(root);},true);document.addEventListener("keydown",(e)=>{if(!e||e.key!=="Escape")return;const open=container.querySelector('.mk-sortbtn[aria-expanded="true"]');if(!open)return;const root=open.closest('.mk-sort[data-mk-sort-root="1"]');const returnFocus=!!root&&root.contains(document.activeElement);srCloseMenu(root);if(returnFocus){try{open.focus({preventScroll:true});}catch(_){}}},true);}
...
function srNavigateToTarget(targetUrl,request){const href=String(targetUrl||"").trim();if(!href)return false;request=request||srBeginRandomNavigation(null,null);if(!srRandomNavigationCurrent(request))return false;try{const a=document.createElement("a");a.href=href;a.rel="noopener";a.style.display="none";document.body.appendChild(a);a.click();if(!srRandomNavigationCurrent(request))return true;request.timers.push(window.setTimeout(()=>{if(!srRandomNavigationCurrent(request))return;try{window.location.href=href;}catch(_){}},60));request.timers.push(window.setTimeout(()=>{if(!srRandomNavigationCurrent(request))return;try{window.location.assign(href);}catch(_){}},180));return true;}catch(_){}
if(!srRandomNavigationCurrent(request))return false;try{window.location.href=href;return true;}catch(_){}
if(!srRandomNavigationCurrent(request))return false;try{window.location.assign(href);return true;}catch(_){}
return false;}
...
return deduped;}
async function loadIndex(){const candidates=["search/search_index.json","/search/search_index.json","search_index.json"];const base=document.baseURI;for(const u of candidates){try{const url=new URL(u,base).toString();const request=__mkFetchSearchIndex(url,{cache:"no-cache"});const j=await request;if(j&&Array.isArray(j.docs))return j;for(const map of[window.__mkSharedJsonPromiseMap,window.__mkMaterialSearchIndexPromises]){if(map&&map[url]===request)delete map[url];}}catch(_){}}
throw new Error("Could not load the search index");}
...
function isConceptPageLocation(loc){const path=safePath(loc);if(!path)return false;if(path.endsWith("/"))return false;if(/^assets\//i.test(path))return false;const segs=path.split("/").filter(Boolean);if(segs.length<3)return false;if(isIndexPage(path))return false;if(isRandomPage(path))return false;if(isUtilityPage(path))return false;return true;}
...
const state={pageDocs:[],hits:[],selectedMap:{},currentExpr:"",draftExpr:null,revisionOptionsOpen:false,masteryOptionsOpen:false,hint:"",viewsError:"",page:1,pageSize:10,selfTestMode:false,aiTestMode:false,masteryFilter:srDefaultMasteryFilter(),sortKey:"best",sortDir:"desc",};function pickRandomSelected(pool=srFilteredHits(state.hits).filter(x=>state.selectedMap[x.doc.location])){pool=srLearningHits(pool);if(!pool.length)return null;return pool[Math.floor(Math.random()*pool.length)].doc;}
...
function srBuildResultRowHtml(x,root){const d=(x&&x.doc)||{};const href=new URL(String(d.location||""),root).toString();const lec=lectureInfoFromTags(d.tags);const courseName=lec?(lec.courseName||srCourseLabelFromLocation(d.location)):"";const metaHtml=lec?`${courseName ? `<span class="csr-course">${escapeHtml(courseName)}</span><span class="csr-meta-sep" aria-hidden="true"> · </span>` : ""}<span class="csr-lecture-no">${escapeHtml(lec.unitLabel || `Lecture ${lec.lectureNum}`)}</span>`:"";const checked=state.selectedMap&&state.selectedMap[d.location]?"checked":"";return`
    <div class="csr-row csr-row--select">
      <label class="csr-check">
        <input type="checkbox"
               aria-label="Select ${escapeHtml(d.title || 'Untitled')}"
...
function srStableMeasure(container){if(!container)return;try{const wrap=container.querySelector(".csr-wrap")||container;const options=container.querySelector(".csr-revision-options");const filters=container.querySelector(".csr-filter-options");const width=Math.round(wrap.getBoundingClientRect().width||window.innerWidth||0);const scopeKey=JSON.stringify([state.currentExpr||'',(state.hits||[]).length,state.masteryFilter||{}]);const layoutKey=`${width}:${options && !options.hidden ? "open" : "closed"}:${filters && !filters.hidden ? "filters-open" : "filters-closed"}:${scopeKey}`;if(container.dataset.srStableLayoutKey!==layoutKey){container.dataset.srStableLayoutKey=layoutKey;delete container.dataset.srStableListMinH;delete container.dataset.srStableActionsMinH;wrap.style.removeProperty("--csr-stable-list-min-h");wrap.style.removeProperty("--csr-stable-actions-min-h");}
const list=container.querySelector(".csr-list");if(list){const h=Math.ceil(list.getBoundingClientRect().height||list.offsetHeight||0);const prev=Number(container.dataset.srStableListMinH||0);const next=Math.max(prev,h);if(next>0){container.dataset.srStableListMinH=String(next);wrap.style.setProperty("--csr-stable-list-min-h",next+"px");}}
...
function srReadableQuery(expression){const value=String(expression||"").trim();const unit=lectureInfoFromTags([value]);return unit&&unit.courseName?unit.courseName+" · "+unit.unitLabel:COURSE_MAP[value.toLowerCase()]||value;}
function srResultSummaryData(){const filtered=srFilteredHits(state.hits);const selected=filtered.filter(x=>state.selectedMap&&state.selectedMap[x.doc.location]);const eligible=srLearningHits(filtered).filter(x=>state.selectedMap&&state.selectedMap[x.doc.location]);const excluded=selected.filter(hit=>!eligible.includes(hit));const retired=excluded.filter(hit=>{const tags=(hit.doc&&hit.doc.tags)||[];return(Array.isArray(tags)?tags:String(tags).split(/[\s,]+/)).some(tag=>/^curriculum-retired$/i.test(String(tag).trim()));}).length;const applied=String(state.currentExpr||"").trim();return{applied,dirty:!!applied&&state.draftExpr!==null&&state.draftExpr!==undefined&&state.draftExpr!==applied,matched:(state.hits||[]).length,filtered:filtered.length,selected:selected.length,eligible:eligible.length,excluded:excluded.length,retired,enrichment:excluded.length-retired,};}
function srRenderResultSummaryHtml(){return`
    <div class="csr-result-summary">
      <h2 class="csr-result-title">Results</h2>
      <p class="csr-applied-query">Last search <code data-sr-applied-query>${escapeHtml(state.currentExpr || "")}</code></p>
      <p class="csr-query-state" data-sr-draft-status role="status" hidden></p>
      <p class="csr-match-count" data-sr-match-count></p>
    </div>`;}
function srUpdateResultSummary(container){if(!container)return;const data=srResultSummaryData();const write=(selector,value)=>{const node=container.querySelector(selector);if(node&&node.textContent!==value)node.textContent=value;return node;};const query=write('[data-sr-applied-query]',srReadableQuery(data.applied));if(query)query.setAttribute('title',data.applied);const status=write('[data-sr-draft-status]',data.dirty?"Draft changed · showing the last search.":"");if(status)status.hidden=!data.dirty;write('[data-sr-match-count]',data.matched===data.filtered?`${data.matched} concepts`:`${data.filtered} of ${data.matched} concepts`);write('[data-sr-selection-count]',data.selected===data.eligible?`${data.selected} selected`:`${data.selected} selected · ${data.eligible} for random revision`);const explanation=container.querySelector('[data-sr-selection-explanation]');if(explanation)explanation.hidden=!data.excluded;write('[data-sr-selection-why]',`${data.excluded} selected concept${data.excluded === 1 ? " is" : "s are"} outside random revision`);const reasons=[];if(data.enrichment)reasons.push(`${data.enrichment} enrichment`);if(data.retired)reasons.push(`${data.retired} retired`);write('[data-sr-selection-reasons]',`${reasons.join(" and ")} concept${data.excluded === 1 ? " remains" : "s remain"} searchable, but random revision uses current curriculum concepts. Save as set uses the same scope by default.`);write('[data-sr-revision-mode]',state.aiTestMode?"AI-test":state.selfTestMode?"Self-test":"Reading");write('[data-sr-revision-help]',state.aiTestMode?"Open a random selected concept with a multiple-choice check. Checking answers needs internet access.":state.selfTestMode?"Open a random selected concept with explanations hidden. Recall first, then reveal.":"Open a random concept from your selection to read at your own pace.");const levels=Object.keys(srDefaultMasteryFilter());const filter=state.masteryFilter||srDefaultMasteryFilter();const included=levels.filter(key=>filter[key]!==false).length;const filterStatus=write('[data-sr-filter-status]',included===levels.length?"All levels":`${included}/${levels.length} levels`);if(filterStatus)filterStatus.setAttribute('data-active',included===levels.length?'false':'true');const empty=container.querySelector('.csr-filter-empty');if(empty)empty.hidden=data.filtered!==0;srFinderRefreshWorkspace(container);}
function srBindListAndPager(container,totalPages){const prev=container.querySelector(".csr-prev");if(prev)prev.addEventListener("click",()=>{if(state.page>1)state.page-=1;renderResults(container,".csr-prev");});const next=container.querySelector(".csr-next");if(next)next.addEventListener("click",()=>{state.page=Math.min(totalPages,state.page+1);renderResults(container,".csr-next");});container.querySelectorAll(".csr-page").forEach(btn=>{btn.addEventListener("click",()=>{const n=parseInt(btn.getAttribute("data-page"),10);if(!Number.isFinite(n))return;state.page=n;renderResults(container,`.csr-page[data-page="${n}"]`);});});container.querySelectorAll('input[type="checkbox"][data-loc]').forEach(cb=>{cb.addEventListener("change",()=>{const loc=cb.getAttribute("data-loc");if(!loc)return;state.selectedMap=state.selectedMap||{};state.selectedMap[loc]=cb.checked;srUpdateResultSummary(container);});});}
function srUpdateResultsListOnly(container){if(!container)return;const filteredHits=srFilteredHits(state.hits);const total=filteredHits.length;const pageSize=state.pageSize||10;const totalPages=Math.max(1,Math.ceil(total/pageSize));state.page=Math.min(Math.max(1,state.page),totalPages);const sorted=getSortedHits(filteredHits);const start=(state.page-1)*pageSize;const end=Math.min(total,start+pageSize);const pageHits=sorted.slice(start,start+pageSize);const root=new URL(document.baseURI);const listEl=container.querySelector(".csr-list");if(listEl)listEl.innerHTML=pageHits.map(x=>srBuildResultRowHtml(x,root)).join("")||``;const oldFoot=container.querySelector(".csr-foot");if(oldFoot)oldFoot.outerHTML=srBuildFooterHtml(total,start,end,totalPages,srBuildPagerHtml(totalPages,state.page));srBindListAndPager(container,totalPages);srUpdateResultSummary(container);typesetMath(container.querySelector(".csr-list")||container);srStableMeasure(container);}
function srBindResultOptions(container){for(const[kind,key,panelSelector]of[['filters','masteryOptionsOpen','.csr-filter-options'],['revision','revisionOptionsOpen','.csr-revision-options']]){const selector=`[data-sr-toggle="${kind}"]`;const button=container.querySelector(selector);const panel=container.querySelector(panelSelector);if(!button||!panel)continue;const setOpen=open=>{if(container.querySelector(selector)!==button)return;state[key]=!!open;panel.hidden=!open;button.setAttribute('aria-expanded',String(!!open));srStableMeasure(container);};button.addEventListener('click',()=>setOpen(panel.hidden));panel.addEventListener('keydown',event=>{if(event.key!=='Escape'||panel.hidden)return;setOpen(false);event.preventDefault();event.stopPropagation();button.focus();});}}
function renderResults(container,focusSelector=""){const active=document.activeElement;let focusAttribute="",focusValue="";if(!focusSelector&&active&&container.contains(active)){focusSelector=[".mk-sortbtn","#cr-all","#cr-none","#cr-selftest","#cr-aitest","#cr-random",".csr-prev",".csr-next",'[data-sr-toggle="filters"]','[data-sr-toggle="revision"]'].find(selector=>container.querySelector(selector)===active)||"";if(!focusSelector&&active.getAttribute){for(const[attribute,selector]of[["data-page",".csr-page"],["data-loc","input[data-loc]"],["data-mastery-filter","input[data-mastery-filter]"]]){const value=active.getAttribute(attribute);if(value){focusSelector=selector;focusAttribute=attribute;focusValue=value;break;}}}}
const restoreFocus=()=>{if(!focusSelector)return;let target=focusAttribute?Array.from(container.querySelectorAll(focusSelector)).find(node=>node.getAttribute(focusAttribute)===focusValue):container.querySelector(focusSelector);if(target&&target.disabled&&(focusSelector===".csr-prev"||focusSelector===".csr-next")){target=container.querySelector('.csr-page[aria-current="page"]');}
if(!target||target.disabled){target=container;target.setAttribute("tabindex","-1");}
try{target.focus({preventScroll:true});}catch(_){}};srCancelRandomNavigation();srEnsureMasteryFilterStylesOnce();if(!String(state.currentExpr||"").trim()&&!(state.hits&&state.hits.length)){container.innerHTML=`<div class="csr-empty-state" role="status">
      <p>Build a query, then choose <strong>Run search</strong>.</p>
    </div>`;srFinderRefreshWorkspace(container);restoreFocus();return;}
if(String(state.currentExpr||"").trim()&&!(state.hits&&state.hits.length)){container.innerHTML=`
    ...
    `;srUpdateResultSummary(container);typesetMath(container);srMaybeScrollToResultsFromLearningPath(container);restoreFocus();return;}
...
`;const actionsHtml=`
  <div class="csr-actions csr-actions--v3">
    <div class="csr-toolbar-main">
    <div class="csr-browse-tools" role="group" aria-label="Browse results">
      <button type="button" class="csr-filter-toggle" data-sr-toggle="filters" aria-controls="csr-filter-panel" aria-expanded="${!!state.masteryOptionsOpen}">Filters <span data-sr-filter-status></span></button>
      <div class="csr-action-sort">${srRenderSortDropdownHtml()}</div>
    </div>
    <div class="csr-revise-tools" role="group" aria-label="Select and revise">
      <p class="csr-selection-count" data-sr-selection-count role="status"></p>
    ...
    <details class="csr-selection-explanation" data-sr-selection-explanation hidden><summary data-sr-selection-why></summary><p data-sr-selection-reasons></p></details>
    <div class="csr-toolbar-panels">
    <div class="csr-filter-options" id="csr-filter-panel" role="group" aria-label="Mastery filters" ${state.masteryOptionsOpen ? "" : "hidden"}>
    ...
    ${window.__mkExamMode ? "" : `<label class="csr-aitest ${srHasShopItem(SR_AI_ITEM_ID)?"":"is-locked"}" title="${srHasShopItem(SR_AI_ITEM_ID)?"":`Unlock ${SR_AI_ITEM_TITLE} · ${SR_AI_ITEM_PRICE} EORbits`}">
      ...
      <p class="csr-query-state" data-sr-revision-help aria-live="polite"></p>
      <a href="about.html#revision-modes">About revision modes</a>
    </div>
    </div>
  </div>
...
`;srUpdateResultSummary(container);srStableMeasure(container);srBindResultOptions(container);const btnAll=container.querySelector("#cr-all");if(btnAll)btnAll.addEventListener("click",()=>{state.selectedMap=state.selectedMap||{};for(const x of srFilteredHits(state.hits))state.selectedMap[x.doc.location]=true;renderResults(container,"#cr-all");});const btnNone=container.querySelector("#cr-none");if(btnNone)btnNone.addEventListener("click",()=>{state.selectedMap=state.selectedMap||{};for(const x of srFilteredHits(state.hits))state.selectedMap[x.doc.location]=false;renderResults(container,"#cr-none");});const cbSelf=container.querySelector("#cr-selftest");const cbAi=container.querySelector("#cr-aitest");if(cbSelf)cbSelf.addEventListener("change",async()=>{if(cbSelf.checked){if(!(await srEnsureSelfTestUnlocked("find-self-test-toggle"))){cbSelf.checked=false;setRandomMode(cbAi&&cbAi.checked?"ai":"normal");srUpdateResultSummary(container);return;}
if(cbAi)cbAi.checked=false;setRandomMode("self");}else{setRandomMode(cbAi&&cbAi.checked?"ai":"normal");}
srUpdateResultSummary(container);});if(cbAi)cbAi.addEventListener("change",async()=>{if(cbAi.checked){if(!(await srEnsureAiUnlocked("find-ai-test-toggle"))){cbAi.checked=false;setRandomMode(cbSelf&&cbSelf.checked?"self":"normal");srUpdateResultSummary(container);return;}
if(cbSelf)cbSelf.checked=false;setRandomMode("ai");}else{setRandomMode(cbSelf&&cbSelf.checked?"self":"normal");}
srUpdateResultSummary(container);});container.querySelectorAll('input[data-mastery-filter]').forEach(cb=>{cb.addEventListener("change",()=>{const key=cb.getAttribute("data-mastery-filter")||"";if(!key)return;state.masteryFilter=state.masteryFilter||srDefaultMasteryFilter();state.masteryFilter[key]=!!cb.checked;srWriteMasteryFilter(state.masteryFilter);state.page=1;srUpdateResultsListOnly(container);});});const btnRandom=container.querySelector("#cr-random");if(btnRandom&&btnRandom.dataset.crDiceBound!=="1"){btnRandom.dataset.crDiceBound="1";const ico=btnRandom.querySelector(".cr-dice__ico");if(ico)setDiceFace(ico,readLastFace());if(ico&&isHoverPointer()){btnRandom.addEventListener("mouseenter",()=>{if(__srDiceState.navigating)return;beginHoverRoll(ico,__srDiceState);});btnRandom.addEventListener("mouseleave",()=>{if(__srDiceState.navigating)return;freezeToRandomFace(ico,__srDiceState);});}
btnRandom.addEventListener("click",async(e)=>{e.preventDefault();e.stopPropagation();if(__srDiceState.navigating)return;const request=srBeginRandomNavigation(container,btnRandom);let committed=false;try{if(state.aiTestMode&&!(await srEnsureAiUnlocked("find-start-ai-test")))return;if(!srRandomNavigationCurrent(request))return;if(state.selfTestMode&&!(await srEnsureSelfTestUnlocked("find-start-self-test")))return;if(!srRandomNavigationCurrent(request))return;const pool=srLearningHits(srFilteredHits(state.hits)).filter(x=>state.selectedMap[x.doc.location]);const locs=pool.map(x=>x.doc.location);const mode=state.aiTestMode?"ai":(state.selfTestMode?"self":"normal");const picked=pickRandomSelected(pool);if(!picked){let notice=container.querySelector('.sr-random-notice');if(!notice){notice=document.createElement('p');notice.className='sr-random-notice';notice.setAttribute('role','status');container.appendChild(notice);}
notice.textContent='Select at least one current course concept to start random study. Supplementary pages remain available through their links.';return;}
const targetUrl=toAbsoluteUrl(picked.location);if(!targetUrl)return;if(!srConsumeGuestAction("random",{source:"find-start-random",title:"Start random",dedupeMs:2500}))return;try{btnRandom.disabled=true;btnRandom.setAttribute("aria-disabled","true");btnRandom.style.pointerEvents="none";}catch(_){}
...
if(!srRandomNavigationCurrent(request))return;await sleep(250);if(!srRandomNavigationCurrent(request))return;try{sessionStorage.setItem("random_custom_candidates_v1",JSON.stringify(locs));}catch(_){}
...
srEnsureSortDropdownStylesOnce();srEnsureSortDropdownBinding(container);const prev=container.querySelector(".csr-prev");if(prev)prev.addEventListener("click",()=>{if(state.page>1)state.page-=1;renderResults(container,".csr-prev");});const next=container.querySelector(".csr-next");if(next)next.addEventListener("click",()=>{state.page=Math.min(totalPages,state.page+1);renderResults(container,".csr-next");});container.querySelectorAll(".csr-page").forEach(btn=>{btn.addEventListener("click",()=>{const n=parseInt(btn.getAttribute("data-page"),10);if(!Number.isFinite(n))return;state.page=n;renderResults(container,`.csr-page[data-page="${n}"]`);});});container.querySelectorAll('input[type="checkbox"][data-loc]').forEach(cb=>{cb.addEventListener("change",()=>{const loc=cb.getAttribute("data-loc");if(!loc)return;state.selectedMap=state.selectedMap||{};state.selectedMap[loc]=cb.checked;srUpdateResultSummary(container);});});typesetMath(container);srMaybeScrollToResultsFromLearningPath(container);restoreFocus();}
function exposeApi(container,context){window.__findSearchV2=window.__findSearchV2||{};window.__findSearchV2.matchCourseToken=matchCourseToken;window.__findSearchV2.getSelectionSnapshot=options=>srGetSelectionSnapshot(context,options);window.__findSearchV2.getPublishedConceptsSnapshot=()=>srGetPublishedConceptsSnapshot(context);window.__findSearchV2.setDraftExpr=(exprText)=>{if(context&&!srSearchContextCurrent(context))return false;state.draftExpr=String(exprText||"").trim();srUpdateResultSummary(container);return true;};window.__findSearchV2.clearEmptyResults=()=>{if(context&&!srSearchContextCurrent(context))return false;if(!container.querySelector(".sr-empty"))return false;if(context){context.queryVersion+=1;context.pendingExpr="";}
state.currentExpr="";state.hits=[];state.selectedMap={};container.innerHTML="";const workspace=srFinderWorkspace(container);if(workspace){workspace.hasResults=false;srFinderShowView(workspace,'editor',false);}
return true;};window.__findSearchV2.runLogicExpr=(exprText)=>{if(context&&!srSearchContextCurrent(context))return false;const raw=String(exprText||"").trim();if(context){context.queryVersion+=1;if(!context.ready){context.pendingExpr=raw;if(context.failed)main(context).catch(e=>console.warn("search-results:",e));return true;}}
if(!raw){state.currentExpr="";state.hint="";state.page=1;state.hits=[];state.selectedMap={};renderResults(container);srFinderCommitWorkspace(container);return true;}
const toks=tokenizeExpr(raw);const v=validateTokens(toks);if(!v.ok)return false;const rpn=toRpn(toks);const ast=rpn?rpnToAst(rpn):null;if(!ast)return false;const hasParen=toks.some(t=>t.k==="LP"||t.k==="RP");const ops=toks.filter(t=>t.k==="OP").map(t=>t.v);const mixedOps=ops.includes("AND")&&ops.includes("OR");const termCount=toks.filter(t=>t.k==="TERM").length;let nextHint="";if(!hasParen&&mixedOps&&termCount>=3){const suggested=astToInfixWithParens(ast);nextHint=`Did you mean: ${suggested}?`;}
...
state.currentExpr=raw;state.hint=nextHint;state.page=1;state.hits=hit;state.selectedMap={};for(const x of hit)state.selectedMap[x.doc.location]=true;renderResults(container);srFinderCommitWorkspace(container);return true;};}
async function main(retryContext){if(retryContext&&!srSearchContextCurrent(retryContext))return;srCancelStableResize();const generation=++__srMainGeneration;if(!isOnFindPage())return;const container=document.getElementById("search-results");if(!container)return;const context={generation,container,path:window.location.pathname,ready:false,failed:false,queryVersion:0,pendingExpr:retryContext?retryContext.pendingExpr:null,};__srActiveContext=context;if(!retryContext){state.draftExpr=null;state.revisionOptionsOpen=false;state.masteryOptionsOpen=false;}
exposeApi(container,context);const workspace=srFinderWorkspace(container);if(workspace){workspace.ready=false;srFinderRefreshWorkspace(container);}
hardUnlockScroll();const unlockIfCurrent=()=>{if(srSearchContextCurrent(context))hardUnlockScroll();};requestAnimationFrame(unlockIfCurrent);setTimeout(unlockIfCurrent,60);container.innerHTML=`<div class="sr-loading">Loading search index…</div>`;if(window.renderMathInElement){window.renderMathInElement(container,{delimiters:[{left:"$$",right:"$$",display:true},{left:"$",right:"$",display:false},{left:"\\(",right:"\\)",display:false},{left:"\\[",right:"\\]",display:true},],throwOnError:false,});}else if(window.MathJax&&window.MathJax.typesetPromise){window.MathJax.typesetPromise([container]).catch(()=>{});}
...
srBindStableResize();if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init);else init();document.addEventListener("DOMContentSwitch",()=>{srCancelRandomNavigation();init();});window.addEventListener("pagehide",()=>{srCancelRandomNavigation();srCancelStableResize();});window.addEventListener("pageshow",()=>{if(!isOnFindPage())return;srScheduleStableResize();hardUnlockScroll();srResetRandomButtonUi(document);const sequence=__srRandomNavigationSequence;const pageUrl=String(window.location.href);setTimeout(()=>{if(sequence!==__srRandomNavigationSequence||pageUrl!==String(window.location.href)||!isOnFindPage())return;hardUnlockScroll();srResetRandomButtonUi(document);},60);});})();