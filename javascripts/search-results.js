...
function renderResults(container){srCancelRandomNavigation();srEnsureMasteryFilterStylesOnce();if(!String(state.currentExpr||"").trim()&&!(state.hits&&state.hits.length)){container.innerHTML=`<div class="csr-empty-state" role="status">
      <p>Build a query, then choose <strong>Run search</strong>.</p>
    </div>`;srFinderRefreshWorkspace(container);return;}
if(String(state.currentExpr||"").trim()&&!(state.hits&&state.hits.length)){container.innerHTML=`
    ...
    `;srUpdateResultSummary(container);typesetMath(container);srMaybeScrollToResultsFromLearningPath(container);return;}
