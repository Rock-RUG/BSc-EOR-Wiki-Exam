...
function srStableMeasure(container){if(!container)return;try{const wrap=container.querySelector(".csr-wrap")||container;const options=container.querySelector(".csr-revision-options");const filters=container.querySelector(".csr-filter-options");const width=Math.round(wrap.getBoundingClientRect().width||window.innerWidth||0);const layoutKey=`${width}:${options && options.open ? "open" : "closed"}:${filters && filters.open ? "filters-open" : "filters-closed"}`;if(container.dataset.srStableLayoutKey!==layoutKey){container.dataset.srStableLayoutKey=layoutKey;delete container.dataset.srStableListMinH;delete container.dataset.srStableActionsMinH;wrap.style.removeProperty("--csr-stable-list-min-h");wrap.style.removeProperty("--csr-stable-actions-min-h");}
...
function renderResults(container){const previousOptions=container.querySelector('.csr-revision-options');if(previousOptions)state.revisionOptionsOpen=!!previousOptions.open;const previousFilters=container.querySelector('.csr-filter-options');if(previousFilters)state.masteryOptionsOpen=!!previousFilters.open;srCancelRandomNavigation();srEnsureMasteryFilterStylesOnce();if(!String(state.currentExpr||"").trim()&&!(state.hits&&state.hits.length)){container.innerHTML=`<div class="csr-empty-state" role="status">
...
`;const actionsHtml=`
  <div class="csr-actions csr-actions--v3">
    <div class="csr-browse-tools" role="group" aria-label="Browse results">
      <details class="csr-filter-options" ${state.masteryOptionsOpen ? "open" : ""}>
        <summary>Filters <span data-sr-filter-status></span></summary>
    ...
    <details class="csr-revision-options" ${state.revisionOptionsOpen ? "open" : ""}>
      <summary>Revision <span class="csr-revision-mode" data-sr-revision-mode></span></summary>
      <p class="csr-selection-help">Select all includes every filtered page.</p>
    ...
    <div class="csr-select-actions">
      <button type="button" class="md-button" id="cr-all" aria-label="Select all">All</button>
      <button type="button" class="md-button" id="cr-none" aria-label="Select none">None</button>
    </div>
...
`;srUpdateResultSummary(container);srStableMeasure(container);const revisionOptions=container.querySelector('.csr-revision-options');if(revisionOptions)revisionOptions.addEventListener('toggle',()=>{if(revisionOptions.isConnected){state.revisionOptionsOpen=revisionOptions.open;srStableMeasure(container);}});const masteryOptions=container.querySelector('.csr-filter-options');if(masteryOptions)masteryOptions.addEventListener('toggle',()=>{if(masteryOptions.isConnected){state.masteryOptionsOpen=masteryOptions.open;srStableMeasure(container);}});const btnAll=container.querySelector("#cr-all");if(btnAll)btnAll.addEventListener("click",()=>{state.selectedMap=state.selectedMap||{};for(const x of srFilteredHits(state.hits))state.selectedMap[x.doc.location]=true;renderResults(container);});const btnNone=container.querySelector("#cr-none");if(btnNone)btnNone.addEventListener("click",()=>{state.selectedMap=state.selectedMap||{};for(const x of srFilteredHits(state.hits))state.selectedMap[x.doc.location]=false;renderResults(container);});const cbSelf=container.querySelector("#cr-selftest");const cbAi=container.querySelector("#cr-aitest");if(cbSelf)cbSelf.addEventListener("change",async()=>{if(cbSelf.checked){if(!(await srEnsureSelfTestUnlocked("find-self-test-toggle"))){cbSelf.checked=false;setRandomMode(cbAi&&cbAi.checked?"ai":"normal");srUpdateResultSummary(container);return;}
