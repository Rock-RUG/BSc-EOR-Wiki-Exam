...
function srStableMeasure(container){if(!container)return;try{const wrap=container.querySelector(".csr-wrap")||container;const options=container.querySelector(".csr-revision-options");const width=Math.round(wrap.getBoundingClientRect().width||window.innerWidth||0);const layoutKey=`${width}:${options && options.open ? "open" : "closed"}`;if(container.dataset.srStableLayoutKey!==layoutKey){container.dataset.srStableLayoutKey=layoutKey;delete container.dataset.srStableListMinH;delete container.dataset.srStableActionsMinH;wrap.style.removeProperty("--csr-stable-list-min-h");wrap.style.removeProperty("--csr-stable-actions-min-h");}
...
function srRenderResultSummaryHtml(){return`
    <div class="csr-result-summary">
      <h2 class="csr-result-title">Search results</h2>
...
function srUpdateResultSummary(container){if(!container)return;const data=srResultSummaryData();const write=(selector,value)=>{const node=container.querySelector(selector);if(node&&node.textContent!==value)node.textContent=value;return node;};write('[data-sr-applied-query]',data.applied);const status=write('[data-sr-draft-status]',data.dirty?"Query panel changed. Run search to update these results.":"");if(status)status.hidden=!data.dirty;write('[data-sr-match-count]',`${data.matched} matched · ${data.filtered} in the current filters`);write('[data-sr-selection-count]',`${data.selected} selected in filtered results · ${data.eligible} eligible for random revision`);write('[data-sr-revision-mode]',state.aiTestMode?"AI-test":state.selfTestMode?"Self-test":"Reading");const empty=container.querySelector('.csr-filter-empty');if(empty)empty.hidden=data.filtered!==0;}
...
`;const actionsHtml=`
  <div class="csr-actions csr-actions--v3">
    <div class="csr-browse-tools" role="group" aria-label="Browse results">
      <div class="csr-tools-title">Browse results</div>
    ...
    <div class="csr-revise-tools" role="group" aria-label="Select and revise">
      <div class="csr-tools-title">Select and revise</div>
    ...
    <details class="csr-revision-options" ${state.revisionOptionsOpen ? "open" : ""}>
      <summary>Revision options <span class="csr-revision-mode" data-sr-revision-mode></span></summary>
      <p class="csr-selection-help">Select all or none applies to every filtered page. Enrichment and retired pages stay readable but are excluded from random revision.</p>
