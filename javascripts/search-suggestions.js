...
function stripHtml(s){if(!s)return"";const div=document.createElement("div");div.innerHTML=String(s);return div.textContent||div.innerText||"";}
...
const isPageLevel=!String(locFull).includes("#");if(isPageLevel&&d.title)entry.title=String(d.title);else if(!entry.title&&d.title)entry.title=String(d.title);for(const tg of getTagsFromDoc(d)){entry.tags.add(tg);for(const v of buildAliasVariants(tg))entry.aliases.add(v);}
for(const al of getAliasesFromDoc(d)){const rawAlias=String(al||"").trim();if(rawAlias)entry.rawAliases.add(rawAlias);entry.aliases.add(al);for(const v of buildAliasVariants(al))entry.aliases.add(v);}
...
const scoreDocKeyword=createScoreFn();function getSortedHits(query,docs){const out=[];let i=0;for(const doc of docs||[]){const score=scoreDocKeyword(query,doc);if(!(score>0))continue;const cov=scoreDocKeyword.coverage?scoreDocKeyword.coverage(query,doc):0;out.push({doc,score,cov,__i:i+=1});}
