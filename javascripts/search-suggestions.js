...
scheduleUpdate(t);}},true);document.addEventListener("focusin",(e)=>{const t=e&&e.target;if(isHeaderSearchInput(t)){if(String(t.value||"").trim())forceRevealSearchOutput(t);scheduleUpdate(t);}},true);document.addEventListener("keydown",(e)=>{const t=e&&e.target;if(!isHeaderSearchInput(t))return;if(e.isComposing||e.keyCode===229||t.__mkSearchComposing)return;const key=String(e.key||"");const q=String(t.value||"").trim();if(key==="Escape"){clearCustomResults(t);return;}
