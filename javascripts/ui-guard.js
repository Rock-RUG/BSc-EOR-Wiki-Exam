...
window.addEventListener('pagehide',cancel);document.addEventListener('DOMContentSwitch',cancel);})();(function installHeaderSearchClose(){if(window.__mkHeaderSearchCloseBound)return;window.__mkHeaderSearchCloseBound=true;const style=document.createElement('style');style.textContent='.md-header .md-search__form > label.md-search__icon[for="__search"]{pointer-events:auto!important;z-index:2;cursor:pointer}';document.head.appendChild(style);window.addEventListener('click',event=>{const label=event.target?.closest?.('.md-header .md-search__form .md-search__icon[for="__search"]');const toggle=document.querySelector('input.md-toggle[data-md-toggle="search"], input#__search');if(!label||!toggle)return;if(!toggle.checked){requestAnimationFrame(()=>{if(toggle.checked)label.closest('.md-search')?.querySelector('input[data-md-component="search-query"]')?.focus({preventScroll:true});});return;}
event.preventDefault();event.stopImmediatePropagation();toggle.checked=false;const shell=label.closest('.md-search');const active=document.activeElement;try{if(shell?.contains(active))active.blur();}catch(_){}
try{shell?.querySelector('input[data-md-component="search-query"]')?.blur();}catch(_){}
window.__mkFindHeaderSearchGraceUntil=0;window.__mkSearchHistoryExplicitCloseUntil=Date.now()+640;for(const el of[shell,document.documentElement,document.body]){el?.classList?.remove('md-search--active','mk-hsf-open');}
toggle.dispatchEvent(new Event('change',{bubbles:true}));},true);})();(function(){const isMobile=(()=>{try{if(!window.matchMedia)return false;const coarse=window.matchMedia('(pointer: coarse)').matches;if(!coarse)return false;const isTablet=window.matchMedia('(min-width: 768px)').matches&&window.matchMedia('(min-height: 700px)').matches;return!isTablet;}catch(_){return false;}})();if(!isMobile)return;const FORCE_TOP_DELAYS=[0,50,140,320];function setHistoryScrollAuto(){try{if('scrollRestoration'in history)history.scrollRestoration='auto';}catch(_){}}
function isFreshDocumentNavigation(){try{const perf=window.performance;const entry=perf&&perf.getEntriesByType&&perf.getEntriesByType('navigation')[0];if(entry&&entry.type)return entry.type==='navigate';if(perf&&perf.navigation)return perf.navigation.type===0;}catch(_){}
return false;}
...
let __mkForceTopSeq=0;let __mkForceTopPath=String(window.location.pathname||'');let __mkForceTopInteracted=false;let __mkPreserveReadingPosition=!!window.location.hash||!isFreshDocumentNavigation();let __mkScrollPageAway=false;function preserveReadingPosition(){__mkForceTopPath=String(window.location.pathname||'');__mkPreserveReadingPosition=true;++__mkForceTopSeq;setHistoryScrollAuto();}
function canForceTop(){return!__mkScrollPageAway&&!__mkPreserveReadingPosition&&!__mkForceTopInteracted&&!window.location.hash&&__mkForceTopPath===String(window.location.pathname||'');}
...
['wheel','touchstart','pointerdown'].forEach((type)=>{window.addEventListener(type,cancelForceTopForInteraction,{passive:true,capture:true});});window.addEventListener('keydown',(event)=>{if(event&&/^(ArrowDown|ArrowUp|PageDown|PageUp|Home|End| )$/.test(String(event.key||''))){cancelForceTopForInteraction(event);}},true);window.addEventListener('pagehide',()=>{__mkScrollPageAway=true;preserveReadingPosition();},{passive:true});window.addEventListener('popstate',preserveReadingPosition,{passive:true});window.addEventListener('hashchange',preserveReadingPosition,{passive:true});setHistoryScrollAuto();function scheduleForceTop(delay,seq){if(!canForceTop())return;window.setTimeout(()=>{try{if(seq!==__mkForceTopSeq||!canForceTop())return;clearSavedScrollState();setScrollTopNow();requestAnimationFrame(()=>{try{if(seq!==__mkForceTopSeq||!canForceTop())return;setScrollTopNow();}catch(_){}});}catch(_){}},Math.max(0,Number(delay)||0));}
function forceTopBurst(delays){const path=String(window.location.pathname||'');if(path!==__mkForceTopPath){__mkForceTopPath=path;__mkForceTopInteracted=false;__mkPreserveReadingPosition=false;}
if(!canForceTop())return;const list=Array.isArray(delays)&&delays.length?delays:FORCE_TOP_DELAYS;clearSavedScrollState();const seq=++__mkForceTopSeq;list.forEach((ms)=>scheduleForceTop(ms,seq));}
...
const colour=mobileThemeColour();if(colour&&meta.getAttribute("content")!==colour)meta.setAttribute("content",colour);const rootStyle=document.documentElement.style;if(rootStyle.getPropertyValue("--mk-mobile-theme-color-active")!==colour){try{rootStyle.setProperty("--mk-mobile-theme-color-active",colour);}catch(_){}}}catch(_){}}
function burstSyncMobileThemeColor(){const seq=++__mkThemeColorBurstSeq;const run=()=>{if(seq!==__mkThemeColorBurstSeq)return;syncMobileThemeColor();};const inFrame=()=>{try{requestAnimationFrame(run);}catch(_){run();}};inFrame();[50,150,300,600].forEach((ms)=>window.setTimeout(inFrame,ms));}
...
function hardClearSearchActiveState(){try{__mkSearchBackdropLatch=false;clearSearchActiveGrace();const shell=getHeaderSearchShell();if(shell&&shell.classList)shell.classList.remove('md-search--active');if(shell&&shell.classList)shell.classList.remove('mk-hsf-open');if(shell&&shell.removeAttribute){try{shell.removeAttribute(FORCE_ACTIVE_ATTR);}catch(_){}}
const html=document.documentElement;const body=document.body;if(html&&html.classList)html.classList.remove('md-search--active');if(html&&html.classList)html.classList.remove('mk-hsf-open');if(body&&body.classList)body.classList.remove('md-search--active');}catch(_){}}
...
syncBackdrop();if(toggle.checked){focusSearchInputIfOpen(toggle);requestAnimationFrame(()=>{if(!toggle.checked)return;markSearchActiveGrace(1600);focusSearchInputIfOpen(toggle);scheduleBackdropMask();});[40,120,260,520].forEach((ms)=>setTimeout(()=>{if(!toggle.checked)return;markSearchActiveGrace(1200);syncBackdrop();focusSearchInputIfOpen(toggle);scheduleBackdropMask();},ms));}},{passive:true});};bindToggle();if(window.__mkBackdropSyncDocV4)return;window.__mkBackdropSyncDocV4=true;try{window.__mkUiGuardOwnsSearchScroll=true;}catch(_){}
document.addEventListener('pointerdown',(e)=>{try{if(!__mkSearchBackdropLatch&&!hasCheapSearchOpenSignal())return;const t=e&&e.target;if(!t||!t.closest)return;if(isHeaderSearchCloseTarget(t)&&getSearchToggle()?.checked)return;const inner=getHeaderSearchInner();if(!inner||!t.closest('.md-search__inner')||!inner.contains(t))return;if(isSearchDropdownUi(t))return;markSearchActiveGrace(960);focusSearchInputNow();scheduleBackdropMask();}catch(_){}},true);document.addEventListener('focusin',(e)=>{try{const t=e&&e.target;if(!t||!t.closest)return;if(!t.closest('.md-header .md-search'))return;markSearchActiveGrace(1400);suppressHeaderSearchClearTooltip();syncMobileHeaderSearchActions();syncBackdrop();scheduleBackdropMask();}catch(_){}},true);document.addEventListener('input',(e)=>{try{const t=e&&e.target;if(!t||!t.matches||!t.matches('input[data-md-component="search-query"]'))return;if(!isHeaderSearchShell(t.closest('.md-search')))return;if(!e.isTrusted&&!String(t.value||'').trim())return;markSearchActiveGrace(1600);suppressHeaderSearchClearTooltip();syncMobileHeaderSearchActions();syncBackdrop();scheduleBackdropMask();}catch(_){}},true);const mo=new MutationObserver(()=>{reassertSearchOpenClassIfLatched();syncBackdrop();});try{mo.observe(document.documentElement,{attributes:true,attributeFilter:['class']});}catch(_){}
...
function bfcacheHardReset(){try{setNoSearchSlide(false);forceCloseSearchUI();forceCloseDrawerUI();hardUnlockScroll();syncMobileHeaderTitleSwap();repairInlineMathPaint();syncBackdrop();}catch(_){}}
...
if(cand.kind==='label'){const forId=cand.forId||(cand.el.getAttribute&&cand.el.getAttribute('for'))||'';if(toggleInputById(forId)){if(String(forId)==='__search'){const toggle=getSearchToggle();if(toggle&&toggle.checked){focusSearchInputIfOpen(toggle);requestAnimationFrame(()=>{focusSearchInputIfOpen(toggle);scheduleBackdropMask();});}else{try{getHeaderSearchInput()?.blur();}catch(_){}
hardClearSearchActiveState();syncBackdrop();}}
...
document.addEventListener('DOMContentSwitch',()=>{init();forceTopBurst([0,70,180,360]);});document.addEventListener('navigation:load',()=>{init();forceTopBurst([0,70,180,360]);});window.addEventListener('load',()=>{forceTopBurst([0,90,220,420]);},{passive:true});window.addEventListener('pageshow',(e)=>{__mkScrollPageAway=false;if(e&&e.persisted){preserveReadingPosition();bfcacheHardReset();}
init();forceTopBurst([0,70,180,360]);},{passive:true});})();