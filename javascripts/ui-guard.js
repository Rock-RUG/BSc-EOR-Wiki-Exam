(function(){const isMobile=(()=>{try{if(!window.matchMedia)return false;const coarse=window.matchMedia('(pointer: coarse)').matches;if(!coarse)return false;const isTablet=window.matchMedia('(min-width: 768px)').matches&&window.matchMedia('(min-height: 700px)').matches;return!isTablet;}catch(_){return false;}})();if(!isMobile)return;const FORCE_TOP_DELAYS=[0,50,140,320];function setHistoryScrollManual(){try{if('scrollRestoration'in history)history.scrollRestoration='manual';}catch(_){}}
...
let __mkForceTopSeq=0;function scheduleForceTop(delay){const seq=++__mkForceTopSeq;window.setTimeout(()=>{try{if(seq!==__mkForceTopSeq)return;clearSavedScrollState();setHistoryScrollManual();setScrollTopNow();requestAnimationFrame(()=>{try{if(seq!==__mkForceTopSeq)return;setScrollTopNow();}catch(_){}});}catch(_){}},Math.max(0,Number(delay)||0));}
function forceTopBurst(delays){const list=Array.isArray(delays)&&delays.length?delays:FORCE_TOP_DELAYS;clearSavedScrollState();setHistoryScrollManual();list.forEach((ms)=>scheduleForceTop(ms));}
