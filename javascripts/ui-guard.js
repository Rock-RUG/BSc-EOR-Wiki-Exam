(function installTouchTopBoundary(){if(window.__mkTouchTopBoundaryV1)return;if(!window.matchMedia||!window.matchMedia('(pointer: coarse)').matches)return;window.__mkTouchTopBoundaryV1=true;let gesture=null;const nativeGesture='input, textarea, select, [contenteditable], canvas, #lp-map-modal, #lp-h1sg-modal, #lp-gps-modal';function reset(){gesture=null;}
document.addEventListener('touchstart',event=>{reset();if(!event.touches||event.touches.length!==1)return;const target=event.target;if(!target||!target.closest||target.closest(nativeGesture))return;const point=event.touches[0],scrollers=[];for(let el=target;el&&el!==document.body&&el!==document.documentElement;el=el.parentElement){if(el.scrollHeight>el.clientHeight&&/^(auto|scroll|overlay)$/.test(getComputedStyle(el).overflowY))scrollers.push(el);}
gesture={id:point.identifier,x:point.clientX,y:point.clientY,scrollers};},{passive:true,capture:true});document.addEventListener('touchmove',event=>{if(!gesture)return;if(!event.touches||event.touches.length!==1){reset();return;}
const point=event.touches[0];if(point.identifier!==gesture.id){reset();return;}
const dx=point.clientX-gesture.x,dy=point.clientY-gesture.y;gesture.x=point.clientX;gesture.y=point.clientY;if(dy<=0||dy<=Math.abs(dx))return;const scroller=document.scrollingElement||document.documentElement;if((window.scrollY||scroller.scrollTop||0)>0)return;if(gesture.scrollers.some(el=>el.isConnected&&el.scrollTop>0))return;if(event.cancelable)event.preventDefault();},{passive:false,capture:true});document.addEventListener('touchend',reset,{passive:true,capture:true});document.addEventListener('touchcancel',reset,{passive:true,capture:true});window.addEventListener('pagehide',reset,{passive:true});})();(function installMaterialClipboardGuard(){if(window.__mkMaterialClipboardGuardV1)return;window.__mkMaterialClipboardGuardV1=true;let requestId=0,feedbackTimer=0;const selector='[data-md-component="search-share"], .md-code__button[data-md-type="copy"]';function selectionState(){const s=window.getSelection&&window.getSelection();const active=document.activeElement;const selection=s?[s.anchorNode,s.anchorOffset,s.focusNode,s.focusOffset]:[];return selection.concat(active&&typeof active.selectionStart==='number'?[active.selectionStart,active.selectionEnd,active.selectionDirection]:[]);}
...
window.addEventListener('pagehide',cancel);document.addEventListener('DOMContentSwitch',cancel);})();(function(){const isMobile=(()=>{try{if(!window.matchMedia)return false;const coarse=window.matchMedia('(pointer: coarse)').matches;if(!coarse)return false;const isTablet=window.matchMedia('(min-width: 768px)').matches&&window.matchMedia('(min-height: 700px)').matches;return!isTablet;}catch(_){return false;}})();if(!isMobile)return;const FORCE_TOP_DELAYS=[0,50,140,320];function setHistoryScrollAuto(){try{if('scrollRestoration'in history)history.scrollRestoration='auto';}catch(_){}}
